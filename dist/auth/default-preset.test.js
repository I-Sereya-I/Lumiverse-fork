...
import { backfillDefaultPresets, BUILTIN_DEFAULT_PRESET_SEED_SETTING_KEY, BUILTIN_DEFAULT_PRESET_SLUG, seedDefaultPreset, } from "./default-preset";
import { SYSTEM_SECRET_PRINCIPAL, SYSTEM_SECRET_PRINCIPAL_EMAIL } from "../services/secrets.service";
function initDefaultPresetTestDb() {
    closeDatabase();
    initDatabase(":memory:");
    const db = getDb();
...
function insertUser(id, createdAt, username = id) {
    getDb().run(`INSERT INTO "user" (id, username, createdAt) VALUES (?, ?, ?)`, [id, username, createdAt]);
}
function insertPreset(input) {
    getDb().run(`INSERT INTO presets
...
function countPresets(userId) {
    const row = getDb()
        .query("SELECT COUNT(*) as count FROM presets WHERE user_id = ?")
        .get(userId);
    return row.count;
}
function findBuiltInPreset(userId) {
    return getDb()
        .query("SELECT id FROM presets WHERE user_id = ? AND json_extract(metadata, '$._lumiverse_preset_slug') = ? LIMIT 1")
...
describe("default preset seeding", () => {
    test("seeds the built-in Loom default and activates it for a new user", () => {
        ...
        const preset = getDb()
            .query(`SELECT id, name, provider, engine,
    ...
    test("startup backfill skips the reserved system principal", () => {
        ...
        getDb().run(`INSERT INTO "user" (id, username, email, createdAt) VALUES (?, 'System', ?, 0)`, [SYSTEM_SECRET_PRINCIPAL, SYSTEM_SECRET_PRINCIPAL_EMAIL]);
        insertUser("u1", 1);
