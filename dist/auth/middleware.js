...
export async function requireAuth(c, next) {
    ...
    if (!session) {
        ...
        if (result.lockout) {
            ...
            return c.json(authLockoutService.buildPayload(result.lockout, "Too many unauthorized requests. Try again later."), 429);
        }
 ...
 */
export async function requireOwnerStrict(c, next) {
    const session = c.get("session");
    if (!session) {
        return c.json({ error: "Unauthorized" }, 401);
    }
