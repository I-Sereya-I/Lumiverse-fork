...
import { ownerCredentialsExist, readOwnerCredentials, writeOwnerCredentials, } from "../crypto/credentials";
    ...
    if (!firstUserId) {
        try {
            ...
            const row = getDb()
                ...
                .get(SYSTEM_SECRET_PRINCIPAL_EMAIL);
 ...
 */
function seedOwnerDirectly(db, username, passwordHash) {
    ...
    db.transaction(() => {
        db.run(`INSERT INTO "user" (id, name, email, emailVerified, username, displayUsername, role, createdAt, updatedAt)
       VALUES (?, ?, ?, 1, ?, ?, 'owner', ?, ?)`, [userId, username, email, normalizedUsername, username, now, now]);
        db.run(`INSERT INTO "account" (id, accountId, providerId, userId, password, createdAt, updatedAt)
       VALUES (?, ?, 'credential', ?, ?, ?, ?)`, [accountId, userId, userId, passwordHash, now, now]);
    })();
    return userId;
}
/**
 ...
 */
function stripEnvKey(envPath, key) {
 ...
 * from .env.
 */
async function migrateOwnerPassword(credentialsPath) {
 ...
 */
function cleanupLegacyEnvSecrets() {
    const envPath = resolve(process.cwd(), ".env");
...
export async function seedOwner() {
    ...
    const userCount = db.query('SELECT COUNT(*) as count FROM "user"').get();
...
export function backfillUserIds() {
    const db = getDb();
    ...
    for (const table of CONTENT_TABLES) {
        try {
            ...
            if (!hasUserIdCol)
                continue;
            const result = db.run(`UPDATE ${table} SET user_id = ? WHERE user_id IS NULL`, [owner.id]);
            if (result.changes > 0) {
