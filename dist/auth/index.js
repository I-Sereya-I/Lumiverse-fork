...
const outstandingNonces = new Map(); // nonce → expiry
export const CREATION_NONCE_HEADER = "x-lumiverse-creation-nonce";
export function allowCreation() {
    ...
    if (outstandingNonces.size >= MAX_OUTSTANDING_NONCES) {
        ...
        if (oldest)
            outstandingNonces.delete(oldest[0]);
    }
    const nonce = crypto.randomUUID();
    outstandingNonces.set(nonce, now + CREATION_NONCE_TTL_MS);
    return nonce;
}
function consumeNonce(expectedNonce) {
    if (!expectedNonce)
        return false;
            ...
            create: {
                ...
                after: async (user) => {
                    ...
                    try {
                        ...
                        console.error(`[Auth] Failed to provision directories for user ${user.id}:`, err instanceof Error ? err.message : err);
                    }
                    try {
                        seedDefaultPreset(user.id, { setActive: true });
                    }
                    catch (err) {
                        console.error(`[Auth] Failed to seed default preset for user ${user.id}:`, err instanceof Error ? err.message : err);
                    }
                },
            },
        },
