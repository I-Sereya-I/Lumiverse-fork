...
const COMPRESSIBLE = /^(?:text\/(?!event-stream)|application\/(?:json|javascript|xml|x-javascript|ecmascript|graphql|wasm|xhtml\+xml|manifest\+json|ld\+json))/i;
...
function negotiate(header) {
    let best = null;
    let bestScore = -1;
    const PRIO = { br: 3, gzip: 2, deflate: 1 };
    for (const part of header.split(",")) {
        const [raw, ...params] = part.trim().split(";");
        const name = raw?.trim().toLowerCase();
        if (!name || !(name in PRIO))
            continue;
        let q = 1;
...
function appendVary(headers, field) {
    ...
    if (fields.includes("*") || fields.some((value) => value.toLowerCase() === field.toLowerCase()))
        return;
    headers.set("Vary", [...fields, field].join(", "));
}
/** Pipe a web ReadableStream through the chosen compressor. */
function compressStream(body, encoding) {
    if (encoding === "br") {
        const nodeIn = Readable.fromWeb(body);
        const compressed = nodeIn.pipe(createBrotliCompress({
