...
import { bunInstallCmd, declaredCapabilitiesFromManifest, detectDangerousBackendCapabilities, PRIVILEGED_PERMISSIONS, shouldUseWindowsSpindleBunSyncFallback, } from "./manager.service";
...
describe("detectDangerousBackendCapabilities", () => {
    ...
    test("hard-blocks dynamic module access even with every capability declared", () => {
        ...
        expect(detectDangerousBackendCapabilities(code, new Set(["dynamic_code_execution", "base64_decode"]))).toContain("dynamic module access");
    });
    ...
    test("does not flag methods named require/import (member calls + definitions)", () => {
        ...
        expect(detectDangerousBackendCapabilities('globalThis.require("node:fs");')).toContain("filesystem module access");
    });
    ...
    `;
        ...
        expect(detectDangerousBackendCapabilities(code, new Set(["dynamic_code_execution"]))).toEqual(["base64 decoding"]);
        expect(detectDangerousBackendCapabilities(code, new Set(["dynamic_code_execution", "base64_decode"]))).toEqual([]);
        ...
        expect(detectDangerousBackendCapabilities(unsafe, new Set(["dynamic_code_execution", "base64_decode"]))).toContain("filesystem module access");
    });
    test("declaredCapabilitiesFromManifest accepts only valid entries", () => {
        ...
        const valid = { ...base, requested_capabilities: ["dynamic_code_execution"] };
        expect(declaredCapabilitiesFromManifest(valid)).toEqual(new Set(["dynamic_code_execution"]));
        const mixed = {
            ...base,
            requested_capabilities: ["dynamic_code_execution", "bogus_value", "base64_decode"],
        };
        expect(declaredCapabilitiesFromManifest(mixed)).toEqual(new Set(["dynamic_code_execution", "base64_decode"]));
    });
...
describe("shouldUseWindowsSpindleBunSyncFallback", () => {
    ...
    test("allows an explicit Windows override", () => {
        expect(shouldUseWindowsSpindleBunSyncFallback("win32", {
