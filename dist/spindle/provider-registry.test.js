...
function key(scope, installationId, kind = "embedding", id = "foo") {
...
describe("provider registry identity", () => {
    ...
    test("rejects duplicate kind and id within one installation scope", () => {
        const registry = new ProviderRegistry();
        const host = { installationId: "inst-a", installScope: "user", authenticatedSubject: "alice" };
        registry.register({ kind: "tts", id: "voice" }, host);
        expect(() => registry.register({ kind: "tts", id: "voice" }, host)).toThrow(/already registered/);
    });
    ...
    test("isolates list and invoke across users and shared operator installations", async () => {
        const registry = new ProviderRegistry();
        registry.register({ kind: "sidecar", id: "tools" }, { installationId: "inst-user-a", installScope: "user", authenticatedSubject: "alice" });
        registry.register({ kind: "sidecar", id: "tools" }, { installationId: "inst-user-b", installScope: "user", authenticatedSubject: "bob" });
        registry.register({ kind: "sidecar", id: "tools" }, { installationId: "inst-op", installScope: "operator", authenticatedSubject: "op-1" });
        ...
        await expect(registry.invoke(key("user:bob", "inst-user-b", "sidecar", "tools"), {}, { callerScope: "user:alice" })).rejects.toThrow(/isolated/);
        await expect(registry.invoke(key("operator:op-1", "inst-op", "sidecar", "tools"), {}, { callerScope: "user:alice" })).rejects.toThrow(/isolated/);
    });
    test("requires an explicit callerScope and never defaults to the provider scope", async () => {
        ...
        registry.register({ kind: "embedding", id: "foo" }, { installationId: "inst-a", installScope: "user", authenticatedSubject: "alice" });
        await expect(registry.invoke(key("user:alice", "inst-a"))).rejects.toThrow(/callerScope/);
        await expect(registry.invoke(key("user:alice", "inst-a"), {}, {})).rejects.toThrow(/callerScope/);
        await expect(registry.invoke(key("user:alice", "inst-a"), {}, { callerScope: undefined })).rejects.toThrow(/callerScope/);
    });
});
