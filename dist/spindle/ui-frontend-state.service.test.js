...
import { clearUserExtensionSettingsTabs, getUserExtensionSettingsTabs, setUserExtensionSettingsTabs, } from "./ui-frontend-state.service";
...
function tab(registrationId, extensionId, tabId, overrides = {}) {
    return {
        registrationId,
        extensionId,
        tabId,
...
describe("backend settings-tab snapshot", () => {
    test("sanitizes snapshots and enforces four-per-extension and thirty-two-global caps", () => {
        const snapshot = [
            ...
            ...Array.from({ length: 5 }, (_, index) => tab(`same-${index}`, "extension.same", `same-${index}`)),
            ...Array.from({ length: 28 }, (_, index) => tab(`global-${index}`, `extension.global-${index}`, `global-${index}`)),
