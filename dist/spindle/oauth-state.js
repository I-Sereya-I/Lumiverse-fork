...
function evictOldestStateEntry() {
    const oldest = stateMap.keys().next();
    if (!oldest.done)
        stateMap.delete(oldest.value);
}
/** Create a new state nonce for the given extension. */
export function createOAuthState(extensionIdentifier) {
 ...
 */
export function validateOAuthState(state, extensionIdentifier) {
    ...
    if (Date.now() > entry.expiresAt)
        return false;
    if (entry.extensionIdentifier !== extensionIdentifier)
        return false;
    return true;
}
...
export function stopOAuthStateSweep() {
    if (_sweepTimer) {
