...
import { canExtensionMutateRegexScript, getEntityExtensionPermission, prepareSpindleRegexMutation, projectActivatedWorldInfoEntryForRpc, WorkerHostContentApi, } from "./worker-host-content-api";
...
describe("worker activated world-info projection", () => {
    test("projects every H13 provenance origin and maps peer books to persona", () => {
        const origins = ["constant", "sticky", "vector"];
...
describe("worker entity-extension RPC permissions", () => {
    ...
    test("routes all entity kinds through the owner-scoped namespace primitive", () => {
        ...
        for (const entity of ["world_book_entry", "character", "preset"]) {
