...
import { env } from "../env";
import { assertOwnedExtensionGitRoot, switchBranch, update, } from "./manager.service";
...
describe.serial("manager Git-root confinement", () => {
    test("rejects a repo with no .git before update or switch mutations", async () => {
        ...
        await expect(switchBranch("n10_missing_git", "main")).rejects.toThrow(/Git root confinement/);
    ...
    test("rejects an extension nested in an outer Git worktree before any mutator", async () => {
        ...
        await expect(switchBranch("n10_outer_repo", "main")).rejects.toThrow(/does not equal Git root/);
    ...
    test("rejects dev_mode branch switching before any Git operation", async () => {
        ...
        await expect(switchBranch("n10_dev_mode_switch", "main")).rejects.toThrow(/Cannot switch branches for dev_mode extension/);
    });
    ...
    test("accepts a standalone Git root and a canonical directory link to it", async () => {
        ...
        await expect(assertOwnedExtensionGitRoot(nestedAlias)).rejects.toThrow(/does not equal Git root/);
    });
    test.skipIf(process.platform !== "win32")("compares Windows Git roots case-insensitively after realpath resolution", async () => {
        const root = workspace();
        const repo = join(root, "CaseSensitiveAlias");
        initGitRoot(repo);
        expect(await assertOwnedExtensionGitRoot(repo.toUpperCase())).toBe(realpathSync.native(repo));
    });
});
