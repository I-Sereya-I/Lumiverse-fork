...
const FULL_THEME_SENTINEL_KEYS = ["--lumiverse-primary", "--lumiverse-bg", "--lumiverse-text", "--lumiverse-border", "--lumiverse-fill", "--lcs-glass-bg"];
...
export class WorkerHostPresentationApi {
    ...
    handleUIGetDrawerTabs(requestId, userId) {
        try {
    ...
    handleUIGetSettingsTabs(requestId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (resolvedUserId)
                this.enforceScopedUser(resolvedUserId);
    ...
    handleUINavigate(requestId, action, tabId, viewId, userId) {
        try {
            ...
            eventBus.emit(EventType.SPINDLE_UI_NAVIGATE, {
    ...
    // ─── Logging ─────────────────────────────────────────────────────────
    async handlePushSend(requestId, title, body, tag, url, userId, icon, rawTitle, image) {
        try {
    ...
    async handlePushGetStatus(requestId, userId) {
        try {
            if (!this.hasPermission("push_notification")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} push_notification — Push notification permission not granted`);
            }
    ...
    async handleWebSearchQuery(requestId, query, count, scrape, userId) {
        try {
    ...
    async handleWebSearchGetSettings(requestId, userId) {
        try {
            if (!this.hasPermission("web_search")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} web_search — Web search permission not granted`);
            }
    ...
    handleUserIsVisible(requestId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
    ...
    handleUserGetRole(requestId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            ...
            const result = row.role === "owner" ? "operator" : row.role === "admin" ? "admin" : "user";
            this.postToWorker({ type: "response", requestId, result });
        }
        catch (err) {
            this.postToWorker({ type: "response", requestId, error: err.message });
    ...
    handleTextEditorOpen(requestId, title, value, placeholder, userId, callerEditorRequestId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            ...
            const unsub = eventBus.on(EventType.SPINDLE_TEXT_EDITOR_RESULT, (msg) => {
                if (msg.payload?.requestId !== editorRequestId)
                    return;
                unsub();
                this.postToWorker({
                    type: "response",
            ...
            eventBus.emit(EventType.SPINDLE_TEXT_EDITOR_OPEN, {
    ...
    handleTextEditorClose(requestId, editorRequestId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
    ...
    // ─── Modal (free tier) ──────────────────────────────────────────────
    handleModalOpen(requestId, title, items, width, maxHeight, persistent, userId, callerModalRequestId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            ...
            const unsub = eventBus.on(EventType.SPINDLE_MODAL_RESULT, (msg) => {
                if (msg.payload?.requestId !== modalRequestId)
                    return;
                unsub();
                this.postToWorker({
                    type: "response",
            ...
            eventBus.emit(EventType.SPINDLE_MODAL_OPEN, {
    ...
    handleModalClose(requestId, openRequestId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            const modalRequestId = `spindle-modal:${this.extensionId}:${openRequestId}`;
            eventBus.emit(EventType.SPINDLE_MODAL_RESULT, { requestId: modalRequestId, dismissedBy: "extension" }, resolvedUserId);
            this.postToWorker({ type: "response", requestId, result: undefined });
        }
        catch (err) {
            this.postToWorker({ type: "response", requestId, error: err.message });
    ...
    handleConfirmOpen(requestId, title, message, variant, confirmLabel, cancelLabel, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            ...
            const unsub = eventBus.on(EventType.SPINDLE_CONFIRM_RESULT, (msg) => {
                if (msg.payload?.requestId !== confirmRequestId)
                    return;
                unsub();
                this.postToWorker({
                    type: "response",
            ...
            eventBus.emit(EventType.SPINDLE_CONFIRM_OPEN, {
    ...
    handleInputPromptOpen(requestId, title, message, placeholder, defaultValue, submitLabel, cancelLabel, multiline, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            ...
            const unsub = eventBus.on(EventType.SPINDLE_INPUT_PROMPT_RESULT, (msg) => {
                if (msg.payload?.requestId !== promptRequestId)
                    return;
                unsub();
                this.postToWorker({
                    type: "response",
            ...
            eventBus.emit(EventType.SPINDLE_INPUT_PROMPT_OPEN, {
    ...
    handleChatSetStyleMode(requestId, chatId, mode, userId) {
        ...
        try {
            ...
            if (mode === "bounded") {
                if (userMap) {
                    ...
                    if (userMap.size === 0)
                        this.chatStyleModes.delete(resolvedUserId);
                }
            }
            else {
                if (!userMap) {
            ...
            eventBus.emit(EventType.SPINDLE_CHAT_STYLE_MODE, {
                extensionId: this.extensionId,
                extensionName: this.manifest.name,
                chatId,
                mode,
    ...
    handleThemeApply(requestId, overrides, userId) {
        if (!this.hasPermission("app_manipulation")) {
            this.postToWorker({
                type: "response",
                requestId,
        ...
        try {
            ...
            if (overrides.variablesByMode) {
                for (const modeKey of ["dark", "light"]) {
    ...
    commitThemeOverrides(userId, overrides) {
        ...
        eventBus.emit(EventType.SPINDLE_THEME_OVERRIDES, {
            extensionId: this.extensionId,
            extensionName: this.manifest.name,
            overrides: nextOverrides,
        }, userId);
    }
    handleThemeApplyPalette(requestId, palette, userId) {
        if (!this.hasPermission("app_manipulation")) {
            this.postToWorker({
                type: "response",
                requestId,
    ...
    handleThemeClear(requestId, userId) {
        if (!this.hasPermission("app_manipulation")) {
            this.postToWorker({
                type: "response",
                requestId,
     ...
     */
    emitPaletteColorOverrides(accent, userId) {
        const strip = (vars) => {
            const out = {};
            for (const [k, v] of Object.entries(vars)) {
                if (!USER_PREFERENCE_KEYS.has(k))
                    out[k] = v;
            }
            return out;
        };
        ...
        for (const uid of connectedUserIds) {
            ...
            eventBus.emit(EventType.SPINDLE_THEME_OVERRIDES, { extensionId: this.extensionId, extensionName: this.manifest.name, overrides }, uid);
        }
    }
    handleThemeGetCurrent(requestId, userId) {
        if (!this.hasPermission("app_manipulation")) {
            this.postToWorker({
                type: "response",
                requestId,
    ...
    async handleColorExtract(requestId, imageId, userId) {
        if (!this.hasPermission("app_manipulation")) {
            this.postToWorker({
                type: "response",
                requestId,
    ...
    handleThemeGenerateVariables(requestId, config) {
        if (!this.hasPermission("app_manipulation")) {
            this.postToWorker({
                type: "response",
                requestId,
    ...
    handleCouncilGetSettings(requestId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
    ...
    handleCouncilGetMembers(requestId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            const settings = councilSettingsSvc.getCouncilSettings(resolvedUserId);
    ...
    handleCouncilGetAvailableLumiaItems(requestId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
    ...
    handleDlcGetCatalog(requestId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
