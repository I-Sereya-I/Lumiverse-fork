...
import { env } from "../env";
import { UPLOAD_READ_CHUNK_BYTES, appendUpload, createUpload, deleteUpload, getMaxUploadBytes, readUploadChunk, } from "./uploads";
...
describe("staged upload chunk reads", () => {
    ...
    test("does not append beyond the declared upload size", async () => {
        ...
        await expect(appendUpload(upload.uploadId, new Blob([new Uint8Array([1, 2])]).stream(), 0)).rejects.toThrow("upload exceeds declared size");
        deleteUpload(upload.uploadId);
    });
});
