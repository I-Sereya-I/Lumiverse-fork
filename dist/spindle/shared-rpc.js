...
export function isValidSharedRpcEndpoint(endpoint) {
...
export function assertValidSharedRpcEndpoint(endpoint) {
    const trimmed = String(endpoint || "").trim();
    if (!isValidSharedRpcEndpoint(trimmed)) {
        throw new Error(`Invalid shared RPC endpoint "${trimmed}". Expected "<extension_id>.<channel>" using lowercase letters, numbers, "_", and "-".`);
    }
    return trimmed;
}
export function normalizeOwnedSharedRpcEndpoint(extensionIdentifier, endpoint) {
