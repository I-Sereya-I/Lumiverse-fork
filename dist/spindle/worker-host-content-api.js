...
import { mapPeerBookSourceToPersona, projectActivationProvenance, } from "./activation-provenance";
import { getDb } from "../db/connection";
import { createHash } from "crypto";
import { mkdtempSync, rmSync } from "fs";
import { extname, join } from "path";
import { tmpdir } from "os";
export function getEntityExtensionPermission(entity) {
    switch (entity) {
...
export function canExtensionMutateRegexScript(script, extensionIdentifier, hasUnrestrictedAccess = false) {
    return hasUnrestrictedAccess || (script.owner_extension_identifier === extensionIdentifier
        && script.preset_id == null);
}
export function prepareSpindleRegexMutation(value, extensionIdentifier, allowUnownedMutation = false) {
...
export function projectActivatedWorldInfoEntryForRpc(entry) {
...
export class WorkerHostContentApi {
    ...
    handleCharactersList(requestId, limit, offset, userId) {
        try {
    ...
    handleCharactersGet(requestId, characterId, userId) {
        try {
            if (!this.hasPermission("characters")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} characters — Characters permission not granted`);
            }
    ...
    handleCharactersCreate(requestId, input, userId) {
        try {
            if (!this.hasPermission("characters")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} characters — Characters permission not granted`);
            }
            ...
            if (input.world_book_ids !== undefined || input.extensions !== undefined) {
                ...
                createInput.extensions = setCharacterWorldBookIds(input.extensions && typeof input.extensions === "object" && !Array.isArray(input.extensions)
                    ? input.extensions
                    : {}, ids);
            }
    ...
    handleCharactersSetAvatar(requestId, characterId, avatar, userId) {
        (async () => {
            try {
                if (!this.hasPermission("characters")) {
                    throw new Error(`${PERMISSION_DENIED_PREFIX} characters — Characters permission not granted`);
    ...
    handleCharactersUpdate(requestId, characterId, input, userId) {
        try {
            if (!this.hasPermission("characters")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} characters — Characters permission not granted`);
            }
    ...
    handleCharactersDelete(requestId, characterId, userId) {
        try {
            if (!this.hasPermission("characters")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} characters — Characters permission not granted`);
            }
    ...
    handleImagesList(requestId, limit, offset, specificity, onlyOwned, characterId, chatId, userId) {
        try {
    ...
    handleImagesGet(requestId, imageId, specificity, onlyOwned, characterId, chatId, userId) {
        try {
            if (!this.hasPermission("images")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} images — Images permission not granted`);
            }
    ...
    handleImagesUpload(requestId, input, userId) {
        (async () => {
            try {
                if (!this.hasPermission("images")) {
                    throw new Error(`${PERMISSION_DENIED_PREFIX} images — Images permission not granted`);
    ...
    handleImagesUploadMany(requestId, items, userId, concurrency) {
        (async () => {
            try {
                if (!this.hasPermission("images")) {
                    throw new Error(`${PERMISSION_DENIED_PREFIX} images — Images permission not granted`);
                ...
                const normalised = new Array(items.length);
                const failures = [];
    ...
    handleImagesUploadFromDataUrl(requestId, dataUrl, originalFilename, ownerCharacterId, ownerChatId, skipThumbnailProcessing, userId) {
        (async () => {
            try {
                if (!this.hasPermission("images")) {
                    throw new Error(`${PERMISSION_DENIED_PREFIX} images — Images permission not granted`);
    ...
    handleImagesDelete(requestId, imageId, userId) {
        try {
            if (!this.hasPermission("images")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} images — Images permission not granted`);
            }
    ...
    handleImagesDeleteMany(requestId, imageIds, userId) {
        void (async () => {
            try {
                if (!this.hasPermission("images")) {
                    throw new Error(`${PERMISSION_DENIED_PREFIX} images — Images permission not granted`);
    ...
    tempExtensionForMediaSource(filename, mimeType) {
    ...
    async resolveMediaSource(source, resolvedUserId) {
        ...
        switch (source.kind) {
            ...
            case "upload": {
                ...
                if (!uploadId)
                    throw new Error("upload media source requires upload_id");
            ...
            case "image": {
                ...
                if (!imageId)
                    throw new Error("image media source requires image_id");
                const image = imagesSvc.getImage(resolvedUserId, imageId);
                if (!image)
                    throw new Error("image media source not found");
                const path = await imagesSvc.getImageFilePath(resolvedUserId, imageId);
                if (!path)
                    throw new Error("image media source file not found");
                return {
                    path,
            ...
            case "audio": {
                ...
                if (!audioId)
                    throw new Error("audio media source requires audio_id");
                const audio = audioSvc.getAudio(resolvedUserId, audioId);
                if (!audio)
                    throw new Error("audio media source not found");
                const path = audioSvc.getAudioFilePath(resolvedUserId, audioId);
                if (!path)
                    throw new Error("audio media source file not found");
                return {
                    path,
    ...
    cleanupResolvedMediaSource(source) {
        try {
            source?.cleanup?.();
        }
        catch {
    ...
    handleMediaAudioConvert(requestId, input) {
        (async () => {
            let source = null;
            try {
                ...
                if (!resolvedUserId)
                    throw new Error("userId is required for operator-scoped extensions");
                this.enforceScopedUser(resolvedUserId);
                source = await this.resolveMediaSource(input?.source, resolvedUserId);
    ...
    handleMediaVideoConvert(requestId, input) {
        (async () => {
            let source = null;
            try {
                if (!this.hasPermission("media")) {
    ...
    handleMediaVideoTranscode(requestId, input) {
        (async () => {
            let source = null;
            try {
                if (!this.hasPermission("media")) {
    ...
    handleMediaVideoRemoveAudio(requestId, input) {
        (async () => {
            let source = null;
            try {
                if (!this.hasPermission("media")) {
    ...
    handleMediaVideoAddAudio(requestId, input) {
        (async () => {
            let video = null;
            let audio = null;
            try {
                ...
                if (!resolvedUserId)
                    throw new Error("userId is required for operator-scoped extensions");
                this.enforceScopedUser(resolvedUserId);
                video = await this.resolveMediaSource(input?.video, resolvedUserId);
                audio = await this.resolveMediaSource(input?.audio, resolvedUserId);
    ...
    handleMediaVideoFromImageAudio(requestId, input) {
        (async () => {
            let image = null;
            let audio = null;
            try {
                ...
                if (!resolvedUserId)
                    throw new Error("userId is required for operator-scoped extensions");
                this.enforceScopedUser(resolvedUserId);
                image = await this.resolveMediaSource(input?.image, resolvedUserId);
                audio = await this.resolveMediaSource(input?.audio, resolvedUserId);
    ...
    handleChatsList(requestId, characterId, limit, offset, userId) {
        try {
            ...
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            this.enforceScopedUser(resolvedUserId);
            const result = chatsSvc.listChats(resolvedUserId, { limit: Math.min(limit || 50, 200), offset: offset || 0 }, characterId);
            this.postToWorker({
                type: "response",
                requestId,
                result: {
    ...
    handleChatsGet(requestId, chatId, userId) {
        try {
            if (!this.hasPermission("chats")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chats — Chats permission not granted`);
            }
    ...
    handleChatsGetActive(requestId, userId) {
        try {
            if (!this.hasPermission("chats")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chats — Chats permission not granted`);
            }
    ...
    handleChatsUpdate(requestId, chatId, input, userId) {
        try {
            if (!this.hasPermission("chats")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chats — Chats permission not granted`);
            }
    ...
    handleChatsDelete(requestId, chatId, userId) {
        try {
            if (!this.hasPermission("chats")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chats — Chats permission not granted`);
            }
    ...
    handleWorldBooksList(requestId, limit, offset, userId) {
        try {
    ...
    handleWorldBooksGet(requestId, worldBookId, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
    ...
    handleWorldBooksCreate(requestId, input, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
    ...
    handleWorldBooksUpdate(requestId, worldBookId, input, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
    ...
    async handleWorldBooksDelete(requestId, worldBookId, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
    ...
    handleWorldBookEntriesList(requestId, worldBookId, limit, offset, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
    ...
    handleWorldBookEntriesGet(requestId, entryId, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
    ...
    handleWorldBookEntriesCreate(requestId, worldBookId, input, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
    ...
    handleWorldBookEntriesUpdate(requestId, entryId, input, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
            ...
            if (!entry)
                throw new Error("World book entry not found");
            this.postToWorker({ type: "response", requestId, result: this.toWorldBookEntryDTO(entry) });
        }
        catch (err) {
            this.postToWorker({ type: "response", requestId, error: err.message });
    ...
    async handleWorldBookEntriesDelete(requestId, entryId, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
    ...
    handleEntityExtensionSet(requestId, entity, entityId, namespace, value, userId) {
        try {
            ...
            const result = setEntityExtensionNamespace(resolvedUserId, entity, entityId, namespace, jsonValue);
            if (!result)
                throw new Error(`${entity} not found or not owned by the effective user`);
    ...
    handleWorldBooksEntrySetExtension(requestId, entryId, namespace, value, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
            ...
            if (typeof namespace !== "string")
                throw new Error("namespace is required");
            const jsonValue = value === null ? null : this.toBatchJson(value);
            const result = worldBooksSvc.setEntityExtensionNamespace(resolvedUserId, "world_book_entry", entryId, namespace, jsonValue);
            if (!result)
                throw new Error("World book entry not found");
            this.postToWorker({ type: "response", requestId, result });
        }
    ...
    asBatchRecord(value, label) {
        if (typeof value !== "object" || value === null || Array.isArray(value)) {
            throw new Error(`${label} must be an object`);
        }
        return value;
    }
    asBatchString(value, label) {
    ...
    toBatchJson(value) {
        ...
        if (typeof value === "number") {
            if (!Number.isFinite(value))
                throw new Error("batch result contains a non-finite number");
            return value;
        }
        if (Array.isArray(value))
            return value.map((item) => this.toBatchJson(item));
        if (typeof value === "object") {
            const result = {};
            for (const [key, item] of Object.entries(value)) {
                if (item !== undefined)
                    result[key] = this.toBatchJson(item);
            }
            return result;
        }
        throw new Error("batch result is not JSON-serializable");
    }
    executeSpindleBatchOperation(userId, operation, expectedRevisions) {
        ...
        if (operation.domain === "world_books" && operation.op === "entries.update") {
            if (!this.hasPermission("world_books"))
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books`);
            ...
            if (expected !== undefined && input.expected_revision === undefined)
                input.expected_revision = expected;
            const entry = worldBooksSvc.updateEntry(userId, entryId, input);
            if (!entry)
                throw new Error("World book entry not found");
        ...
        if (operation.domain === "world_books" && operation.op === "entry.setExtension") {
            ...
            const result = worldBooksSvc.setEntityExtensionNamespace(userId, "world_book_entry", entryId, namespace, args.value === undefined ? null : args.value);
            if (!result)
                throw new Error("World book entry not found");
        ...
        if (operation.domain === "entity_extensions" && operation.op === "setNamespace") {
            ...
            const namespace = this.asBatchString(args.namespace, "namespace");
            const result = worldBooksSvc.setEntityExtensionNamespace(userId, entity, entityId, namespace, args.value === undefined ? null : args.value);
            if (!result)
                throw new Error(`${entity} not found or not owned by the effective user`);
            return this.toBatchJson(result);
        }
        if (operation.domain === "characters" && operation.op === "update") {
            if (!this.hasPermission("characters"))
                throw new Error(`${PERMISSION_DENIED_PREFIX} characters`);
            const characterId = this.asBatchString(id, "characterId");
            const character = charactersSvc.updateCharacter(userId, characterId, this.asBatchRecord(args.input ?? {}, "input"));
            if (!character)
                throw new Error("Character not found");
        ...
        if (operation.domain === "presets" && operation.op === "update") {
            if (!this.hasPermission("presets"))
                throw new Error(`${PERMISSION_DENIED_PREFIX} presets`);
            const presetId = this.asBatchString(id, "presetId");
            const preset = presetsSvc.updatePreset(userId, presetId, this.asBatchRecord(args.input ?? {}, "input"));
            if (!preset)
        ...
        if (operation.domain === "personas" && operation.op === "update") {
            if (!this.hasPermission("personas"))
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas`);
            const personaId = this.asBatchString(id, "personaId");
            const persona = personasSvc.updatePersona(userId, personaId, this.asBatchRecord(args.input ?? {}, "input"));
            if (!persona)
        ...
        if (operation.domain === "regex_scripts" && operation.op === "update") {
            if (!this.hasPermission("regex_scripts"))
                throw new Error(`${PERMISSION_DENIED_PREFIX} regex_scripts`);
            const scriptId = this.asBatchString(id, "scriptId");
            const mutation = prepareSpindleRegexMutation(this.asBatchRecord(args.input ?? {}, "input"), this.manifest.identifier, this.hasUnrestrictedRegexAccess());
            const script = regexScriptsSvc.updateRegexScript(userId, scriptId, mutation.input, mutation.context);
            if (script === null)
                throw new Error("Regex script not found");
    ...
    handleSpindleBatch(requestId, operations, options, userId) {
        try {
            ...
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            this.enforceScopedUser(resolvedUserId);
            const batch = eventBus.withBufferedEvents(() => getDb().transaction(() => {
                const completed = [];
                try {
    ...
    handleDatabanksList(requestId, limit, offset, scope, scopeId, userId) {
        try {
            ...
            const result = databanksSvc.listDatabanks(resolvedUserId, {
                limit: Math.min(limit || 50, 200),
                offset: offset || 0,
            }, {
    ...
    handleDatabanksGet(requestId, databankId, userId) {
        try {
            if (!this.hasPermission("databanks")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
            }
    ...
    handleDatabanksCreate(requestId, input, userId) {
        try {
            if (!this.hasPermission("databanks")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
            }
    ...
    handleDatabanksUpdate(requestId, databankId, input, userId) {
        try {
            if (!this.hasPermission("databanks")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
            }
            ...
            if (!bank)
                throw new Error("Databank not found");
            this.postToWorker({ type: "response", requestId, result: this.toDatabankDTO(bank) });
        }
        catch (err) {
            this.postToWorker({ type: "response", requestId, error: err.message });
    ...
    handleDatabanksDelete(requestId, databankId, userId) {
        (async () => {
            try {
                if (!this.hasPermission("databanks")) {
                    throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
    ...
    handleDatabankDocumentsList(requestId, databankId, limit, offset, userId) {
        try {
            if (!this.hasPermission("databanks")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
            }
    ...
    handleDatabankDocumentsGet(requestId, documentId, userId) {
        try {
            if (!this.hasPermission("databanks")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
            }
    ...
    handleDatabankDocumentsCreate(requestId, databankId, input, userId) {
        (async () => {
            try {
                if (!this.hasPermission("databanks")) {
                    throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
                ...
                const displayName = typeof input.name === "string" && input.name.trim()
                    ? input.name.trim()
                    : filename.replace(/\.[^.]+$/, "");
                const doc = databanksSvc.createDocument(resolvedUserId, databankId, displayName, storedFilename, mimeType, bytes.byteLength, hash);
    ...
    handleDatabankDocumentsUpdate(requestId, documentId, input, userId) {
        try {
            if (!this.hasPermission("databanks")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
            }
    ...
    handleDatabankDocumentsDelete(requestId, documentId, userId) {
        (async () => {
            try {
                if (!this.hasPermission("databanks")) {
                    throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
    ...
    handleDatabankDocumentsGetContent(requestId, documentId, userId) {
        try {
            if (!this.hasPermission("databanks")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
            }
    ...
    handleDatabankDocumentsReprocess(requestId, documentId, userId) {
        (async () => {
            try {
                if (!this.hasPermission("databanks")) {
                    throw new Error(`${PERMISSION_DENIED_PREFIX} databanks — Databanks permission not granted`);
    ...
    handlePersonasList(requestId, limit, offset, userId) {
        try {
            if (!this.hasPermission("personas")) {
    ...
    handlePersonasGet(requestId, personaId, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handlePersonasGetDefault(requestId, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handlePersonasGetActive(requestId, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handlePersonasCreate(requestId, input, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handlePersonasUpdate(requestId, personaId, input, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handlePersonasDelete(requestId, personaId, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handlePersonasSwitch(requestId, personaId, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handlePersonasGetWorldBook(requestId, personaId, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handleGlobalAddonsList(requestId, limit, offset, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handleGlobalAddonsGet(requestId, addonId, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
    ...
    handleGlobalAddonsUpdate(requestId, addonId, input, userId) {
        try {
            if (!this.hasPermission("personas")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} personas — Personas permission not granted`);
            }
            ...
            if (!addon)
                throw new Error("Global add-on not found");
    ...
    async handleWorldBooksGetActivated(requestId, chatId, userId) {
        try {
            if (!this.hasPermission("world_books")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} world_books — World Books permission not granted`);
            }
            ...
            const result = activated.map((e) => projectActivatedWorldInfoEntryForRpc({
                id: e.id,
                comment: e.comment,
                keys: e.keys,
                source: e.source,
                ...
                firstTriggeredForBook: e.firstTriggeredForBook,
            }));
            this.postToWorker({ type: "response", requestId, result });
        }
        catch (err) {
    ...
    handleWorldBooksGetGlobal(requestId, userId) {
        try {
    ...
    handleWorldBooksSetGlobal(requestId, worldBookIds, userId) {
        try {
            const resolvedUserId = this.requireWorldBooksUser(userId);
            ...
            const ids = this.sanitizeWorldBookIds(worldBookIds).filter((id) => worldBooksSvc.getWorldBook(resolvedUserId, id));
    ...
    handleWorldBooksActivateGlobal(requestId, worldBookId, userId) {
        try {
            const resolvedUserId = this.requireWorldBooksUser(userId);
    ...
    handleWorldBooksDeactivateGlobal(requestId, worldBookId, userId) {
        try {
            const resolvedUserId = this.requireWorldBooksUser(userId);
            if (typeof worldBookId !== "string" || !worldBookId.trim()) {
                throw new Error("worldBookId is required");
    ...
    toRegexScriptDTO(s) {
        return {
            id: s.id,
            can_mutate: canExtensionMutateRegexScript(s, this.manifest.identifier, this.hasUnrestrictedRegexAccess()),
            name: s.name,
    ...
    handleRegexScriptsList(requestId, scope, scopeId, target, limit, offset, userId) {
        try {
            if (!this.hasPermission("regex_scripts")) {
            ...
            const result = regexScriptsSvc.listRegexScripts(resolvedUserId, { limit: Math.min(limit || 50, 200), offset: offset || 0 }, filters);
            this.postToWorker({
                type: "response",
                requestId,
                result: {
    ...
    handleRegexScriptsGet(requestId, scriptId, userId) {
        try {
            if (!this.hasPermission("regex_scripts")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} regex_scripts — Regex Scripts permission not granted`);
            }
    ...
    handleRegexScriptsGetActive(requestId, target, characterId, chatId, userId) {
        try {
            if (!this.hasPermission("regex_scripts")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} regex_scripts — Regex Scripts permission not granted`);
            }
    ...
    handleRegexScriptsCreate(requestId, input, userId) {
        try {
            if (!this.hasPermission("regex_scripts")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} regex_scripts — Regex Scripts permission not granted`);
            }
            ...
            const result = regexScriptsSvc.createRegexScript(resolvedUserId, mutation.input, mutation.context);
    ...
    handleRegexScriptsUpdate(requestId, scriptId, input, userId) {
        try {
            if (!this.hasPermission("regex_scripts")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} regex_scripts — Regex Scripts permission not granted`);
            }
            ...
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            this.enforceScopedUser(resolvedUserId);
            const mutation = prepareSpindleRegexMutation(input, this.manifest.identifier, this.hasUnrestrictedRegexAccess());
            const result = regexScriptsSvc.updateRegexScript(resolvedUserId, scriptId, mutation.input, mutation.context);
            if (result === null)
                throw new Error("Regex script not found");
            if (typeof result === "string")
                throw new Error(result);
    ...
    handleRegexScriptsDelete(requestId, scriptId, userId) {
        try {
            if (!this.hasPermission("regex_scripts")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} regex_scripts — Regex Scripts permission not granted`);
            }
