...
function seedExtension(identifier, scope, userId, permissions = []) {
    const db = getDb();
    db.run(`INSERT INTO extensions (id, identifier, name, version, author, github, install_scope, installed_by_user_id)
     VALUES (?, ?, ?, '1.0.0', 'test', 'https://example.test/ext', ?, ?)`, [identifier, identifier, identifier, scope, userId]);
    for (const grant of permissions) {
        db.run(`INSERT INTO extension_grants (id, extension_id, permission, scope)
       VALUES (?, (SELECT id FROM extensions WHERE identifier = ?), ?, ?)`, [crypto.randomUUID(), identifier, grant.permission, grant.scope]);
    }
}
...
function findDenied(posted, permission, operation) {
    return posted.find((message) => message.type ===
        "permission_denied" &&
...
describe("worker-host provider register permission enforcement", () => {
    ...
    test("grants register into the worker's own scope while an ungranted operator scope stays isolated", () => {
        ...
        expect(findDenied(operatorPosted, "providers.embedding.register", "provider_register")).toBeDefined();
    ...
    test("invalid or missing provider kind gets a clean invalid-request denial, not a malformed permission string", () => {
        ...
        handle(host, { type: "provider_register", phase: "register", kind: "bogus", id: "foo" });
        ...
        const denials = posted.filter((message) => message.type === "permission_denied");
