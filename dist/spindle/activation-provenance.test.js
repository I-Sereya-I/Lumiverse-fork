...
import { ACTIVATION_PROVENANCE_FORBIDDEN_KEYS, mapPeerBookSourceToPersona, projectActivationProvenance, projectActivationTraceEntry, } from "./activation-provenance";
function expectNoForbiddenKeys(value) {
    if (Array.isArray(value)) {
        for (const item of value)
            expectNoForbiddenKeys(item);
        return;
    }
...
describe("projectActivationProvenance", () => {
    ...
    test("projects all exact-match source variants and strips nested unknown keys", () => {
        ...
        expect(projectActivationProvenance({
            origin: "keyword",
            activationPass: 1,
            matchedPrimaryKeys: [],
    ...
    test("deeply clones arrays and nested locator objects", () => {
        ...
        expect(projected && "exactMatch" in projected ? projected.exactMatch : undefined).not.toBe(input.exactMatch);
