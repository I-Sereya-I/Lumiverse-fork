...
export async function updateAllExtensions(opts) {
    ...
    const all = await managerSvc.listForUser(opts.userId, opts.isPrivileged ? "owner" : "user");
    const targets = all.filter((ext) => managerSvc.canManageExtension(ext, opts.userId, opts.isPrivileged ? "owner" : "user"));
    ...
    void runBulkUpdate(targets, opts.userId)
        ...
        eventBus.emit(EventType.SPINDLE_BULK_UPDATE_COMPLETE, {
...
export function isBulkUpdateInProgress() {
    return bulkUpdateInProgress;
}
async function runBulkUpdate(targets, userId) {
    const errors = [];
    const plan = targets.map((ext) => ({
        ext,
    ...
    eventBus.emit(EventType.SPINDLE_BULK_UPDATE_PROGRESS, {
        total: plan.length,
        completed: 0,
        failed: 0,
        phase: "starting",
    ...
    for (const entry of plan) {
        ...
        try {
            ...
            console.error(`[Spindle] Bulk update: failed to stop ${entry.ext.identifier}:`, err);
    ...
    for (const entry of plan) {
        ...
        try {
            ...
            console.error(`[Spindle] Bulk update: failed to restart ${entry.ext.identifier}:`, restartErr);
