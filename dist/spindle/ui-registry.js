 ...
 */
import { getUserExtensionSettingsTabs } from './ui-frontend-state.service';
export const BUILT_IN_DRAWER_TABS = [
...
export const BUILT_IN_SETTINGS_TABS = [
...
function mergeKeywords(base, additions) {
    const keywords = new Set(base);
    for (const keyword of additions)
        keywords.add(keyword);
    return [...keywords];
}
function extensionSettingsGroups(tabs) {
...
function dynamicSettingsTabOwner(registrations) {
...
export function getVisibleSettingsTabs(userRole, userId) {
    ...
    const result = visibleCore.map((tab) => {
