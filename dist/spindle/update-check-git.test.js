...
import { isOwnGitRepositoryPath, isOwnGitRepositoryPathAsync, resetGitRepositoryToRemoteHead, } from "./git-repository";
...
async function commit(cwd, message) {
    ...
    await runGit(cwd, "-c", "user.name=Lumiverse Test", "-c", "user.email=lumiverse@example.invalid", "commit", "-m", message);
}
...
describe("extension update Git probe", () => {
    test("reports current and then detects a changed remote HEAD", async () => {
        ...
        expect(await runGit(clone, "rev-parse", "HEAD")).toBe(await runGit(source, "rev-parse", "HEAD"));
        expect(readFileSync(join(clone, "spindle.json"), "utf8")).toBe('{"version":"1.1.0"}');
