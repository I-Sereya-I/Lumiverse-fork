...
beforeAll(async () => {
    ...
    getDb().run(`INSERT OR IGNORE INTO "user" (id, name, email) VALUES
...
function seedRegisterGrant(identifier, scope, userId, kinds) {
    const db = getDb();
    db.run(`INSERT OR IGNORE INTO extensions (id, identifier, name, version, author, github, install_scope, installed_by_user_id)
     VALUES (?, ?, ?, '1.0.0', 'test', 'https://example.test/ext', ?, ?)`, [identifier, identifier, identifier, scope, userId]);
    const effectiveScope = userId ? `${scope}:${userId}` : scope;
    for (const kind of kinds) {
        db.run(`INSERT OR IGNORE INTO extension_grants (id, extension_id, permission, scope)
       VALUES (?, (SELECT id FROM extensions WHERE identifier = ?), ?, ?)`, [crypto.randomUUID(), identifier, `providers.${kind}.register`, effectiveScope]);
    }
}
const ALL_KINDS = ["embedding", "tts", "stt", "sidecar"];
afterEach(() => {
...
function extensionInfo(id, scope, userId) {
    return {
        id,
        identifier: id,
        name: id,
...
describe("worker-host provider RPC", () => {
    ...
    test("isolates list and invoke across users and shared operator installations", async () => {
        ...
        await expect(providerRegistry.invoke({ effectiveScope: "operator:op-1", installationId: "inst-op", kind: "tts", id: "voice" }, {}, { callerScope: "user:alice" })).rejects.toThrow(/isolated/);
    });
    test("aborts timed-out invocations and suppresses late provider_result", async () => {
        ...
        expect(posted.some((message) => message.type === "provider_abort")).toBe(true);
    });
    test("authenticated embedding/TTS/STT/sidecar two-stage broker redacts worker envelopes", async () => {
        ...
        for (const kind of ["embedding", "tts", "stt", "sidecar"]) {
            ...
            expect(prepared.workerView.headers?.Authorization).toBeUndefined();
