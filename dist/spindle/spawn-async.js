 ...
 */
function startStreamDrain(stream) {
    if (!stream) {
        return {
...
export async function spawnAsync(cmd, opts = {}) {
    ...
    try {
        ...
        const stdout = opts.ignoreStdout
            ...
            : startStreamDrain(proc.stdout);
        const stderr = startStreamDrain(proc.stderr);
        ...
        const stderr = hasTimeout && proc.killed
