...
import { macroInterceptorChain, } from "./macro-interceptor";
...
import { setPromptRegexOwnedChats, clearPromptRegexOwner, } from "./prompt-regex-ownership";
...
import { assembleSpindleBlocks } from "./assembly";
...
import { providerRegistry, PROVIDER_BROKER_KINDS, } from "./provider-registry";
import { getSecret } from "../services/secrets.service";
import { getApprovedBrokerOrigins } from "../services/broker-origins.service";
import { readSharedRpcEndpoint, registerSharedRpcRequestEndpoint, syncSharedRpcEndpoint, unregisterSharedRpcEndpoint, unregisterSharedRpcEndpointsByOwner, } from "./shared-rpc-pool.service";
import { getTextContent } from "../llm/types";
import { clearFrontendRuntimeCapabilities, isFrontendRuntimeCapability, registerFrontendRuntimeCapability, unregisterFrontendRuntimeCapability, } from "./frontend-runtime-capabilities";
import { getDb } from "../db/connection";
import { getMessages as getChatMessages, createMessage as createChatMessage, updateMessage as updateChatMessage, deleteMessage as deleteChatMessage, getMessage as getChatMessage, } from "../services/chats.service";
import { existsSync, readFileSync, writeFileSync, mkdirSync, } from "fs";
import { AsyncLocalStorage } from "node:async_hooks";
import { join, resolve } from "path";
const sharedRpcPermissionScope = new AsyncLocalStorage();
let cachedBackendVersion = null;
let cachedFrontendVersion = null;
...
async function readResponseBodyCapped(response, maxBytes) {
    ...
    while (true) {
        ...
        if (total > maxBytes) {
            ...
            throw new Error(`CORS proxy response exceeded ${maxBytes} bytes`);
        }
        chunks.push(value);
    }
 ...
 */
async function readResponseBodyBinaryCapped(response, maxBytes) {
    if (!response.body)
 ...
 */
function validateImageMagicBytes(data, contentType) {
    if (data.length < 2)
        return false;
...
function validateAudioMagicBytes(data, contentType) {
    if (data.length < 4)
        return false;
    ...
    if (data[0] === 0x1A && data[1] === 0x45 && data[2] === 0xDF && data[3] === 0xA3)
        return true;
    // MIDI.
    if (contentType.includes("midi") && data[0] === 0x4D && data[1] === 0x54 && data[2] === 0x68 && data[3] === 0x64)
        return true;
    return false;
}
/** Validate common web font container formats before proxying to widgets. */
function validateFontMagicBytes(data, _contentType) {
    if (data.length < 4)
        return false;
    // WOFF: "wOFF"
...
const THINKING_DISPLAY_VALUES = new Set([
    "auto",
    "summarized",
    "omitted",
]);
function coerceReasoningSettings(raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw))
        return null;
    const r = raw;
    const effort = REASONING_EFFORT_VALUES.has(r.reasoningEffort)
        ? r.reasoningEffort
        : "auto";
    const display = THINKING_DISPLAY_VALUES.has(r.thinkingDisplay)
        ? r.thinkingDisplay
        : "auto";
    return {
 ...
 */
function extractReasoningBindingsDTO(metadata) {
...
export class WorkerHost {
    ...
    interceptorUnregister = null;
    interceptorRegistrationId = null;
    activeInterceptorContexts = new Map();
    contextHandlerUnregister = null;
    messageContentProcessorUnregister = null;
    macroInterceptorUnregister = null;
    worldInfoInterceptorUnregister = null;
    registeredMacroNames = new Set();
    macroValueCache = new Map();
    static MAX_BACKEND_PROCESSES = 16;
    static SHARED_RPC_REQUEST_TIMEOUT_MS = 10_000;
    onWorkerReady = null;
    onWorkerShutdownAck = null;
    onRuntimeExit = null;
    runtimeExitPromise = null;
    runtimeStopping = false;
    runtimeStatsInterval = null;
    installScope;
    installedByUserId;
    storageApi;
    stateApi;
    ...
    interactionApi;
    presentationApi;
    sharedRpcPermissionScopes = new Map();
    constructor(extensionId, manifest, extensionInfo) {
        this.extensionId = extensionId;
        this.manifest = manifest;
        const metadata = (extensionInfo.metadata || {});
    ...
    hasPermission(permission) {
        const scopeId = sharedRpcPermissionScope.getStore();
        if (!scopeId)
            return managerSvc.hasPermission(this.manifest.identifier, permission);
        const scoped = this.sharedRpcPermissionScopes.get(scopeId);
        return Boolean(scoped?.has(permission)) && managerSvc.hasPermission(this.manifest.identifier, permission);
    }
    getStorageRootPath(identifier = this.manifest.identifier) {
    ...
    getRuntimeSampleIntervalMs() {
        if (!this.isRuntimeStatsEnabled())
            return 0;
    ...
    async emitRuntimeStats(phase, startupMs) {
        if (!this.isRuntimeStatsEnabled())
            return;
    ...
    startRuntimeStatsSampling() {
        if (!this.runtime || this.runtime.mode === "worker")
            return;
    ...
    async start() {
        ...
        if (!entryPath) {
            console.log(`[Spindle:${this.manifest.identifier}] No backend entry, skipping worker`);
            return;
        }
        ...
        this.runtime = createRuntimeTransport({
            ...
            onError: (message) => {
                console.error(`[Spindle:${this.manifest.identifier}] Worker error:`, message);
                ...
                try {
                    ...
                    console.warn(`[Spindle:${this.manifest.identifier}] Worker appears dead after error, cleaning up registrations`);
                    this.cleanup();
                }
            },
        ...
        const readyPromise = new Promise((resolve) => {
            const readyTimeout = setTimeout(() => {
                console.warn(`[Spindle:${this.manifest.identifier}] Worker ready timeout (10s) — proceeding`);
                resolve();
            }, 10_000);
            this.onWorkerReady = () => {
    ...
    handleRuntimeTransportFailure(error) {
        // Already torn down by an earlier failure on this stack, bail before recursing.
        if (!this.runtime)
            return;
        const message = error instanceof Error ? error.message : String(error);
        console.warn(`[Spindle:${this.manifest.identifier}] Runtime transport failed, cleaning up: ${message}`);
        this.runtime = null;
        this.cleanup();
    }
    postToWorker(msg) {
     ...
     * no restart needed.
     */
    notifyPermissionChanged(permission, granted, allGranted) {
        this.postToWorker({
     ...
     */
    invokeExtensionTool(toolName, args, timeoutMs = 30_000, councilMember, contextMessages) {
        ...
        for (const [key, value] of Object.entries(args)) {
            if (key === "__userId" || key === "__user_id" || key === "userId")
                continue;
            sanitizedArgs[key] = value;
        }
        const contextMessagesDTO = contextMessages?.map((m) => ({
            role: m.role,
    ...
    requestSharedRpcValue(endpoint, requesterExtensionId, effectivePermissions) {
        const requestId = crypto.randomUUID();
    ...
    handleRpcPoolSync(endpoint, value, policy) {
        syncSharedRpcEndpoint(this.manifest.identifier, endpoint, value, policy);
    }
    handleRpcPoolRegisterHandler(endpoint, policy) {
        registerSharedRpcRequestEndpoint(this.manifest.identifier, endpoint, async (requesterExtensionId, effectivePermissions) => await this.requestSharedRpcValue(endpoint, requesterExtensionId, effectivePermissions), policy);
    }
    handleRpcPoolUnregister(endpoint) {
        unregisterSharedRpcEndpoint(this.manifest.identifier, endpoint);
    }
    async handleRpcPoolRead(requestId, endpoint) {
        try {
            const result = await readSharedRpcEndpoint(endpoint, this.manifest.identifier, managerSvc.getGrantedPermissions);
    ...
    handleCreateOAuthState(requestId) {
    ...
    handleMessageInScope(msg) {
        if (this.storageApi.dispatch(msg)) {
            return;
        }
        if (this.stateApi.dispatch(msg)) {
            return;
        }
        switch (msg.type) {
            ...
            case "intercept_result": {
                ...
                if (interceptParams && Object.keys(interceptParams).length > 0) {
                    if (!this.hasPermission("generation_parameters")) {
                        console.warn(`[Spindle:${this.manifest.identifier}] Stripping interceptor parameters — generation_parameters permission not granted`);
            ...
            case "permissions_get_granted":
                ...
                break;
            case "rpc_pool_sync":
                this.handleRpcPoolSync(msg.endpoint, msg.value, msg.policy);
                break;
            case "rpc_pool_register_handler":
                this.handleRpcPoolRegisterHandler(msg.endpoint, msg.policy);
                break;
            ...
            case "chat_append_message":
                void this.handleChatAppendMessage(msg.requestId, msg.chatId, msg.message, msg.options);
                break;
            case "chat_update_message":
                this.handleChatUpdateMessage(msg.requestId, msg.chatId, msg.messageId, msg.patch);
                break;
            ...
            case "register_context_handler":
                this.handleRegisterContextHandler(msg.priority, msg.timeoutMs);
                break;
            ...
            case "frontend_message": {
                ...
                const targetUserId = this.installScope === "user"
                    ? this.installedByUserId ?? undefined
                    : typeof msg.userId === "string" && msg.userId.length > 0
                        ? msg.userId
                        : undefined;
                eventBus.emit(EventType.SPINDLE_FRONTEND_MSG, {
                    extensionId: this.extensionId,
                    identifier: this.manifest.identifier,
                    data: msg.payload,
                }, targetUserId);
            ...
            case "world_books_entry_set_extension":
                this.contentApi.handleEntityExtensionSet(msg.requestId, msg.entity, msg.entityId, msg.namespace, msg.value, msg.userId);
                break;
            ...
            case "regex_scripts_list":
                this.contentApi.handleRegexScriptsList(msg.requestId, msg.scope, msg.scopeId, msg.target, msg.limit, msg.offset, msg.userId);
                break;
            ...
            case "regex_scripts_get_active":
                this.contentApi.handleRegexScriptsGetActive(msg.requestId, msg.target, msg.characterId, msg.chatId, msg.userId);
                break;
            ...
            // ─── Images (gated: "images") ──────────────────────────────────────
            case "images_list":
                this.contentApi.handleImagesList(msg.requestId, msg.limit, msg.offset, msg.specificity, msg.onlyOwned, msg.characterId, msg.chatId, msg.userId);
                break;
            case "images_get":
                this.contentApi.handleImagesGet(msg.requestId, msg.imageId, msg.specificity, msg.onlyOwned, msg.characterId, msg.chatId, msg.userId);
                break;
            case "images_upload":
            ...
            case "images_upload_from_data_url":
                this.contentApi.handleImagesUploadFromDataUrl(msg.requestId, msg.dataUrl, msg.originalFilename, msg.owner_character_id, msg.owner_chat_id, "skip_thumbnail_processing" in msg ? msg.skip_thumbnail_processing : undefined, msg.userId);
                break;
            case "images_delete":
            ...
            // ─── Toast (free tier) ────────────────────────────────────────────
            case "toast_show":
                this.interactionApi.handleToastShow(msg.toastType, msg.message, msg.title, msg.duration, "userId" in msg ? msg.userId : undefined);
                break;
            case "log":
            ...
            case "modal_open":
                this.presentationApi.handleModalOpen(msg.requestId, msg.title, msg.items, msg.width, msg.maxHeight, msg.persistent, msg.userId, msg.modalRequestId);
                break;
            case "modal_close":
            ...
            case "macros_resolve":
                this.handleMacrosResolve(msg.requestId, msg.template, msg.chatId, msg.characterId, msg.userId, msg.commit !== false);
                break;
            ...
            case "theme_apply_palette":
                this.presentationApi.handleThemeApplyPalette(msg.requestId, msg.palette, msg.userId);
                break;
            case "theme_clear":
            ...
            default:
                ...
                if (msg.requestId) {
                    this.postToWorker({
                        ...
                        error: `Unrecognized message type: "${msg.type}"`,
                    });
                }
                break;
        }
    ...
    static GENERATION_EVENTS = new Set([
    ...
    handleSubscribeEvent(event) {
        ...
        if (!eventType) {
            console.warn(`[Spindle:${this.manifest.identifier}] Unknown event: ${event}`);
            return;
        }
        // Generation lifecycle/streaming events require the generation permission
        if (WorkerHost.GENERATION_EVENTS.has(eventType) &&
            !this.hasPermission("generation")) {
            console.warn(`[Spindle:${this.manifest.identifier}] Generation permission required for event: ${event}`);
            this.postToWorker({
    ...
    // ─── Macro registration ──────────────────────────────────────────────
    handleRegisterMacro(definition) {
        const macroName = String(definition.name || "").trim();
        if (!macroName)
            return;
        const macroOrigin = { kind: "extension", extensionId: this.extensionId };
            ...
            handler: async (ctx) => {
                ...
                return await new Promise((resolvePromise) => {
                    ...
                    const safeEnv = {
                        ...
                        dynamicMacros: Object.fromEntries(Object.entries(ctx.env.dynamicMacros).filter(([, v]) => typeof v === "string")),
                        extra: safeExtra,
                    };
                    try {
                        this.postToWorker({
        ...
        if (!registered) {
            console.warn(`[Spindle:${this.manifest.identifier}] Cannot override macro owned by the system or another extension: ${macroName}`);
            return;
        }
        this.registeredMacroNames.add(macroName);
    }
    handleUnregisterMacro(name) {
    ...
    handleUpdateMacroValue(name, value) {
        const macroName = String(name || "").trim();
        if (!macroName)
            return;
    ...
    handleRegisterInterceptor(registrationId, priority, match) {
        if (!this.hasPermission("interceptor")) {
            console.warn(`[Spindle:${this.manifest.identifier}] Interceptor permission not granted`);
            this.postToWorker({
                type: "permission_denied",
        ...
        const resolveTimeoutMs = () => resolveInterceptorTimeout(this.manifest.interceptorTimeoutMs, this.getScopedUserId());
        this.interceptorUnregister?.();
        ...
        this.interceptorUnregister = interceptorPipeline.register({
            ...
            handler: async (messages, context) => {
                ...
                const messagesWithSourceFlags = messages.map((m) => {
                    ...
                    if (!isChatHistory && !isWorldInfoEntry)
                        return m;
                ...
                const interceptorContext = projectWorldInfoCaptureContext(context, this.extensionId);
                    ...
                    const timeout = setTimeout(() => {
                        setTimeout(() => {
                            if (!this.pendingRequests.has(requestId))
                                return;
                            this.pendingRequests.delete(requestId);
                            reject(new Error(`Interceptor timeout from ${this.manifest.identifier} (${Math.round(timeoutMs / 1000)}s)`));
                        }, 0);
                    }, timeoutMs);
                    this.pendingRequests.set(requestId, {
                        resolve: (val) => {
    ...
    handleUnregisterInterceptor(registrationId) {
        if (registrationId !== this.interceptorRegistrationId)
            return;
        this.interceptorUnregister?.();
        this.interceptorUnregister = null;
        this.interceptorRegistrationId = null;
    ...
    normalizeInterceptorBreakdownEntry(entry, messages) {
    ...
    // ─── Tool registration ───────────────────────────────────────────────
    handleRegisterTool(toolDTO) {
        if (!this.hasPermission("tools")) {
            console.warn(`[Spindle:${this.manifest.identifier}] Tools permission not granted`);
            this.postToWorker({
                type: "permission_denied",
                permission: "tools",
    ...
    isValidProviderKind(kind) {
        return PROVIDER_BROKER_KINDS.includes(kind);
    }
    /** Invalid/missing kinds must not build a malformed `providers..register` permission string. */
    denyInvalidProviderKind(kind, operation) {
        console.warn(`[Spindle:${this.manifest.identifier}] invalid provider kind ${JSON.stringify(kind ?? null)} for ${operation}`);
        this.postToWorker({
            type: "permission_denied",
    ...
    handleProviderRegister(msg) {
        ...
        const permission = `providers.${providerKind}.register`;
        if (!this.hasPermission(permission)) {
            console.warn(`[Spindle:${this.manifest.identifier}] ${PERMISSION_DENIED_PREFIX} ${permission} - Provider registration permission not granted`);
            this.postToWorker({
                type: "permission_denied",
                permission,
        ...
        try {
            ...
            console.warn(`[Spindle:${this.manifest.identifier}] provider_register failed: ${err?.message || err}`);
        }
    }
    handleProviderUnregister(msg) {
        const providerKind = msg.kind;
        if (!this.isValidProviderKind(providerKind)) {
        ...
        try {
            ...
            console.warn(`[Spindle:${this.manifest.identifier}] provider_unregister failed: ${err?.message || err}`);
        }
    }
    // ─── Generation ──────────────────────────────────────────────────────
    async handleGeneration(requestId, input) {
    ...
    handleCancelGeneration(requestId) {
     ...
     */
    async handleGenerationStream(requestId, input) {
        if (!this.hasPermission("generation")) {
            this.postToWorker({
     ...
     */
    resolveEffectiveUserId(requestUserId) {
        const scopedUserId = this.getScopedUserId();
        if (scopedUserId) {
    ...
    getConnectionDispatchDescriptor(userId, connectionId) {
    ...
    getActiveInterceptorContext() {
        ...
        if (!context)
            throw new Error("This operation is only available during an active interceptor callback");
        return context;
    }
    resolveBoundDispatch(context, dispatch) {
    ...
    handleConnectionsResolveDispatch(requestId, connectionId) {
        try {
            if (!this.hasPermission("generation")) {
    ...
    async handleBoundAssembly(requestId, input) {
        try {
            if (!this.hasPermission("generation")) {
            ...
            try {
                const result = await assembleSpindleBlocks(context.userId, this.manifest.identifier, {
                    blocks: input.blocks,
    ...
    async handleBoundQuietGeneration(requestId, input) {
        try {
            if (!this.hasPermission("generation")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} generation — Generation permission not granted`);
            }
    ...
    // ─── Permissions ───────────────────────────────────────────────────────
    handlePermissionsGetGranted(requestId) {
        try {
    ...
    handleChatGetMessages(requestId, chatId) {
        try {
            ...
            const messages = getChatMessages(userId, chatId).map((m) => {
                ...
                const metadata = typeof spindle_metadata === "object" && spindle_metadata
                    ? spindle_metadata
                    : undefined;
                const swipes = Array.isArray(m.swipes) ? m.swipes.slice() : [];
                const swipeId = typeof m.swipe_id === "number" && Number.isFinite(m.swipe_id) ? m.swipe_id : 0;
    ...
    async handleChatAppendMessage(requestId, chatId, message, options) {
        try {
            if (!this.hasPermission("chat_mutation")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chat_mutation — Chat mutation permission not granted`);
            }
            const triggerGeneration = options === true ||
            ...
            if (message.metadata)
                extra.spindle_metadata = message.metadata;
            const created = createChatMessage(chatId, {
            ...
            if (triggerGeneration) {
                const generationOptions = typeof options === "object" &&
                    options !== null &&
    ...
    handleChatUpdateMessage(requestId, chatId, messageId, patch) {
        try {
            if (!this.hasPermission("chat_mutation")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chat_mutation — Chat mutation permission not granted`);
            }
            ...
            if (patch.reasoning && typeof patch.reasoning === "object") {
                ...
                if (r.text !== undefined) {
                    if (r.text === null)
                        ...
                        extra.reasoning = r.text;
                    extraDirty = true;
                }
                if (r.duration !== undefined) {
                    delete extra.reasoning_duration;
                    if (r.duration === null)
                        extra.reasoningDuration = null;
                    else
                        extra.reasoningDuration = r.duration;
                    extraDirty = true;
                }
            }
    ...
    handleChatDeleteMessage(requestId, chatId, messageId) {
        try {
            if (!this.hasPermission("chat_mutation")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chat_mutation — Chat mutation permission not granted`);
            }
    ...
    handleChatSetMessageHidden(requestId, chatId, messageId, hidden) {
        try {
            if (!this.hasPermission("chat_mutation")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chat_mutation — Chat mutation permission not granted`);
            }
    ...
    handleChatSetMessagesHidden(requestId, chatId, messageIds, hidden) {
        try {
            if (!this.hasPermission("chat_mutation")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chat_mutation — Chat mutation permission not granted`);
            }
            ...
            const filtered = messageIds.filter((id) => typeof id === "string" && !!id);
            const userId = this.getChatOwnerId(chatId);
            if (!userId)
                throw new Error("Chat not found");
            this.enforceScopedUser(userId);
    ...
    handleChatIsMessageHidden(requestId, chatId, messageId) {
        try {
            if (!this.hasPermission("chat_mutation")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} chat_mutation — Chat mutation permission not granted`);
            }
    ...
    writeTrackedEvents(events) {
        const file = this.getEventLogPath();
        const content = events.map((e) => JSON.stringify(e)).join("\n");
        writeFileSync(file, content ? `${content}\n` : "", "utf-8");
    }
    enforceTrackedEventPermission() {
    ...
    handleEventsTrack(requestId, eventName, payload, options) {
        try {
    ...
    handleEventsQuery(requestId, filter) {
        try {
            this.enforceTrackedEventPermission();
            if (filter?.chatId) {
            ...
            const rows = this.readTrackedEvents()
                ...
                if (filter?.level && e.level !== filter.level)
                    return false;
                if (!Number.isNaN(sinceMs) && tsMs < sinceMs)
                    return false;
                if (!Number.isNaN(untilMs) && tsMs > untilMs)
                    return false;
                return true;
            })
    ...
    handleEventsGetLatestState(requestId, keys) {
        try {
            this.enforceTrackedEventPermission();
            const remaining = new Set(keys);
            const state = {};
            const events = this.readTrackedEvents().sort((a, b) => Date.parse(b.ts) - Date.parse(a.ts));
            for (const entry of events) {
                if (!entry.payload || typeof entry.payload !== "object")
                    continue;
                for (const key of [...remaining]) {
                    if (Object.prototype.hasOwnProperty.call(entry.payload, key)) {
                        state[key] = entry.payload[key];
    ...
    // ─── CORS proxy ──────────────────────────────────────────────────────
    async handleCorsRequest(requestId, url, options) {
        options = options || {};
        ...
        const binaryMediaType = options?.mediaType === "audio" ? "audio"
        ...
        try {
            ...
            if (declared && parseInt(declared, 10) > CORS_PROXY_MAX_BODY_BYTES) {
                throw new Error(`CORS proxy response too large (declared ${declared} bytes, max ${CORS_PROXY_MAX_BODY_BYTES})`);
            }
            if (isBinary) {
                ...
                if (!isAllowedContentType) {
                    throw new Error(`CORS proxy transparent proxy only serves ${binaryMediaType} data (received Content-Type: ${contentType || "unknown"})`);
                }
    ...
    // ─── Context handler ─────────────────────────────────────────────────
    handleRegisterContextHandler(priority, timeoutMs) {
        if (!this.hasPermission("context_handler")) {
            console.warn(`[Spindle:${this.manifest.identifier}] Context handler permission not granted`);
            this.postToWorker({
                type: "permission_denied",
        ...
        this.contextHandlerUnregister = contextHandlerChain.register({
            ...
            handler: async (context) => {
                ...
                return new Promise((resolve, reject) => {
                    const timeout = setTimeout(() => {
                        this.pendingRequests.delete(requestId);
                        reject(new Error(`Context handler timeout from ${this.manifest.identifier}`));
                    }, budgetMs);
                    this.pendingRequests.set(requestId, {
                        resolve: (val) => {
                            clearTimeout(timeout);
    ...
    handleRegisterMessageContentProcessor(priority) {
        if (!this.hasPermission("chat_mutation")) {
            console.warn(`[Spindle:${this.manifest.identifier}] chat_mutation permission not granted for registerMessageContentProcessor`);
            this.postToWorker({
                type: "permission_denied",
        ...
        this.messageContentProcessorUnregister = messageContentProcessorChain.register({
            ...
            handler: async (ctx) => {
                ...
                return new Promise((resolve, reject) => {
                    const timeout = setTimeout(() => {
                        this.pendingRequests.delete(requestId);
                        reject(new Error(`Message content processor timeout from ${this.manifest.identifier}`));
                    }, 10_000);
                    this.pendingRequests.set(requestId, {
                        resolve: (val) => {
                            clearTimeout(timeout);
    ...
    handleRegisterMacroInterceptor(priority) {
        if (!this.hasPermission("macro_interceptor")) {
            console.warn(`[Spindle:${this.manifest.identifier}] macro_interceptor permission not granted for registerMacroInterceptor`);
            this.postToWorker({
                type: "permission_denied",
        ...
        this.macroInterceptorUnregister = macroInterceptorChain.register({
            ...
            handler: async (ctx) => {
                ...
                return new Promise((resolve, reject) => {
                    const timeout = setTimeout(() => {
                        this.pendingRequests.delete(requestId);
                        console.warn(`[Spindle] Macro interceptor from ${this.manifest.identifier} timed out after 10s; skipping`);
                        resolve(undefined);
                    }, 10_000);
                    this.pendingRequests.set(requestId, {
                        resolve: (val) => {
    ...
    handleRegisterWorldInfoInterceptor(priority) {
        if (!this.hasPermission("generation")) {
            console.warn(`[Spindle:${this.manifest.identifier}] generation permission not granted for registerWorldInfoInterceptor`);
            this.postToWorker({
                type: "permission_denied",
                permission: "generation",
        ...
        this.worldInfoInterceptorUnregister = worldInfoInterceptorChain.register({
            ...
            handler: async (ctx) => {
                ...
                return new Promise((resolve, reject) => {
                    const timeout = setTimeout(() => {
                        this.pendingRequests.delete(requestId);
                        console.warn(`[Spindle] World-info interceptor from ${this.manifest.identifier} timed out after 10s; skipping`);
                        resolve(undefined);
                    }, 10_000);
                    this.pendingRequests.set(requestId, {
                        resolve: (val) => {
    ...
    // ─── Characters (gated: "characters") ──────────────────────────────
    async handleGenerateDryRun(requestId, input, userId) {
        try {
            if (!this.hasPermission("generation")) {
                throw new Error(`${PERMISSION_DENIED_PREFIX} generation — Generation permission not granted`);
            }
            ...
            const messagesDTO = dryRunResult.messages.map((m) => ({
                role: m.role,
                content: m.content,
                name: m.name,
            }));
    ...
    async handleAssemblePrompt(requestId, input, userId) {
        if (!this.hasPermission("generation")) {
            this.postToWorker({
                type: "response",
                requestId,
        ...
        this.generationAbortControllers.set(requestId, abortController);
        try {
            const result = await assembleSpindleBlocks(resolvedUserId, this.manifest.identifier, input, abortController.signal);
            this.postToWorker({ type: "response", requestId, result });
        }
        catch (err) {
            const aborted = abortController.signal.aborted || err?.name === "AbortError";
    ...
    // ─── Push Notifications (gated: "push_notification") ─────────────────
    // ─── Token Counting (free tier) ─────────────────────────────────────
    normalizeTokenCountMessages(messages) {
    ...
    async resolveTokenCountModel(userId, explicitModel, modelSource = "main") {
    ...
    async buildTokenCountResult(userId, input, explicitModel, modelSource = "main") {
    ...
    async handleUploadsGet(requestId, uploadId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
    ...
    async handleUploadsReadChunk(requestId, uploadId, offset, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
    ...
    async handleUploadsDelete(requestId, uploadId, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
    ...
    async handleTokensCountText(requestId, text, model, modelSource, userId) {
        try {
    ...
    async handleTokensCountTextBatch(requestId, texts, model, modelSource, userId) {
        try {
    ...
    async handleTokensCountMessages(requestId, messages, model, modelSource, userId) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
    ...
    async handleTokensCountChat(requestId, chatId, model, modelSource, userId) {
        try {
            ...
            const messages = getChatMessages(chatOwnerId, chatId).map((message) => ({
                role: this.mapChatRole(message.is_user, (message.extra || {})),
                content: message.content,
            }));
    ...
    // ─── Frontend Process Lifecycle (free tier) ─────────────────────────
    // ─── Version (free tier) ────────────────────────────────────────────
    async handleVersionGetBackend(requestId) {
        try {
    ...
    async handleVersionGetFrontend(requestId) {
        try {
    ...
    async handleMacrosResolve(requestId, template, chatId, characterId, userId, commit = true) {
        try {
            const resolvedUserId = this.resolveEffectiveUserId(userId);
            if (!resolvedUserId)
                throw new Error("userId is required for operator-scoped extensions");
            ...
            if (!env) {
                ...
                const { makeAssistantCharacter } = await import("../types/character");
                const persona = personaAddonStatesSvc.resolvePersonaForChatMacros(resolvedUserId, personasSvc.getDefaultPersona(resolvedUserId), null);
                const connection = connectionsSvc.resolveConnection(resolvedUserId);
                env = buildEnv({
