...
import { env } from "../env";
import { validateIdentifier, isValidPermission, isValidCapability, } from "lumiverse-spindle-types";
import { existsSync, mkdirSync, rmSync, readdirSync, renameSync, statSync, copyFileSync, cpSync, realpathSync, } from "fs";
...
import { assertOwnGitRepositoryPath, resetGitRepositoryToRemoteHead, } from "./git-repository";
...
const DANGEROUS_MODULE_LABELS = new Map([
...
const warnedWindowsSpindleBunFallback = new Set();
const backendSafetyScanCache = new Map();
const DANGEROUS_BACKEND_CHECKS = [
    {
...
function normalizeJavaScriptForSafetyScan(content) {
    try {
...
function collectIgnoredSpans(source) {
    const spans = [];
    ...
    const skipQuoted = (start, quote, end) => {
        let i = start + 1;
        while (i < end) {
            if (source[i] === "\\") {
                i += 2;
    ...
    const scanRegex = (start, end) => {
        ...
        while (i < end) {
            ...
            if (ch === "/" && !inClass) {
                i += 1;
                // Consume regex flags.
                while (i < end && /[dgimsuy]/.test(source[i]))
                    i += 1;
                addSpan(start, i);
                return i;
            }
    ...
    const scanCode = (start, end, stopOnTemplateBrace = false) => {
        let i = start;
        while (i < end) {
            if (stopOnTemplateBrace && source[i] === "}")
                return i;
            if (source[i] === "/" && source[i + 1] === "/") {
...
function isIgnoredIndex(index, spans) {
...
function createScannableSources(content) {
...
function matchOutsideIgnored(source, regex) {
    const flags = regex.flags.includes("g") ? regex.flags : `${regex.flags}g`;
    const globalRegex = new RegExp(regex.source, flags);
    const matches = [];
    for (const match of source.text.matchAll(globalRegex)) {
        if (!isIgnoredIndex(match.index, source.ignoredSpans))
            matches.push(match);
    }
    return matches;
}
...
function decodeSimpleStringExpression(raw) {
    const trimmed = raw.trim();
 ...
 */
function resolveStaticModuleSpecifier(raw) {
    const trimmed = raw.trim();
    if (!trimmed)
        return null;
    ...
    if (charCode) {
        ...
        if (codes.some((n) => !Number.isInteger(n) || n < 0 || n > 0x10ffff))
            return null;
    ...
    while (rest) {
        ...
        if (rest[0] === ",")
            break;
        if (rest[0] !== "+")
            return null; // any non-`+` operator/token ⇒ dynamic
...
function addDynamicModuleHits(source, hits) {
    ...
    for (const match of matchOutsideIgnored(source, /(?<![.\w$])(?:require|import)\s*\(([^;\n]{1,300})\)/)) {
        if (match.index !== undefined) {
            ...
            if (/^\s*\{/.test(tail))
                continue; // `require(name) { … }` — a definition
        }
...
function addPropertyAccessHits(source, objectName, properties, label, hits) {
...
function addAliasPropertyHits(source, hits) {
    ...
    for (const [alias, aliasSource] of aliases) {
        addPropertyAccessHits(source, alias, aliasSource === "Bun" ? DANGEROUS_BUN_PROPERTIES : DANGEROUS_PROCESS_PROPERTIES, aliasSource === "Bun" ? "dangerous Bun system API usage" : "dangerous process API usage", hits);
    }
}
function addDestructuringHits(source, hits) {
    for (const match of matchOutsideIgnored(source, /\b(?:const|let|var)\s*\{([^}]+)\}\s*=\s*Bun\b/)) {
        for (const prop of match[1].split(",")) {
            ...
            if (DANGEROUS_BUN_PROPERTIES.has(name))
                hits.add("dangerous Bun system API usage");
        }
    }
    for (const match of matchOutsideIgnored(source, /\b(?:const|let|var)\s*\{([^}]+)\}\s*=\s*process\b/)) {
        for (const prop of match[1].split(",")) {
            const name = prop.trim().split(/\s*:/, 1)[0]?.trim();
            if (DANGEROUS_PROCESS_PROPERTIES.has(name))
 ...
 */
const LABEL_TO_CAPABILITY = new Map([
...
function isEmptyFunctionProbe(source, matchIndex) {
...
export function detectDangerousBackendCapabilities(content, declared = new Set()) {
    const hits = new Set();
    for (const source of createScannableSources(content)) {
        ...
        for (const match of dynExecMatches) {
            ...
            if (matchedText.startsWith("Function") && isEmptyFunctionProbe(source, match.index))
                continue;
 ...
 * no opt-in.
 */
export function declaredCapabilitiesFromManifest(manifest) {
...
async function assertSafeBackendBundle(identifier, backendPath, declared = new Set()) {
    if (!(await Bun.file(backendPath).exists()))
        return;
    ...
    if (cached?.signature === signature) {
        ...
        blocked = detectDangerousBackendCapabilities(await Bun.file(backendPath).text(), declared);
        backendSafetyScanCache.set(backendPath, { signature, blocked });
    }
    if (blocked.length === 0)
        return;
    throw new Error(`Extension "${identifier}" uses blocked backend capabilities: ${blocked.join(", ")}`);
}
/**
...
function extensionDir(identifier) {
    return join(extensionsDir(), identifier);
}
function repoDir(identifier) {
    return join(extensionDir(identifier), "repo");
}
function storageDir(identifier) {
...
async function readManifest(identifier) {
    ...
    if (!manifest.identifier || !validateIdentifier(manifest.identifier)) {
        throw new Error(`Invalid identifier "${manifest.identifier}". Must match /^[a-z][a-z0-9_]*$/`);
    }
    if (!manifest.version)
        throw new Error("Missing version in spindle.json");
    if (!manifest.name)
        throw new Error("Missing name in spindle.json");
    if (!manifest.author)
        throw new Error("Missing author in spindle.json");
...
async function readManifestFromPath(manifestPath, options) {
...
function moveRootRepoToNestedRepo(extRootDir) {
    ...
    for (const entry of entries) {
        if (entry.name === "repo" || entry.name === "storage")
            continue;
...
async function ensureRepoLayoutForIdentifier(identifier) {
...
function insertExtensionFromManifest(manifest) {
    const db = getDb();
    const existing = db
    ...
    db.run(`INSERT INTO extensions (id, identifier, name, version, author, description, github, homepage, permissions, enabled, metadata)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, '{}')`, [
        id,
        manifest.identifier,
        manifest.name,
...
function grantRequestedPermissionsByDefault(identifier, permissions) {
 ...
 */
function syncPermissionGrants(identifier, manifestPermissions, previousPermissions) {
 ...
 */
export async function syncManifestToDb(identifier) {
    let manifest;
    try {
    ...
    if (!row)
        return;
    const dbPermissions = parsePermissionsSafe(row.permissions);
    const manifestPermissions = manifest.permissions || [];
    // Check if the extensions row needs updating
    const metadataChanged = row.name !== manifest.name ||
        row.version !== manifest.version ||
        row.author !== manifest.author ||
        (row.description || "") !== (manifest.description || "") ||
        row.github !== manifest.github ||
        (row.homepage || "") !== (manifest.homepage || "");
    const permissionsChanged = JSON.stringify(dbPermissions) !== JSON.stringify(manifestPermissions);
    if (metadataChanged || permissionsChanged) {
        db.run(`UPDATE extensions SET name = ?, version = ?, author = ?, description = ?,
...
function resolveWithin(base, requestedPath, label) {
...
function applyStorageSeeds(identifier, manifest) {
 ...
 */
export function shouldUseWindowsSpindleBunSyncFallback(platform = process.platform, env = process.env) {
    if (platform !== "win32")
        return false;
    return env[WINDOWS_SPINDLE_ASYNC_BUN_OVERRIDE] !== "1";
}
function warnWindowsSpindleBunSyncFallback(context) {
    ...
    console.warn(`[Spindle] ${context} is using a synchronous Bun subprocess fallback on Windows to avoid known Bun.spawn pipe crashes. Set ${WINDOWS_SPINDLE_ASYNC_BUN_OVERRIDE}=1 to re-enable the async path.`);
}
async function runSpindleBunSubprocess(cmd, opts) {
...
function formatCommandFailure(result, label) {
...
// ─── Build ───────────────────────────────────────────────────────────────
export async function buildExtension(identifier) {
    const repo = repoDir(identifier);
    ...
    if (needsBackendBuild) {
        const proc = await runSpindleBunSubprocess(bunCmd("build", "src/backend.ts", "--outfile", backendEntry, "--target", "bun"), {
            cwd: repo,
    ...
    if (needsFrontendBuild) {
        const proc = await runSpindleBunSubprocess(bunCmd("build", "src/frontend.ts", "--outfile", frontendEntry, "--target", "browser"), {
            cwd: repo,
 ...
 */
function assertSafeGitUrl(rawUrl) {
...
export async function install(githubUrl, options) {
    assertSafeGitUrl(githubUrl);
    const baseDir = extensionsDir();
    mkdirSync(baseDir, { recursive: true });
    const installScope = options?.installScope === "user" ? "user" : "operator";
    const installedByUserId = options?.installedByUserId && options.installedByUserId.trim()
...
async function runGitStep(repo, cmd, label, options) {
    ...
    if (proc.exitCode === 0)
        return;
    throw new Error(`${label} failed: ${spawnFailureReason(proc, options.timeoutMs)}`);
}
...
export async function assertOwnedExtensionGitRoot(repo) {
    let canonicalRepo;
    try {
...
export async function update(identifier) {
    ...
    const srcDir = join(repo, "src");
    const hasBuildableSrc = existsSync(srcDir) &&
    ...
    syncPermissionGrants(identifier, manifest.permissions || [], existingPermissions);
...
export function remove(identifier) {
    ...
    const ext = db
        ...
        .get(identifier);
    if (!ext)
        throw new Error(`Extension not found: ${identifier}`);
...
// ─── Enable / Disable ────────────────────────────────────────────────────
export function enable(identifier) {
    const db = getDb();
    const result = db.run("UPDATE extensions SET enabled = 1, updated_at = unixepoch() WHERE identifier = ?", [identifier]);
    if (result.changes === 0)
        throw new Error(`Extension not found: ${identifier}`);
    ...
    const row = db
        ...
        .get(identifier);
    if (row) {
        const requested = parsePermissionsSafe(row.permissions);
        grantRequestedPermissionsByDefault(identifier, requested);
    }
}
export function disable(identifier) {
    const db = getDb();
    const result = db.run("UPDATE extensions SET enabled = 0, updated_at = unixepoch() WHERE identifier = ?", [identifier]);
    if (result.changes === 0)
        throw new Error(`Extension not found: ${identifier}`);
}
function extensionGrantsHaveScopeColumn() {
    try {
        return getDb().query("PRAGMA table_info('extension_grants')").all()
...
function hostDerivedGrantScope(ext) {
    const installScope = ext.install_scope === "user" || ext.install_scope === "operator" || ext.install_scope === "system"
        ? ext.install_scope
        : "system";
    try {
...
function loadExtensionGrantTarget(identifier) {
    const db = getDb();
    try {
        const ext = db
...
function resolveGrantScope(target, scope) {
    const requested = typeof scope === "string" ? scope.trim() : "";
    return requested || target.scope;
}
export function grantPermission(identifier, permission, scope) {
    ...
    if (extensionGrantsHaveScopeColumn()) {
        db.run(`INSERT OR IGNORE INTO extension_grants (id, extension_id, permission, scope) VALUES (?, ?, ?, ?)`, [crypto.randomUUID(), ext.id, permission, grantScope]);
        return;
    }
    db.run(`INSERT OR IGNORE INTO extension_grants (id, extension_id, permission) VALUES (?, ?, ?)`, [crypto.randomUUID(), ext.id, permission]);
}
export function revokePermission(identifier, permission, scope) {
    const ext = loadExtensionGrantTarget(identifier);
    if (!ext)
        throw new Error(`Extension not found: ${identifier}`);
    const db = getDb();
    const grantScope = resolveGrantScope(ext, scope);
    if (extensionGrantsHaveScopeColumn()) {
        db.run("DELETE FROM extension_grants WHERE extension_id = ? AND permission = ? AND scope = ?", [ext.id, permission, grantScope]);
        return;
    }
    db.run("DELETE FROM extension_grants WHERE extension_id = ? AND permission = ?", [ext.id, permission]);
}
export function getGrantedPermissions(identifier, scope) {
    const ext = loadExtensionGrantTarget(identifier);
    if (!ext)
        return [];
    const db = getDb();
    ...
    const rows = extensionGrantsHaveScopeColumn()
        ? db
            ...
            .all(ext.id, grantScope)
        : db
            .query("SELECT permission FROM extension_grants WHERE extension_id = ?")
            .all(ext.id);
    return rows.map((r) => r.permission);
}
export function hasPermission(identifier, permission, scope) {
    return getGrantedPermissions(identifier, scope).includes(permission);
}
// ─── Queries ─────────────────────────────────────────────────────────────
export async function list() {
    const db = getDb();
    const rows = db.query("SELECT * FROM extensions ORDER BY installed_at DESC").all();
    return Promise.all(rows.map(rowToExtensionInfo));
}
/** Lightweight rows for the periodic remote-HEAD monitor. */
export function listExtensionUpdateCandidates() {
    const db = getDb();
    const rows = db
        .query(`SELECT id, identifier, name, version, branch
       FROM extensions
       WHERE enabled = 1
...
export async function getExtension(id) {
    const db = getDb();
    const row = db.query("SELECT * FROM extensions WHERE id = ?").get(id);
    return row ? rowToExtensionInfo(row) : null;
}
export async function getExtensionForUser(id, userId, role) {
    if (role === "owner" || role === "admin") {
...
export function canManageExtension(extension, userId, role) {
    if (role === "owner" || role === "admin")
        return true;
    const metadata = (extension.metadata || {});
    return (metadata.install_scope === "user" &&
...
export function getManageableExtensionIdsForUser(userId, role) {
    const db = getDb();
    if (role === "owner" || role === "admin") {
        return db.query("SELECT id FROM extensions").all()
            .map((row) => row.id);
    }
    return db
        .query(`SELECT id FROM extensions
         WHERE install_scope = 'user' AND installed_by_user_id = ?`)
        .all(userId).map((row) => row.id);
}
export async function getExtensionByIdentifier(identifier) {
    const db = getDb();
    const row = db
...
export async function getManifest(identifier) {
...
export function getEnabledExtensionIdentifiers() {
    const db = getDb();
    const rows = db
...
async function getFrontendBundlePathFromManifest(identifier, manifest) {
...
export async function getFrontendBundlePath(identifier) {
    return getFrontendBundlePathFromManifest(identifier, await readManifest(identifier));
}
export async function getFrontendBundleCacheKey(identifier) {
...
async function getBackendEntryPathFromManifest(identifier, manifest, options) {
...
export async function getBackendEntryPath(identifier) {
    return getBackendEntryPathFromManifest(identifier, await readManifest(identifier), {
        verifySafety: true,
    });
}
export function getStoragePath(identifier) {
    const dir = storageDir(identifier);
    mkdirSync(dir, { recursive: true });
    return dir;
}
export function getRepoPath(identifier) {
    return repoDir(identifier);
}
export function getStoragePathForExtension(extension) {
    const metadata = (extension.metadata || {});
...
export function getUserExtensionStoragePath(identifier, userId) {
...
export async function importLocalExtensions() {
    ...
    for (const dirName of dirs) {
        ...
        try {
            ...
            else if (await Bun.file(nestedSpindleFilePath).exists())
                manifestPath = nestedSpindleFilePath;
            else if (await Bun.file(nestedSpindleFileJsonPath).exists())
                manifestPath = nestedSpindleFileJsonPath;
            else if (await Bun.file(rootManifestPath).exists())
                manifestPath = rootManifestPath;
            else if (await Bun.file(rootSpindleFilePath).exists())
                manifestPath = rootSpindleFilePath;
            else if (await Bun.file(rootSpindleFileJsonPath).exists())
            ...
            if (manifestPath === rootManifestPath ||
                ...
                if (candidateRoot !== desiredRoot) {
                    if (existsSync(desiredRoot)) {
                        throw new Error(`Target directory already exists for identifier ${manifest.identifier}`);
                    }
...
export function listRemoteBranches(githubUrl) {
    const proc = Bun.spawnSync({
...
export function getBranches(identifier) {
    const repo = repoDir(identifier);
    if (!existsSync(repo)) {
        throw new Error(`Extension repo not found: ${identifier}`);
    }
...
export async function switchBranch(identifier, branch) {
    const repo = repoDir(identifier);
    if (!existsSync(repo)) {
        throw new Error(`Extension repo not found: ${identifier}`);
    }
    const initialManifest = await readManifest(identifier);
    if (initialManifest.dev_mode === true) {
    ...
    await runGitStep(repo, ["git", "config", "remote.origin.fetch", "+refs/heads/*:refs/remotes/origin/*"], "git config remote.origin.fetch", { timeoutMs: LOCAL_GIT_COMMAND_TIMEOUT_MS });
    // Fetch the target branch (--depth=1 to keep it shallow)
    await runGitStep(repo, ["git", "fetch", "--depth", "1", "origin", branch], `git fetch ${branch}`, { timeoutMs: GIT_FETCH_TIMEOUT_MS });
    // Checkout the branch
    await runGitStep(repo, ["git", "checkout", "-B", branch, `origin/${branch}`], `git checkout ${branch}`, { timeoutMs: LOCAL_GIT_COMMAND_TIMEOUT_MS });
    // Re-read manifest
    const manifest = await readManifest(identifier);
    const db = getDb();
    const existing = db
...
// ─── Helpers ─────────────────────────────────────────────────────────────
async function rowToExtensionInfo(row) {
    ...
    return {
        ...
        // first load.
        status: require("./lifecycle").isRunning(row.id) ? "running" : "stopped",
        metadata,
    };
}
