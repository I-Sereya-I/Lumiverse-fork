...
function sanitizeOverrides(input) {
    if (!input || typeof input !== "object")
        return {};
    const out = {};
    for (const [identifier, raw] of Object.entries(input)) {
        if (!identifier)
            continue;
...
export async function getEphemeralPoolConfig() {
    const base = {
    ...
    try {
        ...
        return {
            ...
            extensionDefaultMaxBytes: parsePositiveInt(raw.extensionDefaultMaxBytes, base.extensionDefaultMaxBytes),
...
export async function updateEphemeralPoolConfig(patch) {
    const current = await getEphemeralPoolConfig();
    const next = {
        globalMaxBytes: parsePositiveInt(patch.globalMaxBytes, current.globalMaxBytes),
        extensionDefaultMaxBytes: parsePositiveInt(patch.extensionDefaultMaxBytes, current.extensionDefaultMaxBytes),
        extensionMaxOverrides: patch.extensionMaxOverrides !== undefined
            ? sanitizeOverrides(patch.extensionMaxOverrides)
            : current.extensionMaxOverrides,
        reservationTtlMs: parsePositiveInt(patch.reservationTtlMs, current.reservationTtlMs),
    };
    mkdirSync(CONFIG_DIR, { recursive: true });
    await Bun.write(CONFIG_PATH, JSON.stringify(next, null, 2));
    return next;
}
function getExtensionEphemeralBase(ext) {
    return resolve(managerSvc.getStoragePathForExtension(ext), ".ephemeral");
}
async function getReservations(ext) {
...
async function getExtensionUsage(ext) {
    ...
    if (existsSync(base)) {
        ...
        for (const entry of entries) {
            ...
            if (full === indexPath || full === reservationsPath)
                continue;
            try {
...
export async function getEphemeralPoolOverview(options) {
    ...
    const extensionRows = await Promise.all(extensions.map(async (ext) => {
        ...
        const extensionMaxBytes = cfg.extensionMaxOverrides[ext.identifier] ?? cfg.extensionDefaultMaxBytes;
        return {
            extensionId: ext.id,
