...
import { normalizeSpindleAppNavigationPath, normalizeSpindleHttpsUrl, } from "./url-safety";
...
describe("normalizeSpindleHttpsUrl", () => {
    test("accepts https manifest links", () => {
        expect(normalizeSpindleHttpsUrl("https://github.com/example/repo", "github")).toBe("https://github.com/example/repo");
    });
    test("rejects non-https manifest links", () => {
        expect(() => normalizeSpindleHttpsUrl("local://example", "github")).toThrow("github URL must use https://");
        expect(() => normalizeSpindleHttpsUrl("javascript:alert(1)", "homepage")).toThrow("homepage URL must use https://");
    });
