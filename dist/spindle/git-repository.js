...
function hasOwnGitMetadata(repo) {
    return existsSync(repo) && existsSync(join(repo, ".git"));
}
function topLevelMatchesRepository(repo, topLevel) {
    if (!topLevel)
        return false;
    try {
 ...
 */
export function isOwnGitRepositoryPath(repo) {
    if (!hasOwnGitMetadata(repo))
        return false;
    try {
 ...
 */
export async function isOwnGitRepositoryPathAsync(repo) {
    if (!hasOwnGitMetadata(repo))
        return false;
    const proc = await spawnAsync(["git", "rev-parse", "--show-toplevel"], {
        cwd: repo,
...
export function assertOwnGitRepositoryPath(repo, label) {
    if (!isOwnGitRepositoryPath(repo)) {
        throw new Error(`${label} is not backed by its own Git repository; local extensions can be rebuilt in dev mode but cannot be pulled`);
    }
}
async function runGitStep(repo, args, label, timeoutMs, ignoreStdout = false) {
 ...
 */
export async function resetGitRepositoryToRemoteHead(repo, label) {
    assertOwnGitRepositoryPath(repo, label);
    const branch = await runGitStep(repo, ["symbolic-ref", "--quiet", "--short", "HEAD"], "git symbolic-ref HEAD", LOCAL_GIT_COMMAND_TIMEOUT_MS);
    if (!branch) {
        throw new Error(`${label} has a detached HEAD`);
    }
    await runGitStep(repo, ["fetch", "--quiet", "origin", `refs/heads/${branch}`], `git fetch origin ${branch}`, GIT_FETCH_TIMEOUT_MS, true);
    await runGitStep(repo, ["reset", "--hard", "FETCH_HEAD"], "git reset --hard FETCH_HEAD", LOCAL_GIT_COMMAND_TIMEOUT_MS, true);
    await runGitStep(repo, ["clean", "-fd"], "git clean -fd", LOCAL_GIT_COMMAND_TIMEOUT_MS, true);
}
