...
describe("spawnAsync", () => {
    test("enforces and reports a subprocess timeout", async () => {
        ...
        const result = await spawnAsync([process.execPath, "-e", "await Bun.sleep(10_000)"], { timeoutMs });
    ...
    test("does not wait for descendants holding inherited output pipes", async () => {
        ...
        const startedAt = performance.now();
        const result = await spawnAsync([process.execPath, "-e", childProgram], { timeoutMs });
        expect(result.timedOut).toBe(true);
