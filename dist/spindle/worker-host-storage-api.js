...
import { deleteSecret, getSecret, listSecretKeys, putSecret, validateSecret, } from "../services/secrets.service";
...
export class WorkerHostStorageApi {
    ...
    dispatch(message) {
        ...
        switch (msg.type) {
            ...
            case "user_storage_stat":
                ...
                return true;
            case "enclave_put":
                void this.handleEnclavePut(msg.requestId, msg.key, msg.value, msg.userId);
                return true;
            case "enclave_get":
                void this.handleEnclaveGet(msg.requestId, msg.key, msg.userId);
                return true;
            case "enclave_delete":
                this.handleEnclaveDelete(msg.requestId, msg.key, msg.userId);
                return true;
            case "enclave_has":
                void this.handleEnclaveHas(msg.requestId, msg.key, msg.userId);
                return true;
            case "enclave_list":
            ...
            case "ephemeral_write":
                void this.handleEphemeralWrite(msg.requestId, msg.path, msg.data, msg.ttlMs, msg.reservationId);
                return true;
            case "ephemeral_read_binary":
                this.handleEphemeralReadBinary(msg.requestId, msg.path);
                return true;
            case "ephemeral_write_binary":
                void this.handleEphemeralWriteBinary(msg.requestId, msg.path, msg.data, msg.ttlMs, msg.reservationId);
                return true;
            ...
            case "ephemeral_pool_status":
                void this.handleEphemeralPoolStatus(msg.requestId);
                return true;
            case "ephemeral_request_block":
                void this.handleEphemeralRequestBlock(msg.requestId, msg.sizeBytes, msg.ttlMs, msg.reason);
                return true;
    ...
    getStorageRootPath(identifier = this.identifier) {
        if (identifier === this.identifier && this.options.installScope === "user") {
            if (!this.options.installedByUserId)
    ...
    resolveStoragePath(requestedPath) {
        return this.resolveWithin(this.getStorageRootPath(), requestedPath);
    }
    resolveWithin(basePath, requestedPath) {
    ...
    listFiles(searchDir, hiddenFiles = new Set()) {
    ...
    handleStorageRead(requestId, path) {
        try {
            const fullPath = this.resolveStoragePath(path);
            if (!existsSync(fullPath))
                return this.fail(requestId, "File not found");
    ...
    handleStorageWrite(requestId, path, data) {
        try {
            const fullPath = this.resolveStoragePath(path);
    ...
    handleStorageReadBinary(requestId, path) {
        try {
            const fullPath = this.resolveStoragePath(path);
            if (!existsSync(fullPath))
                return this.fail(requestId, "File not found");
    ...
    handleStorageWriteBinary(requestId, path, data) {
        try {
            const fullPath = this.resolveStoragePath(path);
            mkdirSync(resolve(fullPath, ".."), { recursive: true });
    ...
    handleStorageDelete(requestId, path) {
        try {
            const fullPath = this.resolveStoragePath(path);
            if (existsSync(fullPath)) {
    ...
    handleStorageList(requestId, prefix) {
        try {
    ...
    handleStorageExists(requestId, path) {
        try {
            this.respond(requestId, existsSync(this.resolveStoragePath(path)));
        }
        catch (error) {
            this.fail(requestId, error);
        }
    }
    handleStorageMkdir(requestId, path) {
        try {
            mkdirSync(this.resolveStoragePath(path), { recursive: true });
            this.respond(requestId, true);
        }
        catch (error) {
            this.fail(requestId, error);
    ...
    handleStorageMove(requestId, from, to) {
        try {
    ...
    handleStorageStat(requestId, path) {
        try {
            this.statResponse(requestId, this.resolveStoragePath(path));
        }
        catch (error) {
            this.fail(requestId, error);
        }
    }
    resolveUserId(requestUserId) {
    ...
    handleUserStorageWrite(requestId, path, data, userId) {
        try {
            const fullPath = this.resolveUserStoragePath(path, this.resolveUserId(userId));
            mkdirSync(resolve(fullPath, ".."), { recursive: true });
            writeFileSync(fullPath, data, "utf-8");
    ...
    handleUserStorageReadBinary(requestId, path, userId) {
        try {
            const fullPath = this.resolveUserStoragePath(path, this.resolveUserId(userId));
            if (!existsSync(fullPath))
                return this.fail(requestId, "File not found");
    ...
    handleUserStorageWriteBinary(requestId, path, data, userId) {
        try {
            const fullPath = this.resolveUserStoragePath(path, this.resolveUserId(userId));
            mkdirSync(resolve(fullPath, ".."), { recursive: true });
            writeFileSync(fullPath, data);
    ...
    handleUserStorageDelete(requestId, path, userId) {
        try {
            const fullPath = this.resolveUserStoragePath(path, this.resolveUserId(userId));
            if (existsSync(fullPath)) {
                if (statSync(fullPath).isDirectory())
    ...
    handleUserStorageList(requestId, prefix, userId) {
        try {
    ...
    handleUserStorageExists(requestId, path, userId) {
        try {
            this.respond(requestId, existsSync(this.resolveUserStoragePath(path, this.resolveUserId(userId))));
        }
        catch (error) {
            this.fail(requestId, error);
        }
    }
    handleUserStorageMkdir(requestId, path, userId) {
        try {
            mkdirSync(this.resolveUserStoragePath(path, this.resolveUserId(userId)), { recursive: true });
            this.respond(requestId, true);
        }
        catch (error) {
            this.fail(requestId, error);
    ...
    handleUserStorageMove(requestId, from, to, userId) {
        try {
            const resolvedUserId = this.resolveUserId(userId);
    ...
    handleUserStorageStat(requestId, path, userId) {
        try {
            this.statResponse(requestId, this.resolveUserStoragePath(path, this.resolveUserId(userId)));
        }
        catch (error) {
            this.fail(requestId, error);
        }
    }
    validateEnclaveKey(key) {
    ...
    validateEnclaveValue(value) {
        if (typeof value !== "string")
            throw new Error("Enclave value must be a string");
        if (Buffer.byteLength(value, "utf-8") > ENCLAVE_MAX_VALUE_BYTES)
    ...
    enclaveKey(key) { return `spindle:${this.identifier}:${key}`; }
    async handleEnclavePut(requestId, key, value, userId) {
        try {
    ...
    async handleEnclaveGet(requestId, key, userId) {
        try {
            this.validateEnclaveKey(key);
            this.respond(requestId, await getSecret(this.resolveUserId(userId), this.enclaveKey(key)));
        }
        catch (error) {
            this.fail(requestId, error);
        }
    }
    handleEnclaveDelete(requestId, key, userId) {
        try {
            this.validateEnclaveKey(key);
            this.respond(requestId, deleteSecret(this.resolveUserId(userId), this.enclaveKey(key)));
        }
        catch (error) {
            this.fail(requestId, error);
        }
    }
    async handleEnclaveHas(requestId, key, userId) {
        try {
            this.validateEnclaveKey(key);
            this.respond(requestId, await validateSecret(this.resolveUserId(userId), this.enclaveKey(key)));
        }
        catch (error) {
            this.fail(requestId, error);
        }
    }
    handleEnclaveList(requestId, userId) {
        try {
    ...
    resolveEphemeralPath(requestedPath) {
        return this.resolveWithin(this.getEphemeralBasePath(), requestedPath);
    }
    getReservationsPath(identifier = this.identifier, storageRoot) {
        if (identifier === this.identifier && !storageRoot)
            this.getEphemeralBasePath();
        const base = resolve(storageRoot ?? this.getStorageRootPath(identifier), ".ephemeral");
        mkdirSync(base, { recursive: true });
        return join(base, ".reservations.json");
    }
    readReservations(identifier = this.identifier, storageRoot) {
        const path = this.getReservationsPath(identifier, storageRoot);
        if (!existsSync(path))
            return [];
        try {
            const rows = JSON.parse(readFileSync(path, "utf-8"));
            return Array.isArray(rows) ? rows.filter((row) => Boolean(row) && typeof row.id === "string" && typeof row.sizeBytes === "number" && typeof row.consumedBytes === "number" && typeof row.createdAt === "string" && typeof row.expiresAt === "string") : [];
        }
        catch {
            return [];
        }
    }
    writeReservations(rows, identifier = this.identifier, storageRoot) {
        writeFileSync(this.getReservationsPath(identifier, storageRoot), JSON.stringify(rows, null, 2), "utf-8");
    }
    clearExpiredReservations(identifier = this.identifier) {
    ...
    getIndexPath() { return join(this.getEphemeralBasePath(), ".index.json"); }
    readIndex() {
    ...
    pathKey(fullPath) { return relative(this.getEphemeralBasePath(), fullPath).replaceAll("\\", "/"); }
    upsertIndex(pathKey, sizeBytes, ttlMs) {
    ...
    removeIndex(pathKey) { const index = this.readIndex(); delete index[pathKey]; this.writeIndex(index); }
    usage() {
        ...
        if (existsSync(base)) {
            for (const entry of readdirSync(base, { recursive: true })) {
                ...
                if (fullPath === indexPath || fullPath === reservationsPath)
                    continue;
                try {
                    const stat = statSync(fullPath);
                    if (stat.isFile())
                        files.set(this.pathKey(fullPath), stat.size);
                }
                catch { /* unreadable files do not count */ }
            }
        }
    ...
    async globalUsage() {
        ...
        for (const extension of await managerSvc.list()) {
            const usage = this.usageForExtension(extension);
    ...
    clearExpiredEntries() {
        ...
        for (const [pathKey, metadata] of Object.entries(index)) {
            ...
            if (!metadata.expiresAt || Number.isNaN(expiresAt) || expiresAt > Date.now())
                continue;
    ...
    async enforceQuota(pathKey, incomingSize, reservationId) {
        ...
        if (nextCount > EPHEMERAL_MAX_FILES)
            throw new Error(`Ephemeral storage file quota exceeded (${nextCount}/${EPHEMERAL_MAX_FILES})`);
        if (nextTotal + current.reservedBytes - consumed > extensionMax)
            throw new Error(`Ephemeral extension pool exceeded (${nextTotal + current.reservedBytes - consumed}/${extensionMax} bytes)`);
        const nextGlobal = global.usedBytes - existingSize + incomingSize;
        if (nextGlobal + global.reservedBytes - consumed > config.globalMaxBytes)
            throw new Error(`Ephemeral global pool exceeded (${nextGlobal + global.reservedBytes - consumed}/${config.globalMaxBytes} bytes)`);
        return consumed;
    }
    consumeReservation(reservationId, size) {
        if (size <= 0)
            return;
    ...
    handleEphemeralRead(requestId, path) {
        try {
    ...
    async handleEphemeralWrite(requestId, path, data, ttlMs, reservationId) {
        try {
            const fullPath = this.resolveEphemeralPath(path);
            const pathKey = this.pathKey(fullPath);
            const consumed = await this.enforceQuota(pathKey, Buffer.byteLength(data, "utf-8"), reservationId);
            mkdirSync(resolve(fullPath, ".."), { recursive: true });
            writeFileSync(fullPath, data, "utf-8");
            this.upsertIndex(pathKey, Buffer.byteLength(data, "utf-8"), ttlMs);
            if (reservationId)
                this.consumeReservation(reservationId, consumed);
            this.respond(requestId, true);
        }
        catch (error) {
            this.fail(requestId, error);
    ...
    handleEphemeralReadBinary(requestId, path) {
        try {
            const fullPath = this.resolveEphemeralPath(path);
            if (!existsSync(fullPath))
                return this.fail(requestId, "File not found");
    ...
    async handleEphemeralWriteBinary(requestId, path, data, ttlMs, reservationId) {
        try {
            const fullPath = this.resolveEphemeralPath(path);
            const pathKey = this.pathKey(fullPath);
            ...
            mkdirSync(resolve(fullPath, ".."), { recursive: true });
            writeFileSync(fullPath, data);
            this.upsertIndex(pathKey, data.byteLength, ttlMs);
            if (reservationId)
                this.consumeReservation(reservationId, consumed);
            this.respond(requestId, true);
        }
    ...
    handleEphemeralDelete(requestId, path) {
        try {
            const fullPath = this.resolveEphemeralPath(path);
            if (existsSync(fullPath))
                unlinkSync(fullPath);
            this.removeIndex(this.pathKey(fullPath));
            this.respond(requestId, true);
        }
        catch (error) {
            this.fail(requestId, error);
    ...
    handleEphemeralList(requestId, prefix) {
        try {
            this.respond(requestId, this.listFiles(prefix ? this.resolveEphemeralPath(prefix) : this.getEphemeralBasePath(), new Set([".index.json", ".reservations.json"])));
        }
        catch (error) {
            this.fail(requestId, error);
        }
    }
    handleEphemeralStat(requestId, path) {
        try {
            const fullPath = this.resolveEphemeralPath(path);
            if (!existsSync(fullPath))
                return this.fail(requestId, "File not found");
            const stat = statSync(fullPath);
            const metadata = this.readIndex()[this.pathKey(fullPath)];
    ...
    handleEphemeralClearExpired(requestId) {
        try {
            this.respond(requestId, this.clearExpiredEntries() + this.clearExpiredReservations());
        }
        catch (error) {
            this.fail(requestId, error);
        }
    }
    async handleEphemeralPoolStatus(requestId) {
        try {
            this.clearExpiredEntries();
            this.clearExpiredReservations();
            const extension = this.usage();
            const global = await this.globalUsage();
            const config = await getEphemeralPoolConfig();
            const extensionMax = config.extensionMaxOverrides[this.identifier] ?? config.extensionDefaultMaxBytes;
    ...
    async handleEphemeralRequestBlock(requestId, sizeBytes, ttlMs, reason) {
        try {
            ...
            if (sizeBytes > extensionAvailable)
                throw new Error(`Requested block exceeds extension available pool (${sizeBytes}/${Math.max(0, extensionAvailable)} bytes)`);
            if (sizeBytes > globalAvailable)
                throw new Error(`Requested block exceeds global available pool (${sizeBytes}/${Math.max(0, globalAvailable)} bytes)`);
    ...
    handleEphemeralReleaseBlock(requestId, reservationId) {
        try {
            this.writeReservations(this.readReservations().filter((row) => row.id !== reservationId));
            this.respond(requestId, true);
        }
        catch (error) {
            this.fail(requestId, error);
