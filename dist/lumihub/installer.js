...
import { resolveSealedPresetBlocksForInstall } from "./sealed-presets";
 ...
 */
export async function installCharacter(requestId, userId, payload) {
    if (!userId) {
        return {
            requestId,
            success: false,
...
function stampInstallSource(userId, characterId, payload) {
    try {
        const character = svc.getCharacter(userId, characterId);
        if (!character)
            return;
        const { buildCharacterSlug } = require("./manifest");
 ...
 */
async function importGalleryFromUrls(userId, characterId, urls) {
    ...
    const downloaded = await mapWithConcurrency(urls, 6, async (url) => {
        try {
    ...
    const files = downloaded.filter((f) => f !== null);
    if (files.length === 0)
        return;
    if (files.length > 3) {
 ...
 */
function maybeExtractWorldbook(userId, characterId, characterName, payload) {
    if (!payload.importEmbeddedWorldbook)
        return;
    const character = svc.getCharacter(userId, characterId);
    ...
    try {
        ...
        const nextExtensions = setCharacterWorldBookIds({ ...(character.extensions || {}) }, [...currentIds, worldBook.id]);
...
function notifyCharacterCreated(userId, characterId) {
    const character = svc.getCharacter(userId, characterId);
    if (character) {
...
async function installFromCardData(requestId, userId, payload) {
 ...
 */
async function installFromUrl(requestId, userId, payload) {
...
function importCharacterRegexBestEffort(userId, characterId, extensions) {
    try {
...
async function installFromChub(requestId, userId, payload) {
 ...
 */
export async function installWorldbook(requestId, userId, payload) {
    if (!userId) {
    ...
    try {
        ...
        try {
            ...
            if (wb) {
                ...
                const sourceCreator = (typeof payload.worldbookCreator === "string" && payload.worldbookCreator.trim())
...
export async function installTheme(requestId, userId, payload) {
    if (!userId) {
        return { requestId, success: false, error: "No target user configured for this LumiHub install" };
    }
    try {
...
export async function installPreset(requestId, userId, payload, dependencies = {}) {
    if (!userId) {
        return { requestId, success: false, error: "No target user configured for this LumiHub install" };
    }
    try {
        ...
        const presetVersion = typeof p.presetVersion === "string" ? p.presetVersion
        ...
        const materializedBlocks = await materializeSealedPresetBlocks(userId, blocks, payload.presetId, sealedPresetVersion, sealedPreset, dependencies.resolveSealedBlocks ?? resolveSealedPresetBlocksForInstall);
        ...
        const coverUrl = typeof exported.cover_url === "string" ? exported.cover_url
        ...
        const promptVariables = existing
            ? mergePromptVariableSelections(existing.prompt_order, materializedBlocks, existing.metadata?.promptVariables, incomingPromptVariables)
 ...
 * unresolved install.
 */
function resolveInstallSealedManifest(payload) {
...
function parseSealedManifest(value) {
    if (!isPlainObject(value) || !Array.isArray(value.blocks) || value.blocks.length > 200)
        return null;
    ...
    for (const entry of value.blocks) {
        if (!isPlainObject(entry) || typeof entry.key !== "string" || !entry.key || entry.key.length > 256)
            return null;
...
function sealedManifestsEqual(left, right) {
    if ((left.version ?? null) !== (right.version ?? null))
        return false;
    const normalize = (manifest) => (manifest.blocks ?? [])
 ...
 */
function extractPresetPassthroughMetadata(preset) {
...
function readPresetMetadataField(preset, fieldName) {
...
function clonePresetMetadata(value, fieldName) {
    try {
        return cloneSafePlainJsonObject(value);
    }
    catch {
        throw new Error(`Preset export has invalid ${fieldName}`);
    }
 ...
 */
function mergePromptVariableSelections(existingBlocksValue, updatedBlocksValue, existingValuesValue, incomingValuesValue) {
    const existingBlocks = Array.isArray(existingBlocksValue) ? existingBlocksValue : [];
    const updatedBlocks = Array.isArray(updatedBlocksValue) ? updatedBlocksValue : [];
    ...
    for (const [blockId, bucket] of Object.entries(incomingValues)) {
        if (isPlainObject(bucket))
            merged[blockId] = { ...bucket };
    }
    const existingBlockById = new Map(existingBlocks
        .filter((block) => isPlainObject(block) && typeof block.id === "string")
        .map((block) => [block.id, block]));
    for (const updatedBlock of updatedBlocks) {
        if (!isPlainObject(updatedBlock) || typeof updatedBlock.id !== "string" || !Array.isArray(updatedBlock.variables))
            continue;
        const existingBlock = existingBlockById.get(updatedBlock.id);
        if (!existingBlock || !Array.isArray(existingBlock.variables))
            continue;
        const existingBucket = existingValues[updatedBlock.id];
        if (!isPlainObject(existingBucket))
            continue;
...
function adaptPromptVariableSelection(oldDef, updatedDef, value) {
    ...
    switch (updatedFamily) {
        ...
        case "select": {
            if (updatedDef.type !== "select" || oldDef.type !== "select" || typeof value !== "string")
                return undefined;
            return mapPromptOptionId(oldDef, updatedDef, value);
        }
        case "multiselect": {
            if (updatedDef.type !== "multiselect" || oldDef.type !== "multiselect" || !Array.isArray(value))
                return undefined;
...
function promptVariableFamily(type) {
...
function mapPromptOptionId(oldDef, updatedDef, selectedId) {
    ...
    if (updatedOptions.some((option) => option.id === selectedId))
        return selectedId;
...
function isPromptVariableOption(value) {
    return isPlainObject(value)
        && typeof value.id === "string"
        && typeof value.label === "string"
        && typeof value.value === "string";
}
async function materializeSealedPresetBlocks(userId, blocks, hubPresetId, version, sealedPreset, resolveSealedBlocks) {
    const placeholderKeys = new Set();
    for (const block of blocks) {
        if (!isPlainObject(block) || typeof block.content !== "string")
            continue;
        const key = extractExactSealedPlaceholder(block.content);
        if (key)
            placeholderKeys.add(key);
    }
    if (!placeholderKeys.size)
        return blocks;
    if (!sealedPreset) {
...
function extractExactSealedPlaceholder(content) {
 ...
 */
function extractPresetRegexScripts(exported) {
    const candidates = [
...
function normalizeThemeConfig(value) {
    if (!isPlainObject(value))
        throw new Error("Theme export is missing theme data");
...
function normalizeThemeComponents(value) {
    if (!isPlainObject(value))
        return {};
    const out = {};
    for (const [name, raw] of Object.entries(value)) {
        if (!isPlainObject(raw) || typeof name !== "string" || !name.trim())
            continue;
...
async function importThemeAssets(userId, bundleId, assets) {
    if (!Array.isArray(assets) || assets.length === 0)
        return;
    if (assets.length > 500)
        throw new Error("Theme export contains too many assets");
    for (const raw of assets) {
        ...
        const tags = Array.isArray(raw.tags)
            ? raw.tags.filter((tag) => typeof tag === "string").slice(0, 32)
            : [];
