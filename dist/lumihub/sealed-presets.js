...
const cache = new Map();
export async function resolveSealedPresetBlock(userId, presetMetadata, blockKey) {
    ...
    const manifest = isPlainObject(presetMetadata._lumiverse_sealed_preset)
        ? presetMetadata._lumiverse_sealed_preset
        : null;
...
export async function resolveSealedPresetBlocksForInstall(userId, hubPresetId, version, manifest) {
    if (!hubPresetId || !manifest.blocks?.length)
        return {};
    return fetchSealedBlocks(userId, hubPresetId, version, manifest);
}
async function fetchSealedBlocks(userId, hubPresetId, version, manifest) {
    const config = await getLinkConfig(userId);
    if (!config)
        return {};
    return fetchVerifiedSealedBlocks({ lumihubUrl: config.lumihubUrl, linkToken: config.linkToken }, hubPresetId, version, manifest);
}
/**
 ...
 */
export async function fetchVerifiedSealedBlocks(config, hubPresetId, version, manifest, request = (url, headers) => safeFetch(url, {
    headers,
    timeoutMs: 15_000,
