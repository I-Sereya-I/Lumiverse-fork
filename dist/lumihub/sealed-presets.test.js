...
import { fetchVerifiedSealedBlocks } from "./sealed-presets";
describe("LumiHub sealed preset wire contract", () => {
    test("sends the linked-instance token, requests the exact version, and verifies content", async () => {
        ...
        try {
            const blocks = await fetchVerifiedSealedBlocks({ lumihubUrl: server.url.toString(), linkToken: "instance-secret" }, "preset/id", "v 2", { version: "v 2", blocks: [{ key: "dialogue.frame", sha256 }] }, (url, headers) => fetch(url, { headers }));
    ...
    test("rejects a Hub response whose content does not match the manifest", async () => {
        ...
        await expect(fetchVerifiedSealedBlocks({ lumihubUrl: "https://hub.example", linkToken: "token" }, "preset-id", "1", manifest, async () => Response.json({ blocks: { private: "tampered" } }))).rejects.toThrow("failed hash verification: private");
    });
});
