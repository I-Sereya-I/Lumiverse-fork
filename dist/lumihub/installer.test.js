...
function installPreset(requestId, payload, userId = USER_ID, dependencies = {}) {
    return installPresetForUser(requestId, userId, payload, dependencies);
}
function initInstallerTestDb() {
    closeDatabase();
    initDatabase(":memory:");
    const db = getDb();
...
function installPayload(presetId, preset) {
    return {
        source: "lumihub",
        presetId,
...
describe("LumiHub preset installer metadata", () => {
    ...
    test("keeps older bundled regexes disabled without touching a same-named local folder", async () => {
        ...
        expect(getRegexScript(USER_ID, local.id)).toMatchObject({
            disabled: false,
            folder: "Hub preset",
            preset_id: null,
            metadata: {},
    ...
    test("rejects metadata with an arbitrary prototype", () => {
        const metadata = Object.create({ inherited: true });
        ...
        if (!validation.ok)
            expect(validation.error).toContain("passthroughMetadata");
    });
    ...
    test("updates the installed row while retaining user configuration", async () => {
        ...
        getDb().query("INSERT INTO settings (key, value, user_id, updated_at) VALUES (?, ?, ?, ?)").run("presetProfileDefaults:" + installed.id, JSON.stringify(binding), USER_ID, 123);
        ...
        const savedBinding = getDb().query("SELECT value FROM settings WHERE key = ? AND user_id = ?").get("presetProfileDefaults:" + installed.id, USER_ID);
    ...
    test("uses the manifest slug to update a LumiHub install whose Hub id changed", async () => {
        ...
        const count = getDb().query("SELECT COUNT(*) AS count FROM presets WHERE user_id = ?").get(USER_ID);
