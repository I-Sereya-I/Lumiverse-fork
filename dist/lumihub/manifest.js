...
export { selectLumiHubManifestEntries } from "./manifest-policy";
/**
...
export function buildSlug(creator, name) {
 ...
 */
export function resolvePresetSlug(metadata, name) {
    const md = metadata || {};
    const storedSlug = typeof md._lumiverse_preset_slug === "string" ? md._lumiverse_preset_slug : null;
    const creator = typeof md._lumiverse_preset_creator === "string" && md._lumiverse_preset_creator
 ...
 */
export function buildInstallManifest(userId) {
    const entries = [];
    // Characters
    const characters = charactersSvc.listCharactersForManifest(userId);
    for (const char of characters) {
        const source = char.extensions?._lumiverse_install_source;
        // Prefer canonical Chub slug (fullPath) so it matches LumiHub's card.id lookup
        const chubSlug = char.extensions?._lumiverse_chub_slug;
    ...
    for (const wb of worldBooks) {
        const source = wb.metadata?._lumiverse_install_source;
        const creator = wb.metadata?.source_creator || "unknown";
    ...
    for (const pr of presets) {
        ...
        const source = md._lumiverse_install_source;
