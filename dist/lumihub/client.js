...
import { validateInstallCharacterPayload, validateInstallPresetPayload, validateInstallThemePayload, validateInstallWorldbookPayload, } from "./payload-validation";
...
class LumiHubWSClient {
    ...
    destroy() {
        this.disconnect();
        for (const dispose of this.eventListenerDisposers)
            dispose();
    ...
    async handleInstallCharacter(msg) {
    ...
    async handleInstallWorldbook(msg) {
    ...
    syncStats() {
        try {
            if (!this.connected || !isStatsSharingEnabled(this.userId))
                return;
    ...
    debouncedManifestSync() {
        if (this.manifestSyncTimer)
            clearTimeout(this.manifestSyncTimer);
    ...
    registerManifestListeners() {
        if (this.eventListenersRegistered)
            return;
        this.eventListenersRegistered = true;
        const trigger = (event) => {
            if (event.userId === this.userId)
                this.debouncedManifestSync();
        };
        this.eventListenerDisposers.push(eventBus.on(EventType.CHARACTER_CREATED, trigger), eventBus.on(EventType.CHARACTER_EDITED, trigger), eventBus.on(EventType.CHARACTER_DELETED, trigger), eventBus.on(EventType.CHARACTER_LIBRARY_CHANGED, trigger), eventBus.on(EventType.PRESET_CHANGED, trigger), eventBus.on(EventType.PRESET_DELETED, trigger), eventBus.on(EventType.LUMIHUB_INSTALL_COMPLETED, trigger));
    }
    send(msg) {
...
export function getLumiHubClient(userId) {
...
export function deleteLumiHubClient(userId) {
    clients.get(userId)?.destroy();
    clients.delete(userId);
}
export function disconnectAllLumiHubClients() {
