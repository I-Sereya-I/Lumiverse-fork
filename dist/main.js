...
import { startExtensionUpdateMonitor, stopExtensionUpdateMonitor, } from "./spindle/update-check.service";
...
const { describeImageProcessingRecovery, getImageProcessingRecovery, } = await import("./services/images.service");
const leftoverThumbnails = getImageProcessingRecovery();
if (leftoverThumbnails.pending > 0) {
    console.warn(`[startup] ${describeImageProcessingRecovery(leftoverThumbnails)}. Not auto-started — recover from Operator → Image Processing after changing settings if needed.`);
}
...
if (presetBackfill.seeded > 0 || presetBackfill.upgradedLegacy > 0 || presetBackfill.activated > 0) {
    console.log(`[Auth] Default preset backfill: seeded ${presetBackfill.seeded}, upgraded ${presetBackfill.upgradedLegacy}, activated ${presetBackfill.activated}`);
}
console.log(`[startup] Runner IPC: ${operatorService.ipcAvailable ? "connected" : `unavailable (${operatorService.ipcReason})`}`);
...
const { load: loadTrustedHosts, getSnapshot: getTrustedHostsSnapshot, detectHostnameSuggestions, } = await import("./services/trusted-hosts.service");
loadTrustedHosts();
...
const { load: loadApprovedBrokerOrigins, getApprovedBrokerOrigins, } = await import("./services/broker-origins.service");
...
startAutomaticDatabaseMaintenance(() => db, () => getFirstUserId(), () => getDatabasePath(), () => operatorService.busy, (name, fn) => operatorService.runOperation(name, fn));
...
const lancedbStartupMaintenanceEnabled = !["0", "false", "no", "off"].includes((process.env.LUMIVERSE_LANCEDB_STARTUP_MAINTENANCE ?? "").trim().toLowerCase());
if (lancedbStartupMaintenanceEnabled) {
    setTimeout(() => {
        console.info("[startup] Starting deferred LanceDB maintenance child...");
        import("./services/lancedb-maintenance-supervisor").then(({ runLanceDbMaintenanceInChild }) => runLanceDbMaintenanceInChild({ mode: "startup" })).catch((err) => {
...
setTimeout(() => {
    import("./services/mcp-client-manager").then(({ getMcpClientManager }) => {
        getMcpClientManager().autoConnectAll().catch((err) => console.error("[MCP] Auto-connect failed:", err));
    });
}, 0);
setTimeout(() => {
    import("./services/characters.service").then(({ resumePendingCharacterDeletions }) => {
        resumePendingCharacterDeletions().catch((err) => console.warn("[characters] deletion resume failed:", err instanceof Error ? err.message : err));
    });
}, 5_000);
...
async function gracefulShutdown(signal) {
    ...
    await stopAllExtensions().catch((err) => console.error("[Shutdown] Extension stop error:", err));
