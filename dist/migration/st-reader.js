...
export async function readPNGCharaName(filePath, fs = defaultFs) {
    try {
        ...
        while (offset + 12 <= buf.length) {
            ...
            try {
                ...
                if (value !== null)
                    return extractNameFromBase64JSON(value, keyword);
            }
            catch {
...
function decodeTextChunk(type, data, nullIdx) {
    if (type === "tEXt") {
...
function extractNameFromBase64JSON(raw, keyword) {
    try {
        const decoded = Buffer.from(raw.trim(), "base64").toString("utf8");
        const parsed = JSON.parse(decoded);
        const name = parsed.data?.name || parsed.name || null;
...
export function parseMessageDate(msg) {
    ...
    if (typeof sendDate === "number") {
        if (sendDate > 1_000_000_000_000)
            return Math.floor(sendDate / 1000);
        if (sendDate > 1_000_000_000)
            return Math.floor(sendDate);
        return Math.floor(Date.now() / 1000);
    }
...
export function resolveOriginalAvatarToCharacterId(originalAvatar, filenameToId) {
    if (!filenameToId || filenameToId.size === 0)
        return undefined;
    if (typeof originalAvatar !== "string" || originalAvatar.length === 0)
        return undefined;
...
function parseStHeader(firstLine) {
    try {
...
export function parseStChatJsonl(raw, fallbackName) {
    ...
    for (let i = header.hasHeader ? 1 : 0; i < lines.length; i++) {
        try {
            ...
            const msgSwipes = Array.isArray(msg.swipes) ? msg.swipes : undefined;
...
export function parseStGroupChatJsonl(raw, fallbackName, personaNameToId, filenameToId) {
    ...
    const messages = parsed.messages.map((message) => {
        ...
        if (!message.is_user) {
            ...
            if (!avatarCharId && nameCharId)
                speakerNameFallbackCount++;
            if (charId)
...
// ─── Directory scanners ─────────────────────────────────────────────────────
export async function scanSTData(stDataDir, fs = defaultFs) {
    const counts = {
        characters: 0,
        chatDirs: 0,
        totalChatFiles: 0,
    ...
    if (await fs.exists(charsDir)) {
        ...
        counts.characters = entries.filter((e) => e.isFile && fs.extname(e.name).toLowerCase() === ".png").length;
    }
    ...
    if (await fs.exists(chatsDir)) {
        ...
        for (const dir of charDirs) {
            ...
            counts.totalChatFiles += chatEntries.filter((e) => e.isFile && fs.extname(e.name).toLowerCase() === ".jsonl").length;
        }
    }
    const groupsDir = fs.join(stDataDir, "groups");
    if (await fs.exists(groupsDir)) {
        const entries = await fs.readdir(groupsDir);
        counts.groupChats = entries.filter((e) => e.isFile && fs.extname(e.name).toLowerCase() === ".json").length;
    }
    ...
    if (await fs.exists(groupChatsDir)) {
        ...
        counts.groupChatFiles = entries.filter((e) => e.isFile && fs.extname(e.name).toLowerCase() === ".jsonl").length;
    }
    const worldsDir = fs.join(stDataDir, "worlds");
    if (await fs.exists(worldsDir)) {
        const entries = await fs.readdir(worldsDir);
        counts.worldBooks = entries.filter((e) => e.isFile && fs.extname(e.name).toLowerCase() === ".json").length;
    }
...
export async function readConnectionsFromDisk(stDataDir, fs = defaultFs) {
    const settingsPath = fs.join(stDataDir, "settings.json");
    if (!(await fs.exists(settingsPath)))
        return [];
    try {
...
export async function readProxiesFromDisk(stDataDir, fs = defaultFs) {
    const settingsPath = fs.join(stDataDir, "settings.json");
    if (!(await fs.exists(settingsPath)))
        return [];
    try {
        ...
        return Array.isArray(settings?.proxies)
            ? settings.proxies.filter((proxy) => !!proxy && typeof proxy === "object")
            : [];
    }
    catch {
        return [];
...
export async function readSecretsFromDisk(stDataDir, fs = defaultFs) {
    ...
    try {
        ...
        return secrets && typeof secrets === "object" && !Array.isArray(secrets) ? secrets : {};
    }
    catch {
        return {};
    }
}
export async function scanCharacterPNGs(charsDir, logger, fs = defaultFs) {
    const entries = await fs.readdir(charsDir);
    const pngFiles = entries.filter((e) => e.isFile && fs.extname(e.name).toLowerCase() === ".png");
    const results = [];
...
// ─── Data readers ───────────────────────────────────────────────────────────
export async function readWorldBooksFromDisk(stDataDir, logger, fs = defaultFs) {
    const worldsDir = fs.join(stDataDir, "worlds");
    if (!(await fs.exists(worldsDir)))
        return [];
    const entries = await fs.readdir(worldsDir);
    const jsonFiles = entries.filter((e) => e.isFile && fs.extname(e.name).toLowerCase() === ".json");
    const results = [];
...
export async function readPersonasFromDisk(stDataDir, fs = defaultFs) {
    const settingsPath = fs.join(stDataDir, "settings.json");
    if (!(await fs.exists(settingsPath)))
        return [];
    let personaNames;
 ...
 */
export async function readChatsForCharacter(stDataDir, charDirName, personaNameToId, filenameToId, logger, fs = defaultFs) {
    const chatsDir = fs.join(stDataDir, "chats", charDirName);
    if (!(await fs.exists(chatsDir)))
        return [];
    const entries = await fs.readdir(chatsDir);
    const chatFiles = entries.filter((e) => e.isFile && fs.extname(e.name).toLowerCase() === ".jsonl");
    const results = [];
    for (const chatFileEntry of chatFiles) {
        const filePath = fs.join(chatsDir, chatFileEntry.name);
        try {
            const parsed = parseStGroupChatJsonl(await fs.readText(filePath), fs.basename(chatFileEntry.name, ".jsonl"), personaNameToId, filenameToId);
            if (parsed) {
...
export async function readCharacterChatFile({ stDataDir, charDirName, chatFileName, personaNameToId, filenameToId, fs = defaultFs, }) {
    const filePath = fs.join(stDataDir, "chats", charDirName, chatFileName);
    try {
        const parsed = parseStGroupChatJsonl(await fs.readText(filePath), fs.basename(chatFileName, ".jsonl"), personaNameToId, filenameToId);
        if (!parsed)
            return null;
...
export async function readGroupDefinitions(stDataDir, fs = defaultFs) {
    const groupsDir = fs.join(stDataDir, "groups");
    if (!(await fs.exists(groupsDir)))
        return [];
    const entries = await fs.readdir(groupsDir);
    const groupFiles = entries.filter((e) => e.isFile && fs.extname(e.name).toLowerCase() === ".json");
    const results = [];
...
export async function readGroupChatFileEntries(stDataDir, fs = defaultFs) {
    const groupChatsDir = fs.join(stDataDir, "group chats");
    if (!(await fs.exists(groupChatsDir)))
        return [];
    const entries = await fs.readdir(groupChatsDir);
 ...
 */
export async function readGroupChatFile(stDataDir, chatId, personaNameToId, filenameToId, fs = defaultFs) {
    ...
    if (!(await fs.exists(chatFilePath)))
        return null;
    try {
