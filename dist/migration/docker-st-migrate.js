...
import { importCharacters, importWorldBooks, importPersonas, importChats, importGroupChats, } from "./st-importer";
...
function resolveTargetFlags(target) {
    switch (target) {
...
// ─── Orchestrator ───────────────────────────────────────────────────────────
export async function runDockerSTMigration() {
    ...
    try {
        ...
        if (existingStatus && !env.stForceNewMigration) {
            const status = existingStatus.value;
        ...
        logger.info(`Scan: ${counts.characters} characters, ${counts.totalChatFiles} chats, ` +
