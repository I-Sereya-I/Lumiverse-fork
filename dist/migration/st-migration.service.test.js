...
import { canUseIsolatedStMigration, executeMigration, getLastMigration, resetIsolatedStMigrationState, startStMigration, } from "./st-migration.service";
import { eventBus } from "../ws/bus";
import { EventType } from "../ws/events";
const USER_ID = "st-migration-host-user";
const originalDataDir = env.dataDir;
let workDir = "";
function allOffScope(overrides = {}) {
    return {
        characters: false,
        worldBooks: false,
        personas: false,
...
describe("startStMigration isolation", () => {
    test("falls back in-process when isolated spawn fails to become ready", async () => {
        ...
        await startStMigration("mig-fallback", USER_ID, USER_ID, dataDir, allOffScope(), { type: "local" }, undefined, spawnDeps(spawn));
    ...
    test("relays isolated IPC progress and completion without in-process scan", async () => {
        ...
        await startStMigration("mig-ipc", USER_ID, USER_ID, dataDir, allOffScope({ characters: true }), { type: "local" }, undefined, spawnDeps(spawn));
        const last = getLastMigration();
    ...
    test("relays isolated thumbnailQueue IPC onto the host event bus", async () => {
        ...
        await startStMigration("mig-thumbs", USER_ID, USER_ID, join(workDir, "st-data"), allOffScope(), { type: "local" }, undefined, spawnDeps(spawn));
