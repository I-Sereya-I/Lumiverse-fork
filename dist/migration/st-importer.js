...
import { readWorldBooksFromDisk, readPersonasFromDisk, readCharacterChatFile, readGroupDefinitions, readGroupChatFileEntries, readGroupChatFile, parseDateString, } from "./st-reader";
import { mapWithConcurrency } from "./st-concurrency";
import { extractCardFromPng } from "../services/character-card.service";
import { createCharacter, listCharacterSourceFilenameIds, } from "../services/characters.service";
...
import { emitWorldBookLibraryChanged, importWorldBookBulk, listSillyTavernWorldBookSourceFilenameIds, } from "../services/world-books.service";
...
export async function importCharacters(userId, stDataDir, logger, fs = defaultFs) {
    ...
    for (let batchStart = 0; batchStart < pngFiles.length; batchStart += characterBatchSize()) {
        ...
        const prepared = await mapWithConcurrency(batch, characterReadConcurrency(), async (entry) => {
            const filename = entry.name;
            const stem = fs.basename(filename, ".png");
            const existingId = existingByFilename.get(filename);
            if (existingId)
                return { kind: "existing", filename, stem, characterId: existingId };
            try {
        ...
        if (created.length > 0) {
            try {
                const avatarResults = await uploadImages(userId, created.map((item) => ({
                    data: item.bytes,
                    filename: item.filename,
                    mime_type: "image/png",
                ...
                const attachAvatar = getDb().query(`UPDATE characters
...
export async function importWorldBooks(userId, stDataDir, logger, fs = defaultFs) {
...
export async function importPersonas(userId, stDataDir, worldBookNameToId, logger, fs = defaultFs) {
    const nameToId = new Map();
    let imported = 0;
    let skipped = 0;
    let failed = 0;
    ...
    if (pendingAvatars.length > 0) {
        try {
            const avatarResults = await uploadImages(userId, pendingAvatars.map((item) => ({
                data: item.bytes,
...
export async function importChats(userId, stDataDir, filenameToId, personaNameToId, logger, fs = defaultFs) {
    ...
    const chatItems = work.filter((item) => item.kind === "chat");
    for (const item of work) {
        if (item.kind !== "char-missing")
            continue;
        skippedChars++;
    ...
    for (let batchStart = 0; batchStart < chatItems.length; batchStart += characterBatchSize()) {
        ...
        const prepared = await mapWithConcurrency(batch, chatReadConcurrency(), async (item) => {
            ...
            try {
                ...
                if (!chatData) {
                    return { kind: "unreadable", label: `${item.charDirName}/${item.fileName}` };
                }
                return {
                    kind: "ready",
                    item,
...
export async function importGroupChats(userId, stDataDir, filenameToId, personaNameToId, logger, fs = defaultFs) {
    let imported = 0;
    let failed = 0;
    let skipped = 0;
    let totalMessages = 0;
    ...
    for (const group of groupDefs) {
        for (const chatId of group.chatIds) {
            referencedChatIds.add(chatId.toLowerCase().endsWith(".jsonl") ? fs.basename(chatId, ".jsonl") : chatId);
        }
    }
    ...
    if (unreferencedGroupChatFiles.length > 0) {
        ...
        logger.warn(`${unreferencedGroupChatFiles.length} group chat file(s) were not listed in any groups/*.json chats array and could not be matched to a group`);
    }
    const existingBySource = listChatSourceFilenameIds(userId);
    const work = [];
    ...
    for (let batchStart = 0; batchStart < work.length; batchStart += characterBatchSize()) {
        ...
        const prepared = await mapWithConcurrency(batch, groupChatReadConcurrency(), async (item) => {
            ...
            if (!chatData) {
                return { kind: "unreadable", label: `${item.groupName}/${item.chatId}` };
            }
            return { kind: "ready", item, sourceKey, chatData };
        });
        getDb().transaction(() => {
            for (const preparedItem of prepared) {
                if (preparedItem.kind === "skipped") {
