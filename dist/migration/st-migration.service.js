...
let isolatedUnavailableReason = null;
...
export function canUseIsolatedStMigration(env = process.env) {
    if (env.LUMIVERSE_ST_MIGRATION_SUBPROCESS === "false")
        return false;
    if (isolatedUnavailableReason)
        return false;
    return true;
}
export function resetIsolatedStMigrationState() {
    isolatedUnavailableReason = null;
    warnedIsolatedFallback = false;
}
function warnIsolatedFallback() {
    if (warnedIsolatedFallback || canUseIsolatedStMigration())
        return;
...
function appendLog(migrationId, level, message) {
...
function createWsLogger(migrationId, callerUserId) {
    let lastProgressEmit = null;
    const updateProgress = (phase, label, current, total) => {
        const progress = {
            phase,
            label,
            current,
...
function beginMigrationState(migrationId, callerUserId, targetUserId) {
    const state = {
        migrationId,
        callerUserId,
        targetUserId,
...
function setMigrationPhase(state, phase) {
    state.phase = phase;
    state.progress = {
        phase,
        label: phase,
...
function finishMigrationSuccess(state, results, durationMs, logger) {
...
function finishMigrationFailure(state, errorMsg, logger) {
...
function emitCharacterLibraryChange(state, importedCharacterCount, characterImportAttempted) {
...
async function disconnectRemoteFs(fs) {
    if (fs.type === "local")
        return;
    try {
        await fs.disconnect();
...
export async function executeMigration(migrationId, callerUserId, targetUserId, dataDir, scope, fs = defaultFs) {
    ...
    try {
        const outcome = await runStMigrationPipeline(migrationId, targetUserId, dataDir, scope, logger, fs, { setPhase: (phase) => setMigrationPhase(state, phase) });
...
function defaultIsolatedSpawn(options) {
    return Bun.spawn(options);
}
async function runIsolatedMigration(job, deps) {
    ...
    const settle = async (error) => {
        ...
        if (error)
            finishMigrationFailure(state, error, logger);
        emitCharacterLibraryChange(state, importedCharacterCount, characterImportAttempted);
        currentMigrationId = null;
        if (child) {
            try {
...
export async function startStMigration(migrationId, callerUserId, targetUserId, dataDir, scope, connection = { type: "local" }, fs, deps) {
    const job = {
        migrationId,
        callerUserId,
        targetUserId,
