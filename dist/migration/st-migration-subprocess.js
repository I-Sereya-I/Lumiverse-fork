...
process.on("message", (message) => {
    ...
    void (async () => {
        ...
        try {
            await initialized;
            const outcome = await withFileSystem(job.connection, (fs) => runStMigrationPipeline(job.migrationId, job.targetUserId, job.dataDir, job.scope, logger, fs, {
                setPhase: (phase) => {
...
void initialized.then(() => send({ type: "ready" }), (err) => {
    send({
        type: "failed",
        error: err instanceof Error ? err.message : String(err),
