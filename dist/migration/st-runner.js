...
import { clearStMigrationCheckpoint, loadStMigrationCheckpoint, markStMigrationPhase, saveStMigrationCheckpoint, } from "./st-checkpoint";
import { importCharacters, importChats, importGroupChats, importPersonas, importWorldBooks, } from "./st-importer";
import { importSTConnections } from "./st-connections";
import { scanSTData } from "./st-reader";
export async function runStMigrationPipeline(migrationId, targetUserId, dataDir, scope, logger, fs, hooks = {}) {
    ...
    logger.info(`Using ${budget.logicalThreads} logical threads (reserve ${budget.reserved}): ${budget.workerConcurrency} import workers, ${budget.sharpConcurrency} Sharp threads, ${budget.deferredImageConcurrency} deferred thumbnails.`);
    try {
...
async function runStMigrationPipelineInner(migrationId, targetUserId, dataDir, scope, logger, fs, hooks) {
    ...
    const phaseDone = (phase) => checkpoint.completedPhases.includes(phase);
    ...
    if (scope.characters && counts.characters > 0) {
        if (phaseDone("characters")) {
            ...
            for (const [stem, id] of remapped)
                filenameToId.set(stem, id);
        }
        else {
