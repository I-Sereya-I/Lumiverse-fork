...
import { readConnectionsFromDisk, readProxiesFromDisk, readSecretsFromDisk, } from "./st-reader";
const normalized = (value) => typeof value === "string" ? value.trim() : "";
const normalizedName = (value) => normalized(value).replace(/\s+/g, " ");
function inferProvider(api, apiUrl) {
    const value = api.toLowerCase();
    const url = apiUrl.toLowerCase();
    if (value === "claude")
        return "anthropic";
    if (value === "google" || value === "makersuite")
        return "google";
    if (value === "vertexai")
        return "google_vertex";
    if (value === "openai")
        return url.includes("api.openai.com") || !url ? "openai" : "custom";
    if (value === "infermaticai")
        return "infermatic";
...
function secretLookup(secrets) {
    ...
    for (const [category, entries] of Object.entries(secrets)) {
        ...
        for (const entry of entries) {
            if (normalized(entry.id) && normalized(entry.value))
                byId.set(normalized(entry.id), normalized(entry.value));
        }
        const selected = entries.find((entry) => entry?.active && normalized(entry.value)) ?? entries.find((entry) => normalized(entry.value));
        if (selected && normalized(selected.value))
            activeByCategory.set(category, normalized(selected.value));
    }
    return { byId, activeByCategory };
}
function resolveProfileSecret(profile, provider, lookup) {
    ...
    if (provider === "anthropic")
        categories.push("api_key_claude");
    if (provider === "google")
        categories.push("api_key_makersuite", "api_key_makersuite_custom");
    if (provider === "google_vertex")
        categories.push("api_key_vertexai", "vertexai_service_account_json");
    if (provider === "custom")
        categories.push("api_key_custom", "api_key_generic");
    return categories.map((category) => lookup.activeByCategory.get(category)).find(Boolean);
}
function parseConnectionRow(row) {
...
export async function importSTConnections(userId, dataDir, options, logger, fs) {
    ...
    for (const profile of profiles) {
        ...
        if (normalized(record.mode).toLowerCase() === "tc") {
            skipped++;
            continue;
        }
    ...
    for (const proxy of proxies) {
        ...
        if (!proxyName || proxyName.toLowerCase() === "none" || !api_url) {
            skipped++;
            continue;
        }
        candidates.push({
    ...
    const existingRows = getDb().query("SELECT * FROM connection_profiles WHERE user_id = ?").all(userId);
    ...
    if (options.dryRun)
        return { imported: planned.length, repaired: 0, skipped, failed: 0, dry_run: true };
    const createdIds = [];
    const repaired = [];
    let imported = 0;
    let repairedCount = 0;
    try {
        for (let index = 0; index < planned.length; index++) {
            ...
            const repairTarget = options.repairExisting
                ? existing.find((profile) => profile.metadata.source === "sillytavern"
