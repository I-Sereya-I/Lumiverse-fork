...
import { listCharacterSourceFilenameIds, listCharacterSummaries, } from "../services/characters.service";
...
function cardPng(name) {
    ...
    return embedPngTextChunk(Buffer.from(ONE_BY_ONE_PNG_BASE64, "base64"), "ccv3", Buffer.from(card).toString("base64"));
}
beforeEach(async () => {
    closeDatabase();
    initDatabase(":memory:");
...
describe("SillyTavern world-book bulk migration", () => {
    ...
    test("uses the source-filename expression index for migration identity scans", () => {
        const plan = getDb().query(`EXPLAIN QUERY PLAN
       SELECT id, json_extract(metadata, '$._lumiverse_source_filename')
       FROM world_books
       WHERE user_id = ?
         AND json_extract(metadata, '$._lumiverse_source_filename') = ?`).all(USER_ID, "alpha.json");
...
describe("SillyTavern character bulk migration", () => {
    test("imports in bulk without per-row events and reruns from preloaded identities", async () => {
        ...
        try {
            ...
            const rows = getDb().query(`SELECT name, image_id, avatar_path,
                json_extract(extensions, '$._lumiverse_source_filename') AS source_filename
         FROM characters WHERE user_id = ? ORDER BY name`).all(USER_ID);
            ...
            expect(listCharacterSummaries(USER_ID, { limit: 10, offset: 0 }, { search: "Alice" }).data
                .map((character) => character.name)).toEqual(["Alice"]);
            const images = getDb().query("SELECT has_thumbnail, owner_character_id FROM images WHERE user_id = ?").all(USER_ID);
            ...
            const processedImages = getDb().query("SELECT has_thumbnail FROM images WHERE user_id = ?").all(USER_ID);
