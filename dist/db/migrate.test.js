...
describe("database migrations", () => {
    test("fresh bootstrap applies the preset cache revision exactly once", async () => {
        ...
        try {
            ...
            const columns = db.query("PRAGMA table_info(presets)").all();
            expect(columns.some((column) => column.name === "cache_revision")).toBe(true);
            expect(db.query("SELECT name FROM _migrations WHERE name = ?").get("093_preset_cache_revision.sql")).toEqual({ name: "093_preset_cache_revision.sql" });
            expect(db.query("SELECT name FROM _migrations WHERE name = ?").get("094_regex_actions.sql")).toEqual({ name: "094_regex_actions.sql" });
            const regexColumns = db.query("PRAGMA table_info(regex_scripts)").all();
            ...
            expect(db.query("SELECT name FROM _migrations WHERE name = ?").get("101_regex_script_extension_ownership.sql")).toEqual({ name: "101_regex_script_extension_ownership.sql" });
            const linkColumns = db.query("PRAGMA table_info(lumihub_link)").all();
            expect(linkColumns.some((column) => column.name === "user_id")).toBe(true);
            expect(db.query("SELECT name FROM _migrations WHERE name = ?").get("095_lumihub_link_user_scope.sql")).toEqual({ name: "095_lumihub_link_user_scope.sql" });
            expect(db.query("SELECT name FROM _migrations WHERE name = ?").get("107_world_book_entry_order_index.sql")).toEqual({ name: "107_world_book_entry_order_index.sql" });
            const entryIndexes = db.query("PRAGMA index_list('world_book_entries')").all();
