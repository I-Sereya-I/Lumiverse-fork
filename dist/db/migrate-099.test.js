...
function libraryScopeColumn(db) {
    const columns = db.query("PRAGMA table_info('characters')").all();
...
function indexColumns(db, indexName) {
    const columns = db.query(`PRAGMA index_xinfo('${indexName}')`).all();
    return columns
...
describe("099 character library scope", () => {
    ...
    test("is recorded as already applied after a fresh baseline bootstrap", async () => {
        ...
        try {
            ...
            expect(db.query("PRAGMA table_info('characters')").all().some((column) => column.name === "library_scope")).toBe(true);
            expect(db.query("SELECT COUNT(*) AS count FROM _migrations WHERE name = ?").get(MIGRATION_099)).toEqual({ count: 1 });
            ...
            const indexNames = db.query("PRAGMA index_list('characters')").all().map((index) => index.name);
            ...
            db.run("INSERT INTO characters (id, name, library_scope) VALUES ('shared', 'Shared', 'shared')");
            expect(db.query("SELECT library_scope FROM characters WHERE id = 'shared'").get()).toEqual({
                library_scope: "shared",
            });
            expect(() => db.run("INSERT INTO characters (id, name, library_scope) VALUES ('invalid', 'Invalid', 'invalid')")).toThrow();
            await runMigrations(db);
            expect(db.query("SELECT COUNT(*) AS count FROM _migrations WHERE name = ?").get(MIGRATION_099)).toEqual({ count: 1 });
        }
        finally {
