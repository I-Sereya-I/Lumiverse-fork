...
function isInsideGitRepo(startPath) {
...
function shouldAllowCleanup(migrationsDir) {
    ...
    if (process.env.LUMIVERSE_PRUNE_MIGRATIONS === "true")
        return true;
    if (process.env.LUMIVERSE_PRUNE_MIGRATIONS === "false")
        return false;
    // Default: allow cleanup when not inside a git repo (release installs).
    return true;
}
function cleanupOldMigrations(migrationsDir, db) {
    if (!shouldAllowCleanup(migrationsDir))
        return;
    ...
    for (const name of BASELINE_MIGRATIONS) {
        if (!applied.has(name))
            return; // Baseline not fully applied — unsafe.
    }
    let removed = 0;
    for (const file of readdirSync(migrationsDir)) {
        if (!file.endsWith(".sql"))
            continue;
        if (!BASELINE_SET.has(file))
            continue; // Keep post-baseline migrations.
...
function repairDreamWeaverBaselineDrift(db) {
    const table = db
        .query("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'dream_weaver_sessions'")
        .get();
    if (!table)
        return;
    const columns = db.query("PRAGMA table_info('dream_weaver_sessions')").all();
...
function applyMigrationWithForeignKeysOff(db, file, sql) {
    ...
    try {
        ...
        if (violations.length > 0) {
            console.warn(`[db] WARNING: ${violations.length} foreign key violation(s) present after ${file} ` +
                `(database-wide check; orphaned rows may pre-date this migration). First:`, violations[0]);
        }
    }
    finally {
...
export async function runMigrations(db, migrationsDir) {
    ...
    const migrationCount = db.query("SELECT COUNT(*) as c FROM _migrations").get();
    ...
    const appliedBaseNames = new Set([...applied].map((a) => a.replace(/^\d+_/, "")));
