...
function seedGrants(db) {
    ...
    db.run(`INSERT INTO extension_grants (id, extension_id, permission) VALUES
      ('g-user', 'ext-user', 'tools'),
      ('g-op', 'ext-op', 'tools'),
      ('g-sys', 'ext-sys', 'tools')`);
}
function markMigrationsApplied(db, names) {
...
describe("102 spindle provider scope migration", () => {
    test("applies 102_spindle_provider_scope once on legacy databases and backfills scoped grants", async () => {
        ...
        try {
            ...
            const columns = db.query("PRAGMA table_info('extension_grants')").all();
            ...
            const indexNames = db.query("PRAGMA index_list('extension_grants')").all().map((index) => index.name);
            expect(indexNames).toContain("idx_extension_grants_scope");
            await runMigrations(db, migrationsDir);
            expect(db.query("SELECT COUNT(*) AS count FROM _migrations WHERE name = ?").get(MIGRATION_102)).toEqual({ count: 1 });
            expect(db.query("SELECT COUNT(*) AS count FROM extension_grants").get()).toEqual({ count: 3 });
        }
        finally {
            db.close();
        }
