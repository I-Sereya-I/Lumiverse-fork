...
function extractSchema(db) {
    ...
    for (const obj of objects) {
        if (obj.type === "table") {
            const cols = db.query(`PRAGMA table_info('${obj.name}')`).all();
            ...
            const cols = db.query(`PRAGMA index_info('${obj.name}')`).all().map((c) => c.name);
...
describe("baseline sync", () => {
    ...
    test("baseline carries the scoped extension grants contract", async () => {
        ...
        try {
            ...
            const indexes = db.query("PRAGMA index_list(extension_grants)").all();
            ...
            db.run(`INSERT INTO extensions (id, identifier, name, version, author, github)
