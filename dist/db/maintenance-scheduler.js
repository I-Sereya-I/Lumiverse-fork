import { collectDatabaseStats, getLatestDatabaseWriteAt, readDatabaseMaintenanceSettings, readDatabaseMaintenanceState, runDatabaseMaintenance, } from "./maintenance";
import { eventBus } from "../ws/bus";
const CHECK_INTERVAL_MS = 60 * 1000;
let schedulerTimer = null;
let schedulerRunning = false;
function getDueAt(lastRunAt, intervalHours) {
    if (!intervalHours || intervalHours <= 0)
        return null;
    return (lastRunAt ?? 0) + intervalHours * 60 * 60 * 1000;
}
export async function getAutomaticDatabaseMaintenanceStatus(db, userId, dbPath, operatorBusy) {
    const now = Date.now();
    ...
    if (!settings.autoVacuumEnabled)
        blockedReasons.push("auto vacuum disabled");
    if (operatorBusy)
        blockedReasons.push(`operator busy: ${operatorBusy}`);
    if (settings.vacuumRequireNoVisibleClients && visibility.isVisible)
        blockedReasons.push("visible client session active");
    if (settings.vacuumRequireNoActiveGenerations && activeGenerationCount > 0)
        blockedReasons.push("generation in progress");
    if (vacuumDueAt != null && now < vacuumDueAt)
        blockedReasons.push("vacuum interval not reached");
    if ((settings.vacuumMinDbSizeBytes ?? 0) > 0 && stats.logicalBytes < (settings.vacuumMinDbSizeBytes ?? 0))
        blockedReasons.push("database below minimum size");
    if ((settings.vacuumMinReclaimBytes ?? 0) > 0 && reclaimableBytes < (settings.vacuumMinReclaimBytes ?? 0))
        blockedReasons.push("reclaimable bytes below threshold");
    if ((settings.vacuumMinReclaimPercent ?? 0) > 0 && reclaimablePercent < (settings.vacuumMinReclaimPercent ?? 0))
        blockedReasons.push("reclaimable percent below threshold");
    if (stats.vacuumHasEnoughFreeBytes === false)
        blockedReasons.push("insufficient free disk space for vacuum rewrite");
    if ((settings.vacuumMinIdleMinutes ?? 0) > 0) {
        if (hiddenIdleMinutes == null || hiddenIdleMinutes < (settings.vacuumMinIdleMinutes ?? 0))
            blockedReasons.push("hidden idle timer not satisfied");
        if (lastWriteIdleMinutes != null && lastWriteIdleMinutes < (settings.vacuumMinIdleMinutes ?? 0))
            blockedReasons.push("recent database writes detected");
    }
    return {
        settings,
        state,
...
async function tick(getDb, getUserId, getDbPath, getBusy, runOperation) {
    if (schedulerRunning)
        return;
    ...
    try {
        ...
        try {
            const { reconcileEditAndSendOutbox, dispatchPendingEditAndSendOutbox, } = await import("../services/edit-and-send-dispatcher.service");
...
export function startAutomaticDatabaseMaintenance(getDb, getUserId, getDbPath, getBusy, runOperation, intervalMs = CHECK_INTERVAL_MS) {
...
export function stopAutomaticDatabaseMaintenance() {
    if (schedulerTimer) {
