...
function installMigration(directory) {
    writeFileSync(join(directory, MIGRATION_103), MIGRATION_103_SQL);
}
function columnSnapshot(db, table) {
    return db.query(`PRAGMA table_info('${table}')`).all()
        .map(({ name, type, notnull, dflt_value, pk }) => ({ name, type, notnull, dflt_value, pk }))
        .sort((a, b) => a.name.localeCompare(b.name));
}
function indexSnapshot(db, table) {
    return db.query(`PRAGMA index_list('${table}')`).all()
        .map(({ name, unique }) => ({ name, unique }))
        .sort((a, b) => a.name.localeCompare(b.name));
}
function revisionColumn(db) {
    const column = db.query("PRAGMA table_info('messages')").all()
...
function createPre103MessagesDb(db) {
...
describe("103 edit and send outbox migration", () => {
    ...
    test("reruns 103 without schema or data drift", async () => {
      ...
      `);
        ...
        'out-1', 'client-1', 'u1', 'c1', 'b1', 'm1', 1, 'g1', 'normal', 'pending', 1, 1
            ...
            expect(db.query("SELECT COUNT(*) AS count FROM _migrations WHERE name = ?").get(MIGRATION_103)).toEqual({ count: 1 });
            expect(db.query("SELECT request_id, generation_id FROM edit_and_send_requests WHERE id = 'req-1'").get()).toEqual({ request_id: "client-1", generation_id: "g1" });
            expect(db.query("SELECT status, generation_id FROM generation_outbox WHERE id = 'out-1'").get()).toEqual({ status: "pending", generation_id: "g1" });
        }
        finally {
            db.close();
        }
    ...
    test("matches baseline schema", async () => {
        ...
        try {
            ...
            expect(db.query("SELECT name FROM _migrations WHERE name = ?").get(MIGRATION_103)).toEqual({ name: MIGRATION_103 });
        }
        finally {
            db.close();
        }
