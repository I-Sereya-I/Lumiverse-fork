...
export class InsufficientDiskSpaceError extends Error {
    freeBytes;
    requiredBytes;
    constructor(freeBytes, requiredBytes) {
...
function readFilesystemBytes(path) {
    try {
...
function estimateVacuumRequiredBytes(stats) {
...
export function ensureVacuumDiskHeadroom(stats) {
...
function normalizeMaintenanceSettings(raw) {
    ...
    if (!raw || typeof raw !== "object")
        return next;
    if (raw.optimizeIntervalHours == null)
        next.optimizeIntervalHours = null;
    else if (Number.isFinite(Number(raw.optimizeIntervalHours)))
        next.optimizeIntervalHours = Math.max(1, Math.floor(Number(raw.optimizeIntervalHours)));
    if (raw.analyzeIntervalHours == null)
        next.analyzeIntervalHours = null;
    else if (Number.isFinite(Number(raw.analyzeIntervalHours)))
        next.analyzeIntervalHours = Math.max(1, Math.floor(Number(raw.analyzeIntervalHours)));
    if (typeof raw.autoVacuumEnabled === "boolean")
        next.autoVacuumEnabled = raw.autoVacuumEnabled;
    if (raw.vacuumIntervalHours == null)
        next.vacuumIntervalHours = null;
    else if (Number.isFinite(Number(raw.vacuumIntervalHours)))
        next.vacuumIntervalHours = Math.max(1, Math.floor(Number(raw.vacuumIntervalHours)));
    if (Number.isFinite(Number(raw.vacuumMinIdleMinutes)))
        next.vacuumMinIdleMinutes = Math.max(1, Math.floor(Number(raw.vacuumMinIdleMinutes)));
    if (typeof raw.vacuumRequireNoVisibleClients === "boolean")
        next.vacuumRequireNoVisibleClients = raw.vacuumRequireNoVisibleClients;
    if (typeof raw.vacuumRequireNoActiveGenerations === "boolean")
        next.vacuumRequireNoActiveGenerations = raw.vacuumRequireNoActiveGenerations;
    if (Number.isFinite(Number(raw.vacuumMinReclaimBytes)))
        next.vacuumMinReclaimBytes = Math.max(0, Math.floor(Number(raw.vacuumMinReclaimBytes)));
    if (Number.isFinite(Number(raw.vacuumMinReclaimPercent)))
        next.vacuumMinReclaimPercent = clamp(Number(raw.vacuumMinReclaimPercent), 0, 100);
    if (Number.isFinite(Number(raw.vacuumMinDbSizeBytes)))
        next.vacuumMinDbSizeBytes = Math.max(0, Math.floor(Number(raw.vacuumMinDbSizeBytes)));
    if (typeof raw.vacuumCheckpointMode === "string") {
        ...
        if (["PASSIVE", "FULL", "RESTART", "TRUNCATE"].includes(mode)) {
            next.vacuumCheckpointMode = mode;
        }
    }
    return next;
}
function normalizeMaintenanceState(raw) {
    if (!raw || typeof raw !== "object")
        return {};
    const next = {};
    for (const key of ["lastOptimizeAt", "lastAnalyzeAt", "lastVacuumAt", "lastAutoVacuumAttemptAt", "lastAutoVacuumSuccessAt"]) {
        const value = raw[key];
        if (value == null)
            next[key] = null;
        else if (Number.isFinite(Number(value)))
            next[key] = Math.floor(Number(value));
    }
    next.lastAutoVacuumSkipReason = typeof raw.lastAutoVacuumSkipReason === "string" ? raw.lastAutoVacuumSkipReason : null;
    return next;
}
function readJsonSetting(db, userId, key) {
    if (!userId)
        return null;
    const row = db.query("SELECT value FROM settings WHERE key = ? AND user_id = ?").get(key, userId);
    if (!row)
        return null;
    try {
...
function writeJsonSetting(db, userId, key, value) {
    if (!userId)
        return;
    const now = Math.floor(Date.now() / 1000);
    db.query(`INSERT INTO settings (key, value, user_id, updated_at) VALUES (?, ?, ?, ?)
     ON CONFLICT(key, user_id) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`).run(key, JSON.stringify(value), userId, now);
}
export function getDatabaseTuningSettingKey() {
    return DB_TUNING_SETTINGS_KEY;
}
export function getDatabaseMaintenanceSettingKey() {
    return DB_MAINTENANCE_SETTINGS_KEY;
}
export function getDatabaseMaintenanceStateSettingKey() {
    return DB_MAINTENANCE_STATE_KEY;
}
export function getDatabasePathFallback(path) {
...
export function releaseDatabaseMemory(db) {
    db.run("PRAGMA shrink_memory");
}
export function readDatabaseTuningSettings(db, userId) {
    if (!userId)
        return {};
    try {
        const row = db
            .query("SELECT value FROM settings WHERE key = ? AND user_id = ?")
            .get(DB_TUNING_SETTINGS_KEY, userId);
...
export function readDatabaseMaintenanceSettings(db, userId) {
    return normalizeMaintenanceSettings(readJsonSetting(db, userId, DB_MAINTENANCE_SETTINGS_KEY));
}
export function readDatabaseMaintenanceState(db, userId) {
    return normalizeMaintenanceState(readJsonSetting(db, userId, DB_MAINTENANCE_STATE_KEY));
}
export function writeDatabaseMaintenanceState(db, userId, patch) {
    if (!userId)
        return null;
...
export function collectDatabaseStats(db, dbPath) {
    const path = getDatabasePathFallback(dbPath);
    const pageSize = Number(getPragmaValue(db, "page_size")) || 4096;
    const pageCount = Number(getPragmaValue(db, "page_count")) || 0;
    const freelistCount = Number(getPragmaValue(db, "freelist_count")) || 0;
    const cacheSize = Number(getPragmaValue(db, "cache_size")) || 0;
    ...
    return {
        ...
        journalMode: String(getPragmaValue(db, "journal_mode") || "unknown"),
        synchronous: String(getPragmaValue(db, "synchronous") || "unknown"),
        tempStore: String(getPragmaValue(db, "temp_store") || "unknown"),
        mmapSize: Number(getPragmaValue(db, "mmap_size")) || 0,
        cacheSize,
        cacheBytesApprox,
        walAutocheckpoint: Number(getPragmaValue(db, "wal_autocheckpoint")) || 0,
        journalSizeLimit: Number(getPragmaValue(db, "journal_size_limit")) || 0,
...
export function getLatestDatabaseWriteAt(dbPath) {
    const path = getDatabasePathFallback(dbPath);
...
function computeCacheBytes(stats, settings) {
...
function computeMmapBytes(stats, settings) {
    if (!mmapEnabled()) {
...
export function resolveDatabaseTuning(stats, db, userId) {
...
export function applyAdaptiveDatabasePragmas(db, dbPath, userId) {
...
export function logDatabaseStats(label, stats, tuning) {
    const parts = [
        `[db] ${label}`,
...
export function runStartupDatabaseMaintenance(db, dbPath, userId) {
...
export function healCorruptDatabase(db, dbPath) {
    console.warn(`[db] WARNING: SQLite database is corrupted! Attempting automatic recovery...`);
    try {
        const checksBefore = db.query("PRAGMA integrity_check").all();
    ...
    try {
        const ftsTables = db.query("SELECT name FROM sqlite_master WHERE type='table' AND sql LIKE '%USING fts5%'").all();
    ...
    try {
        const checksAfter = db.query("PRAGMA integrity_check").all();
...
export function runDatabaseMaintenance(db, options = {}) {
    ...
    if (options.checkpointMode) {
        const row = db.query(`PRAGMA wal_checkpoint(${options.checkpointMode})`).get();
        checkpoint = {};
        if (row) {
...
export function startDatabaseMonitor(getDb, dbPath, intervalMs = DEFAULT_MONITOR_INTERVAL_MS) {
    stopDatabaseMonitor();
...
export function stopDatabaseMonitor() {
    if (monitorTimer) {
