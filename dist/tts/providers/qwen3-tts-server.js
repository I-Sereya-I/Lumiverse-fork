import { QWEN_TTS_PROVIDER, parseQwenVoice, qwenApiBaseUrl, qwenSpeakerVoiceId, } from "./qwen3-utils";
...
function parseQwenStreamEvent(rawBlock) {
    const outer = parseSseEventBlock(rawBlock);
    if (!outer)
        return null;
    if (!outer.data)
        return outer;
    if (!/(?:^|\n)(?:event|data):/.test(outer.data))
        return outer;
    return parseSseEventBlock(outer.data) || outer;
}
function parseQwenStreamError(raw) {
    const trimmed = raw.trim();
    if (!trimmed)
        return "Qwen3-TTS Server streaming failed";
...
export class Qwen3TtsServerProvider {
    name = QWEN_TTS_PROVIDER;
    displayName = "Qwen3-TTS Server";
    capabilities = {
        parameters: {
            language: {
                type: "select",
    ...
    async modelHealth(apiUrl) {
        return fetchProviderJson(this.displayName, "model health", `${qwenApiBaseUrl(apiUrl)}/health/models`, { headers: { Accept: "application/json" } });
    }
    ...
    async *synthesizeStream(apiKey, apiUrl, request) {
        ...
        let buffer = "";
        const yieldBlock = async function* (block) {
    ...
    async listVoices(apiKey, apiUrl) {
        const health = await this.modelHealth(apiUrl);
        if (!health.custom_voice_loaded)
            return [];
        const data = await fetchProviderJson(this.displayName, "voice listing", `${qwenApiBaseUrl(apiUrl)}/api/v1/custom-voice/speakers`, { headers: this.headers(apiKey) });
        const voices = [];
