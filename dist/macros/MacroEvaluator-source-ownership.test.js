...
function makeRegexScript(presetId) {
    return {
...
async function withInterceptor(handler, work, priority = 100) {
...
describe("prompt source ownership", () => {
    test("offers character fields without requiring host macro syntax", async () => {
        ...
        const text = await withInterceptor((ctx) => (ctx.sourceHint === "prompt_source:character.description"
            ? "processed source"
            : undefined), async () => (await evaluate("{{description}}", env, registry, { sourceOwner: "host" })).text);
    ...
    test("keeps preset variables and macros entirely on the native evaluator", async () => {
        ...
        const text = await withInterceptor((ctx) => {
            ...
            return "0 to 0";
        }, () => withPromptBlockContext(env, { id: "length-target", role: "system", position: "pre_history", depth: 0 }, async () => (await evaluate("{{floor::{{calc::{{var::words_target}} / 100}}}} to {{ceil::{{calc::{{var::words_target}} / 75}}}}|{{description}}", env, registry, { phase: "prompt", sourceHint: "prompt_source:preset_block", sourceOwner: "host" })).text));
    ...
    test("falls back to native expansion when extensions leave a character field unchanged", async () => {
        ...
        const text = await withInterceptor(() => undefined, async () => (await evaluate("{{description}}", env, registry, { sourceOwner: "host" })).text);
        expect(text).toBe("5");
    });
    test("routes preset-referenced card fields separately and preserves host evaluation", async () => {
        ...
        const text = await withInterceptor((ctx) => {
            ...
            return ctx.template;
        }, async () => (await evaluate("{{description}}|{{personality}}|{{scenario}}|{{mesExamples}}|{{system}}|{{charPostHistoryInstructions}}", env, registry, { sourceOwner: "host" })).text);
    ...
    test("keeps preset regex macros native without changing unowned regex behavior", async () => {
        ...
        await withInterceptor((ctx) => {
            ...
            expect(await applyRegexScripts("token", [makeRegexScript("preset")], "ai_output", undefined, env)).toBe("5");
            expect(await applyRegexScripts("token", [makeRegexScript(null)], "ai_output", undefined, env)).toBe("intercepted");
