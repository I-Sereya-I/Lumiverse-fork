...
export function clearMacroAstCache() {
    astCache.clear();
}
/**
...
function parseMacroExpr(ctx) {
    ...
    while (!ctx.atEnd()) {
        const tok = ctx.peek();
        if (tok.type === TokenType.FLAG_IMMEDIATE) {
            flags.immediate = true;
            ctx.advance();
        }
        else if (tok.type === TokenType.FLAG_DELAYED) {
            flags.delayed = true;
            ctx.advance();
        }
        else if (tok.type === TokenType.FLAG_REEVALUATE) {
            flags.reevaluate = true;
            ctx.advance();
        }
        else if (tok.type === TokenType.FLAG_FILTER) {
            flags.filter = true;
            ctx.advance();
        }
        else if (tok.type === TokenType.FLAG_CLOSE) {
            flags.close = true;
            ctx.advance();
        }
        else if (tok.type === TokenType.FLAG_PRESERVE) {
...
function parseVariableShorthand(ctx, flags, startOffset) {
    const scopeTok = ctx.advance(); // ., $, or @
    const scope = scopeTok.type === TokenType.DOLLAR ? "global" :
    ...
    if (operator === "-=" && operandNodes.length > 0) {
        ...
        if (first.type === "text") {
            ...
            operandNodes.unshift({ type: "text", value: "-" });
        }
    }
    // Consume closing }}
    if (ctx.at(TokenType.MACRO_CLOSE))
    ...
    const args = [[{ type: "text", value: varName }]];
    if (operandNodes.length > 0) {
...
function translateVarShorthand(scope, operator) {
    if (scope === "chat") {
        if (!operator)
            return "getchatvar";
        switch (operator) {
 ...
 */
function pairScopedMacros(nodes) {
    const result = [];
    let i = 0;
...
function findClosingMacro(nodes, startIdx, name) {
    let depth = 0;
...
function pushTextNode(nodes, value) {
    ...
    while ((match = LEGACY_MACRO_REGEX.exec(value)) !== null) {
        ...
        if (name === "bot")
            name = "char"; // Normalize <bot> to char
        nodes.push({
            type: "macro",
            name,
            args: [],
...
function pushRawText(nodes, value) {
    if (nodes.length > 0) {
