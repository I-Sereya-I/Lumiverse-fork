...
const extensionA = { kind: "extension", extensionId: "extension-a" };
const extensionB = { kind: "extension", extensionId: "extension-b" };
function definition(name, value, aliases) {
    return {
        name,
        category: "Test",
        description: "",
...
describe("MacroRegistry ownership", () => {
    test("extensions cannot replace or unregister system and foreign macros", () => {
        ...
        expect(registry.getMacro("owned")?.handler({})).toBe("a");
    });
    ...
    test("system macros beat dynamic macros while dynamic macros beat extensions", async () => {
        ...
        const result = await evaluate("{{system}}/{{preset_value}}", env({ system: "preset", preset_value: "preset" }), registry);
