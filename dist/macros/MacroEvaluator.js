 ...
 */
export async function evaluate(input, env, registry, options) {
    if (!input)
    ...
    const fingerprint = { touched: new Set(), cacheable: true };
    ...
    for (let i = 0; i < MAX_ITERATIONS; i++) {
        ...
        if (runInterceptors) {
            ...
            for (const v of interceptorResult.touchedVars)
                fingerprint.touched.add(v);
        ...
        text = result;
        if (!text.includes("{{"))
            break; // No more macros to resolve
    }
...
const EMPTY_TOUCHED_VARS = new Set();
/**
 ...
 */
async function resolvePromptSource(input, sourceHint, env) {
    ...
    if (env._fingerprint) {
        for (const variable of result.touchedVars)
            env._fingerprint.touched.add(variable);
        if (result.volatile || result.opaque)
            env._fingerprint.cacheable = false;
    }
    return result.text;
}
function wrapEnvForFingerprint(env, fingerprint) {
    const wrappedVars = {
    ...
    return new Proxy(env, {
        get(target, prop, receiver) {
            if (prop === "variables")
                return wrappedVars;
            if (prop === "_fingerprint")
                return fingerprint;
            return Reflect.get(target, prop, receiver);
        },
        set(target, prop, value, receiver) {
            if (prop === "variables" || prop === "_fingerprint")
                return true;
            return Reflect.set(target, prop, value, receiver);
        },
    });
}
function makeRecordingMap(source, scope, sink) {
...
function preprocessLegacy(input) {
...
async function evaluateNodes(nodes, env, registry, globalOffset, depth, state) {
    if (state.halted)
        return "";
 ...
 */
function stripArgFraming(nodes) {
    if (nodes.length === 0)
        return nodes;
    ...
    if (first.type === "text") {
        ...
        if (stripped !== first.value) {
            ...
            if (stripped === "")
                ...
                out[0] = { type: "text", value: stripped };
        }
    }
    const arr = out ?? nodes;
...
async function evaluateMacroNode(node, env, registry, globalOffset, depth, state) {
    ...
    if (origin?.kind !== "system" && dynamicLookup && dynamicLookup.has(dynamicKey)) {
        ...
        if (typeof dynamic === "string") {
            ...
            rawResult = String(await Promise.resolve(dynamic(buildExecContext(node, [], env, registry, globalOffset, depth, state))));
        }
        else if (typeof dynamic === "object" && dynamic.handler) {
            rawResult = String(await Promise.resolve(dynamic.handler(buildExecContext(node, [], env, registry, globalOffset, depth, state))));
        }
        else {
    ...
    if (def.volatile && env._fingerprint)
        env._fingerprint.cacheable = false;
    ...
    if (def.delayArgResolution) {
        ...
        for (const argNodes of node.args) {
            resolvedArgs.push(await evaluateNodes(stripArgFraming(argNodes), env, registry, globalOffset, depth + 1, state));
            if (state.halted)
                return "";
        }
    }
 ...
 */
async function expandIfNeeded(text, env, registry, globalOffset, depth, state) {
    if (!text.includes("{{") || state.halted)
        return text;
...
async function evaluateScopedMacroNode(node, env, registry, globalOffset, depth, state) {
    const def = registry.getMacro(node.name);
    if (!def) {
    ...
    const ctx = {
        ...
        resolveNodes: (nodes) => evaluateNodes(nodes, env, registry, globalOffset, depth + 1, state),
        ...(state.sourceOwner === "host"
            ? {
                resolvePromptSource: (input, sourceHint) => resolvePromptSource(input, sourceHint, env),
            }
            : {}),
        warn: (message) => {
...
function buildExecContext(node, resolvedArgs, env, registry, globalOffset, depth, state) {
    return {
        name: node.name,
        args: resolvedArgs,
        rawArgs: node.args,
...
function parseForEvaluation(input, state) {
    try {
        return parse(input);
    }
    catch (err) {
...
function consumeMacroBudget(nodes, state) {
    if (state.halted)
        return false;
    let count = 0;
    for (const node of nodes) {
        if (node.type === "macro" || node.type === "scoped_macro")
            count++;
    }
    if (count === 0)
        return true;
    state.macroResolutions += count;
    if (state.macroResolutions <= state.maxMacroResolutions)
        return true;
    state.halted = true;
    state.diagnostics.push({
        level: "error",
...
function snapshotEnvForInterceptor(env) {
    const dyn = {};
...
function reconstructMacro(node) {
    let str = "{{";
    if (node.flags.immediate)
        str += "!";
    if (node.flags.delayed)
        str += "?";
    if (node.flags.reevaluate)
        str += "~";
    if (node.flags.filter)
        str += ">";
    if (node.flags.close)
        str += "/";
    if (node.flags.preserveWhitespace)
        str += "#";
    str += node.name;
    for (const arg of node.args) {
        ...
        for (const n of arg) {
            if (n.type === "text")
                ...
                str += reconstructMacro(n);
        }
    }
    str += "}}";
    return str;
}
function reconstructScopedMacro(node) {
    let str = "{{";
    if (node.flags.immediate)
        str += "!";
    if (node.flags.delayed)
...
function reconstructNodes(nodes) {
    let str = "";
    for (const node of nodes) {
        if (node.type === "text")
            str += node.value;
        else if (node.type === "macro")
            str += reconstructMacro(node);
        else
            str += reconstructScopedMacro(node);
    }
    return str;
}
