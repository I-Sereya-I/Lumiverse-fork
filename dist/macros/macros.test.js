...
async function ev(template, env) {
...
describe("Chat transcript examples", () => {
    test("parser and evaluator handle setvar/getvar across chat-style lines", async () => {
        ...
        const macroNames = ast.flatMap((node) => node.type === "macro" ? [node.name] : []);
...
describe("Prompt block placement macros", () => {
    test("reflect the current block configuration only while that block renders", async () => {
        const env = makeEnv();
        expect(await ev("{{promptBlockRole}}/{{promptBlockPosition}}/{{promptBlockDepth}}", env)).toBe("//");
        const resolved = await withPromptBlockContext(env, { role: "assistant_append", position: "in_history", depth: 3 }, () => ev("{{promptBlockRole}}/{{blockPosition}}/{{prompt_block_depth}}", env));
...
describe("Core primitives", () => {
    test("userInput returns the input-bar draft snapshot", async () => {
        expect(await ev("{{userInput}}", makeEnv({ userInput: "Draft text\nwith spacing" }))).toBe("Draft text\nwith spacing");
...
describe("Character Tags macros", () => {
    ...
    test("non-string tag entries are ignored without throwing", async () => {
        const env = makeEnv({
            characterTags: ["fantasy", 3, null, "warrior", "  "],
        });
...
{{/if}}`;
    ...
    test("if supports elseif / elif chains", async () => {
        expect(await ev("{{if::false}}A{{elseif::0}}B{{elseif::yes}}C{{else}}D{{/if}}")).toBe("C");
        expect(await ev("{{if::false}}A{{elif::true}}B{{else}}C{{/if}}")).toBe("B");
    });
...
describe("Variables", () => {
    ...
    test("let binds scoped local variables and restores previous values", async () => {
        ...
        expect(await ev("{{let::name::inner::role::mage}}{{.name}}/{{.role}}{{/let}}|{{.name}}/{{.role}}", env)).toBe("inner/mage|outer/");
...
{{/trim}}`;
    ...
    test("nested macro on its own line stores same value as inline (regression)", async () => {
        ...
        const buildAndGet = async (open) => {
            const env = makeEnv();
            await ev(`${open}::{{newline}}::
    {{getvar::acc}}::
Test String
  }}
}}`, env);
...
describe("Logic macros", () => {
    ...
    test("switch only resolves matched branch result", async () => {
        const env = makeEnv();
        const result = await ev("{{switch::b::a::{{setchatvar::bad::1}}Alpha::b::{{setchatvar::good::1}}Beta::{{setchatvar::defaulted::1}}Default}}", env);
    ...
    test("switch only resolves default when no case matches", async () => {
        const env = makeEnv();
        const result = await ev("{{switch::z::a::{{setchatvar::bad::1}}Alpha::b::{{setchatvar::also_bad::1}}Beta::{{setchatvar::defaulted::1}}Default}}", env);
    ...
    test("scoped switch resolves matching case block only", async () => {
        const env = makeEnv({ localVars: { mode: "dark" } });
        const result = await ev("{{switch::{{.mode}}}}{{case::light}}{{setchatvar::bad::1}}Sun{{/case}}{{case::dark}}Moon{{/case}}{{default}}Star{{/default}}{{/switch}}", env);
        expect(result).toBe("Moon");
        expect(env.variables.chat.has("bad")).toBe(false);
    });
    test("scoped switch resolves scoped default block", async () => {
        expect(await ev("{{switch::missing}}{{case::hit}}Hit{{/case}}{{default}}Fallback {{char}}{{/default}}{{/switch}}")).toBe("Fallback Bob");
    });
...
describe("Chat Utils macros", () => {
    ...
    test("rcounter in a conditional template renumbers cleanly when branches skip", async () => {
        const env = makeEnv();
        const template = "{{if::yes}}{{rcounter::step}}. A\n{{/if}}{{if::no}}{{rcounter::step}}. B\n{{/if}}{{rcounter::step}}. C";
...
describe("Integration: nested and combined macros", () => {
    ...
    test("if with calc comparison", async () => {
        expect(await ev("{{if::{{calc::2 + 2}} == 4}}math works{{/if}}")).toBe("math works");
    });
    test("switch on char name", async () => {
        expect(await ev("{{switch::{{char}}::Alice::user::Bob::character::unknown}}")).toBe("character");
    });
    ...
    test("wrap with conditional content", async () => {
        ...
        expect(await ev("{{wrap::(**::**)::{{.note}}}}", env)).toBe("(**important**)");
    });
    ...
    test("counter in if condition", async () => {
        ...
        expect(await ev("{{if::{{counter::step}} == 5}}five!{{/if}}", env)).toBe("five!");
    });
    test("bullets with dynamic content", async () => {
        expect(await ev("{{bullets::{{char}}::{{user}}}}")).toBe("- Bob\n- Alice");
    });
    ...
    test("replace inside if", async () => {
        expect(await ev("{{if::{{replace::yes::true::yes}} == true}}replaced{{/if}}")).toBe("replaced");
    });
    ...
    test("multi-level nesting", async () => {
        ...
        const env = makeEnv();
        expect(await ev("{{upper::{{default::{{getvar::missing}}::hello}}}}", env)).toBe("HELLO");
    });
});
// ===========================================================================
...
describe("foreach macro", () => {
    ...
    test("exposes 0-based index and 1-based number", async () => {
        expect(await ev("{{foreach::x,y,z}}{{.item_index}}:{{.item_number}} {{/foreach}}")).toBe("0:1 1:2 2:3 ");
    });
    ...
    test("exposes first / last flags", async () => {
        expect(await ev("{{foreach::a,b,c}}{{.item}}={{.item_first}}/{{.item_last}} {{/foreach}}")).toBe("a=true/ b=/ c=/true ");
    });
    test("first/last enable clean separators via if/else", async () => {
        expect(await ev("{{foreach::a,b,c::x}}{{if::{{.x_last}}}}{{.x}}{{else}}{{.x}}, {{/if}}{{/foreach}}")).toBe("a, b, c");
    });
    test("first/last enable clean separators via negated last flag", async () => {
        expect(await ev("{{foreach::a,b,c::x}}{{.x}}{{if::!{{.x_last}}}}, {{/if}}{{/foreach}}")).toBe("a, b, c");
    });
    ...
    test("nests cleanly with distinct variable names", async () => {
        expect(await ev("{{foreach::1,2::n}}{{foreach::a,b::l}}{{.n}}{{.l}} {{/foreach}}{{/foreach}}")).toBe("1a 1b 2a 2b ");
    });
...
describe("map macro", () => {
    ...
    test("supports custom input and output delimiters", async () => {
        expect(await ev("{{map::a|b|c::x::|:: / }}{{.x_number}}={{.x}}{{/map}}")).toBe("1=a / 2=b / 3=c");
    });
...
describe("Multiplayer macros", () => {
    ...
    test("gates content with {{if}}", async () => {
        expect(await ev("{{if::{{isMultiplayer}}}}room of {{playerCount}}{{else}}solo{{/if}}", mpEnv())).toBe("room of 3");
...
describe("filter / some / every macros", () => {
    ...
    test("compose with multiplayer: peers (everyone but the host)", async () => {
        expect(await ev("{{filter::{{players}}::p}}{{ne::{{.p}}::{{hostName}}}}{{/filter}}", mpEnv())).toBe("Bob, Charlie");
        expect(await ev("{{count::{{filter::{{players}}::p}}{{ne::{{.p}}::{{hostName}}}}{{/filter}}}}", mpEnv())).toBe("2");
    });
    test("compose: gate on whether the roster includes a name", async () => {
        expect(await ev("{{if::{{some::{{players}}::p}}{{eq::{{.p}}::Bob}}{{/some}}}}has-bob{{else}}no{{/if}}", mpEnv())).toBe("has-bob");
    });
});
// ===========================================================================
...
describe("foreachMessage macro", () => {
    test("iterates all messages with name + content", async () => {
        expect(await ev("{{foreachMessage}}{{.msg_name}}: {{.msg}}\n{{/foreachMessage}}")).toBe("Alice: Hello, how are you?\n" +
            "Bob: I'm fine, thanks!\n" +
    ...
    test("last N messages, in chronological order", async () => {
        ...
        expect(await ev("{{foreachMessage::2::m}}{{.m}};{{/foreachMessage}}")).toBe("The forest is dark.;I draw my sword.;");
    });
    ...
    test("is_user flag drives branching", async () => {
        expect(await ev("{{foreachMessage::m}}{{if::{{.m_is_user}}}}U{{else}}A{{/if}}{{/foreachMessage}}")).toBe("UAUAU");
    });
    test("first / last bindings", async () => {
        expect(await ev("{{foreachMessage}}{{if::{{.msg_first}}}}<{{/if}}{{.msg_index}}{{if::{{.msg_last}}}}>{{/if}}{{/foreachMessage}}")).toBe("<01234>");
    });
...
describe("foreachVar family", () => {
    test("foreachChatVar iterates a namespaced table in key order", async () => {
        ...
        expect(await ev("{{foreachChatVar::hp_::p}}{{.p}}={{.p_value}};{{/foreachChatVar}}", env)).toBe("Alice=100;Bob=80;");
    });
    test("bindings: id vs full key vs value", async () => {
        const env = makeEnv({ chatVars: { hp_Alice: "100" } });
        expect(await ev("{{foreachChatVar::hp_::p}}{{.p_key}}|{{.p}}|{{.p_value}}{{/foreachChatVar}}", env)).toBe("hp_Alice|Alice|100");
    });
    test("foreachVar (local) and foreachGlobalVar (global)", async () => {
        ...
        expect(await ev("{{foreachVar::item_::i}}[{{.i}}]{{/foreachVar}}", localEnv)).toBe("[shield][sword]");
        const globalEnv = makeEnv({ globalVars: { theme_dark: "1", theme_light: "1" } });
        expect(await ev("{{foreachGlobalVar::theme_::t}}{{.t}};{{/foreachGlobalVar}}", globalEnv)).toBe("dark;light;");
    });
    test("empty prefix iterates the whole scope", async () => {
        const env = makeEnv({ chatVars: { a: "1", b: "2" } });
        expect(await ev("{{foreachChatVar::::k}}{{.k}}={{.k_value}};{{/foreachChatVar}}", env)).toBe("a=1;b=2;");
    });
    ...
    test("compose: sum a stat table", async () => {
        ...
        expect(await ev("{{sum::{{foreachChatVar::hp_::p}}{{.p_value}},{{/foreachChatVar}}}}", env)).toBe("240");
    });
...
describe("Temporal macros", () => {
    test("{{idleDuration}} returns 'unknown' when no last message time is available", async () => {
        const env = makeEnv();
        delete env.extra.lastMessageTime;
