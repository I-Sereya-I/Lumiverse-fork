...
export function registerVariableMacros() {
    ...
    registry.registerMacro({
        ...
        name: "let",
        category: "Variables",
        description: "Temporarily bind local variables for a scoped body, then restore previous values. " +
...
async function runLetMacro(ctx) {
    if (!ctx.isScoped) {
    ...
    for (const [key] of pairs) {
        if (!saved.has(key))
            saved.set(key, local.has(key) ? local.get(key) : undefined);
    }
    try {
