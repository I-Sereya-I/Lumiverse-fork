...
import { getRegexScriptByScriptId, } from "../../services/regex-scripts.service";
import { regexCaptureReplacementsSandboxed, regexReplaceSandboxed, RegexTimeoutError, } from "../../utils/regex-sandbox";
...
export function registerRegexRefMacros() {
    registry.registerMacro({
        builtIn: true,
        name: "regexInstalled",
        category: "Regex",
        description: "Check if a regex script is installed, or apply it to text. " +
        ...
        handler: async (ctx) => {
            ...
            try {
                ...
                if (script.substitute_macros === "raw") {
                    ...
                    const matches = await regexCaptureReplacementsSandboxed(findRegex, script.flags, text, script.replace_string, REGEX_REF_TIMEOUT_MS);
                    if (matches.length > 0) {
                    ...
                    const substituted = await regexReplaceSandboxed(findRegex, script.flags, text, script.replace_string, REGEX_REF_TIMEOUT_MS);
                    ...
                    result = await regexReplaceSandboxed(findRegex, script.flags, text, replaceString, REGEX_REF_TIMEOUT_MS);
                }
                // Apply trim_strings
