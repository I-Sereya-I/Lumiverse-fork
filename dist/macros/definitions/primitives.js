...
export function registerCoreMacros() {
    ...
    registry.registerMacro({
        ...
        handler: (ctx) => {
            ...
            if (!name)
                return "";
            const outlets = ctx.env.extra?.worldInfoOutlets;
    ...
    registry.registerMacro({
        ...
        handler: (ctx) => {
            ...
            if (!name)
                return "";
            const outlets = ctx.env.extra?.personaAddonOutlets;
            const value = outlets?.[name];
            return typeof value === "string" ? value : "";
        },
    });
    registry.registerMacro({
        ...
        aliases: ["wiMarker"],
        category: "Core",
        description: "Resolve all activated world-info entries set to 'At Marker' position, joined by double newlines",
        returnType: "string",
        handler: (ctx) => {
            ctx.env.extra._worldInfoAtMarkerMacroUsed = true;
            const pool = ctx.env.extra?.worldInfoAtMarker;
...
function conditionArgNodes(rawArgs) {
...
async function conditionIsTruthy(ctx, nodes) {
...
function splitOnElseNodes(body) {
