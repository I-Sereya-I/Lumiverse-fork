...
function getCortex(ctx) {
    return ctx.env.extra.cortex ?? null;
}
function hasCortexContent(cortex) {
    return !!(cortex &&
...
export function registerCortexMacros() {
    ...
    registry.registerMacro({
        ...
        handler(ctx) {
            ...
            if (!cortex?.entityContext?.length)
                return "";
    ...
    registry.registerMacro({
        ...
        handler(ctx) {
            ...
            if (!cortex?.entityContext?.length)
                return "";
            const entity = cortex.entityContext.find((e) => e.name.toLowerCase() === entityName.toLowerCase());
            if (!entity || entity.topFacts.length === 0)
                return "";
    ...
    registry.registerMacro({
        ...
        handler(ctx) {
            const cortex = getCortex(ctx);
            if (!cortex?.activeRelationships?.length)
                return "";
    ...
    registry.registerMacro({
        ...
        handler(ctx) {
            const cortex = getCortex(ctx);
            if (!cortex?.memories?.length)
                return "";
            const top = cortex.memories.reduce((a, b) => (a.components.salience > b.components.salience ? a : b));
            return top.content;
        },
    });
    ...
    registry.registerMacro({
        ...
        category: "memory",
        description: "Font color attributions per character from the Memory Cortex. " +
        ...
        handler(ctx) {
            const cortex = getCortex(ctx);
            return cortex?.colorMap ?? "";
        },
    });
}
// ─── Formatting (mirrors entity-context.ts for macro independence) ──
function formatEntitySnapshots(snapshots) {
...
function formatRelationships(edges) {
    if (edges.length === 0)
        return "";
    const lines = ["[ACTIVE RELATIONSHIPS]"];
