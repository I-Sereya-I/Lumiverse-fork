...
export function registerStateMacros() {
    registry.registerMacro({
        ...
        returnType: "string",
        aliases: ["user_input"],
        handler: (ctx) => typeof ctx.env.extra.userInput === "string" ? ctx.env.extra.userInput : "",
    });
    registry.registerMacro({
        builtIn: true,
        terminal: true,
    ...
    registry.registerMacro({
        ...
        handler: (ctx) => ctx.env.promptBlock ? String(ctx.env.promptBlock.depth) : "",
    });
    registry.registerMacro({
        builtIn: true,
        terminal: true,
        ...
        handler: (ctx) => {
            return ctx.env.extra.theme?.mode || "dark";
        },
    });
    registry.registerMacro({
        builtIn: true,
