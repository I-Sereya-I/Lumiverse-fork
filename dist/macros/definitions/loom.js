...
function getLoom(ctx) {
    return (ctx.env.extra.loom ?? {});
}
/** Check if the last message in the chat was from the character (not user). */
function wasCharacterLastSpeaker(ctx) {
...
// ---------------------------------------------------------------------------
function buildSovereignHandPrompt(ctx) {
...
function buildContinuePrompt(ctx) {
    ...
    if (!sov.enabled)
        return "";
    if (!wasCharacterLastSpeaker(ctx))
        return "";
    const charName = ctx.env.names.char || "the character";
    return `## Continuation
...
// ---------------------------------------------------------------------------
function buildSummaryDirectivePrompt(ctx) {
    const userName = ctx.env.names.user || "the user";
    const charName = ctx.env.names.char || "the character";
...
// Registration
// ---------------------------------------------------------------------------
export function registerLoomMacros() {
    // ---- loomSummary ----
