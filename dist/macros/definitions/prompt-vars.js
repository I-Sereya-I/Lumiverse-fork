...
function getValues(ctx) {
    ...
    const byBlock = ctx.env.extra.promptVariablesByBlock;
    if (blockId && byBlock?.[blockId])
        return byBlock[blockId];
    return (ctx.env.extra.promptVariables ?? {});
}
function getDefaults(ctx) {
    const blockId = ctx.env.promptBlock?.id;
    const byBlock = ctx.env.extra.promptVariableDefaultsByBlock;
    if (blockId && byBlock?.[blockId])
        return byBlock[blockId];
    return (ctx.env.extra.promptVariableDefaults ?? {});
}
function getSelections(ctx) {
    const blockId = ctx.env.promptBlock?.id;
    const byBlock = ctx.env.extra.promptVariableSelectionsByBlock;
    if (blockId && byBlock?.[blockId])
        return byBlock[blockId];
    return (ctx.env.extra.promptVariableSelections ?? {});
}
...
function resolveIsOnQuery(ctx, key) {
...
export function registerPromptVarMacros() {
    ...
    registry.registerMacro({
        ...
        handler(ctx) {
            ...
            if (op === "ison")
                return resolveIsOnQuery(ctx, key);
    ...
    registry.registerMacro({
        name: "hasVar",
        category: "state",
        description: "Returns 'true' if the named prompt variable is resolvable (runtime, schema, or default), 'false' otherwise.",
    ...
    registry.registerMacro({
        name: "varDefault",
        category: "state",
        description: "Read the creator-declared default for a prompt variable, ignoring any end-user override.",
        args: [{ name: "name", type: "string", description: "Variable name" }],
