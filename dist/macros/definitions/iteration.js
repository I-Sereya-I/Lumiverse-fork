...
const LOOP_SUFFIXES = ["", "_index", "_number", "_count", "_first", "_last"];
/**
...
function bindLoopVars(local, varName, item, i, count) {
 ...
 */
async function parseIterArgs(ctx) {
    if (!ctx.isScoped) {
...
async function bodyIsTruthy(ctx) {
...
export function registerIterationMacros() {
    // ---- foreach ----
    //
    ...
    registry.registerMacro({
        ...
        name: "foreach",
        category: "Iteration",
        description: "Iterate over a delimited list, resolving the body once per item. " +
    ...
    registry.registerMacro({
        ...
        name: "map",
        category: "Iteration",
        description: "Transform each item in a delimited list with a scoped body. " +
    ...
    registry.registerMacro({
        ...
        name: "filter",
        category: "Iteration",
        description: "Keep list items whose body (an {{if}}-style condition) is truthy; returns a " +
    ...
    registry.registerMacro({
        ...
        name: "some",
        category: "Iteration",
        description: "'true' if any list item's body (an {{if}}-style condition) is truthy, else ''. " +
    ...
    registry.registerMacro({
        ...
        name: "every",
        category: "Iteration",
        description: "'true' if every list item's body (an {{if}}-style condition) is truthy, else ''. " +
    ...
    registry.registerMacro({
        ...
        name: "foreachMessage",
        category: "Iteration",
        description: "Iterate over chat history, resolving the body once per message. Optional first arg: a " +
    ...
    for (const { name, scope, aliases } of VAR_SCOPES) {
        registry.registerMacro({
            ...
            name,
            category: "Iteration",
            description: `Iterate ${scope} variables whose name starts with a prefix (alphabetical order). ` +
 ...
 */
async function runForeachVar(ctx, scopeMap) {
    if (!ctx.isScoped) {
    ...
    const entries = matched.map((k) => [k, scopeMap.get(k) ?? ""]);
    const local = ctx.env.variables.local;
