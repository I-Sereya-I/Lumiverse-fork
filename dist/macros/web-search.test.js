...
describe("webSearchContext macro", () => {
    test("creates an internal slot only while rendering a preset block", async () => {
        const inBlock = await evaluate("before {{webSearchContext}} after", makeEnv({ id: "block-1", role: "system", position: "relative", depth: 0 }), registry);
