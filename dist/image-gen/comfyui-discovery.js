 ...
 */
export function resolveComfyTarget(connection, apiKey) {
...
async function queryNodeInfo(baseUrl, nodeType, opts) {
    try {
...
async function queryAllNodeInfo(baseUrl, opts) {
    try {
...
export async function getComfyUIObjectInfo(apiUrl, forceRefresh = false, opts) {
...
export async function getAvailableNodeTypes(apiUrl, nodeTypes, opts) {
    ...
    if (allNodeInfo && typeof allNodeInfo === "object") {
        ...
        return new Set(nodeTypes.filter((nodeType) => availableNodeTypes.has(nodeType)));
    }
    const available = await Promise.all(nodeTypes.map(async (nodeType) => ({
        nodeType,
        exists: (await queryNodeInfo(apiUrl, nodeType, opts)) !== null,
    })));
    return new Set(available.filter((item) => item.exists).map((item) => item.nodeType));
}
/**
 * Extract the available options for a specific input field from a node's object_info.
 */
function extractOptions(nodeInfo, fieldName) {
    if (!nodeInfo)
        return [];
 ...
 */
export async function discoverCapabilities(apiUrl, forceRefresh = false, opts) {
    const cached = cache.get(apiUrl);
    if (!forceRefresh && cached && Date.now() - cached.timestamp < CACHE_TTL_MS) {
        return cached.capabilities;
    }
    // Query all relevant node types in parallel
    const [checkpointInfo, unetInfo, clipInfo, dualClipInfo, vaeInfo, loraInfo, kSamplerInfo, upscaleModelInfo, ultralyticsInfo, ultimateUpscaleInfo, faceDetailerInfo, controlnetInfo,] = await Promise.all([
    ...
    const modelLoaderType = checkpoints.length > 0 && unets.length > 0 ? "both"
        : unets.length > 0 ? "unet"
            : "checkpoint";
    // Determine CLIP loader type based on actual available clips
    const clipLoaderType = dualClips.length > 0 ? "dual" : clips.length > 0 ? "single" : "none";
    const capabilities = {
        checkpoints,
        unets,
        clips,
...
export function clearCapabilityCache(apiUrl) {
    if (apiUrl) {
        cache.delete(apiUrl);
