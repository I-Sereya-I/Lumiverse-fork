...
export class OpenRouterImageProvider {
    ...
    async generate(apiKey, apiUrl, request) {
        ...
        const sources = params.resolvedSourceImages || params.referenceImages || [];
        ...
        if (params.imageSize)
            imageConfig.image_size = params.imageSize;
        // strength only meaningful for image-to-image — gate on having sources.
        if (sources.length > 0 && params.strength != null)
            imageConfig.strength = Number(params.strength);
    ...
    async validateKey(apiKey, apiUrl) {
        try {
            ...
            if (!models.ok)
                await throwProviderResponseError(this.displayName, "authentication", models);
            return models.ok;
        }
        catch (err) {
     ...
     */
    async toDataUrl(url, signal) {
        if (url.startsWith("data:"))
            return url;
        const res = await fetch(url, { signal });
        if (!res.ok)
            await throwProviderResponseError(this.displayName, "image fetch", res);
    ...
    isKnownImageOnlyModel(model) {
        const id = model.toLowerCase();
        return (id.startsWith("sourceful/") ||
