...
function generatedRequestBody(fetchStub) {
    const body = fetchStub.genBody();
    if (!body)
        throw new Error("Expected a generated request body");
    return body;
}
let baseCounter = 0;
function uniqueBase() {
...
async function captureProviderError(request) {
    try {
        await request;
    }
    catch (error) {
...
describe("SwarmUIImageProvider — rawRequestOverride", () => {
    ...
    test("flat JSON merges into the top-level request body", async () => {
        ...
        await provider.generate("", uniqueBase(), req({
    ...
    test("invalid JSON fails generation with a clear error instead of silently dropping", async () => {
        fetchStub = stubFetch();
        const provider = new SwarmUIImageProvider();
        await expect(provider.generate("", uniqueBase(), req({ rawRequestOverride: '{"refinerupscale": 1.25,}' }))).rejects.toThrow(/not valid JSON/);
        expect(fetchStub.genBody()).toBeUndefined();
    });
    test("non-object JSON is rejected (would otherwise replace the whole body)", async () => {
        fetchStub = stubFetch();
        const provider = new SwarmUIImageProvider();
        await expect(provider.generate("", uniqueBase(), req({ rawRequestOverride: '["refinerupscale"]' }))).rejects.toThrow(/JSON object/);
    });
    test("protected keys cannot be smuggled through the override", async () => {
        fetchStub = stubFetch();
        const provider = new SwarmUIImageProvider();
        await provider.generate("", uniqueBase(), req({ rawRequestOverride: JSON.stringify({ model: "EVIL", steps: 33 }) }));
        const body = generatedRequestBody(fetchStub);
    ...
    test("an optional UNet selection overrides the connection model", async () => {
        fetchStub = stubFetch();
        const response = await new SwarmUIImageProvider().generate("", uniqueBase(), req({ unet: "flux/flux1-dev.safetensors" }));
...
describe("SwarmUIImageProvider — model discovery", () => {
    ...
    test("surfaces HTTP-200 GetNewSession errors before reading session_id", async () => {
        ...
        const error = await captureProviderError(new SwarmUIImageProvider().listModelsBySubtype("", base, "loras"));
    ...
    test("rejects malformed model-listing response shapes", async () => {
        ...
        for (const [index, base] of bases.entries()) {
            ...
            expect(error.detail, invalidResponses[index].label).toBe("SwarmUI returned an invalid model listing response");
        }
    });
    test("accepts an empty files array as an empty library", async () => {
        ...
        await expect(new SwarmUIImageProvider().listModelsBySubtype("", base, "loras")).resolves.toEqual([]);
    });
    test("uses parsed codes for malformed and oversized logical error IDs", async () => {
        ...
        const provider = new SwarmUIImageProvider();
        const malformed = await captureProviderError(provider.listModelsBySubtype("", malformedBase, "loras"));
        expect(malformed).toMatchObject({
            code: "Invalid sub-type.",
            detail: "Invalid sub-type.",
        });
        const oversized = await captureProviderError(provider.listModelsBySubtype("", oversizedBase, "loras"));
        expect(oversized.code).toBe(`${"x".repeat(497)}...`);
        expect(oversized.code).toHaveLength(500);
    ...
    test("bounds pending handshakes while coalescing an admitted key", async () => {
        ...
        const handshakes = admittedBases.map(() => deferred());
        let sessionRequests = 0;
        fetchStub = installFetch((call) => {
            if (!call.url.endsWith("/API/GetNewSession")) {
                throw new Error(`Unexpected fetch: ${call.url}`);
            ...
            if (index < 0) {
                if (base !== bases[128])
                    throw new Error(`Unexpected session base: ${base}`);
                sessionRequests += 1;
    ...
    test("cleans a rejected pending handshake before allowing a fresh session request", async () => {
        const base = uniqueBase();
        const failedHandshake = deferred();
        let sessionRequests = 0;
        fetchStub = installFetch((call) => {
            if (call.url !== `${base}/API/GetNewSession`) {
                throw new Error(`Unexpected fetch: ${call.url}`);
    ...
    test("coalesces concurrent stale-session refreshes and reuses the fresh session", async () => {
        const base = uniqueBase();
        const initialStaleRequests = deferred();
        const freshSessionRequested = deferred();
        const freshSession = deferred();
        const staleResponses = [deferred(), deferred()];
        const listBodies = [];
        let sessionRequests = 0;
        ...
        fetchStub = installFetch((call) => {
            ...
            if (call.url === `${base}/API/ListModels`) {
                ...
                if (body.session_id === "session-0") {
                    ...
                    if (!staleResponse)
                        throw new Error("Unexpected stale ListModels request");
                    staleRequestCount += 1;
                    if (staleRequestCount === 2)
                        initialStaleRequests.resolve();
        ...
        expect(sessionRequests).toBe(2);
        expect(listBodies.filter((body) => body.session_id === "session-1" && body.subtype === "VAE")).toHaveLength(1);
    });
    test("isolates token-scoped model caches and sends only the matching dummy cookie", async () => {
        ...
        fetchStub = installFetch((call) => {
            if (call.url === `${base}/API/GetNewSession`) {
                if (!call.cookie)
                    throw new Error("Expected a dummy token cookie");
