...
export function formatWsError(e) {
    const anyE = e;
    ...
    if (anyE?.error) {
        if (typeof anyE.error === "string")
            return anyE.error;
        if (typeof anyE.error.message === "string" && anyE.error.message)
            return anyE.error.message;
        if (typeof anyE.error.code === "string" && anyE.error.code)
            return anyE.error.code;
    }
    if (typeof anyE?.reason === "string" && anyE.reason)
        return anyE.reason;
    if (typeof anyE?.code === "string" && anyE.code)
        return anyE.code;
    if (typeof anyE?.code === "number")
        return `close code ${anyE.code}`;
    if (typeof anyE?.type === "string" && anyE.type && anyE.type !== "error")
        return anyE.type;
...
export async function openWebSocket(url, opts) {
...
async function waitForWebSocketClose(ws, timeoutMs) {
 ...
 */
export async function closeWebSocketGracefully(ws, closeTimeoutMs = 1_000) {
    if (ws.readyState === WebSocket.CLOSED)
        return;
