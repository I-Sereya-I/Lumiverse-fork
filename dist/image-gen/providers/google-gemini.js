...
export class GoogleGeminiImageProvider {
    ...
    async generate(apiKey, apiUrl, request) {
        ...
        const sources = request.parameters.resolvedSourceImages || request.parameters.referenceImages || [];
        const inputParts = [{ text: request.prompt }];
        for (const src of sources) {
            if (src?.data)
                inputParts.push({ inlineData: { mimeType: src.mimeType || "image/png", data: src.data } });
        }
        const body = {
    ...
    async listModels(apiKey, apiUrl) {
        const data = await fetchProviderJson(this.displayName, "model listing", `${this.baseUrl(apiUrl)}/models?key=${apiKey}`);
        const models = (data.models || [])
            .map((m) => m.name?.replace("models/", "") || m.name)
            .filter((n) => n && n.includes("image"))
            .sort();
        if (models.length === 0)
