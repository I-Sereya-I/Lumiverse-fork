...
export function buildComfyImageViewUrl(baseUrl, image) {
 ...
 */
export async function uploadComfyImage(baseUrl, image, opts) {
...
export function findFirstComfyImageResult(outputs) {
...
function logOutputsShape(label, outputs, promptId) {
    try {
        const summary = {};
        ...
        console.error("[%s] No image found in outputs. promptId=%s nodeCount=%d outputShape=%j", label, promptId, Object.keys(outputs).length, summary);
    }
    catch {
        console.error("[%s] No image found in outputs and failed to log shape. promptId=%s", label, promptId);
    }
}
export async function* executeComfyWorkflowStream(baseUrl, workflow, signal, opts) {
    ...
    if (!imageRes.ok)
        throw new Error(`Failed to fetch ${label} output image: ${imageRes.status}`);
    const imageBuffer = await imageRes.arrayBuffer();
    const base64 = Buffer.from(imageBuffer).toString("base64");
    const mimeType = imageRes.headers.get("content-type") || "image/png";
    return { imageDataUrl: `data:${mimeType};base64,${base64}` };
}
export async function executeComfyWorkflow(baseUrl, workflow, signal, opts) {
...
async function* wsEventStream(ws, promptId, signal) {
    const queue = [];
    let resolve = null;
    let done = false;
    const enqueue = (event) => {
    ...
    ws.addEventListener("message", (evt) => {
        if (typeof evt.data === "string") {
            ...
            const buffer = Buffer.from(evt.data);
