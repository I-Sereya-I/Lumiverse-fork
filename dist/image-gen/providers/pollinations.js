...
export class PollinationsImageProvider {
    ...
    async generate(apiKey, apiUrl, request) {
        ...
        if (request.parameters.enhance != null)
            body.enhance = !!request.parameters.enhance;
        if (request.parameters.quality)
            body.quality = String(request.parameters.quality);
        if (request.parameters.transparent != null)
            body.transparent = !!request.parameters.transparent;
        if (request.parameters.negative_prompt)
            body.negative_prompt = String(request.parameters.negative_prompt);
    ...
    async listModels(apiKey, apiUrl) {
        ...
        if (apiKey)
            headers.Authorization = `Bearer ${apiKey}`;
        const data = await fetchProviderJson(this.displayName, "model listing", `${base}/image/models`, { headers });
