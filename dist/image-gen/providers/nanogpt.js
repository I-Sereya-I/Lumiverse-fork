...
export class NanoGPTImageProvider {
    name = "nanogpt";
    ...
    async generate(apiKey, _apiUrl, request) {
        ...
        if (Array.isArray(refs) && refs.length > 0) {
            requestBody.imageDataUrls = refs
                .filter((r) => !!r.data)
                .map((r) => `data:${r.mimeType || "image/png"};base64,${r.data}`);
            ...
            if (params.numInferenceSteps != null)
                requestBody.num_inference_steps = params.numInferenceSteps;
            if (params.seed != null)
        ...
        if (!b64)
            throw new Error("Nano-GPT returned no image data");
        return {
    ...
    async listModels(apiKey, _apiUrl) {
        const data = await fetchProviderJson(this.displayName, "model listing", "https://nano-gpt.com/api/v1/image-models", {
            method: "GET",
            headers: { Authorization: `Bearer ${apiKey}` },
        });
