 ...
 */
function stubFetch(responseImages = ["aGVsbG8="]) {
    const calls = [];
...
function stubModelFetch(respond) {
    const calls = [];
    const original = globalThis.fetch;
    globalThis.fetch = (async (input, init) => {
        const call = {
...
describe("SdApiImageProvider — txt2img vs img2img routing", () => {
    ...
    test("manual init_images paste field still works as a fallback", async () => {
        fetchStub = stubFetch();
        await provider.generate("", BASE, req({ init_images: "data:image/png;base64,MANUAL" }));
        const call = fetchStub.calls[0];
        expect(call.url).toBe(`${BASE}/sdapi/v1/img2img`);
...
describe("SdApiImageProvider — LoRA discovery", () => {
    ...
    test("accepts an empty canonical LoRA listing", async () => {
        fetchStub = stubModelFetch(() => Response.json([]));
        await expect(new SdApiImageProvider().listModelsBySubtype("", "http://sdapi-empty.example.test", "loras")).resolves.toEqual([]);
    });
    ...
    test("rejects malformed non-array LoRA listing bodies", async () => {
        const invalidBodies = [null, {}, "not a LoRA list"];
        let responseIndex = 0;
        fetchStub = stubModelFetch(() => Response.json(invalidBodies[responseIndex++]));
        for (const index of invalidBodies.keys()) {
            let caught;
            try {
                await new SdApiImageProvider().listModelsBySubtype("", `http://sdapi-malformed-${index}.example.test`, "loras");
            }
            catch (error) {
                caught = error;
            }
    ...
    test("rejects malformed entries in a nonempty LoRA listing", async () => {
        const invalidBodies = [[{}], [{ path: " " }], [null], [[]]];
        let responseIndex = 0;
        fetchStub = stubModelFetch(() => Response.json(invalidBodies[responseIndex++]));
        for (const index of invalidBodies.keys()) {
            let caught;
            try {
                await new SdApiImageProvider().listModelsBySubtype("", `http://sdapi-malformed-entry-${index}.example.test`, "loras");
            }
            catch (error) {
                caught = error;
            }
    ...
    test("preserves the canonical endpoint's non-2xx provider error", async () => {
        fetchStub = stubModelFetch(() => Response.json({ error: "LoRA endpoint unavailable", code: "lora_unavailable" }, { status: 503, statusText: "Service Unavailable" }));
        let caught;
        try {
    ...
    test("wraps a canonical endpoint response-body transport failure as retryable", async () => {
        fetchStub = stubModelFetch(() => new Response(new ReadableStream({
            pull(controller) {
