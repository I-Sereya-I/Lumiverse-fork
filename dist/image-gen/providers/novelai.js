...
export class NovelAIImageProvider {
    ...
    async generate(apiKey, _apiUrl, request) {
        ...
        const charTags = params.characterTags || [];
        ...
        const directorImages = params.resolvedReferenceImages || [];
...
function isNovelAIV4OrLaterModel(model) {
...
async function extractImageFromResponse(res, signal) {
    let fullBuffer;
    ...
    const primaryBuffer = fullBuffer.buffer.slice(fullBuffer.byteOffset, fullBuffer.byteOffset + fullBuffer.byteLength);
    // Strategy 1: Direct PNG scan
    let imageBytes = extractPngFromBuffer(primaryBuffer);
    if (imageBytes)
        return `data:image/png;base64,${uint8ToBase64(imageBytes)}`;
    // Strategy 2: ZIP archive scan
    const pkIndex = findBytes(fullBuffer, [0x50, 0x4b, 0x03, 0x04]);
    if (pkIndex !== -1) {
        const zipSlice = fullBuffer.slice(pkIndex);
        const zipBuffer = zipSlice.buffer.slice(zipSlice.byteOffset, zipSlice.byteOffset + zipSlice.byteLength);
    ...
    if (largestBinary)
        return `data:image/png;base64,${uint8ToBase64(largestBinary)}`;
    throw new Error(`Could not extract image from ${fullBuffer.length} byte NovelAI response`);
}
function findLargestBinary(value, depth = 0) {
...
function findNovelAIStreamError(value, depth = 0) {
    if (!value || typeof value !== "object" || depth >= 4)
        return null;
    const record = value;
...
async function padDirectorRefImage(base64Data) {
