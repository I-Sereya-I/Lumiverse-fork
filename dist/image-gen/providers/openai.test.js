 ...
 */
function stubFetch(responseJson = { data: [{ b64_json: "aGVsbG8=" }] }) {
    const calls = [];
...
describe("OpenAIImageProvider — generations endpoint", () => {
    ...
    test("GPT image models omit response_format and auto params", async () => {
        fetchStub = stubFetch();
        const result = await provider.generate("key", BASE, req("gpt-image-1", { size: "auto", quality: "auto", background: "auto", moderation: "auto" }));
    ...
    test("GPT image models pass through non-auto family params", async () => {
        fetchStub = stubFetch();
        await provider.generate("key", BASE, req("gpt-image-1-mini", {
            size: "1536x1024",
            quality: "high",
...
describe("OpenAIImageProvider — edits endpoint (img2img)", () => {
    ...
    test("multiple sources use the image[] array field", async () => {
        ...
        const form = fetchStub.calls[0].body;
    ...
    test("DALL-E edits still request b64_json", async () => {
        fetchStub = stubFetch();
        await provider.generate("key", BASE, req("dall-e-2", { resolvedSourceImages: [{ data: "QUJD", mimeType: "image/png" }] }));
        const form = fetchStub.calls[0].body;
...
describe("OpenAIImageProvider — URL responses and model listing", () => {
    ...
    test("inlines a url-style response as a base64 data URL", async () => {
        ...
        fetchStub = { calls: [], restore: () => (globalThis.fetch = original) };
