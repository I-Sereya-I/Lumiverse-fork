...
const PARAMETERS = {
    ...
    denoise: {
        ...
        step: 0.05,
        default: 0.6,
        description: "Init image creativity (img2img) — applied when a reference/source image is supplied. Higher = more deviation from the source image, lower = stays closer to it.",
        group: "img2img",
    },
    vae: {
        type: "string",
    ...
    rawRequestOverride: {
        type: "string",
        description: 'Raw JSON merged into the request body — flat SwarmUI params (e.g. {"refinermethod": "PostApply", "refinercontrolpercentage": 0.45}), {"presets": ["name"]} to apply a preset saved in SwarmUI, or a pasted SwarmUI preset export',
        group: "advanced",
    },
};
/**
 ...
 */
function unwrapPresetExport(rawJson) {
...
function parseSwarmResponseError(data, operation) {
...
function isStaleSessionError(err) {
    return err instanceof ProviderRequestError && (err.code === "invalid_session_id"
        || /invalid session/i.test(err.code ?? "")
        || /invalid session/i.test(err.detail ?? ""));
}
function invalidModelListingResponse() {
    return new ProviderRequestError({
        provider: "SwarmUI",
...
export class SwarmUIImageProvider {
    name = "swarmui";
    displayName = "SwarmUI";
    capabilities = {
    ...
    getSession(baseUrl, token, signal) {
        const now = Date.now();
        ...
        const sessionPromise = (async () => {
            ...
            if (token)
                headers.Cookie = `swarm_token=${token}`;
        ...
        void sessionPromise.then(() => {
            if (this.pendingSessions.get(key) !== sessionPromise)
                return;
    ...
    async invalidateSession(baseUrl, token, failedSessionId) {
        const key = this.sessionKey(baseUrl, token);
    ...
    buildHeaders(token) {
        ...
        if (token)
            h.Cookie = `swarm_token=${token}`;
        return h;
    }
    resolvedModel(request) {
        const unet = request.parameters?.unet;
        return typeof unet === "string" && unet.trim() ? unet : request.model;
    }
    async requestModelList(base, token, sessionId, query) {
        const body = {
        ...
        if (!res.ok)
            await throwProviderResponseError("SwarmUI", "model listing", res);
        const data = await res.json();
    ...
    buildBody(sessionId, request) {
        ...
        if (p.steps != null && Number.isFinite(Number(p.steps)))
            body.steps = Number(p.steps);
        if (p.cfgScale != null && Number.isFinite(Number(p.cfgScale)))
            body.cfgscale = Number(p.cfgScale);
        if (p.seed != null && Number.isFinite(Number(p.seed)))
            body.seed = Number(p.seed);
        if (p.sampler)
            body.sampler = String(p.sampler);
        if (p.scheduler)
            body.scheduler = String(p.scheduler);
        const neg = request.negativePrompt || p.negativePrompt;
        if (neg)
            body.negativeprompt = String(neg);
        ...
        if (p.vae)
            body.vae = String(p.vae);
        if (p.clipLModel)
            body.cliplmodel = String(p.clipLModel);
        if (p.clipGModel)
            body.clipgmodel = String(p.clipGModel);
        if (p.t5XXLModel)
            body.txxlmodel = String(p.t5XXLModel);
        if (p.qwenModel)
            body.qwenmodel = String(p.qwenModel);
        if (p.mistralModel)
            body.mistralmodel = String(p.mistralModel);
        if (p.gemmaModel)
            body.gemmamodel = String(p.gemmaModel);
        if (p.llamaModel)
            body.llamamodel = String(p.llamaModel);
        ...
        if (p.loras)
            body.loras = Array.isArray(p.loras) ? p.loras.join(",") : String(p.loras);
        if (p.loraWeights) {
            body.loraweights = Array.isArray(p.loraWeights)
                ? p.loraWeights.map((v) => String(v)).join(",")
    ...
    async fetchImage(baseUrl, imagePath, token, signal) {
        // Already a data URL
    ...
    async generate(apiKey, apiUrl, request) {
        ...
        if (workflow && typeof workflow === "object") {
            const { imageDataUrl } = await executeComfyWorkflow(`${base}/ComfyBackendDirect`, workflow, request.signal, { label: "SwarmUI/ComfyBackendDirect", cookie: token ? `swarm_token=${token}` : undefined, wsTimeoutMs: 15_000 });
            return {
                imageDataUrl,
    ...
    async *generateStream(apiKey, apiUrl, request) {
        ...
        const workflow = request.parameters?.workflow;
        if (workflow && typeof workflow === "object") {
            const stream = executeComfyWorkflowStream(`${base}/ComfyBackendDirect`, workflow, request.signal, { label: "SwarmUI/ComfyBackendDirect", cookie: token ? `swarm_token=${token}` : undefined, wsTimeoutMs: 15_000 });
            while (true) {
        ...
        try {
            ...
            request.signal?.removeEventListener("abort", abortHandler);
            await closeWebSocketGracefully(ws, request.signal?.aborted ? 100 : 1_000);
        }
        if (!imagePath) {
    ...
    async fetchModelList(apiKey, apiUrl, query) {
        const base = this.baseUrl(apiUrl);
        const token = apiKey || undefined;
            ...
            try {
                ...
                if (gp && typeof gp === "object") {
                    ...
                    if (pct != null) {
                        ...
                        const preview = typeof gp.preview === "string" && gp.preview ? gp.preview : undefined;
