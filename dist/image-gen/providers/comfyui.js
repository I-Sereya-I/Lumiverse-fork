 ...
 */
function assertSafeComfyUrl(apiUrl) {
    let parsed;
    try {
...
export class ComfyUIImageProvider {
    ...
    async generate(_apiKey, apiUrl, request) {
        ...
        const { imageDataUrl } = await executeComfyWorkflow(baseUrl, workflow, request.signal, { label: "ComfyUI", wsTimeoutMs: 10_000 });
        return {
            imageDataUrl,
    ...
    async *generateStream(_apiKey, apiUrl, request) {
        const baseUrl = apiUrl || this.capabilities.defaultUrl;
        assertSafeComfyUrl(baseUrl);
        const workflow = request.parameters?.workflow;
        if (!workflow || typeof workflow !== "object") {
        ...
        const stream = executeComfyWorkflowStream(baseUrl, workflow, request.signal, { label: "ComfyUI", wsTimeoutMs: 10_000 });
        while (true) {
    ...
    async validateKey(_apiKey, apiUrl) {
        try {
            const baseUrl = apiUrl || this.capabilities.defaultUrl;
            assertSafeComfyUrl(baseUrl);
    ...
    async listModels(_apiKey, apiUrl) {
        const baseUrl = apiUrl || this.capabilities.defaultUrl;
        assertSafeComfyUrl(baseUrl);
        const data = await fetchProviderJson(this.displayName, "model listing", `${baseUrl}/object_info/CheckpointLoaderSimple`, {
    ...
    async listModelsBySubtype(_apiKey, apiUrl, subtype) {
        const baseUrl = apiUrl || this.capabilities.defaultUrl;
        assertSafeComfyUrl(baseUrl);
...
function getCapabilityList(capabilities, subtype) {
    switch (subtype) {
        case "checkpoints":
        case "checkpoint":
