...
import { _resetForTests, _setApprovedBrokerOriginsForTests, APPROVED_BROKER_ORIGINS_SETTING_KEY, getApprovedBrokerOrigins, InvalidBrokerOriginError, load, normalizeOrigin, setApprovedBrokerOrigins, } from "./broker-origins.service";
...
describe("approved broker origins persistence", () => {
    beforeEach(async () => {
        ...
        getDb().run(baseline);
        getDb()
            .query('INSERT INTO "user" (id, name, email, emailVerified, createdAt, updatedAt) VALUES (?, ?, ?, 1, 0, 0)')
    ...
    test("malformed persisted entries are skipped on load without crashing", () => {
        ...
        getDb()
            .query("UPDATE settings SET value = ? WHERE key = ? AND user_id = ?")
