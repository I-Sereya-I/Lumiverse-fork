const ENRICH_GATE_CRITERIA = [
    {
        key: "grounded",
        label: "Grounded",
        description: "Everything added is supported by or directly extends what the book's entries establish; nothing contradicts them or arrives from outside the book.",
    },
    {
        key: "preserving",
        label: "Preserving",
        description: "Every claim the original entry made survives in substance. Nothing was rewritten away, weakened, or replaced by a different idea.",
    },
    {
        key: "playable",
        label: "Playable",
        description: "The additions are concrete, scene-usable specifics — who, where, what it looks and sounds like, what it costs — not abstract elaboration or filler prose.",
    },
];
...
export const IMPORT_ACTIONS = [
    {
        ...
        seedType: "card",
        readingCue: "a single persona: one person who speaks as themselves and owns the card's voice",
        order: 1,
    },
    {
        id: "world_treatment",
        ...
        seedType: "card",
        readingCue: "a narrator or scenario card: it runs a place or situation, sets scenes and voices many people, and no single persona owns the voice",
        order: 2,
    },
    {
        id: "enrich",
...
export function getImportAction(id) {
    return IMPORT_ACTIONS.find((a) => a.id === id);
}
export function importActionsFor(artifact) {
    return IMPORT_ACTIONS.filter((a) => a.artifact === artifact).sort((a, b) => a.order - b.order);
}
export function readingActionsFor(artifact) {
