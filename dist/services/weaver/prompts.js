...
export function buildExtractionUserMessage(seedText) {
    return `SOURCE MATERIAL:\n\n${seedText.trim()}`;
}
const DEFAULT_SOURCE_NOUN = "dream";
const FACT_SOURCE_MARK = {
...
function formatFacts(reg, facts, sourceNoun = DEFAULT_SOURCE_NOUN) {
    if (facts.length === 0)
        return "(nothing established yet)";
    return facts
        .map((f) => {
        const label = getSlot(reg.slots, f.slot)?.label ?? f.slot;
        const mark = f.source === "extracted"
            ? `from the ${sourceNoun}`
...
function dreamBlock(dream, intent, sourceNoun = DEFAULT_SOURCE_NOUN) {
    const trimmed = dream.trim();
    if (!trimmed)
        return null;
    const ownership = sourceNoun === DEFAULT_SOURCE_NOUN ? "the author's original words" : "the source text, verbatim";
    return `THE ${sourceNoun.toUpperCase()} (${ownership} — ${intent}):\n"""\n${trimmed}\n"""`;
}
function formatDepthNotes(dynamic) {
    if (dynamic.length === 0)
        return null;
    return dynamic
...
export function buildInterviewerPrompt(reg, sourceNoun = DEFAULT_SOURCE_NOUN) {
...
export function buildInterviewerUserMessage(reg, input) {
    const parts = [];
...
export function buildQuestionGatePrompt(reg) {
    const { noun } = reg.subject;
...
export function buildQuestionGateUserMessage(reg, input) {
    const parts = [];
    parts.push(`THE PROPOSED QUESTION:\n${input.prompt}`);
    if (input.why.trim())
        parts.push(`Its stated intent: ${input.why.trim()}`);
...
export function buildSparkPrompt(reg, sourceNoun = DEFAULT_SOURCE_NOUN) {
    const { noun } = reg.subject;
...
export function buildSparkUserMessage(reg, input) {
    const parts = [];
    parts.push(`THE QUESTION:\n${input.prompt}`);
    if (input.why.trim())
        parts.push(`What it pins down: ${input.why.trim()}`);
...
export function buildEnhanceUserMessage(reg, input) {
    const parts = [];
    parts.push(`THE QUESTION:\n${input.prompt}`);
...
export function buildSpilloverPrompt(reg) {
    const { noun } = reg.subject;
    const parts = [];
...
export function buildSpilloverUserMessage(reg, input) {
    return [
        `THE QUESTION:\n${input.prompt}`,
...
export function buildBibleSynthesisPrompt(reg, group, sourceNoun = DEFAULT_SOURCE_NOUN) {
    const { noun } = reg.subject;
...
export function buildBibleSynthesisUserMessage(reg, input) {
    const parts = [];
    const dream = dreamBlock(input.dream, "the texture and specifics to honor; never flatten or genericize them", input.source_noun);
    if (dream)
        parts.push(`${dream}\n`);
...
export function buildBibleWeavePrompt(reg) {
...
export function buildBibleWeaveUserMessage(reg, spine, dreamText = "", sourceNoun = DEFAULT_SOURCE_NOUN) {
    const entries = formatSpineEntries(reg, spine);
    const parts = [];
    const dream = dreamBlock(dreamText, "ground the brief's language in this texture; do not import facts the spine lacks", sourceNoun);
    if (dream)
        parts.push(`${dream}\n`);
    parts.push(`THE ASSEMBLED SPINE (connect these; do not invent beyond them):\n${entries || "(no entries)"}\n\nFor causal_links, reference slots by their id. Valid slot ids:\n${reg.slots.map((s) => `- ${s.id}`).join("\n")}`);
    return parts.join("\n");
}
function gateRubric(criteria) {
    return criteria.map((c) => `- ${c.key} (${c.label}): ${c.description}`).join("\n");
}
export function buildBibleGatePrompt(reg, applicable) {
    const { noun } = reg.subject;
...
function voiceMaterial(reg, spine) {
    return spine.entries.find((e) => e.slot === reg.voiceSlot)?.content.trim() ?? "";
}
export function buildBibleGateUserMessage(reg, spine, dreamText = "", sourceNoun = DEFAULT_SOURCE_NOUN) {
    const entries = formatSpineEntries(reg, spine);
    const links = formatSpineLinks(reg, spine);
    const parts = [];
    const dream = dreamBlock(dreamText, "the source material for the fidelity check", sourceNoun);
    if (dream)
        parts.push(`${dream}\n`);
...
export function buildFieldRenderPrompt(reg, field, narrationMode) {
    const { noun } = reg.subject;
    const focus = field.primarySlots.map((s) => getSlot(reg.slots, s)?.label ?? s).join(", ");
    const narration = field.narrated && narrationMode
...
export function buildFieldRenderUserMessage(reg, input) {
...
export function buildFieldNudgeUserMessage(reg, input) {
...
export function buildFieldGatePrompt(reg, field) {
    const { noun } = reg.subject;
...
export function buildFieldGateUserMessage(reg, input) {
...
export function buildDynamicWeavePrompt(reg) {
    const { noun } = reg.subject;
...
export function buildDynamicWeaveUserMessage(reg, input) {
    const parts = [];
    const dream = dreamBlock(input.dream, "ground the entries' language in this; never import facts the answers lack", input.source_noun);
    if (dream)
        parts.push(dream);
...
export function buildDynamicWeaveGatePrompt(reg) {
    const { noun } = reg.subject;
...
export function buildDynamicWeaveGateUserMessage(reg, input) {
...
function establishedMaterialBlock(reg, material) {
    const parts = [];
    ...
    if (loreLines.length > 0)
        parts.push(`THE LORE SO FAR:\n${loreLines.join("\n")}`);
    return parts.join("\n\n");
}
export function buildPeopleHarvestPrompt(reg) {
...
export function buildPeopleHarvestUserMessage(reg, input) {
    const parts = [establishedMaterialBlock(reg, input.material)];
...
export function buildPeopleProposePrompt(reg) {
    const people = reg.people;
...
export function buildPeopleProposeUserMessage(reg, input) {
    const parts = [establishedMaterialBlock(reg, input.material)];
    const taken = input.takenNames.map((n) => n.trim()).filter(Boolean);
    if (taken.length > 0) {
...
function personBlock(person) {
    ...
    if (person.hook.trim())
        lines.push(`- Hook: ${person.hook.trim()}`);
...
export function buildPersonExtraPrompt(reg) {
    const people = reg.people;
...
export function buildPersonExtraUserMessage(reg, input) {
    return [establishedMaterialBlock(reg, input.material), personBlock(input.person)].join("\n\n");
}
export function buildPersonQuestionPrompt(reg) {
    const people = reg.people;
...
export function buildPersonQuestionUserMessage(reg, input) {
...
export function buildPersonQuestionGatePrompt(reg) {
    const people = reg.people;
...
export function buildPersonQuestionGateUserMessage(reg, input) {
...
export function buildPersonWeavePrompt(reg) {
    const people = reg.people;
...
export function buildPersonWeaveUserMessage(reg, input) {
    const parts = [establishedMaterialBlock(reg, input.material), personBlock(input.person)];
    if (input.revise) {
...
export function buildPersonWeaveGatePrompt(reg) {
    const people = reg.people;
...
export function buildPersonWeaveGateUserMessage(reg, input) {
    return [
        `THE ENTRY:\n${input.content}`,
...
export function buildImportReadingPrompt(actions) {
...
export function buildImportReadingUserMessage(source) {
    return `THE SOURCE:\n"""\n${source.trim()}\n"""`;
}
export function buildEntryEnrichPrompt(work) {
...
export function buildEntryEnrichUserMessage(input) {
    ...
    if (input.others.length > 0) {
        parts.push(`THE REST OF THE BOOK (the established material — stay strictly inside it):\n${input.others
