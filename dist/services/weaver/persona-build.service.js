...
export const PERSONA_SECTIONS = [
    {
        id: "identity",
        label: "Identity",
        guidance: "Who they are at the core: the one-line vibe, what they read as at a glance, age/gender where it matters. Not the name (that is separate).",
    },
    {
        id: "appearance",
        label: "Appearance",
        guidance: "A lean physical read — build/height, hair, eyes, the signature clothing or item that fixes them. Only what matters at a glance, never a full spec sheet.",
    },
    {
        id: "personality",
        label: "Personality",
        guidance: "How they behave and relate: a few sharp, specific traits; how they carry themselves; how they treat the people around them.",
    },
    {
        id: "drive",
        label: "Drive",
        guidance: "The one or two wants that actually pull them through a scene. Concrete, not abstract virtues.",
    },
];
export function serializePersonaBody(sections) {
    return sections
...
function buildHostContextBlock(userId, session) {
...
function buildGenerateUserMessage(seedText, ctx) {
...
export async function generatePersonaDraft(userId, sessionId) {
    ...
    const sections = PERSONA_SECTIONS.map((def) => ({
        id: def.id,
        label: def.label,
...
function buildGreetingSystemPrompt(registerGuidance) {
...
function buildGreetingUserMessage(args) {
    const parts = [];
    if (args.charName)
        parts.push(`CHARACTER: ${args.charName}`);
    if (args.voice)
        parts.push(`{{char}}'S VOICE:\n${args.voice.slice(0, 1000)}`);
    if (args.scenario)
        parts.push(`SCENARIO FRAME:\n${args.scenario.slice(0, 800)}`);
    if (args.firstMes)
        parts.push(`{{char}}'S DEFAULT OPENING (for voice reference — write a DIFFERENT opening):\n${args.firstMes.slice(0, 1000)}`);
...
export async function generatePairedGreeting(userId, sessionId, draft, registerId) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
...
export function commitPersona(userId, session, draft) {
