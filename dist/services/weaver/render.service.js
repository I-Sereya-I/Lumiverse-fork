...
import { criteriaForKind, buildFieldGateVerdict, deriveFieldStatus, } from "./field-gate";
import { buildFieldRenderPrompt, buildFieldRenderUserMessage, buildFieldReviseUserMessage, buildFieldNudgeUserMessage, buildFieldGatePrompt, buildFieldGateUserMessage, } from "./prompts";
import { getNarrationMode } from "./narration";
import { weaverGenerateTextWithUsage, weaverGenerateJsonWithUsage, } from "./llm";
...
function spineContentHash(spine) {
...
function deriveStale(def, field, currentHash) {
    if (def.render === "direct")
        return false;
...
function fieldRowId(sessionId, fieldId) {
...
function parseVerdict(value) {
    if (!value || typeof value !== "object")
        return null;
    const obj = value;
    if (!Array.isArray(obj.criteria) && typeof obj.summary !== "string")
        return null;
...
function persistField(userId, sessionId, fieldId, content, status, provenance, usage) {
    getDb()
        .prepare(`INSERT INTO weaver_fields (id, session_id, user_id, field_name, content, status, provenance, token_usage, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, unixepoch())
       ON CONFLICT(id) DO UPDATE SET
        ...
        .run(fieldRowId(sessionId, fieldId), sessionId, userId, fieldId, content, status, JSON.stringify(provenance), JSON.stringify(usage));
}
/** The live Bible's content fingerprint, or null if there is no Bible yet. */
function currentSpineHash(userId, sessionId) {
...
export function getField(userId, sessionId, fieldId) {
    const row = getDb()
        .prepare(`SELECT * FROM weaver_fields WHERE session_id = ? AND user_id = ? AND field_name = ?`)
        .get(sessionId, userId, fieldId);
    if (!row)
        return null;
...
function failureVerdict(message) {
    return { passed: false, criteria: [], summary: message };
}
async function gateContent(userId, session, reg, field, content, spine, signal) {
...
export async function renderField(userId, sessionId, fieldId, opts = {}) {
    const { signal, force, nudge } = opts;
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    const reg = getBuildRegistry(session.build_type);
    if (!isFieldId(reg.fieldDefs, fieldId))
    ...
    if (existing?.status === "manually_edited" && !force)
        throw new Error(HAND_EDIT_GUARD);
    const bible = getBible(userId, sessionId);
    if (!bible)
        throw new Error("Synthesize a Bible first — there is nothing to render from");
    if (bible.spine.entries.length === 0)
        throw new Error("The Bible has no spine yet — synthesize it first");
    ...
    if (field.render === "direct") {
        ...
        const verdict = content
            ? {
                passed: true,
                criteria: [],
                summary: "Carried straight from the Bible — this is already established, so it isn't generated or gated by the model.",
            }
            : failureVerdict("The Bible has no value for this field yet — go back and complete it.");
        const provenance = { ...provenanceBase, gate: verdict, revised: false };
    ...
    try {
        ...
        const provenance = { ...provenanceBase, gate: gate.verdict, revised, nudge: steer };
    ...
    if (steer)
        addSteer(userId, steer);
    setStage(userId, sessionId, "render");
    return getField(userId, sessionId, field.id);
}
export async function renderAllFields(userId, sessionId, signal) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    const reg = getBuildRegistry(session.build_type);
    ...
    if (bible.spine.entries.length === 0)
        throw new Error("The Bible has no spine yet — synthesize it first");
    const locked = new Set(getFields(userId, sessionId)
...
export async function reRenderField(userId, sessionId, fieldId, opts = {}) {
    return renderField(userId, sessionId, fieldId, opts);
}
export async function reRenderWithNudge(userId, sessionId, fieldId, nudge, opts = {}) {
    const steer = nudge.trim();
    if (!steer)
        throw new Error("Add a nudge to steer the re-render");
    return renderField(userId, sessionId, fieldId, { signal: opts.signal, force: opts.force, nudge: steer });
}
export function editField(userId, sessionId, fieldId, content) {
    if (!isFieldId(regFor(userId, sessionId).fieldDefs, fieldId))
        throw new Error(`Unknown field: ${fieldId}`);
    const existing = getField(userId, sessionId, fieldId);
    if (!existing)
        throw new Error("Render the field first — there is nothing to edit yet");
    const trimmed = content.trim();
    if (!trimmed)
        throw new Error("A field cannot be empty");
    const provenance = {
        ...existing.provenance,
...
export function acceptField(userId, sessionId, fieldId, accepted) {
    if (!isFieldId(regFor(userId, sessionId).fieldDefs, fieldId))
        throw new Error(`Unknown field: ${fieldId}`);
    const existing = getField(userId, sessionId, fieldId);
    if (!existing)
        throw new Error("Render the field first — there is nothing to accept yet");
    const provenance = { ...existing.provenance, accepted };
