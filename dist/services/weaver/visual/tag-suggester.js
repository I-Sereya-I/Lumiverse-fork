...
function extractAppearance(description) {
    const re = /\[([A-Z][A-Z ]*)\]/g;
    const marks = [];
    let m;
...
export function buildTagEvidenceFromCharacter(character) {
    return {
...
function parseAppearanceSections(appearance) {
...
function looksLikeMeasurementOrDate(value) {
    const v = value.trim();
    if (/^\d[\d.,'"\s]*(?:cm|mm|m|kg|lb|lbs|inch|inches|ft|'|")?$/i.test(v))
        return true;
    if (/^\d+(?:[.,]\d+)?(?:\s*[,/]\s*\d+(?:[.,]\d+)?){1,5}\s*(?:"|cm)?$/.test(v))
        return true;
    if (/\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b/i.test(v))
        return true;
    if (/\b\d{1,2}(?:st|nd|rd|th)\b/i.test(v))
        return true;
    if (/^[a-zA-Z]$/.test(v))
        return true;
    return false;
}
function extractSeedTagsFromValue(value, suffix) {
    return value
        .split(/[,\n]/)
        ...
        if (!suffix)
            return part;
        if (part.toLowerCase().includes(suffix.toLowerCase()))
            return part;
        return `${part} ${suffix}`;
    });
}
/** Deterministic fallback seed: quality baseline + portrait basics + parsed appearance. */
export function buildDeterministicTagSeed(evidence) {
    const tags = new Set(["masterpiece", "best quality", "newest", "1girl", "solo", "looking_at_viewer", "portrait"]);
    for (const section of parseAppearanceSections(evidence.appearance)) {
        if (section.key.includes("hair")) {
            for (const tag of extractSeedTagsFromValue(section.value, "hair"))
                if (!looksLikeMeasurementOrDate(tag))
                    tags.add(tag);
        }
...
function buildTagSuggestionMessages(evidence) {
...
export function parseSectionedTagResponse(content) {
...
export async function suggestVisualTags(input) {
