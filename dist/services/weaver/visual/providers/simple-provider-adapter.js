...
export function snapshotParameters(parameters) {
    return Object.fromEntries(Object.entries(parameters).map(([key, value]) => IMAGE_PAYLOAD_KEYS.has(key) ? [key, "[source image]"] : [key, value]));
}
function applySourceImage(provider, source, parameters) {
    switch (provider) {
        case "google_gemini":
        case "openrouter":
        ...
        case "nanogpt":
            ...
            if (parameters.strength == null)
                parameters.strength = INIT_DENOISE_DEFAULT;
            return;
        case "novelai":
        ...
        case "sdapi":
            ...
            if (parameters.denoising_strength == null)
                parameters.denoising_strength = INIT_DENOISE_DEFAULT;
            return;
        case "swarmui":
            parameters.resolvedSourceImages = [{ data: source.data, mimeType: source.mimeType }];
            return;
...
function filterSupportedParameters(provider, parameters) {
    const schema = getImageProvider(provider)?.capabilities.parameters ?? {};
    return Object.fromEntries(Object.entries(parameters ?? {}).filter(([key]) => key in schema));
}
function buildProviderSpecificParameters(provider, asset) {
    switch (provider) {
        case "novelai":
            return {
        ...
        case "sdapi":
        case "swarmui": {
            const stateParams = (asset.provider_state?.params ?? {});
...
export function createSimpleProviderAdapter(provider) {
    return {
        provider,
        ...
        async build(asset, connection) {
            const supportedDefaults = filterSupportedParameters(provider, connection.default_parameters);
            const assetParams = filterSupportedParameters(provider, asset.provider_state?.params);
