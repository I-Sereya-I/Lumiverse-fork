...
import { patchWorkflow } from "../comfyui-workflow-patch";
const INIT_DENOISE_DEFAULT = 0.5;
export function comfyImageInputSupport(connection) {
...
export const comfyUIProviderAdapter = {
    ...
    async validate(asset, connection) {
        ...
        const hasPositivePrompt = config.field_mappings.some((m) => m.mappedAs === "positive_prompt");
    ...
    async build(asset, connection, apiKey) {
        ...
        const normalizedWorkflow = normalizeComfyUIWorkflow(config.workflow_json, objectInfo ?? undefined);
        const patchableWorkflow = normalizedWorkflow.apiWorkflow;
        if (asset.source_image) {
            const uploaded = await uploadComfyImage(target.baseUrl, { data: asset.source_image.data, mimeType: asset.source_image.mimeType }, { cookie: target.cookie });
            values.init_image = uploaded.subfolder
                ? `${uploaded.subfolder}/${uploaded.name}`
                : uploaded.name;
            if (values.denoise === undefined) {
                const stateDenoise = Number(asset.provider_state?.params?.denoise);
                values.denoise = Number.isFinite(stateDenoise) ? stateDenoise : INIT_DENOISE_DEFAULT;
            }
        }
        const patchedWorkflow = patchWorkflow(patchableWorkflow, config.field_mappings, values);
        return {
            request: {
                prompt: asset.prompt,
