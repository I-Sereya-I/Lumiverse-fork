...
import { completeVisualJob, createVisualJob, failVisualJob, getVisualJob, updateVisualJobProgress, } from "./jobs";
function emitJobEvent(event, job) {
...
export function buildVisualAsset(input, connection, sourceImage) {
...
async function generateWithOptionalStreaming(job, input, asset, settingsSnapshot) {
    ...
    const adapter = getVisualProviderAdapter(input.connection.provider);
    if (!adapter) {
    ...
    emitJobEvent(EventType.WEAVER_VISUAL_JOB_PROGRESS, updateVisualJobProgress(job.id, job.userId, {
        stage: "generating",
    ...
    if (provider.generateStream) {
        const stream = provider.generateStream(input.apiKey, input.connection.api_url || provider.capabilities.defaultUrl || "", buildResult.request);
    ...
    const response = await provider.generate(input.apiKey, input.connection.api_url || provider.capabilities.defaultUrl || "", buildResult.request);
    return {
        image_url: response.imageDataUrl,
        settingsSnapshot: finalSettingsSnapshot,
    };
}
async function executeWeaverVisualJob(job, input) {
    console.debug("[Weaver:Visual] Starting job=%s session=%s kind=%s provider=%s", job.id, job.sessionId, job.kind, input.connection.provider);
    try {
        ...
        console.debug("[Weaver:Visual] Job completed. job=%s image_id=%s has_url=%s", job.id, finalResult.image_id ?? "(none)", Boolean(finalResult.image_url));
        emitJobEvent(EventType.WEAVER_VISUAL_JOB_COMPLETED, completeVisualJob(job.id, job.userId, finalResult));
    }
    catch (error) {
        console.error("[Weaver:Visual] Job failed. job=%s error=%s", job.id, toErrorMessage(error));
        emitJobEvent(EventType.WEAVER_VISUAL_JOB_FAILED, failVisualJob(job.id, job.userId, toPublicVisualError(error)));
    }
    finally {
        input.onSettled?.();
    }
}
export function startWeaverVisualJob(input) {
...
export function getWeaverVisualJob(userId, jobId) {
