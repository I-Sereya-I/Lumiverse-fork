export const DESCRIPTION_SECTIONS = [
    {
        ...
        format: "Lead with `{Name} — {one-line essence}.` Then these labeled lines, each on its own:\n" +
    ...
    },
    {
        ...
        format: "Prefix the section literally with `(consult for spatial tracking — never narrated)`. This is a reference sheet, not prose — state details plainly, never rhapsodize. Then:\n" +
    ...
    },
    {
        ...
        format: "First the behavioral signatures as `IF {trigger} THEN {response}.` lines — one per line, drawn from the Bible's tensions. Then a `Conflicts:` line stating the central contradiction as opposed pulls: `{X} vs {Y} ({the bind in a few words})`.",
    },
    {
        tag: "INTENT",
        sourceSlots: ["intents"],
        format: "Three labeled lines: `Super-objective:` the lifelong want that drives them; `Obstacle:` what blocks it (usually an internal pattern, not an external barrier); `Strategy:` how they actually pursue it, turn to turn.",
    },
    {
        tag: "NEGATIVE SPACE",
        sourceSlots: ["negative_space"],
        format: "A few terse lines on what they will NOT say or do, how they deflect when pressed, and the specific tell that shows when they are cornered. This is the shape of what they avoid — keep it concrete and behavioral, not a feelings summary.",
    },
    {
        tag: "GRADIENT",
        sourceSlots: ["gradient"],
        format: "Optionally lead with `(attachment: {style})` when the Bible establishes an attachment style. Then the four bands plus the gate, each labeled:\n" +
    ...
    },
    {
        ...
        format: "Lead with `(axis: {name}, {low pole} → {high pole})`. Then state the band-deltas: how the character's mode, voice, and openness differ at LOW vs HIGH on the axis, and state plainly that hard limits never move at any level. Only the declared deltas shift.",
    },
    {
        tag: "INTIMACY",
        ...
        format: "Structural and non-explicit only: what they are drawn to, what they refuse, where the lines are, and how desire shows up in their behavior. Describe the shape of it — never graphic content.",
    },
];
export function buildDescriptionBundleGuidance() {
