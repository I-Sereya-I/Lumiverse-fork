...
import { emptyPersonaPlan, } from "../../types/weaver";
const VALID_STAGES = [
    "dream",
    "readback",
    "interview",
...
export function nameFromSpine(spineJson, nameSlot) {
    if (typeof spineJson !== "string" || !spineJson.trim())
        return null;
    try {
...
export function nameFromFacts(factsJson, nameSlot) {
    if (typeof factsJson !== "string" || !factsJson.trim())
        return null;
    try {
...
function fillDisplayNames(userId, sessions) {
    ...
    return sessions.map((s) => {
        ...
        const name = nameFromSpine(spines.get(s.id), nameSlot) ?? nameFromFacts(facts.get(s.id), nameSlot);
...
function parsePersonaPlan(value) {
    ...
    try {
        ...
        if (!p || typeof p !== "object")
            return base;
        const pairing = (p.pairing && typeof p.pairing === "object" ? p.pairing : {});
        return {
...
function nextSessionNumber(userId) {
    const row = getDb()
        .prepare(`SELECT COALESCE(MAX(session_number), 0) + 1 AS next
         FROM weaver_sessions
        WHERE user_id = ?`)
        .get(userId);
    return row.next;
}
export function createSession(userId, input = {}) {
    const db = getDb();
    ...
    db.prepare(`INSERT INTO weaver_sessions (
     ...
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, userId, sessionNumber, now, now, buildType, input.seed_type?.trim() || "dream", input.seed_text ?? "", JSON.stringify(input.seed_provenance ?? {}), normalizeOptionalText(input.connection_id), normalizeOptionalText(input.model), normalizeOptionalText(input.persona_id), normalizeOptionalText(input.narration_mode));
...
export function listSessions(userId) {
    const rows = getDb()
        .prepare(`SELECT * FROM weaver_sessions
        WHERE user_id = ?
        ORDER BY updated_at DESC, created_at DESC`)
        .all(userId);
    return fillDisplayNames(userId, rows.map(rowToSession));
}
export function updateSession(userId, sessionId, input) {
...
export function setStage(userId, sessionId, stage) {
