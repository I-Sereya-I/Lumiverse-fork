...
import { getDynamicState, answerDynamicQuestion, clearDynamicLane, } from "./dynamic-question.service";
import { buildDynamicQuestionVerdict } from "./dynamic-question-gate";
import { weaverGenerateJson } from "./llm";
import { buildInterviewerPrompt, buildInterviewerUserMessage, buildQuestionGatePrompt, buildQuestionGateUserMessage, buildSparkPrompt, buildSparkUserMessage, buildEnhancePrompt, buildEnhanceUserMessage, buildSpilloverPrompt, buildSpilloverUserMessage, } from "./prompts";
...
export function getTaste(userId) {
    const row = getDb()
    ...
    try {
        ...
        const steers = Array.isArray(parsed?.steers)
            ? parsed.steers.filter((s) => typeof s === "string" && s.trim())
            : [];
        return { steers };
    }
    catch {
...
const MAX_STEERS = 24;
export function addSteer(userId, steer) {
    ...
    getDb()
        .prepare(`INSERT INTO weaver_taste (user_id, profile, updated_at)
       VALUES (?, ?, unixepoch())
...
export function parseTurnQuestion(value) {
    if (typeof value !== "string")
        return { prompt: "", why: "" };
    try {
        const o = JSON.parse(value);
        const prompt = typeof o?.prompt === "string" ? o.prompt : typeof o?.name === "string" ? o.name : "";
...
function listTurns(userId, sessionId) {
    const rows = getDb()
        .prepare(`SELECT * FROM weaver_interview_turns
...
function remainingTargets(reg, userId, sessionId) {
    ...
    for (const slot of reg.slots) {
        if (!gapSlots.has(slot.id) || !slotHasElicit(reg.slots, slot.id))
            continue;
    ...
    return targets.sort((a, b) => IMPACT_WEIGHT[getSlot(reg.slots, b.slot)?.impact ?? "low"] -
        IMPACT_WEIGHT[getSlot(reg.slots, a.slot)?.impact ?? "low"]);
}
function interviewPhase(session) {
...
function optInAddr(slotId) {
    return `${OPT_IN_PREFIX}:${slotId}`;
}
export function pendingOptIn(reg, facts, gaps, turns, essentialsCovered) {
    if (!essentialsCovered)
        return null;
    for (const slot of reg.slots) {
        if (!slot.optIn)
            continue;
        if (facts.some((f) => f.slot === slot.id && f.fact.trim()))
            continue;
        if (gaps.has(slot.id))
            continue;
        if (turns.some((t) => t.slot === OPT_IN_PREFIX && t.part === slot.id))
            continue;
        return { slot: slot.id };
    }
    return null;
}
export function getInterviewState(userId, sessionId) {
    ...
    const optIn = extraction
        ? pendingOptIn(reg, extraction.committed_facts, new Set(extraction.gaps.map((g) => g.slot)), answered, remaining.length === 0)
        : null;
    return {
...
export function decideOptIn(userId, sessionId, slotId, enabled) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    const reg = getBuildRegistry(session.build_type);
    ...
    if (!slot?.optIn)
        throw new Error("This slot is not an opt-in");
    const decided = listTurns(userId, sessionId).some((t) => t.slot === OPT_IN_PREFIX && t.part === slotId);
    if (!decided) {
        ...
        getDb()
            .prepare(`INSERT INTO weaver_interview_turns
           (id, session_id, user_id, seq, slot, axis, response_kind, response, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, unixepoch())`)
            .run(crypto.randomUUID(), sessionId, userId, nextSeq(sessionId), optInAddr(slotId), JSON.stringify({ prompt, why: "", target: optInAddr(slotId) }), "picked", JSON.stringify(response));
        if (enabled)
            revertSlotsToGaps(userId, sessionId, [slotId]);
    }
    return getInterviewState(userId, sessionId);
}
export function beginInterview(userId, sessionId) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    if (!session.interview_started_at) {
        getDb()
            .prepare(`UPDATE weaver_sessions
...
export function completeInterview(userId, sessionId) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    getDb()
...
function coerceCandidates(value) {
...
function priorityFor(reg, t) {
    ...
    const label = hybrid && partDef ? `${slot?.label ?? t.slot} — ${partDef.label}` : slot?.label ?? t.slot;
...
function fallbackQuestion(reg, t) {
...
export async function generateQuestion(userId, sessionId, input = {}, signal, opts = {}) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    const reg = getBuildRegistry(session.build_type);
    const extraction = getExtraction(userId, sessionId);
    if (!extraction)
        throw new Error("Read the dream first — there is nothing to ask about yet");
    const remaining = remainingTargets(reg, userId, sessionId);
    const dynamic = getDynamicState(userId, sessionId);
    if (!opts.ignoreCap && remaining.length === 0 && dynamic.at_cap)
        return null;
    if (!session.interview_started_at) {
        getDb()
            .prepare(`UPDATE weaver_sessions
    ...
    for (let attempt = 0; attempt < QUESTION_ATTEMPTS; attempt++) {
        ...
        if (!prompt)
            continue;
        let target = typeof proposal.target === "string" ? proposal.target.trim().replace(".", ":") : "";
...
async function harvestSpillover(userId, sessionId, session, input, signal) {
    try {
        const extraction = getExtraction(userId, sessionId);
        if (!extraction)
            return;
        ...
        const raw = Array.isArray(res.facts)
            ? res.facts
            : [];
        ...
        for (const item of raw.slice(0, MAX_SPILLOVER_FACTS)) {
            ...
            if (!isSlotId(reg.slots, slot) || typeof fact !== "string" || !fact.trim())
                continue;
            const part = typeof item.part === "string" && item.part.trim() ? item.part.trim() : undefined;
            const addr = part ? `${slot}:${part}` : slot;
            if (addr === input.target || slot === input.target)
                continue;
...
export async function answerQuestion(userId, sessionId, input, signal) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    ...
    const kind = RESPONSE_KINDS.includes(input.kind) ? input.kind : "typed";
    ...
    getDb()
        .prepare(`INSERT INTO weaver_interview_turns
         (id, session_id, user_id, seq, slot, axis, response_kind, response, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, unixepoch())`)
        .run(id, sessionId, userId, seq, addr, JSON.stringify({ prompt, why: (q.why ?? "").trim(), target: target || addr }), kind, JSON.stringify(content));
    if (input.steer && input.steer.trim())
        addSteer(userId, input.steer);
    await harvestSpillover(userId, sessionId, session, { prompt, target: addr, answer: content, kind }, signal);
    return getInterviewState(userId, sessionId);
}
export async function sparkQuestion(userId, sessionId, input, signal) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    const extraction = getExtraction(userId, sessionId);
...
export async function enhanceAnswer(userId, sessionId, input, signal) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    const extraction = getExtraction(userId, sessionId);
    ...
    if (!draft)
        throw new Error("Write a draft answer first — there is nothing to extend");
    const res = await weaverGenerateJson({
        userId,
        session,
...
export function resetInterview(userId, sessionId) {
    const session = getSession(userId, sessionId);
    if (!session)
        throw new Error("Session not found");
    const turns = listTurns(userId, sessionId);
    ...
    if (revert.length > 0)
        revertSlotsToGaps(userId, sessionId, revert);
    if (optInSlots.length > 0)
        forgetSlots(userId, sessionId, optInSlots);
    getDb()
