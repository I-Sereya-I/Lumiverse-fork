...
mock.module("./embeddings.service", () => ({
    deleteWorldBookEntryEmbeddings: async (userId, entryId) => {
...
mock.module("./vectorization-queue.service", () => ({
    queueWorldBookEntryVectorization: (userId, entryId, priority, supersedesIndexed) => {
...
describe("lorebook edit vector side effects", () => {
    test("baseline indexes authored-order pagination by book", () => {
        const indexes = getDb().query("PRAGMA index_list('world_book_entries')").all();
    ...
    test("content edits mark pending and enqueue replacement instead of deleting Lance rows", () => {
        ...
        if (!entry)
            throw new Error("failed to create vectorized lorebook entry");
        queued.length = 0;
        getDb().query("UPDATE world_book_entries SET vector_index_status = 'indexed', vector_indexed_at = 1 WHERE id = ?").run(entry.id);
