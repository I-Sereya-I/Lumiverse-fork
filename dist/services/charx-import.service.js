...
function importRisuModuleRegexScripts(userId, characterId, module) {
...
async function importRisuExpressionAssets(userId, characterId, assets) {
    if (!assets.length)
        return 0;
 ...
 * a double import).
 */
export function autoImportEmbeddedWorldbook(userId, characterId) {
    const character = svc.getCharacter(userId, characterId);
    if (!character)
        return;
    const charBook = character.extensions?.character_book;
    if (wbSvc.countImportedWorldBookEntries(charBook?.entries) === 0)
        return;
    ...
    try {
        ...
        const nextExtensions = setCharacterWorldBookIds({ ...(character.extensions || {}) }, [...existingIds, worldBook.id]);
 ...
 */
export async function applyCharxModulesAndAssets(userId, character, charxResult, options = {}) {
    ...
    if (lumiverseModules) {
        ...
        if (lumiverseModules.regex_scripts?.length) {
            regexScriptCount = regexSvc.importCharacterBoundRegexScripts(userId, character.id, { lumiverse_modules: { regex_scripts: lumiverseModules.regex_scripts } }, { bundleSource: "charx_bundle" });
        }
        lumiverseModulesSummary = {
            ...
            alternate_field_counts: lumiverseModules.alternate_fields
                ? Object.fromEntries(Object.entries(lumiverseModules.alternate_fields).map(([k, v]) => [k, Array.isArray(v) ? v.length : 0]))
                : undefined,
        };
    }
    // Upload remaining (unconsumed) asset images to the gallery, tracking
    // archive path → image ID for inline asset resolution.
    const remainingGalleryEntries = [];
    for (const [path, assetFile] of assetFiles) {
        if (consumedPaths.has(path))
            continue;
        if (avatarFile && assetFile.name === avatarFile.name)
            continue;
    ...
    await mapWithConcurrency(remainingGalleryEntries, GALLERY_UPLOAD_CONCURRENCY, async ({ path, file: gf }) => {
        ...
        if (options.emitGalleryProgress && galleryTotal > 3) {
            eventBus.emit(EventType.IMPORT_GALLERY_PROGRESS, { characterId: character.id, current: galleryCompleted, total: galleryTotal, filename: gf.name }, userId);
        }
    });
