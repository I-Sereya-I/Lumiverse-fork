...
afterAll(() => {
    if (originalSwarmUI)
        registerImageProvider(originalSwarmUI);
});
describe("character LoRA pipeline splice (SwarmUI)", () => {
    ...
    test("preserves raw provider layers while scaling and bypassing preset/character layers", async () => {
        ...
        expect(sentRequest.parameters?.loras).toBe("provider_first.safetensors,provider_second.safetensors,cinematic_style.safetensors,character_detail.safetensors");
        ...
        expect(sentRequest.parameters?.loras).toBe("provider_first.safetensors,provider_second.safetensors,cinematic_style.safetensors");
        ...
        expect(sentRequest.parameters?.loras).toBe("provider_first.safetensors,provider_second.safetensors,character_detail.safetensors");
        expect(sentRequest.parameters?.loraWeights).toBe("0.3,0.4,0");
        captured = [];
        await generateSceneBackground(USER_ID, chatId, {
            promptMode: "custom",
        ...
        expect(sentRequest.parameters?.loras).toBe("provider_first.safetensors,provider_second.safetensors");
