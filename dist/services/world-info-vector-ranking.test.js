...
import { buildWorldInfoLexicalQueryBatches, rankVectorWorldInfoCandidates, } from "./world-info-vector-ranking";
let entryCounter = 0;
...
function candidate(entry, distance, lexicalScore, lexicalStrength) {
    return {
        entry,
        candidate: {
            entry_id: entry.id,
...
describe("normalizeBm25Scores", () => {
    test("is bounded and exactly invariant under uniform positive scaling", () => {
        ...
        const scaled = normalizeBm25Scores(scores.map((score) => (typeof score === "number" ? score * 97 : score)));
...
describe("per-mention canonical ownership", () => {
    test("gates only mention bonuses while preserving independent relevance", () => {
        ...
        expect(byName.get("Founders Accord").finalScore).toBeGreaterThan(byName.get('"Darling" (Mira Affection)').finalScore);
        const weakestRelevant = Math.min(byName.get("Arden Vale").finalScore, byName.get("Mira Sol").finalScore, byName.get("Founders Accord").finalScore);
        const strongestAssociative = Math.max(byName.get("Vale Residence").finalScore, byName.get('"Darling" (Mira Affection)').finalScore, byName.get("Spirit Protocol").finalScore, byName.get("Ai Rowan").finalScore);
    ...
    test("promotes unanchored lore that connects at least three explicit subjects", () => {
        ...
        expect(byName.get("Council Origins").finalScore).toBeGreaterThan(byName.get("Bridge Officer").finalScore);
        expect(byName.get("Council Origins").finalScore).toBeGreaterThan(byName.get("Vale Residence").finalScore);
        expect(byName.get("Council Origins").finalScore).toBeGreaterThan(byName.get("Distant Biographer").finalScore);
    });
});
