...
function insertChat(userId, characterId) {
...
function seedDefaultPersona(userId, opts = {}) {
...
describe("image-gen parser context macro resolution", () => {
    ...
    test("resolves character card macros before building parser context", async () => {
        ...
        const charInfo = messages.find((message) => message.role === "system" &&
        ...
        const content = charInfo?.content;
    ...
    test("resolves persona block macros independently of Include Characters", async () => {
        ...
        const messages = await buildContextMessages(USER_ID, chatId, getImageGenSettings(USER_ID));
        const persona = messages.find((message) => message.role === "system" &&
            typeof message.content === "string" &&
        ...
        expect(content).not.toContain("{{");
        expect(messages.some((message) => message.role === "system" && String(message.content).startsWith("## Character Information"))).toBe(false);
    });
    ...
    test("resolves world-info outlet macros before building parser context", async () => {
        ...
        if (!entry)
            throw new Error("Failed to create world-book entry");
