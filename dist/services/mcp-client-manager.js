...
class McpClientManager {
    ...
    scheduleReconnect(userId, serverId, reason, immediate = false) {
        const k = this.key(userId, serverId);
        if (this.clients.has(k) || this.reconnectTimers.has(k))
            return;
    ...
    async runScheduledReconnect(userId, serverId, reason) {
        const k = this.key(userId, serverId);
    ...
    async disconnect(userId, serverId, options = {}) {
        const k = this.key(userId, serverId);
        ...
        eventBus.emit(EventType.MCP_SERVER_DISCONNECTED, { id: serverId, name: entry.serverName, reason }, userId);
    }
    async reconnect(userId, server) {
        await this.disconnect(userId, server.id, { reason: "reconnect", resetBackoff: false });
        return this.connect(userId, server);
    }
    updateCachedProfile(userId, server) {
    ...
    async callTool(userId, serverId, toolName, args, timeoutMs = 30_000) {
        const entry = this.clients.get(this.key(userId, serverId));
        if (!entry)
    ...
    async autoConnectAll() {
        ...
        await mapWithConcurrency(servers, MCP_AUTOCONNECT_CONCURRENCY, async (server) => {
            try {
                const status = await this.connect(server.user_id, server);
                if (status.connected) {
    ...
    // ── Private helpers ────────────────────────────────────────────────────────
    async buildHttpTransport(userId, server) {
    ...
    async buildStdioTransport(userId, server) {
    ...
    async discoverTools(client, server) {
        const allTools = [];
        try {
            let cursor;
            do {
