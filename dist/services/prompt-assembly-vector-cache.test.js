...
import { __vectorWiCacheTest, __vectorWiRetrievalTest, } from "./prompt-assembly.service";
...
function fingerprintInput() {
    return {
        ...
        vectorStoreConfig: { provider: "lancedb" },
    };
}
...
describe("vector world-info retrieval cache", () => {
    ...
    test("uses stable object-key ordering while tracking extension changes", () => {
        ...
        expect(__vectorWiCacheTest.buildFingerprint(first)).toBe(__vectorWiCacheTest.buildFingerprint(second));
        second.entries[0].extensions.nested.b = 4;
        expect(__vectorWiCacheTest.buildFingerprint(first)).not.toBe(__vectorWiCacheTest.buildFingerprint(second));
    });
...
describe("vector world-info search fan-out", () => {
    test("searches only attached books containing search-ready entries", () => {
        const searchable = __vectorWiRetrievalTest.getSearchableWorldBookIds(["book-keyword-only", "book-ready", "book-pending"], [
            entry({
                id: "entry-ready",
