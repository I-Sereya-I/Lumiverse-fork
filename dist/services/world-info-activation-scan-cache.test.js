...
import { activateWorldInfo, createWorldInfoActivationScanCache, primeWorldInfoActivationScanCache, } from "./world-info-activation.service";
...
function messages(prefix, contents) {
...
function differingViews(prefix) {
    const make = (name, overrides = {}) => entry({
        id: `${prefix}-${name}`,
        uid: `${prefix}-${name}`,
        comment: name,
        ...overrides,
    ...
    return {
        ...
        native: raw.map((item) => {
            ...
            if (item === constant)
                return { ...item, content: "native-trigger" };
            return item;
        }),
    };
}
describe("world-info activation scan reuse", () => {
    ...
    test("two activation views reuse the message signature and base scan", () => {
        ...
        expect(raw.activatedEntries.map((item) => item.id).sort()).toEqual([constant.id, conditional.id].sort());
    ...
    test("different conditional views share one union scan", () => {
        ...
        const scanCache = createWorldInfoActivationScanCache();
        primeWorldInfoActivationScanCache(scanCache, [nativeEntries, rawEntries]);
        const native = activateWorldInfo({
    ...
    test("primed native and raw views match uncached activation", () => {
        ...
        const scanCache = createWorldInfoActivationScanCache();
        primeWorldInfoActivationScanCache(scanCache, [cachedViews.native, cachedViews.raw], settings);
    ...
    test("same uid with different scan plans stays view-local", () => {
        ...
        const scanCache = createWorldInfoActivationScanCache();
        primeWorldInfoActivationScanCache(scanCache, [[cachedShallow], [cachedDeep]]);
    ...
    test("different message snapshots invalidate cached scan state", () => {
        ...
        const scanCache = createWorldInfoActivationScanCache();
        primeWorldInfoActivationScanCache(scanCache, [[conditional], [conditional]]);
    ...
    test("saturates repeated shared-key outputs after their first match", () => {
        const entries = Array.from({ length: 1_000 }, (_, index) => entry({
            id: `shared-${index}`,
            uid: `shared-${index}`,
            key: ["needle"],
        }));
