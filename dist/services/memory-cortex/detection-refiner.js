...
function dedupeAliases(aliases) {
    ...
    for (const alias of aliases) {
        ...
        if (!canonicalName || !aliasName)
            continue;
        if (!isPlausibleAlias(aliasName, canonicalName))
            continue;
...
function hasBoundary(content, start, end) {
    const before = start > 0 ? content[start - 1] : "";
    const after = end < content.length ? content[end] : "";
    return !/[A-Za-z0-9_]/.test(before) && !/[A-Za-z0-9_]/.test(after);
}
function collectMentionContexts(content, name, padding = 140, maxMatches = 6) {
    ...
    while (windows.length < maxMatches) {
        ...
        cursor = end;
        if (!hasBoundary(content, index, end))
            continue;
...
function countMatchingContexts(contexts, predicate) {
    let count = 0;
...
function hasFactionMembershipSignal(name, contexts) {
    const escaped = escapeRegExp(name);
    return contexts.some((context) => {
        const lowered = context.toLowerCase();
        return (new RegExp(`\\b(?:member|agent|officer|operative|director|leader|captain|employee|staff|soldier|guard|recruit)\\s+(?:of|in|from|with|for)\\s+(?:the\\s+)?${escaped}\\b`, "i").test(context)
...
function hasFactionAssetSignal(name, contexts) {
    const escaped = escapeRegExp(name);
    return contexts.some((context) => (new RegExp(`\\b${escaped}[''\u2019]s\\s+${FACTION_ASSET_RE.source}`, "i").test(context)
        || new RegExp(`\\b${escaped}\\s+${FACTION_ASSET_RE.source}`, "i").test(context)));
}
function hasEventTemporalSignal(name, contexts) {
    const escaped = escapeRegExp(name);
    return contexts.some((context) => (new RegExp(`\\b(?:during|after|before|since|following|at the time of|in the wake of|on the anniversary of)\\s+(?:the\\s+)?${escaped}\\b`, "i").test(context)
...
function hasNegativeCharacterSignal(name, contexts) {
    const escaped = escapeRegExp(name);
    return contexts.some((context) => (new RegExp(`\\b${escaped}${BODY_POSSESSIVE_RE.source}`, "i").test(context)
...
function canonicalizeRelationships(relationships, aliases) {
    if (aliases.length === 0)
    ...
    for (const relationship of relationships) {
        ...
        if (!deduped.has(key))
            deduped.set(key, { ...relationship, source, target });
    }
    return [...deduped.values()];
}
function buildKnownTypeLookup(knownEntities, characterNames, descriptionAliases) {
...
function getDistinctRelationshipCount(name, relationships) {
    const related = new Set();
    for (const rel of relationships) {
        if (normalizeKey(rel.source) === normalizeKey(name))
            related.add(normalizeKey(rel.target));
        if (normalizeKey(rel.target) === normalizeKey(name))
            related.add(normalizeKey(rel.source));
    }
    return related.size;
}
function scoreFactionCandidate(name, contexts, relationships) {
    let score = 0;
    if (FACTION_NAME_RE.test(name))
        score += 0.35;
    if (hasFactionMembershipSignal(name, contexts))
        score += 0.3;
    if (hasFactionAssetSignal(name, contexts))
        score += 0.2;
    if (getDistinctRelationshipCount(name, relationships) >= 2)
        score += 0.15;
    if (hasNegativeCharacterSignal(name, contexts))
        score -= 0.3;
    return Math.max(0, Math.min(1, score));
}
function scoreEventCandidate(name, contexts) {
    let score = 0;
    if (EVENT_HEAD_RE.test(name))
        score += 0.4;
    if (hasEventTemporalSignal(name, contexts))
        score += 0.35;
    const repeatedTemporalSupport = countMatchingContexts(contexts, (context) => EVENT_TEMPORAL_RE.test(context));
    if (repeatedTemporalSupport >= 2)
        score += 0.1;
    if (hasNegativeCharacterSignal(name, contexts))
        score -= 0.35;
    return Math.max(0, Math.min(1, score));
}
function mergeMentionRole(current, next) {
    if (!next)
        return current;
    ...
    if (current === "object" || next === "object")
        return "object";
    if (current === "referenced" || next === "referenced")
        return "referenced";
    return current;
}
function createCanonicalEntity(canonicalName, knownTypeLookup) {
...
export function refineHeuristicDetections(input) {
    ...
    for (const alias of refinedAliases) {
        ...
        if (Math.max(aliasFactionScore, aliasEventScore) >= 0.8)
            continue;
