...
// ─── Emotional Polarity ────────────────────────────────────────
const POSITIVE_EMOTIONS = new Set(["joy", "intimacy", "resolve", "humor", "awe"]);
const NEGATIVE_EMOTIONS = new Set(["grief", "dread", "betrayal", "fury"]);
/**
 * Analyze the emotional polarity of a set of tags.
 */
export function analyzePolarity(tags) {
    let pos = 0, neg = 0, amb = 0;
    for (const tag of tags) {
        if (POSITIVE_EMOTIONS.has(tag))
            pos++;
        else if (NEGATIVE_EMOTIONS.has(tag))
            neg++;
        else
            amb++;
    }
    ...
    if (total === 0)
        return { positive: 0, negative: 0, ambivalent: 0, dominant: "neutral" };
    const dominant = pos > 0 && neg > 0 ? "mixed" :
        pos > neg ? "positive" :
            neg > pos ? "negative" :
                "neutral";
 ...
 */
export function detectEmotionalContext(recentContent) {
 ...
 */
export function calculateEmotionalResonance(queryEmotions, memoryEmotions) {
    if (queryEmotions.length === 0 || memoryEmotions.length === 0)
        return 0;
    const memorySet = new Set(memoryEmotions);
    let matchCount = 0;
    for (const tag of queryEmotions) {
        if (memorySet.has(tag))
            matchCount++;
    }
    if (matchCount === 0) {
    ...
    if (matchCount >= 2)
        score += 0.25;
    if (matchCount >= 3)
        score += (matchCount - 2) * 0.12;
    // Bonus for matching rare/intense emotions (betrayal, death-adjacent)
    const highIntensityTags = new Set(["betrayal", "grief", "fury", "dread"]);
    for (const tag of queryEmotions) {
 ...
 */
export function detectSensoryTriggers(content) {
    const triggers = [];
 ...
 */
export function detectEmotionalTransition(contentBlocks) {
    ...
    if ((firstPolarity.dominant === "positive" && secondPolarity.dominant === "negative") ||
 ...
 */
export function buildEmotionalContext(recentContent, recentBlocks) {
