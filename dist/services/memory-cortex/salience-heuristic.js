...
function getIntensityModifier(keyword, content) {
    const idx = content.toLowerCase().indexOf(keyword.toLowerCase());
    if (idx === -1)
        return 1.0;
    const prefix = content.slice(Math.max(0, idx - 40), idx);
    if (INTENSIFIERS.test(prefix))
        return 1.5;
    if (DIMINISHERS.test(prefix))
        return 0.6;
    return 1.0;
}
...
function isNegatedOrSarcastic(keyword, content) {
    const idx = content.toLowerCase().indexOf(keyword.toLowerCase());
    if (idx === -1)
        return false;
    ...
    if (SARCASM_INDICATORS.some((p) => p.test(sentence)))
        return true;
    const capsPrefix = content.slice(Math.max(0, idx - 20), idx);
    if (/\b[A-Z]{2,}\s*$/.test(capsPrefix))
        return true;
    return false;
}
...
function scoreDialogueContent(content) {
    ...
    for (const block of dialogueBlocks) {
        ...
        if (/\b(I (?:will|won['']t|promise|swear)|you have my word|on my (?:life|honor))\b/i.test(text))
            score += 0.04;
        // Revelation in dialogue
        if (/\b(I (?:need to|have to|should) tell you|the truth is|(?:you|I) (?:should|need to) know|there['']s something|it['']s (?:not|about))\b/i.test(text))
            score += 0.035;
        // Emotional declarations
        if (/\b(I (?:love|hate|trust|forgive|can['']t forgive|miss|need) you|don['']t (?:leave|go|die)|stay (?:with me|here)|come (?:back|home))\b/i.test(text))
            score += 0.04;
        // Decision/agency language
        if (/\b(I(?:['']ve| have)? (?:decided|chosen|made up my mind)|we (?:have to|must|need to|should)|it['']s (?:time|over|done|decided))\b/i.test(text))
            score += 0.03;
        // Name-calling or direct address with emotional weight
        if (/\b(you (?:bastard|traitor|coward|liar|monster|fool)|how (?:dare|could) you)\b/i.test(text))
            score += 0.025;
    }
...
function scoreCharacterAgency(content) {
...
function scoreInformationDensity(content) {
...
const POSITIVE_TAGS = new Set(["joy", "intimacy", "resolve", "humor", "awe"]);
const NEGATIVE_TAGS = new Set(["grief", "dread", "betrayal", "fury"]);
// tension, revelation, melancholy are ambiguous/neutral
function detectPolarityMix(tags) {
    let pos = 0, neg = 0;
...
// ─── Scoring Engine ────────────────────────────────────────────
export function scoreChunkHeuristic(content) {
    const words = content.split(/\s+/);
    ...
    if (hasInternalThought)
        score += 0.06;
    // ── Narrative density: scene craft and subtlety ──
    if (SCENE_CRAFT_MARKERS.test(content))
        score += 0.05;
    const weightedDialogueCount = WEIGHTED_DIALOGUE_PATTERNS.filter((p) => p.test(content)).length;
    if (weightedDialogueCount > 0)
        score += 0.05 * Math.min(1, weightedDialogueCount);
    // ── Lore density ──
    const loreDensityCount = LORE_DENSITY_MARKERS.filter((p) => p.test(content)).length;
    if (loreDensityCount > 0)
        score += 0.05 * Math.min(1, loreDensityCount);
    // ── Intensity markers (reduced — less combat/action bias) ──
    const intensityScore = ((content.match(INTENSITY_MARKERS.punctuation) || []).length) * 0.012 +
 ...
 */
export function detectEmotionalTags(content) {
    const tags = [];
    for (const [tag, patterns] of Object.entries(EMOTIONAL_PATTERNS)) {
        let tagFound = false;
        for (let pi = 0; pi < patterns.length && !tagFound; pi++) {
            ...
            for (const match of content.matchAll(regex)) {
                ...
                if (!isNegatedOrSarcastic(keyword, content)) {
                    ...
                    const intensityMod = getIntensityModifier(keyword, content);
                    tags.push({ tag: tag, confidence: confidence * intensityMod });
                    tagFound = true;
                    break;
                }
            }
