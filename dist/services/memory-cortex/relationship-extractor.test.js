...
describe("extractRelationshipsHeuristic", () => {
    test("keeps extracting directed relationships when many unrelated entity names are present", () => {
        ...
        const relationships = extractRelationshipsHeuristic("Alice kissed Bob before the crowd could react.", ["Alice", "Bob", ...fillerNames]);
    ...
    test("deduplicates repeated entity names before generating relationships", () => {
        const relationships = extractRelationshipsHeuristic("Alice and Bob fought side by side until dawn.", ["Alice", "Bob", "alice", "Bob"]).filter((rel) => rel.source === "Alice" && rel.target === "Bob" && rel.type === "ally");
        expect(relationships).toHaveLength(1);
    });
    test("falls back to emotional context when explicit pair scans are skipped", () => {
        const spacer = " ".repeat(320);
        const relationships = extractRelationshipsHeuristic(`Alice${spacer}Bob`, ["Alice", "Bob"], ["joy"]);
