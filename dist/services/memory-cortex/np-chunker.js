 ...
 */
function isInflectedCommonWord(lower) {
    if (lower.endsWith("ing") && lower.length >= 7) {
        const stem = lower.slice(0, -3);
        if (COMMON_ENGLISH.has(stem))
            return true; // walk → known
        if (COMMON_ENGLISH.has(stem + "e"))
            return true; // danc → dance
        if (COMMON_ENGLISH.has(stem + "ed"))
            return true; // scream → screamed
        if (COMMON_ENGLISH.has(stem + "s"))
            return true; // scream → screams
        // Doubled consonant: running → runn → run
        if (stem.length >= 3 && stem[stem.length - 1] === stem[stem.length - 2]) {
            const short = stem.slice(0, -1);
            if (COMMON_ENGLISH.has(short))
                return true;
            if (COMMON_ENGLISH.has(short + "ed"))
                return true;
        }
        return false;
    }
    if (lower.endsWith("ed") && lower.length >= 7) {
        ...
        if (COMMON_ENGLISH.has(stem))
            return true; // climb → known
        if (COMMON_ENGLISH.has(stem + "e"))
            return true; // creat → create
        if (COMMON_ENGLISH.has(stem + "ing"))
            return true; // climb → climbing
        if (COMMON_ENGLISH.has(stem + "s"))
            return true; // climb → climbs
        // Doubled consonant: stopped → stopp → stop
        if (stem.length >= 3 && stem[stem.length - 1] === stem[stem.length - 2]) {
            const short = stem.slice(0, -1);
            if (COMMON_ENGLISH.has(short))
                return true;
            if (COMMON_ENGLISH.has(short + "ing"))
                return true;
        }
        // -ied → -y: carried → carri → carry
        if (lower.endsWith("ied")) {
            if (COMMON_ENGLISH.has(lower.slice(0, -3) + "y"))
                return true;
        }
        return false;
    }
 ...
 */
export function extractNPCandidates(tokens) {
...
function peekNextCleaned(tokens, i) {
 ...
 */
function isValidNPCandidate(candidate, lowercaseWords) {
    // ── Structural checks ──
    if (/[\[\]{}|<>#@\\~`]/.test(candidate))
        return false;
    if (/^[-—–\s_.]+$/.test(candidate))
        return false;
    if (/^[A-Z]\d/.test(candidate) && candidate.length <= 3)
        return false;
    ...
    if (words.length >= 2) {
        ...
        if (lowerWords.every((w) => COMMON_ENGLISH.has(w) || lowercaseWords.has(w)))
            return false;
        ...
        if (words.length === 2 && TITLE_PREFIXES.has(lowerWords[0])
            && (COMMON_ENGLISH.has(lowerWords[1]) || lowercaseWords.has(lowerWords[1])))
            return false;
    }
    ...
    if (words.length === 1) {
        ...
        if (candidate.length <= 4 && !/[aeiouAEIOU]/.test(candidate))
            return false;
        ...
        if (candidate.length >= 6 && NOT_PROPER_NOUN_SUFFIX.test(lower))
            return false;
        ...
        if (isInflectedCommonWord(lower))
            return false;
    }
    return true;
}
 ...
 */
export function detectSubjectPosition(tokens, npCandidates) {
    if (npCandidates.length === 0)
        return null;
 ...
 */
export function tokenizeForNPChunker(content) {
    // Strip HTML tags
 ...
 */
export function extractNPsFromChunk(content) {
