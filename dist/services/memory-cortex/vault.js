...
function rowToVault(row) {
    return {
        ...
        sourceChatName: row.source_chat_name ?? null,
        name: row.name,
...
function rowToVaultRelation(row) {
    return {
        id: row.id,
        vaultId: row.vault_id,
...
function getChatLinksByIds(linkIds) {
    if (linkIds.length === 0)
        return [];
    const placeholders = linkIds.map(() => "?").join(", ");
    const rows = getDb().query(`SELECT
       l.*,
     ...
     WHERE l.id IN (${placeholders})`).all(...linkIds);
    const linkById = new Map(rows.map((row) => [row.id, rowToChatLink(row)]));
    return linkIds.map((id) => linkById.get(id)).filter((link) => Boolean(link));
}
// ─── Vault CRUD ───────────────────────────────────────────────
/**
 ...
 * time.
 */
export function createVault(userId, chatId, name, description) {
    const db = getDb();
 ...
 */
function snapshotVaultContents(userId, chatId, vaultId, name, description, now, options) {
    const db = getDb();
    return db.transaction(() => {
        if (name !== null && !options?.replaceExisting) {
            db.query(`INSERT INTO cortex_vaults (id, user_id, source_chat_id, name, description, created_at)
         VALUES (?, ?, ?, ?, ?, ?)`).run(vaultId, userId, chatId, name, description ?? "", now);
        }
        ...
        const entityRows = db.query(`SELECT * FROM memory_entities WHERE chat_id = ? AND status != 'inactive'`).all(chatId);
        const entityNameMap = new Map();
        for (const e of entityRows) {
            entityNameMap.set(e.id, e.name);
        }
        const insertEntity = db.query(`INSERT INTO cortex_vault_entities
         (id, vault_id, name, entity_type, aliases, description, status, facts, emotional_valence, salience_avg)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        for (const e of entityRows) {
            insertEntity.run(crypto.randomUUID(), vaultId, e.name, e.entity_type, e.aliases, e.description, e.status, e.facts, e.emotional_valence, e.salience_avg);
        }
        // Copy active relations (only those with both endpoints in our entity set)
        const relationRows = db.query(`SELECT * FROM memory_relations
       WHERE chat_id = ? AND status = 'active'
         AND superseded_by IS NULL AND merged_into IS NULL
         AND contradiction_flag != 'suspect'`).all(chatId);
        const insertRelation = db.query(`INSERT INTO cortex_vault_relations
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        ...
        for (const r of relationRows) {
            ...
            if (!sourceName || !targetName)
                continue;
            insertRelation.run(crypto.randomUUID(), vaultId, sourceName, targetName, r.relation_type, r.relation_label, r.strength, r.sentiment, r.status);
            relationCount++;
        }
        ...
        const chunkRows = db.query(`SELECT cc.id, cc.content, cc.entity_ids, cc.created_at,
       ...
       ORDER BY cc.created_at ASC`).all(chatId);
        const insertChunk = db.query(`INSERT INTO cortex_vault_chunks
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        ...
        for (const cc of chunkRows) {
            ...
            if (cc.entity_ids) {
                try {
                    const ids = JSON.parse(cc.entity_ids);
                    entityNames = ids
            ...
            insertChunk.run(vaultChunkId, vaultId, cc.id, cc.content, cc.salience_score, cc.emotional_tags ?? "[]", JSON.stringify(entityNames), cc.created_at, now);
        }
        // Update counts
        db.query(`UPDATE cortex_vaults SET entity_count = ?, relation_count = ?, chunk_count = ? WHERE id = ?`).run(entityRows.length, relationCount, chunkIdMap.size, vaultId);
 ...
 */
export function listVaults(userId) {
    const rows = getDb().query(`SELECT v.*, c.name AS source_chat_name
     FROM cortex_vaults v
     LEFT JOIN chats c ON c.id = v.source_chat_id
     WHERE v.user_id = ?
     ORDER BY v.created_at DESC`).all(userId);
 ...
 */
export function getVault(userId, vaultId) {
    const db = getDb();
    const vaultRow = db.query(`SELECT v.*, c.name AS source_chat_name
     FROM cortex_vaults v
     LEFT JOIN chats c ON c.id = v.source_chat_id
     WHERE v.id = ? AND v.user_id = ?`).get(vaultId, userId);
    if (!vaultRow)
        return null;
    const entityRows = db.query(`SELECT * FROM cortex_vault_entities WHERE vault_id = ? ORDER BY salience_avg DESC`).all(vaultId);
    const relationRows = db.query(`SELECT * FROM cortex_vault_relations WHERE vault_id = ? ORDER BY strength DESC`).all(vaultId);
    return {
 ...
 */
export function deleteVault(userId, vaultId) {
    const db = getDb();
    const vault = db.query(`SELECT id FROM cortex_vaults WHERE id = ? AND user_id = ?`).get(vaultId, userId);
    if (!vault)
        return false;
 ...
 */
export function getVaultChunks(vaultId) {
    const rows = getDb().query(`SELECT * FROM cortex_vault_chunks WHERE vault_id = ? ORDER BY salience_score DESC`).all(vaultId);
 ...
 * trigger a rebuild.
 */
export function getVaultRow(userId, vaultId) {
    const row = getDb().query(`SELECT v.*, c.name AS source_chat_name
     FROM cortex_vaults v
     LEFT JOIN chats c ON c.id = v.source_chat_id
     WHERE v.id = ? AND v.user_id = ?`).get(vaultId, userId);
 ...
 * auto-retrying.
 */
export async function reindexVault(userId, vaultId) {
    const db = getDb();
    const vaultRow = db.query(`SELECT * FROM cortex_vaults WHERE id = ? AND user_id = ?`).get(vaultId, userId);
    if (!vaultRow)
    ...
    const hasVectorizedSource = sourceChatExists && sourceChatId
        ? (db.query(`SELECT 1 FROM chat_chunks WHERE chat_id = ? AND vectorized_at IS NOT NULL LIMIT 1`).get(sourceChatId) != null)
        : false;
    ...
    if (sourceChatExists && hasVectorizedSource && sourceChatId) {
        ...
        const snapshot = snapshotVaultContents(userId, sourceChatId, vaultId, null, undefined, Math.floor(Date.now() / 1000), { replaceExisting: true });
        if (snapshot.chunkIdMap.size > 0) {
    ...
    if (existingChunks.length > 0) {
        await embeddingsSvc.deleteVaultChunks(userId, vaultId);
        const { embedded } = await embeddingsSvc.rebuildVaultEmbeddings(userId, vaultId, existingChunks.map((c) => ({ vaultChunkId: c.id, content: c.content })));
 ...
 * Rename a vault.
 */
export function renameVault(userId, vaultId, name) {
    const result = getDb().query(`UPDATE cortex_vaults SET name = ? WHERE id = ? AND user_id = ?`).run(name, vaultId, userId);
 ...
 */
export function attachLink(userId, chatId, linkType, opts) {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    ...
    if (linkType === "vault") {
        if (!opts.vaultId)
            throw new ChatLinkError("vaultId is required for vault links");
        // Scope by user_id so a user can't attach someone else's vault by guessing UUIDs.
        const vault = db.query(`SELECT id FROM cortex_vaults WHERE id = ? AND user_id = ?`).get(opts.vaultId, userId);
        if (!vault)
            throw new ChatLinkError("Vault not found", 404);
        // Check for duplicate
        const existing = db.query(`SELECT id FROM cortex_chat_links WHERE chat_id = ? AND link_type = 'vault' AND vault_id = ?`).get(chatId, opts.vaultId);
        if (existing)
            throw new ChatLinkError("This vault is already linked to this chat", 409);
    }
    else {
        if (!opts.targetChatId)
            throw new ChatLinkError("targetChatId is required for interlinks");
        if (opts.targetChatId === chatId)
            throw new ChatLinkError("Cannot interlink a chat with itself");
        // Target chat must also belong to the caller; cross-user interlinks are not supported.
        const chat = db.query(`SELECT id FROM chats WHERE id = ? AND user_id = ?`).get(opts.targetChatId, userId);
        if (!chat)
            throw new ChatLinkError("Target chat not found", 404);
        // Check for duplicate
        const existing = db.query(`SELECT id FROM cortex_chat_links WHERE chat_id = ? AND link_type = 'interlink' AND target_chat_id = ?`).get(chatId, opts.targetChatId);
        if (existing)
            throw new ChatLinkError("This chat is already interlinked with the target", 409);
    }
    db.transaction(() => {
        // Primary link
        const id = crypto.randomUUID();
        db.query(`INSERT INTO cortex_chat_links (id, user_id, chat_id, link_type, vault_id, target_chat_id, label, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, userId, chatId, linkType, opts.vaultId ?? null, opts.targetChatId ?? null, opts.label ?? "", now);
        ...
        if (linkType === "interlink" && opts.bidirectional && opts.targetChatId) {
            const reverseExisting = db.query(`SELECT id FROM cortex_chat_links WHERE chat_id = ? AND link_type = 'interlink' AND target_chat_id = ?`).get(opts.targetChatId, chatId);
            if (!reverseExisting) {
                const reverseId = crypto.randomUUID();
                db.query(`INSERT INTO cortex_chat_links (id, user_id, chat_id, link_type, vault_id, target_chat_id, label, created_at)
           VALUES (?, ?, ?, ?, NULL, ?, ?, ?)`).run(reverseId, userId, opts.targetChatId, "interlink", chatId, opts.label ?? "", now);
 ...
 */
export function getChatLinks(chatId) {
    const rows = getDb().query(`SELECT
       l.*,
       v.name AS vault_name,
       v.entity_count AS vault_entity_count,
     ...
     LEFT JOIN chats tc ON tc.id = l.target_chat_id
     WHERE l.chat_id = ?
     ORDER BY l.priority ASC, l.created_at ASC`).all(chatId);
    return rows.map(rowToChatLink);
}
/**
 * Remove a link. Verifies ownership.
 */
export function removeLink(userId, chatId, linkId) {
    const result = getDb().query(`DELETE FROM cortex_chat_links WHERE id = ? AND chat_id = ? AND user_id = ?`).run(linkId, chatId, userId);
    return result.changes > 0;
}
/**
 * Toggle a link's enabled state.
 */
export function toggleLink(userId, chatId, linkId, enabled) {
    const result = getDb().query(`UPDATE cortex_chat_links SET enabled = ? WHERE id = ? AND chat_id = ? AND user_id = ?`).run(enabled ? 1 : 0, linkId, chatId, userId);
    return result.changes > 0;
}
// ─── Assembly Helpers ─────────────────────────────────────────
/**
 ...
 */
function vaultEntitiesToSnapshots(entities) {
    return entities.map((e) => ({
        ...
        lastSeenAt: null,
        mentionCount: 0,
        topFacts: e.facts.map((f) => f.replace(/^\[i:\d+\]\s*/, "").replace(/^\[branch:[^\]]+\]\s*/, "")),
 ...
 */
function vaultRelationsToEdges(relations) {
 ...
 */
export function getVaultDataForAssembly(userId, vaultId) {
 ...
 */
export function getLinkedCortexData(userId, chatId, visitedChatIds) {
    ...
    for (const link of links) {
        if (link.linkType === "vault" && link.vaultId) {
            ...
            if (visited.has(link.targetChatId))
                continue;
