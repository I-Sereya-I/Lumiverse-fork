...
function inflateKnownEntities(input) {
...
export function runHeuristicAnalysis(input) {
    ...
    const rawEntities = extractEntitiesHeuristic(input.cleanContent, knownEntities, input.characterNames, input.entityWhitelist, input.minConfidence, input.entityExtractionFilters);
    ...
    const rawRelationships = extractRelationshipsHeuristic(input.cleanContent, entityNamesInChunk, input.emotionalTags ?? salienceResult.emotionalTags);
    const relationshipMs = performance.now() - relationshipStart;
    const aliasStart = performance.now();
    const rawAliases = detectNicknameIntroductions(input.cleanContent, knownEntities, input.characterNames);
