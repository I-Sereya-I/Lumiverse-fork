...
export function getConsolidations(chatId, tier) {
    ...
    const rows = tier != null
        ? db.query(query).all(chatId, tier)
        : db.query(query).all(chatId);
    return rows.map(rowToConsolidation);
}
/** Get the most recent arc summary for a chat */
export function getLatestArc(chatId) {
    const row = getDb()
...
export function deleteConsolidationsForChat(chatId) {
    getDb().query("DELETE FROM memory_consolidations WHERE chat_id = ?").run(chatId);
}
export function collectMemorySummarizationTargets(pair, fallbackConnectionId, sidecar) {
...
export function decideMemorySummarizationFallback(status, fallback) {
    if (status === "ok")
        return { persist: true, useExtractive: false };
    if (status === "aborted")
        return { persist: false, useExtractive: false };
    if (fallback === "skip")
        return { persist: false, useExtractive: false };
...
export async function runMemorySummarizationSidecar(options) {
    const { memorySummarization, sidecarReliability, sidecar, sidecarConnectionId, signal, extract, } = options;
    ...
    for (const target of targets) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            if (attempt > 0) {
                ...
                console.info(`[memory-cortex] Consolidation ${target.role} retry attempt ${attempt + 1}/${maxAttempts} after ${delay}ms`);
            }
            ...
            try {
                ...
                if (name !== "AbortError" && !timedOut) {
                    console.warn(`[memory-cortex] Consolidation ${target.role} attempt ${attempt + 1}/${maxAttempts} failed:`, err instanceof Error ? err.message : err);
                }
            }
            finally {
                if (timer)
...
function resolveConsolidationSidecarOptions(userId, sidecarConnectionId, sidecarTimeoutMs, sidecarOptions) {
    if (sidecarOptions) {
        return {
 ...
 */
export async function maybeConsolidate(userId, chatId, config, generateRawFn, sidecarConnectionId, sidecarTimeoutMs, 
...
extraScaffoldTags, sidecarOptions) {
    if (!config.enabled)
        return false;
    const db = getDb();
    ...
    const countRow = db
        .query(`SELECT COUNT(*) as count FROM chat_chunks
       WHERE chat_id = ? AND consolidation_id IS NULL`)
        .get(chatId);
    if (!countRow || countRow.count < config.chunkThreshold)
        return false;
    // Only fetch the batch we actually need
    const batch = db
        .query(`SELECT cc.*, ms.score as salience_score, ms.emotional_tags as salience_emotional_tags
       FROM chat_chunks cc
       LEFT JOIN memory_salience ms ON ms.chunk_id = cc.id
       WHERE cc.chat_id = ? AND cc.consolidation_id IS NULL
       ORDER BY cc.created_at ASC
       LIMIT ?`)
        .all(chatId, config.chunksPerConsolidation);
    if (batch.length === 0)
        return false;
    let summary;
    let title = null;
    const resolvedSidecar = resolveConsolidationSidecarOptions(userId, sidecarConnectionId, sidecarTimeoutMs, sidecarOptions);
    if (resolvedSidecar.signal?.aborted)
        return false;
    if (config.useSidecar && generateRawFn) {
        const decision = await generateConsolidationSummary(batch, generateRawFn, sidecarConnectionId ?? resolvedSidecar.sidecar?.connectionProfileId ?? "", config.maxTokensPerSummary, samplingParameters, extraScaffoldTags, resolvedSidecar);
        if (!decision.persist) {
    ...
    db.query(`INSERT INTO memory_consolidations
     ...
     VALUES (?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(consolidationId, chatId, title, summary, JSON.stringify(batch.map((c) => c.id)), JSON.stringify([...entityIdSet]), batch[0].created_at, batch[batch.length - 1].created_at, batch[0].created_at, batch[batch.length - 1].created_at, salienceCount > 0 ? salienceSum / salienceCount : 0, JSON.stringify([...emotionalTagSet]), estimateTokens(summary), now, now);
    ...
    console.info(`[memory-cortex] Consolidated ${batch.length} chunks into ${consolidationId} for chat ${chatId}`);
    // Check for arc-level consolidation
    await maybeConsolidateArcs(userId, chatId, config, generateRawFn, sidecarConnectionId, sidecarTimeoutMs, samplingParameters, extraScaffoldTags, resolvedSidecar);
    return true;
}
/**
 ...
 */
async function maybeConsolidateArcs(userId, chatId, config, generateRawFn, sidecarConnectionId, sidecarTimeoutMs, samplingParameters, extraScaffoldTags, sidecarOptions) {
    const db = getDb();
    ...
    const countRow = db
       ...
       WHERE chat_id = ? AND tier = 1
        ...
        .get(chatId, chatId);
    if (!countRow || countRow.count < config.arcThreshold)
        return;
    const batch = db
       ...
       LIMIT ?`)
        .all(chatId, chatId, config.arcThreshold);
    const summaries = batch.map((c) => c.summary);
    let arcSummary;
    let arcTitle = null;
    if (sidecarOptions?.signal?.aborted)
        return;
    if (config.useSidecar && generateRawFn) {
        const combined = summaries.join("\n\n---\n\n");
        const decision = await generateArcSummary(combined, generateRawFn, sidecarConnectionId ?? sidecarOptions?.sidecar?.connectionProfileId ?? "", config.maxTokensPerSummary, samplingParameters, sidecarOptions);
        if (!decision.persist) {
            if (decision.status === "aborted") {
    ...
    for (const c of batch) {
        for (const id of safeJsonArray(c.entity_ids))
            entityIdSet.add(id);
        for (const tag of safeJsonArray(c.emotional_tags))
            emotionalTagSet.add(tag);
    ...
    db.query(`INSERT INTO memory_consolidations
      (id, chat_id, tier, title, summary, source_consolidation_ids, entity_ids,
       message_range_start, message_range_end, time_range_start, time_range_end,
       salience_avg, emotional_tags, token_count, created_at, updated_at)
     VALUES (?, ?, 2, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(arcId, chatId, arcTitle, arcSummary, JSON.stringify(batch.map((c) => c.id)), JSON.stringify([...entityIdSet]), batch[0].message_range_start, batch[batch.length - 1].message_range_end, batch[0].time_range_start, batch[batch.length - 1].time_range_end, salienceSum / batch.length, JSON.stringify([...emotionalTagSet]), estimateTokens(arcSummary), now, now);
    console.info(`[memory-cortex] Created arc consolidation ${arcId} from ${batch.length} tier-1 consolidations`);
}
 ...
 */
function extractiveConsolidation(chunks, extraScaffoldTags) {
    const sentences = [];
    ...
    for (const sent of ranked) {
        ...
        if (count >= 3)
            continue; // Max 3 sentences per source chunk
        selected.push(sent);
...
export async function generateConsolidationSummary(chunks, generateRawFn, connectionId, maxTokens, samplingParameters, extraScaffoldTags, sidecarOptions) {
    return runMemorySummarizationSidecar({
        ...
        extract: async (target) => {
            const content = chunks
                .map((c) => stripNonProseTags(c.content || "", { extraScaffoldTags }))
                .join("\n\n---\n\n");
...
async function generateArcSummary(combinedSummaries, generateRawFn, connectionId, maxTokens, samplingParameters, sidecarOptions) {
    return runMemorySummarizationSidecar({
        memorySummarization: sidecarOptions?.memorySummarization,
        sidecarReliability: sidecarOptions?.sidecarReliability,
        sidecarTimeoutMs: sidecarOptions?.sidecarTimeoutMs,
...
function cleanSummaryResponse(content) {
...
export function parseGeneratedSummary(content) {
...
export function parseGeneratedSummaryResponse(response, kind) {
...
export function getSummaryQualityIssue(summary, source, maxWords) {
    ...
    if (summaryWords.length < 8)
        return "it is too short to preserve useful continuity";
    if (summaryWords.length > maxWords)
        return `it exceeds the ${maxWords}-word limit`;
    ...
    const normalize = (words) => words.map((word) => word.toLowerCase());
    ...
    for (let index = 0; index + 7 <= normalizedSummary.length; index++) {
        phraseCount++;
        if (sourcePhrases.has(normalizedSummary.slice(index, index + 7).join(" ")))
            copiedPhrases++;
    }
 ...
 */
export async function consolidateBacklog(userId, chatId, config, generateRawFn, sidecarConnectionId, sidecarTimeoutMs, samplingParameters, extraScaffoldTags) {
    let created = 0;
    while (await maybeConsolidate(userId, chatId, config, generateRawFn, sidecarConnectionId, sidecarTimeoutMs, samplingParameters, extraScaffoldTags)) {
        created++;
    }
    return created;
}
