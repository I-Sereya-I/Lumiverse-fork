...
import { collectMemorySummarizationTargets, decideMemorySummarizationFallback, generateConsolidationSummary, maybeConsolidate, runMemorySummarizationSidecar, } from "./consolidation";
import { DEFAULT_CONSOLIDATION_CONFIG, DEFAULT_CORTEX_CONFIG, } from "./config";
function sidecarOpts(overrides = {}) {
    return {
        memorySummarization: {
...
function initConsolidationTestDb() {
    closeDatabase();
    initDatabase(":memory:");
    const db = getDb();
...
function seedChunks(chatId, count) {
    const db = getDb();
    for (let i = 0; i < count; i++) {
        db.query(`INSERT INTO chat_chunks (id, chat_id, content, created_at, consolidation_id, entity_ids, emotional_tags)
       VALUES (?, ?, ?, ?, NULL, '[]', '[]')`).run(`chunk-${i}`, chatId, `Elena opened the iron gate and walked into the courtyard ${i}. The lanterns were already lit along the wall.`, 1_000 + i);
    }
}
function listConsolidations() {
    return getDb()
...
describe("generateConsolidationSummary secondary fallback", () => {
    test("uses primary when the first generate succeeds", async () => {
        const calls = [];
        const decision = await generateConsolidationSummary([{ content: "Alice crossed the river and told Bob the news." }], async (opts) => {
        ...
        }, "primary-conn", 200, undefined, undefined, sidecarOpts());
    ...
    test("fails over to secondary after primary retries exhaust", async () => {
        ...
        const decision = await generateConsolidationSummary([{ content: "Alice crossed the river and told Bob the news." }], async (opts) => {
            calls.push(`${opts.connectionId}:${opts.parameters.model ?? ""}`);
            if (opts.connectionId === "primary-conn")
    ...
    test("falls back to extractive after primary and secondary fail", async () => {
        const decision = await generateConsolidationSummary([{ content: "Alice crossed the river and told Bob the news." }], async () => {
            throw new Error("503 upstream");
        }, "primary-conn", 200, undefined, undefined, sidecarOpts({
            sidecarReliability: {
                ...DEFAULT_CORTEX_CONFIG.sidecarReliability,
    ...
    test("caller cancellation during primary stops the chain without extractive fallback", async () => {
        ...
        }, "primary-conn", 200, undefined, undefined, sidecarOpts({ signal: controller.signal }));
...
describe("maybeConsolidate secondary fallback writes", () => {
    ...
    test("writes the secondary summary after primary generate fails", async () => {
        const calls = [];
        await maybeConsolidate("user-1", "chat-1", consolidationConfig(), async (opts) => {
            calls.push(opts.connectionId);
            if (opts.connectionId === "primary-conn")
                throw new Error("503 upstream");
            return { content: JSON.stringify({ title: "Courtyard Arrival", summary: "Elena entered the lantern-lit courtyard." }) };
        }, "primary-conn", 5_000, undefined, undefined, sidecarOpts({
            sidecarReliability: {
                ...DEFAULT_CORTEX_CONFIG.sidecarReliability,
                fallback: "heuristic",
                maxRetries: 0,
    ...
    test("does not write when fallback is skip", async () => {
        await maybeConsolidate("user-1", "chat-1", consolidationConfig(), async () => {
            throw new Error("503 upstream");
        }, "primary-conn", 5_000, undefined, undefined, sidecarOpts({
            sidecarReliability: {
                ...DEFAULT_CORTEX_CONFIG.sidecarReliability,
    ...
    test("pre-aborted signal skips generate and does not write", async () => {
        ...
        await maybeConsolidate("user-1", "chat-1", consolidationConfig(), async () => {
            called = true;
            throw new Error("should not run");
        }, "primary-conn", 5_000, undefined, undefined, sidecarOpts({ signal: controller.signal }));
