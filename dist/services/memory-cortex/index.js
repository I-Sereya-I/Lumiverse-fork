...
import { getCortexConfig, isCortexEnabledForChat, listCortexFallbackEndpoints, listCortexSidecarEndpoints, shouldUseCortexSidecar, shouldUseCortexSidecarForChunkAnalysis, } from "./config";
...
import { processChunkFontColors, deleteColorMapForChat, recordColorAttribution, stripFontTags, stripThoughtDelimiters } from "./font-attribution";
...
export { getCortexConfig, isCortexEnabledForChat, putCortexConfig, applyCortexPreset, shouldUseCortexSidecar, shouldUseCortexSidecarForChunkAnalysis, listCortexSidecarEndpoints, updateCortexSidecarEndpoints, migrateSidecarIntoEndpointPairs, getCortexSidecarConnectionId, } from "./config";
export { listCortexSidecarProviders, commitSidecarRegistryProvider, revokeSidecarRegistryProvider, publishSidecarProviderRegistryChanged, registerSidecarEndpoint, } from "./sidecar-adapter";
...
function isCortexEnabledForStoredChat(userId, chatId, config) {
    const row = getDb()
 ...
 */
function getThoughtMarkerHtmlTags(markers) {
...
export { resolveCanonicalId, normalizeEntityName, computeStrength, computeEdgeSalience, computeEdgeDecayRate, computeGraphCentrality, consolidateEdgeTypes, runHeuristicsMigration, getEntitiesNeedingFactExtraction, updateFactExtractionStatus, updateSalienceBreakdown, processProvisionalEntities, getAllRelationsUnfiltered, mergeEntitiesInternal, checkAndAutoMerge, } from "./entity-graph";
export { createVault, listVaults, getVault, getVaultRow, deleteVault, renameVault, attachLink, getChatLinks, removeLink, toggleLink, getVaultDataForAssembly, getLinkedCortexData, reindexVault, getVaultChunks, } from "./vault";
...
const cortexResultCache = new Map();
const inflightCortexQueries = new Map();
 ...
 */
export function getCortexStructuralSignature(config) {
...
export function getCortexRuntimeSignature(config) {
    return JSON.stringify({
        sidecar: {
 ...
 */
export function migrateLegacyChunkSignature(stored) {
    let parsed;
    try {
...
const legacyChunkSignaturesMigrated = new Set();
/**
 ...
 */
export function migrateLegacyChunkSignatures(chatId) {
    if (legacyChunkSignaturesMigrated.has(chatId))
        return 0;
    const db = getDb();
    const rows = db
...
function clearDerivedCortexData(chatId, options = {}) {
    const db = getDb();
    ...
    db.query(options.preserveSalience
        ...
        : "UPDATE chat_chunks SET salience_score = NULL, emotional_tags = NULL, entity_ids = NULL, consolidation_id = NULL, cortex_warmup_signature = NULL, cortex_warmup_completed_at = NULL WHERE chat_id = ?").run(chatId);
}
export function getCortexWarmupCoverage(chatId, warmupSignature) {
    const db = getDb();
  ...
  `).get(chatId, chatId, warmupSignature, chatId, chatId, chatId, chatId);
    const totalChunks = row?.total ?? 0;
    const completedChunks = row?.completed ?? 0;
    const hasDerivedData = !!row?.has_entities ||
        !!row?.has_relations ||
        !!row?.has_salience ||
...
function buildCortexQueryKey(query, config) {
    return JSON.stringify({
        chatId: query.chatId,
        userId: query.userId,
 ...
 */
export function getCachedCortexResult(chatId, requiredExcludedMessageIds) {
...
export function invalidateCortexCache(chatId) {
    cortexResultCache.delete(chatId);
}
/**
 ...
 */
export function primeCortexCache(chatId, result, excludeMessageIds = []) {
    if (result.stats?.timedOut || result.stats?.aborted)
        return;
...
const linkedCortexResultCache = new Map();
...
// Keyed by vaultId.
const autoReindexInFlight = new Set();
...
function scheduleVaultAutoReindex(userId, vaultId) {
    if (autoReindexInFlight.has(vaultId))
        return;
...
export function getCachedLinkedCortexResult(chatId) {
...
export function invalidateLinkedCortexCache(chatId) {
    linkedCortexResultCache.delete(chatId);
}
/** Drop completed warm results without disturbing in-flight cortex work. */
export function clearCortexResultCaches() {
 ...
 */
export function primeLinkedCortexCache(chatId, result) {
    linkedCortexResultCache.set(chatId, { result, queriedAt: Date.now() });
}
// ─── Ingestion Status / Telemetry ──────────────────────────────
const cortexIngestionStatus = new Map();
const cortexIngestionSamples = new Map();
const pendingIngestionStatusBroadcasts = new Map();
let ingestionStatusBroadcastTimer = null;
function getOrCreateIngestionStatus(chatId) {
...
function flushIngestionStatusBroadcasts() {
...
function emitIngestionStatus(userId, status) {
    ...
    if (ingestionStatusBroadcastTimer)
        return;
    ingestionStatusBroadcastTimer = setTimeout(flushIngestionStatusBroadcasts, 0);
}
function updateIngestionStatus(userId, chatId, patch) {
    const next = {
...
function beginIngestionTracking(userId, chatId, chunkId) {
...
function completeIngestionTracking(userId, chatId, chunkId, timings) {
    const current = getOrCreateIngestionStatus(chatId);
...
function failIngestionTracking(userId, chatId, error, sidecarState) {
    const current = getOrCreateIngestionStatus(chatId);
    const pendingJobs = Math.max(0, current.pendingJobs - 1);
    updateIngestionStatus(userId, chatId, {
        status: "error",
...
export function getIngestionStatus(chatId) {
    return cortexIngestionStatus.get(chatId) ?? null;
}
export function getIngestionTelemetry(chatId) {
    const aggregate = cortexIngestionSamples.get(chatId);
...
export function clearIngestionState(chatId) {
...
export function invalidateLinkedCortexCacheForVault(vaultId) {
    const db = getDb();
    const rows = db.query(`SELECT DISTINCT chat_id FROM cortex_chat_links WHERE vault_id = ?`).all(vaultId);
    for (const r of rows)
        linkedCortexResultCache.delete(r.chat_id);
}
/**
 ...
 */
export async function queryLinkedCortex(chatId, userId, config, queryText, signal) {
        ...
        for (const vault of vaults) {
            ...
            promises.push((async () => {
                try {
                    ...
                    if (row && row.chunkCount < 0)
                        return; // sentinel — skip retrieval
                    ...
                    if (result.entityContext.length > 0)
                        vault.entities = result.entityContext;
                    if (result.activeRelationships.length > 0)
                        vault.relations = result.activeRelationships;
                }
                catch (err) {
                    if (signal?.aborted)
                        return;
    ...
    const result = { vaults, interlinks: interlinkResults };
    linkedCortexResultCache.set(chatId, { result, queriedAt: Date.now() });
    return result;
}
// ─── Retrieval ─────────────────────────────────────────────────
 ...
 */
export async function queryCortex(query, config, signal) {
    const cfg = config ?? getCortexConfig(query.userId);
    if (!cfg.enabled)
        return EMPTY_CORTEX_RESULT;
    if (signal?.aborted)
        return EMPTY_CORTEX_RESULT;
...
const sidecarCircuits = new Map();
function sidecarCircuitKey(userId, connectionProfileId) {
    return `${userId}:${connectionProfileId}`;
}
function getSidecarCircuit(userId, connectionProfileId) {
    const key = sidecarCircuitKey(userId, connectionProfileId);
    const existing = sidecarCircuits.get(key);
    if (existing)
        return existing;
    const created = { consecutiveTransientFailures: 0, openedUntil: null };
    sidecarCircuits.set(key, created);
    return created;
}
export function resetCortexSidecarCircuitForTests() {
    sidecarCircuits.clear();
}
export function recordCortexSidecarCircuitOpenForTests(userId, connectionProfileId) {
...
export function isCortexSidecarCircuitOpen(userId, connectionProfileId, now = Date.now()) {
    const circuit = sidecarCircuits.get(sidecarCircuitKey(userId, connectionProfileId));
    if (!circuit?.openedUntil)
        return false;
...
function recordCortexSidecarSuccess(userId, connectionProfileId) {
    sidecarCircuits.set(sidecarCircuitKey(userId, connectionProfileId), {
        consecutiveTransientFailures: 0,
        openedUntil: null,
    });
}
function recordCortexSidecarTransientFailure(userId, connectionProfileId, now = Date.now()) {
...
export function isTransientCortexSidecarError(err, callerAborted) {
    if (callerAborted)
        return false;
...
async function delayWithCallerSignal(ms, signal) {
...
export function collectQueryGenerationTargets(config, fallbackConnectionId) {
    ...
    for (const extra of listCortexFallbackEndpoints(pair)) {
        if (!extra.connectionProfileId || seen.has(extra.connectionProfileId))
            continue;
...
export function decideCortexSidecarFallback(status, fallback) {
    if (status === "ok")
        return { persist: true, useHeuristic: false };
    if (status === "aborted")
        return { persist: false, useHeuristic: false };
    if (fallback === "skip")
        return { persist: false, useHeuristic: false };
    return { persist: true, useHeuristic: true };
}
export async function runQueryGenerationSidecar(options) {
    ...
    const finish = (status, extra = {}) => {
        ...
        const sidecarState = status === "ok" ? "ok"
    ...
    for (const target of targets) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            if (attempt > 0) {
                ...
                console.info(`[memory-cortex] Sidecar ${target.role} retry attempt ${attempt + 1}/${maxAttempts} after ${delay}ms`);
            }
            ...
            try {
                ...
                if (name !== "AbortError" && !timedOut) {
                    console.warn(`[memory-cortex] Sidecar ${target.role} attempt ${attempt + 1}/${maxAttempts} failed:`, err instanceof Error ? err.message : err);
                }
            }
            finally {
                if (timer)
    ...
    if (signal?.aborted)
        return finish("aborted", { role: lastRole, attempts });
    if (!invokedAny)
        return finish("unavailable", { role: lastRole, attempts });
    if (sawTimeout)
        return finish("timeout", { role: lastRole, attempts });
 ...
 */
export function scheduleProcessChunk(data, characterNames, generateRawFn, sidecarConnectionId, descriptionAliases, signal) {
    const db = getDb();
    const queuedRow = db
    ...
    return enqueueChatPipelineTask({
        ...
        preflight: () => {
            ...
            if (!isCortexEnabledForStoredChat(data.userId, data.chatId, cfg)) {
                return { action: "skip", reason: "cortex_disabled" };
            }
            const row = db
                .query("SELECT updated_at, cortex_warmup_signature FROM chat_chunks WHERE id = ? AND chat_id = ?")
                .get(data.chunkId, data.chatId);
            if (!row)
                return { action: "skip", reason: "chunk_deleted" };
            if (queuedRevision != null && row.updated_at !== queuedRevision) {
                return { action: "skip", reason: "chunk_revised" };
            }
            if (row.cortex_warmup_signature === getCortexStructuralSignature(cfg)) {
                return { action: "skip", reason: "already_warm" };
            }
            return { action: "run" };
        },
        run: () => processChunk(data, characterNames, generateRawFn, sidecarConnectionId, descriptionAliases, undefined, false, signal),
    });
}
export async function processChunk(data, characterNames, generateRawFn, sidecarConnectionId, 
...
skipConsolidation = false, signal) {
    const config = getCortexConfig(data.userId);
    if (!isCortexEnabledForStoredChat(data.userId, data.chatId, config))
        return;
    if (signal?.aborted)
        return;
    ...
    try {
        let salienceResult;
        let sidecarEntities = [];
        let sidecarRelationships = [];
        let sidecarFacts = [];
        let sidecarFontColors = [];
        let sidecarDiscoveredAliases = [];
        let sidecarGrading;
        ...
        const rawChunkContent = await regexScriptsSvc.applyMemoryIngestionRegex(data.userId, hydratedChunkContent, { characterId: data.characterId, chatId: data.chatId });
        ...
        for (const e of knownEntities) {
            ...
            for (const alias of e.aliases)
                entityIdByName.set(alias.toLowerCase(), e.id);
        }
        ...
        for (const n of characterNames)
            allowedColorNames.add(n.toLowerCase());
        if (descriptionAliases) {
            for (const [alias] of descriptionAliases.entries())
                allowedColorNames.add(alias.toLowerCase());
        }
        for (const e of knownEntities) {
        ...
        const isAllowedColorName = (name) => allowedColorNames.has(name.trim().toLowerCase());
        ...
        const fontResult = processChunkFontColors(data.chatId, proseContent, [...new Set([...characterNames, ...knownEntities.map((e) => e.name)])], entityIdByName, thoughtDelimiters);
        timings.fontMs = performance.now() - fontStart;
        const cleanContent = fontResult.strippedContent;
        const shouldRunHeuristicWorker = (config.entityTracking && config.entityExtractionMode !== "off") ||
        ...
        let sidecarDecisionState = null;
        let liveSidecarTokenCounter;
        if (sidecarActive) {
            ...
            try {
                const resolved = await resolveCounter(queryTargets[0]?.model || config.sidecar.model || "");
            ...
            const decision = await runQueryGenerationSidecar({
                ...
                sidecarConnectionId,
                signal,
                extract: async (target) => extractWithSidecar(proseContent, generateRawFn, target.connectionProfileId, {
                    characterNames,
            ...
            if (!decision.persist) {
                ...
                console.warn(`[memory-cortex] Sidecar failed after ${decision.attempts} attempt(s); skipping chunk persistence ` +
        ...
        if (extraction) {
            ...
            if (extraction.fontColors.length > 0) {
                ...
                for (const fc of extraction.fontColors) {
                    if (!isAllowedColorName(fc.characterName))
                        continue;
                    ...
                    if (!entityId)
                        continue;
                    recordColorAttribution(data.chatId, fc.hexColor, entityId, fc.usageType, null);
                }
        ...
        const deferredFactAutopilot = db.transaction(() => {
            let deferredAutopilotEntityId = null;
            if (config.salienceScoring) {
                const dbStart = performance.now();
                db.query(`INSERT INTO memory_salience
           ...
           ON CONFLICT(chunk_id) DO UPDATE SET
             ...
             scored_at = excluded.scored_at`).run(crypto.randomUUID(), data.chunkId, data.chatId, salienceResult.score, salienceResult.source, JSON.stringify(salienceResult.emotionalTags), JSON.stringify(salienceResult.statusChanges), JSON.stringify(salienceResult.narrativeFlags), salienceResult.hasDialogue ? 1 : 0, salienceResult.hasAction ? 1 : 0, salienceResult.hasInternalThought ? 1 : 0, salienceResult.wordCount, now, salienceResult.source === "sidecar" ? "sidecar" : null, now);
                db.query("UPDATE chat_chunks SET salience_score = ?, emotional_tags = ? WHERE id = ?").run(salienceResult.score, JSON.stringify(salienceResult.emotionalTags), data.chunkId);
                timings.dbMs += performance.now() - dbStart;
            }
            if (config.entityTracking && config.entityExtractionMode !== "off") {
                const heuristicEntities = heuristicResult?.entities ?? extractEntitiesHeuristic(cleanContent, knownEntities, characterNames, config.entityWhitelist, config.entityPruning.minConfidence, config.entityExtractionFilters);
                ...
                const arbitratedHeuristicEntities = sidecarGrading
                    ...
                    : refinedEntities;
                const mergedEntities = filterEntitiesByExtractionFilters(mergeExtractedEntities(arbitratedHeuristicEntities, sidecarEntities), cleanContent, config.entityExtractionFilters);
                const heuristicRelationshipsRaw = heuristicResult?.relationships ?? refinedFallback?.relationships ?? extractRelationshipsHeuristic(cleanContent, mergedEntities.map((e) => e.name), salienceResult.emotionalTags);
                ...
                const allRelationships = mergeRelationships(heuristicRelationships, sidecarRelationships).filter((rel) => allowedEntityNames.has(rel.source.toLowerCase()) && allowedEntityNames.has(rel.target.toLowerCase()));
                ...
                const entityIds = entityGraph.ingestChunkEntities(data.chatId, data.chunkId, data.createdAt, mergedEntities, allRelationships, salienceResult.score, salienceResult.emotionalTags, cleanContent, undefined, allDiscoveredAliases);
                ...
                for (const attr of fontResult.attributions) {
                    if (!attr.entityName)
                        continue;
                    if (!isAllowedColorName(attr.entityName))
                        continue;
                ...
                for (const ext of mergedEntities) {
                    ...
                    if (entity && !entity.description) {
                        ...
                        if (excerpt)
                            entityGraph.populateEntityDescription(entity.id, excerpt);
                    }
                }
                if (config.entityPruning.enabled) {
                    const chunkCount = db.query("SELECT COUNT(*) as c FROM chat_chunks WHERE chat_id = ?").get(data.chatId);
                ...
                if (salienceResult.statusChanges.length > 0) {
                    for (const change of salienceResult.statusChanges) {
                        ...
                        if (newStatus)
                            entityGraph.updateEntityStatus(entity.id, newStatus);
                ...
                for (const discovered of allDiscoveredAliases) {
                    ...
                    const evidence = "evidence" in discovered ? discovered.evidence : undefined;
            ...
            if (sidecarGrading
                ...
                for (const rejectedName of sidecarGrading.rejectedExistingEntities) {
                    ...
                    if (!entity)
                        continue;
                    if (entityGraph.deleteEntityIfNotUserEdited(entity.id))
                        deletedCount++;
                }
                if (deletedCount > 0) {
                    console.info(`[memory-cortex] Sidecar graded ${deletedCount} existing entit${deletedCount === 1 ? "y" : "ies"} as invalid; removed from graph.`);
                }
            }
            db.query("UPDATE chat_chunks SET cortex_warmup_signature = ?, cortex_warmup_completed_at = ? WHERE id = ?").run(warmupSignature, now, data.chunkId);
        ...
        if (deferredFactAutopilot && config.factManagement.autopilot
            ...
            await curateEntityFactsWithLLM(deferredFactAutopilot, sidecarFacts, chunkImp, config.factManagement.maxFactsPerEntity, generateRawFn, sidecarConnectionId, config);
        }
        ...
        if (sidecarActive && config.sidecarReliability.arbitratesHeuristics) {
            await evaluatePendingReactivations(data.chatId, proseContent, generateRawFn, sidecarConnectionId, config);
        }
        else {
            // Non-arbiter mode: auto-reactivate any pending relations
            autoReactivatePendingRelations(data.chatId);
        }
        const mode = extraction
    ...
    if (config.consolidation.enabled && !skipConsolidation) {
        ...
        consolidation
            .maybeConsolidate(data.userId, data.chatId, config.consolidation, generateRawFn, sidecarConnectionId, config.sidecarTimeoutMs, buildSidecarSamplingParameters(config.sidecar, { includeMaxTokens: false }), config.nonProseScaffoldTags)
            .catch((err) => {
...
export function getRebuildStatus(chatId) {
...
export async function rebuildCortex(userId, chatId, characterNames, generateRawFn, sidecarConnectionId, 
...
onProgress, descriptionAliases, options = {}) {
    ...
    const characterId = db
        ...
        .get(chatId)?.character_id ?? null;
    console.info(`[memory-cortex] ${resumable ? "Warming" : "Rebuilding"} cortex for chat ${chatId} (sidecar: ${sidecarAvailable ? "yes" : "heuristic only"})`);
    ...
    if (!resumable) {
        ...
        if (coverage.requiresFullRebuild) {
            ...
            chunks = db
                .query("SELECT * FROM chat_chunks WHERE chat_id = ? AND (cortex_warmup_signature IS NULL OR cortex_warmup_signature != ?) ORDER BY created_at ASC")
                .all(chatId, warmupSignature);
        }
    }
    ...
    try {
        ...
        if (!sidecarAnalysisActive) {
            ...
            for (let i = 0; i < chunks.length; i++) {
                if (signal?.aborted)
                    break;
                await processChunkFromRaw(chunks[i], chatId, userId, characterId, characterNames, sidecarAvailable ? generateRawFn : undefined, sidecarAvailable ? sidecarConnectionId : undefined, descriptionAliases, true, signal);
            ...
            async function processNextBatch() {
                while (nextChunkIdx < chunks.length) {
                    ...
                    const batchInput = batch.map((chunk, i) => ({
                        ...
                        content: stripNonProseTags(hydrateChunkContentFromMessages(safeJsonArray(chunk.message_ids), chunk.content), {
                            keepFontTags: true,
                            preserveFontInnerTags: getThoughtMarkerHtmlTags(config.thoughtMarkers),
                            extraScaffoldTags: config.nonProseScaffoldTags,
                        }),
                    ...
                    let perChunkHeuristic = new Array(batch.length).fill(null);
                    let perChunkArbiter;
                    let batchExistingEntities;
                    if (arbiterMode) {
                        ...
                        const heuristicAliases = descriptionAliases
                            ? [...descriptionAliases.entries()].map(([alias, canonicalName]) => ({ alias, canonicalName }))
                            : undefined;
                        perChunkHeuristic = await Promise.all(batch.map((_, i) => {
                    ...
                    let sidecarResults;
                    try {
                        sidecarResults = await extractBatchWithSidecar(batchInput, activeGenerateRawFn, activeSidecarConnectionId, {
                            characterNames,
                            perChunkArbiter,
                            batchExistingEntities,
                            descriptionAliases: buildSidecarAliasList(descriptionAliases, entityGraph.getActiveEntities(chatId)),
                            samplingParameters: buildSidecarSamplingParameters(config.sidecar),
                    ...
                    for (let i = 0; i < batch.length; i++) {
                        ...
                        try {
                            ...
                            if (sidecarResult) {
                                await processChunkWithPrecomputedSidecar(chunk, chatId, userId, characterId, characterNames, sidecarResult, descriptionAliases, perChunkHeuristic[i] ?? undefined, signal);
                            }
                            else {
        ...
        if (config.consolidation.enabled && config.consolidation.useSidecar && sidecarAvailable) {
            ...
            emit();
            const created = await consolidation.consolidateBacklog(userId, chatId, config.consolidation, generateRawFn, sidecarConnectionId, config.sidecarTimeoutMs, buildSidecarSamplingParameters(config.sidecar, { includeMaxTokens: false }), config.nonProseScaffoldTags);
            if (created > 0) {
        ...
        setTimeout(() => { if (activeRebuilds.get(chatId) === state)
            activeRebuilds.delete(chatId); }, 5 * 60 * 1000);
        console.info(`[memory-cortex] ${resumable ? "Warmup" : "Rebuild"} complete: ${totalChunks} chunks, ${entities.length} entities, ${relations.length} relations`);
        return result;
    }
    catch (err) {
        ...
        setTimeout(() => { if (activeRebuilds.get(chatId) === state)
            activeRebuilds.delete(chatId); }, 60 * 1000);
        throw err;
    }
}
/** Helper: convert a raw DB chunk row into processChunk input */
async function processChunkFromRaw(chunk, chatId, userId, characterId, characterNames, generateRawFn, sidecarConnectionId, descriptionAliases, skipConsolidation = false, signal) {
    await processChunk({
        chunkId: chunk.id,
    ...
    }, characterNames, generateRawFn, sidecarConnectionId, descriptionAliases, undefined, skipConsolidation, signal);
}
/**
 ...
 */
async function processChunkWithPrecomputedSidecar(chunk, chatId, userId, characterId, characterNames, sidecarResult, descriptionAliases, 
...
precomputedHeuristic, signal) {
    ...
    await processChunk({
        ...
        endMessageIndex: 0,
        createdAt: chunk.created_at,
    }, characterNames, fakeGenerateRaw, "precomputed", descriptionAliases, precomputedHeuristic, true, signal);
}
 ...
 */
export function cortexToMemoryResult(cortexResult, settings) {
 ...
 */
function buildSidecarSamplingParameters(sidecar, opts = {}) {
    const params = {};
 ...
 */
async function curateEntityFactsWithLLM(entityId, _newFacts, _chunkImportance, maxFacts, generateRawFn, connectionId, config) {
    // Read raw facts WITH importance tags to preserve provenance
    const entity = entityGraph.getEntity(entityId);
    if (!entity || entity.facts.length <= maxFacts)
        return;
...
${scoredFacts.map((f, i) => `${i + 1}. [salience:${f.importance}] ${f.text}`).join("\n")}`;
    try {
        ...
        const db = getDb();
        const now = Math.floor(Date.now() / 1000);
        db.query(`UPDATE memory_entities SET facts = ?, fact_extraction_status = 'ok', updated_at = ? WHERE id = ?`).run(JSON.stringify(tagged), now, entityId);
    }
    catch (err) {
 ...
 */
function autoReactivatePendingRelations(chatId) {
    const db = getDb();
    const rows = db.query(`SELECT id, metadata FROM memory_relations
     WHERE chat_id = ? AND status != 'active'
       AND metadata LIKE '%"pending_reactivation":true%'`).all(chatId);
    for (const row of rows) {
 ...
 */
async function evaluatePendingReactivations(chatId, passageContent, generateRawFn, connectionId, config) {
    const db = getDb();
    const pendingRows = db.query(`SELECT r.id, r.source_entity_id, r.target_entity_id, r.relation_type,
            r.relation_label, r.status
     FROM memory_relations r
     WHERE r.chat_id = ? AND r.status != 'active'
       AND r.metadata LIKE '%"pending_reactivation":true%'
       AND r.superseded_by IS NULL AND r.merged_into IS NULL`).all(chatId);
    ...
    const resolveName = (id) => {
        ...
        const row = db.query("SELECT name FROM memory_entities WHERE id = ?").get(id);
    ...
    try {
        ...
        if (!jsonMatch) {
            ...
            for (const c of candidates)
                entityGraph.dismissReactivation(c.id);
            return;
        }
        ...
        for (const c of candidates)
            entityGraph.reactivateRelation(c.id);
    }
}
/**
 ...
 */
function applyEntityGrading(heuristic, grading) {
    ...
    for (const e of heuristic) {
        if (rejected.has(e.name.toLowerCase()))
            continue;
 ...
 */
function applyRelationshipGrading(heuristic, grading) {
    if (heuristic.length === 0 || grading.rejectedHeuristicRelationships.length === 0)
        return heuristic;
    const rejected = new Set(grading.rejectedHeuristicRelationships.map((r) => `${r.source.toLowerCase()}→${r.target.toLowerCase()}:${r.type.toLowerCase()}`));
    return heuristic.filter((r) => !rejected.has(`${r.source.toLowerCase()}→${r.target.toLowerCase()}:${r.type.toLowerCase()}`));
}
function mergeExtractedEntities(heuristic, sidecar) {
...
function hydrateChunkContentFromMessages(messageIds, fallbackContent) {
 ...
 */
function findPrefixMatch(np, characterNames) {
...
export function extractDescriptionAliases(canonicalName, ...descriptions) {
    ...
    for (const desc of descriptions) {
        ...
        while ((match = aliasFieldPatterns.exec(desc)) !== null) {
            ...
            for (const candidate of candidates)
                addAlias(aliases, candidate, canonicalName);
        }
...
function addAlias(map, alias, canonical) {
    ...
    if (!trimmed || !isPlausibleAlias(trimmed, canonical))
        return;
 ...
 */
export function buildSidecarAliasList(descriptionAliases, knownEntities) {
    const merged = new Map();
    for (const e of knownEntities) {
        for (const alias of e.aliases) {
            const lower = alias.toLowerCase();
            if (!merged.has(lower))
                merged.set(lower, e.name);
        }
    }
    if (descriptionAliases) {
 ...
 */
export function mergeDescriptionAliases(...aliasMaps) {
 ...
 */
function getInflectionalVariants(word) {
    if (word.length < 5)
        return [];
 ...
 */
function mergeRelationships(heuristic, sidecar) {
    const merged = new Map();
