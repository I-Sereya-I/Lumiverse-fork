 ...
 */
export function assembleEntitySnapshots(chatId, entityIds, maxSnapshots, preloadedRelations) {
    ...
    return sorted.map((entity) => {
        ...
        const entityRelations = relations.filter((r) => r.sourceEntityId === entity.id || r.targetEntityId === entity.id);
 ...
 * @param preloadedRelations - Optional pre-loaded relations to avoid duplicate queries.
 */
export function getActiveRelationEdges(chatId, entityIds, maxEdges, preloadedRelations) {
 ...
 */
export function formatEntitySnapshots(snapshots) {
 ...
 */
export function formatRelationships(edges) {
    if (edges.length === 0)
        return "";
 ...
 */
export function resolveActiveEntityIds(chatId, queryText) {
 ...
 */
export function resolveEntityIdsByNames(chatId, names) {
    const ids = [];
    for (const name of names) {
