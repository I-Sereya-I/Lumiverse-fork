...
function rowToEntity(row) {
    return {
        ...
        factExtractionStatus: (row.fact_extraction_status ?? "never"),
        factExtractionLastAttempt: row.fact_extraction_last_attempt ?? null,
        salienceBreakdown: safeJsonObject(row.salience_breakdown),
...
function rowToRelation(row) {
    return {
        ...
        createdAt: row.created_at,
        updatedAt: row.updated_at,
        contradictionFlag: (row.contradiction_flag ?? "none"),
...
const ADAPTIVE_ENTITY_TYPES = new Set(["concept", "faction", "event"]);
const CROSS_CHUNK_PROMOTION_TARGETS = ["faction", "event"];
function clampTypeEvidenceWeight(value) {
    if (!Number.isFinite(value))
        return 0.35;
    return Math.max(0.2, Math.min(1, value));
}
function normalizeTypeEvidenceState(raw) {
    ...
    for (const type of ["character", "location", "item", "faction", "concept", "event"]) {
        ...
        if (Number.isFinite(rawScore) && rawScore > 0)
            scores[type] = rawScore;
        if (Number.isFinite(rawCount) && rawCount > 0)
            counts[type] = Math.floor(rawCount);
    }
    return {
        scores,
        counts,
...
function accumulateTypeEvidence(metadata, extracted, observedAt) {
...
function resolveEntityTypeFromEvidence(currentType, metadata, mentionCount) {
    if (!ADAPTIVE_ENTITY_TYPES.has(currentType))
        return currentType;
...
function resolveEntityConfidence(currentConfidence, extracted, mentionCount, resolvedType, metadata) {
    if (currentConfidence === "confirmed" || !extracted.provisional)
        return "confirmed";
    void mentionCount;
    void resolvedType;
    void metadata;
...
function markResolvedType(metadata, resolvedType, resolvedAt) {
    const next = normalizeEntityMetadata(metadata);
    const evidence = next.typeEvidence ?? normalizeTypeEvidenceState(null);
 ...
 */
export function resolveCanonicalId(nameOrId, chatId) {
    const db = getDb();
    // 1. Direct entity ID match
    const byId = db
    ...
    const allEntities = db
        .query(`SELECT id, name, aliases, entity_type FROM memory_entities
       WHERE chat_id = ? ORDER BY mention_count DESC LIMIT ?`)
        .all(chatId, FUZZY_RESOLUTION_LIMIT);
    if (allEntities.length === FUZZY_RESOLUTION_LIMIT) {
        console.warn(`[memory-cortex] resolveCanonicalId: chat ${chatId} has ≥${FUZZY_RESOLUTION_LIMIT} entities — fuzzy match window saturated; consider running consolidation`);
    }
    ...
    for (const row of allEntities) {
        ...
        const aliases = safeJsonArray(row.aliases);
        if (aliases.some((a) => normalizeEntityName(a) === normalized))
            return row.id;
 ...
 *  23 chunks → 0.97 (97%)
 */
export function computeStrength(evidenceCount) {
 ...
 */
function detectAndResolveContradiction(existingRow, incomingSentiment, incomingEvidenceCount, incomingArcId) {
    const existingSentiment = existingRow.sentiment;
    // No contradiction if sentiments have the same sign (or either is neutral)
    if (Math.sign(existingSentiment) === Math.sign(incomingSentiment) ||
...
export function shouldMergeEdges(edgeA, edgeB) {
    if (edgeA.sourceEntityId !== edgeB.sourceEntityId)
        return false;
    if (edgeA.targetEntityId !== edgeB.targetEntityId)
        return false;
    const hierA = TYPE_HIERARCHY[edgeA.relationType];
    if (!hierA || hierA.alwaysIndependent)
        return false;
    // Subsumption: lover subsumes ally
    if (hierA.subsumes?.includes(edgeB.relationType))
        return true;
    // Mergeable: ally/custom when sentiment is close
    if (!hierA.mergeableWith?.includes(edgeB.relationType))
        return false;
 ...
 */
export function computeEdgeDecayRate(strength, arcWeightModifier = 1.0) {
    const base = 0.05;
 ...
 */
export function computeEdgeSalience(strength, decayRate, lastEvidenceTimestamp, now) {
 ...
 */
export function computeGraphCentrality(entityId, chatId) {
    const db = getDb();
    // Get active edges for this entity (not superseded, not suspect)
    const edges = db
        .query(`SELECT source_entity_id, target_entity_id, strength FROM memory_relations
       WHERE chat_id = ? AND status = 'active'
         AND contradiction_flag NOT IN ('suspect')
         AND superseded_by IS NULL AND merged_into IS NULL
         AND (source_entity_id = ? OR target_entity_id = ?)`)
        .all(chatId, entityId, entityId);
    if (edges.length === 0)
        return 0;
    let weightedSum = 0;
...
/** Get entities by IDs */
export function getEntitiesByIds(entityIds) {
...
export function upsertEntity(chatId, extracted, chunkId, chunkTimestamp) {
    const db = getDb();
    ...
    if (existing) {
        ...
        if (existing.userEditedAt !== null) {
            db.query(`UPDATE memory_entities SET
          last_seen_chunk_id = ?,
          last_seen_at = ?,
          mention_count = mention_count + 1,
          updated_at = ?
         WHERE id = ?`).run(chunkId, chunkTimestamp, now, existing.id);
            return existing.id;
        }
        ...
        const nextMetadata = accumulateTypeEvidence(existing.metadata, extracted, chunkTimestamp);
        ...
        metadata = ?,
        updated_at = ?
       WHERE id = ?`).run(chunkId, chunkTimestamp, JSON.stringify(newAliases), resolvedType, resolvedConfidence, JSON.stringify(persistedMetadata), now, existing.id);
        return existing.id;
    }
    // Create new entity
    ...
    const initialAliases = extracted.aliases
        ...
        .filter((alias) => !!alias && isPlausibleAlias(alias, extracted.name));
    ...
    const confidence = resolveEntityConfidence(extracted.provisional ? "provisional" : "confirmed", extracted, 1, resolvedType, initialMetadata);
    const persistedMetadata = markResolvedType(initialMetadata, resolvedType, chunkTimestamp);
    db.query(`INSERT INTO memory_entities
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, 0.0, ?, ?)`).run(id, chatId, extracted.name, resolvedType, JSON.stringify(initialAliases), chunkId, chunkId, chunkTimestamp, chunkTimestamp, chunkTimestamp, JSON.stringify(persistedMetadata), confidence, now, now);
    return id;
}
/**
 ...
 */
export function persistLearnedAlias(entityId, alias, chatId) {
    const cleaned = sanitizeAlias(alias);
    if (!cleaned)
        return false;
    const db = getDb();
    const row = db.query("SELECT name, aliases, user_edited_at FROM memory_entities WHERE id = ?").get(entityId);
    if (!row)
        return false;
    if (row.user_edited_at !== null)
        return false;
    if (!isPlausibleAlias(cleaned, row.name))
        return false;
    ...
    if (lowerSet.has(cleaned.toLowerCase()))
        return false;
    if (cleaned.toLowerCase() === row.name.toLowerCase())
        return false;
    existing.push(cleaned);
    const now = Math.floor(Date.now() / 1000);
    db.query("UPDATE memory_entities SET aliases = ?, updated_at = ? WHERE id = ?").run(JSON.stringify(existing), now, entityId);
    if (chatId) {
        checkAndAutoMerge(chatId, entityId, cleaned);
    }
    return true;
}
const MENTION_ROLE_RANK = {
    absent: 0,
    referenced: 1,
    present: 2,
    object: 3,
...
function strongerMentionRole(a, b) {
    return (MENTION_ROLE_RANK[b] ?? 0) > (MENTION_ROLE_RANK[a] ?? 0) ? b : a;
}
function mergeMentionsIntoEntity(sourceId, targetId) {
    const db = getDb();
    const sourceMentions = db
        .query("SELECT * FROM memory_mentions WHERE entity_id = ?")
        .all(sourceId);
    const getTargetMention = db.query("SELECT * FROM memory_mentions WHERE entity_id = ? AND chunk_id = ?");
    const updateTargetMention = db.query(`UPDATE memory_mentions SET
       role = ?,
       excerpt = ?,
       sentiment = ?,
       created_at = ?
    ...
    for (const sourceMention of sourceMentions) {
        const targetMention = getTargetMention.get(targetId, sourceMention.chunk_id);
        if (!targetMention) {
            moveMention.run(targetId, sourceMention.id);
            continue;
        }
        updateTargetMention.run(strongerMentionRole(targetMention.role, sourceMention.role), targetMention.excerpt ?? sourceMention.excerpt, Math.abs(sourceMention.sentiment) > Math.abs(targetMention.sentiment)
            ? sourceMention.sentiment
            : targetMention.sentiment, Math.min(targetMention.created_at, sourceMention.created_at), targetMention.id);
        deleteMention.run(sourceMention.id);
    }
}
function mergeRelationsIntoEntity(sourceId, targetId) {
    const db = getDb();
    const sourceRelations = db
        .query(`SELECT * FROM memory_relations
       WHERE source_entity_id = ? OR target_entity_id = ?`)
        .all(sourceId, sourceId);
    const getCanonicalRelation = db.query(`SELECT * FROM memory_relations
     WHERE source_entity_id = ? AND target_entity_id = ? AND relation_type = ? AND id != ?`);
    const moveRelation = db.query(`UPDATE memory_relations SET
       source_entity_id = ?,
       target_entity_id = ?
     WHERE id = ?`);
    ...
    for (const relation of sourceRelations) {
        ...
        const canonical = getCanonicalRelation.get(newSourceId, newTargetId, relation.relation_type, relation.id);
        if (canonical) {
 ...
 */
export function mergeEntitiesInternal(sourceId, targetId) {
    ...
    db.transaction(() => {
        ...
        updated_at = ?
       WHERE id = ?`).run(JSON.stringify(targetAliases), JSON.stringify(mergedFacts), source.mentionCount, source.salienceAvg, source.saliencePeak, now, targetId);
 ...
 */
export function checkAndAutoMerge(chatId, entityId, newAlias) {
    const db = getDb();
    const match = db
        .query(`SELECT id, mention_count FROM memory_entities
       WHERE chat_id = ? AND name = ? COLLATE NOCASE AND id != ?`)
        .get(chatId, newAlias, entityId);
    if (!match)
        return null;
    const current = db
    ...
    console.info(`[memory-cortex] Auto-merged entity "${absorbId}" into "${keepId}" via alias "${newAlias}" in chat ${chatId}`);
    return keepId;
}
/** Flip user_edited_at on an entity so rebuilds preserve its curated fields. */
export function markEntityUserEdited(entityId) {
    const now = Math.floor(Date.now() / 1000);
    const result = getDb()
...
export function markRelationUserEdited(relationId) {
    const now = Math.floor(Date.now() / 1000);
    const result = getDb()
        .query("UPDATE memory_relations SET user_edited_at = ?, updated_at = ? WHERE id = ?")
        .run(now, now, relationId);
    return (result.changes ?? 0) > 0;
}
/**
 ...
 */
export function updateEntityStatus(entityId, status, branchId) {
    const now = Math.floor(Date.now() / 1000);
    if (branchId) {
 ...
 */
export function addEntityFacts(entityId, newFacts, branchId, importance, maxFacts = 30) {
    if (newFacts.length === 0)
        return;
    const db = getDb();
    const row = db.query("SELECT facts FROM memory_entities WHERE id = ?").get(entityId);
    if (!row)
        return;
    ...
    for (let fact of newFacts) {
        ...
        if (typeof importance === "number")
            fact = `[i:${Math.round(importance)}] ${fact}`;
        // Add branch provenance tag if provided
        if (branchId)
            fact = `[branch:${branchId}] ${fact}`;
    ...
    db.query(`UPDATE memory_entities SET
      ...
      updated_at = ?
     WHERE id = ?`).run(JSON.stringify(trimmed), newFactsAdded ? 1 : 0, now, entityId);
}
/**
 ...
 */
export function getEntityFacts(entityId, branchId) {
...
export function updateEntityEmotionalValence(entityId, newTags) {
    const db = getDb();
    const row = db.query("SELECT emotional_valence, mention_count FROM memory_entities WHERE id = ?").get(entityId);
    if (!row)
        return;
...
export function updateEntitySalience(entityId, chunkSalience) {
    const db = getDb();
    const row = db.query("SELECT salience_avg, salience_peak FROM memory_entities WHERE id = ?").get(entityId);
    if (!row)
        return;
    ...
    const now = Math.floor(Date.now() / 1000);
    db.query("UPDATE memory_entities SET salience_avg = ?, salience_peak = ?, updated_at = ? WHERE id = ?").run(newAvg, newPeak, now, entityId);
}
/** Update last_mention_timestamp and recent_mention_count for an entity (IMP 2) */
export function updateEntityMentionTimestamp(entityId, timestamp) {
    const now = Math.floor(Date.now() / 1000);
    getDb()
        .query(`UPDATE memory_entities SET
 ...
 */
export function updateFactExtractionStatus(entityId, status) {
    const now = Math.floor(Date.now() / 1000);
    getDb()
        .query(`UPDATE memory_entities SET
 ...
 */
export function getEntitiesNeedingFactExtraction(chatId, salienceThreshold = 0.45, limit = 10) {
    const rows = getDb()
       ...
       LIMIT ?`)
        .all(chatId, salienceThreshold, limit);
    return rows.map(rowToEntity);
}
/**
 ...
 */
export function updateSalienceBreakdown(entityId, chatId) {
    const db = getDb();
    const entity = getEntity(entityId);
    if (!entity)
        return;
    ...
    const mentionComponent = Math.min(1.0, Math.log2(1 + entity.mentionCount) / 7);
    ...
    const total = Math.min(1.0, mentionComponent * 0.35 +
        arcComponent * 0.35 +
        graphComponent * 0.15 +
        frequencyFloor);
    const breakdown = {
    ...
    db.query(`UPDATE memory_entities SET
      ...
      salience_avg = ?,
      updated_at = ?
     WHERE id = ?`).run(JSON.stringify(breakdown), total, now, entityId);
}
/**
 ...
 */
export function mergeEdgePair(canonicalId, absorbedId) {
    const db = getDb();
    const canonical = db.query("SELECT * FROM memory_relations WHERE id = ?").get(canonicalId);
    const absorbed = db.query("SELECT * FROM memory_relations WHERE id = ?").get(absorbedId);
    ...
    db.query(`UPDATE memory_relations SET
      evidence_chunk_ids = ?,
      strength = ?,
      sentiment = ?,
      arc_ids = ?,
      ...
      edge_salience = ?,
      updated_at = ?
     WHERE id = ?`).run(JSON.stringify(mergedEvidence), newStrength, newSentiment, JSON.stringify(mergedArcIds), JSON.stringify(existingLabelAliases), newDecayRate, newEdgeSalience, now, canonicalId);
    // Mark absorbed edge as merged
    db.query(`UPDATE memory_relations SET
      canonical_edge_id = ?,
      merged_into = ?,
      updated_at = ?
     WHERE id = ?`).run(canonicalId, canonicalId, now, absorbedId);
}
/**
 ...
 */
export function consolidateEdgeTypes(chatId) {
    const relations = getAllRelationsUnfiltered(chatId);
    let mergeCount = 0;
    // Group by source→target pair
    const pairMap = new Map();
    for (const rel of relations) {
        if (rel.mergedInto || rel.supersededBy)
            continue; // Skip already merged/superseded
        ...
        for (let i = 0; i < edges.length; i++) {
            for (let j = i + 1; j < edges.length; j++) {
                ...
                if (hierI?.subsumes?.includes(edges[j].relationType)) {
                    ...
                    edges[j] = { ...edges[j], mergedInto: edges[i].id };
                    mergeCount++;
                    continue;
                }
 ...
 */
export function deleteEntityIfNotUserEdited(entityId) {
    const db = getDb();
    const row = db.query("SELECT user_edited_at FROM memory_entities WHERE id = ?").get(entityId);
    if (!row)
        return false;
    if (row.user_edited_at !== null)
        return false;
    db.transaction(() => {
        ...
        db.query("DELETE FROM memory_relations WHERE source_entity_id = ? OR target_entity_id = ?").run(entityId, entityId);
        db.query("DELETE FROM memory_entities WHERE id = ?").run(entityId);
    })();
    return true;
}
export function deleteEntitiesForChat(chatId, opts = {}) {
    const db = getDb();
    ...
    db.transaction(() => {
        ...
        metadata = '{}',
        updated_at = ?
       WHERE chat_id = ? AND user_edited_at IS NOT NULL`).run(Math.floor(Date.now() / 1000), chatId);
        db.query("DELETE FROM memory_entities WHERE chat_id = ? AND user_edited_at IS NULL").run(chatId);
    })();
}
// ─── Mention CRUD ──────────────────────────────────────────────
/** Record an entity mention in a chunk */
export function upsertMention(entityId, chunkId, chatId, role, excerpt, sentiment) {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    ...
    db.query(`INSERT INTO memory_mentions (id, entity_id, chunk_id, chat_id, role, excerpt, sentiment, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(entity_id, chunk_id) DO UPDATE SET
       role = excluded.role,
       excerpt = excluded.excerpt,
       sentiment = excluded.sentiment`).run(crypto.randomUUID(), entityId, chunkId, chatId, role, excerpt, sentiment, now);
}
/** Get all mentions for an entity */
export function getMentionsForEntity(entityId) {
    const rows = getDb()
...
export function getMentionsForChunk(chunkId) {
    const rows = getDb()
...
export function getChunkIdsForEntities(chatId, entityIds) {
    if (entityIds.length === 0)
        return [];
    const placeholders = entityIds.map(() => "?").join(",");
    const rows = getDb()
        .query(`SELECT DISTINCT chunk_id FROM memory_mentions
       WHERE chat_id = ? AND entity_id IN (${placeholders})`)
        .all(chatId, ...entityIds);
    return rows.map((r) => r.chunk_id);
}
/** Delete all mentions for a chat (used in rebuild) */
export function deleteMentionsForChat(chatId) {
 ...
 */
export function upsertRelation(chatId, rel, sourceEntityId, targetEntityId, chunkId, arcId) {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    ...
    const existing = db
       ...
       WHERE source_entity_id = ? AND target_entity_id = ? AND relation_type = ?
        ...
        .get(canonicalSourceId, canonicalTargetId, rel.type);
    if (existing) {
        ...
        edge_salience = ?,
        updated_at = ?
       WHERE id = ?`).run(rel.label || null, newStrength, newSentiment, statusUpdate, JSON.stringify(evidenceIds), now, now, JSON.stringify(existingArcIds), arcId || null, newDecayRate, newEdgeSalience, now, existing.id);
    }
    else {
        ...
        const opposingEdges = db
         ...
         WHERE source_entity_id = ? AND target_entity_id = ?
           AND status = 'active' AND merged_into IS NULL AND superseded_by IS NULL`)
            .all(canonicalSourceId, canonicalTargetId);
        let contradictionFlag = "none";
        let contradictionPeerId = null;
        ...
        for (const opposing of opposingEdges) {
            if (Math.sign(opposing.sentiment) !== Math.sign(rel.sentiment) &&
                ...
                const resolution = detectAndResolveContradiction(opposing, rel.sentiment, 1, // New edge starts with 1 evidence
                arcId ?? null);
              ...
              superseded_by = ?
             WHERE id = ?`).run(crypto.randomUUID(), opposing.id); // Will be updated with the new edge's ID below
              ...
              sentiment_range = ?
             WHERE id = ?`).run(JSON.stringify(sentimentRange), opposing.id);
                }
        ...
        db.query(`INSERT INTO memory_relations
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(newEdgeId, chatId, canonicalSourceId, canonicalTargetId, rel.type, rel.label || null, initialStrength, rel.sentiment, JSON.stringify([chunkId]), now, now, now, JSON.stringify(arcId ? [arcId] : []), arcId || null, arcId || null, initialDecayRate, initialEdgeSalience, contradictionFlag, contradictionPeerId, sentimentRange ? JSON.stringify(sentimentRange) : null, now, now);
        // If we superseded an existing edge, update its superseded_by with the actual new edge ID
        if (opposingEdges.length > 0) {
            for (const opposing of opposingEdges) {
                const oppRow = db.query("SELECT superseded_by FROM memory_relations WHERE id = ?").get(opposing.id);
...
export function getAllRelationsUnfiltered(chatId) {
    const rows = getDb()
...
export function getRelationsForEntities(chatId, entityIds, limit) {
    if (entityIds.length === 0)
        return [];
    const placeholders = entityIds.map(() => "?").join(",");
    ...
    const rows = getDb()
       ...
       ORDER BY edge_salience DESC, strength DESC${limitClause}`)
        .all(chatId, ...entityIds, ...entityIds);
    return rows.map(rowToRelation);
}
/** Get active edges for an entity (used by graph centrality computation) */
export function getActiveEdgesForEntity(chatId, entityId) {
    const rows = getDb()
        .query(`SELECT * FROM memory_relations
       WHERE chat_id = ? AND status = 'active'
         AND superseded_by IS NULL AND merged_into IS NULL
...
export function getPendingReactivationRelations(chatId, entityIds) {
    if (entityIds.length === 0)
        return [];
    const placeholders = entityIds.map(() => "?").join(",");
    const rows = getDb()
...
export function reactivateRelation(relationId) {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    const row = db.query("SELECT metadata FROM memory_relations WHERE id = ?").get(relationId);
    if (!row)
        return;
    const meta = safeJsonObject(row.metadata);
    delete meta.pending_reactivation;
    delete meta.reactivation_evidence_chunk;
    db.query(`UPDATE memory_relations SET status = 'active', metadata = ?, updated_at = ? WHERE id = ?`).run(JSON.stringify(meta), now, relationId);
}
/** Clear the pending_reactivation flag without reactivating (arbiter decided
 *  the relationship should stay dormant). */
export function dismissReactivation(relationId) {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    const row = db.query("SELECT metadata FROM memory_relations WHERE id = ?").get(relationId);
    if (!row)
    ...
    delete meta.pending_reactivation;
    delete meta.reactivation_evidence_chunk;
    db.query(`UPDATE memory_relations SET metadata = ?, updated_at = ? WHERE id = ?`).run(JSON.stringify(meta), now, relationId);
}
 ...
 */
export function deleteRelationsForChat(chatId, opts = {}) {
    const db = getDb();
    if (!opts.preserveUserEdited) {
    ...
    db.transaction(() => {
        ...
        db.query(`UPDATE memory_relations
       ...
       WHERE chat_id = ?
        ...
        updated_at = ?
       WHERE chat_id = ? AND user_edited_at IS NOT NULL AND status != 'broken'`).run(now, chatId);
        // Delete everything else.
        db.query("DELETE FROM memory_relations WHERE chat_id = ? AND user_edited_at IS NULL").run(chatId);
    })();
}
// ─── Batch Ingestion ───────────────────────────────────────────
/**
 ...
 */
export function ingestChunkEntities(chatId, chunkId, chunkTimestamp, extractedEntities, extractedRelationships, chunkSalience, emotionalTags, content, arcId, 
...
discoveredAliases) {
    const db = getDb();
    const entityIdMap = new Map(); // name → entity ID
    const transaction = db.transaction(() => {
        ...
        for (const ext of extractedEntities) {
            ...
            upsertMention(entityId, chunkId, chatId, ext.mentionRole ?? ext.role ?? "present", excerpt, 0);
 ...
 */
export function pruneStaleEntities(chatId, staleAfterMessages) {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    ...
    const staleResult = db.query(`UPDATE memory_entities SET status = 'inactive', status_changed_at = ?, updated_at = ?
     WHERE chat_id = ? AND status = 'active' AND last_seen_at < ?
       AND ((entity_type != 'character' AND mention_count <= 2)
            OR (entity_type = 'character' AND mention_count <= 1))`).run(now, now, chatId, staleThreshold);
    ...
    if (activeCount?.c > MAX_ACTIVE_ENTITIES_PER_CHAT) {
        ...
        const toArchive = db
            .query(`SELECT id FROM memory_entities
         WHERE chat_id = ? AND status != 'inactive'
        ...
        if (toArchive.length > 0) {
            ...
            db.query(`UPDATE memory_entities SET status = 'inactive', status_changed_at = ?, updated_at = ?
    ...
    db.query(`DELETE FROM memory_mentions WHERE chat_id = ? AND entity_id IN (
    ...
    const heavyEntities = db
        .query(`SELECT entity_id, COUNT(*) as c FROM memory_mentions
       WHERE chat_id = ? GROUP BY entity_id HAVING c > ?`)
        .all(chatId, MAX_MENTIONS_PER_ENTITY);
    for (const { entity_id, c } of heavyEntities) {
        const excess = c - MAX_MENTIONS_PER_ENTITY;
        db.query(`DELETE FROM memory_mentions WHERE id IN (
    ...
    db.query(`DELETE FROM memory_relations
     WHERE chat_id = ? AND strength < 0.3
       AND json_array_length(evidence_chunk_ids) <= 1
       AND last_reinforced_at < ?
       AND contradiction_flag != 'complex'`).run(chatId, relationStaleThreshold);
    // Clean up merged edges that are old
    db.query(`DELETE FROM memory_relations
     WHERE chat_id = ? AND merged_into IS NOT NULL AND updated_at < ?`).run(chatId, relationStaleThreshold);
    ...
    if (relationCount?.c > MAX_RELATIONS_PER_CHAT) {
        ...
        db.query(`DELETE FROM memory_relations WHERE id IN (
 ...
 */
export function getProvisionalEntityNames(chatId) {
    const rows = getDb()
 ...
 */
export function getActiveEntities(chatId, limit = 500) {
    const rows = getDb()
        .query(`SELECT * FROM memory_entities
       WHERE chat_id = ? AND status != 'inactive'
 ...
 */
export function populateEntityDescription(entityId, excerpt) {
    if (!excerpt)
        return;
    const db = getDb();
    const row = db.query("SELECT description FROM memory_entities WHERE id = ?").get(entityId);
 ...
 */
export function runHeuristicsMigration(chatId, onProgress) {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    const result = {
        edgesRekeyed: 0,
    ...
    for (const edge of allEdges) {
        ...
        if (canonicalSource && canonicalTarget &&
            ...
            const existingCanonical = db
                .query(`SELECT id FROM memory_relations
           WHERE source_entity_id = ? AND target_entity_id = ? AND relation_type = ? AND id != ?`)
                .get(canonicalSource, canonicalTarget, edge.relation_type, edge.id);
            if (existingCanonical) {
                ...
                db.query(`UPDATE memory_relations SET source_entity_id = ?, target_entity_id = ?, updated_at = ? WHERE id = ?`).run(canonicalSource, canonicalTarget, now, edge.id);
            }
            result.edgesRekeyed++;
        }
    }
    ...
    for (const edge of activeEdges) {
        ...
        updated_at = ?
       WHERE id = ?`).run(newStrength, newDecayRate, newEdgeSalience, lastEvidence, now, edge.id);
    ...
    for (const [, edges] of pairMap) {
        ...
        for (let i = 0; i < edges.length; i++) {
            for (let j = i + 1; j < edges.length; j++) {
                ...
                sentiment_range = ?
               WHERE id = ?`).run(edges[j].id, JSON.stringify(range), edges[i].id);
                        db.query(`UPDATE memory_relations SET
                contradiction_flag = 'complex',
                contradiction_peer_id = ?,
                sentiment_range = ?
               WHERE id = ?`).run(edges[i].id, JSON.stringify(range), edges[j].id);
                    }
                    else {
                        // Suspect: weaker edge is suspect
                        const [stronger, weaker] = evI >= evJ ? [edges[i], edges[j]] : [edges[j], edges[i]];
                        db.query(`UPDATE memory_relations SET contradiction_flag = 'suspect', contradiction_peer_id = ? WHERE id = ?`).run(stronger.id, weaker.id);
                    }
    ...
    console.info(`[memory-cortex] Migration complete: ${result.edgesRekeyed} rekeyed, ${result.strengthsRecomputed} recomputed, ` +
 ...
 */
export function processProvisionalEntities(chatId, corroborationThreshold = 2, maxAge = 50) {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    let promoted = 0;
    let decayed = 0;
    ...
    for (const row of provisionalRows) {
        ...
        db.query(`UPDATE memory_entities SET confidence = 'confirmed', entity_type = ?, metadata = ?, updated_at = ?
       WHERE id = ?`).run(resolvedType, JSON.stringify(persistedMetadata), now, entity.id);
        promoted += 1;
    }
