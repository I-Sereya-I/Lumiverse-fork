...
function buildMentionIndex(content, entityNames) {
    ...
    indexed.sort((a, b) => {
        if (b.mentions.length !== a.mentions.length)
...
function mentionsAreNearby(a, b) {
    let i = 0;
    let j = 0;
    while (i < a.length && j < b.length) {
        const delta = Math.abs(a[i] - b[j]);
        if (delta <= PAIR_PROXIMITY_WINDOW)
            return true;
        if (a[i] < b[j])
            i++;
        else
...
function buildPairContentWindow(content, source, sourceMentions, target, targetMentions) {
    let i = 0;
    let j = 0;
    ...
    const end = Math.min(content.length, Math.max(bestSource + source.length, bestTarget + target.length) + PAIR_CONTEXT_PADDING);
 ...
 */
export function extractRelationshipsHeuristic(content, entityNames, emotionalTags = []) {
    if (entityNames.length < 2)
        return [];
    const indexedEntities = buildMentionIndex(content, entityNames);
    if (indexedEntities.length < 2)
        return [];
    ...
    const candidateDirectedPairs = [];
    const candidateUndirectedPairs = [];
    for (let i = 0; i < names.length; i++) {
        for (let j = i + 1; j < names.length; j++) {
            ...
            if (!mentionsAreNearby(leftMentions, rightMentions))
                continue;
            ...
            candidateDirectedPairs.push({ source: left, target: right, content: pairContent }, { source: right, target: left, content: pairContent });
        }
    }
    ...
    const relationships = [];
    const seen = new Set(); // Deduplicate "A→B type" combos
    ...
    if (relationships.length === 0 && names.length >= 2 && emotionalTags.length > 0) {
        const coRel = inferFromEmotionalContext(names[0], names[1], emotionalTags);
        if (coRel)
            relationships.push(coRel);
    }
    return relationships;
}
// ─── Detection Functions ───────────────────────────────────────
function detectVerbMediated(source, target, content) {
    const results = [];
    ...
    for (const { pattern, signal } of DIRECTED_VERB_PATTERNS) {
        ...
        const directPattern = new RegExp(`${srcEsc}\\s+${pattern.source}\\s+(?:\\w+\\s+){0,2}${tgtEsc}`, "i");
        ...
        const prepPattern = new RegExp(`${srcEsc}\\s+${pattern.source}\\s+(?:\\w+\\s+){0,6}(?:to|at|for|from|with|against|toward|before|after)\\s+(?:\\w+\\s+){0,2}${tgtEsc}`, "i");
...
function detectRelationalNouns(name1, name2, content) {
    const results = [];
    // Check BOTH directions: "name1's [relation]" near name2,
    // AND "name2's [relation]" near name1.
    const pairs = [[name1, name2], [name2, name1]];
    for (const [possessor, other] of pairs) {
        ...
        for (const { pattern, type, sentiment } of RELATIONAL_NOUNS) {
            ...
            const possPattern = new RegExp(`${escPoss}[''\u2019]s\\s+${pattern.source}`, "i");
            ...
            const ofPattern = new RegExp(`\\b${pattern.source}\\s+of\\s+${escPoss}`, "i");
...
function detectCoordinatedAction(name1, name2, content) {
    const esc1 = name1.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const esc2 = name2.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    // "Name1 and Name2 [verb] together" / "Name1 and Name2 fought side by side"
    const togetherPattern = new RegExp(`${esc1}\\s+and\\s+${esc2}\\s+\\w+(?:\\s+\\w+){0,3}\\s*(?:together|side by side|in unison|as one)`, "i");
...
function detectTermsOfAddress(speaker, target, content) {
    ...
    const speakerDialogue = new RegExp(`[""\u201C][^""\u201D]*${ENDEARMENT_TERMS.source}[^""\u201D]*[""\u201D][^.]{0,15}${spkEsc}`, "i");
    if (speakerDialogue.test(content)) {
        ...
        const specificAddressee = window.match(new RegExp(`${spkEsc}\\s+(?:said|told|whispered|called|murmured|spoke)\\s+(?:\\w+\\s+){0,3}(?:to|at)\\s+(\\w+)`, "i"));
        if (specificAddressee) {
            if (specificAddressee[1].toLowerCase() !== target.split(/\s+/)[0].toLowerCase())
                return null;
        }
    ...
    const hostileDialogue = new RegExp(`[""\u201C][^""\u201D]*${HOSTILITY_TERMS.source}[^""\u201D]*[""\u201D][^.]{0,15}${spkEsc}`, "i");
    if (hostileDialogue.test(content)) {
        ...
        const specificAddressee = window.match(new RegExp(`${spkEsc}\\s+(?:shouted|snarled|yelled|screamed|spat|hissed|snapped)\\s+(?:at\\s+)?(\\w+)`, "i"));
        if (specificAddressee) {
            if (specificAddressee[1].toLowerCase() !== target.split(/\s+/)[0].toLowerCase())
                return null;
        }
...
function detectPhysicalProximity(name1, name2, content) {
    const esc1 = name1.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const esc2 = name2.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    // "Name1 ... beside/next to/near ... Name2" (within 80 chars)
    const proximityPattern = new RegExp(`${esc1}.{0,60}\\b(?:beside|next\\s+to|near|across\\s+from|facing|opposite|behind|alongside)\\b.{0,20}${esc2}`, "i");
    ...
    const dialogueExchange = new RegExp(`[""\u201C][^""\u201D]+[""\u201D][^""\u201C]{0,60}${esc1}.{0,120}[""\u201C][^""\u201D]+[""\u201D][^""\u201C]{0,60}${esc2}`, "i");
    const reverseExchange = new RegExp(`[""\u201C][^""\u201D]+[""\u201D][^""\u201C]{0,60}${esc2}.{0,120}[""\u201C][^""\u201D]+[""\u201D][^""\u201C]{0,60}${esc1}`, "i");
...
function inferFromEmotionalContext(name1, name2, emotionalTags) {
