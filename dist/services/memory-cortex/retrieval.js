...
export async function queryCortex(query, config, signal) {
    ...
    if (activeEntityIds.length > 0) {
        ...
        for (const id of entityChunkIds)
            candidateChunkIds.add(id);
    }
    // Always include high-salience chunks (serendipitous recall)
    if (config.salienceScoring) {
        const highSalienceChunkIds = getHighSalienceChunkIds(db, query.chatId, Math.ceil(query.topK * 0.5));
        for (const id of highSalienceChunkIds)
            candidateChunkIds.add(id);
    }
    ...
    try {
        ...
        vectorResults = await searchChatChunksScoped(query.userId, query.chatId, queryVector, candidateChunkIds, query.topK * 3, // Over-fetch for reranking
        query.excludeMessageIds, signal);
    }
    catch (err) {
        if (signal?.aborted)
            return emptyResult(startTime);
    ...
    const scoredMemories = vectorResults.map((vr) => {
        ...
        const isCoreMemory = salienceScore >= config.decay.coreMemoryThreshold ||
            (salience?.narrativeFlags?.length &&
                salience.narrativeFlags.some((f) => config.decay.coreMemoryFlags.includes(f)));
        ...
        if (config.retrieval.emotionalResonance && query.emotionalContext?.length && salience?.emotionalTags?.length) {
            const overlap = salience.emotionalTags.filter((t) => query.emotionalContext.includes(t));
        ...
        return {
            ...
            emotionalTags: (salience?.emotionalTags ?? []),
    ...
    if (query.includeConsolidations && config.formatterMode !== "minimal") {
        selected = replaceSelectedChunksWithConsolidations(db, selected, chunkMetaMap, query.excludeMessageIds);
    }
    // ────────────────────────────────────────────
    // PHASE 5: Entity Context Assembly
    // ────────────────────────────────────────────
    ...
    const relationsLimit = Math.max(config.retrieval.maxEntitySnapshots * 5, config.retrieval.maxRelationships);
    ...
    if (config.retrieval.entityContextInjection && activeEntityIds.length > 0) {
        entitySnapshots = entityContext.assembleEntitySnapshots(query.chatId, activeEntityIds, config.retrieval.maxEntitySnapshots, preloadedRelations);
    }
    if (config.retrieval.relationshipInjection && activeEntityIds.length > 0) {
        activeRelationships = entityContext.getActiveRelationEdges(query.chatId, activeEntityIds, config.retrieval.maxRelationships, preloadedRelations);
    }
...
function getHighSalienceChunkIds(db, chatId, limit) {
    const rows = db
        .query(`SELECT chunk_id FROM memory_salience
       WHERE chat_id = ? AND score >= 0.6
       ORDER BY score DESC LIMIT ?`)
        .all(chatId, limit);
    return rows.map((r) => r.chunk_id);
}
function getRecentVectorizedChunkIds(db, chatId, limit) {
    const rows = db
        .query(`SELECT id FROM chat_chunks
       WHERE chat_id = ? AND vectorized_at IS NOT NULL
       ORDER BY created_at DESC LIMIT ?`)
        .all(chatId, limit);
    return rows.map((r) => r.id);
}
function removeExcludedMessageChunks(db, chatId, candidateChunkIds, excludeMessageIds) {
    if (candidateChunkIds.size === 0 || !excludeMessageIds || excludeMessageIds.length === 0)
        return;
...
function loadSalienceMap(db, chatId, chunkIds) {
    const map = new Map();
    if (chunkIds.size === 0)
        return map;
    ...
    for (let i = 0; i < idArray.length; i += 500) {
        ...
        for (const row of rows) {
            map.set(row.chunk_id, {
                ...
                narrativeFlags: safeJsonArray(row.narrative_flags),
            });
        }
    }
    return map;
 ...
 */
async function searchChatChunksScoped(userId, chatId, queryVector, candidateChunkIds, limit, excludeMessageIds, signal) {
    ...
    const hits = await embeddingsSvc.searchChatChunks(userId, chatId, queryVector, excludeIds, limit, undefined, undefined, candidateChunkIds, signal);
    // Filter to candidate set if we have one
    const filtered = candidateChunkIds.size > 0
        ? hits.filter((h) => candidateChunkIds.has(h.chunk_id))
        : hits;
...
function batchLoadChunkMeta(db, chunkIds) {
    const map = new Map();
    ...
    for (let i = 0; i < chunkIds.length; i += 500) {
        ...
        const placeholders = batch.map(() => "?").join(",");
        const rows = db
            .query(`SELECT id, created_at, updated_at, retrieval_count, entity_ids, consolidation_id,
                COALESCE(message_range_start, 0) as message_range_start,
                COALESCE(message_range_end, 0) as message_range_end
...
function replaceSelectedChunksWithConsolidations(db, selected, chunkMetaMap, excludeMessageIds) {
    ...
    const rows = db.query(`SELECT * FROM memory_consolidations
     WHERE tier = 1 AND id IN (${placeholders})`).all(...consolidationIds);
    const rowsById = new Map(rows.map((row) => [row.id, row]));
    if (rowsById.size === 0)
        return selected;
    const sourceRanges = new Map();
    const rangeRows = db.query(`SELECT consolidation_id,
      ...
      GROUP BY consolidation_id`).all(...consolidationIds);
    ...
    const unsafeConsolidationIds = new Set();
    const excluded = new Set(excludeMessageIds ?? []);
    if (excluded.size > 0) {
        const sourceRows = db.query(`SELECT consolidation_id, message_ids FROM chat_chunks
       WHERE consolidation_id IN (${placeholders})`).all(...consolidationIds);
    ...
    const bestHitByConsolidation = new Map();
    ...
    for (const memory of selected) {
        ...
        if (emitted.has(consolidationId))
            continue;
...
function resolveEntityNames(db, entityIdsJson) {
    if (!entityIdsJson)
        return [];
 ...
 */
function diversitySelect(memories, topK, totalMessages = 200) {
    const sorted = [...memories].sort((a, b) => b.finalScore - a.finalScore);
    const selected = [];
    const coveredWindows = new Map(); // window → highest score
    ...
    for (const mem of sorted) {
        ...
        if (existingScore != null) {
            ...
            if (mem.finalScore - existingScore < -0.15)
                continue;
        }
        selected.push(mem);
...
function batchUpdateRetrievalStats(db, chunkIds) {
    if (chunkIds.length === 0)
        return;
    const now = Math.floor(Date.now() / 1000);
    const stmt = db.query("UPDATE chat_chunks SET retrieval_count = COALESCE(retrieval_count, 0) + 1, last_retrieved_at = ? WHERE id = ?");
 ...
 */
export async function queryVaultCortex(query, config) {
    const startTime = Date.now();
    const db = getDb();
    ...
    const vaultEntities = db.query(`SELECT id, name, entity_type, aliases, description, status, facts,
            emotional_valence, salience_avg
     FROM cortex_vault_entities WHERE vault_id = ? ORDER BY salience_avg DESC`).all(vaultId);
    ...
    const chunkRows = db.query(`SELECT id, source_chunk_id, content, salience_score, emotional_tags,
            entity_names, source_created_at
     FROM cortex_vault_chunks WHERE vault_id = ?`).all(vaultId);
    ...
    try {
        ...
        if (!queryVector || queryVector.length === 0)
            return emptyResult(startTime);
        const hits = await embeddingsSvc.searchVaultChunks(userId, vaultId, queryVector, topK * 3, candidateIds, signal);
    ...
    const scored = vectorResults.map((vr) => {
        ...
        const emotionalTags = safeJsonArray(row?.emotional_tags);
        ...
        return {
            ...
            entityNames,
            messageRange: [0, 0],
            timeRange: [row?.source_created_at ?? 0, row?.source_created_at ?? 0],
        };
    });
    // ── Phase 4: Diversity selection ──
    ...
    if (config.retrieval.entityContextInjection) {
        ...
        const activeInMemories = new Set();
        for (const m of selected)
            for (const n of m.entityNames)
                activeInMemories.add(n);
        ...
        for (const e of orderedEntities) {
            ...
            try {
                emotionalProfile = JSON.parse(e.emotional_valence);
            }
            catch { /* empty */ }
            entitySnapshots.push({
                ...
                lastSeenAt: null,
                mentionCount: 0,
                topFacts: safeJsonArray(e.facts).map((f) => entityGraph.stripFactTags(f)),
                emotionalProfile,
                relationships: [],
            });
        }
    ...
    if (query.includeRelationships !== false && config.retrieval.relationshipInjection) {
        const relRows = db.query(`SELECT source_entity_name, target_entity_name, relation_type, relation_label,
              strength, sentiment
       FROM cortex_vault_relations WHERE vault_id = ?
       ORDER BY strength DESC LIMIT ?`).all(vaultId, config.retrieval.maxRelationships);
