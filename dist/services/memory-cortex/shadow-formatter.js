...
function formatEntitiesShadow(snapshots, budget) {
    ...
    if (parts.length === 0)
        return "";
    return ("[Narrative context — weave naturally into the scene, never recite directly]\n" +
        "The story has established the following. Reference only when organic to the moment:\n\n" +
        parts.join("\n\n"));
}
// ─── Memory Formatting ─────────────────────────────────────────
function formatMemoriesShadow(memories, budget) {
    ...
    if (parts.length === 0)
        return "";
    return ("[Relevant continuity — internalize and weave naturally; never recite this block]\n" +
        parts.join("\n\n"));
}
function formatMemoriesAttributed(memories, budget, speakerName) {
    if (memories.length === 0)
        return "";
    const parts = [];
    let used = 0;
    ...
    if (parts.length === 0)
        return "";
    return ("[What the characters remember — reference through their perspective, not as exposition]\n" +
        parts.join("\n\n"));
}
function formatMemoriesMinimal(memories, budget) {
    const parts = [];
    let used = 0;
    for (const memory of memories) {
        if (memory.source !== "chunk")
            continue;
...
function formatRelationshipsShadow(edges, budget) {
    if (edges.length === 0)
        return "";
    const parts = [];
    let used = 0;
    for (const edge of edges) {
        const sentiment = edge.sentiment > 0.3 ? "warmth" :
...
// ─── Arc Summary ───────────────────────────────────────────────
function formatArc(arcContext, budget, mode) {
    if (!arcContext)
        return "";
 ...
 */
export function formatShadowPrompt(memories, entities, relationships, arcContext, options) {
 ...
 */
export function formatContextSections(entities, relationships, arcContext, options) {
...
// ─── Clinical Fallback ─────────────────────────────────────────
function formatClinical(memories, entities, relationships, arcContext, budget) {
    ...
    return {
        ...
        entitiesIncluded: entities.length,
        memoriesIncluded: memories.length,
        componentsIncluded: ["entities", "relationships", "memories", "arc"].filter((_, i) => !![entText, relText, memText, arcText][i]),
    };
}
/**
 ...
 */
export function formatLinkedCortexSection(vaults, interlinks, options) {
    ...
    for (const vault of vaults) {
        ...
        const formatted = formatShadowPrompt(vault.memories ?? [], vault.entities, vault.relations, vault.arcContext ?? null, { ...options, tokenBudget: Math.max(0, perSourceBudget - estimateTokens(header)) }).text;
        if (formatted)
    ...
    for (const interlink of interlinks) {
        ...
        const formatted = formatShadowPrompt(memories, entityContext, activeRelationships, arcContext, { ...options, tokenBudget: Math.max(0, perSourceBudget - estimateTokens(header)) }).text;
        if (formatted)
            sections.push(`${header}\n${formatted}`);
    }
    const text = sections.join("\n\n");
