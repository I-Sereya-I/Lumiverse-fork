...
function isNameLikeWord(word) {
    ...
    if (!cleaned)
        return false;
    if (ALLOWED_PARTICLES.has(cleaned.toLowerCase()))
        return true;
    return /^[A-Z][a-z]+(?:[-'][A-Z][a-z]+)?$/.test(cleaned)
        || /^[A-Z][a-z]+[A-Z][A-Za-z]*$/.test(cleaned)
        || /^[A-Z]{2,4}$/.test(cleaned);
}
export function isPlausibleAlias(alias, canonicalName) {
    ...
    if (/[.!?;:]$/.test(trimmed))
        return false;
    if (/\b(?:is|are|was|were|be|been|being|has|have|had|do|does|did|can|could|would|should|will|shall)\b/i.test(trimmed))
        return false;
    const words = trimmed.split(/\s+/).filter(Boolean);
    if (words.length > 4)
        return false;
    const first = words[0].toLowerCase().replace(/^["'\u201C\u201D\u2018\u2019]+|["'\u201C\u201D\u2018\u2019.,!?;:]+$/g, "");
    if (PHRASE_START_REJECTS.has(first))
        return false;
    const key = normalizeAliasKey(trimmed);
    if (!key || COMMON_ALIAS_REJECTS.has(key))
        return false;
    if (canonicalName && key === normalizeAliasKey(canonicalName))
        return false;
    if (words.length === 1) {
        if (COMMON_ALIAS_REJECTS.has(first))
            return false;
    ...
    if (significantWords.length === 0)
        return false;
    if (!significantWords.some((word) => /^[A-Z]/.test(word)))
        return false;
    if (significantWords.some((word) => COMMON_ALIAS_REJECTS.has(word.toLowerCase())))
        return false;
