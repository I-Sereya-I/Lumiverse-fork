...
function initEntityGraphTestDb() {
    closeDatabase();
    initDatabase(":memory:");
    const db = getDb();
...
function seedChunk(chatId, id, createdAt) {
    getDb().query("INSERT INTO chat_chunks (id, chat_id, created_at) VALUES (?, ?, ?)").run(id, chatId, createdAt);
}
function makeEntity(name, type, confidence, provisional = false) {
    return {
        name,
        type,
        aliases: [],
...
describe("entity graph type evidence", () => {
    test("promotes a concept to faction after corroborated cross-chunk evidence", () => {
        ...
        expect(entity?.metadata?.typeEvidence?.counts?.faction).toBe(2);
    });
    test("demotes a weak faction back to concept when later evidence contradicts it", () => {
        ...
        expect(entity?.metadata?.typeEvidence?.counts?.concept).toBe(2);
    });
