...
describe("Cortex consolidation formatting", () => {
    test("narrative mode presents scenes and arcs as internal continuity", () => {
        const result = formatShadowPrompt([rawMemory, sceneMemory], entities, relationships, arc, { mode: "shadow", tokenBudget: 2000 }).text;
    ...
    test("attributed mode includes evidence ranges and shared arc continuity", () => {
        const result = formatShadowPrompt([sceneMemory], entities, relationships, arc, { mode: "attributed", tokenBudget: 2000, currentSpeakerName: "Mara" }).text;
    ...
    test("clinical mode labels scene and arc records explicitly", () => {
        const result = formatShadowPrompt([sceneMemory], entities, relationships, arc, { mode: "clinical", tokenBudget: 2000 }).text;
    ...
    test("minimal mode keeps only raw memory chunks", () => {
        const result = formatShadowPrompt([rawMemory, sceneMemory], entities, relationships, arc, { mode: "minimal", tokenBudget: 2000 }).text;
