...
import { extractColorBlocks, stripFontTags, stripThoughtDelimiters, attributeColorBlock, extractMessageOwner, detectParagraphFocalCharacter, processChunkFontColors, formatColorMapForPrompt, getColorMap, } from "./font-attribution";
...
function makeBlock(hexColor, content, offset = 0, paragraphIndex = 0) {
...
describe("full narrative attribution — worst case scenario", () => {
    ...
    test("all blocks in the worst-case narrative are correctly attributed", () => {
        ...
        for (const r of blockResults) {
            if (!r.entityName || r.confidence < 0.6)
                continue;
        ...
        for (let i = 0; i < blockResults.length; i++) {
            ...
            if (result.entityName && result.confidence >= 0.6)
                continue;
            ...
            for (let j = 0; j < blockResults.length; j++) {
                ...
                if (neighbor.block.paragraphIndex !== paragraphIdx)
                    continue;
                if (!neighbor.entityName || neighbor.confidence < 0.65)
                    continue;
        ...
        for (const r of results) {
            ...
            console.log(`  [${status}] P${r.paragraph} ${r.hex} → ${r.got || "NULL"} (expected ${r.expected}, conf=${conf})`);
            if (status === "MISS") {
...
describe("processChunkFontColors", () => {
    test("classifies a configured thought color after non-prose sanitization", () => {
        ...
        const result = processChunkFontColors(chatId, proseContent, ["Melina"], entityIdByName, THOUGHT_DELIMITERS);
    ...
    test("tracks configured thought and speech colors for all 4 characters", () => {
        ...
        processChunkFontColors(chatId, THOUGHT_AND_SPEECH_NARRATIVE, KNOWN_NAMES, entityIdByName, THOUGHT_DELIMITERS);
        ...
        for (const name of KNOWN_NAMES) {
            const usageTypes = byEntity.get(name.toLowerCase()) || new Set();
