 ...
 */
export function extractColorBlocks(content, thoughtDelimiters) {
    const blocks = [];
    ...
    const seenOffsets = new Set(); // Prevent double-matching quoted + unquoted
    for (const pattern of [FONT_TAG_QUOTED, FONT_TAG_UNQUOTED, SPAN_COLOR_PATTERN]) {
        pattern.lastIndex = 0;
        let match;
        while ((match = pattern.exec(content)) !== null) {
            if (seenOffsets.has(match.index))
                continue; // Skip double-match
 ...
 */
function buildParagraphBoundaries(content) {
    const boundaries = [0];
...
function findParagraphIndex(offset, boundaries) {
 ...
 */
export function stripFontTags(content) {
    let result = content;
    // Quoted: <font color="...">
...
function isDelimitedThought(content, delimiters) {
    if (!delimiters?.prefix || !delimiters?.suffix)
        return false;
...
export function stripThoughtDelimiters(content, delimiters) {
    if (!content || !delimiters?.prefix || !delimiters?.suffix)
        return content;
    const prefix = delimiters.prefix.trim();
    const suffix = delimiters.suffix.trim();
    if (!prefix || !suffix)
...
function detectUsageType(coloredContent, thoughtDelimiters) {
    const trimmed = coloredContent.trim();
    // Quoted dialogue: "Hello" or "Hello," she said
    if (/[""\u201C][^""\u201D]+[""\u201D]/.test(trimmed))
        return "speech";
    if (isDelimitedThought(trimmed, thoughtDelimiters))
        return "thought";
    // Asterisk-wrapped narration: *she hesitated*
    if (/^\*[^*]+\*$/.test(trimmed) || /^\*/.test(trimmed))
        return "thought";
    // Italic HTML
    if (/<[ie]m>/i.test(trimmed))
        return "thought";
    return "narration";
}
...
function findSurroundingDialogueAttribution(block, rawContent, knownNames, paragraphBounds) {
    if (block.usageType !== "speech")
        return null;
 ...
 */
export function detectParagraphFocalCharacter(paragraphText, knownNames) {
 ...
 */
function proximityNameScan(rawContent, blockOffset, blockLength, knownNames, paragraphBounds, radius = 200) {
 ...
 */
export function attributeColorBlock(block, messageOwner, knownNames, rawContent = "", focalCharacter = null, multiCharacterContent = false, paragraphBounds) {
    ...
    if (preMatch) {
        ...
        if (matched)
            return { entityName: matched, confidence: 0.88 };
    }
    ...
    if (postMatch) {
        ...
        if (matched)
            return { entityName: matched, confidence: 0.85 };
    }
 ...
 *   - "Oh Name" / "Dear Name"
 */
function isNameVocativeOnly(text, name) {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    ...
    const vocativePattern = new RegExp(`${escaped}\\s*[,!]\\s*(?:can|could|will|would|do|don't|please|help|tell|listen|look|stop|wait|come|go)`, "i");
    if (vocativePattern.test(text))
        return true;
    // "Oh/Dear/Hey Name" — vocative address
    if (new RegExp(`\\b(?:oh|dear|hey|listen)\\s+${escaped}`, "i").test(text))
        return true;
 ...
 */
export function extractMessageOwner(chunkLine) {
 ...
 */
export function getColorMap(chatId) {
    const rows = getDb()
 ...
 */
export function lookupColor(chatId, hexColor) {
    const row = getDb()
 ...
 */
export function recordColorAttribution(chatId, hexColor, entityId, usageType, sampleExcerpt) {
    const db = getDb();
    ...
    if (existing) {
        ...
        db.query(`UPDATE memory_font_colors SET
        sample_count = ?, confidence = ?, usage_type = ?,
        sample_excerpt = COALESCE(?, sample_excerpt),
        updated_at = ?
       WHERE id = ?`).run(newCount, newConfidence, usageType, sampleExcerpt, now, existing.id);
    }
    else {
        ...
        db.query(`INSERT INTO memory_font_colors
        (id, chat_id, entity_id, hex_color, usage_type, confidence, sample_count, sample_excerpt, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, 0.4, 1, ?, ?, ?)`).run(crypto.randomUUID(), chatId, entityId, hexColor, usageType, sampleExcerpt, now, now);
    }
}
/**
 * Delete all font color mappings for a chat (used in rebuild).
 */
export function deleteColorMapForChat(chatId) {
 ...
 */
function isMultiCharacterContent(content, knownNames) {
 ...
 */
export function processChunkFontColors(chatId, content, knownEntityNames, entityIdByName, thoughtDelimiters) {
    ...
    for (const result of blockResults) {
        if (!result.attribution.entityName || result.attribution.confidence < 0.6)
            continue;
    ...
    for (let i = 0; i < blockResults.length; i++) {
        ...
        for (let j = 0; j < blockResults.length; j++) {
            ...
            if (neighbor.block.paragraphIndex !== paragraphIdx)
                continue;
            if (!neighbor.attribution.entityName || neighbor.attribution.confidence < 0.65)
                continue;
 ...
 *   - Thoughts: #fec987
 */
export function formatColorMapForPrompt(chatId) {
    ...
    for (const m of mappings) {
        if (!m.entityId || m.confidence < 0.5)
            continue;
        if (m.usageType !== "speech" && m.usageType !== "thought")
            continue;
        // Resolve entity name
        const entityRow = db.query("SELECT name FROM memory_entities WHERE id = ?").get(m.entityId);
        if (!entityRow)
            continue;
        const name = entityRow.name;
        if (!byEntity.has(name))
            byEntity.set(name, new Map());
        const usageMap = byEntity.get(name);
        const label = USAGE_LABELS[m.usageType] || m.usageType;
