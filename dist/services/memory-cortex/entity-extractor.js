 ...
 */
import { buildProtectedLineEntities, filterEntitiesByExtractionFilters, getDefaultEntityExtractionFilters, } from "./entity-extraction-filters";
...
function hasCharacterSignal(name, content) {
    ...
    if (new RegExp(`\\b(?:${CHARACTER_VERB_SRC})\\s+${escaped}`, "i").test(content))
        return true;
    ...
    if (possMatch) {
        ...
        if (CHAR_POSSESSIVE_NOUNS.test(wordAfter))
            return true;
        ...
        if (/^(had|been|gone|got|gotten|done|always|never|already|just|still|not|being|going|coming|trying|looking|getting|making|having|taking|running|sitting|standing|waiting|sleeping)$/i.test(wordAfter))
            return true;
    }
    // 3b. Comparative possessive: "harder than Melina's had" — Name's in comparison = character
    if (new RegExp(`(?:than|like|as)\\s+${escaped}[''\u2019]s`, "i").test(content))
        return true;
    // 4. Interaction prepositions: "talk to [Name]", "ask [Name]", "tell [Name]"
    if (new RegExp(`\\b(?:talk|speak|spoke|listen)(?:s|ed|ing)?\\s+to\\s+${escaped}\\b`, "i").test(content))
        return true;
    if (new RegExp(`\\b(?:ask|tell|told|show|gave|give|handed|thank|thanked|greet|greeted|hugged|kissed|punched|grabbed|helped)(?:s|ed|ing)?\\s+${escaped}\\b`, "i").test(content))
        return true;
    // 5. "Name and I" / "Name and [pronoun]" — coordinated agents
    if (new RegExp(`${escaped}\\s+and\\s+(?:I|me|we|he|she|they)\\b`, "i").test(content))
        return true;
    // 6. "care of Name" / "waiting for Name" — object of concern
    if (new RegExp(`\\b(?:care of|waiting for|looking for|worried about|afraid of)\\s+${escaped}\\b`, "i").test(content))
        return true;
    ...
    if (new RegExp(`\\bname\\s+is\\s+${escaped}\\b`, "i").test(content))
        return true;
    if (new RegExp(`\\b(?:I'm|I am|they call me|known as|called)\\s+${escaped}\\b`, "i").test(content))
        return true;
    if (new RegExp(`\\byes\\s+(?:THAT|that)\\s+${escaped}\\b`, "i").test(content))
        return true;
    ...
    if (new RegExp(`[""\u201C]\\s*${escaped}\\s*,\\s*${vocativeFollowers}`, "i").test(content))
        return true;
 ...
 */
function hasLocationSuffix(name, content) {
    // Check the name itself
    if (LOCATION_SUFFIXES.test(name))
        return true;
    // Check if the word immediately after the name is a location suffix
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    if (new RegExp(`${escaped}\\s+${LOCATION_SUFFIXES.source}`, "i").test(content))
        return true;
    return false;
}
/**
 ...
 */
function hasLocationPhrase(name, content) {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    // Locative constructions: "here in X", "back in X", "life in X", "live in X"
    if (new RegExp(`\\b(?:here|there|back|life|live[ds]?|born|raised|grew up)\\s+(?:in|at)\\s+(?:the\\s+)?${escaped}`, "i").test(content))
        return true;
    // Movement to/from location: "arrived at X", "traveled to X", "left X", "fled X"
    if (new RegExp(`\\b(?:arrived?|traveled?|journey(?:ed)?|headed|went|go(?:ing)?|return(?:ed)?|fled|left|departed|sailed|flew|drove|walked)\\s+(?:to|from|toward|into|through|across|for)\\s+(?:the\\s+)?${escaped}`, "i").test(content))
        return true;
    // Directional: "across the X", "through the X", "beyond the X"
    if (new RegExp(`\\b(?:across|through|beyond|outside|inside|beneath|above|around)\\s+(?:the\\s+)?${escaped}`, "i").test(content))
        return true;
    // "the streets/walls/gates of X" — part-of-location construction
    if (new RegExp(`\\b(?:streets?|walls?|gates?|outskirts|edge|heart|center|depths?|borders?|lights?|spires?|skyline|rooftops?|towers?|buildings?)\\s+of\\s+(?:the\\s+)?${escaped}`, "i").test(content))
        return true;
    // Possessive + architectural/geographic noun: "New Eridu's spires", "Ashenmere's walls"
    if (new RegExp(`${escaped}[''\u2019]s\\s+(?:central\\s+)?(?:streets?|walls?|gates?|spires?|skyline|towers?|rooftops?|district|quarter|outskirts|borders?|docks?|harbor|port|market|square|plaza|bridge|cathedral|palace|keep|lights?|buildings?|slums?|alleys?|sewers?)`, "i").test(content))
        return true;
    return false;
}
 ...
 */
function hasFactionSignal(name, content) {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    // Name contains a faction noun directly
    if (FACTION_NOUNS.test(name))
        return true;
    ...
    if (camelParts && camelParts.length >= 2) {
        ...
        if (/^(housekeeping|industries|inc|corp|corporation|security|sec|logistics|solutions|services|tech|tek|labs?|ops|med|dynamics|robotics|pharma|bio|works|forge|net|link|sys|soft|ware|guard|watch|corps|force)$/.test(lastPart))
            return true;
    }
    // Collective prefix IN the name: "Sons of Calydon", "Knights of the Rose"
    if (FACTION_PREFIX.test(name))
        return true;
    // Collective prefix BEFORE the captured name: "Sons of [Calydon]"
    // The name itself is "Calydon" but it was captured after "Sons of"
    if (new RegExp(`${FACTION_PREFIX.source}\\s+(?:the\\s+)?${escaped}`, "i").test(content))
        return true;
    // Institutional reference: "friends at X", "contact in X", "works for X"
    if (new RegExp(`\\b(?:friends?|contacts?|people|agents?|officers?|members?|works?|worked|employed)\\s+(?:at|in|from|with|for)\\s+(?:the\\s+)?${escaped}`, "i").test(content))
        return true;
    // "X agent/officer/member/roster/personnel"
    if (new RegExp(`${escaped}\\s+(?:agent|officer|member|operative|enforcer|soldier|guard|patrol|roster|personnel|employee|staff)`, "i").test(content))
        return true;
    // "X van/truck/vehicle" — organizational vehicle (not the entity itself)
    if (new RegExp(`${escaped}\\s+(?:van|truck|vehicle|car|cruiser|ship|vessel)\\b`, "i").test(content))
        return true;
    return false;
}
 ...
 */
function isTitledCharacter(name) {
 ...
 */
function hasItemSignal(name, content) {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    // Vehicle verbs: "drives/rides/piloted the X"
    if (new RegExp(`\\b(?:drives?|drove|rides?|rode|pilots?|piloted|steers?|steered|boards?|boarded|aboard)\\s+(?:the\\s+)?${escaped}`, "i").test(content))
        return true;
    // Weapon verbs: "wielding/drew/sheathed the X"
    if (new RegExp(`\\b(?:wielding|wielded|drew|draw|sheathed?|holstered?|fired|swung|brandished?)\\s+(?:the\\s+)?${escaped}`, "i").test(content))
        return true;
    // Possessive mechanical: "X's engine/hull/blade/handle"
    if (new RegExp(`${escaped}[''\u2019]s\\s+(?:engine|hull|blade|handle|grip|barrel|trigger|hilt|deck|cockpit|wheel|controls?)`, "i").test(content))
        return true;
    // Item nouns in the name
    if (ITEM_NOUNS.test(name))
        return true;
    return false;
}
// ─── Event Detection ───────────────────────────────────────────
...
function inferEntityType(name, context) {
    ...
    if (idx !== -1) {
        ...
        if (LOCATION_PLACE_NOUNS.test(surroundings))
            return "location";
        // Faction nouns in window
        if (FACTION_NOUNS.test(surroundings))
            return "faction";
        ...
        if (ITEM_NOUNS.test(surroundings) && !FACTION_NOUNS.test(surroundings) && !FACTION_PREFIX.test(surroundings) && !nameInDialogue)
            return "item";
        // Event patterns in window
        if (EVENT_PATTERNS.test(surroundings))
            return "event";
    }
    return "concept";
}
 ...
 */
function inferMentionRole(name, content) {
 ...
 */
function buildAliasLookup(knownEntities, characterNames) {
    const map = new Map();
 ...
 */
export function extractEntitiesHeuristic(content, knownEntities, characterNames, whitelist = [], minConfidence = 0, filters = getDefaultEntityExtractionFilters()) {
    ...
    const suppressedSubstrings = new Set();
    ...
    for (const m of camelMatches) {
        ...
        if (!found.has(key) && !effectiveCommon.has(key)) {
            ...
            for (const part of m.match(/[A-Z][a-z]*/g) ?? []) {
                if (part.length >= 2)
                    suppressedSubstrings.add(part.toLowerCase());
            }
        }
    }
    ...
    for (const noun of matches) {
        ...
        if (found.has(key) || effectiveCommon.has(key) || suppressedSubstrings.has(key))
            continue;
        ...
        if (afterNoun && ADJECTIVE_FOLLOWERS.test(afterNoun[1]))
            continue;
        ...
        while (true) {
            ...
            if (idx === -1)
                break;
            if (!sentenceStartPositions.has(idx))
                midSentenceCount++;
    ...
    const prepMatches = content.match(/(?:the|in|at|from|to|toward|through|across|within|beyond|outside|inside)\s+([A-Z][a-z]+(?:[A-Z][a-z]*)*(?:\s+[A-Z][a-z]+(?:[A-Z][a-z]*)*){0,2})/g) || [];
    for (const match of prepMatches) {
        ...
        if (found.has(key) || effectiveCommon.has(key) || suppressedSubstrings.has(key) || name.length <= 2)
            continue;
        ...
        if (nameIdx >= 0) {
            ...
            if (afterName && ADJECTIVE_FOLLOWERS.test(afterName[1]))
                continue;
        }
        {
 ...
 */
export function extractMentionExcerpt(entityName, content, maxLength = 120) {
 ...
 */
export function detectNicknameIntroductions(content, knownEntities, characterNames) {
    const results = [];
    const seenAliases = new Set();
    // Build lookup for known names (canonical + aliases)
    const knownNameSet = new Set();
    const nameToCanonical = new Map();
    const registerCanonicalVariant = (variant, canonical) => {
    ...
    for (const { pattern } of NICKNAME_PATTERNS) {
        ...
        while ((match = pattern.exec(content)) !== null) {
            ...
            if (!aliasRaw || aliasRaw.length < 2)
                continue;
            if (knownNameSet.has(aliasRaw.toLowerCase()))
                continue;
            if (COMMON_WORDS.has(aliasRaw.toLowerCase()))
                continue;
            let canonical = match.groups.canonical?.trim() ?? null;
            if (canonical) {
            ...
            if (seenAliases.has(key))
                continue;
            seenAliases.add(key);
            if (canonical.toLowerCase() === aliasRaw.toLowerCase())
                continue;
            if (!isPlausibleAlias(aliasRaw, canonical))
                continue;
 ...
 */
function findNearestKnownName(content, position, knownEntities, characterNames) {
    ...
    for (const name of characterNames) {
        ...
        const parts = name.split(/\s+/).filter(Boolean);
        if (parts.length > 1) {
            if (!candidateNames.has(parts[0]))
                candidateNames.set(parts[0], name);
            const last = parts[parts.length - 1];
            if (!candidateNames.has(last))
                candidateNames.set(last, name);
        }
    }
    for (const entity of knownEntities) {
        ...
        if (parts.length > 1) {
            if (!candidateNames.has(parts[0]))
                candidateNames.set(parts[0], entity.name);
            const last = parts[parts.length - 1];
            if (!candidateNames.has(last))
                candidateNames.set(last, entity.name);
        }
    }
