...
const TOOL_GRADE_HEURISTIC_CANDIDATES = {
...
const EXTRACTION_TOOLS = [TOOL_SALIENCE, TOOL_ENTITIES, TOOL_RELATIONSHIPS, TOOL_FONT_COLORS];
const ARBITER_EXTRACTION_TOOLS = [...EXTRACTION_TOOLS, TOOL_GRADE_HEURISTIC_CANDIDATES];
const salienceProperties = TOOL_SALIENCE.parameters.properties;
const entityProperties = TOOL_ENTITIES.parameters.properties;
const relationshipProperties = TOOL_RELATIONSHIPS.parameters.properties;
const fontColorProperties = TOOL_FONT_COLORS.parameters.properties;
const gradingProperties = TOOL_GRADE_HEURISTIC_CANDIDATES.parameters.properties;
...
const TOOL_ANALYZE_PASSAGE_BATCH = {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        grading: {
                            ...
                            required: TOOL_GRADE_HEURISTIC_CANDIDATES.parameters.required,
...
export function getToolChoiceParams(provider) {
 ...
 */
export function parseToolCallResults(toolCalls) {
    let importance = 5;
    let emotionalTags = [];
    let narrativeFlags = [];
    let keyFacts = [];
    ...
    let relationships = [];
    let fontColors = [];
    let discoveredAliases = [];
    let gradedHeuristics;
    ...
    relationships = relationships.filter((r) => !ENTITY_BLOCKLIST.has(r.source.toLowerCase().trim()) && !ENTITY_BLOCKLIST.has(r.target.toLowerCase().trim()));
    ...
    discoveredAliases = discoveredAliases.filter((a) => !ENTITY_BLOCKLIST.has(a.canonicalName.toLowerCase().trim()) && !ENTITY_BLOCKLIST.has(a.alias.toLowerCase().trim()));
    return {
...
function validateGradedHeuristics(args) {
    const rejectedEntities = Array.isArray(args?.rejected_heuristic_entities)
        ? args.rejected_heuristic_entities.filter((s) => typeof s === "string" && s.trim().length > 0).map((s) => s.trim())
        : [];
    const transformed = Array.isArray(args?.transformed_heuristic_entities)
        ? args.transformed_heuristic_entities
            ...
            .map((t) => ({ from: t.from.trim(), to: t.to.trim() }))
        : [];
    const rejectedRels = Array.isArray(args?.rejected_heuristic_relationships)
        ? args.rejected_heuristic_relationships
            ...
            .map((r) => ({ source: r.source.trim(), target: r.target.trim(), type: r.type.trim() }))
        : [];
    const rejectedExisting = Array.isArray(args?.rejected_existing_entities)
        ? args.rejected_existing_entities.filter((s) => typeof s === "string" && s.trim().length > 0).map((s) => s.trim())
        : [];
    return {
...
export function getExtractionStructuredParams(_provider, _batch) {
    return {};
}
// ─── Alias Resolution ─────────────────────────────────────────
...
function resolveAliasesInResult(result, knownEntities) {
    ...
    const resolve = (name) => aliasMap.get(name.toLowerCase()) || name;
    ...
    const relationships = result.relationshipsShown
        ...
        if (r.source.toLowerCase() === r.target.toLowerCase())
            return false;
...
function buildResultFromJsonObject(json, options) {
    const entities = validateEntities(json.entities_present).filter((e) => !ENTITY_BLOCKLIST.has(e.name.toLowerCase().trim()));
    const relationships = validateRelationships(json.relationships_shown).filter((r) => !ENTITY_BLOCKLIST.has(r.source.toLowerCase().trim()) &&
 ...
 */
export async function extractWithSidecar(content, generateRawFn, sidecarConnectionId, options) {
...
function buildAliasReconciliationBlock(aliases) {
...
function buildArbiterBlock(arbiter) {
    if (!arbiter)
        return "";
    const sections = [];
 ...
 */
export async function scoreChunkWithSidecar(content, generateRawFn, sidecarConnectionId) {
...
${EXTRACTION_RULES}`;
export async function extractBatchWithSidecar(chunks, generateRawFn, sidecarConnectionId, options) {
    if (chunks.length === 0)
        return [];
    ...
    const perChunkFallback = (idxs) => Promise.all(idxs.map((i) => extractWithSidecar(chunks[i].content, generateRawFn, sidecarConnectionId, {
        ...options,
        arbiter: singleChunkArbiterFor(i),
    }).catch(() => null)));
    const arbiterActiveByIndex = (options?.perChunkArbiter ?? []).map((a) => !!a && (a.heuristicEntities.length > 0 || a.heuristicRelationships.length > 0));
    ...
    try {
        ...
        const batchToolCall = response.tool_calls?.find((call) => !!call && call.name === TOOL_ANALYZE_PASSAGE_BATCH.name);
...
// ─── Diagnostic logging ─────────────────────────────────────────
function logSidecarDispatch(tag, opts) {
...
function logSidecarResponse(tag, opts) {
    const parts = [
...
function extractJsonArray(text) {
    try {
        ...
        if (objStart !== -1 && objEnd > objStart) {
            ...
            if (obj && typeof obj.importance === "number")
                return [obj];
        }
        return null;
    }
...
function validateEmotionalTags(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw.filter((t) => VALID_EMOTIONAL_TAGS.has(t));
}
function validateNarrativeFlags(raw) {
    if (!Array.isArray(raw))
        return [];
...
function validateKeyFacts(raw) {
    ...
    for (const fact of raw) {
        ...
        const hasVerb = words.some((w) => {
            ...
            if (FACT_COMMON_VERBS.has(lower))
                return true;
            if (/(?:ed|es|ing)$/.test(lower) && lower.length >= 4)
                return true;
            return false;
        });
        if (!hasVerb)
...
function validateStatusChanges(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw.filter((s) => s && typeof s.entity === "string" && typeof s.change === "string").map((s) => ({
...
function validateEntities(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw
        .filter((e) => e && typeof e.name === "string" && e.name.length > 0 && isValidEntityName(e.name))
        .map((e) => ({
        name: e.name.trim(),
...
function validateFontColors(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw
        .filter((c) => c &&
...
function validateDiscoveredAliases(raw) {
    if (!Array.isArray(raw))
        return [];
    return raw
        .filter((a) => a &&
