...
export function ttsConnectionSecretKey(id) {
    return `tts_connection_${id}_api_key`;
}
function mergeStoredVoices(providerId, profile, voices) {
...
export function listConnections(userId, pagination) {
    return paginatedQuery("SELECT * FROM tts_connections WHERE user_id = ? ORDER BY updated_at DESC", "SELECT COUNT(*) as count FROM tts_connections WHERE user_id = ?", [userId], pagination, rowToProfile);
}
...
export async function createConnection(userId, input) {
    ...
    getDb()
        .query(`INSERT INTO tts_connections
        (id, user_id, name, provider, api_url, model, voice, is_default, has_api_key, default_parameters, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(id, userId, input.name, input.provider, input.api_url || "", input.model || "", input.voice || "", input.is_default ? 1 : 0, hasApiKey, JSON.stringify(input.default_parameters || {}), JSON.stringify(input.metadata || {}), now, now);
...
export async function duplicateConnection(userId, id) {
    ...
    getDb()
        ...
        (id, user_id, name, provider, api_url, model, voice, is_default, has_api_key, default_parameters, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(newId, userId, `${existing.name} (Copy)`, existing.provider, existing.api_url, existing.model, existing.voice, 0, // never default
    hasApiKey, JSON.stringify(existing.default_parameters), JSON.stringify(existing.metadata), now, now);
...
export async function listConnectionVoices(userId, id) {
    const profile = getConnection(userId, id);
    if (!profile)
        return { voices: [], provider: "", error: "Connection not found" };
    const apiKey = await secretsSvc.getSecret(userId, ttsConnectionSecretKey(id));
...
export async function listConnectionVoicesPreview(userId, input) {
    const existing = input.connection_id ? getConnection(userId, input.connection_id) : null;
    const providerId = input.provider;
    let apiKey = input.api_key;
    if (apiKey === undefined && existing && existing.provider === providerId) {
