 ...
 */
export async function resolveRegexActionEffects(userId, chatId, messageId, scriptId, actionId, instanceId) {
    const chat = chatsSvc.getChat(userId, chatId);
    const source = chatsSvc.getMessage(userId, messageId);
    if (!chat || !source || source.chat_id !== chatId)
    ...
    if (!configuredScript || !configuredAction)
        return { status: "unavailable" };
    const configuredEffects = configuredAction?.effects ?? [];
    if (configuredEffects.length === 0)
        return { status: "resolved", effects: [] };
    if (source.is_user)
        return { status: "user_source" };
    if (configuredScript.disabled ||
    ...
    for (const match of rendered.result.matchAll(ACTION_PAYLOAD_RE)) {
        try {
            ...
            for (let index = 0; index < configuredEffects.length; index++) {
                ...
                const resolved = payload.effects[index];
                if (!resolved || resolved.type !== configured.type) {
                    valid = false;
                    break;
                }
                if (configured.type === "set_state") {
                    ...
                    if (resolved.mode !== configured.mode ||
