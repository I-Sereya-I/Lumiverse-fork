...
function parseRepoRef(input) {
    ...
    if (!raw)
        throw new ResolveError("invalid", "Paste a model URL or owner/model slug.");
    let owner;
    let repo;
    let revision = "main";
    if (/^https?:\/\//i.test(raw)) {
        ...
        if (!HF_HOSTS.has(parsed.hostname.toLowerCase())) {
            throw new ResolveError("invalid", `Only HuggingFace model URLs are supported here (got ${parsed.hostname}). For other hosts, add a direct tokenizer.json URL via Advanced.`);
        }
        ...
        throw new ResolveError("invalid", "Couldn't read a HuggingFace model from that. Paste a URL like https://huggingface.co/owner/model or an owner/model slug.");
    }
...
async function listRepoFiles(ref) {
    ...
    if (!resp.ok) {
        if (resp.status === 404)
            throw new ResolveError("unavailable", "Repository not found on HuggingFace.");
        if (resp.status === 401 || resp.status === 403) {
            ...
            throw new ResolveError("unavailable", usedToken
...
function detectTokenizer(files, ref) {
    const resolveUrl = (f) => `https://huggingface.co/${ref.owner}/${ref.repo}/resolve/${encodeURIComponent(ref.revision)}/${f}`;
    const has = (name) => files.includes(name);
    // Fast HuggingFace tokenizer — the common case (Qwen, GLM, Mistral, Gemma, …).
    if (has("tokenizer.json")) {
        const config = { url: resolveUrl("tokenizer.json") };
        if (has("tokenizer_config.json"))
            config.configUrl = resolveUrl("tokenizer_config.json");
        return {
            kind: "ok",
            type: "huggingface",
            config,
    ...
    if (tiktokenFile) {
        const config = { url: resolveUrl(tiktokenFile) };
        if (has("tokenizer_config.json"))
            config.configUrl = resolveUrl("tokenizer_config.json");
        return {
            kind: "ok",
    ...
    if (has("tokenizer.model")) {
        return {
            kind: "unsupported",
            message: "This repo ships only a SentencePiece `tokenizer.model` (no fast `tokenizer.json`). Our loader needs a `tokenizer.json` — check whether the model offers a 'fast' tokenizer variant.",
...
function suggestNameAndPattern(ref) {
...
export async function resolveTokenizer(input) {
    let ref;
    try {
    ...
    if (det.kind === "unavailable")
        return { ok: false, reason: "unavailable", message: det.message };
    if (det.kind === "unsupported")
        return { ok: false, reason: "unsupported", message: det.message, files: det.files };
