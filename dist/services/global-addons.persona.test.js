...
function insertGlobalAddon(o) {
    getDb().run(`INSERT INTO global_addons (id, user_id, label, content, sort_order, metadata, created_at, updated_at)
     VALUES (?, ?, '', ?, ?, '{}', 0, 0)`, [o.id, o.user_id, o.content, o.sort_order ?? 0]);
}
function makePersona(metadata) {
    return {
        id: "p1",
        name: "Alice",
...
describe("resolvePersonaGlobalAddons", () => {
    test("injects enabled attached global add-ons as _resolvedGlobalAddons", () => {
        ...
        expect(injected.map((a) => a.content)).toEqual(["Global one", "Global two"]);
    });
...
describe("{{persona}} macro includes resolved global add-ons", () => {
    test("renders base + local + global add-on content", () => {
        ...
        const persona = resolvePersonaGlobalAddons(USER, makePersona({
            addons: [{ id: "l1", label: "", content: "Local addon content", enabled: true, sort_order: 0 }],
            attached_global_addons: [{ id: "g1", enabled: true }],
        }));
        const env = buildEnv({
            character: { id: "c1", name: "Bob", description: "" },
            persona,
            chat: { id: "chat1", character_id: "c1", name: "", metadata: {}, created_at: 0, updated_at: 0 },
            messages: [],
            generationType: "normal",
        });
        expect(env.character.persona).toBe("Base persona description.\nLocal addon content\nGlobal addon content");
    });
});
...
function chatMetaWithStates(states) {
...
describe("resolvePersonaForChatMacros applies per-chat add-on bindings over defaults", () => {
    test("local add-on disabled by default but enabled in the chat binding shows up", () => {
        ...
        expect(personaText(resolvePersonaForChatMacros(USER, persona, null))).toBe("Base persona description.");
        expect(personaText(resolvePersonaForChatMacros(USER, persona, chatMetaWithStates({ l1: true })))).toBe("Base persona description.\nLocal addon");
    });
    test("local add-on enabled by default but disabled in the chat binding is hidden", () => {
        ...
        expect(personaText(resolvePersonaForChatMacros(USER, persona, null))).toBe("Base persona description.\nLocal addon");
        expect(personaText(resolvePersonaForChatMacros(USER, persona, chatMetaWithStates({ l1: false })))).toBe("Base persona description.");
    });
    test("global add-on attached-disabled by default but enabled in the chat binding shows up", () => {
        ...
        expect(personaText(resolvePersonaForChatMacros(USER, persona, null))).toBe("Base persona description.");
        expect(personaText(resolvePersonaForChatMacros(USER, persona, chatMetaWithStates({ g1: true })))).toBe("Base persona description.\nGlobal addon");
    });
    test("global add-on attached-enabled by default but disabled in the chat binding is hidden", () => {
        ...
        expect(personaText(resolvePersonaForChatMacros(USER, persona, null))).toBe("Base persona description.\nGlobal addon");
        expect(personaText(resolvePersonaForChatMacros(USER, persona, chatMetaWithStates({ g1: false })))).toBe("Base persona description.");
    });
});
