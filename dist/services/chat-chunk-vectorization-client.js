...
import { CHAT_CHUNK_VECTORIZATION_BATCH_TIMEOUT_MS, CHAT_CHUNK_VECTORIZATION_FORCE_KILL_GRACE_MS, CHAT_CHUNK_VECTORIZATION_WATCHDOG_GRACE_MS, } from "./chat-chunk-vectorization-timeouts";
...
let subprocessUnavailableReason = null;
let subprocess = null;
let ready = false;
let starting = null;
let resolveStarting = null;
let rejectStarting = null;
let inflight = null;
const queue = [];
let expectedExit = false;
let shutdownRequested = false;
function createStartupError(message, stack) {
...
export function isChatChunkVectorizationSubprocessStartupError(err) {
    return err instanceof Error && err.name === CHAT_CHUNK_VECTORIZATION_SUBPROCESS_STARTUP_ERROR_NAME;
}
function buildLaunchError(exitCode, signalCode, error, startup = false) {
...
function clearInflightTimeout(item) {
    if (item?.timeout) {
        clearTimeout(item.timeout);
        item.timeout = null;
    }
}
function terminateSubprocess(proc, { forceAfterMs, signal = "SIGTERM", } = {}) {
    if (!proc)
        return;
    try {
        proc.kill(signal);
    ...
    if (!forceAfterMs || signal === "SIGKILL")
        return;
    setTimeout(() => {
        if (subprocess !== proc)
            return;
...
function handleExit(exitCode, signalCode, error) {
...
function ensureSubprocess() {
    ...
    const launchTimeout = setTimeout(() => {
        const err = createStartupError(`Chat chunk vectorization subprocess did not become ready within ${READY_TIMEOUT_MS}ms`);
        if (subprocess) {
            expectedExit = true;
...
function dispatch(item) {
    ...
    item.timeout = setTimeout(() => {
        const err = new Error(`Chat chunk vectorization subprocess became unresponsive ${Math.floor(CHAT_CHUNK_VECTORIZATION_WATCHDOG_GRACE_MS / 1000)}s after the cooperative batch timeout`);
...
function pumpQueue() {
    ...
    void ensureSubprocess().then(() => {
        if (inflight || queue.length === 0 || !ready)
            return;
...
export function canUseChatChunkVectorizationSubprocess(platform = process.platform, env = process.env) {
    const explicit = env.LUMIVERSE_CHAT_VECTORIZATION_SUBPROCESS;
    if (explicit === "false")
        return false;
    if (platform === "win32" && explicit !== "true")
        return false;
    if (subprocessUnavailableReason)
        return false;
    return true;
}
export function warnChatChunkVectorizationFallback() {
    if (warnedDisabled || canUseChatChunkVectorizationSubprocess())
        return;
    warnedDisabled = true;
    if (process.env.LUMIVERSE_CHAT_VECTORIZATION_SUBPROCESS === "false") {
        console.warn("[vectorization] Chat chunk vectorization subprocess disabled via LUMIVERSE_CHAT_VECTORIZATION_SUBPROCESS=false; falling back to in-process execution.");
        return;
    }
    if (process.platform === "win32" && process.env.LUMIVERSE_CHAT_VECTORIZATION_SUBPROCESS !== "true") {
        console.warn("[vectorization] Chat chunk vectorization subprocess is disabled on Windows by default because this Bun.spawn IPC path has been unstable there. Set LUMIVERSE_CHAT_VECTORIZATION_SUBPROCESS=true to re-enable it.");
        return;
    }
    if (subprocessUnavailableReason) {
        console.warn(`[vectorization] Chat chunk vectorization subprocess disabled for this server process after bootstrap failure (${subprocessUnavailableReason}); falling back to in-process execution.`);
        return;
    }
    console.warn("[vectorization] Chat chunk vectorization subprocess unavailable; falling back to in-process execution.");
}
export function processChatChunkVectorizationBatchInSubprocess(tasks) {
...
export function shutdownChatChunkVectorizationSubprocess() {
