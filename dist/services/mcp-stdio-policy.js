...
const STDIO_ALLOWED_COMMANDS = new Set((process.env.MCP_STDIO_ALLOWED_COMMANDS ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
    .concat(DEFAULT_STDIO_ALLOWED));
const STDIO_ALLOWED_PACKAGES = new Set((process.env.MCP_STDIO_ALLOWED_PACKAGES ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean));
...
function assertStdioCommandAllowed(command) {
    ...
    if (!STDIO_ALLOWED_COMMANDS.has(base)) {
        throw new Error(`MCP stdio command "${base}" is not in the allowlist. ` +
            `Set MCP_STDIO_ALLOWED_COMMANDS to extend it.`);
    }
    return base;
}
function isInlineCodeArg(commandBase, arg) {
...
function packageArgForRunner(commandBase, args) {
    if (commandBase === "npx") {
        for (let i = 0; i < args.length; i++) {
            ...
            if (arg === "--")
                continue;
            if (arg === "--package" || arg === "-p" || arg.startsWith("--package="))
                return null;
            if (arg.startsWith("-"))
                return null;
            return arg;
...
function assertPackageRunnerAllowed(commandBase, args) {
    if (!PACKAGE_RUNNERS.has(commandBase))
        return;
    if (STDIO_ALLOWED_PACKAGES.size === 0) {
        throw new Error(`MCP stdio package runner "${commandBase}" requires MCP_STDIO_ALLOWED_PACKAGES to allow specific packages`);
    }
...
export function assertStdioLaunchAllowed(command, args = []) {
