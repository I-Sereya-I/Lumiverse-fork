...
export async function detectCharacterImportFormat(file) {
    ...
    if (/\.jpe?g$/i.test(nameLower) || file.type === "image/jpeg")
        return "jpeg";
    if (nameLower.endsWith(".json") || file.type === "application/json" || file.type === "text/json")
        return "json";
    return "unknown";
}
/**
 ...
 */
function extractPngTextChunk(buffer, keyword) {
 ...
 */
export function findJpegZipBoundary(data) {
 ...
 */
export function looksLikeJpegZipPolyglot(header) {
 ...
 */
export function decodeRisuModule(data) {
    if (data.length < 7)
        return null;
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    if (data[0] !== RPACK_MAGIC || data[1] !== RPACK_VERSION)
        return null;
 ...
 */
export function convertRisuRegexScripts(regexes, characterId) {
    const results = [];
...
function mapCardToInput(data) {
    ...
    if (Array.isArray(data.alternate_greetings))
        input.alternate_greetings = data.alternate_greetings;
    ...
    if (data.character_version !== undefined)
        extensions.character_version = data.character_version;
 ...
 * mapping.
 */
export function normalizeJannyCharacterInput(input) {
    return {
        ...input,
 ...
 */
export async function extractCardFromPng(file) {
 ...
 */
export function parseCardJson(json) {
...
function imageFileFromBytes(bytes, path) {
    const basename = path.split("/").pop() || "image.png";
    const ext = basename.split(".").pop()?.toLowerCase() || "png";
    return new File([new Uint8Array(bytes).buffer], basename, { type: MIME_MAP[ext] || "image/png" });
}
...
function isExpressionAsset(label) {
    const suffix = label.includes("_") ? label.slice(label.lastIndexOf("_") + 1) : label;
    if (NON_EXPRESSION_NAME_RE.test(suffix.trim()))
        return false;
    if (NON_EXPRESSION_NAME_RE.test(label.trim()))
        return false;
    return true;
}
/** Returns true if an expression label contains NSFW content keywords. */
export function isNsfwExpressionLabel(label) {
 ...
 */
export function analyzeExpressionGroups(labels) {
    const empty = { isMultiCharacter: false, groups: {} };
    if (labels.length < 10)
        return empty;
 ...
 */
export async function extractCardFromCharx(file) {
    const arrayBuf = await file.arrayBuffer();
    if (arrayBuf.byteLength > MAX_CHARX_SIZE) {
        throw new CharacterImportError("file_too_large", `CHARX file too large (${(arrayBuf.byteLength / 1024 / 1024).toFixed(1)} MB, max ${MAX_CHARX_SIZE / 1024 / 1024} MB)`);
    }
    ...
    let polyglotJpegAvatar = null;
    const zipBoundary = findJpegZipBoundary(data);
    if (zipBoundary > 0) {
        polyglotJpegAvatar = new File([data.slice(0, zipBoundary)], "avatar.jpg", { type: "image/jpeg" });
    ...
    const unzipped = unzipSync(data, {
        filter: (entry) => {
            const wanted = entry.name === "card.json" ||
            ...
            if (plannedBytes > MAX_CHARX_DECOMPRESSED_SIZE) {
                throw new CharacterImportError("archive_decompresses_too_large", `CHARX archive decompresses to more than ${MAX_CHARX_DECOMPRESSED_SIZE / 1024 / 1024} MB`);
            }
            return true;
        },
    });
    ...
    if (!cardBytes) {
        throw new CharacterImportError("invalid_archive", "CHARX archive does not contain card.json at the root");
    }
    let json;
    try {
        ...
        throw new CharacterImportError("invalid_card", "Failed to parse card.json from CHARX archive");
    }
    ...
    const imagePaths = Object.keys(unzipped).filter((p) => p !== "card.json" && IMAGE_EXTENSIONS.test(p));
    const avatarPath = imagePaths.find((p) => p.startsWith("assets/icon/image/")) ??
    ...
    const cardAssets = Array.isArray(rawData.assets) ? rawData.assets : [];
    const risuExprAssets = cardAssets.filter((a) => a.type === "x-risu-asset" && a.name && a.uri);
    if (risuExprAssets.length > 0) {
        for (const asset of risuExprAssets) {
            if (!isExpressionAsset(asset.name))
                continue;
            // Resolve embeded:// URI → ZIP path (e.g. "embeded://assets/other/image/X.webp" → "assets/other/image/X.webp")
            const zipPath = asset.uri.replace(/^embeded:\/\//, "");
...
export function fileStem(pathOrName) {
 ...
 */
export function resolveInlineAssetReferences(fields, assetImageMap) {
    if (assetImageMap.size === 0)
        return {};
    // Build tiered lookup tables — higher priority first
    const exactLookup = new Map(); // full path / embeded:// URI
    const baseLookup = new Map(); // basename (with extension)
    const stemLookup = new Map(); // stem (without extension)
    for (const [archivePath, imageId] of assetImageMap) {
        ...
        if (!baseLookup.has(basename))
            baseLookup.set(basename, apiUrl);
        const stem = fileStem(basename);
        if (!stemLookup.has(stem))
            stemLookup.set(stem, apiUrl);
    }
