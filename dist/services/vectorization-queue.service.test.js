...
describe("world-book vectorization queue supersession", () => {
    test("processes mutation replacements even when a stale job wrote indexed", () => {
        ...
        expect(__test__.shouldProcessWorldBookVectorizationJob(indexedRow, job({ supersedesIndexed: true }))).toBe(true);
        expect(__test__.shouldProcessWorldBookVectorizationJob({ ...indexedRow, vector_index_status: "pending" }, job())).toBe(true);
    });
