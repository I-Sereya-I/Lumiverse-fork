...
async function isToolGateEnabled(userId, tool) {
    if (!tool.gatedBy)
        return true;
    switch (tool.gatedBy) {
        case "webSearch": {
        ...
        case "imageGeneration": {
            const imageSettings = settingsSvc.getSetting(userId, "imageGeneration")?.value;
...
export function normalizeCouncilSettings(settings) {
...
export function getCouncilSettings(userId) {
    ...
    const legacySidecar = storedTools.sidecar;
    return normalizeCouncilSettings({
        ...COUNCIL_SETTINGS_DEFAULTS,
        ...stored,
        toolsSettings: {
...
export function putCouncilSettings(userId, partial) {
...
export async function getAvailableTools(userId) {
    ...
    const extensionTools = toolRegistry.getCouncilTools().map((reg) => ({
