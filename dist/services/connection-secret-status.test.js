...
describe("connection API-key status", () => {
    test("downgrades a persisted key flag when the credential is unreadable", async () => {
        ...
        await expect(withReadableApiKeyStatus("user-1", { id: "connection-1", has_api_key: true, name: "Primary" }, (id) => `connection_${id}_api_key`)).resolves.toEqual({ id: "connection-1", has_api_key: false, name: "Primary" });
    });
    ...
    test("preserves pagination metadata while reconciling every profile", async () => {
        spies.push(spyOn(secretsSvc, "getSecretForStatus").mockImplementation(async (_userId, key) => (key.includes("readable") ? "key" : null)));
        const result = await withReadableApiKeyStatuses("user-1", {
            data: [
