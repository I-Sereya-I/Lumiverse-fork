...
import { canUseChatChunkVectorizationSubprocess, isChatChunkVectorizationSubprocessStartupError, processChatChunkVectorizationBatchInSubprocess, shutdownChatChunkVectorizationSubprocess, warnChatChunkVectorizationFallback, } from "./chat-chunk-vectorization-client";
...
import { desiredWorldBookVectorIndexStatus, isWorldBookEntryVectorEligible, worldBookVectorSettingsFingerprint, } from "./world-book-vector-state";
...
function normalizeWorldBookVectorIndexStatus(row) {
...
function mergeVectorizationJobs(existing, incoming) {
...
class VectorizationQueue {
     ...
     */
    add(job) {
        const existing = this.queue.findIndex((j) => j.type === job.type &&
            j.userId === job.userId &&
            j.chatId === job.chatId &&
    ...
    async awaitLanceMaintenanceIdle() {
        ...
        try {
            while (this.queue.length > 0) {
                if (this.queue[0].type === "world_book_entry") {
                    ...
                    if (this.queue.length === 0 || this.queue[0].type !== "world_book_entry")
                        continue;
                    if (!worldBookJobsHaveSettled(this.queue, Date.now())) {
                        this.scheduleProcessing();
                        return;
    ...
    async processChunkBatch(jobs) {
        try {
            const tasks = jobs
                .filter((job) => typeof job.chunkId === "string" && job.chunkId.length > 0)
                .map((job) => ({
                userId: job.userId,
                chatId: job.chatId,
                chunkId: job.chunkId,
            ...
            if (canUseChatChunkVectorizationSubprocess()) {
                try {
                    ...
                    if (!isChatChunkVectorizationSubprocessStartupError(err))
                        throw err;
    ...
    async processWorldBookEntryBatch(jobs) {
        const entryIds = Array.from(new Set(jobs.map((job) => job.worldBookEntryId).filter((id) => !!id)));
      ...
      `)
            .all(jobs[0].userId, ...entryIds);
        const jobsByEntryId = new Map(jobs.map((job) => [job.worldBookEntryId, job]));
        ...
        try {
            ...
            if (configFingerprint) {
                await embeddingsSvc.markWorldBookEntriesVectorErrorIfCurrent(jobs[0].userId, entries.filter(isWorldBookEntryVectorEligible), errorMsg, settingsFingerprint, configFingerprint);
            }
            else {
...
export function queueChunkVectorization(userId, chatId, chunkId, priority = 5) {
    queue.add({
        type: "chunk",
        priority,
        userId,
...
export function queuePendingChatChunkVectorization(userId, chatId, priority = 4) {
    const rows = getDb().query(`SELECT id
     FROM chat_chunks
     WHERE chat_id = ? AND vectorized_at IS NULL
     ORDER BY updated_at ASC, created_at ASC`).all(chatId);
    for (const row of rows) {
        queueChunkVectorization(userId, chatId, row.id, priority);
    }
    return rows.length;
}
export async function queueStaleChatChunkVectorization(limit = CHAT_CHUNK_REQUEUE_LIMIT, priority = 2) {
    const rows = getDb().query(`SELECT cc.id, cc.chat_id, c.user_id
     FROM chat_chunks cc
...
export function queueWorldBookEntryVectorization(userId, entryId, priority = 4, supersedesIndexed = false) {
    queue.add({
...
function sweepWorldBookVectorizationQueue() {
    void (async () => {
        try {
            const users = getDb().query(`SELECT DISTINCT wb.user_id as user_id
         FROM world_book_entries e
         JOIN world_books wb ON wb.id = e.world_book_id
         WHERE e.vectorized = 1`).all();
            for (const { user_id: userId } of users) {
                const cfg = await embeddingsSvc.getEmbeddingConfig(userId);
                if (!cfg.enabled || !cfg.vectorize_world_books || !cfg.has_api_key)
                    continue;
                const rows = getDb().query(`SELECT e.id
           FROM world_book_entries e
           JOIN world_books wb ON wb.id = e.world_book_id
           WHERE wb.user_id = ?
             AND e.vectorized = 1
           ...
           e.updated_at ASC
           LIMIT ?`).all(userId, WORLD_BOOK_SWEEP_LIMIT_PER_USER);
                for (const row of rows) {
...
let _queryCacheCleanupTimer = null;
let _worldBookSweepTimer = null;
export function startVectorizationQueueMaintenance() {
...
export function stopQueryCacheCleanup() {
    if (_queryCacheCleanupTimer) {
        clearInterval(_queryCacheCleanupTimer);
        _queryCacheCleanupTimer = null;
    }
}
export function stopWorldBookVectorizationSweep() {
    if (_worldBookSweepTimer) {
        clearInterval(_worldBookSweepTimer);
        _worldBookSweepTimer = null;
    }
}
export function stopChatChunkVectorizationWorker() {
