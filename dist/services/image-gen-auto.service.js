...
export async function maybeAutoGenerateOnReply(userId, payload) {
    ...
    if (!settings.enabled || settings.autoGenerate === false)
        return false;
    if (!settings.activeImageGenConnectionId)
        return false;
    ...
    try {
        await generateSceneBackground(userId, chatId, attachToMessageId ? { attachToMessageId } : undefined);
        return true;
    }
    catch (err) {
...
export function initImageGenAutoListeners() {
    ...
    eventBus.on(EventType.GENERATION_ENDED, (event) => {
        if (!event.userId)
            return;
        void maybeAutoGenerateOnReply(event.userId, event.payload);
    });
