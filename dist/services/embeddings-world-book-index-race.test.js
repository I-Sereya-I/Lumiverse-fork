...
function readTrackedEntry() {
    const row = getDb().query("SELECT * FROM world_book_entries WHERE id = 'entry-1'").get();
    return makeEntry({
...
beforeEach(() => {
    ...
    updated_at, vector_index_status, vector_indexed_at, vector_index_error
        ...
        .run(entry.id, entry.world_book_id, JSON.stringify(entry.key), JSON.stringify(entry.keysecondary), entry.content, entry.comment, 1, 0, entry.updated_at, entry.vector_index_status, null, null);
});
...
describe("world-book vector commit races", () => {
    test("keeps the source when vector cleanup fails", async () => {
        ...
        await expect(__test__.coordinateWorldBookVectorAndSourceDelete("user", ["entry-1"], async () => { throw new Error("vector store unavailable"); }, () => {
    ...
    test("holds the entry gate through source deletion", async () => {
        const events = [];
        let releaseVectorDelete;
        let vectorDeleteStarted;
        const started = new Promise((resolve) => { vectorDeleteStarted = resolve; });
        const blocked = new Promise((resolve) => { releaseVectorDelete = resolve; });
        const deletion = __test__.coordinateWorldBookVectorAndSourceDelete("user", ["entry-1"], async () => {
    ...
    test("compare-and-set rejects an edit after final snapshot validation", async () => {
        ...
        const dependencies = {
            filterCurrent: async (_userId, entries) => {
                filterCalls += 1;
                if (filterCalls === 2) {
            ...
            deleteRows: async (_userId, entryIds) => {
                for (const entryId of entryIds)
                    vectors.delete(entryId);
            },
            upsertRows: async (rows) => {
                for (const row of rows)
                    vectors.set(row.source_id, [row]);
            },
            markIndexedIfCurrent: (userId, entries, indexedAt) => __test__.updateWorldBookEntriesVectorStateIfCurrent(userId, entries, "indexed", indexedAt, null),
        };
        const commit = await __test__.commitWorldBookVectorWritesIfCurrent("user", [{ entry: oldEntry, rows: [makeRow(oldEntry, [1, 0])] }], "settings", "config", 10, dependencies);
    ...
    test("discards an old commit, lets edit deletion finish, and indexes the replacement", async () => {
        ...
        const oldEntry = makeEntry();
        const oldCommit = await __test__.commitWorldBookVectorWritesIfCurrent("user", [{ entry: oldEntry, rows: [makeRow(oldEntry, [1, 0])] }], "settings", "config", 10, dependencies);
        if (queuedEditDelete)
            await queuedEditDelete;
        ...
        const replacementCommit = await __test__.commitWorldBookVectorWritesIfCurrent("user", [{ entry: replacementEntry, rows: [makeRow(replacementEntry, [0, 1])] }], "settings", "config", 20, dependencies);
