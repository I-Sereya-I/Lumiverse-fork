...
import { scrapeUrl } from "./databank";
import { getWebSearchApiKey, getWebSearchSettings } from "./web-search-settings.service";
...
function resolveSearchEndpoint(apiUrl) {
...
function normalizeSearxngResult(row) {
...
function normalizeExaResult(row) {
    if (!row.url || typeof row.url !== "string")
        return null;
    const title = typeof row.title === "string" && row.title.trim()
        ? row.title.trim()
    ...
    const highlight = Array.isArray(row.highlights)
        ? row.highlights.find((value) => typeof value === "string" && !!value.trim())
        : undefined;
...
function buildContextBlock(query, documents, settings) {
    const lines = [
        `## Web Search Context`,
        `Query: ${query}`,
        "",
    ...
    documents.forEach((doc, index) => {
        ...
        if (doc.snippet)
            lines.push(`Snippet: ${doc.snippet}`);
        if (doc.content) {
            lines.push("");
...
async function fetchSearxngResults(query, requestedCount, settings, apiKey) {
    if (!settings.apiUrl) {
...
async function fetchExaResults(query, requestedCount, settings, apiKey, includeContent) {
    if (!apiKey) {
...
async function fetchTavilyResults(query, requestedCount, settings, apiKey, includeContent) {
    if (!apiKey) {
...
export async function searchWeb(userId, query, requestedCount, options) {
...
export async function searchWebWithConfig(query, requestedCount, settings, apiKey, options) {
    ...
    const documents = await mapWithConcurrency(results.slice(0, scrapeCount), WEB_SEARCH_SCRAPE_CONCURRENCY, async (result) => {
        try {
