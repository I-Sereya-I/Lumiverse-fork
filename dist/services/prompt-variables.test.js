...
describe("coercePromptVariable — select", () => {
    ...
    test("invalid defaultValue + no override falls back to the first option", () => {
        const broken = { ...def, defaultValue: "ghost" };
...
describe("resolvePromptBlockPlacements", () => {
    ...
    test("uses a profile selection over the preset's shared placement selection", () => {
        const resolved = resolvePromptBlockPlacements([block], { metadata: { promptVariables: { "placement-block": { adherence_target: "baseline" } } } }, { "placement-block": { adherence_target: "frontier" } });
        expect(resolved[0]).toMatchObject({ role: "user", position: "in_history", depth: 3 });
    });
    test("inherits the preset placement when an active profile has no saved placement value", () => {
        const resolved = resolvePromptBlockPlacements([block], { metadata: { promptVariables: { "placement-block": { adherence_target: "frontier" } } } }, {});
        expect(resolved[0]).toMatchObject({ role: "user", position: "in_history", depth: 3 });
    });
});
// ---------------------------------------------------------------------------
...
describe("resolvePromptVariables", () => {
    ...
    test("keeps same-named prompt variables scoped to their defining block", async () => {
        ...
        const firstRendered = await withPromptBlockContext(env, first, () => ev("{{var::tone}}/{{.tone}}", env));
        const laterRendered = await withPromptBlockContext(env, later, () => ev("{{var::tone}}/{{.tone}}", env));
    ...
    test("allows block-local setvar writes without leaking them into another block", async () => {
        ...
        const rendered = await withPromptBlockContext(env, block, () => ev("{{setvar::tone::runtime}}{{var::tone}}/{{.tone}}", env));
