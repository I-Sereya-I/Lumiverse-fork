...
import { setMultiplayerPersonaProvider, setMultiplayerWorldInfoProvider, setMultiplayerMacroContextProvider, } from "./prompt-assembly.service";
import { mpidConfig } from "../multiplayer/config";
import { HARD_MAX_PEERS, DEFAULT_FREEFORM_WINDOW_SEC, MIN_FREEFORM_WINDOW_SEC, MAX_FREEFORM_WINDOW_SEC, MAX_DISPLAY_NAME_LEN, MAX_PERSONA_NAME_LEN, MAX_PERSONA_DESCRIPTION_LEN, MAX_ROOM_MESSAGE_BYTES, MAX_AVATAR_URL_LEN, MAX_AVATAR_DATA_URL_LEN, } from "../types/multiplayer";
...
function normalizeSettings(raw) {
    ...
    const freeformWindowSec = clampInt(obj.freeformWindowSec ?? DEFAULT_FREEFORM_WINDOW_SEC, MIN_FREEFORM_WINDOW_SEC, MAX_FREEFORM_WINDOW_SEC);
...
function rowToRoom(row) {
    return {
        ...
        status: row.status,
        turn_strategy: row.turn_strategy,
        freeform_deadline: row.freeform_deadline ?? null,
        turn_order: parseJson(row.turn_order, []),
...
function rowToParticipant(row) {
    return {
        ...
        persona_snapshot: parseJson(row.persona_snapshot, null),
        status: row.status,
...
function sanitizeDisplayName(raw, max = MAX_DISPLAY_NAME_LEN) {
...
function sanitizePersonaText(raw, max) {
    if (typeof raw !== "string")
        return "";
    return raw
        // eslint-disable-next-line no-control-regex
...
function sanitizeAvatarUrl(raw) {
    ...
    if (raw.length > MAX_AVATAR_URL_LEN)
        return null;
    // Same-origin host API path (Phase-1 local avatars).
    if (/^\/api\/v1\/[A-Za-z0-9/_.\-]+(\?[A-Za-z0-9=&_.\-]*)?$/.test(raw))
        return raw;
...
function sanitizeKeyArray(raw) {
    ...
    const out = [];
    for (const k of list) {
        if (out.length >= MAX_PEER_LOREBOOK_KEYS)
            break;
 ...
 */
function sanitizePeerLorebook(raw) {
    ...
    for (const e of list) {
        if (entries.length >= MAX_PEER_LOREBOOK_ENTRIES)
            break;
...
function sanitizePersonaSnapshot(raw) {
    if (!raw || typeof raw !== "object")
        return null;
    const obj = raw;
...
function invalidateChatRoute(chatId) {
...
export function getRoomByChatId(chatId) {
    const row = getDb()
...
export function listParticipants(roomId, opts) {
    ...
    return getDb().query(sql).all(roomId).map(rowToParticipant);
}
function isBanned(roomId, identityKind, identityRef) {
    const row = getDb()
...
// ─── views / payloads ───────────────────────────────────────────────────────────
function toParticipantView(p, currentTurnId) {
    return {
        id: p.id,
        role: p.role,
...
export function buildRoomStateView(room, opts) {
    const participants = listParticipants(room.id, { activeOnly: true }).map((p) => toParticipantView(p, room.current_turn_participant_id));
    const view = {
        roomId: room.id,
        chatId: room.chat_id,
        status: room.status,
    ...
    if (opts?.selfParticipantId)
        view.selfParticipantId = opts.selfParticipantId;
    if (opts?.hostView) {
...
function basePayload(room) {
    return { chatId: room.chat_id, roomId: room.id };
}
function emitTurnChanged(room, extra) {
...
// ─── room CRUD ──────────────────────────────────────────────────────────────────
export function createRoom(hostUserId, chatId, opts = {}) {
    ...
    const turnStrategy = opts.turnStrategy === "freeform" ? "freeform" : "round_robin";
    ...
    getDb().transaction(() => {
        getDb()
            .query(`INSERT INTO multiplayer_rooms
        ...
        getDb()
            .query(`INSERT INTO multiplayer_participants
 ...
 */
export function forkAndCreateRoom(hostUserId, sourceChatId, opts = {}) {
...
export function getRoomStateForHost(hostUserId, roomId) {
    const room = getRoom(roomId);
    if (!room || room.host_user_id !== hostUserId)
        return null;
    const hostParticipant = listParticipants(roomId).find((p) => p.identity_kind === "user" && p.identity_ref === hostUserId);
    return buildRoomStateView(room, { hostView: true, selfParticipantId: hostParticipant?.id });
}
export function updateRoom(hostUserId, roomId, patch) {
    const room = getRoom(roomId);
    if (!room || room.host_user_id !== hostUserId)
        return null;
    const now = Math.floor(Date.now() / 1000);
    ...
    getDb()
        .query("UPDATE multiplayer_rooms SET status = ?, turn_strategy = ?, settings = ?, updated_at = ? WHERE id = ?")
...
export function closeRoom(hostUserId, roomId) {
    const room = getRoom(roomId);
    if (!room || room.host_user_id !== hostUserId)
        return false;
...
// ─── join / leave ───────────────────────────────────────────────────────────────
function upsertParticipant(room, identityKind, identityRef, role, profile) {
    if (room.status === "closed")
        return { ok: false, reason: "closed" };
    if (isBanned(room.id, identityKind, identityRef))
        return { ok: false, reason: "banned" };
    const now = Math.floor(Date.now() / 1000);
    const displayName = sanitizeDisplayName(profile.displayName);
    const persona = sanitizePersonaSnapshot(profile.persona);
    const existing = getDb()
        .query("SELECT * FROM multiplayer_participants WHERE room_id = ? AND identity_kind = ? AND identity_ref = ?")
        .get(room.id, identityKind, identityRef);
    if (existing) {
        // Reconnect / re-join: re-activate the same row (keeps the turn slot).
        const wasActive = existing.status === "active";
        getDb()
            .query("UPDATE multiplayer_participants SET status = 'active', display_name = ?, persona_snapshot = ?, last_seen = ? WHERE id = ?")
            .run(displayName || existing.display_name, persona ? JSON.stringify(persona) : existing.persona_snapshot, now, existing.id);
        const participant = getParticipant(existing.id);
        if (!wasActive && role === "peer")
            ensureInTurnOrder(getRoom(room.id), participant.id);
    ...
    if (role === "peer") {
        const activePeers = getDb()
            ...
            .get(room.id);
        if (activePeers.c >= room.settings.maxPeers)
...
const roomCharacterAvatars = new Map();
/** Record the room's character avatar (host-only; sanitized like persona avatars). */
export function setRoomCharacterAvatar(roomId, rawUrl) {
    const clean = sanitizeAvatarUrl(rawUrl);
    if (clean)
        roomCharacterAvatars.set(roomId, clean);
}
...
const roomParticipantLorebooks = new Map();
 ...
 */
function buildInFlightGeneration(room) {
    ...
    if (entry.generationType === "impersonate")
        return null;
    const active = entry.status === "assembling" ||
    ...
    if (!active)
        return null;
    if (entry.content.length + entry.reasoning.length > MAX_HYDRATION_GEN_BYTES)
        return null;
    return {
        active: true,
...
export function buildHydrationPayload(room, selfParticipantId) {
    ...
    if (generation && payloadBytes() > budget)
        generation = null;
...
function getOrCreatePlaceholderCharacter(userId) {
    const row = getDb()
        .query("SELECT id FROM characters WHERE user_id = ? AND name = ? LIMIT 1")
        .get(userId, MP_PLACEHOLDER_CHARACTER_NAME);
    if (row)
        return row.id;
 ...
 */
export function ensureJoinedRoomChat(userId, input) {
    if (!input.chatId)
        return { ok: false };
    const db = getDb();
    const existing = db.query("SELECT user_id FROM chats WHERE id = ?").get(input.chatId);
    if (existing) {
        ...
        if (input.reconnectToken || input.server) {
            const prev = (chatsSvc.getChat(userId, input.chatId)?.metadata?.joined_room ?? {});
    ...
    if (input.server)
        joinedRoom.server = input.server;
    if (input.reconnectToken)
        joinedRoom.reconnect = input.reconnectToken;
    db.query("INSERT INTO chats (id, user_id, character_id, name, metadata, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)").run(input.chatId, userId, characterId, name, JSON.stringify({ multiplayer: true, joined_room: joinedRoom }), now, now);
    if (Array.isArray(input.messages) && input.messages.length > 0) {
        const mapped = input.messages.slice(0, 200).map((m) => ({
            is_user: !!m.is_user,
 ...
 */
export function getJoinedRoomReconnect(userId, chatId) {
    const jr = chatsSvc.getChat(userId, chatId)?.metadata?.joined_room;
    if (!jr || typeof jr.reconnect !== "string" || !jr.reconnect)
        return null;
    return {
...
export function leaveParticipant(roomId, participantId, opts) {
    const room = getRoom(roomId);
    if (!room)
        return;
...
export function handleDisconnect(roomId, participantId) {
    const participant = getParticipant(participantId);
...
// ─── persona relay ──────────────────────────────────────────────────────────────
export function updateParticipantPersona(roomId, participantId, rawPersona) {
    const room = getRoom(roomId);
    if (!room)
        return null;
    const participant = getParticipant(participantId);
    if (!participant || participant.room_id !== roomId || participant.status !== "active")
        return null;
 ...
 */
export function updateParticipantLorebook(roomId, participantId, rawLorebook) {
    const room = getRoom(roomId);
    if (!room)
        return;
    const participant = getParticipant(participantId);
    ...
    if (participant.role !== "peer")
        return; // the host's lorebook flows through normal assembly
 ...
 */
export function getActivePeerLorebookEntriesForChat(chatId) {
...
// ─── presence / typing ──────────────────────────────────────────────────────────
export function markTyping(roomId, participantId, typing) {
    const room = getRoom(roomId);
    if (!room)
        return;
    const participant = getParticipant(participantId);
...
export function touchParticipant(participantId) {
    getDb()
        .query("UPDATE multiplayer_participants SET last_seen = ? WHERE id = ?")
        .run(Math.floor(Date.now() / 1000), participantId);
}
// ─── turn engine ────────────────────────────────────────────────────────────────
function persistTurn(room) {
    getDb()
        .query("UPDATE multiplayer_rooms SET turn_order = ?, turn_index = ?, current_turn_participant_id = ?, round_counter = ?, freeform_deadline = ?, updated_at = ? WHERE id = ?")
        .run(JSON.stringify(room.turn_order), room.turn_index, room.current_turn_participant_id, room.round_counter, room.freeform_deadline, Math.floor(Date.now() / 1000), room.id);
}
/** Point the turn at `turn_order[index]`, normalizing the index into range. */
function setCurrentTurn(room, index) {
...
function ensureInTurnOrder(room, participantId) {
    if (room.turn_order.includes(participantId))
        return;
...
function removeFromTurnOrder(room, participantId) {
...
function advanceTurn(room) {
    if (room.turn_strategy !== "round_robin" || room.turn_order.length === 0)
        return;
    const next = room.turn_index + 1;
    if (next >= room.turn_order.length)
        room.round_counter += 1;
    setCurrentTurn(room, next);
}
// ── freeform timers (in-process; re-armed on boot) ──
const freeformTimers = new Map();
...
// error.
const freeformSubmitters = new Map();
function clearFreeformTimer(roomId) {
...
function armFreeformTimer(roomId, deadlineSec) {
    clearFreeformTimer(roomId);
    const ms = Math.max(0, deadlineSec * 1000 - Date.now());
    const timer = setTimeout(() => {
        freeformTimers.delete(roomId);
        runFreeformGeneration(roomId).catch((err) => console.error("[multiplayer] freeform generation failed:", err));
    }, ms);
...
export function openFreeformWindow(hostUserId, roomId) {
    const room = getRoom(roomId);
    if (!room || room.host_user_id !== hostUserId || room.status === "closed")
        return null;
    if (room.turn_strategy !== "freeform")
        return null;
...
export function endFreeformWindow(hostUserId, roomId) {
    const room = getRoom(roomId);
    if (!room || room.host_user_id !== hostUserId)
        return false;
    if (room.turn_strategy !== "freeform" || room.freeform_deadline === null)
        return false;
    clearFreeformTimer(roomId);
    runFreeformGeneration(roomId).catch((err) => console.error("[multiplayer] freeform generation failed:", err));
    return true;
}
const freeformGenerating = new Set();
async function runFreeformGeneration(roomId) {
    const room = getRoom(roomId);
    if (!room || room.status === "closed")
        return;
    if (freeformGenerating.has(roomId))
        return; // a concurrent trigger already won
    // Close the window.
 ...
 */
function recordFreeformSubmission(room, participantId) {
...
function checkFreeformComplete(roomId) {
    const room = getRoom(roomId);
    if (!room || room.turn_strategy !== "freeform" || room.freeform_deadline === null)
        return;
    if (freeformGenerating.has(roomId))
        return;
    ...
    if (active.length === 0 || !active.every((p) => set.has(p.id)))
        return;
    // Everyone present has had their say — don't wait out the deadline.
    clearFreeformTimer(roomId);
    void runFreeformGeneration(roomId).catch((err) => console.error("[multiplayer] early freeform generation failed:", err));
}
// ─── peer message submit ──────────────────────────────────────────────────────────
export function submitPeerMessage(roomId, participantId, rawContent, rawRegexAppend) {
    const room = getRoom(roomId);
    if (!room)
        return { ok: false, reason: "not_found" };
    if (room.status === "closed")
    ...
    if (isBanned(roomId, participant.identity_kind, participant.identity_ref))
        return { ok: false, reason: "kicked" };
    ...
    if (room.turn_strategy === "round_robin") {
        triggerHostGeneration(room).catch((err) => console.error("[multiplayer] generation failed:", err));
    }
    else {
...
function normalizePeerRegexAppend(raw) {
...
function writePeerMessage(room, participant, content, regexAppend = []) {
    const name = participantSpeakingName(participant);
    return chatsSvc.createMessage(room.chat_id, {
        is_user: true,
        name,
        content,
        extra: {
...
export function passTurn(roomId, participantId) {
    const room = getRoom(roomId);
    if (!room || room.turn_strategy !== "round_robin")
        return;
    if (room.current_turn_participant_id !== participantId)
        return;
    advanceTurn(room);
 ...
 */
export function resolveHostConnectionId(userId) {
...
async function triggerHostGeneration(room) {
    try {
...
function assertHostRoom(hostUserId, roomId) {
    const room = getRoom(roomId);
    if (!room || room.host_user_id !== hostUserId)
        return null;
    return room;
}
export function hostPromote(hostUserId, roomId, participantId) {
...
export function hostSkip(hostUserId, roomId, participantId) {
    const room = assertHostRoom(hostUserId, roomId);
    if (!room || room.turn_strategy !== "round_robin")
        return false;
    if (room.current_turn_participant_id !== participantId) {
...
export function hostKick(hostUserId, roomId, participantId) {
    const room = assertHostRoom(hostUserId, roomId);
    if (!room)
        return false;
    const participant = getParticipant(participantId);
    if (!participant || participant.room_id !== roomId || participant.role === "host")
        return false;
...
export function hostBan(hostUserId, roomId, participantId, reason) {
    const room = assertHostRoom(hostUserId, roomId);
    if (!room)
        return false;
    const participant = getParticipant(participantId);
    if (!participant || participant.room_id !== roomId || participant.role === "host")
        return false;
    getDb()
        .query(`INSERT OR IGNORE INTO multiplayer_bans (id, room_id, identity_kind, identity_ref, display_name, reason, banned_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`)
        .run(crypto.randomUUID(), roomId, participant.identity_kind, participant.identity_ref, participant.display_name, sanitizePersonaText(reason, 280), Math.floor(Date.now() / 1000));
    leaveParticipant(roomId, participantId, { kicked: true });
    eventBus.publishToRoom(roomId, EventType.ROOM_PARTICIPANT_KICKED, {
        ...basePayload(room),
        participantId,
...
const armedRooms = new Set();
function onUserMessageSent(chatId) {
    const roomId = getRoomIdForChat(chatId);
    if (!roomId)
        return;
    const room = getRoom(roomId);
    if (room?.turn_strategy === "round_robin")
        armedRooms.add(roomId);
}
function onGenerationEnded(chatId) {
    const roomId = getRoomIdForChat(chatId);
    if (!roomId)
        return;
    const room = getRoom(roomId);
...
export function getActivePeerPersonasForChat(chatId) {
    const roomId = getRoomIdForChat(chatId);
    if (!roomId)
        return [];
 ...
 */
function participantSpeakingName(p) {
 ...
 */
export function getMacroContextForChat(chatId) {
    const roomId = getRoomIdForChat(chatId);
    if (!roomId)
        return null;
    const room = getRoom(roomId);
...
export function initMultiplayer() {
    ...
    eventBus.on(EventType.MESSAGE_SENT, (msg) => {
        ...
        if (chatId && message?.is_user)
            onUserMessageSent(chatId);
    });
    eventBus.on(EventType.GENERATION_ENDED, (msg) => {
        const chatId = msg.payload?.chatId;
        if (chatId)
            onGenerationEnded(chatId);
    });
    ...
    try {
        const now = Math.floor(Date.now() / 1000);
        const rows = getDb()
            .query("SELECT id, freeform_deadline FROM multiplayer_rooms WHERE status != 'closed' AND freeform_deadline IS NOT NULL")
            .all();
