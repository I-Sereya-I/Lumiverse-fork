...
import { buildChatChunkEmbeddingSlices, collapseVectorHitsBySourceId, hashChatChunkContent, splitChatChunkContent, } from "./chat-chunk-embedding";
...
describe("buildChatChunkEmbeddingSlices", () => {
    test("stamps stable source metadata across split rows", () => {
        const content = Array.from({ length: 8 }, (_, i) => `[USER | Alice]: message ${i} ${"word ".repeat(120)}`).join("\n");
        const slices = buildChatChunkEmbeddingSlices(content, { chunkId: "chunk-1", messageIds: ["m1", "m2"] }, { forceSplit: true });
        expect(slices.length).toBeGreaterThan(1);
        expect(new Set(slices.map((slice) => slice.metadata.sourceContentHash))).toEqual(new Set([hashChatChunkContent(content)]));
        expect(new Set(slices.map((slice) => slice.metadata.splitCount))).toEqual(new Set([slices.length]));
        expect(slices.map((slice) => slice.metadata.splitIndex)).toEqual(slices.map((_, index) => index));
    });
});
