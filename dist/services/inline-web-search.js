...
function contentHasSlot(content) {
    return typeof content === "string"
        ? content.includes(INLINE_WEB_SEARCH_CONTEXT_SLOT_MARKER)
        : content.some((part) => part.type === "text" && part.text.includes(INLINE_WEB_SEARCH_CONTEXT_SLOT_MARKER));
}
function replaceSlot(content, replacement) {
...
export function stripInlineWebSearchContextSlot(text) {
 ...
 */
export function captureInlineWebSearchContextSlot(message) {
    if (!contentHasSlot(message.content))
        return false;
 ...
 */
export function applyInlineWebSearchContextSlots(messages, context) {
    let placed = false;
    const rendered = messages.map((message) => {
        const carrier = message;
        const template = carrier[INLINE_WEB_SEARCH_CONTEXT_TEMPLATE_KEY];
        if (!template || !contentHasSlot(template))
            return message;
        placed = true;
        const copy = { ...message };
 ...
 */
export function prepareInlineWebSearchMessagesForProvider(messages) {
    const prepared = [];
    for (const message of messages) {
        ...
        if (hasTemplate && !hasVisibleText)
            continue;
        prepared.push(copy);
    }
    return prepared;
}
export const INLINE_WEB_SEARCH_TOOL = {
 ...
 */
export function formatInlineWebSearchContext(context, maxChars = INLINE_WEB_SEARCH_MAX_CONTEXT_CHARS) {
