...
import { isLikelyVideoUpload, normalizeVideoBuffer, resetSilentVideoFfmpegProbe, resolveVideoInputExtension, stripAudioFromVideoBuffer, } from "./silent-video.service";
describe("silent-video.service", () => {
    ...
    test("skips normalization for non-video uploads", async () => {
        resetSilentVideoFfmpegProbe();
        const out = await normalizeVideoBuffer(Buffer.from("not-a-video"), "image/png", "image.png", { codec: "h264", stripAudio: true });
        expect(out).toBeNull();
    });
