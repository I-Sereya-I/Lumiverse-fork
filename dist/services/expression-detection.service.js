 ...
 */
export async function detectExpression(input, generateFn) {
    ...
    let model = input.modelOverride || sidecar.model || undefined;
...
function cleanDetectionResponse(raw) {
    return raw
        .trim()
        .replace(/^```(?:\w+)?\s*/i, "")
        .replace(/```$/i, "")
...
function mostSpecificLabel(labels) {
    return labels
        .slice()
        .sort((a, b) => b.length - a.length || a.localeCompare(b))[0] ?? null;
}
export function resolveDetectedExpressionLabel(rawResponse, labels) {
...
export function getExpressionDetectionSettings(userId) {
    const setting = settingsSvc.getSetting(userId, "expressionDetection");
    if (!setting)
        return { mode: "auto", contextWindow: 5 };
 ...
 */
export async function detectMultiCharacterExpression(input, generateFn) {
    ...
    if (!targetCharacter) {
        targetCharacter = await identifyCharacterLLM(userId, characterNames, recentMessages, generateFn, input.connectionId, input.modelOverride);
    }
    if (!targetCharacter || !groups[targetCharacter])
        return null;
    ...
    if (labels.length === 0)
        return null;
    const detected = await detectExpression({ ...input, labels }, generateFn);
    if (!detected || !groupLabels[detected])
        return null;
 ...
 */
function identifyCharacterHeuristic(recentMessages, characterNames) {
 ...
 */
async function identifyCharacterLLM(userId, characterNames, recentMessages, generateFn, connectionIdOverride, modelOverride) {
    const sidecar = getSidecarSettings(userId);
    let connectionId = connectionIdOverride || sidecar.connectionProfileId;
    let model = modelOverride || sidecar.model || undefined;
    if (!connectionId) {
        const defaultConn = connectionsSvc.resolveConnection(userId);
        if (!defaultConn)
            return null;
