 ...
 */
export function parseFrameHeader(buf, off) {
    if (off + 4 > buf.length)
        return null;
    const b1 = buf[off];
    const b2 = buf[off + 1];
    ...
    if (version === MPEG_RESERVED)
        return null;
    if (layer !== 0x01)
        return null; // 0b01 == Layer III; we only handle MP3.
...
function frameIsVbrHeader(buf, off, h) {
    ...
    if (vbriOff + 4 <= buf.length && buf.toString("latin1", vbriOff, vbriOff + 4) === "VBRI")
        return true;
    return false;
}
...
function indexOfFrameSync(buf, from) {
 ...
 */
function extractAudioFrames(raw) {
...
function walkFrames(buf) {
    ...
    while (off + 4 <= buf.length) {
        ...
        if (!h)
            return { count, clean: false, cbr };
        if (firstLen < 0)
            firstLen = h.frameLength;
        else if (h.frameLength !== firstLen)
            cbr = false;
        count++;
        off += h.frameLength;
    }
 ...
 */
function buildXingHeaderFrame(template, header4, frameCount, byteCount, cbr) {
 ...
 */
export function naiveConcatMp3(segments) {
    for (const seg of segments) {
        if (!isMp3Mime(seg.mime_type)) {
            throw new Error(`naive concat fallback only supports audio/mpeg segments; got ${seg.mime_type}. Install ffmpeg (Termux: pkg install ffmpeg) for multi-format support.`);
        }
    }
    ...
    if (firstSync < 0)
        return audio; // unparseable — ship the raw join, no worse than before
...
async function runFfmpeg(args) {
    ...
    if (!ffmpeg)
        return { ok: false, stderr: "ffmpeg unavailable" };
...
async function muxWithFfmpeg(segments) {
    ...
    try {
        ...
        writeFileSync(listPath, segmentPaths.map((p) => `file '${quoteConcatPath(p)}'`).join("\n") + "\n");
 ...
 */
export async function muxSegments(segments) {
