...
function findExistingCompletionFile(userDir) {
    if (!existsSync(userDir))
        return null;
    for (const entry of readdirSync(userDir)) {
        if (entry.startsWith(`${COMPLETION_BASENAME}.`))
            return join(userDir, entry);
    }
    return null;
}
...
export async function setCompletionSound(userId, file) {
    if (!file)
        throw new Error("Audio file is required");
    if (file.size > MAX_NOTIFICATION_SOUND_BYTES) {
        throw new Error(`Audio file too large (max ${MAX_NOTIFICATION_SOUND_BYTES / (1024 * 1024)}MB)`);
    }
    const arrayBuffer = await file.arrayBuffer();
    const bytes = new Uint8Array(arrayBuffer);
    const detected = detectAudioFormat(bytes);
    if (!detected) {
        throw new Error("Unsupported or invalid audio file. Allowed formats: MP3, WAV, OGG, AAC, M4A.");
    }
...
export function getCompletionSound(userId) {
    const root = resolve(getRootDir());
    const userDir = resolve(root, userId);
    if (!(userDir === root || userDir.startsWith(root + sep)))
        return null;
    if (!existsSync(userDir))
        return null;
...
export function deleteCompletionSound(userId) {
    const root = resolve(getRootDir());
    const userDir = resolve(root, userId);
    if (!(userDir === root || userDir.startsWith(root + sep)))
        return false;
