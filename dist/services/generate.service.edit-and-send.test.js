...
describe("startGeneration edit-and-send", () => {
    ...
    beforeEach(() => {
        ...
        track(spyOn(chatsSvc, "getLastMessage").mockImplementation(() => assistant.swipes.length > 1 ? assistant : baseMessage()));
        ...
        track(spyOn(chatsSvc, "getMessage").mockImplementation((_userId, messageId) => {
            if (messageId === ASSISTANT_ID)
                return assistant;
            if (messageId === USER_MSG_ID)
                return baseMessage();
            return null;
        }));
