...
import { deleteWorldBookFolder, getWorldBook, renameWorldBookFolder, } from "./world-books.service";
function initDb() {
    closeDatabase();
    initDatabase(":memory:");
...
function insertBook(id, userId, folder, updatedAt = 1) {
    getDb().run("INSERT INTO world_books (id, name, description, folder, metadata, created_at, updated_at, user_id) VALUES (?, ?, '', ?, '{}', 1, ?, ?)", [id, id, folder, updatedAt, userId]);
}
beforeEach(initDb);
