...
function sanitizeEntryBase(name, fallback) {
    return name.replace(/[\/\\:*?"<>|\x00-\x1F\x7F]/g, "").trim() || fallback;
}
function uniqueEntryName(base, used) {
    let name = `${base}.json`;
    let suffix = 2;
    while (used.has(name)) {
...
function splitMetadata(metadata) {
    ...
    for (const [key, value] of Object.entries(metadata)) {
        if (key.startsWith("_lumiverse_")) {
            if (key !== "_lumiverse_preset_version")
                lumihubMeta[key] = value;
            continue;
        }
        if (!OWNED_METADATA_KEYS.has(key))
            passthroughMetadata[key] = value;
    }
    return {
...
function exactSealedPlaceholder(content) {
    if (typeof content !== "string")
        return null;
    return content.trim().match(/^\{\{(?:presetBlock|pblock)::([^}]+)\}\}$/)?.[1]?.trim() || null;
}
function sanitizeSealedBlocks(blocks, lumihubMeta) {
...
export function buildPortablePresetExport(userId, preset) {
...
export function preparePresetBulkExport(userId, ids) {
    ...
    for (const [id, entry] of preparedExports) {
        if (now - entry.createdAt > PREPARE_TTL_MS)
...
export function consumePreparedPresetExport(userId, downloadId) {
    const entry = preparedExports.get(downloadId);
    if (!entry || entry.userId !== userId || Date.now() - entry.createdAt > PREPARE_TTL_MS)
        return null;
    preparedExports.delete(downloadId);
    return entry;
}
function controllerSink(controller) {
    return new Writable({
...
export function buildPresetBulkExportStream(userId, presetIds, signal) {
    let archive = null;
            ...
            void (async () => {
                ...
                archive.on("warning", (error) => {
                    if (error.code !== "ENOENT")
                        console.warn(`[preset export] ${error.message}`);
                });
                ...
                for (const id of dedupePresetIds(presetIds)) {
                    ...
                    if (!preset || preset.provider !== "loom")
                        continue;
