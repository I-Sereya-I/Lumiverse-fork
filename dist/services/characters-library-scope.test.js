...
import { InvalidCharacterLibraryScopeError, bulkUpdateCharacterFolders, bulkUpdateCharacterTags, createCharacter, deleteCharacter, deleteCharacterFolder, duplicateCharacter, findCharacterBySourceFilename, getCharacter, getCharacterPreview, getCharactersByIds, listCharacterSummaries, listCharacterTags, listCharacters, normalizeCharacterLibraryScope, renameCharacterFolder, setCharacterSourceFilename, updateCharacter, } from "./characters.service";
...
function summaryIds(userId, scope, options = {}) {
    return listCharacterSummaries(userId, { limit: 100, offset: 0 }, { scope, ...options }).data.map((character) => character.id);
}
function discoverIds(userId, scope) {
    return listCharacterSummaries(userId, { limit: 100, offset: 0 }, { scope, sort: "discover", seed: 37 }).data.map((character) => character.id);
}
function insertChat(id, userId, characterId, name, updatedAt, metadata = "{}") {
    getDb()
...
function insertMessage(id, chatId, index, content) {
...
describe("character library scope", () => {
    test("creates, reads, lists, and summarizes mine and shared characters without crossing tenants", () => {
        ...
        expect(listCharacters(USER_B, { limit: 100, offset: 0 }).data.map((character) => character.id).sort()).toEqual([foreignMine.id, foreignShared.id].sort());
    ...
    test("rejects malformed or conflicting scope values before persisting", () => {
        ...
        for (const value of [null, "", "MINE", "public", 1, {}, []]) {
            ...
            expect(() => createCharacter(USER_A, { name: `bad-${String(value)}`, library_scope: value })).toThrow(InvalidCharacterLibraryScopeError);
            expect(() => createCharacter(USER_A, { name: `bad-mirror-${String(value)}`, extensions: { _lumiverse_library_scope: value } })).toThrow(InvalidCharacterLibraryScopeError);
        }
        expect(() => createCharacter(USER_A, {
            name: "conflict",
            library_scope: "mine",
            extensions: { _lumiverse_library_scope: "shared" },
        })).toThrow(InvalidCharacterLibraryScopeError);
        expect(() => updateCharacter(USER_A, existing.id, { library_scope: "elsewhere" })).toThrow(InvalidCharacterLibraryScopeError);
        expect(() => updateCharacter(USER_A, existing.id, { extensions: { _lumiverse_library_scope: "elsewhere" } })).toThrow(InvalidCharacterLibraryScopeError);
        expect(() => updateCharacter(USER_A, existing.id, {
            library_scope: "mine",
            extensions: { _lumiverse_library_scope: "shared" },
        })).toThrow(InvalidCharacterLibraryScopeError);
        expect(() => listCharacterSummaries(USER_A, { limit: 10, offset: 0 }, { scope: "all" })).toThrow(InvalidCharacterLibraryScopeError);
    ...
    test("filters summaries by a tenant-owned chat and its direct or group participants", () => {
        ...
        insertChat("chat-participants", USER_A, direct.id, "Participants", 20, JSON.stringify({ character_ids: [participant.id, sharedParticipant.id] }));
