...
import { spawnAsync } from "../spindle/spawn-async";
...
let alertReemitTimer = null;
let installationPromise = null;
let lastSmartWarningFingerprint = null;
function currentPlatform(deps) {
    return deps.platform ?? getPlatform();
}
function currentEnv(deps) {
    return deps.env ?? process.env;
}
function runner(deps) {
    return deps.run ?? spawnAsync;
}
function isTermux(deps) {
...
export function getSmartctlCandidates(deps = {}) {
...
async function probeBinary(candidate, deps) {
    try {
...
async function resolveSmartctlBinaryUncached(deps) {
...
export function resetSmartctlResolution() {
    resolvedBinary = undefined;
    resolvingBinary = null;
}
async function hasCommand(command, deps) {
    try {
 ...
 */
export async function getSmartctlInstallPlan(deps = {}) {
    if (isTermux(deps))
        return null;
    const platform = currentPlatform(deps);
    ...
    for (const candidate of candidates) {
        if (await hasCommand(candidate.manager, deps))
            return candidate;
    }
    return null;
}
function isCurrentProcessElevated(platform) {
    if (platform === "win32")
        return false; // Avoid guessing UAC state from a web backend.
    return typeof process.getuid === "function" && process.getuid() === 0;
}
function manualInstallMessage(platform, termux) {
    if (termux)
        return "SMART monitoring is unavailable on Termux by default because Android normally blocks raw disk access.";
    if (platform === "win32")
        return "Install Chocolatey, then install smartmontools, or use the official Windows installer.";
    if (platform === "darwin")
        return "Install Homebrew, then run: brew install smartmontools";
    if (platform === "linux")
        return "Install the smartmontools package with this system's package manager.";
    return "Install smartmontools from your operating system's package repository.";
}
export async function getSmartctlStatus(deps = {}) {
 ...
 */
export async function installSmartctl() {
    if (installationPromise)
        return installationPromise;
    installationPromise = (async () => {
        const before = await getSmartctlStatus();
        if (before.binary)
            return { installed: false, status: before, error: null };
        if (!before.installPlan)
            return { installed: false, status: before, error: before.message };
...
function ataAttributeEntry(report, id, names = []) {
    const table = report.ata_smart_attributes?.table;
    if (!Array.isArray(table))
        return null;
    return table.find((attribute) => attribute?.id === id)
        ?? table.find((attribute) => names.includes(attribute?.name));
}
function ataAttribute(report, id, names = []) {
...
function ataAttributeByName(report, pattern) {
    const table = report.ata_smart_attributes?.table;
    if (!Array.isArray(table))
        return null;
    return table.find((attribute) => typeof attribute?.name === "string" && pattern.test(attribute.name)) ?? null;
}
function boundedPercent(value) {
    return value != null && value >= 0 && value <= 100 ? value : null;
}
function dataUnitsToBytes(value, unitBytes) {
    if (value == null || value < 0)
        return null;
    const bytes = value * unitBytes;
    return Number.isSafeInteger(bytes) ? bytes : null;
}
function driveKind(report, nvme) {
    if (nvme || /^nvme$/i.test(stringOrNull(report?.device?.protocol) ?? ""))
        return "ssd";
    const rotationRate = numberOrNull(report?.rotation_rate);
    if (rotationRate === 0)
        return "ssd";
    if (rotationRate != null && rotationRate > 0)
        return "hdd";
    ...
    const ssdAttribute = ataAttributeByName(report ?? {}, /(?:nand|wear[_ -]?level|lifetime|life[_ -]?(?:left|remain)|media[_ -]?wear|reserve(?:d)?[_ -]?(?:blk|block)|program[_ -]?fail|erase[_ -]?fail)/i);
    return ssdAttribute ? "ssd" : "unknown";
}
function ataLifetimeUsed(report) {
...
function ataLifetimeRemaining(report) {
    const entry = ataAttributeByName(report, /(?:ssd[_ -]?life[_ -]?left|(?:percent|percentage)[_ -]?(?:lifetime|life)[_ -]?(?:remain|left)|media[_ -]?wearout[_ -]?indicator|remaining[_ -]?(?:lifetime|life))/i);
...
function createSsdMetrics(report, nvme) {
...
function createHddMetrics(report) {
    return {
...
function ataAttributeName(attribute) {
 ...
 */
function collectSmartAlertConditions(report, exitStatus, criticalWarning, ssd) {
    const conditions = [];
    const seen = new Set();
    ...
    if (Array.isArray(table)) {
        for (const attribute of table) {
            ...
            if (!type?.includes("pre") || !type.includes("fail") || !whenFailed)
                continue;
    ...
    if (criticalWarning != null && criticalWarning !== 0) {
        if ((criticalWarning & 0b00001) !== 0)
            add("warning", "NVMe available spare capacity is below its threshold.");
        if ((criticalWarning & 0b00010) !== 0)
            add("warning", "NVMe temperature is outside its safe operating range.");
        if ((criticalWarning & 0b00100) !== 0)
            add("failing", "NVMe reliability is degraded.");
        if ((criticalWarning & 0b01000) !== 0)
            add("failing", "NVMe drive is in read-only mode.");
        if ((criticalWarning & 0b10000) !== 0)
            add("warning", "NVMe volatile-memory backup failed.");
        if ((criticalWarning & ~0b11111) !== 0)
            add("warning", "NVMe reports an unknown critical-warning condition.");
    }
    if (ssd) {
...
export function getSystemSmartAlertPayload(snapshot) {
    const drives = snapshot.drives.flatMap((drive) => {
        if ((drive.status !== "warning" && drive.status !== "failing") || drive.alertConditions.length === 0)
            return [];
        const hasFailingCondition = drive.alertConditions.some((condition) => condition.severity === "failing");
        return [{
                device: drive.device,
                model: drive.model,
                status: drive.status === "failing" || hasFailingCondition ? "failing" : "warning",
...
export function summarizeSmartctlReport(report, device, exitStatus, stderr = "") {
    ...
    const ssdWarning = ssd != null && ((ssd.percentageUsed != null && ssd.percentageUsed >= 100)
...
function scanDeviceType(device) {
...
async function readSmartctlJson(run, command) {
...
function diagnosticText(result) {
    ...
    if (!detail)
        return "smartctl could not read SMART data from this device.";
 ...
 */
async function readDriveReport(binary, device, type, run) {
    ...
    if (read.report || isStandbyResult(read))
        return read;
...
async function collectSmartctlSnapshot(deps) {
    const binary = await resolveSmartctlBinary(deps);
    ...
    for (const found of devices) {
        ...
        drives.push(summarizeSmartctlReport(read.report, device, exitStatus, read.report ? read.result.stderr : diagnosticText(read.result)));
    }
    return { checkedAt, drives, error: null };
}
export async function getSmartctlSnapshot(options = {}, deps = {}) {
    const now = (deps.now ?? Date.now)();
    if (!options.force && latestSnapshot) {
        const checkedAt = Date.parse(latestSnapshot.checkedAt);
        if (Number.isFinite(checkedAt) && now - checkedAt < SNAPSHOT_TTL_MS)
...
export function resetSmartctlSnapshot() {
    latestSnapshot = null;
    snapshotPromise = null;
}
async function runScheduledSmartctlCheck() {
    const status = await getSmartctlStatus();
    if (!status.binary)
        return;
...
function hasFailingSmartAlert(payload) {
    return payload.drives.some((drive) => (drive.status === "failing"
        || drive.conditions.some((condition) => condition.severity === "failing")));
}
/** A stable warning identity that excludes the check timestamp. */
function getSmartWarningFingerprint(payload) {
 ...
 */
function emitSmartctlAlert(snapshot) {
    ...
    if (hasFailingSmartAlert(payload)) {
        ...
        if (fingerprint === lastSmartWarningFingerprint)
            return;
...
export function startSmartctlMonitor() {
    if (monitorTimer || process.env.LUMIVERSE_SMART_MONITOR === "false")
        return;
    ...
    alertReemitTimer = setInterval(() => {
        if (latestSnapshot)
            emitSmartctlAlert(latestSnapshot);
    }, ALERT_REEMIT_INTERVAL_MS);
    if (typeof monitorTimer.unref === "function")
        monitorTimer.unref();
    if (typeof alertReemitTimer.unref === "function")
        alertReemitTimer.unref();
}
export function stopSmartctlMonitor() {
    if (monitorTimer)
        clearInterval(monitorTimer);
    if (alertReemitTimer)
        clearInterval(alertReemitTimer);
    monitorTimer = null;
    alertReemitTimer = null;
}
/** Optional unattended provisioning for Docker/root deployments. */
export function initSmartctl() {
    if (process.env.LUMIVERSE_SMARTCTL_AUTO_INSTALL === "true") {
        void installSmartctl().then((result) => {
            if (result.installed)
                console.info("[smartctl] Installed smartmontools automatically.");
            else if (result.error && result.status.availability !== "available")
                console.info(`[smartctl] Automatic installation skipped: ${result.error}`);
