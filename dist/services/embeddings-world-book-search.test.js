...
function hit(sourceId, similarity, id = `${sourceId}-${similarity}`) {
    return {
        id,
        source_id: sourceId,
        content: id,
...
describe("world-book unique-source search", () => {
    test("continues past monopolizing chunks until the unique-entry target is met", async () => {
        ...
        const rows = await __test__.collectWorldBookHitsByUniqueSource(eq("owner_id", "book"), 3, async (filter) => {
    ...
    test("stops when a provider ignores source exclusions", async () => {
        let calls = 0;
        const rows = await __test__.collectWorldBookHitsByUniqueSource(eq("owner_id", "book"), 3, async () => {
            calls += 1;
    ...
    test("chunks large exclusion sets at the provider filter limit", () => {
        const filters = __test__.worldBookSourceExclusionFilters(new Set(Array.from({ length: 501 }, (_, index) => `entry-${index}`)));
