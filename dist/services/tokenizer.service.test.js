...
import { _getCachedTokenizerIdsForTests, _resetForTests, countBreakdown, countWithTokenizer, prewarm, releaseTokenizerMemory, } from "./tokenizer.service";
function createTokenizerTables() {
...
function insertApproximateTokenizer(id) {
    getDb()
        .query("INSERT INTO tokenizer_configs (id, name, type, config, is_built_in, created_at, updated_at) VALUES (?, ?, ?, ?, 0, 0, 0)")
        .run(id, id, "approximate", JSON.stringify({ charsPerToken: 4 }));
}
function insertPattern(id, tokenizerId, pattern, priority) {
    getDb()
        .query("INSERT INTO tokenizer_model_patterns (id, tokenizer_id, pattern, priority, is_built_in, created_at, updated_at) VALUES (?, ?, ?, ?, 0, 0, 0)")
        .run(id, tokenizerId, pattern, priority);
}
function insertConnection(input) {
    getDb()
        .query("INSERT INTO connection_profiles (id, user_id, model, is_default, updated_at) VALUES (?, ?, ?, ?, ?)")
        .run(input.id, input.userId, input.model, input.isDefault ? 1 : 0, input.updatedAt);
}
function putSetting(userId, key, value, updatedAt) {
    getDb()
