...
async function validateTokenizerUrl(url, label) {
    let parsed;
    try {
        parsed = new URL(url);
    }
...
const pendingInstanceLoads = new Map();
...
function countCached(tokenizerId, instance, text) {
...
// ---- Helpers ----
const BENIGN_TOKENIZER_CLASS_WARNING = 'Unknown tokenizer class "TokenizersBackend", attempting to construct from base class.';
function isBenignTokenizerWarning(args) {
    return args.length > 0 && String(args[0]) === BENIGN_TOKENIZER_CLASS_WARNING;
}
function withoutBenignTokenizerWarning(fn) {
...
function getConfig(id) {
    const db = getDb();
    const row = db.query("SELECT * FROM tokenizer_configs WHERE id = ?").get(id);
...
function getAllPatterns() {
    ...
    return rows.map((r) => ({ ...r, is_built_in: !!r.is_built_in }));
}
// ---- Pattern matching ----
...
function getTokenizerIdForModel(modelId) {
...
async function loadOpenAI(config) {
    ...
    if (!encode)
        throw new Error(`Could not find encode function for ${encoding}`);
    return { count: (text) => encode(text).length };
}
async function loadHuggingFace(config) {
    ...
    if (cfg.url) {
        ...
        if (configUrl === cfg.url) {
            ...
            if (!resp.ok)
                throw new Error(`Failed to fetch tokenizer.json from ${cfg.url}: ${resp.status}`);
        ...
        if (!tokenizerResp.ok)
            throw new Error(`Failed to fetch tokenizer.json from ${cfg.url}: ${tokenizerResp.status}`);
        ...
        if (!configResp.ok)
            throw new Error(`Failed to fetch tokenizer_config.json from ${configUrl}: ${configResp.status}`);
        const tokenizerJSON = await tokenizerResp.json();
        const tokenizerConfig = await configResp.json();
        const tokenizer = withoutBenignTokenizerWarning(() => TokenizerLoader.fromPreTrained({ tokenizerJSON, tokenizerConfig }));
        return { count: (text) => tokenizer.encode(text).length };
    }
 ...
 */
function looksLikeStandardTiktokenFormat(bpe) {
    const firstLineEnd = bpe.indexOf("\n");
    if (firstLineEnd < 0)
        return false; // single line — already compressed
 ...
 */
function convertStandardToCompressedBpe(standard) {
...
async function loadTiktoken(config) {
    const { Tiktoken } = await import("js-tiktoken/lite");
    const cfg = config.config;
    if (!cfg.url)
        throw new Error("Tiktoken requires 'url' in config pointing to .model file");
    await validateTokenizerUrl(cfg.url, "tiktoken model url");
    const resp = await fetch(cfg.url, { signal: AbortSignal.timeout(TOKENIZER_FETCH_TIMEOUT_MS), headers: await hfAuthHeaders(cfg.url) });
    if (!resp.ok)
        throw new Error(`Failed to fetch tiktoken model from ${cfg.url}`);
            ...
            if (configResp.ok) {
                ...
                if (configData.added_tokens_decoder) {
                    for (const [id, tok] of Object.entries(configData.added_tokens_decoder)) {
                        if (tok.special) {
                            specialTokens[tok.content] = parseInt(id, 10);
                        }
                    }
                }
            }
    ...
    return { count: (text) => enc.encode(text).length };
}
function loadApproximate(config) {
    const charsPerToken = config.config.charsPerToken || 4;
    return { count: (text) => Math.ceil(text.length / charsPerToken) };
}
// ---- Instance management ----
function touchInstance(tokenizerId, instance) {
...
async function getInstance(tokenizerId) {
...
// ---- Public API ----
export async function countForModel(modelId, text) {
...
export async function countWithTokenizer(tokenizerId, text) {
 ...
 */
export async function verifyConfig(type, config) {
    const synthetic = {
        id: "__verify__",
        name: "__verify__",
        type,
...
export function flattenMessagesForTokenCount(messages) {
    return messages.map((msg) => `${msg.role}\n${msg.content || ""}`).join("\n");
}
export async function countMessagesForModel(modelId, messages) {
    return await countForModel(modelId, flattenMessagesForTokenCount(messages));
}
export async function countBreakdown(modelId, breakdown, chatHistoryMessages) {
    ...
    for (const entry of breakdown) {
        ...
        if (entry.preCountedTokens != null) {
            ...
            const bulk = flattenMessagesForTokenCount(chatHistoryMessages.map((msg) => ({ role: msg.role, content: getTextContent(msg) })));
 ...
 */
export async function resolveCounter(modelId) {
    const tokenizerId = modelId ? getTokenizerIdForModel(modelId) : null;
    if (tokenizerId) {
        const config = getConfig(tokenizerId);
        try {
...
export function invalidate(tokenizerId) {
    instanceCache.delete(tokenizerId);
    pendingInstanceLoads.delete(tokenizerId);
...
export function invalidatePatterns() {
    patternCache = null;
}
/**
...
function parseSettingString(value) {
    try {
...
function collectPrewarmTokenizerIds() {
    const db = getDb();
    const connections = db.query("SELECT id, user_id, model, is_default, updated_at FROM connection_profiles WHERE model IS NOT NULL AND model != ''").all();
    if (connections.length === 0)
        return [];
    const activeSettings = db.query("SELECT user_id, value, updated_at FROM settings WHERE key = ?").all("activeProfileId");
    ...
    for (const [key, userConnections] of connectionsByUser) {
        userConnections.sort((a, b) => Number(b.updated_at ?? 0) - Number(a.updated_at ?? 0) || a.id.localeCompare(b.id));
    ...
    candidates.sort((a, b) => a.priority - b.priority || b.updatedAt - a.updatedAt || a.connectionId.localeCompare(b.connectionId));
    const tokenizerIds = [];
    const seen = new Set();
    for (const candidate of candidates) {
        const tokenizerId = getTokenizerIdForModel(candidate.model);
        if (!tokenizerId || seen.has(tokenizerId))
            continue;
        seen.add(tokenizerId);
        tokenizerIds.push(tokenizerId);
        if (tokenizerIds.length >= MAX_PREWARM_TOKENIZERS)
            break;
    }
    return tokenizerIds;
}
/** Release reconstructable tokenizer state when the host reports low memory. */
export function releaseTokenizerMemory() {
    instanceCache.clear();
...
export function _getCachedTokenizerIdsForTests() {
