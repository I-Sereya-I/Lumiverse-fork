...
export function createSubscription(userId, input) {
    const id = crypto.randomUUID();
    const now = Math.floor(Date.now() / 1000);
    getDb()
        .query(`INSERT INTO push_subscriptions (id, user_id, endpoint, p256dh, auth, user_agent, label, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(user_id, endpoint) DO UPDATE SET
        ...
        .run(id, userId, input.endpoint, input.keys.p256dh, input.keys.auth, input.userAgent ?? "", input.label ?? "", now, now);
    ...
    const row = getDb()
        ...
        .get(userId, input.endpoint);
    return row;
}
export function deleteSubscription(userId, id) {
    const result = getDb()
    ...
    const results = await Promise.allSettled(subs.map(async (sub) => {
        try {
            ...
            if (response.ok || response.status === 201) {
                ...
                console.error(`[push] Push service returned ${response.status} for ${sub.id}: ${body.slice(0, 200)}`);
            }
        }
        catch (err) {
...
async function buildGenerationEndedNotification(payload) {
    ...
    return isError
        ? {
            ...
            body: payload.error.slice(0, 120),
...
export async function dispatchGenerationEndedPush(userId, payload) {
...
export function initPushListeners() {
    eventBus.on(EventType.GENERATION_ENDED, async (event) => {
        ...
        if (!userId)
            return;
        await dispatchGenerationEndedPush(userId, event.payload).catch((err) => {
