...
import { galleryArchiveStem, parseGalleryImageReference, } from "../utils/gallery-image-reference";
 ...
 */
function stripPngTextChunks(pngBuffer, keywords) {
    if (pngBuffer.length < 8 || !pngBuffer.subarray(0, 8).equals(PNG_SIGNATURE)) {
        return pngBuffer;
    }
    const parts = [pngBuffer.subarray(0, 8)];
    let offset = 8;
    let stripped = false;
    while (offset + 12 <= pngBuffer.length) {
        ...
        if (!skip)
            parts.push(pngBuffer.subarray(offset, chunkEnd));
        else
            stripped = true;
        offset = chunkEnd;
        if (type === "IEND")
 ...
 */
export function stripCardTextChunks(buffer) {
 ...
 */
export function embedPngTextChunk(pngBuffer, keyword, textValue) {
    if (pngBuffer.length < 8 || !pngBuffer.subarray(0, 8).equals(PNG_SIGNATURE)) {
...
async function readImageBytes(userId, imageId) {
...
function getExportAvatarImageIds(character) {
    const ids = [
...
export function buildCCSv3Json(userId, character) {
 ...
 */
function mergeWorldBooksForExport(userId, bookIds) {
    if (bookIds.length === 1) {
...
// ── Export: JSON ─────────────────────────────────────────────────────────────
export function exportAsJson(userId, characterId) {
...
// ── Export: PNG ──────────────────────────────────────────────────────────────
export async function exportAsPng(userId, characterId) {
    const character = getCharacter(userId, characterId);
    if (!character)
        return null;
    // Get avatar image
...
function reportCharxProgress(options, progress) {
    try {
...
export async function exportAsCharx(userId, characterId, options = {}) {
    const character = getCharacter(userId, characterId);
    if (!character)
        return null;
    const ccsv3 = buildCCSv3Json(userId, character);
            ...
            for (const [label, imageId] of Object.entries(labels)) {
                assetTasks.push(async () => {
                    ...
                    if (img) {
                        ...
                        if (isNsfwExpressionLabel(label))
    ...
    if (altFields && typeof altFields === "object") {
        const hasAny = Object.values(altFields).some((arr) => Array.isArray(arr) && arr.length > 0);
        if (hasAny) {
    ...
    for (const { groupName, mappings } of expressionGroupMappings) {
        if (Object.keys(mappings).length > 0)
            groupMappings[groupName] = mappings;
    }
    ...
    const hasModules = modules.expressions || modules.expression_groups || modules.alternate_fields || modules.alternate_avatars || modules.avatar_bindings || modules.landing_perspective_layers || modules.world_books?.length || modules.regex_scripts;
    if (hasModules) {
