...
import { cancelEditAndSendOutbox, claimNextEditAndSendOutbox, dispatchEditAndSendRequest, dispatchPendingEditAndSendOutbox, getGenerationOutboxById, getGenerationOutboxByRequest, reconcileEditAndSendOutbox, recoverEditAndSendOutbox, resetEditAndSendDispatcherForTests, setEditAndSendGenerationActiveCheck, setEditAndSendStartGeneration, setEditAndSendStopGeneration, } from "./edit-and-send-dispatcher.service";
function initDispatcherDb() {
    closeDatabase();
    initDatabase(":memory:");
...
function insertMessage(overrides = {}) {
    ...
    const createdAt = typeof overrides.created_at === "number" ? overrides.created_at : Math.floor(Date.now() / 1000);
    const revision = typeof overrides.revision === "number" ? overrides.revision : 1;
    getDb().query(`INSERT INTO messages (id, chat_id, index_in_chat, is_user, content, created_at, revision)
     VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, chatId, indexInChat, isUser, content, createdAt, revision);
    return id;
}
function insertOutbox(overrides = {}) {
    const id = typeof overrides.id === "string" ? overrides.id : crypto.randomUUID();
    const now = Date.now();
    getDb().query(`INSERT INTO generation_outbox (
    ...
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, overrides.request_id ?? "req-1", overrides.user_id ?? "u1", overrides.chat_id ?? "c1", overrides.branch_chat_id ?? "b1", overrides.edited_message_id ?? "m1", overrides.target_message_id ?? null, overrides.target_swipe_index ?? null, overrides.expected_version ?? 1, overrides.generation_id ?? `gen-${id}`, overrides.mode ?? "normal", overrides.status ?? "pending", overrides.lease_owner ?? null, overrides.lease_expires_at ?? null, overrides.attempt_count ?? 0, overrides.next_attempt_at ?? null, overrides.last_error_code ?? null, overrides.terminal_reason ?? null, overrides.dispatched_at ?? null, overrides.completed_at ?? null, overrides.cancelled_at ?? null, overrides.created_at ?? now, overrides.updated_at ?? now);
    return id;
}
beforeEach(() => {
    initDispatcherDb();
...
describe("edit-and-send dispatcher", () => {
    ...
    test("crash/retry/cancellation/reconciliation", async () => {
        ...
        setEditAndSendStartGeneration(async (input) => {
            ...
            if (input.generationId === "gen-fail")
