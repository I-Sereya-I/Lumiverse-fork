...
export function imageGenConnectionSecretKey(id) {
 ...
 */
function parseNanoGptUsageWindow(w, limit) {
...
function resolveNanoGptSubscriptionUsageUrl(profile) {
...
export function listConnections(userId, pagination) {
    return paginatedQuery("SELECT * FROM image_gen_connections WHERE user_id = ? ORDER BY updated_at DESC", "SELECT COUNT(*) as count FROM image_gen_connections WHERE user_id = ?", [userId], pagination, rowToProfile);
}
...
export async function createConnection(userId, input) {
    ...
    getDb()
        .query(`INSERT INTO image_gen_connections
...
export async function duplicateConnection(userId, id) {
    ...
    getDb()
        ...
        (id, user_id, name, provider, api_url, model, is_default, has_api_key, default_parameters, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(newId, userId, `${existing.name} (Copy)`, existing.provider, existing.api_url, existing.model, 0, // never default
...
export async function listConnectionModelsBySubtype(userId, id, subtype) {
    const profile = getConnection(userId, id);
    if (!profile)
        return { models: [], provider: "", error: "Connection not found" };
    const provider = getImageProvider(profile.provider);
