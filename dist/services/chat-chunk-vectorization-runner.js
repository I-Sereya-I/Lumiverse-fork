...
export async function processChatChunkVectorizationBatch(tasks, options) {
    if (tasks.length === 0) {
        ...
        if (chunk && chunk.vectorized_at == null) {
            ...
            try {
                ...
                if (Array.isArray(parsed)) {
                    messageIds = parsed.filter((id) => typeof id === "string");
                }
            }
            catch {
                messageIds = [];
    ...
    const refreshedChats = new Set();
    const failedChunkIds = new Set();
    await embeddingsSvc.embedWithAdaptiveBatching(tasks[0].userId, chunks, batchSize, (chunk) => chunk.content, async (batchChunks, _texts, vectors) => {
        ...
        db.query(`UPDATE chat_chunks SET vectorized_at = ?, vector_model = ? WHERE id IN (${updatePlaceholders})`).run(now, cfg.model, ...writtenChunks.map((chunk) => chunk.id));
        ...
        if (!options?.signal?.aborted && failedItems.length === 1) {
            ...
            try {
                const recovered = await embeddingsSvc.tryRecoverChatChunkEmbeddingWithAutoSplit(tasks[0].userId, chunk.chatId, chunk.id, chunk.content, error, {
                    chunkId: chunk.id,
                    messageIds: chunk.messageIds,
                }, chunk.tokenCount, { signal: options?.signal });
        ...
        for (const chunk of failedItems)
