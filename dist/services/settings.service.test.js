...
describe("settings.service world-book vector tracking", () => {
    test("allows saved theme packs to exceed the generic 2 MB settings cap", () => {
        ...
        const row = getDb().query("SELECT length(value) AS size FROM settings WHERE key = ? AND user_id = ?").get("savedThemes", "u1");
    ...
    test("changing worldBookVectorSettings resets indexed entries back to trackable pending/not_enabled states", () => {
        ...
        const rows = getDb().query("SELECT id, vector_index_status, vector_indexed_at, vector_index_error FROM world_book_entries ORDER BY id ASC").all();
        expect(rows).toEqual([
    ...
    test("rewriting the same worldBookVectorSettings payload does not re-stale entries again", () => {
        ...
        putSetting("u1", "worldBookVectorSettings", { presetMode: "deep", chunkTargetTokens: 720 });
        getDb().run("UPDATE world_book_entries SET vector_index_status = 'indexed', vector_indexed_at = 999, vector_index_error = 'kept' WHERE id = 'eligible'");
        putSetting("u1", "worldBookVectorSettings", { presetMode: "deep", chunkTargetTokens: 720 });
        const row = getDb().query("SELECT vector_index_status, vector_indexed_at, vector_index_error FROM world_book_entries WHERE id = 'eligible'").get();
        expect(row).toEqual({
            vector_index_status: "indexed",
