...
describe("associative regex action claims", () => {
    beforeAll(() => {
      ...
      created_at INTEGER NOT NULL
       ...
       VALUES (?, ?, 0, 0, 'Guide', 'Choose', 1, 0, '["Choose"]', '[1]', '{}', 1)`).run("message-1", "chat-1");
        db.query(`INSERT INTO messages
       (id, chat_id, index_in_chat, is_user, name, content, send_date, swipe_id, swipes, swipe_dates, extra, created_at)
       VALUES (?, ?, 1, 1, 'User', 'Choose', 2, 0, '["Choose"]', '[2]', '{}', 2)`).run("user-message", "chat-1");
    });
    ...
    test("commits resolved state effects into persistent chat variables", () => {
        ...
        expect(claimed.status).toBe("claimed");
        const metadata = JSON.parse(getDb().query("SELECT metadata FROM chats WHERE id = ?").get("chat-1").metadata);
        ...
        expect(duplicate.status).toBe("used");
        const afterDuplicate = JSON.parse(getDb().query("SELECT metadata FROM chats WHERE id = ?").get("chat-1").metadata);
