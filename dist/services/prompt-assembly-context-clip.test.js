...
describe("clipToContextBudget", () => {
    test("surfaces when fixed prompt overhead leaves no room for chat history", async () => {
        ...
        messages[1].__chatHistorySource = true;
        messages[2].__chatHistorySource = true;
    ...
    test("clips only history before a protected context anchor", async () => {
        ...
        for (const message of messages)
            message.__chatHistorySource = true;
        messages[1].__contextAnchorProtected = true;
        messages[2].__contextAnchorProtected = true;
        ...
        expect(messages.every((message) => message.__contextAnchorProtected)).toBe(true);
    });
