...
import { TABLE_REGISTRY, VIA_CHAT_TABLES, VIA_WORLD_BOOK_TABLES, VAULT_TABLES, VIA_VAULT_TABLES, } from "./table-registry";
/**
...
// Runtime cleanup
// ---------------------------------------------------------------------------
async function stopUserExtensions(userId) {
 ...
 */
function collectUserFilePaths(userId) {
    const seen = new Set();
    for (const spec of TABLE_REGISTRY) {
        if (!spec.fileRefs || spec.fileRefs.length === 0)
            continue;
        ...
        const sql = `SELECT ${cols.map(ident).join(", ")} FROM ${ident(spec.table)} ` +
...
function perUserDirectories(userId) {
...
function performSqlWipe(userId) {
    const db = getDb();
    const counts = {};
    ...
    const tx = db.transaction(() => {
        ...
        for (const tbl of VIA_CHAT_TABLES) {
            counts[tbl] = (counts[tbl] || 0) + runDelete(db, `DELETE FROM ${ident(tbl)} WHERE chat_id IN (SELECT id FROM chats WHERE user_id = ?)`, userId);
        }
        for (const tbl of VIA_WORLD_BOOK_TABLES) {
            counts[tbl] = (counts[tbl] || 0) + runDelete(db, `DELETE FROM ${ident(tbl)} WHERE world_book_id IN (SELECT id FROM world_books WHERE user_id = ?)`, userId);
        }
        for (const tbl of VIA_VAULT_TABLES) {
            counts[tbl] = (counts[tbl] || 0) + runDelete(db, `DELETE FROM ${ident(tbl)} WHERE vault_id IN (SELECT id FROM cortex_vaults WHERE user_id = ?)`, userId);
        }
        // 4b) Vault parents (no FK to user, so cascade wouldn't reach them).
        for (const tbl of VAULT_TABLES) {
            counts[tbl] = (counts[tbl] || 0) + runDelete(db, `DELETE FROM ${ident(tbl)} WHERE user_id = ?`, userId);
        }
        ...
        for (const spec of TABLE_REGISTRY) {
            ...
            if (!built)
                continue;
            counts[spec.table] = (counts[spec.table] || 0) + runDelete(db, built.sql, userId, ...built.params);
        }
        // 4d) Tables excluded from export/import but still user-scoped. Today
        //     that's just push_subscriptions (device-bound).
        counts["push_subscriptions"] = runDelete(db, "DELETE FROM push_subscriptions WHERE user_id = ?", userId);
        ...
        counts["secrets"] = runDelete(db, 'DELETE FROM "secrets" WHERE user_id = ?', userId);
        counts["settings"] = runDelete(db, 'DELETE FROM "settings" WHERE user_id = ?', userId);
        counts["session"] = runDelete(db, 'DELETE FROM "session" WHERE userId = ?', userId);
        counts["account"] = runDelete(db, 'DELETE FROM "account" WHERE userId = ?', userId);
        counts["user"] = runDelete(db, 'DELETE FROM "user" WHERE id = ?', userId);
    });
    tx();
    return counts;
}
function runDelete(db, sql, ...params) {
