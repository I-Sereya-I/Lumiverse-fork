...
import { TABLE_REGISTRY, VIA_CHAT_TABLES, VAULT_TABLES, VIA_VAULT_TABLES, EXCLUDED_TABLES, SECRET_SETTING_KEY_PATTERNS, } from "./table-registry";
import { createManifest, ARCHIVE_SCHEMA_VERSION, NDJSON_MAX_RECORD_BYTES, } from "./manifest";
 ...
 */
export function buildExportStream(opts) {
 ...
 */
function buildSelectForTable(table, userId, spec, extraWhere) {
    ...
    switch (spec?.ownership) {
        ...
        case "via_pack":
            ...
            break;
        case "via_vault":
            wheres.push(`${ident(table)}.vault_id IN (SELECT id FROM cortex_vaults WHERE user_id = ?)`);
            params.push(userId);
            break;
        case "via_session":
            wheres.push(`${ident(table)}.session_id IN (SELECT id FROM dream_weaver_sessions WHERE user_id = ?)`);
            params.push(userId);
            break;
        case "via_document":
            wheres.push(`${ident(table)}.document_id IN (SELECT id FROM databank_documents WHERE user_id = ?)`);
            params.push(userId);
            break;
        case "via_installer":
    ...
    if (extraWhere)
        wheres.push(extraWhere);
    if (spec?.extraWhere)
        wheres.push(spec.extraWhere);
    let sql = select;
    if (wheres.length > 0)
 ...
 */
function makeControllerSink(controller) {
    return new Writable({
 ...
 */
function openNdjsonEntry(archive, archivePath) {
    ...
    return {
        write(row) {
            ...
            if (recordBytes > NDJSON_MAX_RECORD_BYTES) {
                throw new Error(`NDJSON record in ${archivePath} is ${recordBytes} bytes; ` +
            ...
            rowCount++;
            if (pendingBytes >= NDJSON_FLUSH_BYTES)
                flush(false);
        },
        close() {
            flush(true);
 ...
 */
async function streamFileIntoArchive(archive, absPath, archivePath, signal) {
    if (!existsSync(absPath))
        return false;
    const file = Bun.file(absPath);
    const entryOpts = { name: archivePath, store: true };
    if (file.size === 0) {
 ...
 */
async function exportLancedbVectors(userId, archive, signal, onProgress) {
    const counts = {};
    let lance;
    try {
                    ...
                    if (vec && typeof vec === "object") {
                        const arr = vec instanceof Float32Array
                            ...
                            : Array.isArray(vec)
                                ...
                                : null;
                        if (arr) {
                            vectorB64 = Buffer.from(arr.buffer, arr.byteOffset, arr.byteLength).toString("base64");
                        }
                    }
...
// Orchestrator
// ---------------------------------------------------------------------------
async function runExport(opts, controller) {
    ...
    let embeddingConfig = { provider: null, model: null, dimension: null };
    try {
    ...
    const fileQueue = [];
    async function exportRegistryTable(spec) {
        if (EXCLUDED_TABLES.has(spec.table))
            return;
        ...
        for (const raw of stmt.iterate(...built.params)) {
            if (signal?.aborted)
                throw signal.reason ?? new DOMException("Aborted", "AbortError");
            // Drop secret-looking settings rows defensively.
            if (spec.table === "settings" && isSecretSettingKey(String(raw.key ?? "")))
                continue;
    ...
    async function exportChildTable(table, ownership) {
        if (EXCLUDED_TABLES.has(table))
            return;
        const synthSpec = { table, ownership };
    ...
    async function exportWorldBookEntries() {
        const table = "world_book_entries";
        const columns = getTableColumns(table);
        if (columns.length === 0)
            return;
        const sql = `SELECT ${columns.map(ident).join(", ")} FROM ${ident(table)} ` +
        ...
        emitProgress(userId, { phase: "table_start", table });
        for (const raw of stmt.iterate(userId)) {
            if (signal?.aborted)
                throw signal.reason ?? new DOMException("Aborted", "AbortError");
            entry.write(raw);
            rowsOut++;
    ...
    try {
        ...
        if (sound) {
            const ok = await streamFileIntoArchive(archive, sound.filepath, `files/notification-sounds/${basename(sound.filepath)}`, signal);
            if (!ok)
                missingFiles.push(sound.filepath);
        }
    }
    catch {
        /* sound optional */
    ...
    await withConcurrency(deduped, FILE_CONCURRENCY, async (entry) => {
        ...
        if (!ok)
            missingFiles.push(entry.absolutePath);
        filesDone++;
        if ((filesDone & 31) === 0) {
    ...
    if (includeVectors) {
        ...
        const { counts: vectorCounts } = await exportLancedbVectors(userId, archive, signal, (table, count) => emitProgress(userId, { phase: "lancedb", table, processed: count }));
    ...
    if (opts.secrets) {
        ...
        for (const key of ctx.secretKeys) {
            ...
            try {
                ...
                const e = err;
                console.warn(`[user-data export] failed to read secret ${key}: ${e?.name || "Error"}: ${e?.message || String(err)}`);
            }
            if (plaintext === null) {
