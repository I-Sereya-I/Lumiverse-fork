...
import { decryptSecret, lookupConsumedTicket, recordConsumedTicket, verifyTicket, TicketError, } from "./secret-ticket.service";
import { putSecret } from "../secrets.service";
import { closeSync, constants as fsConstants, copyFileSync, createReadStream, existsSync, mkdirSync, openSync, readSync, renameSync, statfsSync, statSync, unlinkSync, writeSync, } from "fs";
...
import { parseManifest, embeddingConfigsMatch, NDJSON_FORMAT_VERSION, NDJSON_MAX_RECORD_BYTES, } from "./manifest";
import { IMPORT_ORDER, EXCLUDED_TABLES, SECRET_SETTING_KEY_PATTERNS, } from "./table-registry";
import { sanitizeEntry, safeJoin } from "./sanitize";
// ---------------------------------------------------------------------------
...
const JOBS = new Map();
const USER_RUNNING = new Map(); // userId -> jobId
const USER_UPLOAD_RESERVATIONS = new Map();
let globalImportSlot = null;
...
export function isUserImportRunning(userId) {
 ...
 */
export function reserveImportUpload(userId) {
    if (isUserImportRunning(userId) || globalImportSlot !== null)
        return null;
...
export function releaseImportUpload(userId, jobId) {
    if (USER_UPLOAD_RESERVATIONS.get(userId) === jobId) {
        USER_UPLOAD_RESERVATIONS.delete(userId);
    }
    if (globalImportSlot === jobId)
        globalImportSlot = null;
}
/** Transfer a successful upload reservation into its background job. */
function claimImportReservation(userId, jobId) {
    if (USER_UPLOAD_RESERVATIONS.get(userId) === jobId) {
        USER_UPLOAD_RESERVATIONS.delete(userId);
    }
}
function releaseGlobalImportSlot(jobId) {
    if (globalImportSlot === jobId)
        globalImportSlot = null;
}
...
function startsWithZipMagic(prefix) {
    if (prefix.byteLength < 4)
        return false;
    return (prefix[0] === ZIP_MAGIC[0] &&
 ...
 */
export async function persistUploadedArchive(userId, body, declaredSize, jobId = crypto.randomUUID()) {
    if (declaredSize !== null && declaredSize > MAX_COMPRESSED_BYTES) {
        throw new ArchiveValidationError("size", `archive exceeds ${MAX_COMPRESSED_BYTES / (1024 * 1024 * 1024)} GB cap`);
    }
    ...
    const writeChunk = (chunk) => {
        ...
        if (total + chunk.byteLength > MAX_COMPRESSED_BYTES) {
            throw new ArchiveValidationError("size", `archive exceeds compressed size cap (${total + chunk.byteLength} bytes)`);
        }
        ...
        while (true) {
            ...
            if (!magicChecked) {
                ...
                if (!startsWithZipMagic(header)) {
                    throw new ArchiveValidationError("not_zip", "Uploaded file is not a ZIP archive (missing PK\\x03\\x04 header).");
                }
                magicChecked = true;
                writeChunk(header);
    ...
    if (stat.size > MAX_COMPRESSED_BYTES) {
        ...
        throw new ArchiveValidationError("size", `archive exceeds compressed size cap (${stat.size} bytes)`);
    }
    return { path, jobId };
}
/**
 ...
 */
function readZip64Extra(extra, extraOffset, extraLen) {
    let pos = extraOffset;
...
function isSafeZipNumber(value) {
 ...
 */
async function* scanCentralDirectory(file, cdOffset, cdSize, totalEntries) {
    ...
    while (fetched < cdSize && entriesRead < totalEntries) {
        ...
        const chunk = await readBytes(file, cdOffset + fetched, cdOffset + fetched + readSize);
        ...
        while (pos + 46 <= data.byteLength && entriesRead < totalEntries) {
            ...
            if (view.getUint32(0, true) !== CDH_SIG) {
                throw new ArchiveValidationError("not_zip", `central directory header signature invalid at entry ${entriesRead}`);
            }
            ...
            if (pos + recordSize > data.byteLength)
                break;
            ...
            if (extraLen > 0 &&
                ...
                if (!zip64) {
                    throw new ArchiveValidationError("not_zip", `ZIP64 extra field missing or truncated for ${name || `entry ${entriesRead}`}`);
                }
                let cursor = 0;
            ...
            if (!isSafeZipNumber(compressedSize) ||
...
async function findManifestInCentralDirectory(file, cdOffset, cdSize, totalEntries) {
...
async function locateCentralDirectory(file) {
    ...
    if (eocdNeedsZip64) {
        ...
        const zip64EocdBytes = await readBytes(file, zip64EocdOffset, zip64EocdOffset + ZIP64_EOCD_RECORD_BYTES);
 ...
 */
export async function verifyArchiveFast(archivePath) {
    const file = Bun.file(archivePath);
    const { size, cdOffset, cdSize, totalEntries } = await locateCentralDirectory(file);
    const manifestEntry = await findManifestInCentralDirectory(file, cdOffset, cdSize, totalEntries);
    if (!manifestEntry) {
    ...
    if (manifestEntry.uncompressedSize > MAX_MANIFEST_BYTES) {
        throw new ArchiveValidationError("bad_manifest", `manifest.json declares ${manifestEntry.uncompressedSize} bytes (cap ${MAX_MANIFEST_BYTES})`);
    }
    if (manifestEntry.compressedSize > MAX_MANIFEST_COMPRESSED_BYTES) {
        throw new ArchiveValidationError("bad_manifest", `manifest.json declares ${manifestEntry.compressedSize} compressed bytes (cap ${MAX_MANIFEST_COMPRESSED_BYTES})`);
    }
    if (manifestEntry.compression !== 0 && manifestEntry.compression !== 8) {
        throw new ArchiveValidationError("bad_manifest", `manifest.json uses unsupported compression method ${manifestEntry.compression}`);
    }
    ...
    const lfhHeader = await readBytes(file, manifestEntry.localHeaderOffset, manifestEntry.localHeaderOffset + 30);
    ...
    try {
        ...
        throw new ArchiveValidationError("bad_manifest", `manifest.json decompression failed: ${err.message}`);
    }
    if (bytes.byteLength > MAX_MANIFEST_BYTES) {
        throw new ArchiveValidationError("bad_manifest", `manifest.json decompressed to ${bytes.byteLength} bytes (cap ${MAX_MANIFEST_BYTES})`);
    }
    try {
        ...
        throw new ArchiveValidationError("bad_manifest", `manifest.json parse failed: ${err.message}`);
    }
}
/**
...
export function startImport(opts) {
    ...
    let ticketResolver = () => { };
    const ticketGate = new Promise((resolve) => {
...
async function yieldAndCheck(signal) {
 ...
 */
async function recoverLegacyDuplicatedNdjson(stagingPath, expectedSize, expectedCrc32, signal) {
    ...
    const appendFragment = (fragment) => {
        if (fragment.byteLength === 0)
            return true;
        if (lineBytes + fragment.byteLength > LEGACY_MAX_NDJSON_LINE_BYTES)
            return false;
    ...
    try {
        ...
        while (true) {
            if (signal.aborted)
                throw signal.reason ?? new Error("import cancelled");
            ...
            for (let i = 0; i < read; i++) {
                if (readBuffer[i] !== 0x0a)
                    continue;
                if (!appendFragment(readBuffer.subarray(start, i)) || !consumeLine())
                    return null;
                start = i + 1;
            }
            if (!appendFragment(readBuffer.subarray(start, read)))
                return null;
        ...
        if (lineBytes !== 0)
            return null;
        if (retainedBytes !== expectedSize || finishCrc32(crcState) !== expectedCrc32)
            return null;
        closeSync(outputFd);
        outputFd = null;
        closeSync(inputFd);
...
function readExactSync(fd, position, length) {
    const out = new Uint8Array(length);
    let offset = 0;
    while (offset < length) {
        const read = readSync(fd, out, offset, length - offset, position + offset);
        if (read <= 0)
            throw new ArchiveValidationError("not_zip", "archive is truncated");
        offset += read;
    }
    return out;
}
function readCappedTextFile(path, maxBytes, label) {
...
function assertExtractionDiskCapacity(stagingDir, stagedBytes, binaryBytes) {
    try {
        ...
        if (available < required) {
            throw new Error(`insufficient free disk for import (need ${required} bytes, have ${available} bytes)`);
        }
    }
    catch (err) {
...
function getLocalDataOffset(archiveFd, archiveSize, entry) {
...
async function copyStoredEntry(archiveFd, dataStart, compressedSize, onChunk, signal) {
    ...
    while (offset < compressedSize) {
        ...
        if (read <= 0)
            throw new ArchiveValidationError("not_zip", "archive entry is truncated");
...
async function inflateEntry(archivePath, dataStart, compressedSize, onChunk, signal) {
    if (compressedSize === 0) {
...
// ---------------------------------------------------------------------------
async function extractArchive(job) {
    ...
    for await (const entry of scanCentralDirectory(archive, cdOffset, cdSize, totalEntries)) {
        ...
        if (descriptor.kind === "files")
            declaredBinaryBytes += entry.uncompressedSize;
    }
        ...
        for await (const centralEntry of scanCentralDirectory(archive, cdOffset, cdSize, totalEntries)) {
            ...
            try {
                ...
                if (centralEntry.compression === 0) {
                    await copyStoredEntry(archiveFd, dataStart, centralEntry.compressedSize, onChunk, job.abort.signal);
                }
                else {
                    await inflateEntry(job.archivePath, dataStart, centralEntry.compressedSize, onChunk, job.abort.signal);
                }
            }
            finally {
                closeSync(stagingFd);
            ...
            if (actualByteSize !== centralEntry.uncompressedSize ||
                ...
                let repairedSize = null;
                if (canRecoverLegacyAsyncNdjson &&
                    (descriptor.kind === "database" || descriptor.kind === "lancedb")) {
                    repairedSize = await recoverLegacyDuplicatedNdjson(stagingPath, centralEntry.uncompressedSize, centralEntry.crc32, job.abort.signal);
                }
                if (repairedSize !== null) {
                    ...
                    console.warn(`[user-data import] recovered duplicated legacy NDJSON entry: ${centralEntry.name}`);
                }
                else if (actualByteSize !== centralEntry.uncompressedSize) {
                    throw new ArchiveValidationError("not_zip", `entry size disagrees with central directory (${centralEntry.name}; declared ${centralEntry.uncompressedSize}, extracted ${actualByteSize})`);
                }
                else {
                    throw new ArchiveValidationError("not_zip", `entry CRC32 disagrees with central directory (${centralEntry.name})`);
                }
            }
    ...
    if (statsEntry) {
        try {
            const statsText = readCappedTextFile(statsEntry.stagingPath, MAX_MANIFEST_STATS_BYTES, "manifest-stats.json");
            const stats = JSON.parse(statsText);
            if (stats?.counts)
                buf.manifest.counts = stats.counts;
            if (Array.isArray(stats?.missingFiles))
                buf.manifest.missingFiles = stats.missingFiles;
        }
        catch {
 ...
 */
function ndjsonLineLimitForManifest(manifest) {
 ...
 * per-line size cap.
 */
async function* readNdjson(path, maxLineBytes = NDJSON_MAX_RECORD_BYTES) {
    const decoder = new TextDecoder();
    const buffer = new Uint8Array(ARCHIVE_READ_BYTES);
    const fragments = [];
    let lineBytes = 0;
 ...
 */
function mergeSettingValue(existingValue, importedValue) {
    ...
    const isIdArrayOrEmpty = (v) => Array.isArray(v) &&
        v.every((x) => x && typeof x === "object" && typeof x.id === "string");
    ...
    for (const [k, importedField] of Object.entries(importedValue)) {
        const existingField = existingValue[k];
 ...
 */
async function applySettingsTable(ctx, stagingPath) {
    if (!tableExists("settings"))
        return { imported: 0, skipped: 0, merged: 0 };
    const db = getDb();
    const selectStmt = db.prepare("SELECT value FROM settings WHERE key = ? AND user_id = ?");
    const insertStmt = db.prepare("INSERT INTO settings (key, value, user_id, updated_at) VALUES (?, ?, ?, ?)");
    const updateStmt = db.prepare("UPDATE settings SET value = ?, updated_at = ? WHERE key = ? AND user_id = ?");
    let imported = 0;
    let merged = 0;
    let skipped = 0;
    let lineCount = 0;
    for await (const raw of readNdjson(stagingPath, ctx.ndjsonLineBytes)) {
        if (ctx.signal.aborted)
            throw ctx.signal.reason ?? new Error("import cancelled");
        ...
        const existing = selectStmt.get(key, ctx.userId);
        if (!existing) {
 ...
 */
async function applyTable(ctx, table, stagingPath) {
    ...
    const getInsert = (row) => {
        ...
        const prepared = {
            ...
            statement: db.prepare(`INSERT OR IGNORE INTO ${ident(table)} (${colList}) VALUES (${placeholders})`),
        };
    ...
    for await (const raw of readNdjson(stagingPath, ctx.ndjsonLineBytes)) {
        ...
        if (hasUserId)
            filtered.user_id = ctx.userId;
        if (hasInstalledByUser)
            filtered.installed_by_user_id = ctx.userId;
        // Scrub has_api_key on connection tables — secrets aren't in the archive.
        if (columnSet.has("has_api_key"))
            filtered.has_api_key = 0;
        batch.push(filtered);
        lineCount++;
...
// ---------------------------------------------------------------------------
async function applyBinary(ctx, entry) {
    const dest = (() => {
        switch (entry.bucket) {
            case "images":
            case "thumbnails":
    ...
    if (entry.bucket === "notification-sounds") {
        ...
        try {
            ...
            if (read < head.byteLength)
                head.fill(0, Math.max(read, 0));
        }
        finally {
            closeSync(fd);
        }
...
// ---------------------------------------------------------------------------
async function applyLancedbVectors(ctx, buf, archiveCfg) {
    if (!buf.manifest?.includeVectors)
        return;
    let currentCfg = { provider: null, model: null, dimension: null };
    try {
    ...
    if (!embeddingConfigsMatch(archiveCfg, currentCfg)) {
        ...
        try {
         ...
         WHERE chat_id IN (SELECT id FROM chats WHERE user_id = ?)`, [ctx.userId]);
            db.run("UPDATE databank_chunks SET vectorized_at = NULL, vector_model = NULL WHERE user_id = ?", [ctx.userId]);
            db.run(`UPDATE world_book_entries SET vectorized = 0, vector_index_status = 'not_enabled'
         WHERE world_book_id IN (SELECT id FROM world_books WHERE user_id = ?)`, [ctx.userId]);
    ...
    for (const entry of buf.entries) {
        if (entry.kind !== "text" || entry.origin !== "lancedb")
            continue;
        if (ctx.signal.aborted)
            throw ctx.signal.reason ?? new Error("import cancelled");
 ...
 * user's data.
 */
function scrubOrphanForeignKeys(userId) {
    const db = getDb();
    ...
    for (const [child, col, parent] of nullableFks) {
        ...
        try {
            const childCols = new Set(db.query(`PRAGMA table_info(${ident(child)})`).all().map((c) => c.name));
            if (!childCols.has(col))
                continue;
            if (childCols.has("user_id")) {
                db.run(`UPDATE ${ident(child)} SET ${ident(col)} = NULL
           WHERE user_id = ?
             AND ${ident(col)} IS NOT NULL
             AND ${ident(col)} NOT IN (SELECT id FROM ${ident(parent)})`, [userId]);
            }
...
function scrubMemoryCortexOrphans(userId) {
    const db = getDb();
    try {
        db.run(`UPDATE memory_font_colors SET entity_id = NULL
       WHERE chat_id IN (SELECT id FROM chats WHERE user_id = ?)
         AND entity_id IS NOT NULL
         AND entity_id NOT IN (SELECT id FROM memory_entities)`, [userId]);
    }
    catch (err) {
 ...
 */
async function applySecrets(ctx, stagingPath, smk) {
    let restored = 0;
    let skipped = 0;
    for await (const raw of readNdjson(stagingPath, ctx.ndjsonLineBytes)) {
        if (ctx.signal.aborted)
        ...
        let plaintext;
        try {
            plaintext = await decryptSecret(smk, entry);
        }
        catch (err) {
...
export async function submitTicket(jobId, rawTicket) {
    const job = JOBS.get(jobId);
    if (!job)
    ...
    if (!job.manifest?.archiveId)
        throw new Error("job has no manifest yet");
    ...
    try {
        ...
        if (err instanceof TicketError)
            throw err;
        throw new TicketError("malformed", String(err.message ?? err));
    }
...
export function skipTicket(jobId) {
    const job = JOBS.get(jobId);
    if (!job)
        return false;
    if (job.status !== "awaiting_ticket")
...
async function runImportJob(job) {
    ...
    try {
        ...
        if (secretsIndexEntry) {
            try {
                const text = readCappedTextFile(secretsIndexEntry.stagingPath, MAX_SECRETS_INDEX_BYTES, "secrets/index.json");
        ...
        for await (const raw of readNdjson(buf.binaryJournalPath)) {
            if (job.abort.signal.aborted)
                throw job.abort.signal.reason ?? new Error("cancelled");
            const entry = raw;
            if (entry.kind !== "binary" ||
        ...
        if (ticketResult) {
            ...
            if (secretsEncryptedEntry) {
                ...
                const { restored, skipped } = await applySecrets(ctx, secretsEncryptedEntry.stagingPath, ticketResult.smk);
        ...
        if (fkRestored) {
            try {
                ...
                if (orphans.length > 0) {
                    console.warn(`[user-data import] ${orphans.length} orphan FK row(s) remain after import`, orphans.slice(0, 10));
                }
            }
            catch {
                /* informational */
