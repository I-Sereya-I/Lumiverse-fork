...
import { applyPersonaAddonStates, getChatPersonaAddonStates, getChatPersonaAddonToggleOrder, resolvePersonaAvatarInfo, withChatPersonaAddonState, } from "./persona-addon-states";
describe("persona add-on states", () => {
    test("reads sanitized add-on states for the active persona from chat metadata", () => {
        expect(getChatPersonaAddonStates({
        ...
        }, "personaA")).toEqual({ addonOn: true, addonOff: false });
    });
    ...
    test("applies persona and attached global add-on overrides", () => {
        ...
        expect(applyPersonaAddonStates(persona, {
            personaAddon: true,
            globalAddon: false,
