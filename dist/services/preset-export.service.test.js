...
import { buildPresetBulkExportStream, consumePreparedPresetExport, preparePresetBulkExport, } from "./preset-export.service";
function initTestDb() {
    closeDatabase();
    initDatabase(":memory:");
...
function insertPreset(id, userId, blockKey) {
    getDb().run(`INSERT INTO presets (id, name, provider, parameters, prompt_order, metadata, prompts, user_id)
...
describe("streaming preset bulk export", () => {
    test("prepares a user-scoped one-shot download and streams portable JSON entries", async () => {
        ...
        const bytes = new Uint8Array(await new Response(buildPresetBulkExportStream("u1", entry.presetIds)).arrayBuffer());
