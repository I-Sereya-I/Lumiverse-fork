...
import { buildRuntimeWorldInfoChatPlacements, insertRuntimeWorldInfoIntoTaggedHistory, repositionRuntimeWorldInfoInTaggedHistory, } from "./prompt-assembly.service";
...
function placement(content, depth, direction) {
    return {
        id: content,
        content,
        entryLabel: content,
...
describe("runtime world-info chat placement", () => {
    test("uses only selected rows and restores final lore order", () => {
        ...
        expect(buildRuntimeWorldInfoChatPlacements([
            entry("later", 20),
            entry("earlier", 10),
            entry("equal-a", 15),
            entry("equal-b", 15),
        ], placements).map(({ id }) => id)).toEqual(["earlier", "equal-b", "equal-a", "later"]);
    });
    ...
    test("reapplies placement after older history is clipped", () => {
        ...
        messages.splice(messages.findIndex(({ content }) => content === "h0"), 1);
