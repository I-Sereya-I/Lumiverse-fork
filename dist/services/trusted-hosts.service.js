...
function safeNetworkInterfaces() {
    try {
        ...
        if (!networkInterfacesWarned) {
            ...
            console.warn("[trusted-hosts] Could not enumerate network interfaces:", err?.message ?? err);
        }
        return {};
    }
}
...
function isPlausibleIpLiteral(address, family) {
    if (typeof address !== "string" || address.length === 0)
        return false;
    if (family === "IPv4")
        return IPV4_LITERAL.test(address);
    return IPV6_LITERAL.test(address) && address.includes(":");
}
function rejectMalformedAddress(address, family) {
    if (malformedAddressWarned)
        return;
    malformedAddressWarned = true;
    console.warn(`[trusted-hosts] Ignoring malformed ${family} address from os.networkInterfaces(): ${String(address)}. ` +
...
function originForExplicitUrl(input) {
...
function normalizeTrustedInput(input) {
    ...
    if (!value)
        throw new InvalidTrustedHostError("Host cannot be empty");
    ...
    if (value.startsWith("[")) {
        ...
        if (close < 0)
            throw new InvalidTrustedHostError(`Malformed IPv6 literal: ${input}`);
        host = value.slice(0, close + 1);
        const rest = value.slice(close + 1);
        if (rest.startsWith(":"))
            portStr = rest.slice(1);
        else if (rest.length > 0)
            throw new InvalidTrustedHostError(`Unexpected characters after IPv6 literal: ${input}`);
    }
    else {
    ...
    if (!host)
        throw new InvalidTrustedHostError(`Host cannot be empty: ${input}`);
...
export function setTrustedHosts(hosts) {
    ...
    if (hosts.length > MAX_CONFIGURED_HOSTS) {
        throw new InvalidTrustedHostError(`Too many trusted hosts (max ${MAX_CONFIGURED_HOSTS})`);
    }
    ...
    for (const raw of hosts) {
        ...
        if (baseline.has(entry.host))
            continue; // baseline is implicit, no need to persist
...
async function tailscaleSuggestion(timeoutMs) {
    try {
...
async function buildHostnameSuggestions(baseline) {
...
export function _setTrustedHostsForTests(hosts) {
    configuredHosts = [];
    configuredTrustedInputs = [];
    const seen = new Set();
