...
describe("injectConnectionMetadataFlags", () => {
    test("sets use_responses_api when metadata flag is true", () => {
        const params = {};
        __test__.injectConnectionMetadataFlags({ provider: "openai", metadata: { use_responses_api: true } }, params);
        expect(params.use_responses_api).toBe(true);
    });
    test("does not set use_responses_api when metadata flag is missing", () => {
        const params = {};
        __test__.injectConnectionMetadataFlags({ provider: "openai", metadata: {} }, params);
    ...
    test("uses the chat id as OpenRouter's documented sticky-routing session key", () => {
        const params = {};
        __test__.injectConnectionMetadataFlags({ provider: "openrouter", metadata: {} }, params, "chat-123");
        expect(params.session_id).toBe("lumiverse:chat-123");
    });
    test("does not replace a caller-provided OpenRouter session or cache key", () => {
        const sessionParams = { session_id: "custom-session" };
        __test__.injectConnectionMetadataFlags({ provider: "openrouter", metadata: {} }, sessionParams, "chat-123");
        expect(sessionParams.session_id).toBe("custom-session");
        const cacheKeyParams = { prompt_cache_key: "custom-cache-key" };
        __test__.injectConnectionMetadataFlags({ provider: "openrouter", metadata: {} }, cacheKeyParams, "chat-123");
        expect(cacheKeyParams.session_id).toBeUndefined();
    });
    test("no-op for empty metadata", () => {
        const params = {};
        __test__.injectConnectionMetadataFlags({ provider: "openai", metadata: undefined }, params);
...
describe("prompt breakdown visibility", () => {
    ...
    test("summarizes chat history tokens separately", () => {
        expect(__test__.sumChatHistoryBreakdownTokens([
