...
const SUPPORTED_VIDEO_MIME_TO_EXT = {
    "video/mp4": ".mp4",
    "video/webm": ".webm",
    "video/quicktime": ".mov",
    "video/x-m4v": ".m4v",
...
export function isLikelyVideoUpload(mimeType, originalFilename) {
    const normalizedMime = (mimeType || "").toLowerCase().trim();
    if (normalizedMime.startsWith("video/"))
        return true;
...
async function probeInputDurationMs(ffmpeg, inputPath) {
...
async function consumeFfmpegProgress(stream, inputDurationMs, onProgress) {
    ...
    const emit = () => {
        ...
        if (roundedPercent !== null)
            lastPercentBucket = roundedPercent;
        if (done)
            seenEnd = true;
        onProgress({
            currentTimeMs,
...
async function runFfmpeg(args, options) {
    ...
    const progressTask = wantProgress && proc.stdout
        ? consumeFfmpegProgress(proc.stdout, options?.inputDurationMs, options.onProgress)
        : null;
...
export async function extractVideoPosterBuffer(input, mimeType, originalFilename) {
...
export async function stripAudioFromVideoBuffer(input, mimeType, originalFilename) {
    const ext = resolveVideoInputExtension(mimeType, originalFilename);
    if (!ext)
        return null;
    const hasFfmpeg = await isFfmpegAvailableForSilentVideo();
...
export async function normalizeVideoBuffer(input, mimeType, originalFilename, options) {
    const ext = resolveVideoInputExtension(mimeType, originalFilename);
    if (!ext || !isLikelyVideoUpload(mimeType, originalFilename))
        return null;
    const hasFfmpeg = await isFfmpegAvailableForSilentVideo();
    if (!hasFfmpeg)
        return null;
    ...
    try {
        ...
        const inputDurationMs = options.onProgress
            ...
            : null;
        const codecArgs = options.codec === "hevc"
