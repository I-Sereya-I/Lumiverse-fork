...
import { naiveConcatMp3, parseFrameHeader, sideInfoSize } from "./audio-mux.service";
 ...
 *  file's duration. */
function xingFrame(declaredFrames) {
...
function countAudioFramesAfterHeader(out) {
...
describe("naiveConcatMp3 — duration-correct frame join", () => {
    ...
    test("handles segments that have no Xing header of their own", () => {
        ...
        const bare = (n, base) => Buffer.concat(Array.from({ length: n }, (_, i) => audioFrame(base + i)));
    ...
    test("still rejects non-MP3 segments (those need ffmpeg)", () => {
        expect(() => naiveConcatMp3([mp3Segment(providerMp3(2)), { data: Buffer.from([1, 2, 3]), mime_type: "audio/wav" }])).toThrow(/only supports audio\/mpeg/);
    });
});
