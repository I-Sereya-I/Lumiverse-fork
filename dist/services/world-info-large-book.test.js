...
import { activateWorldInfo, clearWorldInfoActivationCache, finalizeActivatedWorldInfoEntries, normalizeWorldInfoSettings, } from "./world-info-activation.service";
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
let __counter = 0;
...
function asVectorCandidate(entry, finalScore = 0.8) {
    return {
        entry,
        score: finalScore,
...
describe("world info RNG injection", () => {
    ...
    test("raw activation and merge do not advance global RNG when given an isolated source", () => {
        ...
        try {
            ...
            const nativeMerge = mergeActivatedWorldInfoEntries(nativeActivation.activatedEntries, [asVectorCandidate(vector)]);
            ...
            const rawMerge = mergeActivatedWorldInfoEntries(rawActivation.activatedEntries, [asVectorCandidate(vector)], undefined, undefined, undefined, isolatedRandom);
            expect(isolatedRolls).toBe(2);
            expect(globalRolls).toBe(2);
            expect(rawMerge.activatedEntries.map((entry) => entry.id)).toEqual(nativeMerge.activatedEntries.map((entry) => entry.id));
...
describe("mergeActivatedWorldInfoEntries — vector and keyword competition", () => {
    test("includes the source book name in activated entry summaries", () => {
        ...
        const result = mergeActivatedWorldInfoEntries([entry], [], {}, new Map([["book-lore", "character"]]), new Map([["book-lore", "Character Lore"]]));
    ...
    test("accepts all 15 vector candidates when settings are default", () => {
        const vectorCandidates = Array.from({ length: 15 }, (_, i) => asVectorCandidate(makeEntry({ order_value: i + 1, comment: `memory #${i + 1}` })));
    ...
    test("accepts all 15 vector candidates when maxActivatedEntries >= 15", () => {
        const vectorCandidates = Array.from({ length: 15 }, (_, i) => asVectorCandidate(makeEntry({ order_value: i + 1 })));
        for (const cap of [15, 20, 100]) {
            const settings = { maxActivatedEntries: cap };
    ...
    test("FIXED: score-boosted vectors can now displace equal-priority keyword entries at a full cap", () => {
        ...
        const keywordEntries = Array.from({ length: 15 }, (_, i) => makeEntry({ world_book_id: "other-book", key: ["x"], order_value: i + 1, comment: `kw-${i}` }));
        const vectorCandidates = Array.from({ length: 15 }, (_, i) => asVectorCandidate(makeEntry({ order_value: 1000 + i, comment: `v-${i}` }), 0.8));
        const result = mergeActivatedWorldInfoEntries(keywordEntries, vectorCandidates, { maxActivatedEntries: 15 });
    ...
    test("FIXED: low-score vectors (finalScore ≈ 0) do NOT displace keyword entries", () => {
        ...
        // near-zero boost.
        const keywordEntries = Array.from({ length: 15 }, (_, i) => makeEntry({ world_book_id: "other-book", key: ["x"], order_value: i + 1, comment: `kw-${i}` }));
        const vectorCandidates = Array.from({ length: 15 }, (_, i) => asVectorCandidate(makeEntry({ order_value: 1000 + i, comment: `v-${i}` }), 0.01));
        const result = mergeActivatedWorldInfoEntries(keywordEntries, vectorCandidates, { maxActivatedEntries: 15 });
    ...
    test("FIXED: higher-priority keyword entries are still protected from vector displacement", () => {
        ...
        const keywordEntries = Array.from({ length: 15 }, (_, i) => makeEntry({ world_book_id: "other-book", key: ["x"], order_value: i + 1, priority: 50 }));
        const vectorCandidates = Array.from({ length: 15 }, (_, i) => asVectorCandidate(makeEntry({ order_value: 1000 + i, priority: 10 }), 
        /* saturated score */ 3.0));
        const result = mergeActivatedWorldInfoEntries(keywordEntries, vectorCandidates, { maxActivatedEntries: 15 });
        expect(result.keywordActivated).toBe(15);
        expect(result.vectorActivated).toBe(0);
    ...
    test("token budget can also starve vectors — but skip counter is budgetSim, not budgetCap", () => {
        ...
        const vectorCandidates = Array.from({ length: 15 }, () => asVectorCandidate(makeEntry({ content: "x".repeat(1100) })));
...
describe("prompt-local world-info selection content", () => {
    ...
    test("uses selection content for budget and dedup while returning raw content", () => {
        ...
        const result = mergeActivatedWorldInfoEntries([first, duplicate], [], { maxTokenBudget: 1 }, undefined, undefined, undefined, selectionContentByEntryId);
...
describe("mergeActivatedWorldInfoEntries — priority/order tie-breaking", () => {
    test("higher-priority vector entries displace lower-priority keyword entries", () => {
        const keywordEntries = Array.from({ length: 15 }, (_, i) => makeEntry({ key: ["x"], order_value: i + 1, priority: 5 }));
        const vectorCandidates = Array.from({ length: 5 }, (_, i) => asVectorCandidate(makeEntry({ order_value: 1000 + i, priority: 50 })));
        const result = mergeActivatedWorldInfoEntries(keywordEntries, vectorCandidates, { maxActivatedEntries: 15 });
    ...
    test("when budget has headroom, vector entries always win — even at equal priority", () => {
        const keywordEntries = Array.from({ length: 5 }, (_, i) => makeEntry({ key: ["x"], order_value: i + 1, priority: 10 }));
        const vectorCandidates = Array.from({ length: 10 }, (_, i) => asVectorCandidate(makeEntry({ order_value: 1000 + i, priority: 10 })));
        const result = mergeActivatedWorldInfoEntries(keywordEntries, vectorCandidates, { maxActivatedEntries: 20 });
...
describe("mergeActivatedWorldInfoEntries — unified finalization", () => {
    test("content deduplication cannot undo a score-boosted vector winner", () => {
        ...
        const result = mergeActivatedWorldInfoEntries([keywordA, keywordB, keywordDuplicate], [asVectorCandidate(vector, 1)], { maxActivatedEntries: 2 });
        expect(result.deduplicated).toBe(1);
    ...
    test("vector relevance competes under a token-only budget without mutating priority", () => {
        ...
        const result = mergeActivatedWorldInfoEntries([keyword], [asVectorCandidate(vector, 1)], { maxTokenBudget: 40 });
