...
mock.module("./embeddings.service", () => ({
    ...
    deleteWorldBookEntryEmbeddingsBeforeSourceDelete: async (_userId, _entryIds, deleteSource) => await deleteSource(),
}));
...
const { bulkOperateEntries, convertToVectorized, createEntry, createWorldBook, duplicateEntry, getEntry, reorderEntries, setWorldBookSemanticActivation, updateEntry, } = worldBooks;
...
const serviceContracts = worldBooks;
...
function createFixtureEntries() {
    ...
    if (!first || !second)
        throw new Error("failed to create world-book concurrency fixture");
    return { book, first, second };
}
function entrySideEffectState(id) {
    return getDb().query(`SELECT updated_at, revision, vectorized, vector_index_status,
            vector_indexed_at, vector_index_error
       FROM world_book_entries
      WHERE id = ?`).get(id);
}
function bookSideEffectState(id) {
    return getDb().query("SELECT updated_at FROM world_books WHERE id = ?").get(id);
}
...
async function expectAsyncConflict(operation, entryId) {
    const promise = operation();
    await expect(promise).rejects.toThrow();
    const error = await promise.then(() => undefined, (reason) => reason);
    assertCanonicalConflict(error, entryId);
}
function assertCanonicalConflict(error, entryId) {
...
function assertInvalidRevision(error) {
    expect(error).toBeDefined();
    expect(error.code).toBe("WORLD_BOOK_ENTRY_REVISION_INVALID");
}
async function expectAsyncInvalidRevision(operation) {
    const promise = operation();
    await expect(promise).rejects.toThrow();
    const error = await promise.then(() => undefined, (reason) => reason);
...
async function writeNamespace(userId, entryId, namespace, value) {
...
async function expectNamespaceRejection(operation) {
    const promise = operation();
    await expect(promise).rejects.toThrow();
    return await promise.then(() => undefined, (reason) => reason);
}
describe.serial("world-book concurrency contract matrix", () => {
    ...
    describe.serial("world-book H11 optimistic concurrency", () => {
        ...
        test("malformed-present single expectations fail and never become unconditional writes", () => {
            const malformed = [undefined, null, "1", 1.5, -1, {}, []];
        ...
        test("malformed-present reorder maps fail before changing order or revisions", () => {
            ...
            for (const expectedRevisions of malformedMaps) {
                ...
                const error = captureSyncError(() => reorderEntries(OWNER_ID, book.id, [second.id, first.id], expectedRevisions));
                assertInvalidRevision(error);
                expect(getEntry(OWNER_ID, first.id)).toMatchObject({
        ...
        test("mixed fresh and stale bulk writes are rejected atomically", async () => {
            ...
            await expectAsyncConflict(() => bulkOperateEntries(OWNER_ID, book.id, {
                action: "add_keyword",
                entry_ids: [first.id, second.id],
                keyword: "atomicity",
                expected_revisions: {
        ...
        test("stale reorder changes no entry order values or revisions", () => {
            ...
            const error = captureSyncError(() => reorderEntries(OWNER_ID, book.id, [second.id, first.id], {
                [first.id]: first.revision,
                [second.id]: second.revision,
            }));
    ...
    describe.serial("world-book H12 namespaced metadata writes", () => {
        ...
        test("enforces the serialized UTF-8 2 MiB boundary without a partial write", async () => {
            ...
            const error = await expectNamespaceRejection(() => writeNamespace(OWNER_ID, first.id, "oversized", oversizedValue));
            expect(error.code).toBe("NAMESPACE_TOO_LARGE");
        ...
        test("set and delete do not touch entry/parent timestamps, revision, vector state, or events", async () => {
            ...
            const db = getDb();
            db.run(`UPDATE world_books SET updated_at = ? WHERE id = ?`, [4000, book.id]);
        ...
        WHERE id = ?`, [3000, 9, 1, "indexed", 2000, "must-survive", first.id]);
