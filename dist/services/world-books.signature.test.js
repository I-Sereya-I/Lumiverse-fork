...
import { findImportedCharacterBookWorldBook, getWorldBook, getWorldBookListSignature, getWorldBookEntriesSignature, } from "./world-books.service";
function initDb() {
    closeDatabase();
    initDatabase(":memory:");
    const db = getDb();
...
function insertBook(o) {
    getDb().run("INSERT INTO world_books (id, name, description, metadata, created_at, updated_at, user_id, folder) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [o.id, o.name, "", JSON.stringify(o.metadata ?? {}), 0, o.updated_at ?? 0, o.user_id, o.folder ?? ""]);
}
function insertEntry(o) {
    getDb().run("INSERT INTO world_book_entries (id, world_book_id, updated_at) VALUES (?, ?, ?)", [o.id, o.world_book_id, o.updated_at ?? 0]);
}
beforeEach(initDb);
