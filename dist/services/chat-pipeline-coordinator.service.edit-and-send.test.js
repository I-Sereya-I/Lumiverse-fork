...
import { dispatchEditAndSendRequest, resetEditAndSendDispatcherForTests, setEditAndSendStartGeneration, } from "./edit-and-send-dispatcher.service";
import * as coordinator from "./chat-pipeline-coordinator.service";
const USER = "u1";
function initEditAndSendTestDb() {
    closeDatabase();
    initDatabase(":memory:");
    const db = getDb();
...
function seedMessage(id, chatId, content, options) {
    ...
    getDb()
        ...
        .run(id, chatId, options.index, isUser ? 1 : 0, isUser ? "User" : "Assistant", content, 100 + options.index, 0, JSON.stringify([content]), JSON.stringify([100 + options.index]), "{}", null, null, 100 + options.index, options.revision ?? 1);
}
