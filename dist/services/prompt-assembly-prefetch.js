...
export async function prefetchAssemblyData(ctx) {
    ...
    try {
        ...
        const allSettings = profiler.measureSync("settings", () => settingsSvc.getSettingsByKeys(ctx.userId, ALL_SETTINGS_KEYS));
        // ── 2. Core entities (each 1 query) ──────────────────────────────────
        const chat = profiler.measureSync("chat", () => chatsSvc.getChat(ctx.userId, ctx.chatId));
        if (!chat)
            throw new Error("Chat not found");
        const allMessages = profiler.measureSync("messages", () => chatsSvc.getMessages(ctx.userId, ctx.chatId));
        ...
        if (allMessages.length > 200) {
            await profiler.measure("post-messages-yield", () => new Promise(r => setTimeout(r, 0)));
            if (ctx.signal?.aborted)
                throw ctx.signal.reason ?? new DOMException("Aborted", "AbortError");
        }
        ...
        const character = profiler.measureSync("character", () => characterId
        ...
        let persona = profiler.measureSync("persona", () => isTemporaryChat
            ? null
            : applyPersonaAddonStates(personasSvc.resolvePersonaOrDefault(ctx.userId, ctx.personaId), ctx.personaAddonStates));
        // Resolve attached global add-ons for the persona so {{persona}} includes them
        persona = profiler.measureSync("global-addons", () => globalAddonsSvc.resolvePersonaGlobalAddons(ctx.userId, persona));
        const connection = profiler.measureSync("connection", () => connectionsSvc.resolveConnection(ctx.userId, ctx.connectionId));
        ...
        const preset = profiler.measureSync("preset", () => ctx.presetOverride ?? (resolvedPresetId ? presetsSvc.getPreset(ctx.userId, resolvedPresetId) : null));
        // ── 3. Embedding config (1 setting + 1 secret decrypt — only async op) ─
        const embeddingConfig = await profiler.measure("embedding-config", () => embeddingsSvc.getEmbeddingConfig(ctx.userId));
        if (ctx.signal?.aborted)
            throw ctx.signal.reason ?? new DOMException("Aborted", "AbortError");
        ...
        if (embeddingConfig.enabled && messages.length > 0) {
            ...
            const activeDatabankIds = resolveActiveDatabankIds(ctx.userId, ctx.chatId, databankCharIds, {
        ...
        const groupCharacterIds = isGroup ? (chat.metadata.character_ids || []) : [];
        const groupCharacters = profiler.measureSync("group-characters", () => groupCharacterIds.length > 0
            ? charactersSvc.getCharactersByIds(ctx.userId, groupCharacterIds)
            : undefined);
        // ── 5. World book entries (2 queries total) ──────────────────────────
        const globalWorldBooks = allSettings.get("globalWorldBooks") ?? [];
        const chatWorldBookIds = chat.metadata?.chat_world_book_ids ?? [];
        const worldInfoSources = profiler.measureSync("world-book-entries", () => collectWorldInfoSources(ctx.userId, character, persona, globalWorldBooks, chatWorldBookIds, { chat, groupCharacters }));
        ...
        if (worldInfoSources.entries.length > 200) {
            await profiler.measure("post-world-books-yield", () => new Promise(r => setTimeout(r, 0)));
            if (ctx.signal?.aborted)
                throw ctx.signal.reason ?? new DOMException("Aborted", "AbortError");
        }
