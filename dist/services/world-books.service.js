...
import { desiredWorldBookVectorIndexStatus, isWorldBookEntryVectorEligible, } from "./world-book-vector-state";
...
export class WorldBookEntryConflictError extends Error {
    ...
    expectedRevision;
    actualRevision;
    constructor(entryId, expectedRevision, current) {
        const actualRevision = current ? entryRevisionOf(current) : 0;
        super(`World book entry ${entryId} changed since revision ${expectedRevision}; current revision is ${actualRevision}`);
...
function isValidEntryRevision(value) {
...
function parseExpectedRevision(value, present) {
    if (!present)
        return undefined;
...
function parseExpectedRevisions(value, present) {
    if (!present)
        return undefined;
    ...
    for (const [id, rev] of Object.entries(value)) {
...
function readExpectedRevisions(input) {
...
function expectedRevisionFor(expectedRevisions, entryId) {
...
function throwEntryRevisionConflict(entryId, expectedRevision, current) {
    throw new WorldBookEntryRevisionConflictError(entryId, expectedRevision, current);
}
function assertEntryExpectedRevision(entry, expectedRevision) {
...
function readEntryById(userId, entryId) {
    return getEntry(userId, entryId);
}
function emitWorldBookChanged(userId, id) {
...
export function emitWorldBookLibraryChanged(userId, payload) {
    eventBus.emit(EventType.WORLD_BOOK_LIBRARY_CHANGED, payload, userId);
}
function emitWorldBookDeleted(userId, id) {
    eventBus.emit(EventType.WORLD_BOOK_DELETED, { id }, userId);
}
function emitWorldBookEntryChanged(userId, id) {
...
const ENTRY_OUTLET_NAME_KEYS = ["outlet_name", "outletName"];
const ENTRY_WI_MARKER_KEYS = ["wi_marker", "wiMarker"];
const ENTRY_WI_MARKER_SIDE_KEYS = ["wi_marker_side", "wiMarkerSide"];
...
function normalizeEntryOutletName(value) {
...
function normalizeEntryWiMarker(value) {
    if (typeof value !== "string")
        return null;
...
function normalizeEntryWiMarkerSide(value) {
    if (typeof value !== "string")
        return null;
    const normalized = value.trim().toLowerCase();
...
function cloneUnknownRecord(value) {
    if (!value || typeof value !== "object")
        return {};
    return JSON.parse(JSON.stringify(value));
}
function splitManagedEntryExtensions(raw) {
    ...
    for (const key of ENTRY_OUTLET_NAME_KEYS)
        delete next[key];
    for (const key of ENTRY_WI_MARKER_KEYS)
        delete next[key];
    for (const key of ENTRY_WI_MARKER_SIDE_KEYS)
        delete next[key];
    return { extensions: next, outletName, wiMarker, wiMarkerSide };
}
function buildStoredEntryExtensions(raw, outletValue, wiMarkerValue, wiMarkerSideValue) {
    const { extensions, outletName: embeddedOutletName, wiMarker: embeddedWiMarker, wiMarkerSide: embeddedWiMarkerSide, } = splitManagedEntryExtensions(raw);
...
function normalizeVectorIndexStatus(row) {
    if (row.vector_index_status === "not_enabled" ||
...
function getPendingVectorIndexState(entry) {
    return {
...
function shouldResetVectorIndex(input) {
    return (input.vectorized !== undefined ||
...
function touchWorldBook(worldBookId, timestamp = Math.floor(Date.now() / 1000)) {
    getDb().query("UPDATE world_books SET updated_at = ? WHERE id = ?").run(timestamp, worldBookId);
}
function cloneEntryExtensions(extensions) {
...
const HOST_MANAGED_ENTRY_NAMESPACE_KEYS = new Set([
    ...ENTRY_OUTLET_NAME_KEYS,
    ...ENTRY_WI_MARKER_KEYS,
    ...ENTRY_WI_MARKER_SIDE_KEYS,
]);
function entityNamespaceTarget(entity) {
    switch (entity) {
        case "world_book_entry":
            return {
                table: "world_book_entries",
                column: "extensions",
                selectSql: "SELECT e.extensions AS bag FROM world_book_entries e JOIN world_books w ON e.world_book_id = w.id WHERE e.id = ? AND w.user_id = ?",
                ownerPredicate: "EXISTS (SELECT 1 FROM world_books w WHERE w.id = world_book_entries.world_book_id AND w.user_id = ?)",
            };
        case "character":
            return {
                table: "characters",
        ...
        default:
            throw new EntityExtensionNamespaceError("INVALID_NAMESPACE", `Unsupported extension entity ${String(entity)}`);
    }
}
function parseStoredExtensionBag(raw) {
    let parsed;
    try {
        parsed = typeof raw === "string" ? JSON.parse(raw) : raw;
    }
    catch {
        throw new EntityExtensionNamespaceError("INVALID_NAMESPACE", "Stored extension metadata is not valid JSON");
    }
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        throw new EntityExtensionNamespaceError("INVALID_NAMESPACE", "Stored extension metadata must be a JSON object");
    }
    return parsed;
}
function serializeExtensionNamespaceValue(value) {
    let serialized;
    try {
    ...
    if (serialized === undefined) {
        throw new EntityExtensionNamespaceError("INVALID_NAMESPACE", "Extension namespace value must be JSON-serializable");
    }
    if (new TextEncoder().encode(serialized).byteLength > ENTITY_EXTENSION_NAMESPACE_MAX_BYTES) {
        throw new EntityExtensionNamespaceError("NAMESPACE_TOO_LARGE", "Extension namespace value exceeds the 2 MiB serialized UTF-8 limit");
    }
    return serialized;
}
function validateExtensionNamespace(namespace) {
    if (HOST_MANAGED_ENTRY_NAMESPACE_KEYS.has(namespace)) {
        throw new EntityExtensionNamespaceError("HOST_MANAGED_NAMESPACE", `Extension namespace ${JSON.stringify(namespace)} is managed by the host`);
    }
    if (!ENTITY_EXTENSION_NAMESPACE_PATTERN.test(namespace)) {
        throw new EntityExtensionNamespaceError("INVALID_NAMESPACE", `Extension namespace ${JSON.stringify(namespace)} must match ${ENTITY_EXTENSION_NAMESPACE_PATTERN}`);
    }
}
/**
 ...
 */
export function setEntityExtensionNamespace(userId, entity, entityId, namespace, value) {
    const target = entityNamespaceTarget(entity);
    const db = getDb();
    return db.transaction(() => {
        const row = db.query(target.selectSql).get(entityId, userId);
        if (!row)
            return null;
        ...
        if (result.changes === 0)
            return null;
        const updatedRow = db.query(target.selectSql).get(entityId, userId);
        if (!updatedRow)
            return null;
...
function buildSparseEntryMutation(entry, input) {
    const fields = [];
    const values = [];
    ...
    const intFields = ["position", "depth", "order_value", "group_weight", "probability", "scan_depth", "priority", "sticky", "cooldown", "delay", "selective_logic"];
    ...
    const boolFields = ["selective", "constant", "disabled", "group_override", "case_sensitive", "match_whole_words", "use_regex", "prevent_recursion", "exclude_recursion", "delay_until_recursion", "use_probability", "vectorized"];
    ...
    if (input.extensions !== undefined || input.outlet_name !== undefined || input.wi_marker !== undefined || input.wi_marker_side !== undefined) {
        ...
        values.push(buildStoredEntryExtensions(input.extensions ?? entry.extensions, input.outlet_name !== undefined ? input.outlet_name : entry.outlet_name, input.wi_marker !== undefined ? input.wi_marker : entry.wi_marker, input.wi_marker_side !== undefined ? input.wi_marker_side : entry.wi_marker_side));
    }
    return {
        fields,
        values,
        resetsVectorIndex: shouldResetVectorIndex(input),
    };
}
function importExtensionRecord(raw) {
    return raw?.extensions && typeof raw.extensions === "object" && !Array.isArray(raw.extensions)
        ? raw.extensions
        : {};
}
function importValue(raw, extensions, ...keys) {
...
export function normalizeImportedEntries(raw) {
...
export function countImportedWorldBookEntries(raw) {
...
export function normalizeImportedEntryInput(raw, index) {
    const ext = importExtensionRecord(raw);
    const keys = Array.isArray(raw.keys) ? raw.keys
        : Array.isArray(raw.key) ? raw.key
            : typeof raw.key === "string" ? raw.key.split(",").map((k) => k.trim()).filter(Boolean)
                : typeof raw.keys === "string" ? raw.keys.split(",").map((k) => k.trim()).filter(Boolean)
                    : [];
    const secondaryKeys = Array.isArray(raw.secondary_keys) ? raw.secondary_keys
        : Array.isArray(raw.keysecondary) ? raw.keysecondary
            : typeof raw.secondary_keys === "string" ? raw.secondary_keys.split(",").map((k) => k.trim()).filter(Boolean)
                : [];
...
export function materializeCharacterBookEntriesForRuntime(worldBookId, characterBook) {
...
function getEntriesForBook(userId, worldBookId, entryIds) {
    if (entryIds.length === 0)
        return [];
    const uniqueIds = [...new Set(entryIds)];
    const placeholders = uniqueIds.map(() => "?").join(", ");
    const rows = getDb().query(`SELECT e.*
     FROM world_book_entries e
     JOIN world_books w ON e.world_book_id = w.id
     WHERE w.user_id = ? AND e.world_book_id = ? AND e.id IN (${placeholders})`).all(userId, worldBookId, ...uniqueIds);
    return rows.map(rowToEntry);
}
function setEntriesPendingReindex(entries) {
    if (entries.length === 0)
        return;
    const stmt = getDb().query(`UPDATE world_book_entries
...
function deleteWorldBookVectorsAndMaybeRequeue(userId, entry, requeue) {
    ...
    void embeddingsSvc.deleteWorldBookEntryEmbeddings(userId, entry.id).catch((err) => {
        console.warn("[embeddings] Failed to remove world book entry vectors:", err);
    });
}
function queueReindexForEntries(userId, entries) {
    for (const entry of entries) {
...
function queueWorldBookEntriesForIndexing(userId, worldBookId) {
    const rows = getDb().query(`SELECT id
     FROM world_book_entries
     WHERE world_book_id = ?
       AND vectorized = 1
       AND disabled = 0
       AND length(trim(content)) > 0
       AND vector_index_status IN ('pending', 'error', 'not_enabled')`).all(worldBookId);
...
export function listWorldBooksForManifest(userId) {
    const rows = getDb().query("SELECT name, metadata, created_at FROM world_books WHERE user_id = ?").all(userId);
    return rows.map((row) => ({
        name: row.name,
        metadata: JSON.parse(row.metadata),
        created_at: row.created_at,
...
export function listWorldBooks(userId, pagination) {
    return paginatedQuery("SELECT * FROM world_books WHERE user_id = ? ORDER BY updated_at DESC", "SELECT COUNT(*) as count FROM world_books WHERE user_id = ?", [userId], pagination, rowToBook);
}
export function getWorldBook(userId, id) {
    const row = getDb().query("SELECT * FROM world_books WHERE id = ? AND user_id = ?").get(id, userId);
 ...
 * duplicates.
 */
export function findImportedCharacterBookWorldBook(userId, characterId, preferredIds = []) {
    ...
    const row = getDb().query(`SELECT *
      ...
      ORDER BY
        ...
        created_at DESC,
        id ASC
      LIMIT 1`).get(userId, characterId, ...preferred);
    return row ? rowToBook(row) : null;
}
/**
 ...
 */
export function getWorldBookListSignature(userId) {
    const row = getDb()
...
export function listSillyTavernWorldBookSourceFilenameIds(userId) {
    const rows = getDb().query(`SELECT id, json_extract(metadata, '$._lumiverse_source_filename') AS source_filename
     FROM world_books
     WHERE user_id = ?
       AND json_extract(metadata, '$.source') = 'sillytavern_migration'
       AND json_type(metadata, '$._lumiverse_source_filename') = 'text'
     ORDER BY updated_at ASC`).all(userId);
    const result = new Map();
    for (const row of rows)
        result.set(row.source_filename, row.id);
    return result;
}
export function listSillyTavernWorldBookNameIds(userId) {
    const rows = getDb().query(`SELECT id, name
     FROM world_books
     WHERE user_id = ?
       AND json_extract(metadata, '$.source') = 'sillytavern_migration'
     ORDER BY updated_at ASC`).all(userId);
...
export function updateWorldBook(userId, id, input) {
 ...
 */
export function renameWorldBookFolder(userId, oldName, newName) {
 ...
 */
export function deleteWorldBookFolder(userId, name) {
...
function getWorldBookEntryIdsForDelete(userId, worldBookIds) {
    if (worldBookIds.length === 0)
        return [];
    const placeholders = worldBookIds.map(() => "?").join(", ");
    return getDb().query(`SELECT e.id
     FROM world_book_entries e
     JOIN world_books wb ON wb.id = e.world_book_id
     WHERE wb.user_id = ? AND e.world_book_id IN (${placeholders})`).all(userId, ...worldBookIds).map((row) => row.id);
}
export async function deleteWorldBook(userId, id) {
    if (!getWorldBook(userId, id))
        return false;
    const entryIds = getWorldBookEntryIdsForDelete(userId, [id]);
    const deleted = await embeddingsSvc.deleteWorldBookEmbeddingsBeforeSourceDelete(userId, [id], entryIds, () => getDb().query("DELETE FROM world_books WHERE id = ? AND user_id = ?").run(id, userId).changes > 0);
    if (deleted)
        emitWorldBookDeleted(userId, id);
    return deleted;
}
export async function bulkDeleteWorldBooks(userId, ids) {
    const uniqueIds = dedupeWorldBookIds(ids);
    if (uniqueIds.length === 0)
        return { deleted: [] };
    const db = getDb();
    const placeholders = uniqueIds.map(() => "?").join(", ");
    const ownedIds = db.query(`SELECT id FROM world_books WHERE user_id = ? AND id IN (${placeholders})`).all(userId, ...uniqueIds).map((row) => row.id);
    ...
    const deleted = await embeddingsSvc.deleteWorldBookEmbeddingsBeforeSourceDelete(userId, orderedOwnedIds, entryIds, () => {
        const removed = [];
        db.transaction(() => {
            const stmt = db.query("DELETE FROM world_books WHERE id = ? AND user_id = ?");
            for (const id of orderedOwnedIds) {
                if (stmt.run(id, userId).changes > 0)
                    removed.push(id);
            }
        })();
        return removed;
...
export function bulkUpdateWorldBooksFolder(userId, ids, folder) {
    const uniqueIds = dedupeWorldBookIds(ids);
    ...
    db.transaction(() => {
        ...
        for (const id of uniqueIds) {
            if (stmt.run(normalizedFolder, now, id, userId).changes > 0)
                updatedIds.push(id);
        }
    })();
...
export function bulkExportWorldBooks(userId, ids, format = "lumiverse") {
    const entries = {};
...
function dedupeWorldBookIds(ids) {
    return [...new Set(ids)];
}
function sanitizeWorldBookExportFilenameBase(name, fallback) {
    const sanitized = name.replace(/[\/\\:*?"<>|\x00-\x1F\x7F]/g, "").trim();
    return sanitized || fallback;
}
function makeUniqueWorldBookExportEntryName(baseName, usedNames) {
...
function buildWorldBooksExportFilename(date = new Date()) {
    const year = date.getFullYear();
    const month = `${date.getMonth() + 1}`.padStart(2, "0");
    const day = `${date.getDate()}`.padStart(2, "0");
    return `world-books-${year}${month}${day}.zip`;
}
export async function deleteAutoManagedCharacterWorldBooks(userId, characterId) {
    const rows = getDb().query(`SELECT id
       FROM world_books
      WHERE user_id = ?
        AND json_extract(metadata, '$.auto_managed_by_character') = 1
        AND json_extract(metadata, '$.source_character_id') = ?`).all(userId, characterId);
    return (await bulkDeleteWorldBooks(userId, rows.map((row) => row.id))).deleted.length;
}
export function getWorldBookVectorSummary(userId, worldBookId) {
    const row = getDb().query(`SELECT
       COUNT(e.id) AS total,
     ...
     GROUP BY wb.id`).get(worldBookId, userId);
    return row;
}
export function setWorldBookSemanticActivation(userId, worldBookId, enabled) {
    ...
    if (enabled) {
        updatedEntries = db.query(`UPDATE world_book_entries
       SET vectorized = 1,
           ...
           revision = revision + 1
       WHERE world_book_id = ?
         AND length(trim(content)) > 0`).run(now, worldBookId).changes;
    }
    else {
        updatedEntries = db.query(`UPDATE world_book_entries
       SET vectorized = 0,
           ...
           updated_at = ?,
           revision = revision + 1
       WHERE world_book_id = ?`).run(now, worldBookId).changes;
    }
    if (updatedEntries > 0) {
...
export function getConvertToVectorizedPreview(userId, worldBookId) {
    const book = getWorldBook(userId, worldBookId);
    if (!book)
        return null;
...
export function convertToVectorized(userId, worldBookId) {
    const book = getWorldBook(userId, worldBookId);
    if (!book)
        return null;
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    const converted = db.query(`UPDATE world_book_entries
      SET vectorized = 1,
      ...
      WHERE world_book_id = ?
        ...
        AND disabled = 0
        AND length(trim(content)) > 0
        AND vectorized = 0`).run(now, worldBookId).changes;
    if (converted > 0) {
        db.query("UPDATE world_books SET updated_at = ? WHERE id = ?").run(now, worldBookId);
        queueWorldBookEntriesForIndexing(userId, worldBookId);
    }
 ...
 */
function sanitizeEntryFtsQuery(input) {
...
export function listEntriesPaginated(userId, worldBookId, pagination, options) {
    const book = getWorldBook(userId, worldBookId);
    if (!book)
        return { data: [], total: 0, limit: pagination.limit, offset: pagination.offset };
    const sortKey = options?.sortBy && options.sortBy in ENTRY_SORT_COLUMNS
        ? options.sortBy
        : "order";
    ...
    if (!rawSearch) {
        ...
        return paginatedQuery(`SELECT * FROM world_book_entries WHERE world_book_id = ? ORDER BY ${column}${collate} ${direction}, id ASC`, "SELECT COUNT(*) as count FROM world_book_entries WHERE world_book_id = ?", [worldBookId], pagination, rowToEntry);
    }
    ...
    const rows = db
        .query(`SELECT e.* FROM ${fromClause} WHERE ${whereStr} ORDER BY e.${column}${collate} ${direction}, e.id ASC LIMIT ? OFFSET ?`)
...
export function listEntries(userId, worldBookId) {
    const book = getWorldBook(userId, worldBookId);
    if (!book)
        return [];
 ...
 */
export function listEntriesForBooks(userId, bookIds, bookNameMap) {
    if (bookIds.length === 0)
        return new Map();
...
export function getEntry(userId, id) {
    const row = getDb().query("SELECT e.* FROM world_book_entries e JOIN world_books w ON e.world_book_id = w.id WHERE e.id = ? AND w.user_id = ?").get(id, userId);
    return row ? rowToEntry(row) : null;
}
export function createEntry(userId, worldBookId, input, opts) {
    const book = getWorldBook(userId, worldBookId);
    if (!book)
        return null;
    const id = crypto.randomUUID();
    ...
    const storedExtensions = buildStoredEntryExtensions(input.extensions, input.outlet_name, input.wi_marker, input.wi_marker_side);
    getDb()
        .query(`INSERT INTO world_book_entries (
        ...
        .run(id, worldBookId, uid, JSON.stringify(input.key || []), JSON.stringify(input.keysecondary || []), input.content || "", input.comment || "", input.position ?? 0, input.depth ?? 4, input.role || null, input.order_value ?? 100, input.selective ? 1 : 0, input.constant ? 1 : 0, input.disabled ? 1 : 0, input.group_name || "", input.group_override ? 1 : 0, input.group_weight ?? 100, input.probability ?? 100, input.scan_depth ?? null, input.case_sensitive ? 1 : 0, input.match_whole_words ? 1 : 0, input.automation_id || null, input.use_regex ? 1 : 0, input.prevent_recursion ? 1 : 0, input.exclude_recursion ? 1 : 0, input.delay_until_recursion ? 1 : 0, input.priority ?? 10, input.sticky ?? 0, input.cooldown ?? 0, input.delay ?? 0, input.selective_logic ?? 0, input.use_probability !== false ? 1 : 0, vectorized ? 1 : 0, vectorIndexState.vector_index_status, vectorIndexState.vector_indexed_at, vectorIndexState.vector_index_error, storedExtensions, now, now);
...
export function updateEntry(userId, id, input) {
    ...
    if (input.extensions !== undefined
        ...
        fields.push("extensions = ?");
        values.push(buildStoredEntryExtensions(input.extensions ?? existing.extensions, input.outlet_name !== undefined ? input.outlet_name : existing.outlet_name, input.wi_marker !== undefined ? input.wi_marker : existing.wi_marker, input.wi_marker_side !== undefined ? input.wi_marker_side : existing.wi_marker_side));
    }
...
export async function deleteEntry(userId, id, expectedRevision) {
    const parsedExpectedRevision = parseExpectedRevision(expectedRevision, expectedRevision !== undefined);
    // Verify the entry belongs to a world book owned by this user
    const entry = getEntry(userId, id);
    if (!entry)
        return false;
    assertEntryExpectedRevision(entry, parsedExpectedRevision);
    const deleted = await embeddingsSvc.deleteWorldBookEntryEmbeddingsBeforeSourceDelete(userId, [id], () => {
...
export function duplicateEntry(userId, entryId, input) {
...
export function reorderEntries(userId, worldBookId, orderedIds, expectedRevisions) {
    const parsedExpectedRevisions = parseExpectedRevisions(expectedRevisions, expectedRevisions !== undefined);
    const book = getWorldBook(userId, worldBookId);
    if (!book)
        return false;
    ...
    if (uniqueIds.some((id) => !entryMap.has(id)))
        return false;
    ...
    const db = getDb();
    db.transaction(() => {
        const unconditional = db.query("UPDATE world_book_entries SET order_value = ?, updated_at = ?, revision = revision + 1 WHERE id = ? AND world_book_id = ?");
        const conditional = db.query("UPDATE world_book_entries SET order_value = ?, updated_at = ?, revision = revision + 1 WHERE id = ? AND world_book_id = ? AND revision = ?");
...
export async function bulkOperateEntries(userId, worldBookId, input) {
    ...
    const now = Math.floor(Date.now() / 1000);
    const db = getDb();
    const runConditionalMutation = (entryId, sql, params) => {
    ...
    if (input.action === "move") {
        ...
        db.transaction(() => {
            orderedEntries.forEach((entry) => {
                runConditionalMutation(entry.id, `UPDATE world_book_entries
           SET world_book_id = ?, updated_at = ?, revision = revision + 1,
               vector_index_status = ?, vector_indexed_at = NULL, vector_index_error = NULL`, [targetBook.id, now, desiredWorldBookVectorIndexStatus(entry)]);
            });
            touchWorldBook(worldBookId, now);
    ...
    if (input.action === "renumber") {
        ...
        db.transaction(() => {
            orderedEntries.forEach((entry, index) => {
                ...
                runConditionalMutation(entry.id, "UPDATE world_book_entries SET order_value = ?, updated_at = ?, revision = revision + 1", [nextValue, now]);
            });
            touchWorldBook(worldBookId, now);
        })();
        emitWorldBookChanged(userId, worldBookId);
    ...
    if (input.action === "add_keyword") {
        ...
        db.transaction(() => {
            orderedEntries.forEach((entry) => {
                ...
                runConditionalMutation(entry.id, `UPDATE world_book_entries
           SET key = ?, keysecondary = ?, updated_at = ?, revision = revision + 1`, [JSON.stringify(nextPrimary), JSON.stringify(nextSecondary), now]);
            });
            touchWorldBook(worldBookId, now);
        })();
    ...
    if (input.action === "set_position") {
        ...
        db.transaction(() => {
            orderedEntries.forEach((entry) => {
                runConditionalMutation(entry.id, "UPDATE world_book_entries SET position = ?, depth = ?, updated_at = ?, revision = revision + 1", [position, position === 4 ? depth : entry.depth, now]);
            });
            touchWorldBook(worldBookId, now);
        })();
        emitWorldBookChanged(userId, worldBookId);
    ...
    if (input.action === "set_priority" || input.action === "set_depth" || input.action === "set_enabled") {
        ...
        db.transaction(() => {
            orderedEntries.forEach((entry) => {
                runConditionalMutation(entry.id, `UPDATE world_book_entries SET ${field} = ?, updated_at = ?, revision = revision + 1`, [value, now]);
            });
            touchWorldBook(worldBookId, now);
        })();
        emitWorldBookChanged(userId, worldBookId);
    ...
    if (input.action === "set_fields") {
        ...
        db.transaction(() => {
            for (const mutation of mutations) {
                runConditionalMutation(mutation.entry.id, `UPDATE world_book_entries SET ${mutation.fields.join(", ")}, updated_at = ?, revision = revision + 1`, [...mutation.values, now]);
            }
            touchWorldBook(worldBookId, now);
        })();
        for (const mutation of mutations) {
    ...
    if (input.action === "copy") {
        ...
        db.transaction(() => {
            for (const entry of orderedEntries) {
                ...
                if (!next)
                    throw new Error("Unable to copy entry");
                copied.push(next);
            }
        })();
        for (const entry of copied)
            emitWorldBookEntryChanged(userId, entry.id);
        emitWorldBookChanged(userId, targetBook.id);
 ...
 */
function resolveImportOrder(raw, index) {
    // displayIndex — SillyTavern's visual ordering (most reliable for user intent)
    if (raw.displayIndex !== undefined && raw.displayIndex !== null)
        return raw.displayIndex;
...
function bulkInsertEntries(worldBookId, inputs, options = {}) {
    ...
    const insert = db.query(`INSERT INTO world_book_entries (
      id, world_book_id, uid, key, keysecondary, content, comment,
      position, depth, role, order_value, selective, constant, disabled,
      group_name, group_override, group_weight, probability, scan_depth,
      case_sensitive, match_whole_words, automation_id,
    ...
    for (let start = 0; start < inputs.length; start += chunkSize) {
        ...
        const tx = db.transaction(() => {
            for (let i = start; i < end; i++) {
                ...
                const extensionsJson = buildStoredEntryExtensions(input.extensions, input.outlet_name, input.wi_marker, input.wi_marker_side);
                insert.run(id, worldBookId, uid, JSON.stringify(input.key || []), JSON.stringify(input.keysecondary || []), input.content || "", input.comment || "", input.position ?? 0, input.depth ?? 4, input.role || null, input.order_value ?? 100, input.selective ? 1 : 0, input.constant ? 1 : 0, input.disabled ? 1 : 0, input.group_name || "", input.group_override ? 1 : 0, input.group_weight ?? 100, input.probability ?? 100, input.scan_depth ?? null, input.case_sensitive ? 1 : 0, input.match_whole_words ? 1 : 0, input.automation_id || null, input.use_regex ? 1 : 0, input.prevent_recursion ? 1 : 0, input.exclude_recursion ? 1 : 0, input.delay_until_recursion ? 1 : 0, input.priority ?? 10, input.sticky ?? 0, input.cooldown ?? 0, input.delay ?? 0, input.selective_logic ?? 0, input.use_probability !== false ? 1 : 0, vectorized ? 1 : 0, vectorIndexState.vector_index_status, vectorIndexState.vector_indexed_at, vectorIndexState.vector_index_error, extensionsJson, now, now);
                insertedIds.push(id);
                if (vectorIndexState.vector_index_status === "pending")
...
function queueVectorizationsBatch(userId, ids) {
    for (const id of ids) {
        vectorizationQueue.queueWorldBookEntryVectorization(userId, id);
    }
}
// --- World Book Import (standalone JSON) ---
export function importWorldBook(userId, payload, options = {}) {
...
export async function importWorldBookBulk(userId, payload, options = {}) {
    const bookName = payload.name || payload.originalName || "Imported World Book";
    const description = payload.description || "";
    const worldBook = createWorldBook(userId, {
        name: bookName,
...
export function importCharacterBook(userId, characterId, characterName, characterBook, options = {}) {
...
export function importLumiverseWorldBook(userId, characterId, data, options = {}) {
...
export function exportWorldBook(userId, worldBookId, format = "lumiverse") {
    const book = getWorldBook(userId, worldBookId);
    if (!book)
        return null;
    const entries = listEntries(userId, worldBookId);
...
function exportLumiverse(book, entries) {
    return {
        version: 1,
...
function entryToCharacterBookSpec(entry, index) {
    const extensions = {
        ...entry.extensions,
        priority: entry.priority,
        sticky: entry.sticky,
    ...
    if (entry.outlet_name)
        extensions.outlet_name = entry.outlet_name;
    if (entry.wi_marker)
        extensions.wi_marker = entry.wi_marker;
    if (entry.wi_marker_side)
        extensions.wi_marker_side = entry.wi_marker_side;
    return {
        id: index,
        keys: entry.key,
...
function exportCharacterBookFormat(book, entries) {
    return {
        name: book.name,
        description: book.description,
...
function exportSillyTavern(book, entries) {
    return {
        name: book.name,
        description: book.description,
        entries: Object.fromEntries(entries.map((entry, i) => [
            String(i),
            {
                uid: entry.uid,
                keys: entry.key,
