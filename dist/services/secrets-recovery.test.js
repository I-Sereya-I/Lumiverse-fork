...
import { SecretDecryptionError, __test__, isSecretDecryptionError, isSecretDecryptionFailure, } from "./secrets.service";
const spies = [];
afterEach(() => {
...
describe("secret decryption recovery", () => {
    test("wraps WebCrypto authentication failures with an actionable runtime error", () => {
        const operationError = new DOMException("The operation failed for an operation-specific reason", "OperationError");
        ...
        expect(normalized.message).toContain("cannot be decrypted");
        expect(normalized.message).toContain("replace the credential in Settings");
    });
    test("treats unreadable status credentials as missing and warns only once", async () => {
        const warn = spyOn(console, "warn").mockImplementation(() => { });
        spies.push(warn);
        const read = () => Promise.reject(new DOMException("The operation failed for an operation-specific reason", "OperationError"));
        await expect(__test__.recoverUnreadableSecretForStatus("broken-user", "connection_api_key", read)).resolves.toBeNull();
        await expect(__test__.recoverUnreadableSecretForStatus("broken-user", "connection_api_key", read)).resolves.toBeNull();
    ...
    test("does not hide unrelated secret-store failures", async () => {
        await expect(__test__.recoverUnreadableSecretForStatus("database-error-user", "connection_api_key", () => Promise.reject(new Error("database is locked")))).rejects.toThrow("database is locked");
    });
