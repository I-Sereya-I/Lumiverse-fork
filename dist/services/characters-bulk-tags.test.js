...
function getTags(userId, id) {
    const row = getDb().query("SELECT tags FROM characters WHERE id = ? AND user_id = ?").get(id, userId);
    try {
...
describe("bulkUpdateCharacterTags", () => {
    ...
    test("rejects empty ids / bad operation / empty tags", () => {
        ...
        expect(() => bulkUpdateCharacterTags(USER_A, { ids: [a.id], operation: "bad", tags: ["x"] })).toThrow();
