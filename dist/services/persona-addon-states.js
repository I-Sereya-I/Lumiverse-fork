...
function sanitizeAddonStates(addonStates) {
    if (!addonStates || typeof addonStates !== "object")
        return undefined;
    const entries = Object.entries(addonStates).filter(([id, enabled]) => !!id && typeof enabled === "boolean");
    return entries.length > 0 ? Object.fromEntries(entries) : undefined;
}
function sanitizeAddonToggleOrder(order) {
...
export function getChatPersonaAddonStates(metadata, personaId) {
    if (!personaId)
        return undefined;
    const statesByPersona = metadata?.persona_addon_states;
    if (!statesByPersona || typeof statesByPersona !== "object")
        return undefined;
 ...
 */
export function getChatPersonaAddonToggleOrder(metadata, personaId) {
    if (!personaId)
        return undefined;
 ...
 */
export function withChatPersonaAddonState(metadata, personaId, addonId, enabled) {
...
export function applyPersonaAddonStates(persona, addonStates) {
    const states = sanitizeAddonStates(addonStates);
    if (!persona || !states)
        return persona;
    const metadata = persona.metadata || {};
    const addons = Array.isArray(metadata.addons)
        ? metadata.addons.map((addon) => (addon?.id in states ? { ...addon, enabled: states[addon.id] } : addon))
        : metadata.addons;
    const attachedGlobalAddons = Array.isArray(metadata.attached_global_addons)
        ? metadata.attached_global_addons.map((ref) => (ref?.id in states ? { ...ref, enabled: states[ref.id] } : ref))
...
export function personaHasAddon(persona, addonId) {
    if (!persona || !addonId)
        return false;
    const metadata = persona.metadata ?? {};
    return [metadata.addons, metadata.attached_global_addons].some((addons) => Array.isArray(addons) && addons.some((addon) => addon?.id === addonId));
}
/**
 ...
 */
export function resolvePersonaAvatarInfo(persona, addonStates, addonToggleOrder) {
    if (!persona)
        return null;
    const metadata = persona.metadata ?? {};
    const states = sanitizeAddonStates(addonStates) ?? {};
 ...
 */
export function resolvePersonaForChatMacros(userId, persona, chatMetadata) {
