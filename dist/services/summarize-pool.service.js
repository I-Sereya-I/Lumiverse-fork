...
export function startSummarizePool(opts) {
    ...
    eventBus.emit(EventType.SUMMARIZATION_STARTED, { chatId: opts.chatId, generationId: opts.generationId, startedAt: entry.startedAt }, opts.userId);
}
/**
...
export function emitSummarizationProgress(opts) {
    eventBus.emit(EventType.SUMMARIZATION_PROGRESS, {
        chatId: opts.chatId,
        generationId: opts.generationId,
...
export function completeSummarizePool(opts) {
    ...
    eventBus.emit(EventType.SUMMARIZATION_COMPLETED, { chatId: opts.chatId, generationId: opts.generationId }, opts.userId);
}
/**
...
export function failSummarizePool(opts) {
    ...
    eventBus.emit(EventType.SUMMARIZATION_FAILED, { chatId: opts.chatId, generationId: opts.generationId, error: opts.error }, opts.userId);
}
/**
 * Look up the current in-flight summary for (userId, chatId), if any.
 */
export function getSummarizePoolEntry(userId, chatId) {
...
function sweep() {
    ...
    for (const [k, entry] of pool) {
        if (now - entry.startedAt > STALE_TTL_MS) {
            ...
            eventBus.emit(EventType.SUMMARIZATION_FAILED, { chatId: entry.chatId, generationId: entry.generationId, error: "Summary timed out" }, entry.userId);
        }
    }
}
let sweepTimer = null;
export function startSummarizePoolSweep() {
    if (!sweepTimer) {
        sweepTimer = setInterval(sweep, SWEEP_INTERVAL_MS);
    }
}
export function stopSummarizePoolSweep() {
    if (sweepTimer) {
