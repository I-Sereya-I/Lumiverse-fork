 ...
 * nothing to prune.
 */
export function pruneOrphanedWiState(userId, chat) {
    ...
    const globalBookIds = settingsSvc.getSetting(userId, "globalWorldBooks")?.value ?? [];
    const allBookIds = new Set();
    for (const id of charBookIds)
        allBookIds.add(id);
    if (persona?.attached_world_book_id)
        allBookIds.add(persona.attached_world_book_id);
    for (const id of chatBookIds)
        allBookIds.add(id);
    for (const id of globalBookIds)
        allBookIds.add(id);
