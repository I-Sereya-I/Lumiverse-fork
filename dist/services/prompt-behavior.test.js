...
import { DEFAULT_PROMPT_BEHAVIOR, isGenuinelyNewChat, resolveNewChatPromptConfig, resolvePromptBehavior, shouldInjectEmptySendNudge, shouldInjectGroupNudge, } from "./prompt-behavior";
...
describe("Loom prompt behavior", () => {
    test("resolves partial and malformed persisted behavior without overriding explicit opt-outs", () => {
        ...
        expect(resolved.continueNudge).toBe(DEFAULT_PROMPT_BEHAVIOR.continueNudge);
        ...
        expect(resolvePromptBehavior({ newChatPrompt: "[Legacy start]" })
            .newGroupChatPrompt).toBe("[Legacy start]");
    });
    test("recognizes a new chat only until the first non-greeting assistant reply", () => {
        expect(isGenuinelyNewChat([
    ...
    test("injects an empty-send nudge for an assistant-ending chat regardless of user-role preset blocks", () => {
        ...
        expect(shouldInjectEmptySendNudge({
