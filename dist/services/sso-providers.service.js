...
import { env } from "../env";
const VALID_KINDS = new Set(["authelia", "authentik", "keycloak", "custom_oidc"]);
...
let activeProviderRedirectUris = new Map();
...
function decryptSecret(row) {
    if (!row.encrypted_client_secret || !row.client_secret_iv || !row.client_secret_tag)
        return null;
    const decipher = createDecipheriv("aes-256-gcm", encryptionKey(), Buffer.from(row.client_secret_iv, "base64"));
...
function getProviderMetadata(row) {
    return parseJsonObject(row.metadata);
}
function getProviderRedirectOrigin(row) {
...
function parseScopes(value) {
    try {
        ...
        if (Array.isArray(parsed))
            return sanitizeScopes(parsed);
    }
    catch { }
    return [...DEFAULT_SCOPES];
}
function sanitizeScopes(value) {
...
function normalizeUrl(value, field, required = false) {
    if (value === undefined || value === null || value === "") {
        if (required)
            throw new InvalidSsoProviderError(`${field} is required`);
        return "";
    }
    if (typeof value !== "string")
        throw new InvalidSsoProviderError(`${field} must be a string`);
    ...
    if (trimmed.length > MAX_URL_LENGTH)
        throw new InvalidSsoProviderError(`${field} is too long`);
    let url;
    try {
        url = new URL(trimmed);
    }
...
function deriveDiscoveryUrl(issuerUrl, explicitDiscoveryUrl) {
    if (explicitDiscoveryUrl)
        return explicitDiscoveryUrl;
    if (!issuerUrl)
        return "";
...
function normalizeSlug(value) {
    if (typeof value !== "string")
        throw new InvalidSsoProviderError("slug is required");
...
function getRow(id) {
    return getDb().query("SELECT * FROM sso_providers WHERE id = ?").get(id);
}
export function getRedirectUri(slug, origin = process.env.AUTH_BASE_URL || `http://localhost:${env.port}`) {
...
function normalizeMetadata(input, existing) {
    ...
    if (redirectOrigin)
        metadata.redirect_origin = redirectOrigin;
    else
        delete metadata.redirect_origin;
    return JSON.stringify(metadata);
}
export function listSsoProviders() {
    const rows = getDb().query("SELECT * FROM sso_providers ORDER BY created_at DESC, name COLLATE NOCASE").all();
    return rows.map(publicFromRow);
}
export function listSsoLoginOptions() {
    const rows = getDb()
...
export function getSsoProvider(id) {
    const row = getRow(id);
...
export function createSsoProvider(input) {
    ...
    if (!name || name.length > MAX_NAME_LENGTH)
        throw new InvalidSsoProviderError("name is required and must be 1-80 chars");
    ...
    if (!clientId || clientId.length > MAX_CLIENT_ID_LENGTH)
        throw new InvalidSsoProviderError("client_id is required and must be 1-512 chars");
    ...
    if (!secret || secret.length > MAX_SECRET_LENGTH)
        throw new InvalidSsoProviderError("client_secret is required and must be 1-4096 chars");
    const encrypted = encryptSecret(secret);
    const id = crypto.randomUUID();
    const now = nowSec();
    const scopes = JSON.stringify(sanitizeScopes(input?.scopes));
    const metadata = normalizeMetadata(input);
    getDb().query(`INSERT INTO sso_providers (
    ...
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, kind, name, slug, input?.enabled ? 1 : 0, issuerUrl, discoveryUrl, clientId, encrypted.encrypted, encrypted.iv, encrypted.tag, scopes, input?.pkce === false ? 0 : 1, input?.allow_signup ? 1 : 0, metadata, now, now);
    return publicFromRow(getRow(id));
}
export function updateSsoProvider(id, input) {
    ...
    if (typeof nextSecret === "string" && nextSecret.length > 0) {
        if (nextSecret.length > MAX_SECRET_LENGTH)
            throw new InvalidSsoProviderError("client_secret must be 1-4096 chars");
    ...
    const now = nowSec();
    getDb().query(`UPDATE sso_providers SET
      provider_kind = ?, name = ?, slug = ?, enabled = ?, issuer_url = ?, discovery_url = ?, client_id = ?,
      encrypted_client_secret = ?, client_secret_iv = ?, client_secret_tag = ?, scopes = ?, pkce = ?,
      allow_signup = ?, metadata = ?, updated_at = ?
     WHERE id = ?`).run(kind, name, slug, enabled, issuerUrl, discoveryUrl, clientId, encryptedClientSecret, clientSecretIv, clientSecretTag, scopes, pkce, allowSignup, metadata, now, id);
    return publicFromRow(getRow(id));
}
export function deleteSsoProvider(id) {
    return getDb().query("DELETE FROM sso_providers WHERE id = ?").run(id).changes > 0;
}
export function listEnabledSsoAuthConfigs() {
    const rows = getDb().query("SELECT * FROM sso_providers WHERE enabled = 1 ORDER BY created_at ASC").all();
    const configs = [];
    for (const row of rows) {
        const secret = decryptSecret(row);
        if (!row.slug || !row.client_id || !row.discovery_url || !secret)
            continue;
        configs.push({
            providerId: row.slug,
...
export function listSsoUserLinks() {
    const rows = getDb().query(`SELECT
     ...
     ORDER BY u.role = 'owner' DESC, u.username COLLATE NOCASE, p.name COLLATE NOCASE`).all();
    return rows.map((row) => ({
        user_id: row.user_id,
...
export function listCurrentUserSsoLinks(userId) {
    return listSsoUserLinks().filter((link) => link.user_id === userId);
}
export function unlinkCurrentUserSsoProvider(userId, providerId) {
...
export function getSsoRecoveryStatus() {
...
export async function testDiscovery(input) {
    ...
    if (!res.ok)
        return { ok: false, error: `Discovery returned ${res.status}` };
    ...
    return {
        ...
        scopes_supported: Array.isArray(body.scopes_supported) ? body.scopes_supported.filter((s) => typeof s === "string") : undefined,
    };
}
