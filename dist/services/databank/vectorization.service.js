...
export function abortDocumentProcessing(docId) {
...
export function abortDatabankProcessing(databankId) {
 ...
 */
export async function processDocument(userId, docId) {
    ...
    try {
        ...
        if (isProcessingAborted(docId, controller.signal))
            return;
        if (!parsed.text.trim()) {
...
function trackActiveDocument(docId, activeRun) {
    const activeRuns = activeDocuments.get(docId);
    if (activeRuns) {
...
function untrackActiveDocument(docId, activeRun) {
    const activeRuns = activeDocuments.get(docId);
    if (!activeRuns)
        return;
...
function isProcessingAborted(docId, signal) {
    if (signal.aborted) {
        console.info(`[databank] Document ${docId} deleted during processing; aborting cleanly`);
        return true;
    }
...
function isProcessingAbortError(err) {
 ...
 */
function redactProcessingError(err) {
    const msg = (err instanceof Error ? err.message : String(err)) || "";
    if (/embedding|vectoriz/i.test(msg))
        return "Embedding step failed";
    if (/parse|chunk/i.test(msg))
        return "Document parsing failed";
    if (/timeout|abort/i.test(msg))
        return "Processing timed out";
    if (/permission|EACCES|ENOENT/i.test(msg))
        return "Filesystem error";
    if (/quota|rate limit/i.test(msg))
        return "Provider quota exceeded";
    return "Document processing failed";
}
async function vectorizeChunks(userId, doc, chunks, cfg, signal) {
    const failures = [];
    await embeddingsSvc.embedWithAdaptiveBatching(userId, chunks, 
    ...
    Math.max(1, Math.min(cfg.batch_size, 200)), (c) => c.content, async (batch, _texts, vectors) => {
 ...
 */
export async function deleteDatabankVectors(userId, databankId) {
    await embeddingsSvc.deleteDatabankEmbeddings(userId, databankId);
}
function emitStatus(userId, doc, status, error, totalChunks) {
