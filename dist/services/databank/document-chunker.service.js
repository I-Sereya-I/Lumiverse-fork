...
function chunkSection(text, sectionHeader, baseOffset, target, max, overlap) {
    ...
    const flushChunk = () => {
        ...
        for (let i = currentParts.length - 1; i >= 0; i--) {
            ...
            if (overlapCount + partTokens > overlap)
                break;
...
function splitLargeParagraph(text, target, max, overlap, baseOffset, sectionHeader) {
