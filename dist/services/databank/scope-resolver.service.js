 ...
 */
export function resolveActiveDatabankIds(userId, chatId, characterIds, crossRefs) {
    const db = getDb();
    ...
    if (charIds.length === 0) {
        const rows = db
            .query(`SELECT id FROM databanks
    ...
    if (crossRefs?.characterDatabankIds) {
        for (const id of crossRefs.characterDatabankIds)
            allIds.add(id);
    }
    // 4. Chat cross-refs: chat.metadata.chat_databank_ids
    if (crossRefs?.chatDatabankIds) {
        for (const id of crossRefs.chatDatabankIds)
            allIds.add(id);
    }
