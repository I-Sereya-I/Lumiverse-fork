 ...
 */
export async function parseDocument(userId, filePath) {
...
function parseCsv(raw, delimiter) {
    ...
    if (records.length === 0)
        return { text: "", metadata: { format: "csv", rows: 0 } };
...
function parseXml(raw) {
    try {
        const root = Bun.XML.parse(raw, { compact: false });
        const textParts = [];
...
function collectXmlText(root, output) {
    const pending = [root];
...
function decodeRtfHex(_match, hex) {
...
function parseRtf(raw) {
    if (raw.length > MAX_RTF_INPUT_BYTES) {
        throw new Error(`RTF document exceeds parser cap (${MAX_RTF_INPUT_BYTES} bytes)`);
    }
