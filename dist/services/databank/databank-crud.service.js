...
import { rowToDatabank, rowToDocument, rowToChunk, nameToSlug, } from "./types";
// ─── Banks ────────────────────────────────────────────────────
export function createDatabank(userId, input) {
    const id = crypto.randomUUID();
    const now = Math.floor(Date.now() / 1000);
    const db = getDb();
    db.run(`INSERT INTO databanks (id, user_id, name, description, scope, scope_id, enabled, metadata, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, 1, '{}', ?, ?)`, [id, userId, input.name, input.description ?? "", input.scope, input.scopeId ?? null, now, now]);
    const bank = getDatabank(userId, id);
    eventBus.emit(EventType.DATABANK_CHANGED, { databankId: bank.id, databank: bank }, userId);
    return bank;
}
export function listDatabanks(userId, pagination, filters) {
    ...
    return paginatedQuery(dataSql, countSql, params, pagination, (row) => ({ ...rowToDatabank(row), documentCount: row.doc_count }));
}
export function getDatabank(userId, id) {
    const row = getDb()
        .query(`SELECT d.*, (SELECT COUNT(*) FROM databank_documents dd WHERE dd.databank_id = d.id) AS doc_count
...
export function updateDatabank(userId, id, input) {
    const now = Math.floor(Date.now() / 1000);
...
export async function deleteDatabank(userId, id) {
    const db = getDb();
 ...
 */
export function ensureChatDatabank(userId, chatId, chatName) {
    const db = getDb();
    const existing = db
        .query(`SELECT d.*, (SELECT COUNT(*) FROM databank_documents dd WHERE dd.databank_id = d.id) AS doc_count
...
// ─── Documents ────────────────────────────────────────────────
export function createDocument(userId, databankId, name, filePath, mimeType, fileSize, contentHash) {
    const id = crypto.randomUUID();
    const now = Math.floor(Date.now() / 1000);
    ...
    db.run(`INSERT INTO databank_documents
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'pending', '{}', ?, ?)`, [id, databankId, userId, name, slug, filePath, mimeType, fileSize, contentHash, now, now]);
    const document = getDocument(userId, id);
    invalidateDatabankCache(userId, databankId);
    return document;
}
export function listDocuments(userId, databankId, pagination) {
    return paginatedQuery(`SELECT * FROM databank_documents WHERE databank_id = ? AND user_id = ? ORDER BY created_at DESC`, `SELECT COUNT(*) AS count FROM databank_documents WHERE databank_id = ? AND user_id = ?`, [databankId, userId], pagination, rowToDocument);
}
/**
 ...
 * don't count.
 */
export function hasSearchableChunks(userId, databankIds) {
    if (databankIds.length === 0)
        return false;
    const placeholders = databankIds.map(() => "?").join(", ");
    const row = getDb()
        .query(`SELECT 1 AS present FROM databank_documents
       WHERE user_id = ? AND status = 'ready' AND total_chunks > 0
         AND databank_id IN (${placeholders})
       LIMIT 1`)
        .get(userId, ...databankIds);
    return row !== null;
}
export function renameDocument(userId, id, newName) {
    const now = Math.floor(Date.now() / 1000);
    const slug = nameToSlug(newName);
    getDb().run("UPDATE databank_documents SET name = ?, slug = ?, updated_at = ? WHERE id = ? AND user_id = ?", [newName, slug, now, id, userId]);
    const document = getDocument(userId, id);
    if (document)
        invalidateDatabankCache(userId, document.databankId);
    return document;
}
...
export function getDocumentBySlug(userId, slug) {
    const row = getDb()
        .query("SELECT * FROM databank_documents WHERE user_id = ? AND slug = ? AND status = 'ready' LIMIT 1")
        .get(userId, slug);
    return row ? rowToDocument(row) : null;
}
export function searchDocumentsBySlug(userId, partialSlug, databankIds, limit = 10) {
    const db = getDb();
 ...
 */
export function updateDocumentFile(userId, docId, filePath, mimeType, fileSize, contentHash) {
    const document = getDocument(userId, docId);
    const now = Math.floor(Date.now() / 1000);
    getDb().run(`UPDATE databank_documents
     SET file_path = ?, mime_type = ?, file_size = ?, content_hash = ?, error_message = NULL, updated_at = ?
     WHERE id = ? AND user_id = ?`, [filePath, mimeType, fileSize, contentHash, now, docId, userId]);
    if (document)
        invalidateDatabankCache(userId, document.databankId);
}
export function updateDocumentStatus(docId, status, extra) {
    ...
    if (document)
        invalidateDatabankCache(document.user_id, document.databank_id);
}
// ─── Chunks ───────────────────────────────────────────────────
export function insertChunks(chunks) {
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    const stmt = db.query(`INSERT INTO databank_chunks (id, document_id, databank_id, user_id, chunk_index, content, token_count, metadata, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
...
export function getChunksForDocument(docId) {
    const rows = getDb()
...
export function updateChunkVectorization(chunkIds, vectorModel) {
    if (chunkIds.length === 0)
        return;
    const db = getDb();
    const now = Math.floor(Date.now() / 1000);
    const placeholders = chunkIds.map(() => "?").join(",");
    db.run(`UPDATE databank_chunks SET vectorized_at = ?, vector_model = ? WHERE id IN (${placeholders})`, [now, vectorModel, ...chunkIds]);
}
export function deleteChunksForDocument(docId) {
    getDb().run("DELETE FROM databank_chunks WHERE document_id = ?", [docId]);
}
export function getDocumentContent(userId, docId) {
    const rows = getDb()
 ...
 */
export function getFullDocumentText(userId, docId) {
    const rows = getDb()
        .query("SELECT content FROM databank_chunks WHERE document_id = ? AND user_id = ? ORDER BY chunk_index ASC")
        .all(docId, userId);
    if (rows.length === 0)
