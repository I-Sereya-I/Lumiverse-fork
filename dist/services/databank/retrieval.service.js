...
export { databankCacheKey, getCachedDatabankResult, clearCache, invalidateDatabankCache, } from "./retrieval-cache.service";
// ─── Search ───────────────────────────────────────────────────
 ...
 */
export async function searchDatabanks(userId, chatId, databankIds, queryText, limit = 4, signal, onTiming) {
    ...
    try {
        ...
        const result = { chunks, formatted, count: chunks.length };
 ...
 */
export async function searchDirect(userId, databankIds, query, limit = 8) {
    if (databankIds.length === 0)
        return [];
