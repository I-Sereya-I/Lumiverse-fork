...
export async function fuseDatabanks(userId, targetId, sourceId) {
    ...
    if (!target)
        throw new FuseError("not_found", "Target databank not found");
    const source = getDatabank(userId, sourceId);
    if (!source)
        throw new FuseError("not_found", "Source databank not found");
    ...
    const targetHashRows = db
        ...
        .all(targetId, userId);
    const targetHashes = new Set(targetHashRows.map((r) => r.content_hash).filter((h) => Boolean(h)));
    const toMove = [];
    const toSkip = [];
    ...
    if (toMove.length > 0) {
        ...
        movedChunkIds = db
            .query(`SELECT id FROM databank_chunks WHERE document_id IN (${placeholders}) AND user_id = ?`)
            .all(...docIds, userId).map((r) => r.id);
        const tx = db.transaction(() => {
            db.run(`UPDATE databank_documents SET databank_id = ?, updated_at = ? WHERE id IN (${placeholders}) AND user_id = ?`, [targetId, now, ...docIds, userId]);
            db.run(`UPDATE databank_chunks SET databank_id = ? WHERE document_id IN (${placeholders}) AND user_id = ?`, [targetId, ...docIds, userId]);
        });
        tx();
    }
    ...
    eventBus.emit(EventType.DATABANK_CHANGED, { databankId: refreshedTarget.id, databank: refreshedTarget }, userId);
    return {
...
function rewriteCharacterBindings(userId, sourceId, targetId) {
    const db = getDb();
    const rows = db
        .query(`SELECT id, extensions FROM characters
    ...
    for (const row of rows) {
        ...
        if (!Array.isArray(ids) || !ids.includes(sourceId))
            continue;
        ...
        db.run(`UPDATE characters SET extensions = ?, updated_at = ? WHERE id = ? AND user_id = ?`, [JSON.stringify(updatedExt), now, row.id, userId]);
    }
}
function rewriteChatBindings(userId, sourceId, targetId) {
    const db = getDb();
    const rows = db
        .query(`SELECT id, metadata FROM chats
    ...
    for (const row of rows) {
        ...
        db.run(`UPDATE chats SET metadata = ? WHERE id = ? AND user_id = ?`, [JSON.stringify(updatedMeta), row.id, userId]);
    }
}
function rewriteGlobalSettingBindings(userId, sourceId, targetId) {
