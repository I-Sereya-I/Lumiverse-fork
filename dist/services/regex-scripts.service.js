...
import { regexCollectSandboxed, regexCaptureReplacementsSandboxed, regexReplaceSandboxed, regexTestSandboxed, RegexTimeoutError, } from "../utils/regex-sandbox";
import { substituteRegexCaptures as substituteRegexCapturesCore } from "../utils/regex-sandbox-core";
import { buildRegexActionCaptureTemplate, decorateRegexActionReplacements, } from "../utils/regex-actions";
const REGEX_SCRIPT_TIMEOUT_MS = 500;
const REGEX_SLOW_WARNING_MS = 5_000;
const REGEX_PERFORMANCE_ENGINE_VERSION = 2;
export function hasRegexMatchAction(scripts, action) {
    return scripts.some((script) => Array.isArray(script.metadata?.match_actions)
...
function getPresetRegexSettingKey(presetId) {
    return `${PRESET_REGEX_ENABLED_SETTING_PREFIX}${presetId}`;
}
function normalizeStoredPresetRegexIds(value) {
...
function readStoredPresetRegexIdsRecord(userId, presetId) {
    const row = getDb()
        .query("SELECT value FROM settings WHERE key = ? AND user_id = ?")
        .get(getPresetRegexSettingKey(presetId), userId);
    if (!row)
...
function writeStoredPresetRegexIdsWithDb(db, userId, presetId, ids) {
...
function updateStoredPresetRegexIds(userId, presetId, updater) {
    const db = getDb();
...
function deleteStoredPresetRegexIds(userId, presetId) {
    getDb().query("DELETE FROM settings WHERE key = ? AND user_id = ?").run(getPresetRegexSettingKey(presetId), userId);
}
function setPresetBoundScriptEnabledInRestoreList(userId, presetId, scriptId, enabled) {
...
function getRegexPerformanceMetadata(script) {
    ...
    if (typeof value.version !== "number")
        return null;
    if (value.engine_version !== REGEX_PERFORMANCE_ENGINE_VERSION)
        return null;
    return {
        slow: true,
        ...
        source: value.source || "display_backend",
        version: value.version,
        engine_version: value.engine_version,
    };
}
function withoutRegexPerformanceMetadata(metadata) {
...
function shouldResetRegexPerformance(input) {
    return [
        "find_regex",
        "replace_string",
        "flags",
...
function withoutStaleRegexEvidence(metadata) {
...
function isDisplayPerformanceSource(source) {
    return source === "display_client" || source === "display_backend";
}
function performanceSourcesMatch(existing, current) {
    return existing === current || (isDisplayPerformanceSource(existing) && isDisplayPerformanceSource(current));
}
export function reportRegexScriptPerformance(userId, id, issue) {
    const script = getRegexScript(userId, id);
    if (!script)
        return { script: null, newlyFlagged: false, cleared: false };
    ...
    if (!timedOut && issue.elapsedMs < thresholdMs) {
        ...
        if (existing &&
            ...
            getDb().query("UPDATE regex_scripts SET metadata = ? WHERE id = ? AND user_id = ?").run(JSON.stringify(withoutRegexPerformanceMetadata(script.metadata)), id, userId);
    ...
    getDb().query("UPDATE regex_scripts SET metadata = ? WHERE id = ? AND user_id = ?").run(JSON.stringify(nextMetadata), id, userId);
    emitRegexChanged(userId, id);
...
export function reportRegexScriptEvidence(userId, id, patch) {
    const script = getRegexScript(userId, id);
    if (!script)
        return null;
    const metadata = script.metadata && typeof script.metadata === "object" ? { ...script.metadata } : {};
    const evidence = metadata.regex_evidence && typeof metadata.regex_evidence === "object" ? { ...metadata.regex_evidence } : {};
    ...
    getDb().query("UPDATE regex_scripts SET metadata = ? WHERE id = ? AND user_id = ?").run(JSON.stringify(metadata), id, userId);
    emitRegexChanged(userId, id);
    return getRegexScript(userId, id);
}
function resolveCreateDisabledState(input, activePresetId) {
...
function applyPresetBoundActivationWithDb(db, userId, targetPresetId) {
    const rows = db
...
export function rowToRegexScript(row) {
    let target;
    try {
...
export function normalizeRegexActions(value) {
    if (!Array.isArray(value))
        return [];
    const actions = [];
    for (const raw of value.slice(0, MAX_REGEX_ACTIONS)) {
        ...
        if (type === "effects" && compatibleEffects.length === 0)
            continue;
        actions.push({
            id,
            type,
...
function sanitizeRegexPatternForValidation(pattern) {
    return pattern
        .replace(/\{\{[\s\S]*?\}\}/g, "x")
        .replace(/<USER>|<BOT>|<CHAR>/g, "x");
}
function validateRegex(pattern, flags, substituteMacros = "none") {
...
function validateInput(input, isCreate) {
    if (isCreate) {
        const ci = input;
        if (!ci.name?.trim())
            return "name is required";
        if (ci.find_regex === undefined || ci.find_regex === null)
            return "find_regex is required";
    }
    ...
    if (input.actions !== undefined) {
        ...
        if (input.actions.length > MAX_REGEX_ACTIONS)
            return `actions exceeds maximum length (${MAX_REGEX_ACTIONS})`;
        const ids = new Set();
        for (const action of input.actions) {
            if (!action || typeof action !== "object")
                return "actions contains an invalid entry";
            if (!REGEX_ACTION_ID_RE.test(action.id?.trim?.() ?? "")) {
                return "action id must start with a letter and contain only letters, numbers, _, :, . or -";
            }
            if (ids.has(action.id.trim()))
                return `duplicate action id: ${action.id.trim()}`;
            ...
            for (const field of [action.title, action.subtitle, action.content, action.cost ?? "1", action.limit ?? "3"]) {
                if (typeof field !== "string")
                    return "action title, subtitle, content, cost, and limit must be strings";
                if (field.length > MAX_REGEX_ACTION_FIELD_LENGTH)
                    return "action field exceeds maximum length";
            }
            if (action.type !== "effects" && !action.content.trim())
                return `action content is required: ${action.id}`;
            if (action.type === "effects" && action.multi_select)
                return "effects-only actions cannot be multi-select";
            ...
            if (action.effects !== undefined) {
                if (!Array.isArray(action.effects))
                    return "action effects must be an array";
                if (action.effects.length > 16)
                    return "action effects exceeds maximum length (16)";
                for (const effect of action.effects) {
                    ...
                    if (effect.type === "set_state") {
                        ...
                        if (typeof effect.value !== "string")
                            return "state effect value must be a string";
                        if (effect.value.length > MAX_REGEX_ACTION_FIELD_LENGTH)
                            return "state effect value exceeds maximum length";
                    }
                    else if (effect.type === "draft") {
                        if (typeof effect.content !== "string" || !effect.content.trim())
                            return "draft effect content is required";
                        if (effect.content.length > MAX_REGEX_ACTION_FIELD_LENGTH)
                            return "draft effect content exceeds maximum length";
                        if (effect.mode !== "replace" && effect.mode !== "append")
                            return "draft effect mode must be replace or append";
                    }
                }
                if (action.effects.filter((effect) => effect.type === "draft").length > 1)
                    return "an action can contain only one draft effect";
                if (action.effects.filter((effect) => effect.type === "fork").length > 1)
                    return "an action can contain only one fork effect";
    ...
    if (input.placement !== undefined) {
        if (!Array.isArray(input.placement))
            return "placement must be an array";
        for (const p of input.placement) {
            if (!VALID_PLACEMENTS.has(p))
                return `Invalid placement: ${p}`;
        }
    }
...
function isPlainMetadataRecord(value) {
    return typeof value === "object" && value !== null && !Array.isArray(value);
}
function getSpindleExtensionRegexAttribution(metadata) {
    if (!isPlainMetadataRecord(metadata))
        return null;
    const raw = metadata[SPINDLE_EXTENSION_REGEX_METADATA_KEY];
    if (!isPlainMetadataRecord(raw))
        return null;
...
export function getSpindleExtensionRegexFolderVersion(script) {
    const owner = normalizeOptionalId(script.owner_extension_identifier);
    if (!owner || !normalizeOptionalId(script.folder))
        return null;
...
function validateExtensionFolderVersion(context) {
    if (!context || !Object.prototype.hasOwnProperty.call(context, "extensionFolderVersion"))
        return null;
...
function applySpindleExtensionRegexAttribution(input, extensionIdentifier, context, existing) {
...
function mapRegexScriptPersistenceError(err) {
    const message = err instanceof Error ? err.message : String(err ?? "");
    if (message.includes("idx_regex_scripts_script_id")
        || message.includes("UNIQUE constraint failed: regex_scripts.user_id, regex_scripts.script_id")) {
        return "script_id already exists";
    }
    return null;
}
function prepareCharacterBoundImportedScript(input, source) {
...
function getLumiHubPresetRegexAttribution(metadata) {
    if (!isPlainMetadataRecord(metadata))
        return null;
 ...
 */
function preparePresetBoundImportedScript(input, attribution) {
    const importedScriptId = typeof input.script_id === "string"
        ? normalizeScriptId(input.script_id)
        : "";
    const metadata = isPlainMetadataRecord(input.metadata) ? { ...input.metadata } : {};
...
// ── CRUD ─────────────────────────────────────────────────────────────────────
export function listRegexScripts(userId, pagination, filters) {
    ...
    return paginatedQuery(`SELECT * FROM regex_scripts WHERE ${where} ORDER BY sort_order ASC, created_at ASC`, `SELECT COUNT(*) as count FROM regex_scripts WHERE ${where}`, params, pagination, rowToRegexScript);
}
// Prepared statement for hot-path regex fetch
let _stmtRegexById = null;
let _stmtRegexByIdGen = -1;
export function getRegexScript(userId, id) {
    ...
    const row = _stmtRegexById.get(id, userId);
    return row ? rowToRegexScript(row) : null;
}
export function createRegexScript(userId, input, context) {
    const extensionIdentifier = normalizeOptionalId(context?.extensionIdentifier);
    let nextInput = extensionIdentifier
    ...
    if (extensionIdentifier && context) {
        ...
        if (folderVersionError)
            return folderVersionError;
    ...
    try {
        getDb()
            .query(`INSERT INTO regex_scripts (id, user_id, name, script_id, find_regex, replace_string, actions, flags, placement, scope, scope_id, target, min_depth, max_depth, trim_strings, run_on_edit, substitute_macros, disabled, sort_order, description, folder, pack_id, preset_id, character_id, owner_extension_identifier, metadata, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(id, userId, nextInput.name.trim(), nextInput.script_id ?? "", nextInput.find_regex, nextInput.replace_string ?? "", JSON.stringify(nextInput.actions ?? []), nextInput.flags ?? "gi", JSON.stringify(nextInput.placement ?? ["ai_output"]), nextInput.scope ?? "global", nextInput.scope === "global" || !nextInput.scope ? null : (nextInput.scope_id ?? null), JSON.stringify(nextInput.target ?? ["response"]), nextInput.min_depth ?? null, nextInput.max_depth ?? null, JSON.stringify(nextInput.trim_strings ?? []), nextInput.run_on_edit ? 1 : 0, nextInput.substitute_macros ?? "none", disabled ? 1 : 0, nextInput.sort_order ?? 0, nextInput.description ?? "", nextInput.folder ?? "", nextInput.pack_id ?? null, nextInput.preset_id ?? null, nextInput.character_id ?? null, extensionIdentifier, JSON.stringify(nextInput.metadata ?? {}), now, now);
    }
    catch (err) {
...
export function updateRegexScript(userId, id, input, context) {
    const existing = getRegexScript(userId, id);
    if (!existing)
        return null;
    const extensionIdentifier = normalizeOptionalId(context?.extensionIdentifier);
    if (extensionIdentifier && (existing.owner_extension_identifier !== extensionIdentifier
        || existing.preset_id !== null) && !context?.allowUnownedMutation) {
    ...
    if (extensionIdentifier) {
        ...
        if (existing.owner_extension_identifier === extensionIdentifier) {
            ...
            delete metadata[SPINDLE_EXTENSION_REGEX_METADATA_KEY];
            if (attribution)
                metadata[SPINDLE_EXTENSION_REGEX_METADATA_KEY] = attribution;
    ...
    if (nextInput.name !== undefined) {
        fields.push("name = ?");
        values.push(nextInput.name.trim());
    }
    if (nextInput.script_id !== undefined) {
        fields.push("script_id = ?");
        values.push(nextInput.script_id);
    }
    if (nextInput.find_regex !== undefined) {
        fields.push("find_regex = ?");
        values.push(nextInput.find_regex);
    }
    if (nextInput.replace_string !== undefined) {
        fields.push("replace_string = ?");
        values.push(nextInput.replace_string);
    }
    if (nextInput.actions !== undefined) {
        fields.push("actions = ?");
        values.push(JSON.stringify(nextInput.actions));
    }
    if (nextInput.flags !== undefined) {
        fields.push("flags = ?");
        values.push(nextInput.flags);
    }
    if (nextInput.placement !== undefined) {
        fields.push("placement = ?");
        values.push(JSON.stringify(nextInput.placement));
    }
    if (nextInput.scope !== undefined) {
        fields.push("scope = ?");
        values.push(nextInput.scope);
    }
    if (nextInput.scope_id !== undefined) {
        fields.push("scope_id = ?");
        values.push(nextInput.scope_id);
    }
    if (nextInput.target !== undefined) {
        fields.push("target = ?");
        values.push(JSON.stringify(nextInput.target));
    }
    if (nextInput.min_depth !== undefined) {
        fields.push("min_depth = ?");
        values.push(nextInput.min_depth);
    }
    if (nextInput.max_depth !== undefined) {
        fields.push("max_depth = ?");
        values.push(nextInput.max_depth);
    }
    if (nextInput.trim_strings !== undefined) {
        fields.push("trim_strings = ?");
        values.push(JSON.stringify(nextInput.trim_strings));
    }
    if (nextInput.run_on_edit !== undefined) {
        fields.push("run_on_edit = ?");
        values.push(nextInput.run_on_edit ? 1 : 0);
    }
    if (nextInput.substitute_macros !== undefined) {
        fields.push("substitute_macros = ?");
        values.push(nextInput.substitute_macros);
    }
    if (nextInput.disabled !== undefined) {
        fields.push("disabled = ?");
        values.push(nextInput.disabled ? 1 : 0);
    }
    if (nextInput.sort_order !== undefined) {
        fields.push("sort_order = ?");
        values.push(nextInput.sort_order);
    }
    if (nextInput.description !== undefined) {
        fields.push("description = ?");
        values.push(nextInput.description);
    }
    if (nextInput.folder !== undefined) {
        fields.push("folder = ?");
        values.push(nextInput.folder);
    }
    if (hasPresetIdUpdate) {
        fields.push("preset_id = ?");
        values.push(nextPresetId);
    }
    if (nextInput.metadata !== undefined) {
        fields.push("metadata = ?");
        values.push(JSON.stringify(nextInput.metadata));
    }
    if (fields.length === 0)
        return existing;
...
export function deleteRegexScript(userId, id, context) {
    const existing = getRegexScript(userId, id);
    const extensionIdentifier = normalizeOptionalId(context?.extensionIdentifier);
    if (extensionIdentifier && existing && (existing.owner_extension_identifier !== extensionIdentifier
        || existing.preset_id !== null) && !context?.allowUnownedMutation) {
        return EXTENSION_REGEX_OWNERSHIP_ERROR;
    }
    const result = getDb()
 ...
 */
export function deleteRegexScripts(userId, ids) {
    if (ids.length === 0)
        return [];
    const db = getDb();
...
export function duplicateRegexScript(userId, id) {
    const existing = getRegexScript(userId, id);
    if (!existing)
        return null;
    ...
    const now = Math.floor(Date.now() / 1000);
    getDb()
        .query(`INSERT INTO regex_scripts (id, user_id, name, script_id, find_regex, replace_string, actions, flags, placement, scope, scope_id, target, min_depth, max_depth, trim_strings, run_on_edit, substitute_macros, disabled, sort_order, description, folder, metadata, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(newId, userId, existing.name + " (Copy)", "", // script_id intentionally blank on duplicate — must be unique
    existing.find_regex, existing.replace_string, JSON.stringify(existing.actions), existing.flags, JSON.stringify(existing.placement), existing.scope, existing.scope_id, JSON.stringify(existing.target), existing.min_depth, existing.max_depth, JSON.stringify(existing.trim_strings), existing.run_on_edit ? 1 : 0, existing.substitute_macros, existing.disabled ? 1 : 0, existing.sort_order, existing.description, existing.folder, JSON.stringify(existing.metadata), now, now);
    const script = getRegexScript(userId, newId);
    eventBus.emit(EventType.REGEX_SCRIPT_CHANGED, { id: newId, script }, userId);
    return script;
}
export function reorderRegexScripts(userId, orderedIds) {
    const db = getDb();
...
export function toggleRegexScript(userId, id, disabled, context) {
    const existing = getRegexScript(userId, id);
    if (!existing)
        return null;
    const activePresetId = normalizeOptionalId(context?.activePresetId);
...
function toggleRegexScriptRows(userId, rows, disabled, activePresetId) {
    const changedIds = [];
    const skippedIds = [];
    const targets = [];
    for (const row of rows) {
 ...
 */
export function toggleRegexScriptsByIds(userId, ids, disabled, context) {
 ...
 */
export function toggleRegexScriptsByFolder(userId, folder, disabled, context) {
    const activePresetId = normalizeOptionalId(context?.activePresetId);
    const rows = getDb()
...
export function getCharacterBoundScripts(userId, characterId) {
    const rows = getDb()
...
export function getRegexScriptByScriptId(userId, scriptId, context) {
    ...
    const params = [userId, normalizedScriptId, normalizedScriptId];
    ...
    const row = getDb()
       ...
       ORDER BY
         ...
         created_at ASC
       LIMIT 1`)
        .get(...params, chatId, characterId, presetId, presetId, normalizedScriptId);
    return row ? rowToRegexScript(row) : null;
}
// ── Pipeline ─────────────────────────────────────────────────────────────────
/**
 ...
 */
function getScopedScripts(userId, opts, matchConditions, matchParams) {
    const conditions = ["user_id = ?", "disabled = 0", ...matchConditions];
    const params = [userId, ...matchParams];
    const scopeConditions = ["scope = 'global'"];
    if (opts.characterId) {
        scopeConditions.push("(scope = 'character' AND scope_id = ?)");
    ...
    conditions.push(`(${scopeConditions.join(" OR ")})`);
    const rows = getDb()
        .query(`SELECT * FROM regex_scripts WHERE ${conditions.join(" AND ")}
       ORDER BY
...
export function getActiveScripts(userId, opts) {
 ...
 */
export function getActiveMemoryScripts(userId, opts) {
 ...
 */
export async function applyMemoryIngestionRegex(userId, content, opts) {
    if (!content)
        return content;
    const scripts = getActiveMemoryScripts(userId, opts);
    if (scripts.length === 0)
        return content;
    return applyRegexScripts(content, scripts, "memory", undefined, undefined, undefined, { source: "prompt_backend" });
}
/**
 ...
 */
export function getRunOnEditScripts(userId, opts) {
    return getScopedScripts(userId, opts, ["run_on_edit = 1", `instr(target, '"response"') > 0`], []);
}
/**
 ...
 */
function foldFingerprint(acc, result) {
    if (!acc)
        return;
    for (const v of result.touchedVars)
        acc.touchedVars.add(v);
    if (!result.cacheable)
        acc.cacheable = false;
}
function macroOptionsForRegexScript(script) {
    if (script.preset_id) {
        return { sourceOwner: "host", sourceHint: "regex_script:preset" };
    }
    return undefined;
}
async function resolveFindMacros(findRegex, mode, macroEnv, outFingerprint, macroOptions) {
    if (mode === "none")
        return findRegex;
 ...
 */
async function resolveReplacementMacros(replaceString, mode, macroEnv, outFingerprint, macroOptions) {
    if (mode === "none" || mode === "find")
 ...
 */
export async function applyRegexScripts(content, scripts, placement, depth, macroEnv, resolvedTemplates, options) {
    let result = content;
    for (const script of scripts) {
        ...
        try {
            ...
            if (preResolvedFind !== undefined) {
                ...
                findRegex = await resolveFindMacros(findRegex, script.substitute_macros, macroEnv, options?.outFingerprint, macroOptions);
            }
            const regexActions = readRegexActions(script);
            if (regexActions.size > 0) {
                if (options?.outFingerprint
                    && regexActions.has("repeat_back")) {
                    options.outFingerprint.cacheable = false;
                }
                const applied = await applyRegexActions(result, findRegex, script.flags, script.replace_string, regexActions, options, readRepeatPosition(script), readRepeatRawMatch(script), (match, input) => resolveRepeatedMatchReplacement(script, match, input, macroEnv, resolvedTemplates, options));
                if (applied.handled) {
            ...
            const actionMatches = actionCapture
                ? await regexCaptureReplacementsSandboxed(findRegex, script.flags, result, actionCapture.template, REGEX_SCRIPT_TIMEOUT_MS)
                : [];
            if (macroEnv && script.substitute_macros === "raw") {
                ...
                const matches = await regexCaptureReplacementsSandboxed(findRegex, script.flags, result, script.replace_string, REGEX_SCRIPT_TIMEOUT_MS);
                if (matches.length > 0) {
                    const replacements = await Promise.all(matches.map(async ({ replacement }) => {
                    ...
                    result = rebuildFromMatches(result, matches, actionCapture
                ...
                const substituted = rebuildFromMatches(result, matches, actionCapture
                    ? decorateRegexActionReplacements(replacements, actionMatches, actionCapture.unpack, script.id)
                    : replacements);
                ...
                if (preResolvedReplacement !== undefined) {
                    ...
                    replaceString = await resolveReplacementMacros(replaceString, script.substitute_macros, macroEnv, options?.outFingerprint, macroOptions);
                }
                if (actionCapture) {
                    const matches = await regexCaptureReplacementsSandboxed(findRegex, script.flags, result, replaceString, REGEX_SCRIPT_TIMEOUT_MS);
                    result = rebuildFromMatches(result, matches, decorateRegexActionReplacements(matches.map((match) => match.replacement), actionMatches, actionCapture.unpack, script.id));
                }
                else {
                    result = await regexReplaceSandboxed(findRegex, script.flags, result, replaceString, REGEX_SCRIPT_TIMEOUT_MS);
                }
            }
            // Apply trim_strings
            ...
            if (e instanceof RegexTimeoutError) {
                ...
                console.warn(`[RegexScripts] Script "${script.name}" (${script.id}) exceeded ${REGEX_SCRIPT_TIMEOUT_MS}ms, skipping`);
                continue;
            }
...
function readRegexActions(script) {
    const raw = script.metadata?.match_actions;
    if (!Array.isArray(raw))
        return new Set();
    return new Set(raw.filter((action) => action === "move_top"
        || action === "move_bottom"
        || action === "repeat_back"));
}
function readRepeatPosition(script) {
    const value = script.metadata?.repeat_position;
    return typeof value === "string" ? value : undefined;
}
function readRepeatRawMatch(script) {
    return script.metadata?.repeat_raw_match === true;
}
async function resolveRepeatedMatchReplacement(script, match, input, macroEnv, resolvedTemplates, options) {
    let replacement = script.replace_string;
    const macroOptions = macroOptionsForRegexScript(script);
    if (script.substitute_macros === "raw" || script.substitute_macros === "after") {
        replacement = substituteRegexCapturesCore(replacement, match.fullMatch, match.groups, match.index, input, match.namedGroups);
        if (macroEnv) {
        ...
        if (preResolved !== undefined) {
            ...
            && script.substitute_macros !== "none"
            && script.substitute_macros !== "find") {
            replacement = await resolveReplacementMacros(replacement, script.substitute_macros, macroEnv, options?.outFingerprint, macroOptions);
        }
        replacement = substituteRegexCapturesCore(replacement, match.fullMatch, match.groups, match.index, input, match.namedGroups);
    }
    if (script.actions.length > 0 && options?.source === "display_backend") {
        const capture = buildRegexActionCaptureTemplate(script.actions);
        const actionReplacement = substituteRegexCapturesCore(capture.template, match.fullMatch, match.groups, match.index, input, match.namedGroups);
        return decorateRegexActionReplacements([replacement], [{
                index: match.index,
                matchLength: match.fullMatch.length,
                replacement: actionReplacement,
            }], capture.unpack, script.id)[0];
    }
    return replacement;
}
async function applyRegexActions(content, pattern, flags, replacement, actions, options, repeatPosition, repeatRawMatch = false, resolveRepeatedMatch) {
    ...
    const effectiveFlags = movesTop || movesBottom
        ...
        : flags;
    const matches = await regexCollectSandboxed(pattern, effectiveFlags, content, REGEX_SCRIPT_TIMEOUT_MS);
    if (matches.length === 0) {
        if (!actions.has("repeat_back")
            || options?.previousContent === undefined)
            return { handled: true, content };
        const prior = await regexCollectSandboxed(pattern, effectiveFlags, options.previousContent, REGEX_SCRIPT_TIMEOUT_MS);
        if (prior.length === 0)
            return { handled: true, content };
    ...
    if (movesTop || movesBottom) {
        ...
        const moved = substituteRegexCapturesCore(replacement, match.fullMatch, match.groups, match.index, content, match.namedGroups);
        const remainder = rebuildFromMatches(content, [{ index: match.index, matchLength: match.fullMatch.length }], [""]);
        return {
            handled: true,
            content: movesTop
                ? `${moved}\n${remainder}`
...
export async function testRegex(findRegex, replaceString, flags, content, matchActions = []) {
    try {
        const out = await regexTestSandboxed(findRegex, flags, content, replaceString, TEST_REGEX_TIMEOUT_MS);
        if (matchActions.length > 0) {
            const applied = await applyRegexActions(content, findRegex, flags, replaceString, new Set(matchActions), undefined);
...
export function exportRegexScripts(userId, options) {
    const db = getDb();
    let rows;
...
export function getRegexScriptsByPackId(userId, packId) {
    const rows = getDb()
        .query("SELECT * FROM regex_scripts WHERE user_id = ? AND pack_id = ? ORDER BY sort_order ASC, created_at ASC")
        .all(userId, packId);
    return rows.map(rowToRegexScript);
}
export function getRegexScriptsByPresetId(userId, presetId) {
    const rows = getDb()
...
function getLumiHubSourceFolder(script, attribution, fallback, version) {
    ...
    if (folder.endsWith(historicalSuffix))
        return folder.slice(0, -historicalSuffix.length).trim() || fallback;
    return folder;
}
function chooseAvailableRegexFolder(userId, desiredFolder, allowedOccupantIds, reserveLumiHubNamespace = false) {
    const db = getDb();
    ...
    for (const candidate of candidates) {
        const occupants = db
            ...
            .all(userId, candidate);
        if (occupants.every((row) => allowedOccupantIds.has(row.id)))
            return candidate;
    }
 ...
 */
export function retireLumiHubPresetRegexScriptsForUpdate(userId, options) {
    ...
    const acceptedHubIds = new Set([hubPresetId, previousHubPresetId].filter((id) => !!id));
    ...
    const matching = rows.flatMap((script) => {
        if (preserveIds.has(script.id))
            return [];
        const attribution = getLumiHubPresetRegexAttribution(script.metadata);
        if (attribution?.id && !acceptedHubIds.has(attribution.id))
            return [];
        ...
        if (!attribution?.id && !previousHubPresetId)
            return [];
    ...
    const archiveKey = (version, folderName) => `${version ?? ""}\u0000${folderName}`;
    for (const { script, version, folderName } of archived) {
        const key = archiveKey(version, folderName);
        const group = archiveGroups.get(key) ?? { version, folderName, ids: new Set() };
        group.ids.add(script.id);
        archiveGroups.set(key, group);
    }
    const foldersByArchiveKey = new Map();
    ...
    db.transaction(() => {
        ...
        for (const { script } of replaced)
            deleteRow.run(script.id, userId);
        const updateRow = db.query("UPDATE regex_scripts SET disabled = 1, folder = ?, metadata = ?, updated_at = ? WHERE id = ? AND user_id = ?");
        for (const { script, version, folderName } of archived) {
            ...
            updateRow.run(foldersByArchiveKey.get(archiveKey(version, folderName)), JSON.stringify(metadata), now, script.id, userId);
        }
...
export function resolveLumiHubPresetRegexInstallFolder(userId, presetId, hubPresetId, presetName, sourceFolder = presetName) {
    ...
    const allowedIds = new Set(getRegexScriptsByPresetId(userId, presetId)
        .filter((script) => {
        const attribution = getLumiHubPresetRegexAttribution(script.metadata);
 ...
 */
export function installLumiHubPresetRegexScripts(userId, options) {
    ...
    try {
        if (options.scripts.length > 0) {
            const imported = importPresetBoundRegexScripts(userId, options.presetId, options.presetName, options.scripts, {
                source: "lumihub",
        ...
        newIds = getRegexScriptsByPresetId(userId, options.presetId)
            ...
            .map((script) => script.id);
        if (newIds.length > 0)
            deleteRegexScripts(userId, newIds);
...
export function activatePresetBoundRegexScripts(userId, presetId) {
...
export function switchPresetBoundRegexScripts(userId, opts) {
    ...
    const result = db.transaction(() => {
        if (previousPresetId) {
            const enabledRows = db
                .query("SELECT id FROM regex_scripts WHERE user_id = ? AND preset_id = ? AND disabled = 0 ORDER BY sort_order ASC, created_at ASC")
                .all(userId, previousPresetId);
...
export function getRegexScriptsByCharacterId(userId, characterId) {
    const rows = getDb()
 ...
 */
export function deleteRegexScriptsByPresetId(userId, presetId) {
    const db = getDb();
    const rows = db
 ...
 */
export function deleteRegexScriptsByCharacterId(userId, characterId) {
    const db = getDb();
    const rows = db
...
function convertStPlacement(placement) {
    const result = [];
...
export function importRegexScripts(userId, payload, context) {
    const errors = [];
    let imported = 0;
    let skipped = 0;
    // Extract top-level folder override (e.g. preset name)
    const folderOverride = typeof payload?.folder === "string" && payload.folder.trim()
        ? payload.folder.trim()
        : undefined;
    // Extract top-level preset_id ownership link so preset deletion can cascade
    const presetIdOverride = typeof payload?.preset_id === "string" && payload.preset_id.trim()
        ? payload.preset_id.trim()
        : undefined;
    // Extract top-level character_id ownership link so character deletion can cascade
    const characterIdOverride = typeof payload?.character_id === "string" && payload.character_id.trim()
    ...
    for (let i = 0; i < scripts.length; i++) {
        ...
        if (typeof result === "string") {
            ...
            const { id: _id, user_id: _userId, pack_id: _packId, preset_id: _importedPresetId, character_id: _characterId, created_at: _createdAt, updated_at: _updatedAt, ...updates } = item;
            if (presetIdOverride)
                updates.preset_id = presetIdOverride;
 ...
 */
export function importCharacterBoundRegexScripts(userId, characterId, extensions, options) {
    ...
    if (Array.isArray(stScripts) && stScripts.length > 0) {
        const result = importRegexScripts(userId, {
            scripts: stScripts.map((s) => s && typeof s === "object"
 ...
 */
export function importPresetBoundRegexScripts(userId, presetId, presetName, scripts, attribution) {
