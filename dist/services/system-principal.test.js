...
const { putSecret, SYSTEM_SECRET_PRINCIPAL, SYSTEM_SECRET_PRINCIPAL_EMAIL } = await import("./secrets.service");
const { getFirstUserId } = await import("../auth/seed");
async function initBaselineDb() {
    closeDatabase();
    initDatabase(":memory:");
...
describe("system principal provisioning", () => {
    ...
    test("a squatter holding the reserved email fails loudly instead of silent skip + FK error", async () => {
        ...
        getDb()
            .query(`INSERT INTO "user" (id, name, email, emailVerified, username, role, createdAt, updatedAt)
         VALUES ('squatter-id', 'system', ?, 1, 'system', 'user', 5, 5)`)
            .run(SYSTEM_SECRET_PRINCIPAL_EMAIL);
        await expect(putSecret(SYSTEM_SECRET_PRINCIPAL, "extension:inst-a:key3", "v")).rejects.toThrow(/held by account "squatter-id"/);
        // No secret row was persisted against a dangling FK target.
        expect(getDb().query("SELECT COUNT(*) as count FROM secrets WHERE user_id = ?").get(SYSTEM_SECRET_PRINCIPAL)).toEqual({ count: 0 });
    });
});
