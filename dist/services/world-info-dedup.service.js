 ...
 */
export function deduplicateWorldInfoEntries(entries, sources, bookSourceMap) {
    if (entries.length <= 1)
        return { entries: [...entries], removed: [] };
...
// Tier 1: Exact content match
// ---------------------------------------------------------------------------
function runTierExact(group, removedIds, removed, sources, bookSourceMap) {
...
// Tier 2: Normalized content match
// ---------------------------------------------------------------------------
function runTierNearExact(group, removedIds, removed, sources, bookSourceMap) {
...
// Tier 3: Fuzzy Jaccard similarity
// ---------------------------------------------------------------------------
function runTierFuzzy(group, removedIds, removed, sources, bookSourceMap) {
    // Build token sets for eligible entries
    const eligible = [];
    for (const e of group) {
        if (removedIds.has(e.id))
            continue;
        if (!e.content || e.content.length < MIN_CONTENT_LENGTH_FOR_FUZZY)
            continue;
        eligible.push({ entry: e, tokens: buildTokenSet(normalizeContent(e.content)) });
    }
    for (let i = 0; i < eligible.length; i++) {
        if (removedIds.has(eligible[i].entry.id))
            continue;
        for (let j = i + 1; j < eligible.length; j++) {
            if (removedIds.has(eligible[j].entry.id))
                continue;
 ...
 */
function resolveDuplicates(dupes, tier, similarity, removedIds, removed, sources, bookSourceMap) {
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (winner.constant && loser.constant)
            continue;
        removedIds.add(loser.id);
        removed.push({
            removedEntryId: loser.id,
 ...
 */
function compareEntryPrecedence(a, b, sources, bookSourceMap) {
    ...
    if (aSource === "keyword" && bSource !== "keyword")
        return -1;
    if (bSource === "keyword" && aSource !== "keyword")
        return 1;
