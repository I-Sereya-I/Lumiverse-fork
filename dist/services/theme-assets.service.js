...
function getThemeAssetsDir() {
    const dir = join(env.dataDir, THEME_ASSETS_DIR);
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
    return dir;
}
function ensureUserBundleDir(userId, bundleId) {
...
function normalizeBundleId(bundleId) {
    ...
    if (trimmed.length > 128)
        throw new Error("bundle_id is too long");
    if (!/^[A-Za-z0-9._-]+$/.test(trimmed))
        throw new Error("bundle_id contains invalid characters");
    return trimmed;
}
...
function normalizeSlug(rawSlug, originalFilename) {
    ...
    const normalizedParts = parts.map((part) => {
        if (part === "." || part === "..")
            throw new Error("slug contains invalid path traversal");
    ...
    if (normalizedParts[0] !== "assets")
        normalizedParts.unshift("assets");
...
function resolveStorageType(mimeType) {
...
function getThemeAssetRow(userId, id) {
    return getDb()
        .query("SELECT * FROM theme_assets WHERE id = ? AND user_id = ?")
        .get(id, userId);
}
function getThemeAssetRowBySlug(userId, bundleId, slug) {
    return getDb()
        .query("SELECT * FROM theme_assets WHERE user_id = ? AND bundle_id = ? AND slug = ?")
        .get(userId, bundleId, slug);
}
function nextAvailableSlug(userId, bundleId, desiredSlug) {
...
async function saveFileAsset(userId, bundleId, id, file) {
...
async function buildStoredAssetContent(userId, row) {
...
export function listThemeAssets(userId, bundleId) {
    ...
    const rows = getDb()
        ...
        .all(userId, normalizedBundleId);
    return rows.map(rowToThemeAsset);
}
export function getThemeAsset(userId, id) {
    const row = getThemeAssetRow(userId, id);
    return row ? rowToThemeAsset(row) : null;
}
export async function createThemeAsset(userId, input) {
    ...
    try {
        getDb()
            .query(`INSERT INTO theme_assets
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(id, userId, bundleId, slug, storageType, imageId, fileName, input.file.name, mimeType, input.file.size || 0, JSON.stringify(tags), JSON.stringify(metadata), now, now);
    }
    catch (err) {
        if (imageId)
            imagesSvc.deleteImage(userId, imageId);
        if (fileName) {
...
export function updateThemeAsset(userId, id, input) {
...
export function deleteThemeAsset(userId, id) {
    const existing = getThemeAssetRow(userId, id);
    if (!existing)
        return false;
...
export async function optimizeThemeAssetToWebp(userId, id) {
    const row = getThemeAssetRow(userId, id);
    if (!row)
        return null;
    ...
    const optimizedFile = new File([Uint8Array.from(optimizedBuffer)], replaceFileExtension(row.original_filename || "asset", ".webp"), { type: "image/webp" });
    const nextImage = await imagesSvc.uploadImage(userId, optimizedFile);
    const now = Math.floor(Date.now() / 1000);
    try {
        getDb()
            .query("UPDATE theme_assets SET image_id = ?, original_filename = ?, mime_type = ?, byte_size = ?, updated_at = ? WHERE id = ? AND user_id = ?")
...
export async function getThemeAssetContentById(userId, id) {
    const row = getThemeAssetRow(userId, id);
    if (!row)
        return null;
    return buildStoredAssetContent(userId, row);
}
export async function getThemeAssetContentBySlug(userId, bundleId, slug) {
    const normalizedBundleId = normalizeBundleId(bundleId);
