...
export function seedTokenizers() {
    const db = getDb();
    const upsertConfig = db.prepare(`INSERT INTO tokenizer_configs (id, name, type, config, is_built_in, updated_at)
     VALUES (?, ?, ?, ?, 1, unixepoch())
    ...
    const upsertPattern = db.prepare(`INSERT INTO tokenizer_model_patterns (id, tokenizer_id, pattern, priority, is_built_in, updated_at)
     VALUES (?, ?, ?, ?, 1, unixepoch())
