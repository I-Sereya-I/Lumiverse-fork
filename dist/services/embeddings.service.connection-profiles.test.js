...
import { EMBEDDING_ERROR_CODES, EMBEDDING_SETTINGS_KEY, EmbeddingError, areProfileDimensionsCompatible, cachedEmbedTexts, embedTexts, embeddingProfileSecretKey, getEmbeddingConfig, isUsableProfileId, selectFallbackChain, updateEmbeddingConfig, } from "./embeddings.service";
...
const spies = [];
function sk(userId, key) {
    return `${userId}::${key}`;
}
function putCfg(value) {
    settings.set(sk(USER, EMBEDDING_SETTINGS_KEY), value);
}
function openaiBody(dims = 2) {
...
function asFetchStub(fn) {
    const stub = Object.assign(fn.bind(globalThis), { preconnect() { } });
    return stub;
}
function enabledProfiles(overrides = {}) {
    return {
        enabled: true,
...
beforeEach(() => {
    ...
    spies.push(spyOn(settingsSvc, "getSetting").mockImplementation((userId, key) => {
        const value = settings.get(sk(userId, key));
        return value === undefined ? null : { key, value, updated_at: 0 };
    }), spyOn(settingsSvc, "putSetting").mockImplementation((userId, key, value) => {
        settings.set(sk(userId, key), value);
        return { key, value, updated_at: 0 };
    }), spyOn(secretsSvc, "getSecret").mockImplementation(async (userId, key) => {
        return secrets.get(sk(userId, key)) ?? null;
    }), spyOn(secretsSvc, "getSecretForStatus").mockImplementation(async (userId, key) => {
        return secrets.get(sk(userId, key)) ?? null;
    }), spyOn(secretsSvc, "putSecret").mockImplementation(async (userId, key, value) => {
        secrets.set(sk(userId, key), value);
    }), spyOn(secretsSvc, "deleteSecret").mockImplementation((userId, key) => {
...
describe("embedding connection profiles", () => {
    test("migrates old single-provider config into a UUID profile, not a literal default id", async () => {
        ...
        const stored = settings.get(sk(USER, EMBEDDING_SETTINGS_KEY));
    ...
    test("preserves unknown provider ids and Vertex project/region controls", async () => {
        ...
        expect(updated.secretRef).toBeUndefined();
    });
    ...
    test("falls back when primary is unavailable and reports embedding_fallback_exhausted", async () => {
        ...
        try {
            ...
            expect(err.code).toBe(EMBEDDING_ERROR_CODES.FALLBACK_EXHAUSTED);
            expect(err.message).not.toContain("embedding-profile/");
        }
    ...
    test("reports embedding_provider_unavailable when the only profile cannot run", async () => {
        ...
        try {
            ...
            expect(err).toBeInstanceOf(EmbeddingError);
            expect(err.code).toBe(EMBEDDING_ERROR_CODES.PROVIDER_UNAVAILABLE);
        }
    });
    test("caller aborts primary without starting fallback", async () => {
        ...
        const urls = [];
        let releasePrimary;
        spies.push(spyOn(globalThis, "fetch").mockImplementation(asFetchStub((input, init) => {
            const url = String(input);
            urls.push(url);
    ...
    test("caller abort leaves no partial cache or write and returns stable cancellation", async () => {
        ...
        secrets.set(sk(USER, embeddingProfileSecretKey(FALLBACK_ID)), "fallback-key");
        const controller = new AbortController();
        spies.push(spyOn(globalThis, "fetch").mockImplementation(asFetchStub((_input, init) => {
            return new Promise((_resolve, reject) => {
    ...
    test("resolves the opaque secret only at embedding driver invocation", async () => {
        ...
        spies[2]?.mockRestore();
        spies[3]?.mockRestore();
        spies[2] = spyOn(secretsSvc, "getSecret").mockImplementation(async (userId, key) => {
            secretReads.push(key);
            return secrets.get(sk(userId, key)) ?? null;
        });
        spies[3] = spyOn(secretsSvc, "getSecretForStatus").mockImplementation(async (userId, key) => {
    ...
    test("redacts secret values and refs from provider errors", async () => {
        ...
        secrets.set(sk(USER, embeddingProfileSecretKey(PRIMARY_ID)), secret);
        spies.push(spyOn(globalThis, "fetch").mockImplementation(asFetchStub(async () => {
            return new Response(`denied Bearer ${secret} at embedding-profile/${PRIMARY_ID}/apiKey`, { status: 401 });
        })));
        try {
            await embedTexts(USER, ["hello"]);
...
function isEmbeddingAbort(err) {
    if (!err || typeof err !== "object")
        return false;
    const name = err.name;
    return name === "AbortError" || /abort/i.test(err.message || "");
}
