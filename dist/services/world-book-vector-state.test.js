...
import { desiredWorldBookVectorIndexStatus, isWorldBookEntryVectorEligible, isWorldBookEntryVectorSearchReady, worldBookVectorSettingsFingerprint, worldBookVectorTrackingFingerprint, } from "./world-book-vector-state";
describe("world-book-vector-state", () => {
    ...
    test("entry tracking fingerprint changes when vector-relevant fields change", () => {
        ...
        expect(worldBookVectorTrackingFingerprint({ ...base })).toBe(worldBookVectorTrackingFingerprint({ ...base }));
        expect(worldBookVectorTrackingFingerprint({ ...base, content: "Different lore" })).not.toBe(worldBookVectorTrackingFingerprint({ ...base }));
        expect(worldBookVectorTrackingFingerprint({ ...base, world_book_id: "book-2" })).not.toBe(worldBookVectorTrackingFingerprint({ ...base }));
        expect(worldBookVectorTrackingFingerprint({ ...base, updated_at: 101 })).not.toBe(worldBookVectorTrackingFingerprint({ ...base }));
    });
