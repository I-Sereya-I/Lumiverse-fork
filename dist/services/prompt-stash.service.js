...
function normalizeStoredEntry(value) {
    if (!isObject(value) || typeof value.id !== "string" || !value.id.trim() || !isObject(value.block))
        return null;
...
function readStash(userId) {
    ...
    if (!Array.isArray(raw))
        return [];
    return raw.map(normalizeStoredEntry).filter((entry) => entry !== null);
}
...
export function listPromptStash(userId) {
    return readStash(userId).sort((a, b) => b.updatedAt - a.updatedAt);
}
export function addPromptBlockToStash(userId, block, sourcePreset) {
    if (block.marker)
        throw new Error("Only prompt blocks can be added to the stash");
    const entries = readStash(userId);
    if (entries.length >= MAX_STASH_ENTRIES)
        throw new Error(`Prompt stash is limited to ${MAX_STASH_ENTRIES} blocks`);
    const now = Date.now();
    const entry = {
 ...
 */
export function removePromptBlockFromStash(userId, stashId) {
    const entries = readStash(userId);
    if (!entries.some((entry) => entry.id === stashId))
        return false;
    const remaining = entries.filter((entry) => entry.id !== stashId);
    const db = getDb();
    const rows = db.query("SELECT id, prompt_order FROM presets WHERE user_id = ?").all(userId);
    const changedPresetIds = [];
    const update = db.query("UPDATE presets SET prompt_order = ?, updated_at = ?, cache_revision = cache_revision + 1 WHERE id = ? AND user_id = ?");
    const now = Math.floor(Date.now() / 1000);
    const transaction = db.transaction(() => {
        for (const row of rows) {
            let blocks;
            try {
                blocks = JSON.parse(row.prompt_order);
            }
            catch {
                continue;
            }
...
function emitPresetChanges(userId, presetIds) {
    const db = getDb();
    for (const presetId of presetIds) {
        const row = db.query("SELECT * FROM presets WHERE id = ? AND user_id = ?").get(presetId, userId);
        if (!row)
            continue;
        const preset = {
...
export function blockFromStash(entry) {
    return {
        ...entry.block,
        id: crypto.randomUUID(),
        enabled: true,
...
export function syncStashedBlocksAcrossPresets(userId, excludedPresetId, stashIds) {
    if (stashIds.length === 0)
        return;
    const entries = new Map(readStash(userId).map((entry) => [entry.id, entry]));
    const wanted = new Set(stashIds);
    const db = getDb();
    const rows = db.query("SELECT id, prompt_order FROM presets WHERE user_id = ? AND id != ?").all(userId, excludedPresetId);
    const changedPresetIds = [];
    const update = db.query("UPDATE presets SET prompt_order = ?, updated_at = ?, cache_revision = cache_revision + 1 WHERE id = ? AND user_id = ?");
    const now = Math.floor(Date.now() / 1000);
    const transaction = db.transaction(() => {
