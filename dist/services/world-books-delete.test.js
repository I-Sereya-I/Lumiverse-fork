...
mock.module("./embeddings.service", () => ({
    deleteWorldBookEmbeddingsBeforeSourceDelete: async (userId, worldBookIds, _lockEntryIds, deleteSource) => {
        const placeholders = worldBookIds.map(() => "?").join(", ");
        sourceVisibleDuringCleanup = worldBookIds.length === 0 || getDb().query(`SELECT COUNT(*) AS count FROM world_books WHERE user_id = ? AND id IN (${placeholders})`).get(userId, ...worldBookIds).count === worldBookIds.length;
        cleanupStarted?.();
        if (cleanupBarrier)
    ...
    deleteWorldBookEntryEmbeddingsBeforeSourceDelete: async (userId, entryIds, deleteSource) => {
        const placeholders = entryIds.map(() => "?").join(", ");
        sourceVisibleDuringCleanup = entryIds.length === 0 || getDb().query(`SELECT COUNT(*) AS count FROM world_book_entries WHERE id IN (${placeholders})`).get(...entryIds).count === entryIds.length;
        if (failCleanup)
            throw new Error("vector cleanup failed");
...
function insertBook(id, userId, metadata = {}) {
...
function insertEntry(id, bookId) {
...
describe("world-book cleanup-first deletion", () => {
    ...
    test("bulk activation switches selected entries to a single activation method", async () => {
        ...
        expect(getDb().query("SELECT constant, vectorized, vector_index_status, vector_indexed_at, vector_index_error FROM world_book_entries WHERE id = ?").get("entry-1")).toEqual({
            constant: 1,
            vectorized: 0,
    ...
    test("auto-managed character cleanup awaits vector deletion", async () => {
        ...
        cleanupBarrier = new Promise((resolve) => { releaseCleanup = resolve; });
        const started = new Promise((resolve) => { markCleanupStarted = resolve; });
