...
import { computeMessageTokenCount, } from "./message-token-count";
...
describe("computeMessageTokenCount", () => {
    ...
    test("returns undefined when no model can be resolved", async () => {
        expect(await computeMessageTokenCount("u1", "text", undefined, makeDeps({ resolveModel: () => null }))).toBeUndefined();
        expect(await computeMessageTokenCount("u1", "text", undefined, makeDeps({ resolveModel: () => undefined }))).toBeUndefined();
    });
