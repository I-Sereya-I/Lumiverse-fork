...
import { bulkUpdateCharacterFolders, createCharacter, deleteCharacterFolder, duplicateCharacter, getCharacter, listCharacterSummaries, renameCharacterFolder, } from "./characters.service";
...
describe("character folder operations", () => {
    ...
    test("bulk move deduplicates ids, skips missing and foreign characters, and trims the target", () => {
        ...
        const updated = bulkUpdateCharacterFolders(USER_A, [first.id, first.id, second.id, otherUser.id, "missing"], " Cast ");
        expect(updated.map((character) => character.id).sort()).toEqual([first.id, second.id].sort());
