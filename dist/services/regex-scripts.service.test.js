...
import { activatePresetBoundRegexScripts, applyRegexScripts, createRegexScript, deleteRegexScript, exportRegexScripts, getCharacterBoundScripts, getRegexScript, getRegexScriptByScriptId, getRegexScriptsByPresetId, getSpindleExtensionRegexFolderVersion, importRegexScripts, importCharacterBoundRegexScripts, installLumiHubPresetRegexScripts, importPresetBoundRegexScripts, resolveLumiHubPresetRegexInstallFolder, retireLumiHubPresetRegexScriptsForUpdate, reportRegexScriptPerformance, reportRegexScriptEvidence, switchPresetBoundRegexScripts, toggleRegexScript, toggleRegexScriptsByIds, toggleRegexScriptsByFolder, updateRegexScript, } from "./regex-scripts.service";
import { initMacros } from "../macros";
const USER_ID = "u1";
function mustGetScript(id) {
    const script = getRegexScript(USER_ID, id);
    expect(script).not.toBeNull();
    return script;
}
function runtimeScript(overrides) {
    return {
        id: "runtime-script",
        user_id: USER_ID,
        name: "Runtime script",
...
describe("extension regex ownership", () => {
    test("attributes an explicitly versioned Spindle folder without affecting unversioned scripts", () => {
        ...
        }, { extensionIdentifier: "extension.a", extensionFolderVersion: "2.4.0" });
    ...
    test("preserves, replaces, and clears protected Spindle folder attribution on update", () => {
        const created = createRegexScript(USER_ID, {
            ...
            find_regex: "versioned",
            folder: "Extension scripts",
        }, { extensionIdentifier: "extension.a", extensionFolderVersion: "1.0.0" });
    ...
    test("rejects invalid Spindle folder-version values", () => {
        expect(createRegexScript(USER_ID, {
            ...
            find_regex: "invalid",
            folder: "Extension scripts",
        }, { extensionIdentifier: "extension.a", extensionFolderVersion: 2 })).toBe("folder_version must be a string or null");
        expect(createRegexScript(USER_ID, {
            name: "Long version",
            find_regex: "long",
            folder: "Extension scripts",
        }, { extensionIdentifier: "extension.a", extensionFolderVersion: "v".repeat(101) })).toBe("folder_version exceeds maximum length (100 characters)");
    });
    ...
    test("treats unattributed, foreign, and preset-bound scripts as read-only without disabling them", async () => {
        const legacy = createRegexScript(USER_ID, { name: "Legacy", find_regex: "legacy" });
        ...
        expect(await applyRegexScripts("legacy", [mustGetScript(legacy.id)], "ai_output")).toBe("");
    });
    test("allows explicitly-authorized editors to mutate protected scripts without taking ownership", () => {
        ...
        const updatedLegacy = updateRegexScript(USER_ID, legacy.id, { name: "Edited legacy" }, context);
        ...
        expect(updateRegexScript(USER_ID, bound.id, { name: "Edited bound" }, context).name)
            .toBe("Edited bound");
...
describe("regex export", () => {
    ...
    test("preset export uses saved enablement even when preset is inactive", () => {
        ...
        expect(mustGetScript(enabled.id).disabled).toBe(true);
        expect(mustGetScript(disabled.id).disabled).toBe(true);
        const out = exportRegexScripts(USER_ID, { presetId: "preset-1" });
...
describe("preset-bound regex activation", () => {
    test("switching presets restores only the active preset's saved enabled set", () => {
        ...
        const presetOneEnabledId = presetOneEnabled.id;
        const presetOneDisabledId = presetOneDisabled.id;
        const presetTwoEnabledId = presetTwoEnabled.id;
...
describe("regex performance reporting", () => {
    ...
    test("clears a matching backend warning after a fast backend execution", async () => {
        ...
        await applyRegexScripts("one", [mustGetScript(script.id)], "ai_output", undefined, undefined, undefined, { source: "prompt_backend" });
...
describe("regex JSON overwrite imports", () => {
    test("standalone imports overwrite content while preserving preset ownership", () => {
        ...
        const updated = mustGetScript(created.id);
        expect(updated.name).toBe("Updated");
    ...
    test("isolates LumiHub preset script IDs and stamps version attribution", () => {
        ...
        const result = importPresetBoundRegexScripts(USER_ID, "preset-historical", "Historical preset", [{
        ...
        expect(mustGetScript(global.id)).toMatchObject({
            find_regex: "global",
            preset_id: null,
            script_id: "shared_preset_import",
        });
    ...
    test("archives only attributed older preset regexes and never a same-named local folder", () => {
        ...
        importPresetBoundRegexScripts(USER_ID, "preset-historical", "Historical preset", [{ name: "Bundled v1", find_regex: "v1", disabled: false }], { source: "lumihub", hubPresetId: "hub-preset-1", presetVersion: "1.0.0" });
        ...
        const currentFolder = resolveLumiHubPresetRegexInstallFolder(USER_ID, "preset-historical", "hub-preset-1", "Historical preset");
        expect(currentFolder).toBe("Historical preset · LumiHub");
        importPresetBoundRegexScripts(USER_ID, "preset-historical", "Historical preset", [{ name: "Bundled v2", find_regex: "v2", disabled: false }], {
            source: "lumihub",
            hubPresetId: "hub-preset-1",
            presetVersion: "2.0.0",
    ...
    test("moves a legacy unqualified LumiHub folder into the reserved namespace on update", () => {
        importPresetBoundRegexScripts(USER_ID, "preset-legacy-folder", "Legacy folder preset", [{ name: "Bundled v1", find_regex: "v1", disabled: false }], { source: "lumihub", hubPresetId: "hub-legacy-folder", presetVersion: "1.0.0" });
        expect(resolveLumiHubPresetRegexInstallFolder(USER_ID, "preset-legacy-folder", "hub-legacy-folder", "Legacy folder preset")).toBe("Legacy folder preset · LumiHub");
    ...
    test("preserves every payload folder through a LumiHub update", () => {
        const install = (version) => installLumiHubPresetRegexScripts(USER_ID, {
...
describe("raw capture processing", () => {
    test("applies macros without transferring a 300-group match to the host", async () => {
        ...
        expect(await applyRegexScripts("a".repeat(groupCount), [script], "ai_output", undefined, macroEnv)).toBe("A-a-a0");
    });
});
describe("find-only macro processing", () => {
    test("resolves the find pattern without resolving the replacement", async () => {
        ...
        expect(await applyRegexScripts("A", [script], "ai_output", undefined, macroEnv)).toBe("{{upper::x}}");
    });
});
describe("regex match actions", () => {
    test("moves only the first matching replacement to the top", async () => {
        ...
        expect(await applyRegexScripts("a [x] b [x]", [script], "ai_output")).toBe("<[x]>\na  b [x]");
    });
    test("moves a capture-expanded replacement to the bottom", async () => {
        ...
        expect(await applyRegexScripts("a [x] b", [script], "ai_output")).toBe("a  b\n<x>");
    });
    test("repeats a previous same-role match only when the current text misses", async () => {
        ...
        expect(await applyRegexScripts("new", [script], "ai_output", undefined, undefined, undefined, { previousContent: "old <status>ready</status>" })).toBe("new\n<strong>ready</strong>");
        expect(await applyRegexScripts("<status>new</status>", [script], "ai_output", undefined, undefined, undefined, { previousContent: "old <status>ready</status>" })).toBe("<strong>new</strong>");
    });
    ...
    test("resolves captures before raw macros in a carried replacement", async () => {
        ...
        expect(await applyRegexScripts("new", [script], "ai_output", undefined, macroEnv, undefined, { previousContent: "old <status>ready</status>" })).toBe("new\n<strong>READY</strong>");
    });
    test("uses explicit repeat position metadata after replacement normalization", async () => {
        ...
        expect(await applyRegexScripts("new", [script], "ai_output", undefined, undefined, undefined, { previousContent: "old [x]" })).toBe("new");
    });
    test("supports every repeat placement", async () => {
        ...
        for (const [repeatPosition, result] of Object.entries(expected)) {
            ...
            expect(await applyRegexScripts("new", [script], "ai_output", undefined, undefined, undefined, { previousContent: "old [x]" })).toBe(result);
        }
    });
});
describe("associative regex actions", () => {
    test("renders omitted optional named captures as empty action attributes", async () => {
        ...
        expect(typeof created).not.toBe("string");
        const output = await applyRegexScripts("<choice>North</choice>", [created], "ai_output", undefined, undefined, undefined, { source: "display_backend" });
    ...
    test("persists actions and resolves their capture templates per replacement", async () => {
        ...
        const output = await applyRegexScripts('[North|the trail]', [script], "ai_output", undefined, undefined, undefined, { source: "display_backend" });
    ...
    test("rejects captured state keys while preserving legacy actions without effects", () => {
        ...
        expect(legacy.actions[0].effects).toBeUndefined();
    ...
    test("resolves combined state, draft, and fork effects without a legacy send", async () => {
        ...
        expect(typeof created).not.toBe("string");
        const script = created;
        const output = await applyRegexScripts("[route:the rooftops]", [script], "ai_output", undefined, undefined, undefined, { source: "display_backend" });
        const encoded = output.match(/data-lumiverse-regex-action="([^"]+)"/)?.[1];
        expect(encoded).toBeTruthy();
    ...
    test("documentation examples import and resolve their action captures", async () => {
        const scenePayload = await Bun.file(new URL("../../user-docs/docs/assets/examples/regex-actions/scene-card-action.json", import.meta.url)).json();
        const multiPayload = await Bun.file(new URL("../../user-docs/docs/assets/examples/regex-actions/multi-select-scene-planner.json", import.meta.url)).json();
        ...
        const sceneOutput = await applyRegexScripts(`<scene><location>Moonlit Courtyard</location><description>A silver gate waits.</description><choice>Open the gate</choice></scene>`, [sceneScript], "ai_output", undefined, undefined, undefined, { source: "display_backend" });
        ...
        const multiOutput = await applyRegexScripts(`<scene-options><title>Sleeping City</title><budget>3</budget><route cost="2">Take the rooftops</route><companion cost="1">Bring Lyra</companion><tone cost="1">Keep it tense</tone></scene-options>`, [multiScript], "ai_output", undefined, undefined, undefined, { source: "display_backend" });
...
describe("regex evidence reporting", () => {
    ...
    test("report-evidence route rejects invalid bodies with 400 and accepts a valid patch", async () => {
        ...
        const post = (body) => app.request(url, {
            method: "POST",
