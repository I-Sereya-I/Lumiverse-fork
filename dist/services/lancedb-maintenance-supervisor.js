...
export function isLanceDbMaintenanceRunning() {
    return activeMaintenance !== null;
}
export function runLanceDbMaintenanceInChild(options) {
...
function maintenanceCommand(runtimePath) {
...
async function run({ mode, tableNames }) {
    ...
    const { pauseLanceDbForExternalMaintenance, refreshLanceDbAfterExternalMaintenance, } = await import("./vector-store/providers/lancedb");
