...
import { getPersonaBinding, resolveProfile, setPersonaBinding, setChatBinding, updateChatPromptVariables, updateCharacterPromptVariables, updateConnectionPromptVariables, getChatBinding, } from "./preset-profiles.service";
...
describe("persona preset profiles", () => {
    test("binds a preset state snapshot to a persona", () => {
        ...
        db.run("INSERT INTO presets (id, user_id, name, provider) VALUES (?, ?, ?, ?)", ["preset-1", USER, "RP", "openai"]);
    ...
    test("captures prompt-variable selections with the profile snapshot", () => {
        ...
        db.run("INSERT INTO personas (id, user_id, name, metadata) VALUES (?, ?, ?, '{}')", ["persona-1", USER, "Mode switcher"]);
        db.run("INSERT INTO presets (id, user_id, name, provider) VALUES (?, ?, ?, ?)", ["preset-1", USER, "RP", "openai"]);
        const binding = setPersonaBinding(USER, "persona-1", "preset-1", { style: true }, { style: { tone: "warm" } });
    ...
    test("lets a persona profile override a character profile but not a chat profile", () => {
        ...
        for (const id of ["persona-preset", "character-preset", "chat-preset"]) {
            db.run("INSERT INTO presets (id, user_id, name, provider) VALUES (?, ?, ?, ?)", [id, USER, id, "openai"]);
        }
        ...
        expect(resolveProfile(USER, "character-preset", "chat-1", "character-1", { personaId: "persona-1" })).toMatchObject({ preset_id: "persona-preset", source: "persona", source_id: "persona-1" });
        settingsSvc.putSetting(USER, "presetProfile:chat:chat-1", {
            preset_id: "chat-preset",
            block_states: { chatBlock: true },
            captured_at: 1,
        });
        expect(resolveProfile(USER, "character-preset", "chat-1", "character-1", { personaId: "persona-1" })).toMatchObject({ preset_id: "chat-preset", source: "chat", source_id: "chat-1" });
    });
});
