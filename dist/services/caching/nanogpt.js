...
export function isNanoGptClaudeModel(model) {
    if (!model)
        return false;
 ...
 */
export function applyNanoGptCaching(ctx, input) {
    ...
    const forceCacheCapableRouting = cacheSetting.forceCacheCapableRouting === true ||
    ...
    if (!isClaude && !forceCacheCapableRouting)
        return input;
    ...
    if (isClaude) {
        ...
        const cutAfterRaw = cacheSetting.cutAfterMessageIndex ?? cacheSetting.cut_after_message_index;
        if (typeof cutAfterRaw === "number" && Number.isInteger(cutAfterRaw) && cutAfterRaw >= 0) {
            payload.cutAfterMessageIndex = cutAfterRaw;
        }
        const explicitRaw = cacheSetting.explicitCacheControl ?? cacheSetting.explicit_cache_control;
        if (explicitRaw === true) {
