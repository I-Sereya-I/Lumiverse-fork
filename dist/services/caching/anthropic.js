...
function resolveConfig(metadata) {
    ...
    const breakpoints = record.breakpoints && typeof record.breakpoints === "object" && !Array.isArray(record.breakpoints)
        ? record.breakpoints
        : {};
    return {
        enabled: true,
...
function leadingSystemIndices(messages) {
    const indices = [];
...
function lastNonSystemIndex(messages) {
 ...
 */
function evenlySpacedIndices(indices, limit) {
    if (limit <= 0 || indices.length === 0)
        return [];
    ...
    for (let ordinal = 1; ordinal <= limit; ordinal++) {
        ...
        if (selected[selected.length - 1] !== index)
            selected.push(index);
    }
    return selected;
}
function applyMessageBreakpoints(messages, config, systemCheckpointLimit) {
    if (!config.enabled)
        return messages;
    ...
    if (systemIndices.length === 0 && lastConversationIdx === -1)
        return messages;
    const checkpointIndices = new Set(systemIndices);
    if (lastConversationIdx !== -1)
        checkpointIndices.add(lastConversationIdx);
    return messages.map((message, index) => {
        if (!checkpointIndices.has(index))
            return message;
        return { ...message, cache_control: config.cacheControl };
    });
}
function applyToolBreakpoints(tools, config) {
    if (!tools || !config.enabled || !config.breakpoints.tools)
        return tools;
 ...
 */
export function applyAnthropicCaching(ctx, input) {
    const cacheSetting = ctx.metadata?.prompt_caching;
    const params = cacheSetting === true ||
    ...
    const reservedBreakpoints = (config.breakpoints.tools && input.tools?.length ? 1 : 0) +
        (config.breakpoints.messages && lastNonSystemIndex(input.messages) !== -1 ? 1 : 0);
    const systemCheckpointLimit = Math.max(0, MAX_CACHE_BREAKPOINTS - reservedBreakpoints);
    return {
        params,
        messages: applyMessageBreakpoints(input.messages, config, systemCheckpointLimit),
