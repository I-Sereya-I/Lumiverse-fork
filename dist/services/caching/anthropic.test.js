...
describe("applyAnthropicCaching", () => {
    ...
    test("leaves messages and tools untouched without breakpoint config (no system message)", () => {
        ...
        const result = applyAnthropicCaching({ provider: "anthropic", metadata: { prompt_caching: true } }, { params: {}, messages, tools });
        expect(result.messages).toBe(messages);
        expect(result.tools).toBe(tools);
    });
    test("caches the system prefix by default when caching is enabled", () => {
        const result = applyAnthropicCaching({ provider: "anthropic", metadata: { prompt_caching: true } }, {
            params: {},
            messages: [
                { role: "system", content: "sys" },
                { role: "user", content: "hello" },
