...
import { defaultVectorStoreConfig, hasSameVectorStoreCredentialTarget, normalizeVectorStoreConfig, } from "./vector-store-config.service";
describe("normalizeVectorStoreConfig", () => {
    ...
    it("allows disabling the Milvus RPC timeout with 0", () => {
        expect(normalizeVectorStoreConfig({
            provider: "milvus",
            milvus: { address: "localhost:19530", requestTimeoutMs: 0 },
        }).milvus?.requestTimeoutMs).toBe(0);
    });
    ...
    it("NEVER carries secrets into the persisted config object", () => {
        ...
        expect(cfg.qdrant_api_key).toBeUndefined();
    });
