...
export function isHfHost(hostname) {
...
export async function hasHfToken() {
    const ownerId = getFirstUserId();
    if (!ownerId)
        return false;
...
export async function setHfToken(token) {
    const ownerId = getFirstUserId();
    if (!ownerId)
        throw new Error("No owner account found to store the HuggingFace token against.");
 ...
 */
export async function hfAuthHeaders(url) {
    let host;
    try {
