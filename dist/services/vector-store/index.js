...
import { getResolvedVectorStoreConfig, getVectorStoreConnectionSecrets, } from "../vector-store-config.service";
 ...
 */
export async function buildVectorStore(config, secrets) {
 ...
 */
export async function getActiveVectorStore() {
    if (!activeStore) {
...
export function resetActiveVectorStore() {
    if (activeStore) {
