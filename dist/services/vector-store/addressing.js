...
const WORLD_BOOK_COLLECTION = "embeddings_world_books";
// ---------------------------------------------------------------------------
// Identity + collection selection
// ---------------------------------------------------------------------------
/** Canonical composite row id. Stable across providers. */
export function rowId(userId, sourceType, sourceId, chunkIndex) {
...
export function collectionForSourceType(sourceType) {
...
export function selectCollection(rows) {
...
export function inSet(field, values) {
    return { op: "in", field, values };
}
export function notInSet(field, values) {
 ...
 */
export function andFilter(parts) {
    const clauses = parts.filter((p) => p != null);
    if (clauses.length === 1)
        return clauses[0];
    return { op: "and", clauses };
}
/** `user_id = … AND source_type = … AND owner_id = …` — the universal scope. */
export function ownerScope(userId, sourceType, ownerId) {
...
export function ownersScope(userId, sourceType, ownerIds) {
...
export function sourceIdsIn(ids) {
    const arr = ids ? [...ids] : [];
    if (arr.length === 0 || arr.length > MAX_SOURCE_FILTER_IDS)
        return null;
    return inSet("source_id", arr);
}
/** `source_id NOT IN (…)`, or `null` when the set exceeds {@link MAX_SOURCE_FILTER_IDS}. */
export function sourceIdsNotIn(ids) {
    const arr = ids ? [...ids] : [];
    if (arr.length === 0 || arr.length > MAX_SOURCE_FILTER_IDS)
        return null;
 ...
 */
export function toSimilarity(raw, scoreKind) {
 ...
 */
export function distanceFromSimilarity(similarity) {
 ...
 */
export function reciprocalRankFusion(vectorHits, lexicalHits) {
    ...
    for (let i = 0; i < lexicalHits.length; i++) {
        ...
        if (existing) {
            ...
            if (existing.lexicalScore == null && hit.lexicalScore != null)
 ...
 */
export function mmrSelect(candidates, queryVector, k, lambda = 0.7) {
...
export function isRetryableStorageError(err) {
    ...
    if (/\(413\)|\(500\)|\(503\)/.test(m))
        return true;
    if (/resources? exhausted|failed to allocate|hashjoininput/i.test(m))
        return true;
    if (/RESOURCE_EXHAUSTED|message larger than max|received message larger/i.test(m))
        return true;
    return false;
}
/**
 ...
 */
export async function adaptiveStorageBatch(items, initialBatchSize, processBatch, opts) {
    const isRetryable = opts.isRetryable ?? isRetryableStorageError;
    const run = async (batch, currentSize) => {
        try {
            ...
            if (isRetryable(error) && currentSize > 1) {
                ...
                console.warn(`[vector-store] ${opts.label}: batch of ${batch.length} failed (${error.message}); retrying in sub-batches of ${half}`);
