...
import { ensureVectorIndex, isCrossProcessLockFromPriorProcessInstance, isRetryableLanceWriteConflict, pauseLanceDbForExternalMaintenance, raceWithSignal, shouldUseCrossProcessWriteLock, sweepEmptyIndexDirs, WORLD_BOOK_EMBEDDINGS_TABLE, } from "./lancedb";
describe("lancedb write conflict handling", () => {
    ...
    test("recognizes a stale lock when a restarted container reuses its PID", () => {
        expect(isCrossProcessLockFromPriorProcessInstance({ pid: 1, acquiredAt: 1_000 }, 1, 2_000)).toBe(true);
    });
    test("keeps a lock acquired by the current process instance", () => {
        expect(isCrossProcessLockFromPriorProcessInstance({ pid: 1, acquiredAt: 2_000 }, 1, 1_000)).toBe(false);
    });
    test("detects Lance retryable commit conflicts from Windows warning text", () => {
        const err = new Error("lance error: Retryable commit conflict for version 786: "
    ...
    test("does not rebuild every scalar and FTS index after Lance optimize", () => {
        ...
        try {
          ...
          `,
            ...
            const { before, after } = JSON.parse(resultLine.slice(resultMarker.length));
        ...
        try {
          ...
          `,
            ...
            const resultLine = result.stdout
                ...
                .find((line) => line.startsWith(resultMarker));
            expect(resultLine).toBeDefined();
            const payload = JSON.parse(resultLine.slice(resultMarker.length));
