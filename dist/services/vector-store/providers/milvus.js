...
import { adaptiveStorageBatch, andFilter, cosineSimilarity, inSet, isRetryableStorageError, } from "../addressing";
...
export class MilvusStore {
    id = "milvus";
    capabilities = milvusCapabilities(null);
    config;
    password;
    tuningProfile;
    hybridSearchConfig;
    ...
    client = null;
    loaded = new Set();
    hybridFallbackCollections = new Set();
    constructor(config, password, tuningProfile, hybridSearchConfig) {
        if (!config?.address) {
    ...
    async init() {
        ...
        const version = await this.client.getVersion().then((res) => res?.version || res?.data || null).catch(() => null);
        this.capabilities = milvusCapabilities(version);
        const health = await this.client.checkHealth().catch((err) => {
            throw new Error(`Milvus health check failed for ${cfg.address}: ${describeMilvusError(err)}`);
        });
        if (health?.isHealthy === false)
            throw new Error("Milvus health check reported unhealthy.");
    }
    ...
    async getStoredDimension(collection) {
        ...
        const field = desc?.schema?.fields?.find((f) => f.name === "vector");
        const dim = field?.dim ?? field?.type_params?.find?.((p) => p.key === "dim")?.value;
    ...
    async upsert(collection, rows) {
        ...
        await adaptiveStorageBatch(rows, initialBatchSize, async (batch) => {
    ...
    async getRowsByFilter(collection, filter, limit = 10_000) {
        ...
        return rows.map(milvusRowToVectorRow).filter((row) => row != null);
    }
    ...
    async vectorSearch(opts) {
        ...
        return results.map((row) => milvusSearchRowToHit(row, opts.withVector)).filter((hit) => hit != null);
    }
    async lexicalSearch(opts) {
        if (opts.signal?.aborted || !this.capabilities.nativeLexical || !opts.queryText.trim())
            return [];
        const client = await this.getClient();
        const name = this.collectionName(opts.collection);
        if (!(await this.hasCollection(name)) || !(await this.hasSparseField(name)))
            return [];
        await this.loadCollection(name);
        try {
            const res = await client.search({
            ...
            if (opts.signal?.aborted)
                return [];
            const results = Array.isArray(res?.results) ? res.results : [];
            return results.map((row) => milvusSearchRowToHit(row, opts.withVector, true)).filter((hit) => hit != null);
        }
        catch (err) {
    ...
    async hybridSearch(opts) {
        ...
        try {
            ...
            const results = Array.isArray(res?.results) ? res.results : [];
            let hits = results
                .map((row) => milvusSearchRowToHit(row, true))
                .filter((hit) => hit != null)
                .map((hit) => {
            ...
            const unresolvedIds = hits
                .filter((hit) => hit.similarity == null && hit.id)
                .map((hit) => hit.id);
            if (unresolvedIds.length > 0 && !opts.signal?.aborted) {
                try {
                    ...
                    const rescoredById = new Map(rescored.map((hit) => [hit.id, hit]));
                    hits = hits.map((hit) => {
                    ...
                    if (opts.signal?.aborted)
                        return [];
                    console.warn("[vector-store] Milvus could not dense-rescore %d native hybrid hit(s); leaving their cosine similarity unknown: %s", unresolvedIds.length, describeMilvusError(err));
                }
            }
            if (opts.signal?.aborted)
                return [];
            ...
            if (fusedHits.length > 0) {
                ...
                console.warn("[vector-store] Milvus hybrid search returned 0 rows for %s despite dense/BM25 hits; switching this collection to app-side fusion for the rest of the process.", name);
                return fusedHits;
            }
            return [];
        }
            ...
            this.hybridFallbackCollections.add(name);
            console.warn("[vector-store] Milvus hybrid search failed for %s; switching to app-side dense/BM25 fusion: %s", name, describeMilvusError(err));
            return this.appSideHybridSearch(opts);
        }
    }
    ...
    async optimize(_collections) {
        for (const collection of _collections ?? ["embeddings", "embeddings_world_books"]) {
            const name = this.collectionName(collection);
            if (await this.hasCollection(name)) {
                await this.flush(name);
                await this.loadCollection(name, true);
    ...
    async health(collection) {
        ...
        const [rowCount, dimension, indexes] = await Promise.all([
            ...
            client.listIndexes({ collection_name: name }).then((res) => res?.index_descriptions || res?.index_names || []).catch(() => []),
        ]);
        return {
            exists: true,
            rowCount,
            ...
            indexes: Array.isArray(indexes) ? indexes.map((idx) => ({ name: String(idx.index_name || idx.name || idx), type: idx.index_type })) : [],
            dimension,
        };
    }
    async reset() {
    ...
    async close() {
        if (this.client?.closeConnection)
    ...
    async loadCollection(name, refresh = false) {
        if (!refresh && this.loaded.has(name))
            return;
        const client = await this.getClient();
    ...
    async appSideHybridSearch(opts) {
    ...
    async ensureScalarIndexes(name) {
        const client = await this.getClient();
    ...
    async hasSparseField(name) {
        const client = await this.getClient();
        const desc = await client.describeCollection({ collection_name: name, cache: true });
        return !!desc?.schema?.fields?.some((field) => field.name === SPARSE_FIELD);
    }
    async ensureSparseIndexIfPresent(name) {
        if (!this.capabilities.nativeLexical || !(await this.hasSparseField(name).catch(() => false)))
            return;
        const client = await this.getClient();
        await assertOk(client.createIndex({
            collection_name: name,
...
function milvusRowToVectorRow(row) {
    const vector = parseVector(row?.vector);
    if (!row?.id || vector.length === 0)
        return null;
    return {
        id: String(row.id),
...
function milvusSearchRowToHit(row, withVector, lexicalOnly = false) {
    if (!row?.source_id)
        return null;
    return {
        id: String(row.id || ""),
...
function fuseHits(vectorHits, lexicalHits) {
    if (vectorHits.length === 0 && lexicalHits.length === 0)
        return [];
...
function clipVarchar(value) {
    if (value.length <= VARCHAR_MAX)
        return value;
    return `${value.slice(0, VARCHAR_MAX - 22)}\n[content clipped]`;
}
function milvusTuning(profile) {
    switch (profile) {
        case "low_latency":
            return {
...
function milvusHybridCandidateLimit(requestedLimit, hybridSearchConfig) {
    const candidateMultiplier = Math.max(1, Math.floor(hybridSearchConfig?.candidateMultiplier ?? DEFAULT_HYBRID_CANDIDATE_MULTIPLIER));
    const candidateCap = Math.max(1, Math.floor(hybridSearchConfig?.candidateCap ?? DEFAULT_HYBRID_CANDIDATE_CAP));
    return Math.max(requestedLimit, Math.min(requestedLimit * candidateMultiplier, candidateCap));
}
function milvusConnectTimeoutMs(config) {
...
function milvusRequestTimeoutMs(config) {
...
async function assertTcpReachable(address, ssl, timeoutMs) {
...
function parseMilvusAddress(address, ssl) {
...
function describeMilvusError(err) {
