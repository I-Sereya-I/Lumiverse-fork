...
import { getChatPersonaAddonStates, getChatPersonaAddonToggleOrder, personaHasAddon, } from "../services/persona-addon-states";
const app = new Hono();
function collectPersonaImageIds(persona) {
    const ids = new Set();
    if (persona.image_id)
        ids.add(persona.image_id);
    ...
    const originalImageId = typeof persona.metadata?.original_image_id === "string"
        ...
        : null;
    if (originalImageId)
        ids.add(originalImageId);
    for (const addons of [persona.metadata?.addons, persona.metadata?.attached_global_addons]) {
        if (!Array.isArray(addons))
            continue;
        for (const addon of addons) {
            if (typeof addon?.avatar_image_id === "string")
                ids.add(addon.avatar_image_id);
            if (typeof addon?.avatar_crop_image_id === "string")
                ids.add(addon.avatar_crop_image_id);
        }
    }
    return [...ids];
}
...
function sanitizeImportedPersonaMetadata(userId, value) {
    const metadata = isRecord(value)
        ? JSON.parse(JSON.stringify(value))
        : {};
    let skippedAssetReferences = 0;
    const keepOwnedImageReference = (record, key) => {
        const imageId = record[key];
        if (typeof imageId !== "string" || !imageId)
            return;
        if (images.getImage(userId, imageId))
            return;
        delete record[key];
...
app.post("/bulk-update", async (c) => {
    ...
    if (ids.length === 0)
        return c.json({ error: "ids must contain persona ids" }, 400);
    const input = {};
    if (typeof body.folder === "string")
        input.folder = body.folder;
    ...
    if (body.toggle_narrator === true)
        input.toggle_narrator = true;
    if (Object.keys(input).length === 0)
        return c.json({ error: "No bulk update action supplied" }, 400);
...
app.post("/bulk-delete", async (c) => {
    ...
    for (const persona of personas) {
        if (svc.deletePersona(userId, persona.id))
    ...
    const avatarPaths = new Set(personas.map((persona) => persona.avatar_path).filter((path) => !!path));
    for (const avatarPath of avatarPaths) {
        if (!svc.isPersonaAvatarPathReferenced(userId, avatarPath))
            await files.deleteAvatar(avatarPath);
    }
...
app.delete("/:id", async (c) => {
    ...
    if (persona.avatar_path)
        await files.deleteAvatar(persona.avatar_path);
...
app.put("/:id/addons/:addonId/avatar", async (c) => {
    ...
    if (!images.getImage(userId, body.image_id))
...
app.post("/:id/addons/:addonId/avatar", async (c) => {
    ...
    if (!updated) {
        ...
        if (cropImage)
            images.deleteImageIfUnreferenced(userId, cropImage.id);
        return c.json({ error: "Add-on is not attached to this persona" }, 404);
    }
    for (const imageId of oldImageIds)
        images.deleteImageIfUnreferenced(userId, imageId);
...
app.post("/:id/avatar", async (c) => {
    ...
    if (cropImage)
        nextMetadata.avatar_crop_image_id = cropImage.id;
    else
        delete nextMetadata.avatar_crop_image_id;
