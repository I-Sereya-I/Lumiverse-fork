...
const { listEmbeddingDrivers, commitEmbeddingRegistryProvider, revokeEmbeddingRegistryProvider, publishEmbeddingProviderRegistryChanged, } = await import("../services/embeddings.service");
...
describe("embeddings routes provider registry", () => {
    test("emits scoped provider_changed add remove and change after registry commit", () => {
        ...
        expect(emitCalls.map((call) => call.payload.action)).toEqual(["add", "change", "remove"]);
    ...
    test("renders unavailable and timeout fallback", () => {
        providerRegistry.register({ kind: "embedding", id: "down-embed", description: { status: "unavailable" } }, host);
        providerRegistry.register({ kind: "embedding", id: "slow-embed", description: { availability: "timeout" } }, host);
        const listed = listEmbeddingDrivers({ userId: USER_ID });
    ...
    test("denied registration is not visible to consumers", () => {
        providerRegistry.register({ kind: "embedding", id: "denied-embed", description: { denied: true } }, host);
        providerRegistry.register({ kind: "embedding", id: "other-user-embed" }, { installationId: "inst-other", installScope: "user", authenticatedSubject: "intruder" });
    ...
    test("an omitted caller is restricted to system providers", () => {
        ...
        providerRegistry.register({ kind: "embedding", id: "system-embed" }, { installationId: "inst-system", installScope: "system" });
    ...
    test("provider failure is isolated", () => {
        ...
        providerRegistry.register({ kind: "embedding", id: "broken-embed", description: Object.create(null, {
