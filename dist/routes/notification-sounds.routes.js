...
app.get("/completion", (c) => {
    ...
    response.headers.set("Content-Disposition", `inline; filename="${stored.filename}"`);
    return response;
});
app.post("/completion", async (c) => {
    ...
    if (file.size > svc.MAX_NOTIFICATION_SOUND_BYTES) {
        return c.json({ error: "Audio file too large", maxBytes: svc.MAX_NOTIFICATION_SOUND_BYTES }, 413);
    }
    try {
