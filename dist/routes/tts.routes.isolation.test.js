...
async function listedProviderIds(userId) {
...
describe("tts routes multi-tenant isolation", () => {
    ...
    test("another user cannot resolve or invoke alice's provider", async () => {
        ...
        await expect(providerRegistry.invoke(aliceKey("alice-private-tts"), {}, { callerScope: `user:${BOB}` })).rejects.toThrow("provider invoke is isolated to the caller scope");
        await expect(providerRegistry.invoke(aliceKey("alice-private-tts"), {}, { callerScope: "system" })).rejects.toThrow("provider invoke is isolated to the caller scope");
    });
    test("real caller scope is threaded into invokes: users may invoke system providers but never other users' providers", async () => {
        ...
        await expect(providerRegistry.invoke(sysKey, {}, { callerScope: `user:${BOB}` })).rejects.toThrow("gate_passed");
        ...
        await expect(providerRegistry.invoke(aliceKey2, {}, { callerScope: "system" })).rejects.toThrow("provider invoke is isolated to the caller scope");
    });
});
