...
app.post("/:id/documents", async (c) => {
    ...
    if (!bank)
        return c.json({ error: "Databank not found" }, 404);
...
app.post("/:id/documents/scrape", async (c) => {
    ...
    try {
        ...
        const doc = databank.createDocument(userId, databankId, docName, filename, `text/${ext === ".md" ? "markdown" : "plain"}`, Buffer.byteLength(fileContent), hash);
...
app.post("/mentions/resolve", async (c) => {
    ...
    if (!content)
        return c.json({ error: "Document has no content" }, 404);
