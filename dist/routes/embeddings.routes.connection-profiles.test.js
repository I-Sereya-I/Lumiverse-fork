...
describe("embeddings routes connection profiles", () => {
    test("GET /config returns dedicated profiles with hasSecret and no secret refs", async () => {
        spies.push(spyOn(embeddingsSvc, "getEmbeddingConfig").mockResolvedValue(structuredClone(configDto)));
    ...
    test("PUT /config forwards connectionProfiles and primary/fallback ids", async () => {
        ...
        const forwarded = update.mock.calls.at(-1)?.[1];
    ...
    test("POST /test returns embedding_fallback_exhausted", async () => {
        spies.push(spyOn(embeddingsSvc, "getEmbeddingConfig").mockResolvedValue(structuredClone(configDto)), spyOn(embeddingsSvc, "testEmbeddingConfig").mockRejectedValue(Object.assign(new Error("All embedding providers failed"), {
    ...
    test("POST /test returns embedding_provider_unavailable", async () => {
        spies.push(spyOn(embeddingsSvc, "getEmbeddingConfig").mockResolvedValue(structuredClone(configDto)), spyOn(embeddingsSvc, "testEmbeddingConfig").mockRejectedValue(Object.assign(new Error("Embedding provider is unavailable"), {
