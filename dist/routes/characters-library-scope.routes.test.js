...
function initCharactersScopeTestDb() {
    closeDatabase();
    initDatabase(":memory:");
...
function insertCharacter(id, userId, name, libraryScope, tags, folder, extensions, createdAt, updatedAt) {
    getDb().query(`INSERT INTO characters (
    id, user_id, name, library_scope, folder, tags, extensions, created_at, updated_at
  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, userId, name, libraryScope, folder, JSON.stringify(tags), JSON.stringify(extensions), createdAt, updatedAt);
}
function seedCharacters() {
    insertCharacter(MINE_ID, USER_ID, "Mine Character", "mine", ["mine-tag", "common"], "Shelf", { _lumiverse_library_scope: "mine", world_book_ids: [MINE_BOOK_ID] }, 1, 10);
    insertCharacter(SHARED_ID, USER_ID, "Shared Character", "shared", ["shared-tag", "common"], "Shelf", { _lumiverse_library_scope: "shared" }, 2, 20);
    insertCharacter(SHARED_UNLISTED_ID, USER_ID, "Shared Without Chat", "shared", ["shared-only"], "Other Shelf", { _lumiverse_library_scope: "shared" }, 3, 30);
    insertCharacter(OTHER_ID, OTHER_USER_ID, "Other User Character", "shared", ["private-tag"], "Shelf", { _lumiverse_library_scope: "shared" }, 4, 40);
    ...
    VALUES (?, ?, ?, '{}', ?, ?, ?)`).run(PREVIEW_CHAT_ID, MINE_ID, "Preview Chat", 30, 40, USER_ID);
    getDb().query(`INSERT INTO chats
    (id, character_id, name, metadata, created_at, updated_at, user_id)
    VALUES (?, ?, ?, '{}', ?, ?, ?)`).run(SHARED_CHAT_ID, SHARED_ID, "Shared Chat", 31, 41, USER_ID);
    getDb().query(`INSERT INTO messages
    (id, chat_id, index_in_chat, is_user, name, content, send_date, created_at)
    VALUES (?, ?, 0, 0, '', ?, 40, 40)`).run("message-preview", PREVIEW_CHAT_ID, "Preview message");
}
const app = new Hono();
...
async function characterFrom(response) {
    const body = await bodyOf(response);
    return body.character ?? body;
}
function storedCharacter(id) {
    const row = getDb().query("SELECT * FROM characters WHERE id = ?").get(id);
    if (!row || typeof row !== "object")
        throw new Error(`Missing stored character ${id}`);
    const record = row;
...
describe("character summary scope filtering", () => {
    test("omitted scope returns every owner row while explicit scope partitions summaries", async () => {
        ...
        expect(all.data.map((item) => item.id).sort()).toEqual([MINE_ID, SHARED_ID, SHARED_UNLISTED_ID].sort());
        ...
        const mine = await bodyOf(mineResponse);
        ...
        const shared = await bodyOf(sharedResponse);
        expect(shared.data.map((item) => item.id).sort()).toEqual([SHARED_ID, SHARED_UNLISTED_ID].sort());
    ...
    test("threads chat_id through the summary query and rejects malformed scope strictly", async () => {
        ...
        expect(await bodyOf(malformed)).toEqual({
...
describe("owner-only character collections", () => {
    test("lists all owned scopes and tags without exposing another user's rows", async () => {
        ...
        const list = await bodyOf(listResponse);
        expect(list.data.map((item) => item.id).sort()).toEqual([MINE_ID, SHARED_ID, SHARED_UNLISTED_ID].sort());
        expect(list.data.some((item) => item.id === OTHER_ID)).toBe(false);
        const tagsResponse = await app.request("/tags");
        expect(tagsResponse.status).toBe(200);
        const tags = Object.fromEntries((await bodyOf(tagsResponse)).map((item) => [item.tag, item.count]));
        expect(tags).toEqual({
            common: 2,
            "mine-tag": 1,
            "shared-tag": 1,
...
describe("character CRUD scope classification", () => {
    test("creates with optional mine default or shared library_scope and mirrors the persisted scope", async () => {
        ...
        const names = (await bodyOf(await app.request("/summary"))).data.map((item) => item.name);
...
describe("character imports", () => {
    ...
    test("classifies multipart imports from the library_scope form field", async () => {
        ...
        form.append("file", new File([JSON.stringify({
                ...
                extensions: { _lumiverse_library_scope: "shared" },
            })], "form-card.json", { type: "application/json" }));
        ...
        invalidForm.append("file", new File([JSON.stringify({ name: "Bad Form Import" })], "bad-card.json", { type: "application/json" }));
...
describe("homepage character preview", () => {
    test("serves the sole nested preview shape for an owned character", async () => {
        ...
        expect(response.status).toBe(200);
        const preview = await bodyOf(response);
