...
import { readComfyUIConfig, readComfyUIWorkflowLibrary, syncActiveComfyUIWorkflowToLibrary, writeComfyUIConfig, writeComfyUIWorkflowLibrary, } from "../image-gen/comfyui-workflow-storage";
...
function isComfyCapableConnection(provider) {
    return provider === "comfyui" || provider === "swarmui";
}
function summarizeComfyWorkflowLibrary(library) {
    return {
...
function getComfyCapableConnection(userId, connectionId) {
    const connection = svc.getConnection(userId, connectionId);
    if (!connection)
        return { connection: null, error: "Connection not found", status: 404 };
    if (!isComfyCapableConnection(connection.provider)) {
        return { connection: null, error: "Connection does not support ComfyUI workflows", status: 400 };
    }
    return { connection, error: null, status: null };
}
async function resolveComfyConnectionTarget(userId, connection) {
...
app.get("/", async (c) => {
    ...
    const result = await withReadableApiKeyStatuses(userId, svc.listConnections(userId, pagination), imageGenConnectionSecretKey);
...
app.post("/:id/comfyui/workflow/import", async (c) => {
    ...
    if (detectComfyUIWorkflowFormat(workflow) === "ui_workflow") {
        return c.json({
            error: "That's the UI workflow export. Import the API-format export instead: in ComfyUI enable Settings -> Enable Dev mode Options, then use Save (API Format).",
        }, 400);
    }
...
app.put("/:id/comfyui/workflow/mappings", async (c) => {
    ...
    const config = { ...existing, field_mappings: body.mappings };
    await svc.updateConnection(userId, connectionId, {
        metadata: syncActiveComfyUIWorkflowToLibrary(connection.metadata, config, Date.now()),
    });
...
app.post("/:id/comfyui/workflows/:workflowId/activate", async (c) => {
    ...
    const metadata = writeComfyUIWorkflowLibrary(writeComfyUIConfig(connection.metadata, entry.config), next);
...
app.get("/:id/comfyui/capabilities", async (c) => {
    ...
    const target = await resolveComfyConnectionTarget(userId, connection);
    const capabilities = await discoverCapabilities(target.baseUrl, c.req.query("refresh") === "1", { cookie: target.cookie });
...
app.get("/:id/nanogpt-usage", async (c) => {
    ...
    if (!result)
        return c.json({ error: "Failed to fetch NanoGPT usage" }, 502);
    return c.json(result);
});
/** List models for a specific component subtype (e.g. "vae", "text_encoders") */
app.get("/:id/models/:subtype", async (c) => {
    const userId = c.get("userId");
    const result = await svc.listConnectionModelsBySubtype(userId, c.req.param("id"), c.req.param("subtype"));
    return c.json(result);
});
