...
app.put("/settings/character/:characterId", async (c) => {
    ...
    const body = await c.req.json();
    try {
        const updated = councilProfilesSvc.putCharacterBinding(userId, c.req.param("characterId"), body ?? {});
        return c.json(updated);
    }
    catch (e) {
...
app.put("/settings/chat/:chatId", async (c) => {
    ...
    const body = await c.req.json();
    try {
        const updated = councilProfilesSvc.putChatBinding(userId, c.req.param("chatId"), body ?? {});
        return c.json(updated);
    }
    catch (e) {
