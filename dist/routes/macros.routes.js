...
import { normalizePromptBlockText, resolvePromptBlockPlacements, populateLumiaLoomContext, resolvePromptVariables, } from "../services/prompt-assembly.service";
...
async function buildEnvFromIds(userId, body) {
        ...
        if (chat) {
            ...
            if (character) {
                const persona = isTemporaryChatMetadata(chat.metadata)
                    ? null
                    : resolvePersonaForChatMacros(userId, personasSvc.resolvePersonaOrDefault(userId, body.persona_id), chat.metadata);
    ...
    if (body.character_id) {
        ...
        if (character) {
            ...
            const persona = resolvePersonaForChatMacros(userId, personasSvc.resolvePersonaOrDefault(userId, body.persona_id), null);
            const connection = connectionsSvc.resolveConnection(userId, body.connection_id);
            const chat = {
                id: "",
...
function seedCortexPreviewContext(userId, chatId, env) {
    const config = memoryCortex.getCortexConfig(userId);
    const chat = chatsSvc.getChat(userId, chatId);
    if (!memoryCortex.isCortexEnabledForChat(config, chat?.metadata))
        return;
    ...
    const relations = memoryCortex
        .getRelations(chatId)
        .filter((relation) => entityIds.has(relation.sourceEntityId) ||
...
function seedPromptVariablesForPreview(env, body) {
    if (!Array.isArray(body.prompt_blocks) || body.prompt_blocks.length === 0)
        return;
    const preset = {
        id: "preview",
        name: "Preview",
...
async function evaluateWithPromptBlockContext(template, env, body) {
    if (!body.prompt_blocks?.length)
        return evaluate(template, env, registry);
