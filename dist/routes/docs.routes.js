...
function extractDocumentTitle(markdown, path) {
...
async function buildGuideCatalog() {
    ...
    for await (const relativePath of glob.scan({
        ...
        const file = Bun.file(resolve(DOCS_ROOT, relativePath));
        const markdown = await file.text();
        entries.push({
            path: normalizedPath,
            title: extractDocumentTitle(markdown, normalizedPath),
        });
    }
...
function resolveDocumentPath(requestPath) {
    ...
    if (filePath !== DOCS_ROOT &&
        !filePath.startsWith(`${DOCS_ROOT}${sep}`)) {
        return null;
    }
    return filePath;
}
async function documentResponse(requestPath) {
...
docsRoutes.get("/file/:encoded", async (c) => {
    ...
    const requestPath = Buffer.from(encoded, "base64url").toString("utf8");
    if (!requestPath) {
        return c.json({ error: "Invalid document key" }, 400);
    }
...
docsRoutes.get("/*", async (c) => {
    ...
    let requestPath;
    try {
        requestPath = decodeURIComponent(c.req.path.slice(prefix.length));
    }
    catch {
