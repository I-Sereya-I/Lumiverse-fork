...
function touchesRuntimeConfig(input) {
...
app.put("/:id", async (c) => {
    ...
    if (transportType === "stdio") {
        try {
            assertStdioLaunchAllowed(body.command !== undefined ? body.command : existing.command, body.args !== undefined ? body.args : existing.args);
        }
        catch (err) {
            return c.json({ error: err.message || "Invalid MCP stdio launch configuration" }, 400);
        }
