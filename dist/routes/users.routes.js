...
function isOwnerSession(c) {
    return c.get("session")?.user?.role === "owner";
}
function canManageTarget(c, targetRole) {
    if (isOwnerSession(c))
        return true;
...
app.post("/me/password", passwordLimiter, async (c) => {
    ...
    getDb().run('UPDATE account SET password = ? WHERE userId = ? AND providerId = ?', [hashed, session.user.id, "credential"]);
    // Revoke all other sessions so stolen tokens are invalidated
    getDb().run("DELETE FROM session WHERE userId = ? AND id != ?", [session.user.id, session.session.id]);
...
admin.post("/:id/reset-password", passwordLimiter, async (c) => {
    ...
    const hashed = await hashPassword(body.newPassword);
    const result = getDb().run('UPDATE account SET password = ? WHERE userId = ? AND providerId = ?', [hashed, id, "credential"]);
    if (result.changes === 0) {
        return c.json({ error: "User not found" }, 404);
    }
...
admin.post("/:id/unban", async (c) => {
    ...
    const result = getDb().run('UPDATE "user" SET banned = 0, banReason = NULL, banExpires = NULL WHERE id = ?', [id]);
    if (result.changes === 0) {
        return c.json({ error: "User not found" }, 404);
    }
    return c.json({ success: true });
