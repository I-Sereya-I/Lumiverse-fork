...
function applyAssetHeaders(response, mimeType, filename) {
...
app.get("/", (c) => {
    ...
    if (!bundleId)
        return c.json({ error: "bundle_id query param is required" }, 400);
    try {
...
app.post("/", async (c) => {
    ...
    if (!file)
        return c.json({ error: "asset file is required" }, 400);
...
app.get("/bundles/:bundleId/*", async (c) => {
    ...
    const slug = c.req.path.startsWith(routePrefix)
        ...
        : "";
    if (!slug)
        return c.json({ error: "Asset slug is required" }, 400);
    try {
