...
import { getEphemeralPoolOverview, getEphemeralPoolConfig, updateEphemeralPoolConfig, } from "../spindle/ephemeral-pool.service";
...
app.get("/updates", async (c) => {
    const viewer = getViewer(c);
    const manageableIds = new Set(managerSvc.getManageableExtensionIdsForUser(viewer.userId, viewer.role));
    const snapshot = updateCheckSvc.getExtensionUpdateSnapshot();
    return c.json({
        ...snapshot,
        updates: snapshot.updates.filter((update) => manageableIds.has(update.extensionId)),
    });
});
...
app.get("/ephemeral/overview/me", async (c) => {
    ...
    const visibleIds = new Set((await managerSvc.listForUser(viewer.userId, viewer.role)).map((ext) => ext.id));
    const visibleExtensions = overview.extensions.filter((row) => visibleIds.has(row.extensionId));
    const visibleUsedBytes = visibleExtensions.reduce((sum, row) => sum + row.usedBytes, 0);
    const visibleReservedBytes = visibleExtensions.reduce((sum, row) => sum + row.reservedBytes, 0);
    return c.json({
        role: viewer.role,
        ...
        global: {
            ...
            availableBytes: Math.max(0, overview.global.maxBytes - visibleUsedBytes - visibleReservedBytes),
        },
...
app.put("/ephemeral/config", requireOwner, ephemeralReauthLimiter, async (c) => {
    try {
        ...
        const account = getDb()
            ...
            .get(session.user.id, "credential");
        if (!account) {
            return c.json({ error: "Invalid credentials" }, 403);
        }
...
app.post("/install", requireOwner, async (c) => {
    ...
    try {
        ...
        const requestedScope = typeof body.scope === "string" && body.scope.trim() ? body.scope.trim() : "";
        const installScope = requestedScope === "user" ? "user" : "operator";
        const installedByUserId = installScope === "user"
...
app.post("/:id/restart", async (c) => {
    try {
        ...
        if (!canManageExtension(c, ext))
            return c.json({ error: "Forbidden" }, 403);
        if (!ext.enabled)
            return c.json({ error: "Extension is not enabled" }, 400);
        eventBus.emit(EventType.SPINDLE_EXTENSION_STATUS, {
            extensionId: ext.id,
...
app.post("/:id/permissions", async (c) => {
    try {
        ...
        const isPrivileged = viewer.role === "owner" || viewer.role === "admin";
        if (body.grant) {
            const privilegedRequested = body.grant.filter((p) => PRIVILEGED_PERMISSIONS.has(p));
