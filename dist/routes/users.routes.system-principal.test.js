...
const { putSecret, SYSTEM_SECRET_PRINCIPAL, SYSTEM_SECRET_PRINCIPAL_EMAIL, } = await import("../services/secrets.service");
...
function insertUser(id, email, role, createdAt) {
    getDb()
...
describe("admin user routes vs reserved system principal", () => {
    ...
    test("DELETE of the system principal is rejected with 409 without cascading secrets", async () => {
        ...
        expect(getDb().query('SELECT COUNT(*) as count FROM "user" WHERE id = ?').get(SYSTEM_SECRET_PRINCIPAL)).toEqual({ count: 1 });
        expect(getDb().query("SELECT COUNT(*) as count FROM secrets WHERE user_id = ?").get(SYSTEM_SECRET_PRINCIPAL)).toEqual({ count: 1 });
    });
    ...
    test("deleting a regular user still works", async () => {
        ...
        expect(getDb().query('SELECT COUNT(*) as count FROM "user" WHERE id = ?').get("u2")).toEqual({ count: 0 });
    });
});
