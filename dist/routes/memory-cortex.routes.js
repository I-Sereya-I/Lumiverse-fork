 ...
 */
function ensureChatOwnership(c, chatId) {
    const userId = c.get("userId");
    const chat = getChat(userId, chatId);
    if (!chat)
        return { ok: false, response: c.json({ error: "Chat not found" }, 404) };
...
function uniqueStringIds(value, max = 5000) {
...
async function resolveCortexParticipants(userId, chat) {
    const characterNames = [];
    const aliasMaps = [];
    if (!chat)
        return { characterNames, descriptionAliases: undefined };
...
const passiveWarmups = new Set();
function getChatChunkCount(chatId) {
    const row = getDb()
        .query("SELECT COUNT(*) as chunkCount FROM chat_chunks WHERE chat_id = ?")
        .get(chatId);
    return row?.chunkCount ?? 0;
}
function parseStoredCortexFreshness(chat) {
...
function getStoredChatMemoryHash(chat) {
    const hash = chat?.metadata?.ltcm_config_hash;
    return typeof hash === "string" && hash.trim().length > 0 ? hash : null;
}
function buildCortexFreshnessSnapshot(chatId, cortexConfig, ltcmConfigHash) {
    return {
        ltcmConfigHash,
...
function isCortexFresh(chat, snapshot) {
...
function stampCortexFreshnessSnapshot(userId, chatId, snapshot) {
    const chat = getChat(userId, chatId);
    if (!chat)
        return;
 ...
 */
function stampCortexRebuildAttempt(userId, chatId) {
    const chat = getChat(userId, chatId);
    if (!chat)
        return;
    const existing = parseStoredCortexFreshness(chat);
...
function startTrackedCortexRebuild(options) {
    ...
    if (existing) {
        ...
        if (!isForce || existing.kind === "force")
            return;
...
function launchTrackedCortexRebuild(options, startAfter) {
    ...
    entry.done = (startAfter ?? Promise.resolve()).then(() => enqueueChatPipelineTask({
        chatId,
        kind: source === "warmup" ? "cortex_warmup" : "cortex_rebuild",
        exclusive: true,
        preflight: () => (abort.signal.aborted
            ? { action: "skip", reason: "aborted_before_start" }
            : { action: "run" }),
        run: () => memoryCortex.rebuildCortex(userId, chatId, characterNames, generateRawFn, sidecarConnectionId, (rebuildState) => {
        ...
        }, descriptionAliases, { resumable: source === "warmup", warmupSignature: snapshot.rebuildSignature, signal: abort.signal }),
    }).then((scheduled) => {
        if (scheduled.status !== "completed" || !scheduled.value)
            return;
        ...
        if (cortexRebuildsInFlight.get(chatId) === entry)
            cortexRebuildsInFlight.delete(chatId);
    }));
}
...
async function performChatWarmup(userId, chatId, force) {
    const chat = getChat(userId, chatId);
    if (!chat) {
        return {
            status: "skipped",
    ...
    try {
        memoryCortex.migrateLegacyChunkSignatures(chatId);
    }
    catch (err) {
...
function startPassiveChatWarmup(userId, chatId) {
    const key = `${userId}:${chatId}`;
    if (passiveWarmups.has(key))
        return false;
...
function inspectCortexSidecarEndpoint(userId, endpoint) {
...
function collectConfiguredSidecarEndpoints(cortexConfig) {
    ...
    ].filter((endpoint) => !!endpoint?.connectionProfileId);
}
export function resolveCortexSidecarAdapter(userId, cortexConfig) {
    if (!memoryCortex.shouldUseCortexSidecar(cortexConfig))
        return {};
    const candidates = collectConfiguredSidecarEndpoints(cortexConfig);
    if (candidates.length === 0)
        return { unavailableReason: "sidecar_not_configured" };
    let lastUnavailable = "sidecar_not_configured";
    let firstReady = null;
    for (const endpoint of candidates) {
        ...
        if (inspected.unavailableReason)
            lastUnavailable = inspected.unavailableReason;
    }
    if (!firstReady?.connectionProfileId || !firstReady.provider) {
        return { unavailableReason: lastUnavailable };
    }
    const generateRawFn = memoryCortex.createCortexSidecarGenerateRawAdapter({
        userId,
...
export function describeCortexSidecarHealth(userId, cortexConfig) {
    const listed = memoryCortex.listCortexSidecarEndpoints(cortexConfig);
    const queryPrimary = inspectCortexSidecarEndpoint(userId, listed.queryGeneration.primary);
    const queryFallbacks = (listed.queryGeneration.fallbacks ?? []).map((endpoint) => inspectCortexSidecarEndpoint(userId, endpoint));
    ...
    const summaryFallbacks = (listed.memorySummarization.fallbacks ?? []).map((endpoint) => inspectCortexSidecarEndpoint(userId, endpoint));
...
app.get("/health", async (c) => {
    ...
    const probeConnectivity = ["1", "true", "yes", "on"].includes((c.req.query("probeConnectivity") || "").toLowerCase());
    const config = memoryCortex.getCortexConfig(userId);
    const embeddings = await embeddingsSvc.getEmbeddingConfig(userId);
    const checks = [];
    const pushCheck = (key, label, status, message) => {
        checks.push({ key, label, status, message });
    };
    const getProbeErrorMessage = (err, fallback) => {
    ...
    pushCheck("cortex_enabled", "Memory Cortex enabled", config.enabled ? "pass" : "warn", config.enabled
        ? "Memory Cortex is enabled."
        : "Memory Cortex is turned off. Turn it on before expecting entity tracking or cortex retrieval.");
    pushCheck("embeddings_enabled", "Embeddings enabled", embeddings.enabled ? "pass" : "fail", embeddings.enabled
    ...
    pushCheck("chat_vectorization_enabled", "Chat vectorization enabled", embeddings.vectorize_chat_messages ? "pass" : "fail", embeddings.vectorize_chat_messages
    ...
    pushCheck("embedding_api_key", "Embedding API key", embeddings.has_api_key ? "pass" : "fail", embeddings.has_api_key
        ? "An embedding API key is configured."
        : "No embedding API key is configured. Embeddings cannot be generated until a key is saved.");
    pushCheck("embedding_dimensions", "Embedding dimensions", embeddings.dimensions ? "pass" : "warn", embeddings.dimensions
    ...
    const sidecarConnectionId = sidecarEndpoints.queryGeneration.primary.connectionProfileId
        ...
        || null;
    const sidecarRequired = config.entityExtractionMode === "sidecar" ||
    ...
    if (sidecarRequired && !sidecarConnectionId) {
        pushCheck("sidecar_required", "Sidecar connection", "fail", "Sidecar-assisted cortex features are enabled, but no sidecar connection is selected.");
    }
    else if (sidecarConnectionId && !sidecarProfile) {
        pushCheck("sidecar_exists", "Sidecar connection", sidecarRequired ? "fail" : "warn", "The selected sidecar connection profile no longer exists.");
    }
    else if (sidecarConnectionId && !sidecarProvider) {
        pushCheck("sidecar_provider", "Sidecar provider", sidecarRequired ? "fail" : "warn", `The selected provider "${sidecarProfile?.provider}" is not available.`);
    }
    else if (sidecarConnectionId && !sidecarHasApiKey) {
        pushCheck("sidecar_api_key", "Sidecar API key", sidecarRequired ? "fail" : "warn", "The selected sidecar connection is missing its API key.");
    }
    else if (sidecarRequired) {
        pushCheck("sidecar_ready", "Sidecar readiness", "pass", "A valid sidecar connection is configured for cortex features that require it.");
    }
    else if (sidecarConnectionId) {
        pushCheck("sidecar_optional", "Sidecar connection", "info", "A sidecar connection is configured, but current cortex modes can still run in heuristic mode.");
    }
    else {
        pushCheck("sidecar_optional", "Sidecar connection", "info", "No sidecar connection is configured. Heuristic cortex mode can still run without it.");
    }
    ...
    if (embeddingConnectivity.attempted) {
        pushCheck("embedding_probe", "Embedding connectivity", embeddingConnectivity.success ? "pass" : "fail", embeddingConnectivity.message);
    }
    if (sidecarConnectivity.attempted) {
        pushCheck("sidecar_probe", "Sidecar connectivity", sidecarConnectivity.success ? "pass" : "fail", sidecarConnectivity.message);
    }
    let chatReport = null;
    if (chatId) {
        const chat = getChat(userId, chatId);
        if (!chat) {
            ...
            pushCheck("chat_exists", "Selected chat", "fail", "The requested chat could not be found.");
        }
        else {
            const db = getDb();
            const messageCount = db.query("SELECT COUNT(*) as c FROM messages WHERE chat_id = ?").get(chatId)?.c ?? 0;
            ...
            pushCheck("chat_loaded", "Selected chat", "pass", `Chat "${chat.name || chatId}" is available for diagnostics.`);
            if (messageCount === 0) {
                pushCheck("chat_messages", "Chat messages", "warn", "This chat has no messages yet, so there is nothing for Memory Cortex to process.");
            }
            else if (vectorStatus.totalChunks === 0) {
                pushCheck("chat_chunks", "Chat chunks", "fail", "No chat_chunks exist for this chat. Recompile chat memory after enabling chat vectorization.");
            }
            else if (vectorStatus.vectorizedChunks === 0) {
                pushCheck("chat_chunks_vectorized", "Chunk vectorization", "fail", "Chat chunks exist, but none are vectorized yet. Check your embedding setup, then recompile chat memory.");
            }
            else if (vectorStatus.pendingChunks > 0) {
                pushCheck("chat_chunks_vectorized", "Chunk vectorization", "warn", `${vectorStatus.pendingChunks} chat chunk(s) are still waiting on vectorization.`);
            }
            else {
                pushCheck("chat_chunks_vectorized", "Chunk vectorization", "pass", "Chat chunks exist and are vectorized.");
            }
            if (rebuildStatus.status === "processing") {
                pushCheck("cortex_rebuild", "Cortex rebuild", "info", `A rebuild is currently running (${rebuildStatus.percent ?? 0}% complete).`);
            }
            else if (stats.entityCount > 0 || stats.relationCount > 0 || stats.consolidationCount > 0) {
                pushCheck("cortex_output", "Cortex output", "pass", `Cortex data exists for this chat (${stats.entityCount} entities, ${stats.relationCount} relations).`);
            }
            else if (messageCount >= 4 && config.enabled) {
                pushCheck("cortex_output", "Cortex output", "warn", "No cortex entities or relations were found for this chat yet. Run a rebuild after chat vectorization is healthy.");
            }
            else {
                pushCheck("cortex_output", "Cortex output", "info", "This chat may still be too small for meaningful cortex extraction.");
            }
        }
    }
    else {
        pushCheck("chat_context", "Selected chat", "info", "No chat was selected. Open diagnostics from an active chat to see chunk and cortex stats for that chat.");
    }
    const summary = {
...
app.get("/chats/:chatId/entities", (c) => {
    ...
    const db = getDb();
    const mentions = db.query(`SELECT entity_id, excerpt, message_ids FROM (
...
app.post("/chats/:chatId/entities/bulk-delete", async (c) => {
    ...
    db.transaction(() => {
        for (const batch of batches) {
         ...
         WHERE chat_id = ?
           AND (source_entity_id IN (${placeholders}) OR target_entity_id IN (${placeholders}))`).run(chatId, ...batch, ...batch);
            db.query(`DELETE FROM memory_mentions WHERE chat_id = ? AND entity_id IN (${placeholders})`).run(chatId, ...batch);
            const result = db.query(`DELETE FROM memory_entities WHERE chat_id = ? AND id IN (${placeholders})`).run(chatId, ...batch);
...
app.put("/chats/:chatId/entities/:entityId", async (c) => {
    ...
    if (body.facts !== undefined) {
        updates.push("facts = ?");
        params.push(JSON.stringify(body.facts));
    }
...
app.delete("/chats/:chatId/entities/:entityId", (c) => {
    ...
    db.query(`DELETE FROM memory_relations WHERE chat_id = ? AND (source_entity_id = ? OR target_entity_id = ?)`).run(chatId, entityId, entityId);
...
app.get("/chats/:chatId/colors", (c) => {
    ...
    const { getDb } = require("../db/connection");
    const db = getDb();
    const enriched = colorMap.map((m) => {
        let entityName = null;
        if (m.entityId) {
            const row = db.query("SELECT name FROM memory_entities WHERE id = ?").get(m.entityId);
...
app.put("/chats/:chatId/colors/:id", async (c) => {
    ...
    if (body.entityId !== undefined) {
        ...
        if (entityId !== null) {
            ...
            const owns = getDb().query("SELECT 1 FROM memory_entities WHERE id = ? AND chat_id = ?").get(entityId, chatId);
            if (!owns)
                return c.json({ error: "Target entity not found in this chat" }, 404);
        }
    ...
    const result = getDb().query(`UPDATE memory_font_colors SET ${updates.join(", ")} WHERE id = ? AND chat_id = ?`).run(...params);
    if ((result.changes ?? 0) === 0)
        return c.json({ error: "Color attribution not found" }, 404);
    return c.json({ success: true });
});
// ─── Relations ─────────────────────────────────────────────────
...
app.post("/chats/:chatId/relations", async (c) => {
    ...
    const endpoints = db.query("SELECT id FROM memory_entities WHERE chat_id = ? AND id IN (?, ?)").all(chatId, sourceEntityId, targetEntityId);
    ...
    const existing = db.query(`SELECT id FROM memory_relations
     WHERE source_entity_id = ? AND target_entity_id = ? AND relation_type = ?
       AND merged_into IS NULL`).get(sourceEntityId, targetEntityId, relationType);
    if (existing) {
    ...
    db.query(`INSERT INTO memory_relations
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, '[]', ?, ?, ?, '{}', ?, ?, ?)`).run(id, chatId, sourceEntityId, targetEntityId, relationType, relationLabel, strength, sentiment, now, now, status, now, now, now);
...
app.put("/chats/:chatId/relations/:relationId", async (c) => {
    ...
    const body = await c.req.json();
    const db = getDb();
    const existing = db.query("SELECT * FROM memory_relations WHERE id = ? AND chat_id = ?").get(relationId, chatId);
    if (!existing)
    ...
    if (body.strength !== undefined) {
        ...
        if (!Number.isFinite(s))
            return c.json({ error: "strength must be a number in [0,1]" }, 400);
    ...
    if (body.sentiment !== undefined) {
        ...
        if (!Number.isFinite(s))
            return c.json({ error: "sentiment must be a number in [-1,1]" }, 400);
    ...
    db.query(`UPDATE memory_relations SET ${updates.join(", ")} WHERE id = ? AND chat_id = ?`).run(...params);
...
app.delete("/chats/:chatId/relations/:relationId", (c) => {
    ...
    const relationId = c.req.param("relationId");
    const db = getDb();
    const result = db.query("DELETE FROM memory_relations WHERE id = ? AND chat_id = ?").run(relationId, chatId);
    if (result.changes === 0)
        return c.json({ error: "Relation not found" }, 404);
    return c.json({ success: true });
});
...
function parseChunkMessageIds(raw) {
 ...
 */
function resolveChunkMessageReferences(db, chatId, rows) {
    ...
    for (const ids of chunksOf(uniqueIds, 500)) {
        ...
        const messages = db.query(`SELECT id, index_in_chat FROM messages WHERE chat_id = ? AND id IN (${placeholders})`).all(chatId, ...ids);
        for (const message of messages)
            messageNumbers.set(message.id, message.index_in_chat);
    }
...
function enrichConsolidationsWithMessageReferences(db, chatId, consolidations) {
    const chunkMessageIds = new Map();
    ...
    for (const ids of chunksOf(consolidations.map((consolidation) => consolidation.id), 500)) {
        if (ids.length === 0)
            continue;
        const placeholders = ids.map(() => "?").join(", ");
        const rows = db.query(`WITH RECURSIVE source_tree(root_id, consolidation_id) AS (
         SELECT id, id
       ...
       WHERE leaf.chat_id = ?
       ORDER BY source_tree.root_id, cc.created_at ASC`).all(chatId, ...ids, chatId, chatId, chatId);
    ...
    const references = resolveChunkMessageReferences(db, chatId, consolidations.map((consolidation) => ({
...
app.get("/chats/:chatId/chunks", (c) => {
    ...
    const { getDb } = require("../db/connection");
    const db = getDb();
    const rows = db.query(`SELECT cc.id, cc.chat_id, cc.content, cc.token_count, cc.message_count,
     ...
     LIMIT ? OFFSET ?`).all(chatId, limit, offset);
    const countRow = db.query("SELECT COUNT(*) as c FROM chat_chunks WHERE chat_id = ?").get(chatId);
...
app.get("/chats/:chatId/salience", (c) => {
    ...
    const { getDb } = require("../db/connection");
    const db = getDb();
    const rows = db.query(`SELECT ms.*, cc.content as chunk_content, cc.message_count
     FROM memory_salience ms
     LEFT JOIN chat_chunks cc ON cc.id = ms.chunk_id
     WHERE ms.chat_id = ?
     ORDER BY ms.score DESC
     LIMIT ? OFFSET ?`).all(chatId, limit, offset);
    const countRow = db.query("SELECT COUNT(*) as c FROM memory_salience WHERE chat_id = ?").get(chatId);
...
app.put("/chats/:chatId/settings", async (c) => {
    ...
    const existingSettings = isRecord(chat.metadata?.cortex_settings)
        ...
        : {};
    if (body.enabled)
        delete existingSettings.enabled;
    else
        existingSettings.enabled = false;
    const updated = chatsSvc.mergeChatMetadata(owned.userId, chatId, { cortex_settings: Object.keys(existingSettings).length ? existingSettings : undefined });
    if (!updated)
...
app.get("/chats/:chatId/rebuild-status", (c) => {
    ...
    const scheduler = getChatPipelineStatus(chatId);
    if (!status)
        return c.json({ status: "idle", scheduler });
    return c.json({ ...status, scheduler });
});
...
app.put("/vaults/:id", async (c) => {
    ...
    if (!ok)
        return c.json({ error: "Vault not found or not owned" }, 404);
    return c.json({ success: true });
});
...
app.post("/chats/:chatId/links", async (c) => {
    ...
    if (bidirectionalRaw !== undefined && optionalBoolean(bidirectionalRaw) === undefined) {
...
app.patch("/chats/:chatId/links/:linkId", async (c) => {
    ...
    if (!ok)
        return c.json({ error: "Link not found or not owned" }, 404);
    memoryCortex.invalidateLinkedCortexCache(chatId);
    eventBus.emit(EventType.CORTEX_LINK_CHANGED, {
