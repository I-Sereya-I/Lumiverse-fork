...
function getGalleryFiles(formData) {
    const parts = [...formData.getAll("images"), ...formData.getAll("image")];
    return parts.filter((part) => part instanceof File && part.size > 0);
}
...
app.post("/bulk", async (c) => {
    ...
    if (files.length > 100)
        return c.json({ error: "Maximum 100 images per request" }, 400);
    const { valid, oversized } = partitionBySize(files);
    if (valid.length === 0) {
        return c.json({ error: "All files exceed the per-image size cap", maxBytes: MAX_IMAGE_UPLOAD_BYTES, skipped: oversized }, 413);
    }
