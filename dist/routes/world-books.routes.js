...
import { collectVectorActivatedWorldInfoDetailed, getWorldInfoVectorQueryDetails, mergeActivatedWorldInfoEntries, } from "../services/prompt-assembly.service";
import { collectWorldInfoSources, resolveWorldInfoCharacters, } from "../services/world-info-sources.service";
import { activateWorldInfo } from "../services/world-info-activation.service";
...
const WORLD_BOOK_EXPORT_FORMATS = ["lumiverse", "character_book", "sillytavern"];
function parseBulkWorldBookIds(value) {
    if (!Array.isArray(value) || value.length === 0)
        return null;
    return value.every((id) => typeof id === "string") ? value : null;
}
function isWorldBookExportFormat(value) {
    return typeof value === "string" && WORLD_BOOK_EXPORT_FORMATS.includes(value);
}
function parseBulkWorldBookExportFormat(value) {
    if (value === undefined)
        return "lumiverse";
...
function buildRerankLead(options) {
    ...
    const distance = typeof options.distance === "number" && Number.isFinite(options.distance)
        ...
        : null;
    const metrics = [score, distance].filter((value) => Boolean(value));
...
function buildDiagnosticVectorOutcome(code, options = {}) {
...
function traceDiagnosticVectorHitOutcomes(vectorEntries, dispositions, settingsInput) {
...
function resolveDiagnosticVectorTraceOutcome(item, shortlistedOutcomes, options) {
...
app.post("/bulk-move-folder", async (c) => {
    ...
    if (typeof body?.folder !== "string")
        return c.json({ error: "folder must be a string" }, 400);
...
app.post("/folders/rename", async (c) => {
    ...
    if (!oldName)
        return c.json({ error: "old_name is required" }, 400);
    if (!newName)
        return c.json({ error: "new_name is required" }, 400);
...
app.delete("/:id", async (c) => {
    const userId = c.get("userId");
    if (!(await svc.deleteWorldBook(userId, c.req.param("id"))))
        return c.json({ error: "Not found" }, 404);
...
app.post("/:id/diagnostics", async (c) => {
    ...
    const attachmentSources = {
        character: worldInfoCharacters.some((sourceCharacter) => getCharacterWorldBookIds(sourceCharacter.extensions).includes(bookId)),
    ...
    const queryDetails = await getWorldInfoVectorQueryDetails(userId, messages, chat.id, worldInfoSettings);
    ...
    const wiSources = collectWorldInfoSources(userId, character, persona, globalWorldBooks, chatWorldBookIds, { chat });
    const bookEntries = wiSources.entries.filter((entry) => entry.world_book_id === bookId);
    const selectedEligibleEntries = bookEntries.filter((entry) => entry.vectorized && !entry.disabled && (entry.content || "").trim().length > 0);
    ...
    const vectorDetail = isAttached
        ? await collectVectorActivatedWorldInfoDetailed(userId, chat.id, wiSources.worldBookIds, wiSources.entries, messages, undefined, worldInfoSettings)
        : {
            entries: [],
            candidateTrace: [],
            queryPreview,
    ...
    const mergedWorldInfo = mergeActivatedWorldInfoEntries(wiResult.activatedEntries, vectorDetail.entries, worldInfoSettings, wiSources.bookSourceMap, wiSources.bookNameMap);
    const vectorHitOutcomes = traceDiagnosticVectorHitOutcomes(vectorDetail.candidateTrace.filter((item) => item.retrievalStage === "shortlisted"), mergedWorldInfo.vectorDispositions, worldInfoSettings);
    ...
    const vectorKeywordOverlapCount = selectedVectorEntries.reduce((count, item) => count + (keywordHitIds.has(item.entry.id) ? 1 : 0), 0);
    const displacedFreshVectorHits = selectedVectorEntries.filter((item) => {
        if (keywordHitIds.has(item.entry.id))
            return false;
    ...
    if (mergedWorldInfo.evictedByBudget > 0 &&
    ...
    if (selectedVectorEntries.length > 0 &&
    ...
    if (displacedFreshVectorHits.length > 0) {
        ...
        for (const item of displacedFreshVectorHits) {
            ...
            if (existing)
                ...
                grouped.set(outcome.code, { outcome, count: 1 });
        }
    ...
    return c.json({
        ...
        vector_hits: vectorTrace.filter((item) => item.final_outcome_code !== "rejected_by_similarity_threshold" &&
 ...
 */
function coerceLorebookPayload(payload) {
    if (!payload || typeof payload !== "object")
        return payload;
    if (payload.entries)
        return payload; // already lorebook-shaped
...
app.post("/import-character-book", async (c) => {
    ...
    if (existing) {
        if (!currentIds.includes(existing.id)) {
            const nextExtensions = setCharacterWorldBookIds({ ...(character.extensions || {}) }, [...currentIds, existing.id]);
    ...
    const nextExtensions = setCharacterWorldBookIds({ ...(character.extensions || {}) }, [...currentIds, result.worldBook.id]);
    await charSvc.updateCharacter(userId, characterId, { extensions: nextExtensions });
    return c.json({ world_book: result.worldBook, entry_count: result.entryCount }, 201);
});
// --- Entry endpoints ---
...
app.get("/:id/entries", (c) => {
    ...
    const sortBy = rawSortBy && VALID_ENTRY_SORT_KEYS.has(rawSortBy) ? rawSortBy : undefined;
...
app.post("/:id/entries/reorder", async (c) => {
    ...
    try {
        ...
        if (!success)
            return c.json({ error: "Unable to reorder entries" }, 400);
...
app.post("/:id/entries/:eid/duplicate", async (c) => {
    ...
    try {
        ...
        if (!duplicated)
            return c.json({ error: "Target world book not found" }, 404);
...
app.patch("/:id/entries/:eid/extensions/:namespace", async (c) => {
    ...
    const body = await c.req.json();
    try {
        const result = svc.setEntityExtensionNamespace(userId, "world_book_entry", entry.id, c.req.param("namespace"), body?.value);
        if (!result)
            return c.json({ error: "Not found" }, 404);
        return c.json(result);
    }
...
app.delete("/:id/entries/:eid", async (c) => {
    const userId = c.get("userId");
    if (!(await svc.deleteEntry(userId, c.req.param("eid"))))
        return c.json({ error: "Not found" }, 404);
    return c.json({ success: true });
});
