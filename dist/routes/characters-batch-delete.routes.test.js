...
beforeEach(async () => {
    ...
    getDb().query("INSERT INTO images (id, user_id, filename, original_filename) VALUES (?, ?, ?, ?)").run(TARGET_IMAGE_ID, USER_ID, "target.webp", "target.webp");
    getDb().query("INSERT INTO images (id, user_id, filename, original_filename) VALUES (?, ?, ?, ?)").run(SHARED_IMAGE_ID, USER_ID, "shared.webp", "shared.webp");
    getDb().query("UPDATE characters SET image_id = ? WHERE id = ?").run(TARGET_IMAGE_ID, TARGET_ID);
    getDb().query("INSERT INTO character_gallery (id, user_id, character_id, image_id, created_at) VALUES (?, ?, ?, ?, 1)").run("target-gallery", USER_ID, TARGET_ID, SHARED_IMAGE_ID);
    getDb().query("INSERT INTO character_gallery (id, user_id, character_id, image_id, created_at) VALUES (?, ?, ?, ?, 1)").run("member-gallery", USER_ID, MEMBER_ID, SHARED_IMAGE_ID);
    getDb().query("INSERT INTO chats (id, user_id, character_id, name) VALUES ('solo-chat', ?, ?, 'Solo')").run(USER_ID, TARGET_ID);
    getDb().query(`INSERT INTO chats (id, user_id, character_id, name, metadata)
     VALUES ('group-chat', ?, ?, 'Group', ?)`).run(USER_ID, MEMBER_ID, JSON.stringify({ group: true, character_ids: [TARGET_ID, MEMBER_ID, OTHER_ID] }));
});
...
describe("POST /batch-delete", () => {
    test("deletes owned characters and cleans dependent records without removing shared images", async () => {
        ...
        const group = getDb().query("SELECT metadata FROM chats WHERE id = 'group-chat'").get();
