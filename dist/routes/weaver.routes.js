...
const app = new Hono();
function publicWeaverError(err) {
    if (err instanceof Error) {
        if (err.message.includes("no connection"))
            return "Choose a text connection before running the Weaver.";
        if (err.message.includes("no model"))
            return "Choose a model before running the Weaver.";
        if (err.message === "Seed is empty — nothing to read back")
            return err.message;
        if (err.message === "Read the dream first — there is nothing to synthesize")
            return err.message;
        if (err.message.includes("invalid JSON"))
            return "The model's response could not be parsed. Try again or switch models.";
        if (err.message.includes("no usable options"))
            return "The model returned no usable options. Try again or switch models.";
        if (err.message.includes("unusable Bible"))
            return "The model returned an unusable Bible. Try again or switch models.";
        if (err.message.startsWith("Synthesize a Bible first"))
            return err.message;
        if (err.message.startsWith("The Bible has no spine yet"))
            return err.message;
        if (err.message.startsWith("Unknown field"))
            return err.message;
        if (err.message.startsWith("This field is hand-edited"))
            return err.message;
        if (err.message.startsWith("Render the field first"))
            return err.message;
        if (err.message === "A field cannot be empty")
            return err.message;
        if (err.message === "Add a nudge to steer the re-render")
            return err.message;
...
function weaverTextTimeoutMs(userId) {
    const seconds = tuningSvc.getWeaverTuning(userId).text_timeout_seconds ?? WEAVER_TEXT_TIMEOUT_DEFAULT_SECONDS;
    return seconds * 1000;
}
function weaverTextCall(c, userId) {
    const timeoutMs = weaverTextTimeoutMs(userId);
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(new DOMException(`Weaver generation timed out after ${timeoutMs / 1000}s`, "TimeoutError")), timeoutMs);
...
app.get("/narration-modes", (c) => c.json(NARRATION_MODES.map((m) => ({ id: m.id, label: m.label }))));
app.get("/persona-registers", (c) => c.json(PERSONA_REGISTERS.map((r) => ({ id: r.id, label: r.label }))));
...
app.post("/import/enrich/:bookId/entries/:entryId", async (c) => {
    ...
    const body = (await c.req.json().catch(() => ({})));
    try {
        const result = await importSvc.enrichEntry(userId, c.req.param("bookId"), c.req.param("entryId"), { connection_id: body.connection_id ?? null, model: body.model ?? null });
        return c.json(result);
    }
    catch (err) {
        const message = err instanceof Error ? err.message : "Could not enrich this entry";
        if (message === "No such entry in this book")
            return c.json({ error: message }, 404);
        return c.json({ error: message }, 400);
    }
});
function requestedBuildType(c) {
...
app.get("/slots", (c) => {
    ...
    if (!buildType)
        return c.json({ error: "Unknown build type" }, 400);
...
app.patch("/sessions/:id/extraction", async (c) => {
    ...
    try {
        ...
        const message = err instanceof Error ? err.message : "Update failed";
        if (message === "Extraction not found")
            return c.json({ error: message }, 404);
        return c.json({ error: message }, 400);
    }
});
app.get("/sessions/:id/interview", (c) => {
    const userId = c.get("userId");
    if (!svc.getSession(userId, c.req.param("id")))
        return c.json({ error: "Session not found" }, 404);
...
app.post("/sessions/:id/interview/enhance", async (c) => {
    ...
    try {
        ...
        if (message === "Session not found")
            return c.json({ error: message }, 404);
        if (message.startsWith("Write a draft answer first"))
            return c.json({ error: message }, 400);
        return c.json({ error: publicWeaverError(err) }, 400);
    }
    finally {
...
app.post("/sessions/:id/bible/entries/:slot/resynthesize", async (c) => {
    ...
    const body = (await c.req.json().catch(() => ({})));
    try {
        const bible = await bibleSvc.resynthesizeEntry(userId, c.req.param("id"), c.req.param("slot"), body.nudge);
        return c.json(bible);
    }
    catch (err) {
...
app.patch("/sessions/:id/bible", async (c) => {
    ...
    try {
        ...
        const message = err instanceof Error ? err.message : "Update failed";
        if (message === "No Bible to edit yet")
            return c.json({ error: message }, 404);
        return c.json({ error: message }, 400);
    }
});
...
app.post("/sessions/:id/finalize", async (c) => {
    ...
    try {
        ...
        if (body?.books && typeof body.books === "object" && !Array.isArray(body.books)) {
            for (const [roleId, enabled] of Object.entries(body.books)) {
                if (typeof enabled === "boolean")
                    books[roleId] = enabled;
            }
        }
...
function growableSession(c, userId) {
    const session = svc.getSession(userId, c.req.param("id"));
    if (!session)
        return { error: c.json({ error: "Session not found" }, 404) };
...
app.get("/sessions/:id/hub", (c) => {
    ...
    if (!character)
        return c.json({ error: "The finalized card no longer exists" }, 404);
    ...
    const books = getCharacterWorldBookIds(character.extensions)
        ...
        id: b.id,
        name: b.name,
        role: (b.metadata ?? {}).weaver_role ?? null,
    ...
    return c.json({
        ...
        people: reg.people
            ? {
                question_target: tuningSvc.getWeaverTuning(userId).named_question_target ?? reg.people.namedQuestionTarget,
            }
            : null,
...
app.post("/sessions/:id/persona/greeting", async (c) => {
    ...
    try {
        ...
        if (!body?.draft)
            return c.json({ error: "Missing persona draft" }, 400);
        const greeting = await personaBuildSvc.generatePairedGreeting(userId, c.req.param("id"), body.draft, typeof body.register === "string" ? body.register : "neutral");
...
function visualMacroValues(character) {
    if (!character)
        return {};
    return {
...
async function loadSourceImage(userId, imageId) {
...
app.post("/sessions/:id/visual/generate", async (c) => {
    ...
    if (!connection)
        return c.json({ error: "Image connection not found" }, 404);
    let sourceImage;
    if (body.source_image_id) {
        const adapter = getVisualProviderAdapter(connection.provider);
        if (!adapter)
            return c.json({ error: "That image provider is not supported for Visual Studio." }, 400);
        ...
        if (!loaded)
            return c.json({ error: "The source image no longer exists." }, 404);
        sourceImage = loaded;
        if (kindMeta.variants && body.variant?.trim()) {
            const variant = resolveExpressionVariant(body.variant);
            const composed = composeExpressionPrompts(variant, support.mechanism, body.prompt?.trim() ?? "", body.negative_prompt?.trim() ?? "");
    ...
    if (kindMeta.variants) {
        if (!body.variant?.trim())
            return c.json({ error: "Choose an expression to generate." }, 400);
        if (!sourceImage)
            return c.json({ error: "Commit a portrait first — expressions are derived from it." }, 400);
    }
    if (!body.prompt?.trim()) {
    ...
    const job = startWeaverVisualJob({
        ...
        persistResult: async ({ job, result }) => {
            ...
            const image = await imagesSvc.saveImageFromDataUrl(userId, result.image_url, `${imagesSvc.IMAGE_GEN_FILENAME_PREFIX}weaver-${characterId}-${job.kind}-${job.id}.png`, {
...
app.post("/sessions/:id/visual/commit/avatar", async (c) => {
    ...
    if (!character)
        return c.json({ error: "Image or character not found" }, 404);
...
app.post("/sessions/:id/visual/commit/expressions", async (c) => {
    ...
    for (const [label, imageId] of Object.entries(body.mappings ?? {})) {
        ...
        if (!cleanLabel || typeof imageId !== "string" || !imageId.trim())
            continue;
