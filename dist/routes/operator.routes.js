...
import { detectHostnameSuggestions, getSnapshot as getTrustedHostsSnapshot, InvalidTrustedHostError, setTrustedHosts, } from "../services/trusted-hosts.service";
import { getApprovedBrokerOrigins, InvalidBrokerOriginError, setApprovedBrokerOrigins, } from "../services/broker-origins.service";
import { getSharpSettingsStatus, putSharpSettings, } from "../services/sharp-settings.service";
import { discardImageProcessingQueue, getImageProcessingQueueSnapshot, recoverImageProcessingQueue, } from "../services/images.service";
import { getDnsSettingsStatus, putDnsSettings, } from "../services/dns-settings.service";
import { getDiskWarningSettingsStatus, putDiskWarningSettings, } from "../services/disk-warning-settings.service";
import { getSmartctlSnapshot, getSmartctlStatus, installSmartctl, } from "../services/smartctl.service";
import { InvalidSettingError } from "../services/settings.service";
import { parseExtensionSecretKey, providerRegistry, } from "../spindle/provider-registry";
import { putSecret, deleteSecret, listSecretKeys, SYSTEM_SECRET_PRINCIPAL, } from "../services/secrets.service";
const app = new Hono();
...
app.put("/spindle-secrets/:key", async (c) => {
    ...
    if (!parseExtensionSecretKey(key)) {
        return c.json({ error: "key must be namespaced as extension:<installationId>:<name>" }, 400);
    }
    const body = await c.req.json().catch(() => null);
...
function requireIPC(c) {
    if (!operatorService.ipcAvailable) {
        return c.json({ error: "Runner IPC not available. Start with ./start.sh or bun run runner." }, 503);
    }
    return null;
}
