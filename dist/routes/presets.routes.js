...
function parseBulkPresetIds(value) {
    if (!Array.isArray(value) || value.length === 0 || value.length > MAX_BULK_PRESET_IDS)
        return null;
...
app.post("/bulk-delete", async (c) => {
    ...
    if (!ids)
        return c.json({ error: "ids must be a non-empty array of at most 200 strings" }, 400);
...
app.post("/bulk-export/prepare", async (c) => {
    ...
    if (!prepared)
        return c.json({ error: "No exportable presets found" }, 404);
...
app.get("/bulk-export/:downloadId", (c) => {
    ...
    if (!prepared)
        return c.json({ error: "Export session not found. Prepare the export again." }, 404);
    const stream = presetExportSvc.buildPresetBulkExportStream(prepared.userId, prepared.presetIds, c.req.raw.signal);
...
app.post("/stash", async (c) => {
    ...
    try {
        ...
        const sourcePreset = typeof body.sourcePresetId === "string"
            ...
            : null;
        return c.json(stashSvc.addPromptBlockToStash(userId, body.block, sourcePreset ? { id: sourcePreset.id, name: sourcePreset.name } : undefined), 201);
    }
    catch (err) {
...
app.get("/:id", (c) => {
    ...
    if (!preset)
        return c.json({ error: "Not found" }, 404); // deleted between lookups
    c.header("ETag", etag);
    c.header("Cache-Control", REVALIDATE_PRIVATE);
    c.header("Vary", "Cookie, Accept-Encoding");
...
app.put("/:id", async (c) => {
    const userId = c.get("userId");
    const body = await c.req.json();
    if (typeof body.expected_cache_revision !== "number"
...
app.delete("/:id", (c) => {
    const userId = c.get("userId");
    if (!svc.deletePreset(userId, c.req.param("id")))
        return c.json({ error: "Not found" }, 404);
    return c.json({ success: true });
});
