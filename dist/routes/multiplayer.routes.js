...
app.post("/rooms", roomMutationLimiter, async (c) => {
    ...
    if ("error" in result) {
        ...
        if (result.error === "fork_failed")
            return c.json({ error: "Could not fork the chat" }, 500);
...
app.post("/rooms/:roomId/remote/enable", roomMutationLimiter, async (c) => {
    const userId = c.get("userId");
    const roomId = c.req.param("roomId");
    if (!svc.getRoomStateForHost(userId, roomId))
        return c.json({ error: "Not found" }, 404);
    ...
    if (!registered)
        return c.json({ error: "Identity Server unreachable" }, 502);
...
app.post("/rooms/:roomId/remote/invite", roomMutationLimiter, async (c) => {
    ...
    if (!invite)
        return c.json({ error: "Could not create remote invite" }, 502);
...
app.post("/rooms/:roomId/participants/:participantId/ban", async (c) => {
    const userId = c.get("userId");
    const body = await c.req.json().catch(() => ({}));
    const ok = svc.hostBan(userId, c.req.param("roomId"), c.req.param("participantId"), typeof body.reason === "string" ? body.reason : undefined);
    return c.json({ ok }, ok ? 200 : 400);
});
...
app.post("/rooms/:roomId/freeform/start", (c) => {
    ...
    if (!room)
        return c.json({ error: "Not found or not a freeform room" }, 400);
