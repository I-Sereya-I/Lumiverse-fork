...
import { getActiveVectorStore } from "../services/vector-store";
const app = new Hono();
...
app.get("/health", requireOwnerStrict, async (c) => {
    try {
        const store = await getActiveVectorStore();
        const collections = ["embeddings", "embeddings_world_books"];
        const entries = await Promise.all(collections.map(async (name) => [name, await store.health(name)]));
...
function combineVectorHealth(tables) {
...
app.post("/chats/:chatId/recompile", async (c) => {
    ...
    if (chunks.length > 0) {
        await embeddingsSvc.reindexChatMessages(userId, chatId, chunks.map(ch => ({
            chunkId: ch.id,
            content: ch.content,
