...
import { startStMigration, isMigrationRunning, getActiveMigration, getLastMigration, } from "../migration/st-migration.service";
...
async function ensureMigrationTargetAccess(c, targetUserId) {
...
function parseConnectionConfig(body) {
    if (!body.connection || body.connection.type === "local") {
        return { type: "local" };
    }
    return body.connection;
}
async function parseObjectBody(c) {
    let body;
    try {
...
app.post("/execute", async (c) => {
    ...
    void startStMigration(migrationId, callerUserId, targetUserId, effectiveDataDir, migrationScope, config, config.type === "local" ? localFs : undefined);
