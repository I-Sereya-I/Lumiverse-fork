...
import "../tts/index";
const app = new Hono();
function qwenVoiceMutationStatus(message, fallback = 502) {
    const normalized = message.toLowerCase();
    if (normalized.includes("not found"))
        return 404;
    if (normalized.includes("does not use qwen"))
        return 400;
    ...
    if (normalized.includes("already exists"))
        return 409;
    if (normalized.includes("exceeds the 15 mb limit"))
        return 413;
    if (normalized.includes("does not currently have the base voice-cloning model loaded"))
        return 409;
    return fallback;
}
...
app.get("/", async (c) => {
    ...
    const result = await withReadableApiKeyStatuses(userId, svc.listConnections(userId, pagination), svc.ttsConnectionSecretKey);
...
app.post("/:id/qwen/custom-voices", async (c) => {
    ...
    try {
        const result = await qwenSvc.createQwenCustomVoice(userId, c.req.param("id"), {
            ...
            sourceFilename: typeof audio?.name === "string" ? audio.name : undefined,
            audioData: new Uint8Array(await audio.arrayBuffer()),
            xVectorOnlyMode: typeof xVectorOnlyMode === "string"
...
app.delete("/:id/qwen/custom-voices/:voiceId", async (c) => {
    const userId = c.get("userId");
    try {
        const result = await qwenSvc.deleteQwenCustomVoice(userId, c.req.param("id"), decodeURIComponent(c.req.param("voiceId")));
        if (!result.success) {
