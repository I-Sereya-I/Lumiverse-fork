...
const APPLY_VALID_PLACEMENTS = new Set(["user_input", "ai_output", "world_info", "reasoning"]);
const APPLY_VALID_FLAGS = new Set(["d", "g", "i", "m", "s", "u", "v", "y"]);
const APPLY_VALID_MACRO_MODES = new Set(["none", "find", "raw", "escaped", "after"]);
function isStringRecord(value) {
    return typeof value === "object" && value !== null && !Array.isArray(value);
}
function normalizeResolvedMap(value) {
...
function normalizeDisplayScripts(value, userId) {
    if (!Array.isArray(value))
        return "scripts must be an array";
    if (value.length > APPLY_MAX_SCRIPT_COUNT)
        return `scripts exceeds maximum length (${APPLY_MAX_SCRIPT_COUNT})`;
    const scripts = [];
    for (const raw of value) {
        if (!isStringRecord(raw))
            return "scripts contains an invalid entry";
        ...
        if (findRegex === undefined)
            return "script find_regex is required";
        if (findRegex.length > APPLY_MAX_PATTERN_LENGTH)
            return "script find_regex exceeds maximum length";
        if (!validateFlags(flags))
            return "script flags are invalid";
        if (!Array.isArray(placement) || !placement.every((p) => (typeof p === "string" && APPLY_VALID_PLACEMENTS.has(p)))) {
            return "script placement is invalid";
        }
        const normalizedTarget = Array.isArray(target) ? target : (typeof target === "string" ? [target] : ["display"]);
        if (!normalizedTarget.includes("display"))
            return "only display regex scripts can be applied";
        if (!APPLY_VALID_MACRO_MODES.has(substituteMacros))
            return "script substitute_macros is invalid";
        scripts.push({
            id,
            user_id: userId,
            ...
            trim_strings: Array.isArray(raw.trim_strings)
                ? raw.trim_strings.filter((trim) => typeof trim === "string")
                : [],
...
app.get("/active", (c) => {
    ...
    const target = c.req.query("target");
    if (!target)
        return c.json({ error: "target query param is required" }, 400);
    const characterId = c.req.query("character_id");
    const chatId = c.req.query("chat_id");
...
app.post("/apply", async (c) => {
    ...
    if (content.length > APPLY_MAX_CONTENT_LENGTH)
        return c.json({ error: "content exceeds maximum length" }, 413);
    const scripts = normalizeDisplayScripts(body.scripts, userId);
    if (typeof scripts === "string")
        return c.json({ error: scripts }, 400);
    ...
    const dynamicMacros = isStringRecord(body.dynamic_macros)
        ? Object.fromEntries(Object.entries(body.dynamic_macros).filter(([, v]) => typeof v === "string"))
        : undefined;
...
app.post("/test", async (c) => {
    ...
    if (!find_regex || content === undefined)
        return c.json({ error: "find_regex and content are required" }, 400);
    const actions = Array.isArray(match_actions)
        ? match_actions.filter((action) => action === "move_top"
            || action === "move_bottom"
            || action === "repeat_back")
        : [];
...
app.post("/bulk-delete", async (c) => {
    ...
    if (!Array.isArray(ids))
        return c.json({ error: "ids must be an array" }, 400);
    const stringIds = ids.filter((v) => typeof v === "string" && v.length > 0);
...
app.post("/bulk-toggle", async (c) => {
    ...
    if (!Array.isArray(body?.ids))
        return c.json({ error: "ids must be an array" }, 400);
    const stringIds = body.ids.filter((v) => typeof v === "string" && v.length > 0);
...
app.delete("/:id", (c) => {
    const userId = c.get("userId");
    if (!svc.deleteRegexScript(userId, c.req.param("id")))
        return c.json({ error: "Not found" }, 404);
    return c.json({ success: true });
});
...
app.post("/:id/report-evidence", async (c) => {
    ...
    if (!patch)
        return c.json({ error: "evidence patch requires a valid last_ok_ms, last_ok_at, or quarantined field" }, 400);
