...
describe("memory-cortex routes provider registry", () => {
    ...
    test("renders unavailable and timeout fallback", () => {
        providerRegistry.register({ kind: "sidecar", id: "down-sidecar", description: { status: "unavailable" } }, host);
        providerRegistry.register({ kind: "sidecar", id: "slow-sidecar", description: { availability: "timeout" } }, { ...host, installationId: "inst-slow" });
    ...
    test("denied registration is not visible to consumers", () => {
        providerRegistry.register({ kind: "sidecar", id: "denied-sidecar", description: { denied: true } }, host);
        providerRegistry.register({ kind: "sidecar", id: "other-sidecar" }, { installationId: "inst-other", installScope: "user", authenticatedSubject: "intruder" });
    ...
    test("an omitted caller is restricted to system providers", () => {
        ...
        providerRegistry.register({ kind: "sidecar", id: "system-sidecar" }, { installationId: "inst-system", installScope: "system" });
    ...
    test("provider failure is isolated", () => {
        ...
        providerRegistry.register({ kind: "sidecar", id: "broken-sidecar", description: Object.create(null, {
