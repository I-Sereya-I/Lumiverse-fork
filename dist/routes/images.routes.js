...
import { detectImageContentType, isSupportedProxyImageContentType, normalizeImageContentType, validateImageMagicBytes, } from "../utils/image-signature";
...
function parseWallpaperVideoCodec(value) {
    if (!value)
        return undefined;
    const normalized = value.trim().toLowerCase();
    if (!normalized)
        return undefined;
    if (normalized === "h264" || normalized === "hevc")
        return normalized;
    return null;
}
function parseWallpaperUploadProgressId(value) {
    if (!value)
        return undefined;
...
function resolveImageContentType(filepath, fallbackMimeType) {
...
async function readResponseBodyTextCapped(response, maxBytes) {
...
async function fetchRemoteImageAsset(rawUrl, depth = 0) {
...
app.post("/wallpapers", async (c) => {
    ...
    const primaryVideoCodec = videoCodec ?? "h264";
    ...
    const image = await svc.uploadImage(userId, file, {
        ...
        on_progress: uploadId
            ? (progress) => {
                eventBus.emit(EventType.WALLPAPER_UPLOAD_PROGRESS, {
                    uploadId,
                    ...progress,
                }, userId);
            }
