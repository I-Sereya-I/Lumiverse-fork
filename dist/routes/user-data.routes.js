...
import { createTicket, consumePrepareEntry, stashPrepareEntry, } from "../services/user-data/secret-ticket.service";
import { listSecretKeys, getSecret as readSecret } from "../services/secrets.service";
import { persistUploadedArchive, startImport, getJob, reserveImportUpload, releaseImportUpload, cancelJob, verifyArchiveFast, submitTicket, skipTicket, ArchiveValidationError, MAX_COMPRESSED_BYTES, } from "../services/user-data/import.service";
...
function lookupUserSlug(userId) {
    try {
        const row = getDb()
...
// ─── Export ──────────────────────────────────────────────────────────────
function streamingResponse(stream, filename) {
    ...
    if (includeSecrets) {
        ...
        for (const key of candidates) {
            try {
                ...
                const e = err;
                console.warn(`[user-data export] preflight skip ${key}: ${e?.name || "Error"}: ${e?.message || String(err)}`);
                unreachableSecrets.push(key);
            }
        }
...
app.get("/export/archive/:archiveId", (c) => {
    ...
    if (!entry || entry.userId !== userId) {
        return c.json({ error: "Export session not found. Call /export/prepare first." }, 404);
    }
    // Reuse the filename pinned at prepare time so the archive and its paired
    // ticket share the exact same HHMMSS suffix on disk.
    const filename = entry.archiveFilename || `lumiverse-${lookupUserSlug(userId)}-${exportTimestamp()}.lvbak`;
    const stream = buildExportStream({
        userId: entry.userId,
        ...
        producerVersion: null,
        archiveId,
        secrets: entry.includeSecrets && entry.smk
...
app.post("/import", async (c) => {
    ...
    try {
        ...
        if (declared > MAX_COMPRESSED_BYTES) {
            return c.json({ error: "archive exceeds compressed size cap", maxBytes: MAX_COMPRESSED_BYTES }, 413);
        }
        try {
            ...
            if (ct.startsWith("multipart/form-data")) {
                ...
                return c.json({
                    error: "multipart archive uploads are not supported; send the archive as the raw request body",
            ...
            if (err instanceof ArchiveValidationError) {
                const status = err.code === "size" ? 413 : err.code === "not_zip" ? 415 : 400;
                return c.json({ error: err.message, code: err.code }, status);
            }
            return c.json({ error: err?.message || "upload failed" }, 400);
        }
        eventBus.emit(EventType.USER_IMPORT_PROGRESS, { jobId, phase: "verifying" }, userId);
        try {
        ...
        if (!jobStarted)
            releaseImportUpload(userId, jobId);
    }
});
...
app.post("/import/:jobId/ticket", async (c) => {
    ...
    if (job.status !== "awaiting_ticket") {
        return c.json({ error: "Job is not awaiting a ticket", status: job.status }, 409);
    }
    let raw;
    try {
    ...
    try {
        ...
        if (err instanceof TicketError) {
            const status = err.code === "archive_mismatch" || err.code === "binding_mismatch"
                ? 409
