...
app.get("/", async (c) => {
    ...
    const result = await withReadableApiKeyStatuses(userId, svc.listConnections(userId, pagination), svc.connectionSecretKey);
...
app.post("/pollinations/auth-url", async (c) => {
    ...
    if (!redirectUrl)
        return c.json({ error: "redirect_url is required" }, 400);
    let parsed;
    try {
