...
describe("world-book P8 REST mutation contracts", () => {
    ...
    test("writes one H12 namespace without touching host-managed entry fields", async () => {
        ...
        const response = await app.request(`http://localhost/world-books/${book.id}/entries/${entry.id}/extensions/example_ext`, {
            method: "PATCH",
            headers: { "content-type": "application/json", "x-test-user": USER_ID },
        ...
        const hostManaged = await app.request(`http://localhost/world-books/${book.id}/entries/${entry.id}/extensions/wi_marker`, {
            method: "PATCH",
            headers: { "content-type": "application/json", "x-test-user": USER_ID },
