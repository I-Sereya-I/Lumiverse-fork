...
app.get("/auth", async (c) => {
    ...
    if (!connectionId && !connectionName)
        return c.json({ error: "connection_id or connection_name is required" }, 400);
    if (!callbackUrl)
    ...
    if (connectionId) {
        ...
        if (conn.provider !== "openrouter")
            return c.json({ error: "Connection is not an OpenRouter profile" }, 400);
    }
...
app.get("/generation/:connectionId/:generationId", async (c) => {
    ...
    if (!result)
        return c.json({ error: "Failed to fetch generation stats" }, 502);
    return c.json(result);
});
// ── Upstream Providers ────────────────────────────────────────────────────────
/**
...
app.get("/providers/:connectionId", async (c) => {
    ...
    if (!result)
        return c.json({ error: "Failed to fetch providers" }, 502);
