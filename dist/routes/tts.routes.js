...
app.post("/synthesize/stream", async (c) => {
    ...
    const stream = new ReadableStream({
        start(controller) {
            ...
            const emitAudioPayload = (payload) => sendEvent("audio", payload);
            const emitChunkedBytes = (kind, mimeType, data) => {
                if (kind === "audio_file") {
            ...
            void (async () => {
                ...
                try {
                    try {
                        ...
                        for await (const chunk of generator) {
                            if (closed || requestSignal.aborted)
                                return;
                            if (chunk.done || chunk.data.byteLength === 0)
                                continue;
                            const kind = chunk.kind || "bytes";
                            const mimeType = chunk.mimeType || fallbackMimeType;
                            emittedAudio = true;
                            if (!emitChunkedBytes(kind, mimeType, chunk.data))
                                return;
                        }
                    }
                    catch (streamErr) {
                        if (closed || requestSignal.aborted || isAbortError(streamErr))
                            return;
                        if (!emittedAudio) {
                            try {
                                if (!(await emitFallbackAudio()) || closed || requestSignal.aborted)
                                    return;
                    ...
                    if (closed || requestSignal.aborted || isAbortError(err))
                        return;
...
app.post("/save-message-audio", async (c) => {
    ...
    const priorAudio = existingAttachments.find((a) => a && a.type === "audio" && a.swipe_id === targetSwipeId);
    ...
    const filename = (typeof formData.get("filename") === "string" ? formData.get("filename") : "") || `tts-${messageId}-swipe-${targetSwipeId}.mp3`;
