...
app.post("/test", requireOwner, async (c) => {
    const body = await c.req.json();
    if (!body.tokenizer_id || !body.text)
        return c.json({ error: "tokenizer_id and text are required" }, 400);
    try {
...
app.post("/count", publicTokenizerBodyLimit, async (c) => {
    const body = await c.req.json();
    if (!body.model_id || !body.text)
        return c.json({ error: "model_id and text are required" }, 400);
...
app.post("/patterns", requireOwner, async (c) => {
    const body = await c.req.json();
    if (!body.tokenizer_id || !body.pattern)
        return c.json({ error: "tokenizer_id and pattern are required" }, 400);
    try {
...
app.post("/patterns/test", publicTokenizerBodyLimit, async (c) => {
    ...
    if (!tokenizerId)
        return c.json({ matched: false, tokenizer_id: null, tokenizer_name: null });
