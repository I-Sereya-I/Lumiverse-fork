...
describe("PUT /broker-origins live enforcement", () => {
    ...
    test("tightened allowlist is enforced by the singleton registry without restart", async () => {
        ...
        expect(() => providerRegistry.register({
            kind: "embedding",
            id: "pre-tighten",
            broker: { kind: "embedding", url: "https://evil.test/embed" },
        }, { installationId: "inst-a", installScope: "user", authenticatedSubject: "alice" })).not.toThrow();
        providerRegistry.reset();
        ...
        expect(() => providerRegistry.register({
            ...
            id: "post-tighten",
            broker: { kind: "embedding", url: "https://evil.test/embed" },
        }, { installationId: "inst-a", installScope: "user", authenticatedSubject: "alice" })).toThrow(/not approved/);
