...
const app = new Hono();
function chatRoute(handler, extras) {
    return async (c) => {
...
app.post("/council-retry", async (c) => {
    ...
    if (!resolved)
        return c.json({ error: "No pending council retry for this generation" }, 404);
...
app.get("/breakdown/:messageId", async (c) => {
    ...
    if (!data)
        return c.json({ error: "No breakdown found for this message" }, 404);
    return c.json(data);
});
