...
import { getActivatedWorldInfoEntriesForChat, resolveWorldInfoOutlets, } from "../services/prompt-assembly.service";
import { resolveRegexActionEffects } from "../services/associative-regex-effects.service";
import { personaHasAddon, withChatPersonaAddonState, } from "../services/persona-addon-states";
import { dispatchEditAndSendRequest } from "../services/edit-and-send-dispatcher.service";
async function runMessageContentProcessors(ctx, userId, signal) {
    if (messageContentProcessorChain.count === 0)
        return ctx;
    return messageContentProcessorChain.run(ctx, userId, signal);
}
function editRegexOptions(scripts, userId, chatId, message) {
    const hasRepeatBack = regexScriptsSvc.hasRegexMatchAction(scripts, "repeat_back");
    if (!hasRepeatBack || (message?.index_in_chat ?? -1) <= 0)
        return undefined;
    const previousContent = message
        ? svc.getPreviousSameRoleContent(userId, chatId, message.is_user, message.id)
        : undefined;
    return {
...
async function processChatGreeting(userId, chat) {
    if (messageContentProcessorChain.count === 0)
        return;
    const msgs = svc.listMessagesTail(userId, chat.id, 1);
    const greeting = msgs.data[0];
    if (!greeting || greeting.is_user || greeting.extra?.greeting !== true)
        return;
    const ctx = {
        chatId: chat.id,
    ...
    const update = {};
    if (processed.content !== greeting.content)
        update.content = processed.content;
    if (processed.extra && processed.extra !== greeting.extra)
        update.extra = processed.extra;
...
function parseDisplayPreprocessItem(input) {
...
async function runDisplayPreprocessItem(userId, chatId, item, signal) {
...
app.get("/recent", (c) => {
    ...
    const sort = sortParam === "name" || sortParam === "recent" || sortParam === "created" ? sortParam : undefined;
...
app.post("/bulk-delete", async (c) => {
    ...
    const ids = body.ids.filter((value) => typeof value === "string" && value.length > 0);
...
app.post("/:id/mute/:characterId", (c) => {
    ...
    if (!updated)
        return c.json({ error: "Not found or not a group chat member" }, 404);
...
app.post("/:id/members/:characterId", async (c) => {
    ...
    if (!updated)
        return c.json({ error: "Not found, not a group chat, character not found, or already a member" }, 400);
    if (!body.skip_greeting)
        await processChatGreeting(userId, updated);
    return c.json(updated);
});
app.delete("/:id/members/:characterId", (c) => {
    ...
    if (!updated)
        return c.json({ error: "Not found, not a group chat, not a member, or cannot remove (minimum 2 members)" }, 400);
    return c.json(updated);
});
app.patch("/:id/members/:characterId/alternate-fields", async (c) => {
    ...
    const updated = svc.setGroupMemberAlternateFields(userId, c.req.param("id"), c.req.param("characterId"), selections);
    if (!updated) {
...
app.patch("/:id/appearance", async (c) => {
    ...
    if (!action)
        return c.json({ error: "Invalid appearance action" }, 400);
    const result = svc.applyChatAppearance(userId, c.req.param("id"), action);
    if (!result)
        return c.json({ error: "Invalid character, avatar, or binding" }, 400);
...
app.put("/:id/persona-addons/:personaId/:addonId", async (c) => {
    ...
    const metadata = withChatPersonaAddonState(chat.metadata, persona.id, c.req.param("addonId"), body.enabled);
...
app.get("/:id", (c) => {
    ...
    const character_display_owner = chat.character_id
        ...
        : null;
    if (c.req.query("messages") === "false")
        return c.json({ ...chat, character_display_owner });
...
app.post("/import", async (c) => {
    ...
    if (!body.character_id)
        return c.json({ error: "character_id is required" }, 400);
    if (!body.chat || !body.messages)
        return c.json({ error: "chat and messages are required" }, 400);
    try {
        ...
        const bulkMessages = body.messages.map((m) => ({
...
app.post("/import-st-group", async (c) => {
    ...
    for (const [id, character] of characters.entries()) {
        ...
        if (sourceFilename.toLowerCase().endsWith(".png"))
            addKey(sourceFilename.slice(0, -4));
    }
    const personas = personasSvc.listPersonas(userId, { limit: 10000, offset: 0 });
    const personaNameToId = new Map();
    for (const persona of personas.data) {
        if (persona.name)
            personaNameToId.set(persona.name, persona.id);
    }
    const parsed = parseStGroupChatJsonl(await file.text(), (file.name || "group").replace(/\.jsonl$/i, ""), personaNameToId, filenameToId);
    if (!parsed) {
        return c.json({ error: "No valid messages found in file" }, 400);
    }
    try {
...
app.post("/:chatId/edit-and-send", async (c) => {
    ...
    const branchChat = body.branchChat;
...
app.get("/:chatId/messages/search", (c) => {
    ...
    if (query.length === 0)
        return c.json({ data: [], total: 0, message_total: 0, truncated: false });
    if (query.length > 500)
...
app.post("/:chatId/messages", async (c) => {
    ...
    if (processed.extra !== undefined)
        body.extra = processed.extra;
...
app.post("/:chatId/messages/regex-actions/claim", async (c) => {
    ...
    if (result.status === "not_found")
        return c.json({ error: "Source message not found", messages: result.messages }, 404);
    if (result.status === "forbidden")
        return c.json({ error: "composable effects require an assistant source message", messages: result.messages }, 403);
    ...
    if (body.content !== undefined) {
        ...
        if (chat) {
            ...
            if (editScripts.length > 0) {
                const placement = existing?.is_user ? "user_input" : "ai_output";
                body.content = await regexScriptsSvc.applyRegexScripts(body.content, editScripts, placement, 0, undefined, undefined, editRegexOptions(editScripts, userId, chatId, existing));
            }
        }
...
app.put("/:chatId/messages/:id/swipe/:idx", async (c) => {
    ...
    if (chat) {
        ...
        if (editScripts.length > 0) {
            const placement = existing?.is_user ? "user_input" : "ai_output";
            finalContent = await regexScriptsSvc.applyRegexScripts(finalContent, editScripts, placement, 0, undefined, undefined, editRegexOptions(editScripts, userId, chatId, existing));
        }
    }
...
app.delete("/:chatId/messages/:id/swipe/:idx", (c) => {
    ...
    if (!msg)
        return c.json({ error: "Not found, invalid index, or last swipe" }, 404);
    return c.json(msg);
});
app.post("/:chatId/display-preprocess", async (c) => {
    ...
    if (body && typeof body === "object" && Array.isArray(body.items)) {
        ...
        const processed = await Promise.all(items.map((item) => runDisplayPreprocessItem(userId, chatId, item, c.req.raw.signal)));
