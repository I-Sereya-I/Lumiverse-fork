...
app.post("/lucid-cards/import", async (c) => {
    ...
    try {
        ...
        if (!res.ok)
            return c.json({ error: `Pack not found: ${res.status}` }, 404);
...
app.delete("/:id", (c) => {
    const userId = c.get("userId");
    if (!svc.deletePack(userId, c.req.param("id")))
        return c.json({ error: "Not found" }, 404);
...
app.delete("/:id/lumia-items/:itemId", (c) => {
    const userId = c.get("userId");
    if (!svc.deleteLumiaItem(userId, c.req.param("itemId")))
        return c.json({ error: "Not found" }, 404);
    return c.json({ success: true });
});
// --- Loom Item endpoints ---
...
app.delete("/:id/loom-items/:itemId", (c) => {
    const userId = c.get("userId");
    if (!svc.deleteLoomItem(userId, c.req.param("itemId")))
        return c.json({ error: "Not found" }, 404);
    return c.json({ success: true });
});
// --- Loom Tool endpoints ---
...
app.delete("/:id/loom-tools/:toolId", (c) => {
    const userId = c.get("userId");
    if (!svc.deleteLoomTool(userId, c.req.param("toolId")))
        return c.json({ error: "Not found" }, 404);
    return c.json({ success: true });
});
