...
integration.use("/*", async (c, next) => {
    ...
    if (!auth) {
        ...
        if (result.lockout) {
            ...
            return c.json(authLockoutService.buildPayload(result.lockout, "Too many invalid token attempts. Try again later."), 429);
        }
...
function requireScope(scope) {
    return async (c, next) => {
        if (!c.get("streamDeckScopes").includes(scope))
            return c.json({ error: "Forbidden", required_scope: scope }, 403);
        return next();
    };
}
...
integration.get("/characters/:id/avatar", requireScope("characters:read"), async (c) => {
    ...
    if (!image || !image.mime_type.startsWith("image/"))
        return c.json({ error: "Not found" }, 404);
