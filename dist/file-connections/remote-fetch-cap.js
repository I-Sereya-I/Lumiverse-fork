...
function readEnvTimeout() {
    ...
    if (!raw)
        return DEFAULT_REMOTE_FETCH_TIMEOUT_MS;
    const parsed = Number(raw);
    if (!Number.isFinite(parsed) || parsed <= 0)
        return DEFAULT_REMOTE_FETCH_TIMEOUT_MS;
    return Math.floor(parsed);
 ...
 */
export async function readResponseBuffer(response, maxBytes, label) {
    const declared = response.headers.get("content-length");
    if (declared) {
        const n = parseInt(declared, 10);
        if (Number.isFinite(n) && n > maxBytes) {
            throw new Error(`Remote file "${label}" too large: ${n} bytes (max ${maxBytes})`);
        }
    }
    if (!response.body)
    ...
    while (true) {
        ...
        if (total > maxBytes) {
            ...
            throw new Error(`Remote file "${label}" exceeded ${maxBytes} bytes`);
        }
        chunks.push(value);
    }
