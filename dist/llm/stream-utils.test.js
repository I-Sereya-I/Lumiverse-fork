...
describe("fetchWithPreflightAbort", () => {
    test("aborts the provider request before response headers arrive", async () => {
        ...
        try {
            ...
            const pending = fetchWithPreflightAbort("https://provider.test/stream", {}, controller.signal);
    ...
    test("does not forward later aborts after response headers arrive", async () => {
        ...
        try {
            const controller = new AbortController();
            const response = await fetchWithPreflightAbort("https://provider.test/stream", {}, controller.signal);
            controller.abort(new DOMException("Stopped", "AbortError"));
