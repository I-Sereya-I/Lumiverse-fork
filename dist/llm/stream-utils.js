...
const responseConnections = new WeakMap();
...
export function closeConnection(res) {
...
export async function cancelStreamAndCloseConnection(reader, res) {
    await reader.cancel().catch(() => { });
    closeConnection(res);
}
export async function fetchWithPreflightAbort(input, init, signal) {
    if (signal?.aborted) {
...
export async function readJsonWithAbort(res, signal, maxBytes = DEFAULT_MAX_JSON_BYTES) {
    if (!res.body) {
...
export function createCooperativeYielder(every, signal) {
    let count = 0;
