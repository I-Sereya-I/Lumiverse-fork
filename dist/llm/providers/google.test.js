...
describe("GoogleProvider tool calling wire shape", () => {
    ...
    test("replays an optional non-tool thought signature only when enabled", () => {
        ...
        const enabled = provider.buildBody({
            ...request,
        ...
        const disabled = provider.buildBody({ ...request, parameters: {} });
        expect(disabled.contents[1].parts[0].thoughtSignature).toBe("context_engineering_is_the_way_to_go");
    });
...
describe("GoogleProvider web search grounding", () => {
    test.each(["googleSearch", "google_search", "enable_web_search"])("adds google_search for the %s parameter", (parameter) => {
        const provider = new GoogleProvider();
        const body = provider.buildBody({
            model: "gemini-2.5-flash",
    ...
    test("preserves response grounding metadata in provider usage", async () => {
        ...
        try {
            const provider = new GoogleProvider();
            const result = await provider.generate("key", "https://generativelanguage.googleapis.com", {
                model: "gemini-2.5-flash",
                messages: [{ role: "user", content: "Latest news" }],
                parameters: { enable_web_search: true },
                tools: [],
