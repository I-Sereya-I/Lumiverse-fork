...
export function isGoogleSearchUnsupported(model) {
    return GOOGLE_SEARCH_UNSUPPORTED.some((pattern) => pattern.test(model || ""));
}
function isGoogleSearchEnabled(params) {
    if (params.googleSearch !== undefined)
        return params.googleSearch === true;
    if (params.google_search !== undefined)
        return params.google_search === true;
    return params.enable_web_search === true;
}
/** Build the provider-native Google Search tool, or null when it cannot be used. */
export function buildGoogleSearchTool(providerName, model, params, hasFunctionDeclarations) {
    if (!isGoogleSearchEnabled(params))
        return null;
    if (hasFunctionDeclarations) {
        console.warn(`[${providerName}] Google Search grounding skipped: it cannot be combined with inline function tools.`);
        return null;
    }
    if (isGoogleSearchUnsupported(model)) {
        console.warn(`[${providerName}] Google Search grounding skipped: model ${model} does not support it.`);
        return null;
    }
...
export function appendGoogleSearchTool(providerName, body, googleSearchTool) {
    if (!googleSearchTool)
        return;
    ...
    const alreadyHasSearch = body.tools.some((tool) => tool?.google_search !== undefined || tool?.googleSearch !== undefined);
    if (alreadyHasSearch)
        return;
    const hasCustomFunctions = body.tools.some((tool) => Array.isArray(tool?.functionDeclarations) && tool.functionDeclarations.length > 0);
    if (hasCustomFunctions) {
        console.warn(`[${providerName}] Google Search grounding skipped: it cannot be combined with custom function declarations.`);
        return;
    }
