 ...
 */
import { registerRoomAuthValidator } from "../services/room-auth";
import { deriveRoomSecret } from "./room-secret";
import { peekMpidClaims, verifyMpidToken } from "./mpid-token";
export const identityServerAttestationValidator = {
    ...
    async tryValidate(req, expectedRoomId) {
        ...
        if (peek?.typ !== "relay-direct" || !peek.rid)
            return null;
        if (expectedRoomId && peek.rid !== expectedRoomId)
            return null;
...
export function registerIdentityServerAttestation() {
