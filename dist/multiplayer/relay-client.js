...
export function isRemoteBridgeActive(roomId) {
    return bridges.has(roomId);
}
export async function startRelayBridge(roomId) {
    ...
    bridge.unsubs.push(eventBus.onRoomBroadcast((rid, event, payload) => {
        if (rid !== bridge.roomId)
            return;
    ...
    if (typeof roomHb.unref === "function") {
        roomHb.unref();
    }
...
export function stopRelayBridge(roomId) {
    ...
    if (bridge.roomHeartbeatTimer)
        clearInterval(bridge.roomHeartbeatTimer);
    stopHeartbeat(bridge);
    for (const unsub of bridge.unsubs)
        unsub();
    try {
        bridge.ws?.close();
    }
...
async function connect(bridge) {
    ...
    try {
        ...
        ws.onclose = () => {
            if (bridge.ws !== ws)
                return; // ignore a stale socket's late close
            bridge.ws = null;
            stopHeartbeat(bridge);
            if (!bridge.stopped)
...
function rehydratePeers(bridge) {
    if (bridge.memberToParticipant.size === 0)
        return;
...
function forceReconnect(bridge) {
    const ws = bridge.ws;
    bridge.ws = null;
    stopHeartbeat(bridge);
    try {
...
function sendFrame(bridge, frame) {
    const ws = bridge.ws;
    if (!ws || ws.readyState !== WebSocket.OPEN)
        return;
    try {
...
function handleRelayFrame(bridge, raw) {
    let frame;
    try {
    ...
    if (frame.t === "ctrl") {
        const d = frame.d;
        if (d?.type === "invite_consumed")
            void cycleInviteCode(bridge.roomId);
        return;
    }
 ...
 */
async function cycleInviteCode(roomId) {
    const room = mp.getRoom(roomId);
    if (!room)
        return;
    ...
    if (!invite)
        return;
    eventBus.emit(EventType.ROOM_INVITE_CODE, { roomId, code: invite.code, expiresAt: invite.expiresAt, server: mpidConfig.url }, room.host_user_id);
}
