...
function applyLockoutHeaders(response, retryAfterSeconds) {
...
app.use("/api/*", async (c, next) => {
    ...
    if (lockout) {
        ...
        return applyLockoutHeaders(c.json(authLockoutService.buildPayload(lockout, "Too many authentication failures from this client. Try again later."), 429), Math.max(1, Math.ceil(lockout.retryAfterMs / 1000)));
    }
    return next();
});
app.use("/api/*", async (c, next) => {
...
app.use("/api/*", async (c, next) => {
    ...
    if (!env.trustAnyOrigin && origin && !isOriginAllowed(origin)) {
        ...
        if (result.lockout) {
            return applyLockoutHeaders(c.json(authLockoutService.buildPayload(result.lockout, "Too many invalid-origin requests. Try again later."), 429), Math.max(1, Math.ceil(result.lockout.retryAfterMs / 1000)));
        }
...
app.use("/api/auth/*", async (c, next) => {
    ...
    if (!result.lockout)
        return;
    c.res = applyLockoutHeaders(c.json(authLockoutService.buildPayload(result.lockout, "Too many failed sign-in attempts. Try again later."), 429), Math.max(1, Math.ceil(result.lockout.retryAfterMs / 1000)));
});
