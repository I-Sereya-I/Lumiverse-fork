        ...
        async onOpen(_event, ws) {
            ...
            try {
                ...
                if (roomToken) {
                    ...
                    const rawPeer = ws.raw;
                    // Token connections are always peers → subscribe to the feed topic.
                    if (rawPeer)
                        eventBus.subscribeClientToRoom(rawPeer, join.room.id, join.participant.id, { feed: true });
        ...
        async onMessage(event, ws) {
            try {
                ...
                if (rawDataBytes > sizeLimit) {
                    console.warn(`[WS] dropped oversized message: ${rawDataBytes} bytes ` +
                ...
                if (data.type === "room_leave") {
                    ...
                    if (raw)
                        eventBus.unsubscribeClientFromRoom(raw, roomAuth.roomId, roomAuth.participantId);
                    roomAuth = null;
                    return;
                }
                if (data.type === "room_message") {
                    if (!roomAuth)
                        return;
                    const result = multiplayerSvc.submitPeerMessage(roomAuth.roomId, roomAuth.participantId, data.content, data.associative_regex_append);
                    if (!result.ok) {
                ...
                if (data.type === "SPINDLE_BACKEND_MSG") {
                    ...
                    if (data.payload &&
                        ...
                        data.payload.type === "message_tag_intercepted") {
                ...
                if (data.type === "SPINDLE_FRONTEND_PROCESS_EVENT") {
                    ...
                    if (!extensionId || !processId || !processEvent || !userId)
                        return;
                    const ext = await managerSvc.getExtensionForUser(extensionId, userId, userRole);
                    if (!ext)
                        return;
                    ...
                    host.handleFrontendProcessEvent(processId, userId, processEvent, typeof data.error === "string" ? data.error : undefined);
                    return;
                }
                if (data.type === "SPINDLE_FRONTEND_PROCESS_MSG") {
                    const extensionId = typeof data.extensionId === "string" ? data.extensionId : null;
                    const processId = typeof data.processId === "string" ? data.processId : null;
                    if (!extensionId || !processId || !userId)
                        return;
                    const ext = await managerSvc.getExtensionForUser(extensionId, userId, userRole);
                    if (!ext)
                        return;
                ...
                if (data.type === "SPINDLE_COMMAND_INVOKE") {
                    ...
                    if (!extensionId || !commandId || !userId)
                        return;
                    const ext = await managerSvc.getExtensionForUser(extensionId, userId, userRole);
                    if (!ext)
                        return;
                ...
                if (err instanceof SyntaxError)
                    return;
                console.error("[WS] onMessage handler failed:", err instanceof Error ? err.stack || err.message : err);
            }
        },
        onClose(_event, ws) {
            const raw = ws.raw;
