...
function parseOptionalNonNegativeIntEnv(name) {
    const raw = process.env[name];
    if (!raw)
        return undefined;
    const parsed = parseInt(raw, 10);
...
function parseRatioOrPercentEnv(name, fallback) {
    const raw = process.env[name];
    if (!raw)
        return fallback;
...
function parseEphemeralOverrides(raw) {
    if (!raw)
        return {};
    const out = {};
...
export function loadEnv() {
    ...
    const diskWarningUsageThreshold = parseRatioOrPercentEnv("LUMIVERSE_DISK_WARNING_USAGE_PERCENT", DEFAULT_DISK_WARNING_USAGE_THRESHOLD);
    const diskWarningMinFreeBytes = parsePositiveIntEnv("LUMIVERSE_DISK_WARNING_MIN_FREE_BYTES", DEFAULT_DISK_WARNING_MIN_FREE_BYTES);
    ...
    const safeThemeMode = ["1", "true", "yes", "on"].includes((process.env.LUMIVERSE_SAFE_THEME || "").trim().toLowerCase());
    ...
    const trustedProxies = (process.env.TRUSTED_PROXIES ?? "")
        ...
        .filter(Boolean);
    const spindleEphemeralGlobalMaxBytes = parsePositiveIntEnv("SPINDLE_EPHEMERAL_GLOBAL_MAX_BYTES", 500 * 1024 * 1024);
    const spindleEphemeralExtensionDefaultMaxBytes = parsePositiveIntEnv("SPINDLE_EPHEMERAL_EXTENSION_DEFAULT_MAX_BYTES", 50 * 1024 * 1024);
    const spindleEphemeralReservationTtlMs = parsePositiveIntEnv("SPINDLE_EPHEMERAL_RESERVATION_TTL_MS", 10 * 60 * 1000);
    const spindleEphemeralExtensionMaxOverrides = parseEphemeralOverrides(process.env.SPINDLE_EPHEMERAL_EXTENSION_MAX_OVERRIDES);
    ...
    const sqliteMmapEnabled = process.env.LUMIVERSE_SQLITE_MMAP_ENABLED === "true" &&
