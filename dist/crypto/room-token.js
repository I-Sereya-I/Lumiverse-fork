 ...
 */
function getRoomTokenKey() {
    if (!_keyPromise) {
        _keyPromise = (async () => {
            const identityKey = getEncryptionKeyBytes();
            const baseKey = await crypto.subtle.importKey("raw", identityKey, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
            const tag = new TextEncoder().encode("lumiverse-room-token-v1");
            const subkey = await crypto.subtle.sign("HMAC", baseKey, tag);
            return crypto.subtle.importKey("raw", new Uint8Array(subkey), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
        })();
    }
    return _keyPromise;
}
/** Test/maintenance hook: drop the cached signing key (e.g. after identity rotation). */
export function resetRoomTokenKeyCache() {
    _keyPromise = null;
}
// ─── public API ───────────────────────────────────────────────────────────────
 ...
 */
export async function verifyRoomToken(token, expectedRoomId) {
    ...
    const key = await getRoomTokenKey();
    const expectedSig = new Uint8Array(await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(signingInput)));
    let providedSig;
    try {
    ...
    let claims;
    try {
        claims = JSON.parse(stringFromB64url(p));
    }
    catch {
        return null;
    }
    if (!claims || claims.v !== 1 || claims.rol !== "peer")
        return null;
    if (typeof claims.rid !== "string" || typeof claims.sub !== "string")
        return null;
    ...
    if (claims.iat > now + 60)
        return null; // post-dated beyond allowed skew
    if (expectedRoomId && claims.rid !== expectedRoomId)
        return null;
    return claims;
}
