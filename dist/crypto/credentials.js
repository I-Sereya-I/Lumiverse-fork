 ...
 */
export async function ownerCredentialsExist(filePath) {
 ...
 */
export async function readOwnerCredentials(filePath) {
 ...
 */
export async function writeOwnerCredentials(filePath, username, passwordHash) {
    ...
    if (!(await written.exists()) || written.size === 0) {
        throw new Error(`Failed to write credentials file — file is empty or missing after write: ${filePath}`);
    }
    return credentials;
}
