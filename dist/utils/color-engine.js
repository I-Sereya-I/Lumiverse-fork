 ...
 */
export function ensureContrast(foreground, background, minRatio) {
 ...
 */
export function constrainLuminance(color, minLum, maxLum) {
...
function chooseQuantizationStep(avgDeviation) {
    if (avgDeviation < 12)
        return 36;
    if (avgDeviation < 22)
        return 28;
...
function scoreCandidate(color, weight) {
...
function analyzePixels(data, channels, pixelCount) {
    let rSum = 0;
    let gSum = 0;
    let bSum = 0;
    let opaqueCount = 0;
    ...
    for (let i = 0; i < pixelCount; i++) {
        ...
        if (channels === 4 && data[offset + 3] < 48)
            continue;
        const r = data[offset];
        const g = data[offset + 1];
        const b = data[offset + 2];
        const lum = luminance(r, g, b);
        if (lum < DARK_SURFACE_MIN_LUM)
            tooDarkCount++;
        if (lum > LIGHT_SURFACE_MAX_LUM)
            tooLightCount++;
        rSum += r;
        gSum += g;
        bSum += b;
...
function dominantFromPixels(data, channels, pixelCount) {
    return analyzePixels(data, channels, pixelCount).dominant;
}
function kMeansPalette(pixels, k, maxIterations = 12) {
    if (pixels.length === 0)
        return [];
    if (k > pixels.length)
        k = pixels.length;
    ...
    const centroids = sorted.slice(0, k).map((b) => ({ ...b.lab }));
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
        ...
        for (let i = 0; i < centroids.length; i++) {
            ...
            if (deltaE(next, centroids[i]) > 0.5)
                moved = true;
            centroids[i] = next;
        }
        if (!moved)
...
function buildKMeansPalette(data, channels, pixelCount, desiredCount) {
    const pixels = [];
    for (let i = 0; i < pixelCount; i++) {
        const offset = i * channels;
        if (channels === 4 && data[offset + 3] < 48)
...
function classifySwatches(palette, populations) {
...
const HUE_BINS = 12;
const LUM_BINS = 5;
function selectDistinctColors(candidates, desiredCount, diversityScore) {
    if (candidates.length === 0)
        return [];
    const baseThreshold = diversityScore > 0.32 ? 24 :
    ...
    const scored = candidates
        ...
        .sort((a, b) => b.score - a.score);
    const chosen = [];
    function tooClose(item) {
        return chosen.some((sel) => deltaE(sel.lab, item.lab) < 8);
    }
    function passesMinDistance(item, minDist) {
        if (chosen.length === 0)
            return true;
    ...
    while (chosen.length < desiredCount) {
        ...
        for (const item of scored) {
            if (tooClose(item))
                continue;
            if (!passesMinDistance(item, minDist))
                continue;
            let priority = item.score;
            if (hueCounts[item.hueBin] === 0)
                priority += 85;
            else if (hueCounts[item.hueBin] === 1)
                priority += 35;
            if (lumCounts[item.lumBin] === 0)
                priority += 65;
            else if (lumCounts[item.lumBin] === 1)
                priority += 20;
...
function pickFallbackHue(dominant, average) {
    const candidates = [
 ...
 */
function pickSeedHue(palette, dominant, average) {
 ...
 */
function deriveSurfaceFromSeed(seedHue, mode) {
...
function buildFallbackPalette(dominant, average) {
...
function pickReadableTextColor(surface, tint, minRatio) {
...
function tuneAccentForSurface(accentBase, surface, mode) {
    const accentHsl = rgbToHsl(accentBase.r, accentBase.g, accentBase.b);
    let tuned = hslToRgb(accentHsl.h, clamp(accentHsl.s, 32, 88), mode === "dark" ? clamp(accentHsl.l, 42, 74) : clamp(accentHsl.l, 26, 40));
    tuned = mode === "dark"
...
function dedupeColors(colors) {
    ...
    for (const color of colors) {
        if (unique.some((existing) => colorDistance(existing, color) < 9))
            continue;
        unique.push(color);
    }
    return unique;
}
function pickAccentBase(colors, surface, seedHue) {
...
function deriveReadableScheme(surfaceBase, accentBase, mode, _luminanceProfile) {
...
function deriveUiSchemes(palette, dominant, average, luminanceProfile = { mostlyTooDark: false, mostlyTooLight: false }) {
...
function deriveAmbientGradient(palette, dominant, average, swatches) {
...
const REF_DARK_BG = { r: 10, g: 10, b: 15 };
const REF_LIGHT_BG = { r: 250, g: 250, b: 252 };
const DARK_MODE_MAX_LUM = 215;
const LIGHT_MODE_MIN_LUM = 50;
const MIN_VIBRANT_SAT = 20;
function pickMostVibrant(palette, dominant, regions, flatness, average) {
    const candidates = [
...
export function deriveCharacterNameVars(palette, dominant, regions, flatness, average) {
...
function deriveSecondaryTone(seed, surface, mode) {
    const hsl = rgbToHsl(seed.r, seed.g, seed.b);
    let secondary = hslToRgb(hsl.h, clamp(hsl.s, mode === "dark" ? 20 : 16, mode === "dark" ? 58 : 48), mode === "dark" ? clamp(hsl.l, 42, 60) : clamp(hsl.l, 30, 46));
...
export function deriveCharacterOverlay(palette, ui) {
 ...
 */
export function extractColorsFromRawPixels(data, width, height, channels) {
    const pixelCount = width * height;
    const fullAnalysis = analyzePixels(data, channels, pixelCount);
    const regionDefs = getRegions(width, height);
    const regions = {};
    const flatness = { full: fullAnalysis.dominant.flatness };
    ...
    for (const [name, rect] of Object.entries(regionDefs)) {
        ...
        regions[name] = result.color;
        flatness[name] = result.flatness;
