...
function ensureReadable(color, isDark) {
    ...
    if (Math.abs(clampedL - lPct) < 1 && Math.abs(clampedS - sPct) < 1)
        return color;
 ...
 */
function constrainSurface(color, isDark) {
    const rgb = parseColorToRgb(color);
    if (!rgb)
        return color;
...
function adjustHex(hex, factor) {
    ...
    if (!parsed)
        return hex;
    const adjust = (c) => Math.round(clamp(c + factor * 255, 0, 255));
...
// ── Main generator ──
export function generateThemeVariables(input) {
