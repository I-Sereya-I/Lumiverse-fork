...
import { ipMatchesRule, parseCidrRule, parseIp, selectForwardedClientIp, } from "./client-ip";
describe("parseIp", () => {
    test("parses IPv4", () => {
...
describe("selectForwardedClientIp", () => {
    ...
    test("walks right-to-left past known proxies", () => {
        ...
        const selected = selectForwardedClientIp(["203.0.113.5", "10.0.0.3", "10.0.0.2"], (ip) => proxies.has(ip));
    ...
    test("spoofed trusted-proxy address on the left is not selected", () => {
        ...
        const selected = selectForwardedClientIp(["10.0.0.2", "198.51.100.9", "10.0.0.2"], (ip) => proxies.has(ip));
        expect(selected).toBe("198.51.100.9");
    });
