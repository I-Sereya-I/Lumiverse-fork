...
function repairQuotedColorTagBoundaries(text) {
    const repair = (_, openTag, openQuote, inner, closeTag, closeQuote) => {
        const trimmedInner = inner.trimEnd();
        const lastChar = trimmedInner[trimmedInner.length - 1];
        if (lastChar && QUOTE_CHARS.has(lastChar))
            return _;
...
function repairUnterminatedFontColorQuotes(text) {
    return text.replace(/<font\b([^>]*?\bcolor\s*=\s*)(["'])([^"'>]*)(>)/gi, (_match, before, quote, value) => `<font${before}${quote}${value}${quote}>`);
}
function findUnescapedChar(text, target, from) {
 ...
 */
function findFontScopeBoundary(segment) {
    ...
    if (closingQuote) {
        const closingIndex = findUnescapedChar(segment, closingQuote, leadingWhitespace + 1);
        if (closingIndex >= 0)
 ...
 */
function closeUnterminatedFontTags(text) {
...
function trimEdgeWhitespaceInEmphasis(text, delimiter) {
    const delimiters = [3, 2, 1];
    let result = text;
    for (const size of delimiters) {
        const marker = delimiter.repeat(size).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const pattern = new RegExp(`(^|[\\s([{"'“‘«>—–-])(${marker})(?!\\${delimiter})([^\\n]*?)${marker}(?!\\${delimiter})(?=$|[\\s)\\]},.!?:;"'”’»<—–-])`, "g");
        result = result.replace(pattern, (match, prefix, openingMarker, body) => {
            if (!/^[ \t]+|[ \t]+$/.test(body))
                return match;
...
function trimEdgeWhitespaceInQuotes(text) {
    const patterns = [STRAIGHT_QUOTE_RE, CURLY_DOUBLE_QUOTE_RE, ANGLE_QUOTE_RE];
    let result = text;
    for (const pattern of patterns) {
        result = result.replace(pattern, (match, prefix, openQuote, body, closeQuote) => {
            if (!/^[ \t]+|[ \t]+$/.test(body))
                return match;
            const trimmed = body.replace(/^[ \t]+|[ \t]+$/g, "");
            if (!trimmed)
...
function healUnshieldedSegment(text) {
 ...
 */
function healAroundMatches(text, pattern) {
    let healed = "";
    let cursor = 0;
...
export function healFormattingArtifacts(text) {
    if (!text)
        return text;
