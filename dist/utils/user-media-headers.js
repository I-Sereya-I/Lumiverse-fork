...
const SAFE_INLINE_MEDIA_RE = /^(?:image\/(?:avif|bmp|gif|jpeg|jpg|png|apng|webp|svg\+xml)|video\/|audio\/)/;
...
export function classifyUserMediaContentType(contentType) {
    const base = (contentType ?? "").split(";")[0].trim().toLowerCase();
    if (!base || base === "application/octet-stream")
        return "unknown";
    if (ACTIVE_CONTENT_RE.test(base))
        return "active";
 ...
 */
export function userMediaServingHeaders(contentType) {
    ...
    const headers = {
        ...
        "Content-Security-Policy": "default-src 'none'; style-src 'unsafe-inline'; img-src 'self'; media-src 'self'; font-src 'self'; sandbox",
    };
    if (kind === "active") {
