...
function hasAvifBrand(data) {
    if (data.length < 16)
        return false;
    if (readAscii(data, 4, 8) !== "ftyp")
        return false;
...
export function isSupportedProxyImageContentType(contentType) {
    return SUPPORTED_PROXY_IMAGE_CONTENT_TYPES.has(contentType);
}
export function detectImageContentType(data) {
    if (data.length < 2)
        return null;
    if (data.length >= 4
        && data[0] === 0x89
...
export function validateImageMagicBytes(data, contentType) {
