...
function toAbsoluteHttpUrl(baseUrl, rawUrl) {
...
function scoreCandidate(url, pageHost) {
    let score = 0;
    if (IMAGE_EXTENSION_RE.test(url))
        score += 4;
    try {
        const parsed = new URL(url);
        if (parsed.hostname === pageHost)
            score += 1;
        if (parsed.hostname.endsWith(`.${pageHost}`) || pageHost.endsWith(`.${parsed.hostname}`))
            score += 1;
    }
    catch { }
    return score;
}
export function extractRemoteImageUrlFromHtml(pageUrl, html) {
