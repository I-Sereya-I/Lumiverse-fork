...
export function decorateRegexActionHtml(html, actions) {
    if (actions.length === 0 || !html.includes("<"))
        return html;
    ...
    return html.replace(OPEN_TAG_RE, (tag) => {
        if (/^<\//.test(tag) || /\bdata-lumiverse-regex-action\s*=/.test(tag))
            return tag;
 ...
 */
export function buildRegexActionCaptureTemplate(actions) {
    const nonce = `\u0002LRA:${crypto.randomUUID()}:`;
    const fieldCount = actions.reduce((count, action) => (count + 5 + (action.effects ?? []).filter((effect) => effect.type !== "fork").length), 0);
    ...
    for (const action of actions) {
        ...
        for (const effect of action.effects ?? []) {
            if (effect.type === "set_state")
                template += effect.value + markers[templateMarker++];
            else if (effect.type === "draft")
                template += effect.content + markers[templateMarker++];
        }
    }
    return {
        template,
        unpack(replacement, scriptId, instanceId) {
            if (!replacement.startsWith(markers[0]))
                return [];
            ...
            for (const action of actions) {
                ...
                for (const effect of action.effects ?? []) {
                    ...
                    if (effect.type === "set_state")
                        ...
                        effects.push({ ...effect, content: value });
                }
                const [title, subtitle, content, cost, limit] = fields;
                resolved.push({
                    ...action,
                    title,
                    subtitle,
...
export function decorateRegexActionReplacements(replacements, actionMatches, unpack, scriptId) {
    return replacements.map((replacement, index) => decorateRegexActionHtml(replacement, actionMatches[index]
        ? unpack(actionMatches[index].replacement, scriptId, `${scriptId}:${actionMatches[index].index}:${actionMatches[index].index + actionMatches[index].matchLength}`)
        : []));
}
