...
export function stripDetailsBlocks(content) {
    let result = content;
    let prev;
    let iter = 0;
    do {
        if (++iter > MAX_FILTER_ITERATIONS)
            break;
        prev = result;
...
export function stripLoomTags(content) {
    let result = content;
 ...
 */
export function stripHtmlFormattingTags(content) {
    let result = content;
    ...
    do {
        if (++iter > MAX_FILTER_ITERATIONS)
            break;
        prev = result;
        result = result.replace(/<\s*([a-zA-Z][\w:-]*)\b[^>]*>[\s\S]*?<\s*\/\s*\1\s*>/g, (match, tag) => HTML_INLINE_TAGS.has(tag.toLowerCase()) ? match : " ");
    } while (result !== prev);
    result = result.replace(/<\s*([a-zA-Z][\w:-]*)\b[^>]*\/\s*>/g, (match, tag) => HTML_INLINE_TAGS.has(tag.toLowerCase()) ? match : " ");
    result = result.replace(/<\s*\/\s*([a-zA-Z][\w:-]*)\s*>/g, (match, tag) => HTML_INLINE_TAGS.has(tag.toLowerCase()) ? match : " ");
    result = result.replace(/<\s*([a-zA-Z][\w:-]*)\b[^>]*>/g, (match, tag) => HTML_INLINE_TAGS.has(tag.toLowerCase()) ? match : " ");
...
export function collapseExcessiveNewlines(content) {
...
export function stripAllHtmlTagsPreserveContent(content) {
    return content
        .replace(/<\s*br\s*\/?>/gi, "\n")
...
function stripCustomReasoningBlocks(content, prefix, suffix) {
    ...
    let result = content.replace(new RegExp(`\\s*${escapedPrefix}[\\s\\S]*?${escapedSuffix}\\s*`, "g"), " ");
    // Strip trailing unclosed custom reasoning blocks (interrupted generation)
    result = result.replace(new RegExp(`\\s*${escapedPrefix}[\\s\\S]*$`), "");
    return result;
}
/**
 ...
 */
export function stripScaffoldTagBlocks(content, tagNames) {
 ...
 */
export function stripAllXmlTagsAndContent(content) {
    let result = content;
    ...
    do {
        ...
        result = result.replace(/<\s*([a-zA-Z][\w:-]*)\b[^>]*>[\s\S]*?<\s*\/\s*\1\s*>/g, " ");
        iterations++;
...
function normalizeHtmlTagNames(tagNames) {
    if (!tagNames?.length)
        return [];
...
function preserveConfiguredInnerTags(content, tagNames) {
    if (tagNames.length === 0)
        return stripAllHtmlTagsPreserveContent(content);
    const preserved = [];
    const protectedContent = content.replace(new RegExp(`<\\s*\\/?\\s*(?:${tagNames.join("|")})\\b[^>]*>`, "gi"), (match) => {
        preserved.push(match);
        return `\x00FTI${preserved.length - 1}\x00`;
    });
    return stripAllHtmlTagsPreserveContent(protectedContent).replace(/\x00FTI(\d+)\x00/g, (_, index) => preserved[Number(index)] ?? "");
}
/** Within a preserved font block, strip nested formatting but keep its prose. */
function cleanFontBlockInner(fontBlock, preserveInnerTags = []) {
 ...
 */
export function stripNonProseTags(content, options) {
    let result = content;
    ...
    if (options?.keepFontTags) {
        ...
        result = result.replace(/<font\b[^>]*>([\s\S]*?)<\/font\s*>/gi, (_, inner) => stripAllHtmlTagsPreserveContent(inner));
        result = result.replace(/<span\s+style\s*=\s*["'][^"']*color\s*:[^"']*["'][^>]*>([\s\S]*?)<\/span\s*>/gi, (_, inner) => stripAllHtmlTagsPreserveContent(inner));
        result = stripAllXmlTagsAndContent(result);
    }
 ...
 */
export function sanitizeForVectorization(content, options) {
