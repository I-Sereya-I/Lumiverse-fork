 ...
 */
function pinDnsCache(hostname, port) {
    try {
 ...
 */
async function resolveViaDoh(hostname, endpoint, timeoutMs) {
    ...
    const query = async (type) => {
        ...
        for (const ans of body.Answer) {
            if (ans.type === type && typeof ans.data === "string")
                ips.push(ans.data);
        }
        return ips;
    };
    try {
        const [v4, v6] = await Promise.all([
            query(DNS_TYPE_A).catch(() => []),
            query(DNS_TYPE_AAAA).catch(() => []),
        ]);
        return { v4, v6 };
    }
    finally {
...
export async function validateHost(hostname, options) {
    ...
    if (hostname.startsWith("[") || hostname.includes(":")) {
        ...
        if (allowsLoopback(options) && isLoopbackIPv6(bare))
            return;
        if (options?.allowPrivate && isPrivateIPv6(bare))
            return;
    ...
    const v4Addrs = new Set();
    const v6Addrs = new Set();
    const resolvers = [
        resolve4(hostname).then((addrs) => addrs.forEach((a) => v4Addrs.add(a)), () => { }),
        resolve6(hostname).then((addrs) => addrs.forEach((a) => v6Addrs.add(a)), () => { }),
        lookup(hostname, { all: true }).then((addrs) => {
            for (const a of addrs) {
                if (a.family === 4)
                    v4Addrs.add(a.address);
                else if (a.family === 6)
                    v6Addrs.add(a.address);
            }
        }, () => { }),
    ];
    let timedOut = false;
    ...
    if (v4Addrs.size === 0 && v6Addrs.size === 0) {
        throw new SSRFError(timedOut
            ? `DNS resolution timed out after ${dnsTimeoutMs}ms for hostname: ${hostname}`
            : `Could not resolve hostname: ${hostname}`);
    }
    for (const ip of v4Addrs) {
        if (allowsLoopback(options) && isLoopbackIPv4(ip))
            continue;
        if (options?.allowPrivate && isPrivateIPv4(ip))
            continue;
        if (isPrivateIPv4(ip)) {
    ...
    for (const ip of v6Addrs) {
        if (allowsLoopback(options) && isLoopbackIPv6(ip))
            continue;
        if (options?.allowPrivate && isPrivateIPv6(ip))
            continue;
        if (isPrivateIPv6(ip)) {
            throw new SSRFError(`URL resolves to private IP: ${ip} (from ${hostname})`);
        }
