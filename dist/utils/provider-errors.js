...
function getErrorCauseMessage(err) {
    if (!(err instanceof Error))
        return "";
    const cause = err.cause;
    return getErrorMessage(cause);
}
export function describeTransportError(err, fallback = "Provider request failed") {
    ...
    if (/^fetch failed$/i.test(message) && causeMessage)
        return causeMessage;
    return message;
}
...
export function clampErrorMessage(message) {
    if (!message)
        return "";
...
export function parseProviderErrorBody(raw) {
 ...
 */
export async function readBoundedText(res, maxBytes = 16 * 1024) {
    if (!res.body) {
        try {
...
export async function throwProviderResponseError(provider, operation, res) {
...
export async function fetchProviderJson(provider, operation, input, init) {
    let res;
    try {
    ...
    if (!res.ok)
        await throwProviderResponseError(provider, operation, res);
    try {
...
function cleanProviderMessage(message) {
    ...
    if (payloadMatch) {
        ...
        if (parsed.detail)
            return `${payloadMatch[1]} ${parsed.detail}`;
    }
    return message;
}
export function describeProviderError(err, fallback = "Provider request failed") {
    if (err instanceof ProviderRequestError) {
