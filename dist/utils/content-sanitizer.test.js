...
import { sanitizeForVectorization, stripHtmlFormattingTags, stripNonProseTags, } from "./content-sanitizer";
describe("sanitizeForVectorization", () => {
    test("preserves text inside common HTML formatting wrappers", () => {
        expect(sanitizeForVectorization('Before <font color="#ff0000">hidden text</font> after')).toBe("Before hidden text after");
        expect(sanitizeForVectorization("<p>First paragraph.</p><p>Second paragraph.</p>")).toBe("First paragraph.\n\nSecond paragraph.");
    });
    ...
    test("strips unknown XML-like wrappers without dropping their content", () => {
        expect(sanitizeForVectorization("A <custom-tag attr=\"x\">wrapped fact</custom-tag> B")).toBe("A wrapped fact B");
    });
    test("strips scaffold tags and their content", () => {
        expect(sanitizeForVectorization("Prose. <status>HP: 100/100\nMP: 50/50</status> More prose.")).toBe("Prose. More prose.");
        expect(sanitizeForVectorization("Before <hud>Level: 5 | XP: 1200</hud> after")).toBe("Before after");
        expect(sanitizeForVectorization("Text <dice>3d6: 14</dice> rest")).toBe("Text rest");
        expect(sanitizeForVectorization("A <inventory>sword, shield</inventory> B")).toBe("A B");
        expect(sanitizeForVectorization("Narrative <tool_call>function(args)</tool_call> continues")).toBe("Narrative continues");
    });
});
describe("stripNonProseTags", () => {
    test("removes <details> blocks and their content", () => {
        expect(stripNonProseTags("Visible <details>private note</details> still visible")).toBe("Visible still visible");
    });
    test("removes lumia_ooc blocks and their content", () => {
        expect(stripNonProseTags("Prose. <lumia_ooc>director note about scene</lumia_ooc> More prose.")).toBe("Prose. More prose.");
    ...
    test("strips font tags by default", () => {
        expect(stripNonProseTags('Before <font color="#ff0000">red text</font> after')).toBe("Before red text after");
    });
    test("preserves font tags when keepFontTags is set", () => {
        expect(stripNonProseTags('Before <font color="#ff0000">red text</font> after', { keepFontTags: true })).toBe('Before <font color="#ff0000">red text</font> after');
    });
    test("preserves configured thought delimiters inside font blocks until attribution", () => {
        ...
        expect(stripNonProseTags('<font color="#c8a2c8"><thinking>Juniper must not hesitate.</thinking></font>', options)).toBe('<font color="#c8a2c8"><thinking>Juniper must not hesitate.</thinking></font>');
        ...
        expect(stripNonProseTags('<details><font color="#c8a2c8"><thinking>private reasoning</thinking></font></details>', options)).toBe("");
    });
    test("preserves color span tags when keepFontTags is set", () => {
        expect(stripNonProseTags('Before <span style="color: #abc">tinted</span> after', { keepFontTags: true })).toBe('Before <span style="color: #abc">tinted</span> after');
    });
    test("kills font tags that live inside a non-prose block even when keepFontTags is set", () => {
        expect(stripNonProseTags('Prose. <details><font color="#fff">hidden colored note</font></details> More.', { keepFontTags: true })).toBe("Prose. More.");
        expect(stripNonProseTags('Prose. <lumia_ooc><font color="#fff">director colored note</font></lumia_ooc> More.', { keepFontTags: true })).toBe("Prose. More.");
    });
    test("strips non-font tags AND their inner content (strict prose mode)", () => {
        ...
        expect(stripNonProseTags("A <b>bold</b> and <span>plain</span> and <em>italic</em>.", {
            keepFontTags: true,
        })).toBe("A and and .");
    });
    test("preserves inline formatting INSIDE a font block as plain text", () => {
        ...
        expect(stripNonProseTags('<font color="#ff00ff">Hello, said <b>Juniper</b> calmly.</font>', { keepFontTags: true })).toBe('<font color="#ff00ff">Hello, said Juniper calmly.</font>');
    });
    ...
    test("preserves a top-level font block while killing a sibling UI block", () => {
        ...
        expect(stripNonProseTags(input, { keepFontTags: true })).toBe('<font color="#abc">colored prose</font> trailing text.');
    });
    test("strips Spindle extension XML tags and their inner content", () => {
        ...
        expect(stripNonProseTags(input, { keepFontTags: true })).toBe("Prose before. Prose after.");
    });
    test("strips Spindle tags nested inside a loom narrative block", () => {
        ...
        expect(stripNonProseTags(input, { keepFontTags: true })).toBe("Intro. Context. More context. End.");
    });
