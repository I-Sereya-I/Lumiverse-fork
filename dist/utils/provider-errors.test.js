...
import { describeTransportError, parseProviderErrorBody, readBoundedText, throwProviderResponseError, ProviderRequestError, } from "./provider-errors";
describe("describeTransportError", () => {
    test("explains Bun socket disconnects without exposing verbose fetch guidance", () => {
        const message = describeTransportError(new Error("The socket connection was closed unexpectedly. For more information, pass `verbose: true` in the second argument to fetch()"));
