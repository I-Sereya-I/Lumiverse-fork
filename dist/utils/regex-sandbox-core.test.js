...
import { runRegexRequest, substituteRegexCaptures, } from "./regex-sandbox-core";
function collect(pattern, flags, input) {
    return runRegexRequest({ id: "collect", op: "collect", pattern, flags, input });
}
function captureReplacements(pattern, flags, input, replacement) {
...
function legacyCaptureReplacements(pattern, flags, input, replacement) {
    return collect(pattern, flags, input).map((match) => ({
        index: match.index,
        matchLength: match.fullMatch.length,
        replacement: substituteRegexCaptures(replacement, match.fullMatch, match.groups, match.index, input, match.namedGroups),
    }));
}
describe("capture-replacements regex operation", () => {
    test("preserves raw-mode capture substitution semantics", () => {
        ...
        expect(captureReplacements(pattern, "g", input, replacement)).toEqual(legacyCaptureReplacements(pattern, "g", input, replacement));
    });
    ...
    test("handles global zero-length and sticky matches identically", () => {
        for (const testCase of [
            ...
            expect(captureReplacements(testCase.pattern, testCase.flags, testCase.input, testCase.replacement)).toEqual(legacyCaptureReplacements(testCase.pattern, testCase.flags, testCase.input, testCase.replacement));
        }
    });
